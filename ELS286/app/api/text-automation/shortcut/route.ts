...
async function loadAutomation(showId: string, token: string) {
  ...
  if (automation.shortcut_token !== token && !(await tokenExists(admin, token))) throw new Error("Invalid Shortcut token.");
...
async function loadUniversalAutomations(token: string) {
  const admin = createSupabaseAdminClient();
  if (!admin) throw new Error("SUPABASE_SERVICE_ROLE_KEY is missing.");
  if (!(await tokenExists(admin, token))) throw new Error("Invalid universal Shortcut token.");
  const { data, error } = await admin
    .from("show_text_automations")
    .select("show_id, enabled, sending_method, shortcut_token")
    .eq("enabled", true)
...
async function loadDueEventMessagesGlobal(request: Request, admin: ReturnType<typeof createSupabaseAdminClient>, token: string, limit: number, senderUserId = "") {
...
export async function GET(request: Request) {
  ...
  const senderUserId = senderFilterValue(params.get("sender_user_id") || params.get("queued_by_user_id") || "");
  ...
  try {
    if (allActive) {
      ...
      if (testMode) {
        const eventMessages = await loadDueEventMessagesGlobal(request, admin, token, limit, senderUserId);
