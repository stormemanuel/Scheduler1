...
function messageRateForCall(call: SubCallRow, _crew: CrewRow, show: ShowRow, masterRates: MasterRateRow[], _meta: Record<string, string>) {
  ...
  const cityMasterRate = masterRates.find((rate) => normalizeRole(rate.city_name || "Default") === targetCity && roleMatches(rate.role_name, effectiveRoleName));
  const defaultMasterRate = masterRates.find((rate) => normalizeRole(rate.city_name || "Default") === "default" && roleMatches(rate.role_name, effectiveRoleName));
