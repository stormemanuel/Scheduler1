...
function normalizeScheduleTemplateForDisplay(value: string | null | undefined) {
  ...
  if (!cleaned || cleaned === oldDefault) return defaultScheduleTemplate;
  return cleaned;
}
...
function findPositionOption(options: PositionOption[], roleName: string, masterRateId?: string | null, preferredCity = "Default") {
  ...
  // The event's rate city controls the selected rate. An older sub-call may
  // still carry the Default rate row ID, so prefer the matching city override
  // for the same role before honoring that stored ID.
  const preferredCityKey = normalize(preferredCity || "Default") || "default";
  const preferred = roleMatches.find((option) => normalize(option.city_name || "Default") === preferredCityKey);
  if (preferred) return preferred;

  const defaultOption = roleMatches.find((option) => normalize(option.city_name || "Default") === "default");
...
function matchingMasterRateForEvent(
  ...
  const keys = roleKeys(effectiveRoleName);
  const matchesPosition = (rate: MasterRateRecord, city: string) => {
    if (normalize(rate.city_name || "Default") !== normalize(city || "Default")) return false;
    const rateRole = normalize(rate.role_name);
    if (keys.has(rateRole)) return true;
    const rateKeys = roleKeys(rate.role_name);
    return [...keys].some((key) => rateKeys.has(key));
  };

  // City overrides must win even when this sub-call was originally saved with
  // the Default row ID. This lets existing calls immediately follow a show's
  // selected rate city without requiring the user to rebuild them.
  const cityRate = masterRates.find((rate) => matchesPosition(rate, rateCity));
  if (cityRate) return cityRate;

  const defaultRate = masterRates.find((rate) => matchesPosition(rate, "Default"));
