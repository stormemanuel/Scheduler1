  ...
  return (
    <div className="card compact" style={{ background: "#fffdf2", borderColor: "#f4c542", marginTop: compact ? 0 : 16 }}>
      ...
      <p className="muted small" style={{ marginTop: 0 }}>
        Use the current official IRS Form W-9 PDF, fill it out, sign/date it, then upload the completed copy here.
      </p>
      <div className="toolbar">
        <a className="ghost" href={IRS_W9_FORM_URL} target="_blank" rel="noopener noreferrer">Open official IRS W-9 PDF</a>
      </div>
      <p className="muted small" style={{ marginBottom: 0 }}>
        The IRS form is fillable in many browsers and PDF apps. Save the completed signed PDF to your device before uploading it to ELS.
      </p>
    </div>
  );
}
...
export default function OnboardingPage({ params }: PageProps) {
  ...
  useEffect(() => {
    ...
    if (mode) setRequestType(normalizeRequestType(mode));
    ...
    void fetch("/api/onboarding", {
      ...
      .then(async (response) => {
        ...
        if (active && result.request_type) setRequestType(normalizeRequestType(result.request_type));
      })
      .catch((error) => {
  ...
  async function submit() {
    ...
    if (requestType === "w9_only") {
      if (!form.w9_document_url && !form.w9_status_note.trim()) {
        setMessage({ kind: "error", text: "Please upload your signed W-9 or add a note before submitting." });
        return;
      }
    } else {
    ...
    try {
      const response = await fetch("/api/onboarding", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "submit", token: effectiveToken, request_type: requestType, ...form }),
      });
      const result = await response.json().catch(() => ({}));
      ...
      <main style={{ maxWidth: 760, margin: "32px auto", padding: "0 16px" }}>
        <section className="card">
          ...
          <p className="muted" style={{ marginTop: 0 }}>
            Please upload your signed W-9 through this secure link. Do not send SSN, EIN, or tax information by regular text or email.
          </p>
        </section>
        ...
        <section className="card" style={{ marginTop: 16 }}>
          ...
          <div className="card compact" style={{ background: "#fbfcfd" }}>
            <h3 style={{ marginTop: 0 }}>W-9 tax document only</h3>
            <p className="muted small">
              This request is only for the W-9 needed for ELS tax/1099 records. You do not need to redo the full onboarding packet.
            </p>
