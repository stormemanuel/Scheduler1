...
import type { PayrollEventSummary, PayrollPageData, PayrollStatusRecord, PayrollCrewShowRow, PayrollPaymentStatus } from "@/lib/payroll-types";
  ...
  for (const assignment of assignments) {
    try {
      ...
      row.calls.push({
        ...
        coordinationOwnerUserId: safeText(assignment.coordination_owner_user_id) || safeText(show.assigned_coordinator_user_id) || null,
        coordinationOwnerName: safeText(assignment.coordination_owner_name) || (show.assigned_coordinator_user_id ? (coordinatorNameById.get(show.assigned_coordinator_user_id) ?? "Assigned coordinator") : null),
        coordinationFeeWaived: Boolean(assignment.coordination_fee_waived),
        coordinationPaymentStatus: (safeText(assignment.coordination_owner_user_id) || show.assigned_coordinator_user_id) ? (coordinatorPayrollByShowUser.get(`${show.id}:${safeText(assignment.coordination_owner_user_id) || show.assigned_coordinator_user_id}`)?.payment_status ?? "unpaid") : "unpaid",
        coordinationPaid: (safeText(assignment.coordination_owner_user_id) || show.assigned_coordinator_user_id) ? Boolean(coordinatorPayrollByShowUser.get(`${show.id}:${safeText(assignment.coordination_owner_user_id) || show.assigned_coordinator_user_id}`)?.paid) : false,
        coordinationScheduledFor: (safeText(assignment.coordination_owner_user_id) || show.assigned_coordinator_user_id) ? (coordinatorPayrollByShowUser.get(`${show.id}:${safeText(assignment.coordination_owner_user_id) || show.assigned_coordinator_user_id}`)?.scheduled_for ?? null) : null,
        coordinationOverrideAmount: (safeText(assignment.coordination_owner_user_id) || show.assigned_coordinator_user_id) ? (coordinatorPayrollByShowUser.get(`${show.id}:${safeText(assignment.coordination_owner_user_id) || show.assigned_coordinator_user_id}`)?.payout_override ?? null) : null,
        coordinationNotes: (safeText(assignment.coordination_owner_user_id) || show.assigned_coordinator_user_id) ? (coordinatorPayrollByShowUser.get(`${show.id}:${safeText(assignment.coordination_owner_user_id) || show.assigned_coordinator_user_id}`)?.notes ?? "") : "",
        coordinationPaymentStatusId: (safeText(assignment.coordination_owner_user_id) || show.assigned_coordinator_user_id) ? (coordinatorPayrollByShowUser.get(`${show.id}:${safeText(assignment.coordination_owner_user_id) || show.assigned_coordinator_user_id}`)?.id ?? null) : null,
      });
...
export async function getPayrollPageData(): Promise<PayrollPageData> {
  try {
    ...
    const [showsRes, laborDaysRes, subCallsRes, assignmentsRes, crewRes, positionsRes, cityPoolsRes, ratesRes, clientRatesRes, payrollRes, financialsRes, expenseItemsRes, profilesRes, coordinatorPayrollRes] = await Promise.all([
      ...
      supabase.from("sub_calls").select("id, labor_day_id, area, po_number, role_name, master_rate_id, start_time, end_time, crew_needed, notes, day_type, one_hour_walkaway").order("start_time", { ascending: true }).limit(20000),
      ...
      supabase.from("show_financials").select("*").limit(10000),
      supabase.from("show_expense_items").select("id, show_id, category, description, amount, tax_treatment, receipt_status, expense_date, notes, created_by, created_at, updated_at").order("created_at", { ascending: false }).limit(20000),
      supabase.from("profiles").select("id, email, full_name, role, is_active").limit(1000),
      supabase.from("coordinator_payroll").select("id, show_id, coordinator_user_id, paid, payment_status, payout_override, notes, scheduled_for").limit(10000),
    ]);
    ...
    let assignmentRows = assignmentsRes.data ?? [];
    ...
    if (!assignmentsMissing && assignmentsCoordinationColumnsMissing) {
      const fallbackAssignmentsRes = await supabase
        ...
        .limit(20000);
      assignmentRows = fallbackAssignmentsRes.data ?? [];
    ...
    const coordinatorPayrollMissing = Boolean(coordinatorPayrollRes.error && (coordinatorPayrollRes.error.message.includes('relation "coordinator_payroll" does not exist') || coordinatorPayrollRes.error.message.includes('schema cache')));
    const error =
      showsRes.error ||
      laborDaysRes.error ||
      subCallsRes.error ||
      ...
      profilesRes.error ||
      (coordinatorPayrollMissing ? null : coordinatorPayrollRes.error);
    ...
    const subCalls = (subCallsRes.data ?? []).map((row) => {
      const typed = row as { id: string; labor_day_id: string; area: string | null; po_number?: string | null; role_name: string | null; master_rate_id?: string | null; start_time: string | null; end_time: string | null; crew_needed: number | null; notes: string | null; day_type?: string | null; one_hour_walkaway?: boolean | null };
      return { id: safeText(typed.id), labor_day_id: safeText(typed.labor_day_id), area: safeText(typed.area, "Imported Call"), po_number: safeText(typed.po_number) || null, role_name: safeText(typed.role_name, "General AV"), master_rate_id: safeText(typed.master_rate_id) || null, start_time: safeText(typed.start_time), end_time: safeText(typed.end_time), crew_needed: Number.isFinite(Number(typed.crew_needed)) ? Number(typed.crew_needed) : 1, notes: safeText(typed.notes), day_type: safeText(typed.day_type) || null, one_hour_walkaway: Boolean(typed.one_hour_walkaway) } satisfies SubCallRecord;
    ...
    const crewRecords = (crewRes.data ?? []).map((row) => {
      const typed = row as { id: string; name: string | null; description: string | null; city_pool_id: string | null; group_name: string | null; tier: string | null; email: string | null; phone: string | null; address?: string | null; lead_from?: string | null; other_city: string | null; ob: boolean | null; onboarding_texted_called?: boolean | null; onboarding_response?: boolean | null; onboarding_paperwork_sent?: boolean | null; onboarding_successfully_onboarded?: boolean | null; onboarding_called_placed_tier?: boolean | null; notes: string | null; conflict_companies: string[] | null };
    ...
    const coordinatorPayrollStatuses = coordinatorPayrollMissing ? [] : normalizeCoordinatorPayrollRows(coordinatorPayrollRes.data ?? []);
