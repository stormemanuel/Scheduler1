...
function DateArrowInput({
  ...
  return (
    <div ref={wrapperRef} style={{ position: "relative" }}>
      <div style={{ display: "grid", gridTemplateColumns: "minmax(0, 1fr) 48px", gap: 8, alignItems: "center" }}>
        <input
          type="text"
      ...
      <div className="card compact" style={{ borderColor: "#111827", marginTop: 12 }}>
        ...
        {editorMode === "call" ? (
          <div className="grid" style={{ gap: 14, marginTop: 12 }}>
            ...
            <div className="small muted">Default block auto-detects from Start/End: 1–5 hours = Half day, over 5 hours = Full day. Choose Hourly when the call should pay/bill by actual time. Individual workers can still be adjusted after they are assigned.</div>
      ...
      <div className="grid events-shell">
        <section className="card">
          ...
          {eventBuckets.map((bucket) => {
            ...
            const isCollapsedPastBucket = isPastBucket && !showPastShows;
            return (
              <div key={bucket} className="list" style={{ marginTop: 14 }}>
                <div className="row" style={{ alignItems: "center", justifyContent: "space-between", gap: 12, marginBottom: 6 }}>
                  <h4 className={`event-bucket-heading ${showBucketBadgeClass(bucket)}`} style={{ margin: 0 }}>{bucket}</h4>
                  {isPastBucket ? (
                    <button type="button" className="ghost" onClick={() => setShowPastShows((current) => !current)}>
                      {showPastShows ? "Hide past shows" : `Show past shows${bucketShows.length ? ` (${bucketShows.length})` : ""}`}
                    </button>
                  ) : null}
                </div>
                {isCollapsedPastBucket ? (
                  <p className="small muted">Past shows hidden. Click “Show past shows” to reveal them.</p>
                ) : (
                  <>
