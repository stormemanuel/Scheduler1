...
function scheduleBoothRepairNumber(call: { area?: string | null; sub_call_reference_number?: string | null }) {
  const reference = scheduleBoothRepairText(call.sub_call_reference_number);
...
export async function repairActiveScheduleQueueBoothLines(
  ...
  const { data: callData, error: callError } = await admin
    .from("sub_calls")
    .select("id, labor_day_id, area, sub_call_reference_number")
