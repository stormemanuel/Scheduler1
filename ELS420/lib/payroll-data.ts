...
function payrollSummaryForShow(show: ShowRecord, financial?: ShowFinancialRecord | null): PayrollEventSummary {
  ...
  return {
    ...
    financialNotes: safeText(financial?.notes),
    expenseItems: [],
...
export async function getPayrollSummaryData(requestedYear?: number): Promise<PayrollPageData> {
  ...
  try {
    ...
    const [
      ...
      readClient.from("client_rate_overrides").select("id, client_id, city_name, role_name, full_day, half_day, overtime_multiplier, doubletime_multiplier").limit(10000),
      showIds.length
        ...
        : Promise.resolve(emptyResult),
      showIds.length
        ? readClient.from("show_financials").select("show_id, estimated_revenue_override, expenses, notes, tax_reserve_done, tax_reserve_done_at, consecrated_hands_done, consecrated_hands_done_at").in("show_id", showIds).limit(5000)
        : Promise.resolve(emptyResult),
      showIds.length
        ? readClient.from("coordinator_payroll").select("id, show_id, coordinator_user_id, paid, payment_status, payout_override, notes, scheduled_for").in("show_id", showIds).limit(10000)
        : Promise.resolve(emptyResult),
    ...
    timer.mark("summary graph/status/rates");
    ...
    const coordinatorCompensationMissing = Boolean(coordinatorCompensationRes.error && (/coordinator_compensation_settings|schema cache|relation/i.test(coordinatorCompensationRes.error.message || "")));
    const graphErrorText = safeText((graphInitialRes.error as { message?: string } | null)?.message).toLowerCase();
    const graphOptionalColumnsMissing = Boolean(graphInitialRes.error && (
    ...
    let graphRes: any = graphInitialRes;
    if (graphOptionalColumnsMissing && showIds.length) {
      graphRes = await readClient
        .from("labor_days")
        .select(`
          id, show_id, labor_date,
          sub_calls (
            id, labor_day_id, area, po_number, role_name, master_rate_id,
            start_time, end_time, crew_needed, notes, day_type, one_hour_walkaway,
            assignments (
              id, sub_call_id, crew_id, status, start_time, end_time, day_type
            )
    ...
    const errorChecks: Array<{ label: string; error: any; rows: number | null }> = [
      { label: "GET /api/payroll?mode=summary -> labor days/sub-calls/assignments lookup", error: graphRes.error, rows: graphRes.data?.length ?? null },
      ...
      { label: "GET /api/payroll?mode=summary -> coordinator payroll lookup", error: coordinatorPayrollMissing ? null : coordinatorPayrollRes.error, rows: coordinatorPayrollRes.data?.length ?? null },
      { label: "GET /api/payroll?mode=summary -> coordinator compensation lookup", error: coordinatorCompensationMissing ? null : coordinatorCompensationRes.error, rows: coordinatorCompensationRes.data?.length ?? null },
    ];
    ...
    const assignmentRows: SummaryAssignmentRow[] = [];
    for (const dayRow of graphRes.data ?? []) {
      ...
      for (const callRow of nestedCalls ?? []) {
        ...
        for (const assignment of nestedAssignments ?? []) assignmentRows.push(assignment as SummaryAssignmentRow);
      }
    }

    const laborDays = laborRows.map((row: unknown) => {
    ...
    const subCalls = callRows.map((row: unknown) => {
    ...
    if (!coordinatorCompensationMissing) {
    ...
    const financialByShow = new Map((financialsMissing ? [] : ((financialsRes.data ?? []) as ShowFinancialRecord[])).map((row) => [safeText(row.show_id), row] as const));
