...
export async function buildImportComparison(admin: { from: (table: string) => any }, payload: ImportedPayload, existingEvent: ExistingImportShowMatch): Promise<ImportComparison> {
  ...
  const findSafeExistingMatch = (call: ImportedSubCall) => {
    ...
    const candidates = sameDateRoleRows
      .map((row) => {
        ...
        const areaScore = areaSimilarityScore(row.area, call.area);
        const poExact = Boolean(importedPo && existingPo && importedPo === existingPo);
        return { row, existingPo, existingReference, boothStrength, areaScore, poExact };
      })
      .filter((candidate) => candidate.areaScore >= 0.78 || candidate.poExact);

    const poMatches = candidates.filter((candidate) => candidate.poExact);
    const poAreaMatches = poMatches.filter((candidate) => candidate.areaScore >= 0.78);
    const sameTimePoAreaMatches = poAreaMatches.filter((candidate) => {
      const existing = existingChangeValue(candidate.row, call.labor_date);
      return existing.start_time === normalizeTimeKey(call.start_time) && existing.end_time === normalizeTimeKey(call.end_time);
    });
    if (sameTimePoAreaMatches.length === 1) {
      return { row: sameTimePoAreaMatches[0].row, reason: "Matched existing sub-call by same area, PO number, date, position, and time." };
    }
    if (poAreaMatches.length === 1) {
      return { row: poAreaMatches[0].row, reason: "Matched existing sub-call by same area, PO number, date, and position." };
    }
    if (poMatches.length === 1) {
      return { row: poMatches[0].row, reason: "Matched existing sub-call by PO number, labor date, and position." };
    }
    const exactPoBoothArea = candidates.filter(
      (candidate) => candidate.poExact && candidate.boothStrength === "exact" && candidate.areaScore >= 0.78
    );
    if (exactPoBoothArea.length === 1) {
      return { row: exactPoBoothArea[0].row, reason: "Matched existing sub-call by similar area, exact booth, and exact PO." };
    }
    const exactBoothOneMissingPo = candidates.filter(
      (candidate) =>
        !candidate.poExact &&
        candidate.areaScore >= 0.78 &&
        candidate.boothStrength === "exact" &&
        (!importedPo || !candidate.existingPo)
    );
    if (exactBoothOneMissingPo.length === 1) {
      return { row: exactBoothOneMissingPo[0].row, reason: "Matched existing sub-call by similar area and exact booth; one side has no PO." };
    }
    const exactPoPartialBooth = candidates.filter(
      (candidate) => candidate.poExact && candidate.areaScore >= 0.78 && (candidate.boothStrength === "partial" || candidate.boothStrength === "missing")
    );
    if (exactPoPartialBooth.length === 1) {
      return { row: exactPoPartialBooth[0].row, reason: "Matched existing sub-call by similar area and exact PO; booth is missing or TBD on one side." };
    }
    const similarOnly = candidates.filter((candidate) => candidate.areaScore >= 0.78 && !candidate.poExact);
    if (similarOnly.length === 1) {
      return {
        row: similarOnly[0].row,
        reason: "Similar area name found, but booth/PO did not safely confirm the match.",
        reviewOnly: true,
      };
    }
    if (candidates.length > 1) {
      return {
        row: null,
        reason: "Multiple possible existing sub-call matches found. Select the correct match before applying.",
        reviewOnly: true,
      };
    }
    return null;
  ...
  payload.subCallGroups.forEach((call, index) => {
    ...
    const poDayExact = importedPoDay ? existingByPoDay.get(importedPoDay) : undefined;
    const comparableExact = existingByComparable.get(comparable);
    const fuzzyMatch = !poDayExact && !comparableExact ? findSafeExistingMatch(call) : null;
    const exactish = poDayExact || comparableExact || (fuzzyMatch && !fuzzyMatch.reviewOnly ? fuzzyMatch.row || undefined : undefined);
    const matchReason = poDayExact
      ? "Matched existing sub-call by PO number, labor date, and position."
      : comparableExact
        ? "Matched existing sub-call by area, booth/reference, date, and position."
        : fuzzyMatch?.reason || "";
    ...
    if (!exactish) {
      ...
      const positionKey = `${areaKey}|${normalizeRoleNameForMatch(call.role_name)}`;
  ...
  const summary = {
    ...
    needsReview: changes.filter((change) => change.category === "needs_review").length,
