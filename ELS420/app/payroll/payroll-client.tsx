...
const PAYROLL_SUMMARY_TIMEOUT_MS = 10000;
...
export default function PayrollClient({ initialRows, initialEventSummaries, availableYears, initialYear, initialSummaryLoaded, initialError, initialDiagnostics = null }: Props) {
  ...
  const summaryFailed = summaryRequestCompleted && Boolean(currentYearError) && summaryEvents.length === 0 && yearRows.length === 0;
  ...
  async function selectPayrollYear(nextYear: number, force = false) {
    ...
    const timeoutId = window.setTimeout(() => controller.abort(), PAYROLL_SUMMARY_TIMEOUT_MS);
    try {
      const response = await fetch(`/api/payroll?year=${encodeURIComponent(String(nextYear))}`, { cache: "no-store", signal: controller.signal });
      ...
      const nextMessage = error instanceof DOMException && error.name === "AbortError"
        ? "Payroll totals are taking too long to load."
        : error instanceof Error ? error.message : `Unable to load ${nextYear} payroll.`;
      setYearErrors((current) => ({ ...current, [nextYear]: nextMessage }));
      setLoadedYears((current) => new Set([...current, nextYear]));
