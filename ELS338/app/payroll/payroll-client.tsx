...
export default function PayrollClient({ initialRows, availableYears, initialYear, initialError }: Props) {
  ...
  async function loadPayrollEventDetails(showId: string) {
    ...
    try {
      ...
      if (!nextRows.length && !nextEvents.some((event) => event.showId === showId && event.rows.length)) {
        throw new Error("No payroll rows loaded for this show yet. Check that crew are assigned to the event sub-calls, then try opening payroll details again.");
      }
