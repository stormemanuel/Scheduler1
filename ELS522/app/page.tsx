...
import { normalizeRole, requirePage } from "@/lib/auth";
import { createSupabaseServerClient } from "@/lib/supabase-server";
import type { ShowRecord } from "@/lib/events-types";
import { OverviewCalendar, type OverviewCalendarShow } from "@/app/components/global-search";
...
async function getOverviewData(session: Awaited<ReturnType<typeof requirePage>>): Promise<OverviewResult> {
  ...
  const lookbackKey = `${new Date().getUTCFullYear()}-01-01`;
    ...
    <section className="card">
      ...
      <div className="list" style={{ marginTop: 12 }}>
        {shows.slice(0, 8).map((show) => (
          <Link key={show.id} href={`/events?show_id=${encodeURIComponent(show.id)}`} className={`overview-event-card event-card-${status}`}>
...
export default async function HomePage() {
  ...
  const canCreateElsEvent = isOwnerAdmin || Boolean(session.access?.can_edit_event_details);
  const pendingFeedbackCount = overviewData.pendingFeedbackCount;
  const calendarShows = shows.map((show) => ({
    id: show.id,
    name: show.name || "",
    client: show.client || "",
    venue: show.venue || "",
    show_start: show.show_start || "",
    show_end: show.show_end || show.show_start || "",
    calendar_dates: show.calendar_dates || [],
  })) satisfies OverviewCalendarShow[];
  ...
  return (
    <div className="grid" style={{ gap: 16 }}>
      ...
      <OverviewCalendar shows={calendarShows} canCreateElsEvent={canCreateElsEvent} />
    </div>
  );
}
