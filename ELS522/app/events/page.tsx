...
export default async function EventsPage({ searchParams }: { searchParams?: Promise<{ q?: string; feedback?: string; review?: string; show_id?: string; sub_call_id?: string; create?: string; date?: string }> }) {
  ...
  const initialCreateDate = params?.create === "1" && typeof params?.date === "string" && /^\d{4}-\d{2}-\d{2}$/.test(params.date) ? params.date : "";
