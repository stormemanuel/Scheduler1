export type ShowRecord = {
  ...
  show_end: string;
  notes: string;
  pending_action_text?: string | null;
  pending_action_visible_to_coordinator?: boolean | null;
...
export const ASSIGNMENT_RATE_OVERRIDE_NOTE_CODE = "assignment_rate_override";

export function assignmentRateOverrideFromNotes(notes: AssignmentNoteRecord[], assignmentId: string) {
  const matching = notes
    .filter((note) => note.assignment_id === assignmentId && note.note_code === ASSIGNMENT_RATE_OVERRIDE_NOTE_CODE)
    .sort((a, b) => String(b.created_at || "").localeCompare(String(a.created_at || "")))[0];
  const amount = Number(String(matching?.custom_note || "").replace(/[^0-9.]/g, ""));
...
export type TextSendingMethod = "manual" | "shortcut" | "android_messages" | "provider" | "resend_email";
export type TextDeliveryMode = "apple_shortcut" | "android_messages" | "resend_email";
...
export type TextMessageQueueRecord = {
  ...
  delivery_mode?: TextDeliveryMode | null;
...
export function communicationChecklistStage(row: Pick<TextMessageQueueRecord, "message_type" | "reminder_key">): CommunicationChecklistStage | null {
  ...
  if (messageType === "schedule" || reminderKey === "7_day" || reminderKey.startsWith("7_day_") || reminderKey.startsWith("manual_7_day")) return "schedule";
  if (reminderKey === "3_day" || reminderKey.startsWith("3_day_") || reminderKey.startsWith("manual_3_day")) return "3_day";
  if (reminderKey === "day_before" || reminderKey.startsWith("day_before_") || reminderKey.startsWith("manual_day_before")) return "day_before";
  if (reminderKey === "day_of" || reminderKey.startsWith("day_of_") || reminderKey.startsWith("manual_day_of") || reminderKey.startsWith("daily_after_first_day") || reminderKey.startsWith("manual_daily_after_first_day")) return "day_of";
  return null;
}
