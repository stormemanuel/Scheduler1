...
function coordinatorPaymentForEvent(event: PayrollEventSummary): PayrollCoordinatorPaymentSummary | null {
  const first = event.rows.find((row) => row.showAssignedCoordinatorUserId);
  if (!first?.showAssignedCoordinatorUserId) return null;
  let fullDayTechDays = 0;
  let halfDayTechs = 0;
  for (const row of event.rows) {
    for (const call of row.calls) {
      const label = String(call.payLabel || "").toLowerCase();
      if (label.includes("half") || (call.durationHours !== null && call.durationHours <= 5)) halfDayTechs += 1;
      else fullDayTechDays += 1;
    }
  }
  const projectedAmount = coordinatorProjectedAmount(fullDayTechDays, halfDayTechs);
  const paymentStatus = first.coordinatorPaymentStatus === "paid" || first.coordinatorPaymentStatus === "scheduled" ? first.coordinatorPaymentStatus : "unpaid";
  const overrideAmount = first.coordinatorOverrideAmount ?? null;
  return {
    showId: event.showId,
    coordinatorUserId: first.showAssignedCoordinatorUserId,
    coordinatorName: first.showAssignedCoordinatorName || "Assigned coordinator",
    fullDayTechDays,
    halfDayTechs,
    projectedAmount,
    overrideAmount,
    ...
    scheduledFor: first.coordinatorScheduledFor ?? null,
    notes: first.coordinatorNotes ?? "",
    statusId: first.coordinatorPaymentStatusId ?? null,
  };
}
...
function buildEventSummaries(rows: PayrollCrewShowRow[]) {
  ...
  return [...events.values()]
    .map((event) => {
      ...
      const coordinatorPayment = coordinatorPaymentForEvent(sortedEvent);
      const coordinatorCost = coordinatorPayment?.payableAmount ?? 0;
...
function plForEvents(events: PayrollEventSummary[]) {
  const totals = events.reduce((acc, event) => {
    ...
    acc.contractLabor += event.payableTotal + (event.coordinatorPayment?.payableAmount ?? 0);
...
export default function PayrollClient({ initialRows, availableYears, initialError }: Props) {
  ...
  function companyPLRows(includeReserves: boolean): Array<Array<string | number | null | undefined>> {
    ...
    const rows = summaryEvents.map((event) => includeReserves ? [
      ...
      event.payableTotal + (event.coordinatorPayment?.payableAmount ?? 0),
      event.expenses,
  ...
  function eventPayrollDocument(event: PayrollEventSummary): ExportDocument {
    ...
    return {
      ...
      meta: [
        ...
        ["Contract Labor", money(event.payableTotal + (event.coordinatorPayment?.payableAmount ?? 0))],
  ...
  function exportOwnerPLCsv() {
    downloadCsv(`ELS_${year}_${reportView}_company_PL.csv`, [
      ...
      [],
      ["Show", "Client", "Venue", "Dates", "Estimated Revenue", "Contract Labor", "Expenses", "Net Profit", "Tax Reserve Set Aside", "Consecrated Hands Done", "Financial Notes"],
      ...summaryEvents.map((event) => [event.showName, event.showClient, event.showVenue, `${event.showStart} to ${event.showEnd}`, event.estimatedRevenue, event.payableTotal + (event.coordinatorPayment?.payableAmount ?? 0), event.expenses, event.estimatedProfit, event.taxReserveDone ? "YES" : "NO", event.consecratedHandsDone ? "YES" : "NO", event.financialNotes]),
    ]);
  }
  ...
  function exportEventCsv(event: PayrollEventSummary) {
    downloadCsv(`${event.showName.replace(/[^a-z0-9]+/gi, "_")}_payroll.csv`, [
      ...
      [event.showName, event.estimatedRevenue, event.payableTotal + (event.coordinatorPayment?.payableAmount ?? 0), event.expenses, event.estimatedProfit, event.taxReserve, event.taxReserveDone ? "YES" : "NO", event.consecratedHandsDonation, event.consecratedHandsDone ? "YES" : "NO"],
      [],
      ["Show", "Crew", "Roles", "Estimated", "Override", "Payable", "Scheduled For", "Status", "Phone", "Email", "Notes"],
      ...event.rows.map((row) => [row.showName, row.crewName, row.roles.join(" / "), row.estimatedTotal, row.overrideAmount ?? "", payable(row), row.scheduledFor || "", paymentStatusLabel(paymentStatusFor(row)), row.crewPhone, row.crewEmail, row.notes]),
      ...(event.coordinatorPayment ? [[event.showName, event.coordinatorPayment.coordinatorName, "Labor Coordinator", event.coordinatorPayment.projectedAmount, event.coordinatorPayment.overrideAmount ?? "", event.coordinatorPayment.payableAmount, event.coordinatorPayment.scheduledFor || "", paymentStatusLabel(event.coordinatorPayment.paymentStatus), "", "", event.coordinatorPayment.notes]] : []),
    ]);
  }
            ...
            <div className="row" style={{ alignItems: "flex-start" }}>
              <div>
                ...
                <div className="pl-grid" style={{ marginTop: 12 }}>
                  ...
                  <span className="badge">Labor {money(event.payableTotal + (event.coordinatorPayment?.payableAmount ?? 0))}</span>
                ...
                <div className="row" style={{ alignItems: "flex-end" }}>
                  <div>
                    ...
                    <div className="toolbar" style={{ marginTop: 8 }}>
                      ...
                      <span className="badge">{paymentStatusLabel(event.coordinatorPayment.paymentStatus)}</span>
                    </div>
                  </div>
                  ...
                  <label className="field" style={{ minWidth: 160 }}>
                    ...
                    <input
                      ...
                      onChange={(inputEvent) => {
                        ...
                        const nextStatus = event.coordinatorPayment?.paymentStatus === "paid" ? "paid" : nextDate ? "scheduled" : "unpaid";
                      ...
                      onBlur={(inputEvent) => {
                        const nextDate = inputEvent.currentTarget.value || null;
                        const nextStatus = event.coordinatorPayment?.paymentStatus === "paid" ? "paid" : nextDate ? "scheduled" : "unpaid";
                      ...
                      <tr key={row.key} style={{ borderBottom: "1px solid var(--line)", verticalAlign: "top" }}>
                        ...
                        <td style={{ padding: "12px 8px", minWidth: 150 }}>
                          <input
                            ...
                            onChange={(event) => {
                              ...
                              const nextStatus = paymentStatusFor(row) === "paid" ? "paid" : nextDate ? "scheduled" : "unpaid";
                              updateLocal(row.key, { scheduledFor: nextDate, paymentStatus: nextStatus, paid: nextStatus === "paid" });
                            }}
                            onBlur={(event) => {
                              const nextDate = event.currentTarget.value || null;
                              const nextStatus = paymentStatusFor(row) === "paid" ? "paid" : nextDate ? "scheduled" : "unpaid";
                        ...
                        <td style={{ padding: "12px 8px", minWidth: 140 }}>
                          <select
                            ...
                            style={{ minWidth: 130, padding: "8px 10px", border: "1px solid var(--line)", borderRadius: 10 }}
                          >
                            <option value="unpaid">Unpaid</option>
                            <option value="scheduled">Scheduled</option>
                            <option value="paid">Paid</option>
