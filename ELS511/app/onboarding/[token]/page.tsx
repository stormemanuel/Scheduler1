...
type RequestType = "full_onboarding" | "w9_only" | "contract_only" | "profile_photo_only" | "purchase_order";
...
type PublicPurchaseOrder = {
  ...
  baseAmount: number;
  days: Array<{
    key: string; laborDate: string; description: string; defaultAmount: number;
    note: string;
    overtimeHours: number; overtimeRate: number; doubleTimeHours: number; doubleTimeRate: number;
  }>;
  adjustments: {
    overtimeHours: number; overtimeRate: number; doubleTimeHours: number; doubleTimeRate: number;
    mealPenaltyCount: number; mealPenaltyRate: number; shortTurnHours: number; shortTurnRate: number;
    otherDescription: string; otherAmount: number;
    lateHours: number; lateHourlyRate: number; negativeDescription: string; negativeAmount: number;
  };
  adjustmentAmounts: { overtime: number; doubleTime: number; mealPenalty: number; shortTurn: number; other: number; lateDeduction: number; negativeAdjustment: number };
  total: number;
  sentAt: string | null;
...
const PROFILE_CROP_OUTPUT_SIZE = 320;
...
const CONTRACT_CERTIFICATIONS = [
  ...
  "Contractor understands that payment is owed only for services actually performed, documented, and approved by Emanuel Labor Services LLC.",
...
async function createCroppedProfileFile(crop: ProfileCropState) {
  ...
  return new File([blob], `${fileBaseName(crop.file.name)}-profile-thumb.webp`, {
    type: "image/webp",
    lastModified: Date.now(),
  });
}
...
async function prepareUploadFile(kind: UploadKind, file: File) {
  if (kind === "profile_photo") return compressImage(file, 320, 320, 0.76);
...
function hasCompleteHomeAddress(form: FormState) {
  return Boolean(
    form.home_address_line1.trim() &&
    form.home_city.trim() &&
    form.home_state.trim() &&
    form.home_zip.trim(),
  );
}
...
function hasDigitalW9MailingAddress(form: FormState) {
  const parsed = parseCityStateZip(form.digital_w9.tax_city_state_zip);
  return Boolean(
    form.digital_w9.tax_address_line_1.trim() &&
    (form.digital_w9.tax_city_state_zip.trim() ||
      (parsed.city && parsed.state && parsed.zip)),
  );
}

function missingHomeAddressFields(form: FormState) {
  if (hasDigitalW9MailingAddress(form)) return [];
  const missing: string[] = [];
  if (!form.home_address_line1.trim()) missing.push("street address");
  if (!form.home_city.trim()) missing.push("city");
  if (!form.home_state.trim()) missing.push("state");
  if (!form.home_zip.trim()) missing.push("ZIP");
  return missing;
}
  ...
  return (
    <div className="card compact" style={{ background: "#fbfcfd", marginTop: 14 }}>
      ...
      >
        ...
        <p><strong>Payment for Work Actually Performed.</strong></p>
        <p>Contractor is compensated only for services actually performed, documented, and approved by Emanuel Labor Services LLC. Being listed, scheduled, requested, confirmed, or placed on standby for an assignment does not by itself guarantee payment unless Emanuel Labor Services LLC separately approves such payment in writing.</p>
        <p>No payment is owed for any shift, portion of a shift, or assignment that Contractor does not actually work, including but not limited to a no-show, call-out, refusal of assignment, late arrival after replacement, early departure without approval, or failure to perform the requested services.</p>
        <p>All payable time must be supported by approved time records, sign-in/sign-out records, client confirmation, crew lead confirmation, or other documentation accepted by Emanuel Labor Services LLC.</p>
        <p>Emanuel Labor Services LLC may, at its sole discretion, approve cancellation pay, standby pay, travel pay, or other special compensation only when confirmed in writing by an authorized ELS representative.</p>
...
export default function OnboardingPage({ params }: PageProps) {
  ...
  const [requestLoading, setRequestLoading] = useState(Boolean(tokenFromParams));
  const [requestLoadFailed, setRequestLoadFailed] = useState(false);
  const [purchaseOrder, setPurchaseOrder] = useState<PublicPurchaseOrder | null>(null);
  const [purchaseOrderChangeRequest, setPurchaseOrderChangeRequest] = useState("");
      ...
      .then(async (response) => {
        ...
        if (active && result.request_type) {
          ...
          if (normalized === "purchase_order") {
            const canonicalToken = String(result.canonical_token || "").trim();
            if (canonicalToken && canonicalToken !== effectiveToken) {
              window.history.replaceState(null, "", `/onboarding/${encodeURIComponent(canonicalToken)}?mode=purchase_order`);
              setToken(canonicalToken);
            }
            setPurchaseOrder(result.purchase_order as PublicPurchaseOrder);
            setPurchaseOrderChangeRequest(String(result.purchase_order?.changeRequest || ""));
            return;
          }
  ...
  useEffect(() => {
    const effectiveToken = token || tokenFromParams;
    if (!effectiveToken || typeof window === "undefined" || requestType === "purchase_order") return;
  ...
  }, [form, token, tokenFromParams, requestType]);
  ...
  async function submit() {
    ...
    if (requestType === "contract_only") {
      ...
      const missingHomeFields = missingHomeAddressFields(form);
      if (!hasCompleteHomeAddress(form) && !hasDigitalW9MailingAddress(form)) {
        setMessage({ kind: "error", text: `Please complete the Home address section near the top of this form. Missing: ${missingHomeFields.join(", ")}.` });
        return;
      }
  ...
  async function submitPurchaseOrderAction(action: "approve_purchase_order" | "request_purchase_order_changes") {
    const effectiveToken = token || tokenFromParams;
    if (!effectiveToken || !purchaseOrder) return;
    if (action === "request_purchase_order_changes" && !purchaseOrderChangeRequest.trim()) {
      setMessage({ kind: "error", text: "Please describe the change you need." });
      return;
    }
    ...
    try {
      const response = await fetch("/api/onboarding", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action, token: effectiveToken, change_request: purchaseOrderChangeRequest }),
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok || !result.ok) throw new Error(result.message || "Unable to update this Purchase Order.");
      setPurchaseOrder(result.purchase_order as PublicPurchaseOrder);
      setMessage({ kind: "success", text: result.message || "Purchase Order updated." });
      window.scrollTo({ top: 0, behavior: "smooth" });
    } catch (error) {
      setMessage({ kind: "error", text: error instanceof Error ? error.message : "Unable to update this Purchase Order." });
    } finally {
  ...
  if (requestLoading) {
    return <main style={{ maxWidth: 760, margin: "48px auto", padding: "0 16px" }}><section className="card"><h1>Opening secure request</h1><p className="muted">Please wait while ELS verifies this link…</p></section></main>;
  }

  if (requestLoadFailed) {
    return <main style={{ maxWidth: 760, margin: "48px auto", padding: "0 16px" }}><section className="card" style={{ borderTop: "6px solid #b42318" }}><h1>{requestType === "purchase_order" ? "Purchase Order" : "Secure ELS link"}</h1><p className="error">{message?.text || "This link is invalid or no longer available."}</p><p className="muted">Please use the newest link sent by Emanuel Labor Services or contact ELS for assistance.</p></section></main>;
  }

  if (requestType === "purchase_order") {
    if (!purchaseOrder) {
      return <main style={{ maxWidth: 760, margin: "48px auto", padding: "0 16px" }}><section className="card"><h1>Purchase Order</h1><p className={message?.kind === "error" ? "error" : "muted"}>{message?.text || "Loading your Purchase Order…"}</p></section></main>;
    }
    const poMoney = (value: number) => new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(Number(value || 0));
    const dateTime = (value: string | null) => value ? new Date(value).toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short" }) : "—";
    const poDate = (value: string) => {
      const [year, month, day] = String(value || "").slice(0, 10).split("-").map(Number);
      return year && month && day ? new Date(Date.UTC(year, month - 1, day)).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "UTC" }) : "Scheduled day";
    };
    const dayLines = (purchaseOrder.days || []).flatMap((day) => {
      ...
      const date = poDate(day.laborDate);
      return [
        { label: `${date} — Default day pay`, detail: [day.description, day.note ? `Day note: ${day.note}` : ""].filter(Boolean).join(" · "), amount: day.defaultAmount },
        overtime > 0 ? { label: `${date} — Overtime`, detail: `${day.overtimeHours} hr × ${poMoney(day.overtimeRate)}`, amount: overtime } : null,
        doubleTime > 0 ? { label: `${date} — Double time`, detail: `${day.doubleTimeHours} hr × ${poMoney(day.doubleTimeRate)}`, amount: doubleTime } : null,
      ];
    });
    const lines = [
      ...((purchaseOrder.days || []).length ? dayLines : [{ label: "Base show pay", detail: "", amount: purchaseOrder.baseAmount }]),
      !(purchaseOrder.days || []).length && purchaseOrder.adjustmentAmounts.overtime > 0 ? { label: "Overtime", detail: `${purchaseOrder.adjustments.overtimeHours} hr × ${poMoney(purchaseOrder.adjustments.overtimeRate)}`, amount: purchaseOrder.adjustmentAmounts.overtime } : null,
      !(purchaseOrder.days || []).length && purchaseOrder.adjustmentAmounts.doubleTime > 0 ? { label: "Double time", detail: `${purchaseOrder.adjustments.doubleTimeHours} hr × ${poMoney(purchaseOrder.adjustments.doubleTimeRate)}`, amount: purchaseOrder.adjustmentAmounts.doubleTime } : null,
      purchaseOrder.adjustmentAmounts.mealPenalty > 0 ? { label: "Meal penalty", detail: `${purchaseOrder.adjustments.mealPenaltyCount} occurrence${purchaseOrder.adjustments.mealPenaltyCount === 1 ? "" : "s"} × ${poMoney(purchaseOrder.adjustments.mealPenaltyRate)}`, amount: purchaseOrder.adjustmentAmounts.mealPenalty } : null,
      purchaseOrder.adjustmentAmounts.shortTurn > 0 ? { label: "Under-8-hour turnaround", detail: `${purchaseOrder.adjustments.shortTurnHours} hr × ${poMoney(purchaseOrder.adjustments.shortTurnRate)}`, amount: purchaseOrder.adjustmentAmounts.shortTurn } : null,
      purchaseOrder.adjustmentAmounts.other > 0 ? { label: purchaseOrder.adjustments.otherDescription || "Other adjustment", detail: "", amount: purchaseOrder.adjustmentAmounts.other } : null,
      purchaseOrder.adjustmentAmounts.lateDeduction > 0 ? { label: "Late arrival deduction", detail: `${purchaseOrder.adjustments.lateHours} hr × ${poMoney(purchaseOrder.adjustments.lateHourlyRate)}`, amount: -purchaseOrder.adjustmentAmounts.lateDeduction } : null,
      purchaseOrder.adjustmentAmounts.negativeAdjustment > 0 ? { label: purchaseOrder.adjustments.negativeDescription || "Negative adjustment", detail: "Deduction", amount: -purchaseOrder.adjustmentAmounts.negativeAdjustment } : null,
    ].filter((line): line is { label: string; detail: string; amount: number } => Boolean(line));
    const awaitingResponse = purchaseOrder.status === "delivered" || purchaseOrder.status === "sent_awaiting_delivery";
    const statusLabel: Record<string, string> = { sent_awaiting_delivery: "Sent — awaiting your response", delivered: "Delivered — awaiting your response", approved: "Approved", auto_approved: "Automatically approved — no response", changes_requested: "Changes requested", delivery_failed: "Delivery failed", revised: "Revised" };
    return (
      <main style={{ maxWidth: 820, margin: "32px auto", padding: "0 16px" }}>
        <section className="card" style={{ borderTop: "6px solid #0f5f3d" }}>
          <div className="row" style={{ alignItems: "flex-start" }}><div><div className="muted small">Emanuel Labor Services</div><h1 style={{ margin: "4px 0" }}>Purchase Order</h1><div style={{ color: "#0f5f3d", fontWeight: 900 }}>{purchaseOrder.displayNumber || `Revision ${purchaseOrder.revision}`}</div></div><span className="badge" style={{ background: awaitingResponse ? "#fff3bf" : "#e8f5ed", color: "#18372a", fontWeight: 900 }}>{statusLabel[purchaseOrder.status] || purchaseOrder.status.replace(/_/g, " ")}</span></div>
          {message ? <p className={message.kind === "error" ? "error" : "success"}>{message.text}</p> : null}
          <div className="grid grid-2" style={{ marginTop: 18 }}><div><strong>Contractor</strong><div>{purchaseOrder.contractorName}</div></div><div><strong>Show</strong><div>{purchaseOrder.showName}</div><div className="muted small">{[purchaseOrder.showClient, purchaseOrder.showVenue].filter(Boolean).join(" · ")}</div></div><div><strong>Show dates</strong><div>{[purchaseOrder.showStart, purchaseOrder.showEnd].filter(Boolean).join(" to ") || "—"}</div></div><div><strong>Response deadline</strong><div>{purchaseOrder.approvalDueAt ? dateTime(purchaseOrder.approvalDueAt) : "Begins after confirmed delivery"}</div></div></div>
        </section>
        <section className="card" style={{ marginTop: 16 }}>
          <h2 style={{ marginTop: 0 }}>Pay summary</h2>
          <div style={{ border: "1px solid var(--line)", borderRadius: 12, overflow: "hidden" }}>{lines.map((line) => <div key={line.label} className="row" style={{ padding: "12px 14px", borderBottom: "1px solid var(--line)", background: line.amount < 0 ? "#fff4f2" : undefined }}><div><strong>{line.label}</strong>{line.detail ? <div className="muted small">{line.detail}</div> : null}</div><strong style={{ color: line.amount < 0 ? "#b42318" : undefined }}>{poMoney(line.amount)}</strong></div>)}<div className="row" style={{ padding: "15px 14px", background: "#f5faf7", fontSize: 20 }}><strong>Total</strong><strong>{poMoney(purchaseOrder.total)}</strong></div></div>
          <p style={{ background: "#fff7d6", borderLeft: "4px solid #d4a62a", padding: 12, marginTop: 16 }}>Please review this purchase order and either approve it or request changes within 48 hours. If no response is received within 48 hours of delivery, the purchase order will be recorded as approved and payment will be processed for the stated amount.</p>
          {awaitingResponse ? <><label className="field" style={{ marginTop: 16 }}><strong>What needs to be changed?</strong><textarea rows={4} value={purchaseOrderChangeRequest} onChange={(event) => setPurchaseOrderChangeRequest(event.target.value)} placeholder="Explain the date, hours, rate, penalty, or other item that needs review…" /><span className="small muted">A written response is required before you can submit Request changes.</span></label><div className="toolbar" style={{ marginTop: 12 }}><button className="primary" type="button" disabled={purchaseOrderSaving} onClick={() => void submitPurchaseOrderAction("approve_purchase_order")}>{purchaseOrderSaving ? "Saving…" : `Approve ${poMoney(purchaseOrder.total)}`}</button><button className="ghost" type="button" disabled={purchaseOrderSaving || !purchaseOrderChangeRequest.trim()} onClick={() => void submitPurchaseOrderAction("request_purchase_order_changes")}>Request changes</button></div></> : <>{purchaseOrder.status === "changes_requested" && purchaseOrder.changeRequest ? <div style={{ marginTop: 16, padding: 12, background: "#fff7d6", borderLeft: "4px solid #d4a62a" }}><strong>Your requested changes</strong><div style={{ marginTop: 4, whiteSpace: "pre-wrap" }}>{purchaseOrder.changeRequest}</div></div> : null}<p className="muted" style={{ marginBottom: 0 }}>{purchaseOrder.status === "changes_requested" ? "ELS has received your request and will send a revised Purchase Order if the amount changes." : "No action is currently required."}</p></>}
        </section>
      </main>
    );
  }
      ...
      <section className="card" style={{ marginTop: 16 }}>
        ...
        >
          ...
          <p className="muted small" style={{ marginTop: 0 }}>
            ...
            to a private WebP thumbnail around 320×320 for your Crew profile.
          </p>
          <label className="field">
