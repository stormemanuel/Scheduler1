...
import type { PayrollCrewShowRow, PayrollEventSummary, PayrollYearTechSummary, PayrollPaymentStatus, PayrollCoordinatorPaymentSummary, PayrollPerformanceDiagnostics } from "@/lib/payroll-types";
import type { PurchaseOrderAdjustments } from "@/lib/payroll-calculations";
import type { ShowExpenseItemRecord } from "@/lib/financial-types";
import { estimateCoordinatorCompensation, formatPayrollDate, formatPayrollTime, money, purchaseOrderDisplayNumber } from "@/lib/payroll-calculations";
...
type Props = {
  ...
  initialEventSummaries: PayrollEventSummary[];
  availableYears: number[];
  initialYear: number;
  initialSummaryLoaded: boolean;
  initialError: string | null;
  initialDiagnostics?: PayrollPerformanceDiagnostics | null;
};
...
type YearLoadProgress = { done: number; total: number; failed: number };
type ExpenseDraft = {
  category: string;
  description: string;
  amount: string;
...
type PurchaseOrderDraft = {
  baseAmount: string;
  days: PurchaseOrderDayDraft[];
  overtimeHours: string; overtimeRate: string;
  doubleTimeHours: string; doubleTimeRate: string;
  mealPenaltyCount: string; mealPenaltyRate: string;
  shortTurnHours: string; shortTurnRate: string;
  otherDescription: string; otherAmount: string;
  lateHours: string; lateHourlyRate: string;
  negativeDescription: string; negativeAmount: string;
};

type PurchaseOrderDayDraft = {
  key: string;
  laborDate: string;
  description: string;
  note: string;
  calculatedDefaultAmount: string;
...
function purchaseOrderDraftFor(row: PayrollCrewShowRow): PurchaseOrderDraft {
  const po = row.purchaseOrder;
  const adjustments = po?.adjustments;
  const savedDaysByDate = new Map((po?.days ?? []).map((day) => [day.laborDate, day]));
  const callsByDate = new Map<string, typeof row.calls>();
  for (const call of row.calls) {
    const laborDate = String(call.laborDate || "").slice(0, 10);
    if (!laborDate) continue;
    callsByDate.set(laborDate, [...(callsByDate.get(laborDate) ?? []), call]);
  }
  const days: PurchaseOrderDayDraft[] = [...callsByDate.entries()].sort(([left], [right]) => left.localeCompare(right)).map(([laborDate, calls]) => {
    const saved = savedDaysByDate.get(laborDate);
    const description = [...new Set(calls.map((call) => [call.payLabel, call.roleName, call.area].filter(Boolean).join(" — ")).filter(Boolean))].join("; ");
    const calculatedDefaultAmount = calls.reduce((sum, call) => sum + Number(call.amount || 0), 0);
    return {
      key: saved?.key || laborDate,
      laborDate,
      description: description || saved?.description || "Scheduled labor day",
      note: saved?.note || "",
      calculatedDefaultAmount: currencyText(calculatedDefaultAmount),
      defaultAmount: currencyText(saved?.defaultAmount ?? calculatedDefaultAmount),
      overtimeHours: saved?.overtimeHours ? String(saved.overtimeHours) : "",
      overtimeRate: saved?.overtimeRate ? currencyText(saved.overtimeRate) : "",
      doubleTimeHours: saved?.doubleTimeHours ? String(saved.doubleTimeHours) : "",
      doubleTimeRate: saved?.doubleTimeRate ? currencyText(saved.doubleTimeRate) : "",
    };
  });
  if (!days.length && po?.days?.length) {
    days.push(...po.days.map((day) => ({
      key: day.key,
      laborDate: day.laborDate,
      description: day.description,
      note: day.note || "",
      calculatedDefaultAmount: currencyText(day.defaultAmount),
      defaultAmount: currencyText(day.defaultAmount),
      overtimeHours: day.overtimeHours ? String(day.overtimeHours) : "",
      overtimeRate: day.overtimeRate ? currencyText(day.overtimeRate) : "",
      doubleTimeHours: day.doubleTimeHours ? String(day.doubleTimeHours) : "",
      doubleTimeRate: day.doubleTimeRate ? currencyText(day.doubleTimeRate) : "",
    })));
  }
  const legacyBaseAmount = !po?.days?.length ? po?.baseAmount ?? row.overrideAmount : null;
  if (days.length && legacyBaseAmount !== null && legacyBaseAmount !== undefined) {
    const target = Math.max(0, Number(legacyBaseAmount) || 0);
    const current = days.reduce((sum, day) => sum + poDraftNumber(day.defaultAmount), 0);
    let allocated = 0;
    days.forEach((day, index) => {
      const amount = index === days.length - 1
        ? Math.max(0, Math.round((target - allocated) * 100) / 100)
        : Math.max(0, Math.round((current > 0 ? poDraftNumber(day.defaultAmount) / current * target : index === 0 ? target : 0) * 100) / 100);
      allocated += amount;
      day.defaultAmount = currencyText(amount);
    });
  }
  if (days.length && !po?.days?.length) {
    days[0] = {
      ...days[0],
      overtimeHours: adjustments?.overtimeHours ? String(adjustments.overtimeHours) : "",
      overtimeRate: adjustments?.overtimeRate ? currencyText(adjustments.overtimeRate) : "",
      doubleTimeHours: adjustments?.doubleTimeHours ? String(adjustments.doubleTimeHours) : "",
      doubleTimeRate: adjustments?.doubleTimeRate ? currencyText(adjustments.doubleTimeRate) : "",
    };
  }
  const dayBaseAmount = days.reduce((sum, day) => sum + poDraftNumber(day.defaultAmount), 0);
  return {
    baseAmount: currencyText(days.length ? dayBaseAmount : po?.baseAmount ?? row.overrideAmount ?? row.estimatedTotal),
    days,
    overtimeHours: days.length ? "" : adjustments?.overtimeHours ? String(adjustments.overtimeHours) : "",
    overtimeRate: days.length ? "" : adjustments?.overtimeRate ? currencyText(adjustments.overtimeRate) : "",
    doubleTimeHours: days.length ? "" : adjustments?.doubleTimeHours ? String(adjustments.doubleTimeHours) : "",
    doubleTimeRate: days.length ? "" : adjustments?.doubleTimeRate ? currencyText(adjustments.doubleTimeRate) : "",
    mealPenaltyCount: adjustments?.mealPenaltyCount ? String(adjustments.mealPenaltyCount) : "",
    mealPenaltyRate: adjustments?.mealPenaltyRate ? currencyText(adjustments.mealPenaltyRate) : "",
    shortTurnHours: adjustments?.shortTurnHours ? String(adjustments.shortTurnHours) : "",
    shortTurnRate: adjustments?.shortTurnRate ? currencyText(adjustments.shortTurnRate) : "",
    otherDescription: adjustments?.otherDescription || "",
    otherAmount: adjustments?.otherAmount ? currencyText(adjustments.otherAmount) : "",
    lateHours: adjustments?.lateHours ? String(adjustments.lateHours) : "",
    lateHourlyRate: adjustments?.lateHourlyRate ? currencyText(adjustments.lateHourlyRate) : "",
    negativeDescription: adjustments?.negativeDescription || "",
    negativeAmount: adjustments?.negativeAmount ? currencyText(adjustments.negativeAmount) : "",
  };
}

function poDraftNumber(value: string) {
  const parsed = Number(value || 0);
  return Number.isFinite(parsed) && parsed >= 0 ? Math.round(parsed * 100) / 100 : 0;
}

function purchaseOrderAdjustmentsFromDraft(draft: PurchaseOrderDraft): PurchaseOrderAdjustments {
  return {
    overtimeHours: poDraftNumber(draft.overtimeHours), overtimeRate: poDraftNumber(draft.overtimeRate),
    doubleTimeHours: poDraftNumber(draft.doubleTimeHours), doubleTimeRate: poDraftNumber(draft.doubleTimeRate),
    mealPenaltyCount: Math.floor(poDraftNumber(draft.mealPenaltyCount)), mealPenaltyRate: poDraftNumber(draft.mealPenaltyRate),
    shortTurnHours: poDraftNumber(draft.shortTurnHours), shortTurnRate: poDraftNumber(draft.shortTurnRate),
    otherDescription: draft.otherDescription.trim(), otherAmount: poDraftNumber(draft.otherAmount),
    lateHours: poDraftNumber(draft.lateHours), lateHourlyRate: poDraftNumber(draft.lateHourlyRate),
    negativeDescription: draft.negativeDescription.trim(), negativeAmount: poDraftNumber(draft.negativeAmount),
  };
}

function purchaseOrderDaysFromDraft(draft: PurchaseOrderDraft) {
  return draft.days.map((day) => ({
    key: day.key,
    laborDate: day.laborDate,
    description: day.description.trim(),
    note: day.note.trim(),
    defaultAmount: poDraftNumber(day.defaultAmount),
    overtimeHours: poDraftNumber(day.overtimeHours),
    overtimeRate: poDraftNumber(day.overtimeRate),
    doubleTimeHours: poDraftNumber(day.doubleTimeHours),
    doubleTimeRate: poDraftNumber(day.doubleTimeRate),
  }));
}

function purchaseOrderDraftTotal(draft: PurchaseOrderDraft) {
  const a = purchaseOrderAdjustmentsFromDraft(draft);
  const days = purchaseOrderDaysFromDraft(draft);
  const dayTotal = days.reduce((sum, day) => sum + day.defaultAmount + day.overtimeHours * day.overtimeRate + day.doubleTimeHours * day.doubleTimeRate, 0);
  const baseAndPremiums = days.length ? dayTotal : poDraftNumber(draft.baseAmount) + a.overtimeHours * a.overtimeRate + a.doubleTimeHours * a.doubleTimeRate;
  return Math.max(0, Math.round((baseAndPremiums + a.mealPenaltyCount * a.mealPenaltyRate + a.shortTurnHours * a.shortTurnRate + a.otherAmount - a.lateHours * a.lateHourlyRate - a.negativeAmount) * 100) / 100);
}

function purchaseOrderPreviewLines(draft: PurchaseOrderDraft) {
  const adjustments = purchaseOrderAdjustmentsFromDraft(draft);
  const days = purchaseOrderDaysFromDraft(draft);
  const lines: Array<{ label: string; detail: string; amount: number }> = [];
  if (days.length) {
    for (const day of days) {
      const date = formatPayrollDate(day.laborDate) || "Scheduled day";
      lines.push({ label: `${date} — Default day pay`, detail: [day.description, day.note ? `Day note: ${day.note}` : ""].filter(Boolean).join(" · "), amount: day.defaultAmount });
      const overtime = Math.round(day.overtimeHours * day.overtimeRate * 100) / 100;
      const doubleTime = Math.round(day.doubleTimeHours * day.doubleTimeRate * 100) / 100;
      if (overtime > 0) lines.push({ label: `${date} — Overtime`, detail: `${day.overtimeHours} hr × ${money(day.overtimeRate)}`, amount: overtime });
      if (doubleTime > 0) lines.push({ label: `${date} — Double time`, detail: `${day.doubleTimeHours} hr × ${money(day.doubleTimeRate)}`, amount: doubleTime });
    }
  } else {
    lines.push({ label: "Base show pay", detail: "", amount: poDraftNumber(draft.baseAmount) });
    const overtime = Math.round(adjustments.overtimeHours * adjustments.overtimeRate * 100) / 100;
    const doubleTime = Math.round(adjustments.doubleTimeHours * adjustments.doubleTimeRate * 100) / 100;
    if (overtime > 0) lines.push({ label: "Overtime", detail: `${adjustments.overtimeHours} hr × ${money(adjustments.overtimeRate)}`, amount: overtime });
    if (doubleTime > 0) lines.push({ label: "Double time", detail: `${adjustments.doubleTimeHours} hr × ${money(adjustments.doubleTimeRate)}`, amount: doubleTime });
  }
  const mealPenalty = adjustments.mealPenaltyCount * adjustments.mealPenaltyRate;
  const shortTurn = adjustments.shortTurnHours * adjustments.shortTurnRate;
  const lateDeduction = adjustments.lateHours * adjustments.lateHourlyRate;
  if (mealPenalty > 0) lines.push({ label: "Meal penalty", detail: `${adjustments.mealPenaltyCount} occurrence${adjustments.mealPenaltyCount === 1 ? "" : "s"} × ${money(adjustments.mealPenaltyRate)}`, amount: mealPenalty });
  if (shortTurn > 0) lines.push({ label: "Under-8-hour turnaround", detail: `${adjustments.shortTurnHours} hr × ${money(adjustments.shortTurnRate)}`, amount: shortTurn });
  if (adjustments.otherAmount > 0) lines.push({ label: adjustments.otherDescription || "Other adjustment", detail: "", amount: adjustments.otherAmount });
  if (lateDeduction > 0) lines.push({ label: "Late arrival deduction", detail: `${adjustments.lateHours} hr × ${money(adjustments.lateHourlyRate)}`, amount: -lateDeduction });
  if (adjustments.negativeAmount > 0) lines.push({ label: adjustments.negativeDescription || "Negative adjustment", detail: "Deduction", amount: -adjustments.negativeAmount });
  return lines;
}

function purchaseOrderPreviewNumber(row: PayrollCrewShowRow, draft: PurchaseOrderDraft) {
  const purchaseOrder = row.purchaseOrder;
  if (!purchaseOrder?.referenceNumber) return "Reference assigned when saved";
  const days = purchaseOrderDaysFromDraft(draft);
  const adjustments = purchaseOrderAdjustmentsFromDraft(draft);
  const baseAmount = days.length ? Math.round(days.reduce((sum, day) => sum + day.defaultAmount, 0) * 100) / 100 : poDraftNumber(draft.baseAmount);
  const changed = purchaseOrder.baseAmount !== baseAmount || JSON.stringify(purchaseOrder.days) !== JSON.stringify(days) || JSON.stringify(purchaseOrder.adjustments) !== JSON.stringify(adjustments);
  const locked = !["draft", "ready", "revised", "delivery_failed"].includes(purchaseOrder.status);
  return `${purchaseOrder.referenceNumber}-R${changed && locked ? purchaseOrder.revision + 1 : purchaseOrder.revision}`;
}

function purchaseOrderStatusLabel(status: string | null | undefined) {
  return ({ draft: "Draft", ready: "Ready", revised: "Revised — reapproval required", sent_awaiting_delivery: "Sent — awaiting delivery", delivered: "Delivered — awaiting approval", approved: "Approved", auto_approved: "Automatically approved — no response", changes_requested: "Changes requested", delivery_failed: "Delivery failed" } as Record<string, string>)[String(status || "")] || "Not prepared";
}

function purchaseOrderChangeRequestIsUnread(purchaseOrder: PayrollCrewShowRow["purchaseOrder"]) {
  if (!purchaseOrder || purchaseOrder.status !== "changes_requested" || !String(purchaseOrder.changeRequest || "").trim()) return false;
  if (!purchaseOrder.changeRequestViewedAt) return true;
  if (!purchaseOrder.changeRequestedAt) return false;
  return new Date(purchaseOrder.changeRequestViewedAt).getTime() < new Date(purchaseOrder.changeRequestedAt).getTime();
}
...
function eventPayrollIsFullyPaid(event: PayrollEventSummary) {
  if (event.rows.length) {
    if (!event.rows.every((row) => paymentStatusFor(row) === "paid")) return false;
  } else if (event.payableTotal <= 0 || event.paidTotal < event.payableTotal) {
    return false;
  }
...
async function fetchPayrollRequest(url: string) {
  // Do not attach an AbortController to Payroll reads. The yearly summary can legitimately
  // take longer than a few seconds on a large year, and aborting the browser request also
  // cancels the in-flight server/Supabase work (surfacing as "signal is aborted without reason").
  return fetch(url, { cache: "no-store", headers: { Accept: "application/json" } });
}

async function readPayrollResponse<T>(response: Response): Promise<T> {
  const raw = await response.text();
  if (!raw) {
    throw new Error(`Payroll returned an empty response (HTTP ${response.status}).`);
  }
  try {
    return JSON.parse(raw) as T;
  } catch {
    const preview = raw.replace(/\s+/g, " ").trim().slice(0, 220);
    throw new Error(`Payroll returned a non-JSON response (HTTP ${response.status})${preview ? `: ${preview}` : "."}`);
  }
}

export default function PayrollClient({ initialRows, initialEventSummaries, availableYears, initialYear, initialSummaryLoaded, initialError, initialDiagnostics = null }: Props) {
  const [rows, setRows] = useState(initialRows);
  const [eventSummaries, setEventSummaries] = useState<PayrollEventSummary[]>(initialEventSummaries);
  const [year, setYear] = useState(initialYear);
  const [loadedYears, setLoadedYears] = useState<Set<number>>(() => initialSummaryLoaded ? new Set([initialYear]) : new Set<number>());
  const [attemptedYears, setAttemptedYears] = useState<Set<number>>(() => initialError ? new Set([initialYear]) : new Set<number>());
  const [yearErrors, setYearErrors] = useState<Record<number, string>>(() => initialError ? { [initialYear]: initialError } : {});
  ...
  const [yearLoadProgress, setYearLoadProgress] = useState<Record<number, YearLoadProgress>>({});
  ...
  const [paidShowsOpen, setPaidShowsOpen] = useState(true);
  const [paidShowsVisible, setPaidShowsVisible] = useState(25);
  ...
  const [taxDataYears, setTaxDataYears] = useState<Set<number>>(() => new Set(initialRows.map((row) => row.showYear)));
  const [loadingTaxYear, setLoadingTaxYear] = useState<number | null>(null);
  ...
  const [purchaseOrderDrafts, setPurchaseOrderDrafts] = useState<Record<string, PurchaseOrderDraft>>({});
  const [selectedPurchaseOrderKeys, setSelectedPurchaseOrderKeys] = useState<Set<string>>(new Set());
  const [previewPurchaseOrderKey, setPreviewPurchaseOrderKey] = useState<string | null>(null);
  const [savingPurchaseOrderKey, setSavingPurchaseOrderKey] = useState<string | null>(null);
  const [sendingPurchaseOrderShowId, setSendingPurchaseOrderShowId] = useState<string | null>(null);
  const [message, setMessage] = useState(initialError || "");
  const [diagnostics, setDiagnostics] = useState<PayrollPerformanceDiagnostics | null>(initialDiagnostics);
  ...
  const paidTotal = useMemo(() => Math.round(summaryEvents.reduce((sum, event) => sum + Number(event.paidTotal || 0), 0) * 100) / 100, [summaryEvents]);
  const unpaidTotal = useMemo(() => Math.round(summaryEvents.reduce((sum, event) => sum + Number(event.unpaidTotal || 0), 0) * 100) / 100, [summaryEvents]);
  const payableTotal = Math.round((paidTotal + unpaidTotal) * 100) / 100;
  const loadedEventCount = useMemo(() => summaryEvents.filter((event) => loadedDetailShowIds.has(event.showId)).length, [summaryEvents, loadedDetailShowIds]);
  const savedSnapshotCount = useMemo(() => summaryEvents.filter((event) => event.payrollSnapshotStatus === "saved").length, [summaryEvents]);
  const summaryRequestCompleted = loadedYears.has(year);
  const currentYearError = yearErrors[year] || "";
  const currentLoadProgress = yearLoadProgress[year] || null;
  const summaryFailed = Boolean(currentYearError) && summaryEvents.length === 0 && yearRows.length === 0;
  const partialSummary = Boolean(currentYearError) && summaryEvents.length > 0;
  const totalScopeLabel = summaryFailed
    ? "Payroll show index could not load. Open diagnostics below, then retry."
    : partialSummary
      ? `Partial payroll calculation: ${Math.max(0, (currentLoadProgress?.done ?? 0) - (currentLoadProgress?.failed ?? 0))}/${currentLoadProgress?.total ?? summaryEvents.length} shows calculated · ${currentLoadProgress?.failed ?? 0} failed. ${currentYearError}`
      : summaryRequestCompleted
        ? `${savedSnapshotCount}/${summaryEvents.length} paid-show snapshots · ${loadedEventCount} open show detail${loadedEventCount === 1 ? "" : "s"} loaded`
        : currentLoadProgress?.total
          ? `Loading open Payroll details: ${currentLoadProgress.done}/${currentLoadProgress.total}`
          : "Loading Payroll dashboard";
  const summaryMoney = (value: number) => summaryFailed ? "Could not load" : summaryEvents.length || summaryRequestCompleted || (currentLoadProgress?.done ?? 0) > 0 ? money(value) : "Loading...";
  ...
  async function selectPayrollYear(nextYear: number, force = false) {
    const previousYear = year;
    setYear(nextYear);
    setExpandedPaidEventIds(new Set());
    setPaidShowsOpen(true);
    setPaidShowsVisible(25);
    setTaxCenterOpen(false);
    setOpenRows(new Set());
    if (loadedYears.has(nextYear) && !force) return;

    setLoadingYear(nextYear);
    setMessage(`Loading ${nextYear} Payroll dashboard…`);
    setAttemptedYears((current) => new Set([...current, nextYear]));
    setYearLoadProgress((current) => ({ ...current, [nextYear]: { done: 0, total: 0, failed: 0 } }));

    try {
      const indexResponse = await fetchPayrollRequest(`/api/payroll?mode=index&year=${encodeURIComponent(String(nextYear))}`);
      const indexResult = await readPayrollResponse<{
      ...
      if (!indexResponse.ok) throw new Error(indexResult.message || indexResult.error || `Unable to load ${nextYear} payroll show list (HTTP ${indexResponse.status}).`);
      if (indexResult.error) throw new Error(indexResult.error);

      const returnedYear = Number(indexResult.loadedYear ?? nextYear);
      const showIndex = Array.isArray(indexResult.eventSummaries) ? indexResult.eventSummaries : [];
      setRows((current) => current.filter((row) => row.showYear !== returnedYear));
      setEventSummaries((current) => [...current.filter((event) => event.showYear !== returnedYear), ...showIndex]);
      setYearOptions((current) => [...new Set([...current, ...(indexResult.availableYears ?? []), returnedYear])].sort((a, b) => b - a));
      setYear(returnedYear);
      setDiagnostics(indexResult.diagnostics ?? null);
      const showsNeedingLiveDetails = showIndex.filter((event) => !eventPayrollIsFullyPaid(event));
      setYearLoadProgress((current) => ({ ...current, [returnedYear]: { done: 0, total: showsNeedingLiveDetails.length, failed: 0 } }));

      if (!showIndex.length) {
        setLoadedYears((current) => new Set([...current, returnedYear]));
        setYearErrors((current) => {
          const next = { ...current };
          delete next[returnedYear];
          return next;
        });
        setMessage(`No ${returnedYear} shows were found for Payroll.`);
        return;
      }

      if (!showsNeedingLiveDetails.length) {
        setYearLoadProgress((current) => ({ ...current, [returnedYear]: { done: 0, total: 0, failed: 0 } }));
        setLoadedYears((current) => new Set([...current, returnedYear]));
        setAttemptedYears((current) => new Set([...current, returnedYear]));
        setYearErrors((current) => {
          const next = { ...current };
          delete next[returnedYear];
          return next;
        });
        setMessage(`Payroll loaded from paid-show snapshots for ${showIndex.length} ${returnedYear} show${showIndex.length === 1 ? "" : "s"}.`);
        return;
      }

      setMessage(`Loaded paid-show snapshots. Loading ${showsNeedingLiveDetails.length} open or unpaid ${returnedYear} show${showsNeedingLiveDetails.length === 1 ? "" : "s"} with full Payroll rows…`);
      const failedShows: string[] = [];
      let completed = 0;
      const queue = [...showsNeedingLiveDetails];
      ...
      await Promise.all(Array.from({ length: workerCount }, async () => {
        while (queue.length) {
          ...
          if (!event) return;
          try {
            await fetchAndStorePayrollEventDetails(event.showId, returnedYear, { suppressMessage: true });
          } catch (error) {
            failedShows.push(`${event.showName}: ${error instanceof Error ? error.message : "Unable to load show payroll."}`);
          } finally {
            completed += 1;
            const failed = failedShows.length;
            setYearLoadProgress((current) => ({ ...current, [returnedYear]: { done: completed, total: showsNeedingLiveDetails.length, failed } }));
          }
        }
      }));
      ...
      setYearErrors((current) => {
        const next = { ...current };
        if (failedShows.length) next[returnedYear] = `Some open payroll rows failed to load: ${failedShows.slice(0, 3).join(" | ")}${failedShows.length > 3 ? ` (+${failedShows.length - 3} more)` : ""}`;
        else delete next[returnedYear];
        return next;
      });
      setMessage(failedShows.length
        ? `Payroll loaded with ${failedShows.length} open show${failedShows.length === 1 ? "" : "s"} needing retry. Paid shows are shown from saved snapshots.`
        : `Payroll loaded. Paid shows are shown from snapshots; unpaid and partially paid shows are open with full Payroll rows.`);
    } catch (error) {
      setYear(previousYear);
      const nextMessage = error instanceof Error ? error.message : `Unable to load ${nextYear} Payroll show list.`;
      setYearErrors((current) => ({ ...current, [nextYear]: nextMessage }));
  ...
  async function openTaxCenterForYear() {
    const targetYear = year;
    setTaxCenterOpen(true);
    if (loadingTaxYear === targetYear) return;

    const eventsForYear = summaryEvents.filter((event) => event.showYear === targetYear);
    if (!eventsForYear.length && !loadedYears.has(targetYear)) {
      setMessage(`Tax Center opened. Payroll is still loading the ${targetYear} show list; 1099 candidates will appear once the show list is ready.`);
      return;
    }
    const showsMissingTaxRows = eventsForYear.filter((event) => !loadedDetailShowIds.has(event.showId));

    if (taxDataYears.has(targetYear) || showsMissingTaxRows.length === 0) {
      setTaxDataYears((current) => new Set([...current, targetYear]));
      setMessage("");
      return;
    }

    setLoadingTaxYear(targetYear);
    setMessage(`Tax Center opened. Loading ${showsMissingTaxRows.length} paid or archived show${showsMissingTaxRows.length === 1 ? "" : "s"} for 1099 detail…`);
    const failedShows: string[] = [];
    let completed = 0;
    const queue = [...showsMissingTaxRows];
    const workerCount = Math.min(3, queue.length);
    try {
      await Promise.all(Array.from({ length: workerCount }, async () => {
        while (queue.length) {
          ...
          if (!event) return;
          try {
            await fetchAndStorePayrollEventDetails(event.showId, targetYear, { suppressMessage: true });
          } catch (error) {
            failedShows.push(`${event.showName}: ${error instanceof Error ? error.message : "Unable to load show payroll."}`);
          } finally {
            completed += 1;
            setMessage(`Tax Center open. Loading 1099 detail: ${completed}/${showsMissingTaxRows.length} show${showsMissingTaxRows.length === 1 ? "" : "s"} checked.`);
          }
        }
      }));
      setTaxDataYears((current) => new Set([...current, targetYear]));
      setMessage(failedShows.length
        ? `Tax Center opened, but ${failedShows.length} show${failedShows.length === 1 ? "" : "s"} could not load for 1099 detail: ${failedShows.slice(0, 3).join(" | ")}${failedShows.length > 3 ? ` (+${failedShows.length - 3} more)` : ""}`
        : "Tax Center loaded.");
    } finally {
      setLoadingTaxYear(null);
    }
  }

  async function fetchAndStorePayrollEventDetails(showId: string, targetYear: number, options: { suppressMessage?: boolean } = {}) {
    const response = await fetchPayrollRequest(`/api/payroll?mode=event&year=${encodeURIComponent(String(targetYear))}&show_id=${encodeURIComponent(showId)}`);
    const result = await readPayrollResponse<{
    ...
    if (result.error && !nextRows.length) throw new Error(result.error);
    if (!nextRows.length && !nextEvents.some((event) => event.showId === showId)) {
      throw new Error("No payroll details loaded for this show yet. Check that the event still exists, then try opening payroll details again.");
    }
    ...
    if (!options.suppressMessage) setMessage(result.error || "");
    return { nextRows, nextEvents, loadedYear: Number(result.loadedYear ?? targetYear), error: result.error || null };
  }
  ...
  async function loadPayrollEventDetails(showId: string) {
    ...
    setMessage("");
    try {
      const result = await fetchAndStorePayrollEventDetails(showId, year);
      setLoadedYears((current) => new Set([...current, result.loadedYear]));
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (!loadedYears.has(initialYear) && !attemptedYears.has(initialYear)) {
  ...
  async function retryPayrollTotals() {
    setAttemptedYears((current) => {
      const next = new Set(current);
      next.delete(year);
      return next;
    });
    await selectPayrollYear(year, true);
  }
...
(summary: PayrollYearTechSummary, reviewAction: "approve_w9" | "request_correction" | "mark_needs_review") {
  ...
  function currentPurchaseOrderDraft(row: PayrollCrewShowRow) {
    return purchaseOrderDrafts[row.key] ?? purchaseOrderDraftFor(row);
  }

  function updatePurchaseOrderDraft(row: PayrollCrewShowRow, patch: Partial<PurchaseOrderDraft>) {
    setPurchaseOrderDrafts((current) => ({ ...current, [row.key]: { ...(current[row.key] ?? purchaseOrderDraftFor(row)), ...patch } }));
  }

  function updatePurchaseOrderDayDraft(row: PayrollCrewShowRow, dayKey: string, patch: Partial<PurchaseOrderDayDraft>) {
    const current = currentPurchaseOrderDraft(row);
    updatePurchaseOrderDraft(row, { days: current.days.map((day) => day.key === dayKey ? { ...day, ...patch } : day) });
  }

  function togglePurchaseOrderSelection(rowKey: string, checked: boolean) {
    setSelectedPurchaseOrderKeys((current) => {
      const next = new Set(current);
  ...
  async function savePurchaseOrder(row: PayrollCrewShowRow, quiet = false) {
    const draft = currentPurchaseOrderDraft(row);
    setSavingPurchaseOrderKey(row.key);
    if (!quiet) setMessage("");
    try {
      if (poDraftNumber(draft.negativeAmount) > 0 && !draft.negativeDescription.trim()) {
        throw new Error("Enter a reason for the negative adjustment.");
      }
      const response = await fetch("/api/payroll", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "save_purchase_order", show_id: row.showId, crew_id: row.crewId, base_amount: poDraftNumber(draft.baseAmount), days: purchaseOrderDaysFromDraft(draft), adjustments: purchaseOrderAdjustmentsFromDraft(draft) }),
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok || !result.ok) throw new Error(result.message || "Unable to save Purchase Order.");
      setRows((current) => current.map((item) => item.key === row.key ? { ...item, purchaseOrder: result.purchase_order, overrideAmount: result.row?.payout_override ?? result.purchase_order?.total ?? item.overrideAmount } : item));
      if (!quiet) setMessage(result.message || `Purchase Order saved for ${row.crewName}.`);
      ...
      setSavingPurchaseOrderKey(null);
    }
  }

  async function sendPurchaseOrders(event: PayrollEventSummary, selectedOnly: boolean) {
    const targets = event.rows.filter((row) => !selectedOnly || selectedPurchaseOrderKeys.has(row.key));
    if (!targets.length) {
      setMessage(selectedOnly ? "Select at least one contractor for this show." : "This show does not have any contractor payroll rows.");
      return;
    }
    setSendingPurchaseOrderShowId(event.showId);
    setMessage(`Preparing ${targets.length} Purchase Order${targets.length === 1 ? "" : "s"}…`);
    try {
      for (const row of targets) await savePurchaseOrder(row, true);
      const response = await fetch("/api/payroll", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "send_purchase_orders", show_id: event.showId, crew_ids: targets.map((row) => row.crewId) }),
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok && !result.results) throw new Error(result.message || "Unable to send Purchase Orders.");
      const byCrewId = new Map((Array.isArray(result.results) ? result.results : []).filter((item: { purchase_order?: unknown }) => item.purchase_order).map((item: { crew_id: string; purchase_order: unknown }) => [item.crew_id, item.purchase_order]));
      setRows((current) => current.map((row) => row.showId === event.showId && byCrewId.has(row.crewId) ? { ...row, purchaseOrder: byCrewId.get(row.crewId) as PayrollCrewShowRow["purchaseOrder"] } : row));
      setSelectedPurchaseOrderKeys((current) => new Set([...current].filter((key) => !targets.some((row) => row.key === key))));
      const failures = (Array.isArray(result.results) ? result.results : []).filter((item: { ok?: boolean }) => !item.ok).map((item: { crew_id: string; message: string }) => `${event.rows.find((row) => row.crewId === item.crew_id)?.crewName || "Contractor"}: ${item.message}`);
      setMessage(`${result.message || "Purchase Orders processed."}${failures.length ? ` ${failures.join(" ")}` : ""}`);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to send Purchase Orders.");
    } finally {
      setSendingPurchaseOrderShowId(null);
    }
  }
  ...
  async function markPurchaseOrderChangeViewed(row: PayrollCrewShowRow) {
    try {
      const response = await fetch("/api/payroll", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "mark_purchase_order_change_viewed", show_id: row.showId, crew_id: row.crewId }),
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok || !result.ok) throw new Error(result.message || "Unable to mark the change request viewed.");
      setRows((current) => current.map((item) => item.key === row.key ? { ...item, purchaseOrder: result.purchase_order } : item));
      window.dispatchEvent(new CustomEvent("els:payroll-change-request-count", { detail: Math.max(0, Number(result.unread_count || 0)) }));
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to mark the change request viewed.");
    }
  }

  function toggleDetails(row: PayrollCrewShowRow) {
    setOpenRows((current) => {
      const next = new Set(current);
    ...
    if (!openRows.has(row.key) && purchaseOrderChangeRequestIsUnread(row.purchaseOrder)) void markPurchaseOrderChangeViewed(row);
  }
  ...
  function updateEventFinancials(showId: string, patch: EventFinancialPatch) {
    ...
    setEventSummaries((current) => current.map((event) => event.showId === showId ? {
      ...event,
      ...
      expenseItems: patch.expenseItems === undefined ? event.expenseItems : patch.expenseItems,
      taxReserveDone: patch.taxReserveDone === undefined ? event.taxReserveDone : patch.taxReserveDone,
      taxReserveDoneAt: patch.taxReserveDoneAt !== undefined ? patch.taxReserveDoneAt : patch.taxReserveDone === undefined ? event.taxReserveDoneAt : (patch.taxReserveDone ? (event.taxReserveDoneAt ?? now) : null),
      consecratedHandsDone: patch.consecratedHandsDone === undefined ? event.consecratedHandsDone : patch.consecratedHandsDone,
      consecratedHandsDoneAt: patch.consecratedHandsDoneAt !== undefined ? patch.consecratedHandsDoneAt : patch.consecratedHandsDone === undefined ? event.consecratedHandsDoneAt : (patch.consecratedHandsDone ? (event.consecratedHandsDoneAt ?? now) : null),
    } : event));
  }
  ...
  async function updateAndSaveEventFinancials(event: PayrollEventSummary, patch: EventFinancialPatch) {
    const next = financialSnapshot(event, patch);
    updateEventFinancials(event.showId, next);
    await saveEventFinancials(event, patch);
  }
  ...
  async function saveEventFinancials(event: PayrollEventSummary, patch: EventFinancialPatch = {}) {
    ...
    try {
      const body: Record<string, unknown> = { show_id: event.showId };
      const includeAllFinancialFields = Object.keys(patch).length === 0;
      if (includeAllFinancialFields || patch.revenueOverride !== undefined) body.estimated_revenue_override = next.revenueOverride;
      if (includeAllFinancialFields || patch.expenses !== undefined) body.expenses = next.expenses;
      if (includeAllFinancialFields || patch.financialNotes !== undefined) body.notes = next.financialNotes;
      if (patch.taxReserveDone !== undefined || patch.taxReserveDoneAt !== undefined) {
        body.tax_reserve_done = next.taxReserveDone;
        body.tax_reserve_done_at = next.taxReserveDoneAt;
      }
      if (patch.consecratedHandsDone !== undefined || patch.consecratedHandsDoneAt !== undefined) {
        body.consecrated_hands_done = next.consecratedHandsDone;
        body.consecrated_hands_done_at = next.consecratedHandsDoneAt;
      }
      ...
      if (!response.ok || !result.ok) {
        const fallback = patch.consecratedHandsDone !== undefined
          ? "Could not save Charity payment status."
          : patch.taxReserveDone !== undefined
            ? "Could not save tax reserve status."
      ...
      if (patch.consecratedHandsDone !== undefined) {
        setMessage(patch.consecratedHandsDone ? "Charity marked paid." : "Charity marked unpaid.");
      } else if (patch.taxReserveDone !== undefined) {
        setMessage(patch.taxReserveDone ? "Tax reserve marked set aside." : "Tax reserve marked open.");
      } else {
      ...
      if (patch.consecratedHandsDone !== undefined || patch.taxReserveDone !== undefined) {
        updateEventFinancials(event.showId, {
  ...
  function companyPLRows(includeReserves: boolean): Array<Array<string | number | null | undefined>> {
    const header = includeReserves
      ? ["Show", "Client", "Venue", "Dates", "Estimated Revenue", "Contract Labor", "Expenses", "Net Profit", "Tax Reserve", "Tax Reserve Set Aside", "Charity", "Charity Done", "Tax + Charity", "Pure Profit", "Financial Notes"]
      : ["Show", "Client", "Venue", "Dates", "Estimated Revenue", "Contract Labor", "Expenses", "Net Profit", "Tax Reserve Set Aside", "Charity Done", "Financial Notes"];
  ...
  function companyPLDocument(includeReserves: boolean): ExportDocument {
    return {
      ...
      meta: includeReserves ? [
        ...
        ["Charity", money(plTotals.consecratedHandsDonation)],
...
(): Array<Array<string | number | null | undefined>> {
  ...
  function eventPayrollDocument(event: PayrollEventSummary): ExportDocument {
    ...
    return {
      ...
      meta: [
        ...
        ["Charity Done", event.consecratedHandsDone ? "YES" : "NO"],
      ],
  ...
  function exportOwnerPLWithReservesCsv() {
    downloadCsv(`ELS_${year}_${reportView}_owner_PL_with_reserves.csv`, [
      ["View", viewLabel(reportView)],
      ["Year", "Estimated Revenue", "Contract Labor", "Expenses", "Net Profit", "Tax Reserve", "Charity", "Tax + Charity", "Pure Profit"],
  ...
  function exportEventCsv(event: PayrollEventSummary) {
    downloadCsv(`${event.showName.replace(/[^a-z0-9]+/gi, "_")}_payroll.csv`, [
      ["Show P&L", "Estimated Revenue", "Contract Labor", "Expenses", "Net Profit", "Tax Reserve", "Tax Reserve Set Aside", "Charity", "Charity Done"],
  ...
  if (rows.length === 0 && yearEventSummaries.length === 0 && !loadedYears.has(year) && loadingYear === year) {
    return (
  ...
  return (
    <>
      ...
      {message ? (
        <div className={["failed", "run supabase", "safe mode", "could not", "unable", "abort", "signal", "error", "non-json", "empty response"].some((term) => message.toLowerCase().includes(term)) ? "error" : "success"}>
          <span>{message}</span>
          {currentYearError ? <button className="ghost" type="button" style={{ marginLeft: 12 }} disabled={loadingYear === year} onClick={() => void retryPayrollTotals()}>{loadingYear === year ? "Retrying..." : "Retry Totals"}</button> : null}
        </div>
      ) : null}
      {diagnostics ? (
        <details className="card compact">
          <summary><strong>Payroll diagnostics</strong> <span className="muted small">({diagnostics.mode}, {diagnostics.totalMs} ms)</span></summary>
          <div className="small muted" style={{ marginTop: 8 }}>
            {diagnostics.showCount} show{diagnostics.showCount === 1 ? "" : "s"} · {diagnostics.rowCount} payroll row{diagnostics.rowCount === 1 ? "" : "s"} · {new Date(diagnostics.generatedAt).toLocaleString()}
          </div>
          {diagnostics.summaryRequest ? (
            <div className="card compact" style={{ marginTop: 10, boxShadow: "none" }}>
              <strong>Summary request: {diagnostics.summaryRequest.status === "ok" ? "Succeeded" : "Failed"}</strong>
              <div className="small muted">Request: {diagnostics.summaryRequest.request}</div>
              <div className="small muted">HTTP: {diagnostics.summaryRequest.httpStatus} · Supabase reached: {diagnostics.summaryRequest.reachedSupabase ? "Yes" : "No"} · Rows: {diagnostics.summaryRequest.returnedRows ?? "Unknown"}</div>
              <div className="small muted">Response: {diagnostics.summaryRequest.responseShape}</div>
              {diagnostics.summaryRequest.supabaseCode || diagnostics.summaryRequest.supabaseMessage ? (
                <div className="small muted">Supabase: {diagnostics.summaryRequest.supabaseCode ? `${diagnostics.summaryRequest.supabaseCode} · ` : ""}{diagnostics.summaryRequest.supabaseMessage}</div>
              ) : null}
              {diagnostics.summaryRequest.supabaseDetails ? <div className="small muted">Details: {diagnostics.summaryRequest.supabaseDetails}</div> : null}
              {diagnostics.summaryRequest.supabaseHint ? <div className="small muted">Hint: {diagnostics.summaryRequest.supabaseHint}</div> : null}
            </div>
          ) : null}
          <div className="grid grid-3" style={{ marginTop: 10 }}>
            {diagnostics.timings.map((timing) => (
              <div className="badge" key={`${timing.label}-${timing.ms}`}>{timing.label}: {timing.ms} ms</div>
            ))}
          </div>
        </details>
      ) : null}
      ...
      <section className="grid grid-3">
        <div className="card compact">
          ...
          <div style={{ fontSize: 30, fontWeight: 850 }}>{summaryMoney(payableTotal)}</div>
          <div className="muted small">{totalScopeLabel}</div>
        </div>
        <div className="card compact">
          <div className="muted small">Marked paid</div>
          <div style={{ fontSize: 30, fontWeight: 850, color: "#067647" }}>{summaryMoney(paidTotal)}</div>
          <div className="muted small">{totalScopeLabel}</div>
        </div>
        <div className="card compact">
          <div className="muted small">Still unpaid</div>
          <div style={{ fontSize: 30, fontWeight: 850, color: "#b42318" }}>{summaryMoney(unpaidTotal)}</div>
          <div className="muted small">{totalScopeLabel}</div>
        </div>
      </section>
          ...
          <section className="card">
            <div className="row" style={{ alignItems: "center" }}>
              ...
              <div className="toolbar" style={{ justifyContent: "flex-end" }}>
                {loadingTaxYear === year ? <span className="badge event-badge-upcoming">Loading 1099 detail…</span> : null}
                <button className="ghost" type="button" disabled={queueingBulkW9 || missing1099Summaries.length === 0 || loadingTaxYear === year} onClick={() => void queueMissingW9Requests()}>
                  {queueingBulkW9 ? "Queueing..." : `Queue missing W-9s (${missing1099Summaries.length})`}
                </button>
                <button className="ghost" type="button" disabled={loadingTaxYear === year} onClick={() => setShowTechTotals(true)}>Open tech totals</button>
            ...
            <div className="mobile-table" style={{ overflowX: "auto", marginTop: 14 }}>
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  ...
                  {taxCenterRows.length === 0 ? <tr><td colSpan={6} className="muted" style={{ padding: 16 }}>{loadingTaxYear === year ? "Tax Center is loading paid-show detail. Candidates will appear here as the rows finish loading." : `No contractors have reached the ${year} ${money(reportingThreshold)} paid threshold in this view.`}</td></tr> : null}
                </tbody>
              </table>
            </div>
          </section>
        ...
        <section className="card">
          <div className="row" style={{ alignItems: "center" }}>
            ...
            <div className="toolbar" style={{ justifyContent: "flex-end" }}>
              ...
              <button className="primary" type="button" disabled={loadingTaxYear === year} onClick={() => void openTaxCenterForYear()}>{loadingTaxYear === year ? "Loading Tax Center…" : "Open Tax Center"}</button>
            </div>
          </div>
        </section>
      )}
      ...
      <section className="card accent-card">
        <div className="row" style={{ alignItems: "center" }}>
          <div>
            ...
            <p className="muted" style={{ margin: 0 }}>{viewLabel(reportView)}. Totals load from paid-show snapshots plus live open-show payroll rows. Paid shows stay archived at the bottom; unpaid and partially paid shows stay open with controls visible. Current view excludes future shows from payable, unpaid, and P&L totals. Estimated view includes all scheduled shows for planning. Contract labor includes tech payouts plus non-waived coordinator pay.</p>
            ...
            <p className="muted small" style={{ margin: "8px 0 0" }}>{totalScopeLabel}</p>
          </div>
          <div className="toolbar" style={{ justifyContent: "flex-end" }}>
        ...
        <div className="pl-grid" style={{ marginTop: 14 }}>
          <div className="card compact"><div className="muted small">Estimated revenue</div><strong>{summaryMoney(plTotals.estimatedRevenue)}</strong></div>
          <div className="card compact"><div className="muted small">Contract labor</div><strong>{summaryMoney(plTotals.contractLabor)}</strong><div className="muted small">Tech payouts + non-waived coordinator pay</div></div>
          <div className="card compact"><div className="muted small">Expenses</div><strong>{summaryMoney(plTotals.expenses)}</strong></div>
          <div className="card compact"><div className="muted small">Net profit</div><strong>{summaryMoney(plTotals.estimatedProfit)}</strong></div>
        </div>
      </section>
      ...
      <section className="card">
        <div className="row" style={{ alignItems: "center" }}>
          <div>
            ...
            <p className="muted" style={{ margin: 0 }}>Unpaid and partially paid shows stay open with every assigned tech visible. Fully paid shows move to the Paid shows archive at the bottom and load from saved snapshots.</p>
          </div>
      ...
      <div className="list">
        {detailedEvents.map((event) => {
          const eventDetailsLoaded = loadedDetailShowIds.has(event.showId);
          return (
          <section key={event.showId} className="card" style={{ borderTop: "4px solid var(--brand)" }}>
            <div className="row" style={{ alignItems: "flex-start" }}>
              <div>
                ...
                <div className="toolbar" style={{ marginTop: 10 }}>
                  <span className="badge">{eventDetailsLoaded ? `${event.rows.length} techs` : "Details not loaded"}</span>
                  <span className={event.payrollSnapshotStatus === "saved" ? "badge event-badge-current" : "badge"}>
                    {event.payrollSnapshotStatus === "saved" ? "Paid snapshot" : eventDetailsLoaded ? "Live payroll" : "Loading live payroll"}
                  </span>
                ...
                <div className="pl-grid" style={{ marginTop: 12 }}>
                  ...
                  <span className={event.consecratedHandsDone ? "badge event-badge-current" : "badge"}>Charity {event.consecratedHandsDone ? "done" : "open"}</span>
                </div>
              </div>
              <div className="toolbar" style={{ justifyContent: "flex-end" }}>
                {!eventDetailsLoaded ? (
            ...
            {!eventDetailsLoaded ? (
              <div className="card compact" style={{ marginTop: 14, background: "#fbfcfd", boxShadow: "none" }}>
                ...
                <p className="muted small" style={{ marginBottom: 0 }}>This unpaid or partially paid show is queued to load its tech rows, coordinator pay, P&amp;L controls, and tax details automatically. Use the button if you need to force it now.</p>
              </div>
            ) : (
              <>
            ...
            <div className="card compact" style={{ marginTop: 14, background: "#f3f8f5", boxShadow: "none", borderLeft: "4px solid #0f5f3d" }}>
              <div className="row" style={{ alignItems: "center" }}>
                <div>
                  <strong>Independent contractor Purchase Orders</strong>
                  <div className="small muted">Open a contractor’s Details to add overtime, double time, meal penalties, under-8-hour turnaround pay, or another adjustment. Empty items stay off the Purchase Order.</div>
                </div>
                <div className="toolbar" style={{ justifyContent: "flex-end" }}>
                  <span className="badge">{event.rows.filter((row) => selectedPurchaseOrderKeys.has(row.key)).length} selected</span>
                  <button className="ghost" type="button" disabled={sendingPurchaseOrderShowId === event.showId || !event.rows.some((row) => selectedPurchaseOrderKeys.has(row.key))} onClick={() => void sendPurchaseOrders(event, true)}>Send selected</button>
                  <button className="primary" type="button" disabled={sendingPurchaseOrderShowId === event.showId || !event.rows.length} onClick={() => void sendPurchaseOrders(event, false)}>{sendingPurchaseOrderShowId === event.showId ? "Preparing and sending…" : "Send to all in payroll"}</button>
                </div>
              </div>
            </div>
              ...
              <div className="row" style={{ alignItems: "flex-end" }}>
                <label className="field" style={{ minWidth: 180 }}>
                  ...
                  <input
                    ...
                    onKeyDown={currencyEnterToBlur}
                    onBlur={() => commitMoneyDraft(`revenue:${event.showId}`, event.revenueOverride, (value) => void updateAndSaveEventFinancials(event, { revenueOverride: value }))}
                  />
                </label>
                <label className="field" style={{ minWidth: 180 }}>
                  ...
                  <input
                    ...
                    onKeyDown={currencyEnterToBlur}
                    onBlur={() => {
                      if (!event.expenseItems.length) commitMoneyDraft(`expenses:${event.showId}`, event.expenses, (value) => void updateAndSaveEventFinancials(event, { expenses: value ?? 0 }), 0);
                    }}
                  />
                ...
                <label className="field" style={{ minWidth: 210 }}>
                  ...
                  <span className="small" style={{ display: "grid", gap: 8 }}>
                    <label style={{ display: "flex", alignItems: "center", gap: 8 }}><input type="checkbox" checked={event.taxReserveDone} disabled={savingFinancialShowId === event.showId} onChange={(e) => void updateAndSaveEventFinancials(event, { taxReserveDone: e.target.checked })} /> Taxes set aside ({money(event.taxReserve)})</label>
                    <label style={{ display: "flex", alignItems: "center", gap: 8 }}><input type="checkbox" checked={event.consecratedHandsDone} disabled={savingFinancialShowId === event.showId} onChange={(e) => void updateAndSaveEventFinancials(event, { consecratedHandsDone: e.target.checked })} /> Charity ({money(event.consecratedHandsDonation)})</label>
                  </span>
                </label>
                <label className="field" style={{ minWidth: 260, flex: 1 }}>
                  ...
                  <input
                    ...
                    onBlur={(e) => void updateAndSaveEventFinancials(event, { financialNotes: e.currentTarget.value })}
                  />
                </label>
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                ...
                <tbody>
                  {event.rows.map((row) => {
                    ...
                    const poDraft = currentPurchaseOrderDraft(row);
                    return (
                      <tr key={row.key} style={{ borderBottom: "1px solid var(--line)", verticalAlign: "top" }}>
                        <td style={{ padding: "12px 8px", minWidth: 190 }}>
                          <label style={{ display: "flex", alignItems: "center", gap: 8 }}><input type="checkbox" checked={selectedPurchaseOrderKeys.has(row.key)} onChange={(event) => togglePurchaseOrderSelection(row.key, event.target.checked)} /><strong>{row.crewName}</strong></label>
                          <div className="muted small">{row.crewPhone || row.crewEmail || "No contact"}</div>
                          <div style={{ marginTop: 6 }}><span className="badge" style={{ background: row.purchaseOrder?.status === "changes_requested" || row.purchaseOrder?.status === "delivery_failed" ? "#fee2e2" : row.purchaseOrder?.status === "approved" || row.purchaseOrder?.status === "auto_approved" ? "#dcfce7" : "#fff7d6", fontWeight: 800 }}>{purchaseOrderStatusLabel(row.purchaseOrder?.status)}</span></div>
                          {purchaseOrderChangeRequestIsUnread(row.purchaseOrder) ? <div style={{ marginTop: 6 }}><span className="nav-notification-badge">New request</span></div> : null}
                        </td>
                        ...
                        <td style={{ padding: "12px 8px" }}>
                          <button className="ghost" type="button" onClick={() => toggleDetails(row)}>{isOpen ? "Hide" : row.purchaseOrder?.status === "changes_requested" ? "Open change request" : "View"}</button>
                          {isOpen ? (
                            <div className="card compact" style={{ marginTop: 10, minWidth: 360 }}>
                              <div style={{ borderBottom: "1px solid var(--line)", paddingBottom: 14, marginBottom: 14 }}>
                                <div className="row" style={{ alignItems: "center" }}><strong>Purchase Order pay</strong><span className="badge">Total {money(purchaseOrderDraftTotal(poDraft))}</span></div>
                                {row.purchaseOrder?.changeRequest ? <p className="error" style={{ margin: "8px 0" }}><strong>Contractor requested:</strong> {row.purchaseOrder.changeRequest}</p> : null}
                                <div className="grid grid-2" style={{ marginTop: 10 }}>
                                  <div className="field"><span>Current status</span><strong>{purchaseOrderStatusLabel(row.purchaseOrder?.status)}</strong>{row.purchaseOrder ? <span className="small muted">{purchaseOrderDisplayNumber(row.purchaseOrder) || "Reference assigned when saved"}</span> : <span className="small muted">Reference assigned when saved</span>}{row.purchaseOrder?.approvalDueAt ? <span className="small muted">Due {new Date(row.purchaseOrder.approvalDueAt).toLocaleString()}</span> : null}</div>
                                  <div className="field"><span>Default day pay</span><strong>{money(poDraft.days.reduce((sum, day) => sum + poDraftNumber(day.defaultAmount), 0))}</strong><span className="small muted">The sum of the scheduled day lines below.</span></div>
                                  {poDraft.days.length ? <div style={{ gridColumn: "1 / -1", display: "grid", gap: 10 }}>
                                    {poDraft.days.map((day) => <div key={day.key} style={{ border: "1px solid var(--line)", borderRadius: 12, padding: 12, background: "#f8fafc" }}>
                                      <div className="row" style={{ alignItems: "flex-start", marginBottom: 10 }}><div><strong>{formatPayrollDate(day.laborDate) || "Scheduled day"}</strong><div className="small muted">{day.description}</div></div><span className="badge">{money(poDraftNumber(day.defaultAmount) + poDraftNumber(day.overtimeHours) * poDraftNumber(day.overtimeRate) + poDraftNumber(day.doubleTimeHours) * poDraftNumber(day.doubleTimeRate))}</span></div>
                                      <div className="grid grid-2">
                                        <label className="field">Default day pay<input inputMode="decimal" value={day.defaultAmount} onChange={(event) => updatePurchaseOrderDayDraft(row, day.key, { defaultAmount: sanitizeCurrencyInput(event.target.value) })} />{poDraftNumber(day.defaultAmount) !== poDraftNumber(day.calculatedDefaultAmount) ? <button className="ghost" type="button" style={{ marginTop: 6 }} onClick={() => updatePurchaseOrderDayDraft(row, day.key, { defaultAmount: day.calculatedDefaultAmount })}>Use current crew rate {money(poDraftNumber(day.calculatedDefaultAmount))}</button> : <span className="small muted">Matches the current crew-pay rate.</span>}</label>
                                        <div className="field"><span>Day</span><strong>{formatPayrollDate(day.laborDate)}</strong></div>
                                        <label className="field">Overtime hours<input inputMode="decimal" value={day.overtimeHours} onChange={(event) => updatePurchaseOrderDayDraft(row, day.key, { overtimeHours: sanitizeCurrencyInput(event.target.value) })} /></label>
                                        <label className="field">Overtime hourly rate<input inputMode="decimal" value={day.overtimeRate} onChange={(event) => updatePurchaseOrderDayDraft(row, day.key, { overtimeRate: sanitizeCurrencyInput(event.target.value) })} /></label>
                                        <label className="field">Double-time hours<input inputMode="decimal" value={day.doubleTimeHours} onChange={(event) => updatePurchaseOrderDayDraft(row, day.key, { doubleTimeHours: sanitizeCurrencyInput(event.target.value) })} /></label>
                                        <label className="field">Double-time hourly rate<input inputMode="decimal" value={day.doubleTimeRate} onChange={(event) => updatePurchaseOrderDayDraft(row, day.key, { doubleTimeRate: sanitizeCurrencyInput(event.target.value) })} /></label>
                                        <label className="field" style={{ gridColumn: "1 / -1" }}>Day note (optional)<textarea rows={2} value={day.note} onChange={(event) => updatePurchaseOrderDayDraft(row, day.key, { note: event.target.value })} placeholder="Missing meal, hours, rate detail, or anything specific to this day…" /><span className="small muted">Shown with this day on the contractor's Purchase Order only when you enter a note.</span></label>
                                      </div>
                                    </div>)}
                                  </div> : <>
                                    <label className="field">Base show pay<input inputMode="decimal" value={poDraft.baseAmount} onChange={(event) => updatePurchaseOrderDraft(row, { baseAmount: sanitizeCurrencyInput(event.target.value) })} /></label>
                                    <label className="field">Overtime hours<input inputMode="decimal" value={poDraft.overtimeHours} onChange={(event) => updatePurchaseOrderDraft(row, { overtimeHours: sanitizeCurrencyInput(event.target.value) })} /></label>
                                    <label className="field">Overtime hourly rate<input inputMode="decimal" value={poDraft.overtimeRate} onChange={(event) => updatePurchaseOrderDraft(row, { overtimeRate: sanitizeCurrencyInput(event.target.value) })} /></label>
                                    <label className="field">Double-time hours<input inputMode="decimal" value={poDraft.doubleTimeHours} onChange={(event) => updatePurchaseOrderDraft(row, { doubleTimeHours: sanitizeCurrencyInput(event.target.value) })} /></label>
                                    <label className="field">Double-time hourly rate<input inputMode="decimal" value={poDraft.doubleTimeRate} onChange={(event) => updatePurchaseOrderDraft(row, { doubleTimeRate: sanitizeCurrencyInput(event.target.value) })} /></label>
                                  </>}
                                  <label className="field">Meal penalty occurrences<input inputMode="numeric" value={poDraft.mealPenaltyCount} onChange={(event) => updatePurchaseOrderDraft(row, { mealPenaltyCount: event.target.value.replace(/\D/g, "").slice(0, 2) })} /></label>
                                  <label className="field">Amount per meal penalty<input inputMode="decimal" value={poDraft.mealPenaltyRate} onChange={(event) => updatePurchaseOrderDraft(row, { mealPenaltyRate: sanitizeCurrencyInput(event.target.value) })} /></label>
                                  <label className="field">Under-8-hour turnaround hours<input inputMode="decimal" value={poDraft.shortTurnHours} onChange={(event) => updatePurchaseOrderDraft(row, { shortTurnHours: sanitizeCurrencyInput(event.target.value) })} /></label>
                                  <label className="field">Turnaround hourly rate<input inputMode="decimal" value={poDraft.shortTurnRate} onChange={(event) => updatePurchaseOrderDraft(row, { shortTurnRate: sanitizeCurrencyInput(event.target.value) })} /></label>
                                  <label className="field">Other adjustment label<input value={poDraft.otherDescription} onChange={(event) => updatePurchaseOrderDraft(row, { otherDescription: event.target.value })} placeholder="Travel day, kit fee…" /></label>
                                  <label className="field">Other adjustment amount<input inputMode="decimal" value={poDraft.otherAmount} onChange={(event) => updatePurchaseOrderDraft(row, { otherAmount: sanitizeCurrencyInput(event.target.value) })} /></label>
                                  <label className="field">Late / missed hours deduction<input inputMode="decimal" value={poDraft.lateHours} onChange={(event) => updatePurchaseOrderDraft(row, { lateHours: sanitizeCurrencyInput(event.target.value) })} placeholder="Hours missed" /></label>
                                  <label className="field">Late deduction hourly rate<input inputMode="decimal" value={poDraft.lateHourlyRate} onChange={(event) => updatePurchaseOrderDraft(row, { lateHourlyRate: sanitizeCurrencyInput(event.target.value) })} placeholder="Rate per hour" /></label>
                                  <label className="field">Negative adjustment reason<input value={poDraft.negativeDescription} onChange={(event) => updatePurchaseOrderDraft(row, { negativeDescription: event.target.value })} placeholder="Equipment damage, advance…" /></label>
                                  <label className="field">Negative adjustment amount<input inputMode="decimal" value={poDraft.negativeAmount} onChange={(event) => updatePurchaseOrderDraft(row, { negativeAmount: sanitizeCurrencyInput(event.target.value) })} placeholder="Amount to deduct" /></label>
                                </div>
                                <div className="toolbar" style={{ marginTop: 10 }}><button className="primary" type="button" disabled={savingPurchaseOrderKey === row.key} onClick={() => void savePurchaseOrder(row)}>{savingPurchaseOrderKey === row.key ? "Saving…" : row.purchaseOrder ? "Save Purchase Order revision" : "Save Purchase Order"}</button><button className="ghost" type="button" onClick={() => setPreviewPurchaseOrderKey((current) => current === row.key ? null : row.key)}>{previewPurchaseOrderKey === row.key ? "Close preview" : "Preview Purchase Order"}</button><span className="small muted">Late and negative adjustments reduce the total. Only non-zero categories appear to the contractor.</span></div>
                                {previewPurchaseOrderKey === row.key ? <div style={{ marginTop: 14, border: "2px solid #0f5f3d", borderRadius: 14, overflow: "hidden" }}>
                                  <div style={{ padding: "14px 16px", background: "#f5faf7", borderBottom: "1px solid var(--line)" }}><div className="small muted">Contractor preview</div><strong style={{ fontSize: 18 }}>Purchase Order — {row.showName}</strong><div style={{ color: "#0f5f3d", fontWeight: 900 }}>{purchaseOrderPreviewNumber(row, poDraft)}</div><div className="small muted">{row.crewName} · {[row.showClient, row.showVenue].filter(Boolean).join(" · ")}</div></div>
                                  {purchaseOrderPreviewLines(poDraft).map((line, index) => <div key={`${line.label}-${index}`} className="row" style={{ padding: "11px 14px", borderBottom: "1px solid var(--line)", background: line.amount < 0 ? "#fff4f2" : "white" }}><div><strong>{line.label}</strong>{line.detail ? <div className="small muted">{line.detail}</div> : null}</div><strong style={{ color: line.amount < 0 ? "#b42318" : undefined }}>{money(line.amount)}</strong></div>)}
                                  <div className="row" style={{ padding: "14px", background: "#f5faf7", fontSize: 18 }}><strong>Total</strong><strong>{money(purchaseOrderDraftTotal(poDraft))}</strong></div>
                                </div> : null}
                              </div>
                              <label className="field">
                                Payroll notes
          ...
          <section className="card" style={{ borderTop: "4px solid #16a34a" }}>
            <div className="row" style={{ alignItems: "center" }}>
              <div>
                ...
                <p className="muted" style={{ margin: 0 }}>Paid shows stay at the bottom from the start and use saved snapshots for immediate totals. Expand one only when you need historical row-level controls.</p>
              </div>
              <div className="toolbar" style={{ justifyContent: "flex-end" }}>
                <span className="badge event-badge-current">{collapsedPaidEvents.length} paid</span>
                <button className="ghost" type="button" onClick={() => { setPaidShowsOpen((current) => !current); setPaidShowsVisible(25); }}>
                  {paidShowsOpen ? "Hide paid shows" : "Show paid shows"}
                </button>
              </div>
            </div>
            {paidShowsOpen ? (
              <>
                <div className="list" style={{ marginTop: 14 }}>
                  {collapsedPaidEvents.slice(0, paidShowsVisible).map((event) => (
                      ...
                      <div className="row" style={{ alignItems: "center" }}>
                        <div>
                          ...
                          <div className="toolbar" style={{ marginTop: 8 }}>
                            ...
                            <span className="badge">{event.rows.length ? `${event.rows.length} techs` : "Details not loaded"}</span>
                            ...
                            <span className={event.consecratedHandsDone ? "badge event-badge-current" : "badge event-badge-upcoming"}>
                              Charity {event.consecratedHandsDone ? "Paid" : "Unpaid"} {money(event.consecratedHandsDonation)}
                            </span>
                          </div>
                        </div>
                        <div className="toolbar" style={{ justifyContent: "flex-end" }}>
                          ...
                          <button className="ghost" type="button" onClick={() => exportEventCsv(event)}>CSV</button>
                          <button className="primary" type="button" onClick={() => { setPaidEventExpanded(event.showId, true); void loadPayrollEventDetails(event.showId); }}>Expand payroll</button>
                        </div>
                      </div>
                    </div>
                  ))}
                </div>
                {collapsedPaidEvents.length > paidShowsVisible ? (
                  <div className="toolbar" style={{ marginTop: 12, justifyContent: "center" }}>
                    <button className="ghost" type="button" onClick={() => setPaidShowsVisible((current) => current + 25)}>Load 25 more paid shows</button>
                  </div>
                ) : null}
              </>
            ) : null}
