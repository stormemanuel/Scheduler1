...
function buildExportHtml(document: ExportDocument) {
  ...
  const sections = document.sections.map((section) => {
    const rawAccentColor = /^#[0-9a-f]{6}$/i.test(asText(section.accentColor)) ? asText(section.accentColor) : "#062a31";
    ...
    return `<section class="export-section" style="background: ${accentBackground};">
      ${section.heading ? `<div class="section-heading-bar"><h2>${htmlEscape(section.heading)}</h2></div>` : ""}
...
<style>
  ...
  .export-section { border: 1px solid #d9e2e6; border-left: 3px solid #94a3ab; border-radius: 5px; padding: 10px 12px; page-break-inside: avoid; }
