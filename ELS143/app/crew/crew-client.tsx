          ...
          <div className="row" style={{ alignItems: "flex-start" }}>
            <div>
              ...
              <p className="small muted" style={{ margin: "4px 0 0" }}>
                Master Pool shows every contact. Coordinator pools show only contacts added by that coordinator.
              </p>
            </div>
              ...
              <div className="grid grid-3" style={{ marginTop: 12 }}>
                {cityPoolsForDisplay.map((pool) => (
                  <label key={pool.id} className="checkbox-card">
                    ...
                    <span><strong>{pool.name}</strong><small>{selectedOwnerAllowedPoolSet.has(pool.id) ? "Workspace visible — own contacts only" : "Hidden from coordinator"}</small></span>
                  </label>
                ))}
              </div>
