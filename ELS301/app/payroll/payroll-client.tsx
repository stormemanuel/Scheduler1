...
export default function PayrollClient({ initialRows, availableYears, initialYear, initialError }: Props) {
  ...
  const unpaidEvents = useMemo(() => events.filter((event) => !fullyPaidEventIds.has(event.showId)), [events, fullyPaidEventIds]);
  const paidEvents = useMemo(() => events.filter((event) => fullyPaidEventIds.has(event.showId)), [events, fullyPaidEventIds]);
  ...
  function setEventExpanded(showId: string, expanded: boolean) {
  ...
  function renderExpandedPayroll(event: PayrollEventSummary) {
    return (
      <section className="card" style={{ borderTop: "4px solid var(--brand)" }}>
        <div className="row" style={{ alignItems: "flex-start" }}>
          ...
          <div className="toolbar" style={{ justifyContent: "flex-end" }}>
            ...
            <button className="ghost" type="button" onClick={() => setEventExpanded(event.showId, false)}>Collapse payroll</button>
          </div>
        </div>
  ...
  function renderPayrollSummary(event: PayrollEventSummary, isPaid: boolean) {
    const expanded = expandedEventIds.has(event.showId);
    const coordinator = event.coordinatorPayment;
    const coordinatorIsOpen = Boolean(
      coordinator
      && Number(coordinator.payableAmount || 0) > 0
      && coordinator.paymentStatus !== "paid"
      && !coordinatorFeeWaived(coordinator),
    );

    return (
      <div key={event.showId} className="list" style={{ gap: 10 }}>
        <div
          className="card compact"
          style={{
            boxShadow: "none",
            background: isPaid ? "#f7fff9" : "#fffaf0",
            borderColor: isPaid ? "#b7dfc6" : "#f5d58a",
          }}
        >
          <div className="row" style={{ alignItems: "center" }}>
            <div>
              ...
              <div className="muted small">
                {event.showClient || "No client"} • {event.showVenue || "No venue"} • {formatPayrollDate(event.showStart)} to {formatPayrollDate(event.showEnd)}
              </div>
              <div className="toolbar" style={{ marginTop: 8 }}>
                <span className={isPaid ? "badge event-badge-current" : "badge event-badge-upcoming"}>
                  {isPaid ? "Paid complete" : "Payment open"}
                </span>
                <span className="badge">{event.rows.length} techs</span>
                <span className="badge">Crew paid {money(event.paidTotal)}</span>
                {!isPaid ? <span className="badge">Crew unpaid {money(event.unpaidTotal)}</span> : null}
                {coordinator ? (
                  <span className={coordinatorIsOpen ? "badge event-badge-upcoming" : "badge"}>
                    Coordinator {coordinatorIsOpen ? "open" : "paid"} {money(coordinator.payableAmount)}
                  </span>
                ) : null}
            ...
            <div className="toolbar" style={{ justifyContent: "flex-end" }}>
              ...
              <button className="ghost" type="button" onClick={() => exportEventCsv(event)}>CSV</button>
              <button className="primary" type="button" onClick={() => setEventExpanded(event.showId, !expanded)}>
                {expanded ? "Collapse payroll" : "Expand payroll"}
              </button>
            </div>
          </div>
        </div>
        {expanded ? renderExpandedPayroll(event) : null}
      </div>
    );
  }
  ...
  return (
    <div className="grid" style={{ gap: 16 }}>
      ...
      <div className="list">
        <section className="card" style={{ borderTop: "4px solid #d97706" }}>
          <div className="row" style={{ alignItems: "center" }}>
            <div>
              <h2 style={{ marginBottom: 4 }}>Unpaid Shows</h2>
              <p className="muted" style={{ margin: 0 }}>Shows with any open crew or coordinator payment appear here first. Each show stays collapsed until you open it.</p>
            </div>
            <span className="badge event-badge-upcoming">
              {unpaidEvents.filter((event) => !expandedEventIds.has(event.showId)).length} collapsed
            </span>
          </div>
          <div className="list" style={{ marginTop: 14 }}>
            {unpaidEvents.map((event) => renderPayrollSummary(event, false))}
            {unpaidEvents.length === 0 ? <p className="muted" style={{ margin: 0 }}>No unpaid shows match the current filters.</p> : null}
          </div>
        </section>
        ...
        <section className="card" style={{ borderTop: "4px solid #16a34a" }}>
          <div className="row" style={{ alignItems: "center" }}>
            <div>
              ...
              <p className="muted" style={{ margin: 0 }}>Shows appear here after every crew payment and the assigned coordinator payment are marked Paid. Each show stays collapsed until you open it.</p>
            </div>
            <span className="badge event-badge-current">
              {paidEvents.filter((event) => !expandedEventIds.has(event.showId)).length} collapsed
            </span>
          </div>
          <div className="list" style={{ marginTop: 14 }}>
            {paidEvents.map((event) => renderPayrollSummary(event, true))}
            {paidEvents.length === 0 ? <p className="muted" style={{ margin: 0 }}>No paid shows match the current filters.</p> : null}
          </div>
        </section>
