...
function nonNegativeRateNumber(value: unknown) {
  const parsed = Number(value);
  return Number.isFinite(parsed) && parsed >= 0 ? parsed : null;
}

function matchingExactClientRate(rates: MasterRateRecord[], roleName: string, cityName: string) {
  const requested = normalizedPayrollRateRole(roleName) || normalizePayrollText(roleName);
  const cityKey = normalizePayrollText(cityName || "Default");
  return rates.find((rate) => {
    const rateRole = normalizedPayrollRateRole(rate.role_name) || normalizePayrollText(rate.role_name);
    return normalizePayrollText(rate.city_name || "Default") === cityKey && rateRole === requested;
  }) ?? null;
}
...
export function estimateAssignmentRevenue(options: {
  ...
  const defaultClientRate = matchingExactClientRate(clientRates, call.role_name, "Default");
  const selectedCityClientRate = normalizePayrollText(rateCity) === normalizePayrollText("Default")
    ? null
    : matchingExactClientRate(clientRates, call.role_name, rateCity);
  const configuredClientRateTable = clientRates.length > 0;
  const fallback = configuredClientRateTable ? null : fallbackFor(call.role_name, clientBillingRates);
  const rateFull =
    nonNegativeRateNumber(cityOverride?.full_day) ??
    nonNegativeRateNumber(defaultOverride?.full_day) ??
    (defaultClientRate ? nonNegativeRateNumber(defaultClientRate.full_day) : null) ??
    (selectedCityClientRate ? nonNegativeRateNumber(selectedCityClientRate.full_day) : null);
  ...
  const halfDayRate = halfDayFromFullDay(fullDayRate);
  const source = nonNegativeRateNumber(cityOverride?.full_day) !== null
    ? `${cityOverride?.city_name || rateCity} client-specific rate`
    : nonNegativeRateNumber(defaultOverride?.full_day) !== null
      ? "Client-specific Default rate"
      : defaultClientRate
        ? (Number(defaultClientRate.full_day || 0) > 0 ? "Default client billing rate" : "Default client billing rate not set")
        : selectedCityClientRate
          ? (Number(selectedCityClientRate.full_day || 0) > 0 ? `${selectedCityClientRate.city_name || rateCity} client billing fallback` : `${selectedCityClientRate.city_name || rateCity} client billing rate not set`)
          : configuredClientRateTable
            ? "Client billing position not set"
            : "Legacy ELS default client rate";
