...
export async function getCrewPageData() {
  ...
  const [cityPoolsRes, crewGroupsRes, crewResInitial, positionsRes, unavailableRes, ratesRes, extraPoolsRes, ratingsRes, profilesRes, accessSettingsRes] = await Promise.all([
    ...
    // Select the complete crew row instead of naming every optional migration column.
    // A single not-yet-run optional migration must not make the Crew page fall back
    // to a minimal row that hides otherwise valid onboarding statuses and files.
    dataClient
      .from("crew")
      .select("*")
  ...
  const crewRecords: CrewRecord[] = (crewRes.data ?? []).filter((row) => {
    const typed = row as { id?: string | null; created_by?: string | null; city_pool_id?: string | null; onboarding_status?: string | null; coordinator_hidden_at?: string | null; coordinator_hidden_by?: string | null };
