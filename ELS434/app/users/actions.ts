...
export async function inviteUserAction(formData: FormData) {
  ...
  if (createWithPassword) {
    return { ok: true, message: `Login created for ${email}. You can test it now, then send them the email and temporary password.${messagingModeMessage}` };
  }
  return { ok: true, message: `Invite sent to ${email}.${messagingModeMessage}` };
}
