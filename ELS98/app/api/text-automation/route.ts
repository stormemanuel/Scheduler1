...
type SubCallRow = { id: string; labor_day_id: string; area: string | null; role_name: string | null; start_time: string | null; end_time: string | null; crew_needed: number | null; notes: string | null; sort_order?: number | null; day_type?: string | null };
...
function assignmentDayType(assignment: AssignmentRow, call: SubCallRow) {
  const duration = durationHoursBetween(assignmentStartTime(assignment, call), assignmentEndTime(assignment, call));
...
function nextScheduleByCrew(show: ShowRow, days: LaborDayRow[], calls: SubCallRow[], assignments: AssignmentRow[], crewRows: CrewRow[], settings: AutomationSettings) {
  ...
  for (const item of result.values()) {
    ...
    const nextCall = item.firstCall ? `${formatDate(item.firstCall.day.labor_date)} ${assignmentTimeRange(item.firstCall.assignment, item.firstCall.call)} ${item.firstCall.call.role_name || "Crew"} ${item.firstCall.call.area || ""}` : "Schedule TBD";
    item.values = {
...
async function fetchShowBundle(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, showId: string) {
  ...
  const callsRes = dayIds.length ? await admin.from("sub_calls").select("id, labor_day_id, area, role_name, start_time, end_time, crew_needed, notes, sort_order, day_type").in("labor_day_id", dayIds) : { data: [], error: null };
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const queueOptions = isManualReminderAction
      ...
      : {};
    const queueRows = buildQueueRows(mode, settings, bundle, queueOptions);
