...
export async function getEventsPageData() {
  ...
  const subCallsResInitial = visibleLaborDayIds.length
    ? await supabase
        .from("sub_calls")
        .select("id, labor_day_id, area, location, role_name, master_rate_id, message_rate, start_time, end_time, crew_needed, notes, sort_order, day_type")
        .in("labor_day_id", visibleLaborDayIds)
        .order("labor_day_id", { ascending: true })
        .order("sort_order", { ascending: true })
        .order("start_time", { ascending: true })
    : emptyResult;
  const subCallExtraColumnsMissing = Boolean(subCallsResInitial.error && (subCallsResInitial.error.message.includes("master_rate_id") || subCallsResInitial.error.message.includes("message_rate") || subCallsResInitial.error.message.includes("location") || subCallsResInitial.error.message.includes("sort_order") || subCallsResInitial.error.message.includes("day_type")));
  ...
  const subCalls = (subCallsRes.data ?? []).map((row) => {
    const typed = row as { id: string; labor_day_id: string; area: string | null; location?: string | null; role_name: string | null; master_rate_id?: string | null; message_rate?: string | number | null; start_time: string; end_time: string | null; crew_needed: number | null; notes: string | null; sort_order?: number | null; day_type?: string | null; created_by?: string | null };
    return { id: typed.id, labor_day_id: typed.labor_day_id, area: typed.area ?? "", location: typed.location ?? "", role_name: typed.role_name ?? "", master_rate_id: typed.master_rate_id ?? null, message_rate: typed.message_rate != null ? String(typed.message_rate) : null, start_time: typed.start_time, end_time: typed.end_time ?? "", crew_needed: typed.crew_needed ?? 1, notes: typed.notes ?? "", sort_order: typed.sort_order ?? 0, day_type: typed.day_type || "full_day" } as SubCallRecord;
  });
