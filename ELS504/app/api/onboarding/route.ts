...
async function purchaseOrderRequest(admin: SupabaseAdmin, token: string) {
  ...
  const match = ((payrollResult.data || []) as Array<{ id: string; show_id: string; crew_id: string; notes?: string | null }>).find((row) => extractPurchaseOrderFromNotes(row.notes)?.token === token);
...
async function scopedCrewForOnboarding(admin: SupabaseAdmin, context: OnboardingSenderContext, crewId: string) {
  ...
  const referredCrew = ownsCrew ? false : await coordinatorHasOnboardingAssociation(admin, crewId, context.userId);
  if (!ownsCrew && !referredCrew) return null;
...
async function coordinatorOnboardingDashboard(admin: SupabaseAdmin) {
  ...
  const coordinatorRequests = ((requestRes.data || []) as Record<string, unknown>[]).filter((row) => requestWasSentByUser(row, context.userId));
  ...
  try {
    ...
    if (action === "read_request") {
      ...
      if (purchaseOrderRecord) {
        ...
        return NextResponse.json({ ok: true, request_type: "purchase_order", purchase_order: publicPurchaseOrder({ ...purchaseOrderRecord, purchaseOrder }) });
      }
    }
