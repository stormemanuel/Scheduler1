...
export async function setTemporaryPasswordAction(formData: FormData) {
  ...
  return {
    ok: true,
    message: `Temporary password successfully set for ${typedProfile?.full_name || typedProfile?.email || authUser.user?.email || "this user"}. Updated: ${new Date().toLocaleString("en-US", { timeZone: "America/Chicago" })}.`,
  };
}
...
export async function sendPasswordResetEmailAction(formData: FormData) {
  ...
  return { ok: true, message: `Password reset request accepted for ${email}. Redirect URL: ${redirectTo}` };
}
