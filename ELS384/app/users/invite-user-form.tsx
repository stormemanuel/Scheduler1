  ...
  return (
    <div className="card compact accent-card">
      ...
      <form className="list" action={(formData) => run("temporary", formData)}>
        ...
        <button type="submit" className="ghost" disabled={pending}>{pending ? "Working..." : "Set Temporary Password"}</button>
      </form>
      <form action={(formData) => run("email", formData)} style={{ marginTop: 10 }}>
        <input type="hidden" name="userId" value={userId} />
        <button type="submit" className="ghost" disabled={pending || !email}>Send Password Reset Email</button>
      </form>
