...
export default function PayrollClient({ initialRows, availableYears, initialError }: Props) {
  ...
  async function openSecureW9Document(summary: PayrollYearTechSummary, download = false) {
    ...
    try {
      ...
      setMessage(download ? `Secure W-9 download opened for ${summary.crewName}.` : `Secure W-9 opened for ${summary.crewName}. Link expires in 10 minutes.`);
    } catch (error) {
  ...
  async function updateW9Review(summary: PayrollYearTechSummary, reviewAction: "approve_w9" | "request_correction" | "mark_needs_review") {
    const defaultNote = reviewAction === "approve_w9" ? "Reviewed and approved for 2026 1099 prep." : reviewAction === "request_correction" ? "Please upload a corrected signed W-9." : "Needs owner review.";
            ...
            <tbody>
              {taxCenterRows.map((summary) => (
                <tr key={summary.crewId} style={{ borderBottom: "1px solid var(--line)" }}>
                  ...
                  <td style={{ padding: "10px 8px" }}>
                    <div className="toolbar" style={{ gap: 8 }}>
                      {summary.w9DocumentUrl && !w9Ready(summary) ? <button className="primary" type="button" disabled={reviewingW9CrewId === summary.crewId} onClick={() => void updateW9Review(summary, "approve_w9")}>Approve</button> : null}
              ...
              {taxCenterRows.length === 0 ? <tr><td colSpan={5} className="muted" style={{ padding: 16 }}>No contractors have reached the 2026 $2,000 paid threshold in this view.</td></tr> : null}
            </tbody>
          </table>
        </div>
      </section>
