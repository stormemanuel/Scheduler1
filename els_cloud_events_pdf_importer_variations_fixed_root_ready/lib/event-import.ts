...
const TIME_PATTERN = "\\d{1,2}(?::\\d{2})(?::\\d{2})?\\s*(?:a\\.?m\\.?|p\\.?m\.?)";
...
function extractShowDateRange(line: string, fallbackYear: number) {
  ...
  const parts = value.split(/\s+-\s+|\s+-|\s+to\s+/i).map((part) => compactSpaces(part)).filter(Boolean);
...
function preprocessPdfLines(text: string) {
  ...
  return repairSplitDateLines(lines);
}
...
function parsePdfText(text: string, overrides: Partial<ImportedEventData["show"]>): ImportedEventData {
  ...
  for (const rawLine of lines) {
    ...
    if (isLikelySectionLine(line)) {
      if (isSectionContinuation(line) && currentSection !== "Imported Call") {
        currentSection = compactSpaces(`${currentSection} ${line}`);
      } else {
        currentSection = line;
      }
      continue;
