...
function EventsClientInner({
  ...
  function defaultSubCallDayTime(): SubCallDayTimeDraft {
    return { start_time: "", end_time: "", use_default: false };
  }

  function standardSubCallDayTime(): SubCallDayTimeDraft {
    return { start_time: STANDARD_SUB_CALL_START_TIME, end_time: STANDARD_SUB_CALL_END_TIME, use_default: false };
  }
  ...
  function setDayTimeDraft(dayId: string, patch: Partial<SubCallDayTimeDraft>) {
    setCallDayTimes((current) => {
      ...
      return { ...current, [dayId]: { ...existing, ...patch, use_default: patch.use_default ?? false } };
    });
  }
  ...
  async function startEditCall(dayId: string, call: SubCallRecord) {
    ...
    setCallDayTimes({ [dayId]: { start_time: call.start_time, end_time: call.end_time || "", use_default: false } });
  ...
  async function saveCall() {
    ...
    const mainPayloadForNewDay = (dayId: string) => {
      ...
      return {
        ...
        start_time: startTime,
        end_time: endTime,
        day_type: safeDayType(callForm.day_type) || nextAutoBlock("full_day", startTime, endTime) || "full_day",
      };
    };
    const mainPayloadForExistingCall = (targetCall: SubCallRecord) => {
      ...
      return {
        ...
        start_time: startTime,
        end_time: endTime,
        day_type: safeDayType(callForm.day_type) || nextAutoBlock("full_day", startTime, endTime) || safeDayType(targetCall.day_type) || "full_day",
      };
    };
                          ...
                          if (checked) {
                            ...
                            setCallDayTimes((current) => {
                              ...
                              for (const item of seriesItems) {
                                if (!next[item.day.id]) next[item.day.id] = { start_time: item.call.start_time, end_time: item.call.end_time || "", use_default: false };
                              }
                              return next;
                            });
                          }
                  ...
                  <div className="card compact" style={{ marginTop: 12, boxShadow: "none", background: "#fff" }}>
                    ...
                    <div className="list" style={{ marginTop: 10 }}>
                      {seriesItems.map((item, index) => {
                        ...
                        const draft = callDayTimes[item.day.id] || { start_time: item.call.start_time, end_time: item.call.end_time || "", use_default: false };
