...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: existingSubCall } = await admin
    .from("sub_calls")
    .select("assigned_coordinator_user_id, sub_call_group_id")
    .eq("id", id)
    .maybeSingle();
  ...
  const payload = {
    ...
    notes: notesWithSubCallReferenceMarker(String(body.notes || '').trim(), String(body.sub_call_reference_number || '').trim()),
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const cancellation = await recordRemovedAssignmentsAndCancelQueue(admin, {
    ...
    reason: deleteEntireSubCall ? "Entire sub-call deleted." : "Sub-call day deleted.",
  });
