...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: existingSubCall } = await admin
    .from("sub_calls")
    .select("id, labor_day_id, area, location, po_number, role_name, message_rate, start_time, end_time, crew_needed, day_type, one_hour_walkaway, assigned_coordinator_user_id, sub_call_group_id")
    .eq("id", id)
    .maybeSingle();
