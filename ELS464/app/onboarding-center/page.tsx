...
function onboardingEmailHref(email: unknown, body: unknown) {
  ...
  return `mailto:${encodeURIComponent(address)}?subject=${encodeURIComponent("Emanuel Labor Services Onboarding")}&body=${encodeURIComponent(emailBody)}`;
}
...
export default function OnboardingCenterPage() {
  ...
  function rowOnboardingEmailHref(row: ReviewRow) {
    const email = text(row.crew.email) || text(row.payload.invite_email);
    const displayName = text(row.crew.name) || text(row.payload.invite_name) || "there";
    const invitation = onboardingInvitationMessage(displayName, row.link, access.current_user_name, row.request_type);
    return onboardingEmailHref(email, invitation);
  }
  ...
  async function sendCoordinatorOnboardingByEmail(row: CoordinatorOnboardingRow) {
    ...
    try {
      ...
      const href = onboardingEmailHref(row.email, body);
      if (!href) throw new Error("Unable to prepare onboarding email.");
      window.location.assign(href);
      setMessage(`Onboarding email prepared for ${row.name || "crew member"} using the existing secure link when available.`);
      await Promise.all([loadCoordinatorDashboard(), loadRows()]);
    } catch (error) {
            ...
            <section key={row.id} className="card">
              <div className="row" style={{ alignItems: "flex-start", gap: 14 }}>
                ...
                <div className="toolbar">
                  ...
                  {rowOnboardingEmailHref(row) ? <a className="ghost" href={rowOnboardingEmailHref(row)}>Send onboarding by email</a> : null}
