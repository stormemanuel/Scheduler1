...
function onboardingEmailBody(body: unknown) {
  const raw = String(body || "").replace(/\r\n/g, "\n").trim();
  const urlMatch = raw.match(/https?:\/\/[^\s<>]+/i);
  if (!urlMatch) return raw.replace(/\n/g, "\r\n");

  const secureUrl = urlMatch[0].replace(/[),.;!?]+$/, "");
...
function onboardingEmailHref(email: unknown, body: unknown) {
  ...
  const emailBody = onboardingEmailBody(body);
  return `mailto:${encodeURIComponent(address)}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(emailBody)}`;
}
...
function ContactDetail({ record, ratingSummary, cityPools, targetGroups, bulkCityId, bulkGroupName, detailTab, onTab, onEdit, onDelete, onAcknowledgeCoordinatorHidden, onRestoreCoordinatorHidden, onMove, onQueueOnboarding, onQueueW9Only, onQueueContractOnly, onQueueProfilePhotoOnly, onAdminDocumentUploaded, onBulkCityChange, onBulkGroupChange, saving, canSeeAdminAudit, canSendOnboarding, directAndroidOnboarding }: ContactDetailProps) {
  ...
  async function sendFullOnboardingByEmailFromContact() {
    ...
    try {
      ...
      const href = onboardingEmailHref(record.email, material.message_body);
          ...
          <div className="card compact" style={{ background: "#fbfcfd" }}>
            <div className="row" style={{ alignItems: "flex-start", gap: 12 }}>
              ...
              <div className="toolbar" style={{ justifyContent: "flex-end", flexWrap: "wrap" }}>
                ...
                <button className="ghost" type="button" disabled={saving || onboardingMessageLoading || !record.email} onClick={() => void sendFullOnboardingByEmailFromContact()}>
              ...
              {!onboardingMessageLoading && onboardingMessageMaterial?.message_body ? (
                <>
                  ...
                  <div className="toolbar" style={{ marginTop: 10, flexWrap: "wrap" }}>
                    ...
                    {record.email && onboardingEmailHref(record.email, onboardingMessageMaterial.message_body) ? (
                      <a
                        className="ghost"
                        href={onboardingEmailHref(record.email, onboardingMessageMaterial.message_body)}
                      >
