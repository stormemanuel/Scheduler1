...
async function queueOnboardingRequestForShortcut(record: CrewRecord, requestType: OnboardingRequestType = "full_onboarding") {
  const response = await fetch("/api/onboarding", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "create_request", crew_id: record.id, request_type: requestType, queue_text: true }),
  });
  const data = await response.json().catch(() => ({}));
...
async function prepareOnboardingRequestForEmail(record: CrewRecord) {
  const response = await fetch("/api/onboarding", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "create_request", crew_id: record.id, request_type: "full_onboarding", queue_text: false }),
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.message || "Unable to prepare onboarding email.");
  return data as OnboardingMessageMaterial;
}
...
function onboardingEmailSubject(name: unknown) {
  const crewName = String(name || "").trim();
  return crewName ? `Emanuel Labor Services Onboarding - ${crewName}` : "Emanuel Labor Services Onboarding";
}

function onboardingEmailHref(email: unknown, name: unknown, body: unknown) {
  const recipient = String(email || "").trim();
  if (!recipient) return "";
  return `mailto:${recipient}?subject=${encodeURIComponent(onboardingEmailSubject(name))}&body=${encodeURIComponent(String(body || ""))}`;
}
                      ...
                      <div className="crew-inline-detail" style={{ borderBottom: "1px solid var(--line)", background: "#fff", padding: 14 }}>
                        {draft && editingId === record.id ? (
                          ...
                          <ContactDetail
                            ...
                            onQueueOnboarding={async () => {
                              try {
                                const result = await queueOnboardingRequestForShortcut(record, "full_onboarding");
                                setMessageKind("success");
...
type ContactDetailProps = {
  ...
  onQueueOnboarding: () => Promise<OnboardingMessageMaterial | undefined>;
  onQueueW9Only: () => void;
...
function ContactDetail({ record, ratingSummary, cityPools, targetGroups, bulkCityId, bulkGroupName, detailTab, onTab, onEdit, onDelete, onAcknowledgeCoordinatorHidden, onRestoreCoordinatorHidden, onMove, onQueueOnboarding, onQueueW9Only, onQueueContractOnly, onQueueProfilePhotoOnly, onAdminDocumentUploaded, onBulkCityChange, onBulkGroupChange, saving, canSeeAdminAudit, canSendOnboarding, directAndroidOnboarding }: ContactDetailProps) {
  ...
  async function sendFullOnboardingFromContact() {
    ...
    try {
      ...
      let material = result?.link && result?.message_body ? result : await loadActiveOnboardingMessage(record);
  ...
  async function sendFullOnboardingByEmail() {
    if (!record.email) return;
    onTab("onboarding");
    setOnboardingMessageLoading(true);
    setOnboardingMessageNotice("");
    try {
      const result = await prepareOnboardingRequestForEmail(record);
      let material = result?.link && result?.message_body ? result : await loadActiveOnboardingMessage(record);
      if (material?.found === false) material = null;
      if (!material?.message_body) throw new Error("Unable to prepare the onboarding email message.");
      setOnboardingMessageMaterial(material);
      const href = onboardingEmailHref(record.email, record.name, material.message_body);
      if (!href) throw new Error("This crew contact does not have an email address saved.");
      setOnboardingMessageNotice(`Email prepared for ${record.email}.`);
      window.location.assign(href);
    } catch (error) {
      setOnboardingMessageNotice(error instanceof Error ? error.message : "Unable to prepare the onboarding email.");
    } finally {
      setOnboardingMessageLoading(false);
    }
  }
          ...
          {canSendOnboarding ? (
            <>
              <button
                ...
                onClick={() => {
                  if (!record.phone) return;
                  if (directAndroidOnboarding) {
                    void sendFullOnboardingFromContact();
                    return;
                  }
                  const confirmed = window.confirm(
              ...
              <button
                ...
                onClick={() => void sendFullOnboardingByEmail()}
              >
                {record.email ? "Email onboarding" : "Email needed for onboarding"}
              </button>
            </>
          ) : null}
          {canSeeAdminAudit ? (
          ...
          <div className="card compact" style={{ background: "#fbfcfd" }}>
            <div className="row" style={{ alignItems: "flex-start", gap: 12 }}>
              ...
              <div className="toolbar" style={{ justifyContent: "flex-end" }}>
                ...
                <button className="ghost" type="button" disabled={saving || onboardingMessageLoading || !record.email} onClick={() => void sendFullOnboardingByEmail()}>
                  {record.email ? "Email onboarding" : "Email needed"}
                </button>
              </div>
            </div>
            <div className="grid grid-2">
              ...
              {!onboardingMessageLoading && onboardingMessageMaterial?.message_body ? (
                <>
                  ...
                  <div className="toolbar" style={{ marginTop: 10, flexWrap: "wrap" }}>
                    ...
                    {record.email && onboardingEmailHref(record.email, record.name, onboardingMessageMaterial.message_body) ? (
                      <a
                        className="ghost"
                        href={onboardingEmailHref(record.email, record.name, onboardingMessageMaterial.message_body)}
                      >
                        Email onboarding
                      </a>
                    ) : null}
        ...
        <div className="list">
          <div className="card compact" style={{ background: "#fbfcfd" }}>
            <div className="row" style={{ alignItems: "flex-start", gap: 12 }}>
              ...
              <div className="toolbar" style={{ justifyContent: "flex-end" }}>
                <button className="primary" type="button" disabled={saving || !record.phone} onClick={onQueueOnboarding}>
