...
function onboardingEmailHref(email: unknown, name: unknown, body: unknown) {
  const recipient = text(email);
  if (!recipient) return "";
  return `mailto:${recipient}?subject=${encodeURIComponent(onboardingEmailSubject(name))}&body=${encodeURIComponent(text(body))}`;
}
...
export default function OnboardingCenterPage() {
  ...
  async function emailCoordinatorOnboarding(row: CoordinatorOnboardingRow) {
    if (coordinatorReadOnly) {
      setMessage("View as user mode is read-only. Exit the preview to email onboarding.");
      return;
    }
    if (!row.email) {
    ...
    setCoordinatorBusyId(`email-${row.id}`);
    setMessage("");
    try {
      const response = await fetch("/api/onboarding", {
        method: "POST",
      ...
      const result = (await response.json()) as { ok?: boolean; link?: string; message_body?: string; message?: string };
      if (!response.ok || !result.ok) throw new Error(result.message || "Unable to prepare onboarding email.");
      const body = result.message_body || onboardingInvitationMessage(row.name, result.link || "", access.current_user_name);
      const href = onboardingEmailHref(row.email, row.name, body);
      if (!href) throw new Error("Unable to prepare onboarding email.");
      setMessage(`Onboarding email prepared for ${row.email}.`);
      window.location.assign(href);
      void loadCoordinatorDashboard();
      void loadRows();
    } catch (error) {
    ...
    <div className="grid" style={{ gap: 16 }}>
              ...
              <article key={row.id} className="card compact" style={{ boxShadow: "none" }}>
                <div className="row" style={{ alignItems: "flex-start", gap: 12 }}>
                  ...
                  <div className="toolbar" style={{ justifyContent: "flex-end" }}>
                    ...
                    >
                      {coordinatorBusyId === row.id ? "Queueing..." : row.onboarding_complete ? "Send onboarding again" : row.onboarding_request_sent_at ? "Send reminder with link" : "Send onboarding with link"}
                    </button>
                    <button
                      className="ghost"
                      type="button"
                      disabled={coordinatorReadOnly || coordinatorBusyId === `email-${row.id}` || !row.email}
                      onClick={() => void emailCoordinatorOnboarding(row)}
                    >
                      {coordinatorBusyId === `email-${row.id}` ? "Preparing email..." : row.email ? "Email onboarding" : "Email needed"}
                    </button>
                  </div>
                </div>
              </article>
      ...
      <section className="card">
        ...
        <div className="toolbar" style={{ marginTop: 12 }}>
          ...
          {lastInviteMessage && lastInviteEmail ? (
            <a className="ghost" href={onboardingEmailHref(lastInviteEmail, lastInviteName || "Crew", lastInviteMessage)}>
              Send onboarding by email
            </a>
          ) : null}
          <span className="muted small">Matches by email/phone, reuses an active link when available, and otherwise creates a hidden pending contact. Full-onboarding reminders repeat every two weeks and stop automatically after submission. Text-message sending must be opened from the app on the sender's cell phone. Email opens your email app with the recipient, subject, secure link, and message already filled in.</span>
        </div>
            ...
            <section key={row.id} className="card">
              <div className="row" style={{ alignItems: "flex-start", gap: 14 }}>
                ...
                <div className="toolbar">
                  ...
                  {text(crew.email || payload.invite_email) && row.link ? (
                    <a
                      className="ghost"
                      href={onboardingEmailHref(crew.email || payload.invite_email, displayName, onboardingInvitationMessage(displayName, row.link, access.current_user_name, row.request_type))}
                    >
                      Email onboarding
                    </a>
                  ) : null}
          ...
          <section key={row.id} className="card">
            <div className="row" style={{ alignItems: "flex-start", gap: 14 }}>
              ...
              <div className="toolbar">
                ...
                {!isApproved && text(crew.email || payload.invite_email) && row.link ? (
                  <a
                    className="ghost"
                    href={onboardingEmailHref(crew.email || payload.invite_email, text(payload.legal_name) || text(crew.name) || text(payload.invite_name), onboardingInvitationMessage(text(payload.legal_name) || text(crew.name) || text(payload.invite_name), row.link, access.current_user_name, row.request_type))}
                  >
                    Email onboarding
                  </a>
                ) : null}
