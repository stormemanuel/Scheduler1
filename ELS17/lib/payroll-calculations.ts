...
export function estimateAssignmentPay(options: {
  ...
  const halfDayRate = masterHalf ?? fallbackHalf ?? (crewFullDay ? Math.round((crewFullDay / 2) * 100) / 100 : null);
  ...
  return estimateByBlock({ duration, fullDayRate, halfDayRate, source });
}

function estimateByBlock(options: { duration: number | null; fullDayRate: number; halfDayRate: number | null; source: string }): AssignmentPayEstimate {
...
export function estimateAssignmentRevenue(call: SubCallRecord): AssignmentPayEstimate {
  const duration = callDurationHours(call);
  const fallback = fallbackFor(call.role_name, clientBillingRates);
  const fullDayRate = rateNumber(fallback?.full_day) ?? 0;
  const halfDayRate = rateNumber(fallback?.half_day);
  return estimateByBlock({ duration, fullDayRate, halfDayRate, source: "Client rate sheet" });
}
