...
export async function updateUserAccessAction(formData: FormData) {
  ...
  if (role === "coordinator") {
    ...
    if (compensationError) {
      compensationMessage = /coordinator_compensation_settings|schema cache|relation/i.test(compensationError.message || "")
        ? " Coordinator payout rates were not saved because the ELS295 compensation SQL has not been run."
        : ` Coordinator payout rate issue: ${compensationError.message}`;
    } else {
