...
export function estimateAssignmentPay(options: {
  ...
  // Payroll must use the crew-pay rate tables first. Older crew profile rates may
  // still contain client billing/day-rate values, so they are only a last-resort
  // fallback when no Default/city master rate exists for the role.
  const fullDayRate = masterFull ?? crewFullDay ?? fallback?.full_day ?? 0;
  const halfDayRate = masterHalf ?? (crewFullDay ? Math.round((crewFullDay / 2) * 100) / 100 : null) ?? fallback?.half_day ?? null;
