  ...
  async function importContactsCsv(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (const [index, values] of rows.slice(1).entries()) {
        ...
        const rate = Number(valueFromRow(row, ["rate", "day rate", "pay rate"]).replace(/[^0-9.]/g, "") || 0);
        ...
        <div className="crew-bigin-shell" style={{ display: "grid", gridTemplateColumns: "260px minmax(360px, 1fr) minmax(360px, 0.95fr)", minHeight: 720 }}>
          ...
          <section style={{ borderRight: "1px solid var(--line)", padding: 16 }}>
            ...
            <div style={{ border: "1px solid var(--line)", borderRadius: 14, overflow: "hidden" }}>
              <div className="row small muted" style={{ padding: "10px 12px", background: "#f8fafc", borderBottom: "1px solid var(--line)", alignItems: "center" }}>
                <span style={{ width: 28 }} />
              ...
              {visibleCrew.length === 0 ? (
                ...
                return (
                  ...
                  >
                    ...
                    <span className="small" style={{ width: 120, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{positionSummary(record)}</span>
                    <span className="small muted" style={{ width: 90, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{record.group_name || "Ungrouped"}</span>
                  </button>
                );
              })}
            </div>
