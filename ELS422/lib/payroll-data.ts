...
export async function getPayrollSummaryData(requestedYear?: number): Promise<PayrollPageData> {
  ...
  try {
    ...
    const [
      ...
      showIds.length
        ? readClient.from("show_payroll").select("id, show_id, crew_id, role_name, paid, payment_status, payout_override, notes, scheduled_for").in("show_id", showIds).limit(10000)
        : Promise.resolve(emptyResult),
      showIds.length
        ? readClient.from("show_financials").select("id, show_id, estimated_revenue_override, expenses, notes, tax_reserve_done, tax_reserve_done_at, consecrated_hands_done, consecrated_hands_done_at, created_at, updated_at").in("show_id", showIds).order("updated_at", { ascending: false }).limit(5000)
        : Promise.resolve(emptyResult),
      showIds.length
    ...
    if (coordinatorCompensationRes.error && !coordinatorCompensationMissing) optionalWarnings.push(`coordinator compensation unavailable: ${supabaseErrorMessage(coordinatorCompensationRes.error)}`);
    ...
    if (!financialsRes.error) {
      // The query is newest-first. Keep only the newest row per show so duplicate legacy rows
      // cannot make a persisted Consecrated Hands / tax-reserve checkoff appear to revert.
      for (const row of (financialsRes.data ?? []) as ShowFinancialRecord[]) {
        const showId = safeText(row.show_id);
        if (showId && !financialByShow.has(showId)) financialByShow.set(showId, row);
      }
    }
