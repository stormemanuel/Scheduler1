...
export default function PayrollClient({ initialRows, initialEventSummaries, availableYears, initialYear, initialSummaryLoaded, initialError, initialDiagnostics = null }: Props) {
  ...
  async function selectPayrollYear(nextYear: number, force = false) {
    ...
    try {
      ...
      for (let attempt = 0; attempt < 2; attempt += 1) {
        try {
          response = await fetchWithTimeout(`/api/payroll?year=${encodeURIComponent(String(nextYear))}`, 9000);
          break;
        } catch (error) {
      ...
      if (!response) {
        throw new Error(timeoutError ? "Payroll totals are taking too long to load. Retry Totals to try again." : `Unable to load ${nextYear} payroll.`);
      }
  ...
  async function openTaxCenterForYear() {
    ...
    try {
      const response = await fetchWithTimeout(`/api/payroll?mode=tax&year=${encodeURIComponent(String(year))}`, 15000);
      const result = (await response.json()) as {
        crewRows?: PayrollCrewShowRow[];
        eventSummaries?: PayrollEventSummary[];
        loadedYear?: number;
      ...
      if (!response.ok) throw new Error(result.message || result.error || `Unable to load ${year} Tax Center data.`);
      ...
      const nextMessage = error instanceof DOMException && error.name === "AbortError"
        ? "Tax Center data is taking too long to load. Try opening it again."
        : error instanceof Error ? error.message : `Unable to load ${year} Tax Center data.`;
      setMessage(nextMessage);
    } finally {
...
(): Array<Array<string | number | null | undefined>> {
  ...
  return (
    <>
      ...
      {message ? (
        <div className={message.toLowerCase().includes("failed") || message.toLowerCase().includes("run supabase") || message.toLowerCase().includes("safe mode") || message.toLowerCase().includes("could not") || message.toLowerCase().includes("unable") ? "error" : "success"}>
          <span>{message}</span>
