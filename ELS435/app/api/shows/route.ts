...
import { getEventsCrewLookupData, getEventsPageData } from "@/lib/events-data";
...
export async function GET(request: Request) {
  ...
  if (action === "event-detail") {
    ...
    if (!showId) return NextResponse.json({ ok: false, message: "Show is required." }, { status: 400 });
    const data = await getEventsPageData({ selectedShowId: showId, showLimit: 1 });
    if (data.error) return NextResponse.json({ ok: false, message: data.error }, { status: 400 });

    const shows = data.shows.filter((show) => show.id === showId);
    if (!shows.length) return NextResponse.json({ ok: false, message: "Show not found or not available to this user." }, { status: 404 });
    const showIds = new Set(shows.map((show) => show.id));
    const laborDays = data.laborDays.filter((day) => showIds.has(day.show_id));
    const laborDayIds = new Set(laborDays.map((day) => day.id));
    const subCalls = data.subCalls.filter((call) => laborDayIds.has(call.labor_day_id));
    const subCallIds = new Set(subCalls.map((call) => call.id));
    const assignments = data.assignments.filter((assignment) => subCallIds.has(assignment.sub_call_id));
    ...
    return NextResponse.json({
      ...
      coordinatorScheduleStatuses: data.coordinatorScheduleStatuses.filter((status) => showIds.has(status.show_id)),
      crewRecords,
    });
  }

  if (action !== "past-events") {
