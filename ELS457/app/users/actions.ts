...
async function saveCoordinatorCompensationSettingsForLinkedUsers(
  ...
  const targetIds = uniqueCoordinatorCompensationTargetIds(formData, primaryUserId);
  let firstFailure = "";
  let savedCount = 0;
...
async function saveCoordinatorCompensationMetadataForLinkedUsers(
  ...
  let saved = 0;
  let firstError = "";
  for (const targetId of uniqueCoordinatorCompensationTargetIds(formData, primaryUserId)) {
...
export async function updateUserAccessAction(formData: FormData) {
  ...
  const messagingMetadataResult = await saveUserMessagingMetadata(admin, userId, messagingMode, deviceType);
