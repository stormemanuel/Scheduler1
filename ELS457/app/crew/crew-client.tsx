...
async function queueOnboardingRequestForShortcut(record: CrewRecord, requestType: OnboardingRequestType = "full_onboarding") {
  ...
  return data as { link?: string; message?: string; request_type?: string; delivery_mode?: string; queued?: QueuedOnboardingText | null };
}
...
function smsHrefWithBody(phone: unknown, body: unknown) {
  ...
  return `smsto:${normalized}?body=${encodeURIComponent(String(body || ""))}`;
}

type QueuedOnboardingText = { id?: string; phone?: string | null; body?: string | null };

function maybeOpenAndroidOnboardingText(result: { delivery_mode?: string; queued?: QueuedOnboardingText | null }) {
  if (result.delivery_mode !== "android_messages" || !result.queued) return false;
  const href = smsHrefWithBody(result.queued.phone, result.queued.body);
