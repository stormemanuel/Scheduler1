...
function ratingSummaryText(summary: CrewRatingSummary | null | undefined) {
  ...
  return `${starDisplay(summary.average)} ${summary.average.toFixed(1)}/5 (${summary.count})`;
}
...
function poolNamesForRecord(record: CrewRecord) {
  return [record.city_name, ...(record.additional_city_pool_names ?? [])].filter((name, index, list) => Boolean(name) && list.indexOf(name) === index).join(", ") || "Unassigned";
}
...
export default function CrewClient({ cityPools: initialCityPools, crewGroups: initialGroups, initialCrew, masterRates, initialRatings }: CrewClientProps) {
  ...
  const ratingSummaryByCrew = useMemo(() => {
    const map = new Map<string, { total: number; count: number; lastRatingAt: string }>();
    for (const rating of ratings) {
      if (!rating.crew_id || !Number(rating.rating)) continue;
      const existing = map.get(rating.crew_id) ?? { total: 0, count: 0, lastRatingAt: "" };
      existing.total += Number(rating.rating || 0);
      existing.count += 1;
    ...
    for (const [crewId, row] of map.entries()) {
      summaries.set(crewId, { average: row.total / Math.max(1, row.count), count: row.count, lastRatingAt: row.lastRatingAt });
    }
    return summaries;
  }, [ratings]);
  ...
  const targetGroupsForBulkCity = useMemo(() => {
    ...
    crewRecords.filter((record) => crewBelongsToCityPool(record, cityId)).forEach((record) => names.add(record.group_name || "Ungrouped"));
  ...
  async function deleteSelectedCityPool() {
    if (!selectedCity || selectedCityId === UNASSIGNED_CITY) {
      setMessageKind("error");
      setMessage("Select a city pool to delete.");
      return;
    }
    ...
    const warning = assignedCount > 0
      ? `Delete city pool ${selectedCity.name}? ${assignedCount} contact${assignedCount === 1 ? "" : "s"} in this pool will move to Unassigned.`
      : `Delete empty city pool ${selectedCity.name}?`;
    ...
    try {
      ...
      setMessage(assignedCount > 0
        ? `City pool deleted. ${assignedCount} contact${assignedCount === 1 ? "" : "s"} moved to Unassigned.`
        : "City pool deleted.");
      router.refresh();
    } catch (error) {
      setMessageKind("error");
  ...
  function buildCsvImportDraft(row: Record<string, string>): CrewDraft | null {
    ...
    const tier = valueFromRow(row, ["tier", "group", "crew group"]);
    ...
    const positions = positionsFromSkillsets([skills, capabilities].filter(Boolean).join(", "));

    return {
      ...blankDraft(cityPools, city?.id, groupName),
      name: name.trim(),
      description: "Imported from crew CSV.",
  ...
  async function importContactsCsv(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      for (const [index, values] of rows.slice(1).entries()) {
        ...
        const nextDraft = buildCsvImportDraft(row);
        if (!nextDraft) {
        ...
        importedOrMerged.push(recordFromDraft({ ...nextDraft, id: result.id }, cityPools));
      }
      ...
      if (importedOrMerged.length) {
        setCrewRecords((current) => {
          ...
          for (const importedRecord of importedOrMerged) {
            ...
            if (existingIndex >= 0) next[existingIndex] = importedRecord;
            else next.unshift(importedRecord);
          }
          return next;
        });
      ...
      setMessage(errors.length
        ? `Imported/merged ${importedOrMerged.length} contacts (${createdCount} new, ${mergedCount} merged). ${errors.slice(0, 3).join(" ")}${errors.length > 3 ? " …" : ""}`
        : `Imported/merged ${importedOrMerged.length} contacts (${createdCount} new, ${mergedCount} merged). Duplicates were matched by email or phone and updated instead of copied.`);
      router.refresh();
    } catch (error) {
      setMessageKind("error");
        ...
        <div className="crew-bigin-shell" style={{ display: "grid", gridTemplateColumns: "260px minmax(360px, 1fr)", minHeight: 720 }}>
          <aside style={{ borderRight: "1px solid var(--line)", padding: 16, background: "#fbfcfd" }}>
            ...
            <div style={{ borderTop: "1px solid var(--line)", paddingTop: 14, marginTop: 14 }}>
              ...
              <button className="ghost" type="button" onClick={ensureTravelTechsPool} disabled={saving} style={{ marginTop: 10, width: "100%" }}>Create/use Travel Techs pool</button>
              <button className="ghost" type="button" onClick={renameSelectedCityPool} disabled={saving || !selectedCity || selectedCityId === UNASSIGNED_CITY} style={{ marginTop: 10, width: "100%" }}>Rename selected city pool</button>
              <button
                className="ghost danger"
                type="button"
                onClick={deleteSelectedCityPool}
                disabled={saving || !selectedCity || selectedCityId === UNASSIGNED_CITY}
                style={{ marginTop: 10, width: "100%" }}
                title="Deletes the selected city pool. Contacts in that pool move to Unassigned."
              >
                Delete selected city pool
              </button>
              <div className="muted small" style={{ marginTop: 8 }}>Contacts in a deleted pool move to Unassigned.</div>
            </div>
...
function ContactDetail({ record, ratingSummary, cityPools, targetGroups, bulkCityId, bulkGroupName, detailTab, onTab, onEdit, onDelete, onMove, onBulkCityChange, onBulkGroupChange, saving }: ContactDetailProps) {
    ...
    <div>
        ...
        <div className="row" style={{ alignItems: "flex-start" }}>
          <div className="row" style={{ gap: 12, alignItems: "center", justifyContent: "flex-start" }}>
            ...
            <div>
              ...
              <div className="small"><strong>Rating:</strong> {ratingSummaryText(ratingSummary)}</div>
            </div>
          </div>
          <div className="toolbar" style={{ justifyContent: "flex-end" }}>
      ...
      {detailTab === "info" ? (
        <div className="list">
          ...
          <InfoRow label="Show rating average" value={ratingSummaryText(ratingSummary)} />
          <InfoRow label="Rating count" value={ratingSummary?.count ? `${ratingSummary.count} saved show rating${ratingSummary.count === 1 ? "" : "s"}` : "No saved show ratings"} />
          <InfoRow label="Primary city pool" value={record.city_name || "Unassigned"} />
          <InfoRow label="Additional pools" value={(record.additional_city_pool_names ?? []).length ? record.additional_city_pool_names.join(", ") : "None"} />
      ...
      <div className="card compact" style={{ background: "#fbfcfd" }}>
        <div className="row" style={{ alignItems: "center", marginBottom: 8 }}>
          <div>
            ...
            <div className="muted small">Use this for nearby cities or travel techs. The contact stays one record and can appear in multiple pools.</div>
          </div>
        </div>
