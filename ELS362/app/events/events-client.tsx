    ...
    return [...byCrew.entries()]
      .map(([crewId, item]) => {
        ...
        const scheduleLines = sortedCalls.map(({ show, day, call, assignment }) => {
          ...
          const detailText = [
            `Area: ${boothLabel(call.area) || "Area TBD"}`,
          ].filter(Boolean).join(" – ");
          return `${formatMessageDate(day.labor_date)}${showLabel} – ${formatAssignmentMessageTimeRange(assignment, call)} – ${detailText} – ${blockText} – ${rateText}`;
        });
