...
function EventsClientInner({
  ...
  const availabilityCandidates = useMemo(() => {
    ...
    const selectedRole = safeText(availabilityCandidateRole);
  ...
  }, [availabilityCandidateSearch, availabilityCandidateCity, availabilityCandidateRole, availabilityIncludeAssigned, availabilityAssignedCrewIds, availabilityRatingStats, crewRecords]);
  ...
  async function queueSelectedAvailabilityMessages() {
    ...
    const data = await request("/api/text-automation", "POST", {
      ...
      role_name: availabilityCandidateRole,
...
<body>
  ...
  async function saveCall() {
    ...
    const invalidExtraPayload = extraPositionPayloads.find((payload) => !payload.area || !payload.role_name || !payload.start_time || !payload.end_time);
    if (invalidExtraPayload) {
      setMsg({ kind: "error", text: "Area, position, start time, and end time are required for every extra position row." });
      return;
    }
    ...
    if (editingCallId) {
      ...
      const extraDayIds = editCallApplyAllDays
      ...
      for (const dayId of extraDayIds) {
        for (const extraPayload of extraPositionPayloads) {
      ...
      setMsg({ kind: "success", text: extraPositionPayloads.length ? `${updatedText}; ${createdText}.${skippedText}${addDaysText}` : `${updatedText}.${addDaysText}` });
    } else {
      const items = targetDayIds.flatMap((dayId) => [
        { ...mainPayloadForNewDay(dayId), labor_day_id: dayId },
        ...extraPositionPayloads.map((payload) => ({ ...payload, labor_day_id: dayId })),
      ]);
        ...
        <div className="card compact">
          ...
          <div className="card compact accent-card" style={{ marginTop: 14 }}>
            ...
            <label className="field" style={{ marginTop: 12 }}>
              ...
              <span className="small muted">Message dates: {formatMessageDate(selectedShow.show_start)} through {formatMessageDate(selectedShow.show_end)}. Placeholders: {"{first_name}"}, {"{show_name}"}, {"{venue}"}, {"{show_start}"}, {"{show_end}"}, {"{position}"}, {"{city}"}, {"{coordinator_name}"}.</span>
            </label>
            <div className="toolbar" style={{ marginTop: 10 }}>
            ...
            <div className="card compact" style={{ marginTop: 12, background: "#fbfcfd", boxShadow: "none", borderLeft: editingCallId ? "4px solid #d4a62a" : undefined }}>
              ...
              {editingCallId ? (
                <div className="card compact sub-call-multi-day" style={{ marginTop: 10, boxShadow: "none" }}>
                  <div className="row" style={{ alignItems: "flex-start" }}>
                    <div>
                      <strong>Add queued position rows to multiple labor days</strong>
                      <div className="small muted">The main position above is edited only on its current day. Any new position rows you add below will be created on every selected labor day using the same booth/area details.</div>
                    </div>
                    <div className="toolbar">
                      <button type="button" className="ghost" onClick={() => setExtraPositionTargetDayIds(visibleLaborDays.map((day) => day.id))}>Select all</button>
                      <button type="button" className="ghost" onClick={() => setExtraPositionTargetDayIds(editingDayTargetId ? [editingDayTargetId] : [])}>Current day only</button>
                    </div>
                  </div>
                  <div className="grid grid-3" style={{ marginTop: 10 }}>
                    {visibleLaborDays.map((day) => (
                      <label key={day.id} className="checkline">
                        <input
                          type="checkbox"
                          checked={extraPositionTargetDayIds.includes(day.id)}
                          onChange={(event) => {
                            const checked = event.currentTarget.checked;
                            setExtraPositionTargetDayIds((current) => checked ? [...new Set([...current, day.id])] : current.filter((id) => id !== day.id));
                          }}
                        />
                        <span>{day.labor_date}{day.label ? ` · ${day.label}` : ""}</span>
                      </label>
                      ...
                      return (
                        <div key={row.id} className="card compact" style={{ boxShadow: "none" }}>
                          ...
                          <div className="grid" style={{ gridTemplateColumns: "repeat(5, minmax(0, 1fr))", gap: 12 }}>
                            <label className="field"><span>Start</span><input type="time" value={row.start_time} placeholder="8:00 AM" onChange={(event) => updateMultiPositionRow(row.id, { start_time: event.target.value })} /></label>
                            <label className="field"><span>End</span><input type="time" value={row.end_time} placeholder="6:00 PM" onChange={(event) => updateMultiPositionRow(row.id, { end_time: event.target.value })} /></label>
                          ...
                          <div className="toolbar" style={{ marginTop: 8 }}>
                            <button type="button" className="ghost" onClick={() => updateMultiPositionRow(row.id, { start_time: STANDARD_SUB_CALL_START_TIME, end_time: STANDARD_SUB_CALL_END_TIME })}>Use 8:00 AM–6:00 PM</button>
                          </div>
