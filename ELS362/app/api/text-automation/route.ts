...
function subCallScheduleDetail(call: { area?: string | null; location?: string | null; po_number?: string | null; notes?: string | null }, fallbackLocation = "") {
  const location = safeText(call.location) || safeText(fallbackLocation);
  return [
    `Area: ${safeText(call.area) || "Area TBD"}`,
    location ? `Location: ${location}` : "",
    safeText(call.po_number) ? `PO: ${safeText(call.po_number)}` : "",
  ].filter(Boolean).join(" - ");
}
...
function nextScheduleByCrew(show: ShowRow, days: LaborDayRow[], calls: SubCallRow[], assignments: AssignmentRow[], crewRows: CrewRow[], masterRates: MasterRateRow[], settings: AutomationSettings, senderName = "") {
  ...
  for (const assignment of [...assignments].sort((a, b) => Number(a.sort_order || 0) - Number(b.sort_order || 0))) {
    ...
    current.lines.push({
      ...
      text: `${formatDate(day.labor_date)} - ${assignmentTimeRange(assignment, call)} - ${subCallScheduleDetail(call, show.event_location || show.venue || "")} - ${blockLabel} - ${rateText}`,
    });
  ...
  for (const [crewId, item] of baseByCrew.entries()) {
    ...
    const pushPlan = (args: { type: string; enabled: boolean; dueIso: string; messageType: string; reminderKey: string; body: string; row: typeof firstRow }) => {
      ...
      plan.push({
        ...
        assignment_label: `${formatDate(args.row.day.labor_date)} ${assignmentTimeRange(args.row.assignment, args.row.call)} · ${args.row.call.role_name || "Crew"} · ${subCallScheduleDetail(args.row.call, bundle.show.event_location || bundle.show.venue || "")}`,
