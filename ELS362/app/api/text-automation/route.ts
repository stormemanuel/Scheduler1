...
function subCallScheduleDetail(call: { area?: string | null; location?: string | null; po_number?: string | null; notes?: string | null }) {
  return [
    `Area: ${safeText(call.area) || "Area TBD"}`,
  ].filter(Boolean).join(" - ");
}
...
function nextScheduleByCrew(show: ShowRow, days: LaborDayRow[], calls: SubCallRow[], assignments: AssignmentRow[], crewRows: CrewRow[], masterRates: MasterRateRow[], settings: AutomationSettings, senderName = "") {
  ...
  const result = new Map<string, { crew: CrewRow; lines: Array<{ sortKey: string; text: string }>; meetUps: string[]; rates: string[]; poNumbers: string[]; firstCall: { day: LaborDayRow; call: SubCallRow; assignment: AssignmentRow } | null; values: Record<string, string> }>();
  ...
  for (const assignment of [...assignments].sort((a, b) => Number(a.sort_order || 0) - Number(b.sort_order || 0))) {
    ...
    const current = result.get(crew.id) || { crew, lines: [], meetUps: [], rates: [], poNumbers: [], firstCall: null, values: {} as Record<string, string> };
    ...
    current.lines.push({
      ...
      text: `${formatDate(day.labor_date)} - ${assignmentTimeRange(assignment, call)} - ${subCallScheduleDetail(call)} - ${blockLabel} - ${rateText}`,
    });
  ...
  for (const item of result.values()) {
    ...
    const crewLeadName = safeText(meta.crew_lead_name) || safeText(leadCrew?.name);
    const crewLeadPhone = safeText(meta.crew_lead_phone) || safeText(leadCrew?.phone);
  ...
  for (const [crewId, item] of baseByCrew.entries()) {
    ...
    const pushPlan = (args: { type: string; enabled: boolean; dueIso: string; messageType: string; reminderKey: string; body: string; row: typeof firstRow }) => {
      ...
      plan.push({
        ...
        assignment_label: `${formatDate(args.row.day.labor_date)} ${assignmentTimeRange(args.row.assignment, args.row.call)} · ${args.row.call.role_name || "Crew"} · ${subCallScheduleDetail(args.row.call)}`,
