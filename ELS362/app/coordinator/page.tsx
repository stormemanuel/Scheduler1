...
import type { ShowRecord, SubCallRecord, AssignmentRecord, LaborDayRecord, TextMessageQueueRecord } from "@/lib/events-types";
...
function formatCommunicationTime(value: string | null | undefined) {
  const clean = safeText(value);
  if (!clean) return "—";
  const parsed = new Date(clean);
  return Number.isNaN(parsed.getTime()) ? clean : parsed.toLocaleString("en-US");
}
...
export default async function CoordinatorPage({ searchParams }: { searchParams?: Promise<CoordinatorSearchParams> }) {
  ...
  if (admin) {
    const compensationRes = await admin
  ...
  const showById = new Map<string, ShowRecord>(allShows.map((show): [string, ShowRecord] => [show.id, show]));
  const coordinatorUsers = appUsers.filter((user) => normalizeRole(user.role) === "coordinator");
  const coordinatorUserIds = new Set(coordinatorUsers.map((user) => user.id));
  const coordinatorEmails = new Set(coordinatorUsers.map((user) => safeText(user.email).toLowerCase()).filter(Boolean));
  const coordinatorMessages = ((data.textMessageQueue || []) as TextMessageQueueRecord[])
    .filter((row) => Boolean(
      (row.queued_by_user_id && coordinatorUserIds.has(row.queued_by_user_id))
      || coordinatorEmails.has(safeText(row.queued_by_email).toLowerCase())
      || coordinatorEmails.has(safeText(row.queued_by_name).toLowerCase()),
    ))
    .sort((a, b) => safeText(b.sent_at || b.created_at).localeCompare(safeText(a.sent_at || a.created_at)))
    .slice(0, 75);
  ...
  const rows = shows.map((show: ShowRecord) => {
    ...
    const coordinatorBuckets = [...coordinatorBucketMap.values()].map((bucket) => ({
      ...bucket,
      fee: coordinatorFee(bucket.fullDayTechDays, bucket.halfDayTechs, coordinatorCompensationByUser.get(bucket.coordinatorId)),
    }));
    ...
    <div className="grid" style={{ gap: 16 }}>
      ...
      {canViewInternalCoordinatorPay ? (
        <section className="card">
          <div className="row" style={{ alignItems: "flex-start", marginBottom: 12 }}>
            <div>
              <h3 style={{ marginBottom: 4 }}>Coordinator Messages and Reminders</h3>
              <p className="muted" style={{ margin: 0 }}>
                Owner/admin history for show texts queued by coordinator users. Apple Shortcut sends are marked automatically; copied or manually sent messages can be marked sent from the event Text Automation section.
              </p>
            </div>
            <span className="badge">{coordinatorMessages.length} recent</span>
          </div>
          {coordinatorMessages.length ? (
            <div className="list">
              {coordinatorMessages.map((message) => {
                const sender = message.queued_by_user_id ? userById.get(message.queued_by_user_id) : null;
                const show = showById.get(message.show_id);
                return (
                  <div key={message.id} className="card compact" style={{ boxShadow: "none" }}>
                    <div className="row" style={{ alignItems: "flex-start" }}>
                      <div>
                        <strong>{show?.name || "Unknown show"} · {message.crew_name || "Crew member"}</strong>
                        <div className="small muted">
                          Sent by {sender?.full_name || message.queued_by_name || sender?.email || message.queued_by_email || "Coordinator"} · {message.message_type} / {message.reminder_key}
                        </div>
                        <div className="small muted">
                          Queued {formatCommunicationTime(message.created_at)} · {message.sent_at ? `Sent ${formatCommunicationTime(message.sent_at)}` : `Status ${message.status}`}
                        </div>
                      </div>
                      <span className="badge">{message.status}</span>
                    </div>
                    <details style={{ marginTop: 8 }}>
                      <summary className="small" style={{ cursor: "pointer", fontWeight: 700 }}>View message</summary>
                      <pre style={{ whiteSpace: "pre-wrap", overflowWrap: "anywhere", marginBottom: 0 }}>{message.body}</pre>
                    </details>
                  </div>
                );
              })}
            ...
            <p className="muted" style={{ margin: 0 }}>No coordinator-queued show messages have been recorded yet.</p>
          )}
        </section>
      ) : null}
