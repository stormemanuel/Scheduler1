...
export function OnboardingReviewNavLink({ href, label }: OnboardingNavLinkProps) {
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      if (document.visibilityState !== "hidden") void loadReviewCount();
    }, 300000);
...
export function CoordinatorNotificationNavLink({ href, label }: OnboardingNavLinkProps) {
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      if (document.visibilityState !== "hidden") void loadUnreadCount();
    }, 300000);
...
export function AppBadgeSync({ role }: { role: string }) {
  useEffect(() => {
    ...
    void loadCount();
    const intervalId = window.setInterval(() => {
      if (document.visibilityState !== "hidden") void loadCount();
    }, 300000);
...
export function CoordinatorMessagesPanel({ role }: { role: string }) {
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      if (document.visibilityState !== "hidden") void loadRows();
    }, 300000);
    const directIntervalId = window.setInterval(() => {
      if (document.visibilityState !== "hidden") void loadDirectMessages();
    }, 300000);
