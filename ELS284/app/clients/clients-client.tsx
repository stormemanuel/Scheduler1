  ...
  return [...roleNames.entries()]
    ...
    .map(([roleKey, roleName]) => {
      ...
      return {
        ...
        half_day: rateValue(override?.half_day),
        ...
        inherited_half_day: inherited?.half_day ?? null,
...
export default function ClientsClient({
  ...
  function applyClientRateBulkAdjust() {
    ...
    const nextRows = clientRateDraftRows.map((row) => {
      ...
      const halfDay = adjustedClientPrice(row.half_day, row.inherited_half_day, delta);
      if (fullDay !== row.full_day) adjustedFields += 1;
      if (halfDay !== row.half_day) adjustedFields += 1;
      return { ...row, full_day: fullDay, half_day: halfDay };
    });

    if (!adjustedFields) {
      setMsg({ kind: "error", text: "There are no full-day or half-day prices available to adjust in this rate card." });
      return;
    }
  ...
  async function saveClientCityRates() {
    ...
    const rows = clientRateDraftRows
      ...
      .map((row) => ({
        ...
        half_day: row.half_day.trim() || null,
    ...
    const savedRows = (Array.isArray(data.rows) ? data.rows : []).map((row: ClientCityRateOverrideRecord) => ({
      ...
      half_day: row.half_day === null || row.half_day === undefined ? null : Number(row.half_day),
                ...
                <table style={{ width: "100%", minWidth: 900, borderCollapse: "collapse" }}>
                  <thead>
                    <tr>
                      ...
                      <th style={{ textAlign: "left", padding: "8px 6px", borderBottom: "1px solid var(--line)" }}>Half day override</th>
                      ...
                      <tr key={row.key}>
                        ...
                        <td style={{ padding: "8px 6px", verticalAlign: "top" }}>
                          <input
                            ...
                            step="0.01"
                            value={row.half_day}
                            onChange={(event) => updateClientRateRow(row.key, { half_day: event.target.value })}
                            placeholder={displayMoneyRate(row.inherited_half_day)}
                          />
                          <div className="small muted" style={{ marginTop: 4 }}>Inherited: {displayMoneyRate(row.inherited_half_day)}</div>
                        </td>
                        <td style={{ padding: "8px 6px", verticalAlign: "top" }}>
                          <input
                            type="number"
