...
async function bulkSaveRateTable(formData: FormData, tableName: "master_rates" | "client_rates", successMessage: string) {
  ...
  if (upserts.length) {
    for (const row of upserts) {
      ...
      if (row.id) {
        const { error } = await admin.from(tableName).update(saveRow).eq("id", row.id);
        if (error) return { ok: false, message: error.message };
        continue;
      }
      ...
      if (existingRow?.id) {
        const { error } = await admin.from(tableName).update(saveRow).eq("id", existingRow.id);
        if (error) return { ok: false, message: error.message };
      } else {
        const { error } = await admin.from(tableName).insert({ ...saveRow, id: randomUUID() });
        if (error) return { ok: false, message: error.message };
      }
    }
  }
