...
function EventsClientInner({
  ...
  function buildSignInSheetDocument(): ExportDocument | null {
    if (!selectedShow) return null;
    const sections = visibleLaborDays.map((day): ExportDocument["sections"][number] => {
      const dayCalls = displayCalls
        .filter((item) => item.day.id === day.id)
        .sort((a, b) => boothLabel(a.call.area).localeCompare(boothLabel(b.call.area)) || a.call.start_time.localeCompare(b.call.start_time) || safeText(a.call.role_name).localeCompare(safeText(b.call.role_name)));
      const rows = dayCalls.flatMap((item) => {
        const callAssignments = getCallAssignments(item.call.id);
        const base = {
          area: boothLabel(item.call.area),
          time: formatTimeRange(item.call),
          role: item.call.role_name || "Crew",
        };
        if (callAssignments.length) {
          return callAssignments.map(({ assignment, crew }) => [
            base.area,
        ...
        const blankCount = Math.max(1, Number(item.call.crew_needed || 1));
      ...
      return {
        heading: `${formatFullDate(day.labor_date)}${day.label ? ` - ${day.label}` : ""}`,
        subheading: dayCalls.length ? `${dayCalls.length} sub-call${dayCalls.length === 1 ? "" : "s"}` : "No sub-calls for this day.",
        accentColor: "#17313b",
        columns: ["Area / Booth", "Call Time", "Position", "Crew Name", "Phone", "Sign In", "Sign Out", "Notes"],
    ...
    return {
      layout: "wide-table",
      title: "ELS Sign-In / Sign-Out Sheet",
      subtitle: `${selectedShow.name} • ${selectedShow.show_start} - ${selectedShow.show_end}`,
      meta: [
        ["Event", selectedShow.name || ""],
        ...
        ["Location", eventLocationLabel(selectedShow)],
        ["Dates", `${selectedShow.show_start} - ${selectedShow.show_end}`],
        ["ELS Coordinator", assignedCoordinatorUser?.full_name || assignedCoordinatorUser?.email || currentUserName || currentUserEmail || ""],
        ["Crew Lead", selectedShowMessageMeta.crew_lead_name ? [selectedShowMessageMeta.crew_lead_name, selectedShowMessageMeta.crew_lead_phone ? formatPhone(selectedShowMessageMeta.crew_lead_phone) : ""].filter(Boolean).join(" • ") : ""],
      ],
      sections,
    };
  }

  async function exportSignInSheetPdf() {
    if (!selectedShow) { setMsg({ kind: "error", text: "Select a show first." }); return; }
    await runDocumentExport(
      "Sign-in sheet PDF",
      buildSignInSheetDocument,
      exportDocumentPdfQueued,
      `${selectedShow.name}_sign_in_out_sheet`,
      "No event sign-in sheet is available yet.",
      "Sign-in sheet PDF opened. Choose Save as PDF in the print window.",
    );
  }

  async function exportSignInSheetDocx() {
    if (!selectedShow) { setMsg({ kind: "error", text: "Select a show first." }); return; }
    await runDocumentExport(
      "Sign-in sheet DOCX",
      buildSignInSheetDocument,
            ...
            <div className="grid" style={{ gap: 16 }}>
              <div className="card compact">
                ...
                <div className="toolbar" style={{ marginTop: 12 }}>
                  ...
                  <button type="button" className="ghost" onClick={exportSignInSheetPdf}>PDF Sign-In Sheet</button>
                  <button type="button" className="ghost" onClick={exportSignInSheetDocx}>Sign-In DOCX</button>
