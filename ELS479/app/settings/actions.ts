...
async function bulkSaveRateTable(formData: FormData, tableName: "master_rates" | "client_rates", successMessage: string) {
  ...
  let mirrorMessage = "";
  if (tableName === "master_rates" && savedRows.length) {
    let mirroredCount = 0;
    const mirrorErrors: string[] = [];
    for (const row of savedRows) {
      const { data: existingClientRows, error: clientLookupError } = await admin
        .from("client_rates")
        .select("id")
        .eq("city_name", row.city_name)
        .ilike("role_name", row.role_name)
        .limit(1);
      if (clientLookupError) {
        mirrorErrors.push(clientLookupError.message);
        break;
      }
      if ((existingClientRows ?? []).length) continue;
      const { error: clientInsertError } = await admin.from("client_rates").insert({
        id: randomUUID(),
        city_name: row.city_name,
        role_name: row.role_name,
        full_day: 0,
        ...
        doubletime_multiplier: row.doubletime_multiplier || 2.0,
      });
      if (clientInsertError) {
        mirrorErrors.push(clientInsertError.message);
        continue;
      }
      mirroredCount += 1;
    }
    if (mirroredCount) mirrorMessage = ` ${mirroredCount} new position${mirroredCount === 1 ? "" : "s"} also mirrored into Client billing at $0.`;
    if (mirrorErrors.length) mirrorMessage += " Client-rate placeholders could not be stored, but the Client Rates screen will still mirror Crew Rate position names until billing rates are entered.";
  }
  ...
  return { ok: true, message: `${successMessage}${mirrorMessage}`, rows: savedRows };
}
