...
export default function PayrollClient({ initialRows, initialEventSummaries, availableYears, initialYear, initialError, initialDiagnostics = null }: Props) {
  const hasInitialPayrollPayload = initialRows.length > 0 || initialEventSummaries.length > 0 || Boolean(initialError) || Boolean(initialDiagnostics);
  const [rows, setRows] = useState(initialRows);
  const [eventSummaries, setEventSummaries] = useState<PayrollEventSummary[]>(initialEventSummaries);
  const [year, setYear] = useState(initialYear);
  const [loadedYears, setLoadedYears] = useState<Set<number>>(() => hasInitialPayrollPayload ? new Set([initialYear]) : new Set<number>());
