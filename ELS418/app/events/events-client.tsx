...
function importPreviewStatusLabel(section: GroupedImportPreviewSection) {
  ...
  const rows = section.dates.flatMap((date) => date.rows);
  const hasExisting = rows.some((row) => row.changes.some((change) => change.existingSubCallId || change.kind === "changed" || change.kind === "unchanged"));
  return hasExisting ? "MATCHED EXISTING AREA" : "NEW AREA";
}
...
function buildGroupedImportPreview(preview: ImportPreviewState, selectedChangeIds: string[], excludedIndexes: number[] = []) {
  ...
  preview.subCallPreview.forEach((call, index) => {
    ...
    const groupKey = importPreviewGroupKey(call);
    const areaName = safeText(call.area) || "Area TBD";
  ...
  return {
    ...
    summary: {
      ...
      crewRequested,
      needsReview: (preview.needsReview || []).length + groups.reduce((total, group) => total + group.warnings.length, 0),
      comparisonSummary,
          ...
          <div className="list" style={{ marginTop: 12 }}>
            ...
            {importPreview ? (() => {
              ...
              const additionIds = comparisonChanges
                .filter((change) => change.kind === "new" && !(typeof change.importedIndex === "number" && excludedSetForSelection.has(change.importedIndex)))
                .map((change) => change.id);
              const changeIds = comparisonChanges.filter((change) => change.kind === "changed").map((change) => change.id);
              ...
              const setRowExcluded = (index: number, excluded: boolean) => {
                ...
                if (excluded) {
                  const idsForRow = comparisonChanges
                    .filter((change) => change.importedIndex === index)
                    .map((change) => change.id);
                  if (idsForRow.length) {
                    setSelectedImportChangeIds((current) => current.filter((id) => !idsForRow.includes(id)));
                  }
                }
              };
              const setGroupExcluded = (group: GroupedImportPreviewSection, excluded: boolean) => {
                const indexes = group.dates.flatMap((date) => date.rows.map((row) => row.index));
                setExcludedImportIndexes((current) => {
                  const next = new Set(current);
                ...
                if (excluded) {
                  const idsForGroup = comparisonChanges
                    .filter((change) => typeof change.importedIndex === "number" && indexes.includes(change.importedIndex))
                    .map((change) => change.id);
                  if (idsForGroup.length) {
                    setSelectedImportChangeIds((current) => current.filter((id) => !idsForGroup.includes(id)));
                  }
                }
              };
                        ...
                        <summary style={{ cursor: "pointer", listStyle: "none" }}>
                          <div className="row" style={{ alignItems: "flex-start" }}>
                            ...
                            <div className="row" style={{ gap: 8, justifyContent: "flex-end" }}>
                              ...
                              {group.dates.flatMap((date) => date.rows).some((row) => row.changes.some((change) => change.kind === "new")) ? (
                                <button
                                  type="button"
                                  className="ghost danger"
                                  onClick={(event) => {
                            ...
                            <div key={`${group.key}-${dateGroup.date}`} className="card compact" style={{ boxShadow: "none", background: "#f9fafb", borderLeft: "4px solid #17313b" }}>
                              ...
                              <div className="list" style={{ marginTop: 10 }}>
                                {dateGroup.rows.map(({ call, index, changes }) => {
                                  ...
                                  const selectableChanges = changes.filter((change) => change.kind === "new" || change.kind === "changed");
                                    ...
                                    >
                                      <div className="row" style={{ alignItems: "flex-start" }}>
                                        ...
                                        <div className="toolbar" style={{ justifyContent: "flex-end" }}>
                                          ...
                                          {changes.some((change) => change.kind === "new") ? (
                        ...
                        {groupedPreview.excludedRows.map(({ call, index }) => (
                          <div key={`excluded-${call.key}-${index}`} className="card compact" style={{ boxShadow: "none", background: "#fff" }}>
                            <div className="row" style={{ alignItems: "flex-start" }}>
                              ...
                              <button type="button" className="ghost" onClick={() => setRowExcluded(index, false)}>Restore</button>
                            </div>
                          </div>
                        ))}
                      </div>
