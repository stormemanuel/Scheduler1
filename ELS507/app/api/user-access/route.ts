...
import { VIEW_AS_USER_COOKIE, sharedCoordinatorUserIds } from "@/lib/auth";
...
function accessCanEditColumnMissing(message: string | null | undefined) {
  return String(message || "").includes("can_edit_event_details");
}
...
async function resolveAllowedCityPoolIds(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, rawValues: string[]) {
  ...
  if (error) return { ids: requested, message: `Could not verify city pool records: ${error.message}` };
  ...
  if (missing.length) return { ids, message: `Could not save city assignment: ${missing.join(", ")} was not found in city pools.` };
  return { ids, message: "" };
}
...
export async function GET(request: NextRequest) {
  ...
  if (action === "coordinator_notifications") {
    ...
    if (countOnly) {
      ...
      const unviewedRes = await admin
        .from("coordinator_event_notifications")
        .select("id", { count: "exact", head: true })
        .eq("user_id", auth.user.id)
        .is("viewed_at", null);
      if (unviewedRes.error && coordinatorNotificationsMissing(unviewedRes.error.message)) {
        return NextResponse.json({ ok: true, setup_missing: true, unread_count: 0 });
      }
      if (unviewedRes.error) return NextResponse.json({ message: unviewedRes.error.message }, { status: 400 });
      const notificationsRes = await admin
        .from("coordinator_event_notifications")
        .select("id")
        .eq("user_id", auth.user.id)
        .limit(250);
      if (notificationsRes.error && coordinatorNotificationsMissing(notificationsRes.error.message)) {
        return NextResponse.json({ ok: true, setup_missing: true, unread_count: unviewedRes.count || 0 });
      }
      if (notificationsRes.error) return NextResponse.json({ message: notificationsRes.error.message }, { status: 400 });
      const notificationIds = (notificationsRes.data || []).map((row) => String((row as { id?: string }).id || "")).filter(Boolean);
      ...
      if (notificationIds.length) {
        const messageRes = await admin
          ...
          .is("read_at", null);
        if (messageRes.error && coordinatorNotificationsMissing(messageRes.error.message)) {
          unreadAdminReplies = 0;
        } else if (messageRes.error) {
          return NextResponse.json({ message: messageRes.error.message }, { status: 400 });
        } else {
          unreadAdminReplies = messageRes.count || 0;
        }
      }
      return NextResponse.json({ ok: true, setup_missing: false, unread_count: (unviewedRes.count || 0) + unreadAdminReplies });
    }
    if (role === "owner" || role === "admin") {
      const { data, error } = await admin
        .from("coordinator_event_notifications")
...
export async function PATCH(request: Request) {
  ...
  const rawAllowedCityPoolIds = Array.isArray(body.allowed_city_pool_ids)
    ? Array.from(new Set<string>(body.allowed_city_pool_ids.map((id: unknown) => String(id || "").trim()).filter(Boolean)))
    : [];

  if (!userId) return NextResponse.json({ message: "User is required." }, { status: 400 });

  if (String(body.action || "") === "save_coordinator_pool_sharing") {
    const profilesResult = await admin
      .from("profiles")
      .select("id, role, is_active");
    if (profilesResult.error) return NextResponse.json({ message: `Could not load coordinators: ${profilesResult.error.message}` }, { status: 400 });

    const coordinatorIds = new Set((profilesResult.data || [])
      ...
      .filter(Boolean));
    if (!coordinatorIds.has(userId)) return NextResponse.json({ message: "The selected active coordinator was not found." }, { status: 404 });

    const requestedIds = Array.isArray(body.shared_coordinator_user_ids)
      ? Array.from(new Set<string>(body.shared_coordinator_user_ids.map((id: unknown) => String(id || "").trim()).filter((id: string) => id && id !== userId && coordinatorIds.has(id))))
      : [];
    const targetGroup = new Set([userId, ...requestedIds]);
    const authUsersResult = await admin.auth.admin.listUsers({ page: 1, perPage: 1000 });
    if (authUsersResult.error) return NextResponse.json({ message: `Could not load coordinator accounts: ${authUsersResult.error.message}` }, { status: 400 });
    const authUserById = new Map(authUsersResult.data.users.map((user) => [user.id, user]));
    const missingAuthIds = [...targetGroup].filter((id) => !authUserById.has(id));
    if (missingAuthIds.length) return NextResponse.json({ message: "One or more selected coordinators do not have an active login account." }, { status: 400 });

    const nextSharesByUser: Record<string, string[]> = {};
    for (const coordinatorId of coordinatorIds) {
      const authUser = authUserById.get(coordinatorId);
      if (!authUser) continue;
      const currentShares = sharedCoordinatorUserIds(authUser).filter((id) => coordinatorIds.has(id) && id !== coordinatorId);
      const nextShares = targetGroup.has(coordinatorId)
        ? [...targetGroup].filter((id) => id !== coordinatorId).sort()
        : currentShares.filter((id) => !targetGroup.has(id)).sort();
      nextSharesByUser[coordinatorId] = nextShares;
      if (JSON.stringify([...currentShares].sort()) === JSON.stringify(nextShares)) continue;
      const updateResult = await admin.auth.admin.updateUserById(coordinatorId, {
        user_metadata: {
          ...(authUser.user_metadata || {}),
          els_shared_coordinator_user_ids: nextShares,
        },
      });
      if (updateResult.error) return NextResponse.json({ message: `Could not save shared coordinator pool: ${updateResult.error.message}` }, { status: 400 });
    }

    return NextResponse.json({ ok: true, shared_coordinator_user_ids_by_user: nextSharesByUser, shared_count: requestedIds.length });
  }
  ...
  if (resolvedPools.message && !resolvedPools.ids.length && rawAllowedCityPoolIds.length) {
    return NextResponse.json({ message: resolvedPools.message }, { status: 400 });
  }
  const allowedCityPoolIds = resolvedPools.ids;
  ...
  let existingResult = await admin
    ...
    .eq("user_id", userId)
    .maybeSingle();
  if (accessCanEditColumnMissing(existingResult.error?.message)) {
  ...
  if (existingResult.error) {
    return NextResponse.json({ message: `Could not load current access settings: ${existingResult.error.message}` }, { status: 400 });
  }

  const existingRow = existingResult.data as { allowed_pages?: string[] | null; restrict_events_to_owner?: boolean | null; restrict_crew_to_owner?: boolean | null; can_edit_event_details?: boolean | null } | null;
  const accessPayload = {
    user_id: userId,
  ...
  let { error } = await admin.from("user_access_settings").upsert(accessPayload);
  if (accessCanEditColumnMissing(error?.message)) {
    const { can_edit_event_details: _canEditEventDetails, ...fallbackPayload } = accessPayload;
    const retry = await admin.from("user_access_settings").upsert(fallbackPayload);
    error = retry.error;
  }

  if (error) return NextResponse.json({ message: `Could not save city assignment: ${error.message}` }, { status: 400 });
