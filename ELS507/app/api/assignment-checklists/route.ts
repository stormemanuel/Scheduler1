...
function pendingActionColumnsMissing(error: { message?: string } | null | undefined) {
  return /pending_action_text|pending_action_visible_to_coordinator|schema cache/i.test(String(error?.message || ""));
}
...
async function completedShowSnapshotMessage(showId: string, cycleId: string) {
  try {
    const snapshot = await savePayrollSnapshotForShow(showId, { source: "show_process_checklist_complete", showProcessCycleId: cycleId });
    if (!snapshot.ok) return ` Show checklist is complete, but Payroll snapshot was not saved: ${snapshot.message}`;
    return snapshot.skipped ? "" : " Payroll totals snapshot saved.";
  } catch (error) {
    return ` Show checklist is complete, but Payroll snapshot was not saved: ${error instanceof Error ? error.message : "unknown error"}`;
  }
}
...
export async function POST(request: Request) {
  ...
  if (String(body.scope || "") === "coordinator_process") {
    ...
    try {
      ...
      if (action === "update_item") {
        ...
        if (error) throw error;
      } else if (action === "send_week_before_reminder") {
        const { data: show, error: showError } = await auth.admin
          .from("shows")
          .select("name, client, venue, event_location, show_start, show_end")
          .eq("id", showId)
          .maybeSingle();
        if (showError) throw showError;
        const typedShow = show as { name?: string | null; client?: string | null; venue?: string | null; event_location?: string | null; show_start?: string | null; show_end?: string | null } | null;
        const { error } = await auth.admin.from("coordinator_event_notifications").insert({
          user_id: coordinatorUserId,
          show_id: showId,
          notification_type: "week_before_reminder_due",
          title: `Send week-before reminder: ${typedShow?.name || "Assigned event"}`,
          body: [
            `Event: ${typedShow?.name || "Assigned event"}`,
            ...
            `Dates: ${typedShow?.show_start || "TBD"} to ${typedShow?.show_end || typedShow?.show_start || "TBD"}`,
            "The schedule was marked sent. Please send or confirm the week-before reminder, then check Step 4A in the coordinator checklist.",
          ].filter(Boolean).join("\n"),
  ...
  if (String(body.scope || "") === "show_process") {
    ...
    if (action === "save_pending_action" || action === "clear_pending_action") {
      if (!showId) return NextResponse.json({ message: "Show is required." }, { status: 400 });
      const text = action === "clear_pending_action" ? "" : clean(body.pending_action_text).slice(0, 120);
      const visibleToCoordinator = action === "clear_pending_action" ? false : Boolean(body.pending_action_visible_to_coordinator);
      const { data, error } = await auth.admin
        .from("shows")
        .update({
          pending_action_text: text || null,
          pending_action_visible_to_coordinator: text ? visibleToCoordinator : false,
        })
        .eq("id", showId)
        .select("id, pending_action_text, pending_action_visible_to_coordinator")
        .single();
      if (error) {
        const message = pendingActionColumnsMissing(error)
          ? "Run the ELS400 pending action SQL, refresh the app, then save this pending action again."
          : error.message;
        return NextResponse.json({ message }, { status: 400 });
      }
      return NextResponse.json({ ok: true, show: data, message: text ? "Pending action saved." : "Pending action cleared." });
    }
    ...
    if (action === "complete_all") {
      try {
        ...
        return NextResponse.json({ ok: true, row: completedRow, message: `Show checklist marked complete.${await completedShowSnapshotMessage(showId, cycleId)}` });
      } catch (error) {
    ...
    return NextResponse.json({ ok: true, row: finalRow, message: isComplete ? `Show checklist complete.${await completedShowSnapshotMessage(showId, cycleId)}` : "Show checklist updated." });
  }
