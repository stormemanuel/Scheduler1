...
async function buildGoogleEventPayload(admin: ReturnType<typeof createSupabaseAdminClient>, showId: string, options: GoogleCalendarSyncOptions = {}): Promise<GoogleEventBundle> {
  ...
  const dayRowsWithSubCalls = dayRows.filter((day) => (callsByDay.get(day.id) || []).length > 0 && cleanGoogleText(day.labor_date));
  if (options.onlySubCallDays && !dayRowsWithSubCalls.length) {
    throw new Error("This show has no labor days with sub-calls yet. Add sub-calls before syncing only sub-call days.");
  }
  const syncStartDate = options.onlySubCallDays ? cleanGoogleText(dayRowsWithSubCalls[0]?.labor_date).slice(0, 10) : startDate;
  const syncEndDate = options.onlySubCallDays ? cleanGoogleText(dayRowsWithSubCalls[dayRowsWithSubCalls.length - 1]?.labor_date).slice(0, 10) : endDate;
  ...
  const calendarEventName = cleanGoogleText(options.eventName) || showName;
  const calendarLocation = joinGoogleParts([typedShow.venue, typedShow.event_location || typedShow.rate_city], ", ") || undefined;
  const monthReminderDate = shiftIsoDateByMonths(syncStartDate, -1);
  const weekReminderDate = shiftIsoDateByDays(syncStartDate, -7);
  const paymentDueDate = shiftIsoDateByDays(syncEndDate, 21);

  const main: GoogleEventPayload = {
    summary: `ELS: ${calendarEventName}`,
    colorId: "2",
  ...
  const mainEvents: GoogleMainEvent[] = options.onlySubCallDays
    ? dayRowsWithSubCalls.map((day) => {
        const laborDate = cleanGoogleText(day.labor_date).slice(0, 10);
        const calls = callsByDay.get(day.id) || [];
        const dayCallLines = calls.map((call) => joinGoogleParts([
          call.start_time && call.end_time ? `${call.start_time}-${call.end_time}` : call.start_time || "",
          call.area || "Area",
          call.role_name || "Role",
          call.crew_needed ? `${call.crew_needed} crew` : "",
        ...
        return {
          taskType: `show_day_${laborDate}` as const,
          payload: {
            ...main,
            summary: `ELS: ${calendarEventName}${day.label ? ` — ${cleanGoogleText(day.label)}` : ""}`,
            description: [
              "Synced from Emanuel Labor Services.",
              "",
              `Client: ${cleanGoogleText(typedShow.client) || "Not set"}`,
              `Venue: ${cleanGoogleText(typedShow.venue) || "Not set"}`,
              typedShow.event_location ? `Event Location: ${cleanGoogleText(typedShow.event_location)}` : "",
              `Labor day: ${formatDisplayDate(laborDate)}${day.label ? ` — ${cleanGoogleText(day.label)}` : ""}`,
              "",
              dayCallLines.length ? "Sub-calls:" : "No sub-calls listed.",
              ...dayCallLines,
              "",
              "ELS remains the master schedule. Update the show inside ELS, then sync again.",
            ].filter((line) => line !== "").join("\n"),
            start: { date: laborDate },
            end: { date: plusOneDate(laborDate) },
            extendedProperties: { private: { els_show_id: showId, els_source: "Emanuel Labor Services", els_task_type: `show_day_${laborDate}` } },
          },
        };
      })
    : [{ taskType: "show", payload: main }];
  ...
  const actionEvents: GoogleActionEvent[] = [
    {
      ...
      payload: {
        ...
        description: [
          ...
          `Show dates: ${formatDisplayDate(syncStartDate)} to ${formatDisplayDate(syncEndDate)}`,
          "",
          "Open ELS → Events → this show → Send Schedules and Reminders.",
        ].join("\n"),
    ...
    },
    {
      ...
      payload: {
        ...
        description: [
          ...
          `Show ended: ${formatDisplayDate(syncEndDate)}`,
  ...
  return { main, mainEvents, actionEvents };
}
...
async function syncSingleShowToGoogle(admin: ReturnType<typeof createSupabaseAdminClient>, userId: string, showId: string, options: GoogleCalendarSyncOptions = {}) {
  if (!admin) throw new Error("SUPABASE_SERVICE_ROLE_KEY is missing.");
  const { accessToken, connection, calendarId } = await getGoogleAccessForUser(admin, userId);
  const bundle = await buildGoogleEventPayload(admin, showId, options);
  ...
  const existingMainEventIds = new Map<string, string>();
  ...
  for (const linkedEvent of linkedEvents) {
    ...
    if (taskType === "show" || taskType.startsWith("show_day_")) {
      if (linkedEvent.id && !existingMainEventIds.has(taskType)) existingMainEventIds.set(taskType, linkedEvent.id);
    } else if (taskType && linkedEvent.id && !existingActionEventIds.has(taskType)) {
      existingActionEventIds.set(taskType, linkedEvent.id);
    }
  }
  if (existingLink?.google_event_id && !existingMainEventIds.size) existingMainEventIds.set("show", existingLink.google_event_id);

  const mainEventResults = await Promise.all(bundle.mainEvents.map(async (mainEvent) => {
    const result = await upsertGoogleActionEvent(
      accessToken,
      calendarId,
      existingMainEventIds.get(mainEvent.taskType) || null,
      mainEvent.payload,
    );
    return { task_type: mainEvent.taskType, google_event_id: result.id, google_event_html_link: result.htmlLink || null };
  }));
  const currentMainTaskTypes = new Set(bundle.mainEvents.map((event) => event.taskType));
  await Promise.all([...existingMainEventIds.entries()].map(async ([taskType, eventId]) => {
    if (currentMainTaskTypes.has(taskType as GoogleMainEvent["taskType"])) return;
  ...
  const primaryMainEvent = mainEventResults[0];

  await upsertGoogleEventLink(admin, {
    user_id: userId,
    show_id: showId,
    calendar_id: calendarId,
    google_event_id: primaryMainEvent?.google_event_id || null,
    google_event_html_link: primaryMainEvent?.google_event_html_link || null,
  ...
  return {
    ...
    google_event_id: primaryMainEvent?.google_event_id || null,
    google_event_html_link: primaryMainEvent?.google_event_html_link || null,
    main_event_count: mainEventResults.length,
    main_events: mainEventResults,
...
async function handleGoogleCalendarConnect(request: Request) {
  ...
  if (new URL(request.url).searchParams.get("return_to") === "overview") {
    response.cookies.set("els_google_calendar_oauth_return", "overview", { httpOnly: true, sameSite: "lax", secure: true, path: "/", maxAge: 10 * 60 });
  }
  return response;
}
...
async function handleGoogleCalendarCallback(request: Request) {
  const auth = await requireSignedIn();
  const returnToOverview = request.headers.get("cookie")?.split(";").map((item) => item.trim()).some((item) => item === "els_google_calendar_oauth_return=overview") || false;
  const baseRedirect = new URL(returnToOverview ? "/" : "/events", new URL(request.url).origin);
  if (!auth.ok) {
  ...
  try {
    ...
    response.cookies.delete("els_google_calendar_oauth_return");
    return response;
  } catch (callbackError) {
...
type GoogleOverviewCalendarEvent = {
  id?: string;
  summary?: string;
  location?: string;
  description?: string;
...
async function handleGoogleCalendarOverviewEvents(request: Request) {
  const auth = await requireSignedIn();
  if (!auth.ok) return auth.response;
  const admin = createSupabaseAdminClient();
  ...
  try {
    ...
    if (!Number.isFinite(startMs) || !Number.isFinite(endMs) || endMs <= startMs || endMs - startMs > 370 * 24 * 60 * 60 * 1000) {
      return NextResponse.json({ message: "A valid calendar range of one year or less is required." }, { status: 400 });
    }
    const { accessToken, connection, calendarId } = await getGoogleAccessForUser(admin, auth.user.id);
    const googleUrl = new URL(`https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendarId)}/events`);
    googleUrl.searchParams.set("timeMin", new Date(startMs).toISOString());
    googleUrl.searchParams.set("timeMax", new Date(endMs).toISOString());
    googleUrl.searchParams.set("singleEvents", "true");
    googleUrl.searchParams.set("showDeleted", "false");
    googleUrl.searchParams.set("orderBy", "startTime");
    googleUrl.searchParams.set("maxResults", "2500");
    const data = await googleFetchJson<{ items?: GoogleOverviewCalendarEvent[] }>(googleUrl.toString(), {
      headers: { Authorization: `Bearer ${accessToken}` },
    });
    const events = (data.items || []).filter((event) => {
      ...
      return privateFields.els_source !== "Emanuel Labor Services" || privateFields.els_task_type === "personal_calendar";
    }).map((event) => {
      const privateFields = event.extendedProperties?.private || {};
      return {
        id: event.id || "",
        title: cleanGoogleText(event.summary) || "Busy",
        start: event.start?.dateTime || event.start?.date || "",
        end: event.end?.dateTime || event.end?.date || "",
        all_day: Boolean(event.start?.date && !event.start?.dateTime),
        location: cleanGoogleText(event.location),
        description: cleanGoogleText(event.description),
        html_link: event.htmlLink || "",
        can_delete: privateFields.els_task_type === "personal_calendar" && privateFields.els_created_by === auth.user.id,
      };
    });
    return NextResponse.json({ ok: true, connected: true, account_email: connection.account_email || null, events });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Google Calendar could not be loaded.";
    return NextResponse.json({ ok: false, connected: false, message }, { status: /not connected/i.test(message) ? 409 : 400 });
  }
}

async function handleGoogleCalendarCreatePersonalEvent(request: Request) {
  const auth = await requireSignedIn();
  if (!auth.ok) return auth.response;
  const admin = createSupabaseAdminClient();
  if (!admin) return NextResponse.json({ message: "SUPABASE_SERVICE_ROLE_KEY is missing." }, { status: 500 });
  try {
    ...
    const typedBody = body as { title?: string; date?: string; all_day?: boolean; start_time?: string; end_time?: string; location?: string; notes?: string; timezone?: string };
    const title = cleanGoogleText(typedBody.title);
    const date = cleanGoogleText(typedBody.date).slice(0, 10);
    const allDay = Boolean(typedBody.all_day);
    const startTime = cleanGoogleText(typedBody.start_time);
    const endTime = cleanGoogleText(typedBody.end_time);
    const timezone = cleanGoogleText(typedBody.timezone) || "America/Chicago";
    if (!title || !/^\d{4}-\d{2}-\d{2}$/.test(date)) return NextResponse.json({ message: "A title and valid date are required." }, { status: 400 });
    if (!allDay && (!/^\d{2}:\d{2}$/.test(startTime) || !/^\d{2}:\d{2}$/.test(endTime) || endTime <= startTime)) {
      return NextResponse.json({ message: "Choose an end time after the start time." }, { status: 400 });
    }
    const { accessToken, connection, calendarId } = await getGoogleAccessForUser(admin, auth.user.id);
    const payload: GoogleEventPayload = {
      summary: title,
      location: cleanGoogleText(typedBody.location),
      description: cleanGoogleText(typedBody.notes),
      colorId: "5",
      start: allDay ? { date } : { dateTime: `${date}T${startTime}:00`, timeZone: timezone },
      end: allDay ? { date: plusOneDate(date) } : { dateTime: `${date}T${endTime}:00`, timeZone: timezone },
      visibility: "private",
      reminders: { useDefault: true },
      extendedProperties: { private: { els_source: "Emanuel Labor Services", els_task_type: "personal_calendar", els_created_by: auth.user.id } },
    };
    const event = await googleFetchJson<GoogleOverviewCalendarEvent>(`https://www.googleapis.com/calendar/v3/calendars/${encodeURIComponent(calendarId)}/events`, {
      method: "POST",
      headers: { Authorization: `Bearer ${accessToken}`, "Content-Type": "application/json" },
      body: JSON.stringify(payload),
    });
    return NextResponse.json({ ok: true, connected: true, account_email: connection.account_email || null, event: { id: event.id || "", html_link: event.htmlLink || "" }, message: "Added to ELS and Google Calendar." });
  } catch (error) {
    const message = error instanceof Error ? error.message : "The Google Calendar item could not be created.";
    return NextResponse.json({ ok: false, message }, { status: /not connected/i.test(message) ? 409 : 400 });
  }
}

async function handleGoogleCalendarDeletePersonalEvent(request: Request) {
  const auth = await requireSignedIn();
  if (!auth.ok) return auth.response;
  const admin = createSupabaseAdminClient();
  if (!admin) return NextResponse.json({ message: "SUPABASE_SERVICE_ROLE_KEY is missing." }, { status: 500 });
  try {
    const body = await request.json().catch(() => ({}));
    const eventId = cleanGoogleText((body as { event_id?: string }).event_id);
    if (!eventId) return NextResponse.json({ message: "event_id is required." }, { status: 400 });
    const { accessToken, calendarId } = await getGoogleAccessForUser(admin, auth.user.id);
    ...
    const existing = await googleFetchJson<GoogleOverviewCalendarEvent>(eventUrl, { headers: { Authorization: `Bearer ${accessToken}` } });
    const privateFields = existing.extendedProperties?.private || {};
    if (privateFields.els_task_type !== "personal_calendar" || privateFields.els_created_by !== auth.user.id) {
      return NextResponse.json({ message: "Only personal items created from ELS can be deleted here. Open other events in Google Calendar." }, { status: 403 });
    }
    await googleFetchJson<Record<string, never>>(eventUrl, { method: "DELETE", headers: { Authorization: `Bearer ${accessToken}` } });
    return NextResponse.json({ ok: true, message: "Personal item deleted from Google Calendar." });
  } catch (error) {
    return NextResponse.json({ ok: false, message: error instanceof Error ? error.message : "The calendar item could not be deleted." }, { status: 400 });
  }
}
...
async function handleGoogleCalendarSyncShow(request: Request) {
  ...
  if (!admin) return NextResponse.json({ message: "SUPABASE_SERVICE_ROLE_KEY is missing." }, { status: 500 });
  const body = await request.json().catch(() => ({}));
  const typedBody = body as { show_id?: string; only_sub_call_days?: boolean; event_name?: string };
  const showId = cleanGoogleText(typedBody.show_id);
  ...
  const result = await syncSingleShowToGoogle(admin, auth.user.id, showId, {
    onlySubCallDays: Boolean(typedBody.only_sub_call_days),
    eventName: cleanGoogleText(typedBody.event_name),
  });
  const dayText = result.main_event_count === 1 ? "1 show entry" : `${result.main_event_count} show-day entries`;
  return NextResponse.json({ ok: true, ...result, connected: true, show_synced: true, message: `${dayText} and 3 action reminders synced to Google Calendar.` });
}
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (action === "overview-events") return handleGoogleCalendarOverviewEvents(request);
...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (action === "create-personal-event") return handleGoogleCalendarCreatePersonalEvent(request);
  if (action === "delete-personal-event") return handleGoogleCalendarDeletePersonalEvent(request);
