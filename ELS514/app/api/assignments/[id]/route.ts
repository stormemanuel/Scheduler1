import { NextResponse } from "next/server";
import { createSupabaseAdminClient, createSupabaseServerClient, findCrewAssignmentConflict, recordRemovedAssignmentsAndCancelQueue } from "@/lib/supabase-server";
import { normalizeRole } from "@/lib/auth";

async function requireSignedIn() {
  const supabase = await createSupabaseServerClient();
  if (!supabase) return { ok: false as const, response: NextResponse.json({ message: "Supabase is not configured." }, { status: 500 }) };
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) return { ok: false as const, response: NextResponse.json({ message: "Unauthorized." }, { status: 401 }) };
  return { ok: true as const, user };
}

async function isOwnerAdmin(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, userId: string) {
  const { data } = await admin.from("profiles").select("role").eq("id", userId).maybeSingle();
  const role = normalizeRole((data as { role?: string | null } | null)?.role);
  return role === "owner" || role === "admin";
}

async function resolveCoordinationOwner(
  admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>,
  requestedOwnerId: string | null,
  crewId: string,
) {
  const ownerId = String(requestedOwnerId || "").trim();
  if (!ownerId) return { ok: true as const, userId: null as string | null, name: null as string | null };
  if (ownerId === crewId) {
    return { ok: false as const, message: "Coordination owner cannot be the crew member being assigned." };
  }
  const { data, error } = await admin
    .from("profiles")
    .select("id, full_name, email, role")
    .eq("id", ownerId)
    .maybeSingle();
  if (error) return { ok: false as const, message: error.message };
  const profile = data as { id?: string | null; full_name?: string | null; email?: string | null; role?: string | null } | null;
  const role = normalizeRole(profile?.role || "");
  if (!profile?.id || !["owner", "admin", "coordinator"].includes(role)) {
    return { ok: false as const, message: "Coordination owner must be an active ELS owner, admin, or coordinator user." };
  }
  return { ok: true as const, userId: String(profile.id), name: String(profile.full_name || profile.email || "ELS user") };
}

async function canAccessAssignment(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, userId: string, assignmentId: string) {
  if (await isOwnerAdmin(admin, userId)) return true;
  const { data: assignment } = await admin.from("assignments").select("sub_call_id").eq("id", assignmentId).maybeSingle();
  const subCallId = String((assignment as { sub_call_id?: string | null } | null)?.sub_call_id || "");
  if (!subCallId) return false;
  const { data: call } = await admin.from("sub_calls").select("labor_day_id, assigned_coordinator_user_id").eq("id", subCallId).maybeSingle();
  const typedCall = call as { labor_day_id?: string | null; assigned_coordinator_user_id?: string | null } | null;
  if (typedCall?.assigned_coordinator_user_id === userId) return true;
  const laborDayId = String(typedCall?.labor_day_id || "");
  if (!laborDayId) return false;
  const { data: day } = await admin.from("labor_days").select("show_id").eq("id", laborDayId).maybeSingle();
  const showId = String((day as { show_id?: string | null } | null)?.show_id || "");
  if (!showId) return false;
  const { data: show } = await admin.from("shows").select("created_by, assigned_coordinator_user_id").eq("id", showId).maybeSingle();
  const typedShow = show as { created_by?: string | null; assigned_coordinator_user_id?: string | null } | null;
  if (typedShow?.created_by === userId || typedShow?.assigned_coordinator_user_id === userId) return true;
  const accessByUser = await admin.from("event_user_access").select("id").eq("show_id", showId).eq("user_id", userId).limit(1);
  if (!accessByUser.error && accessByUser.data?.length) return true;
  if (accessByUser.error?.message?.includes("user_id")) {
    const accessByProfile = await admin.from("event_user_access").select("id").eq("show_id", showId).eq("user_profile_id", userId).limit(1);
    return Boolean(!accessByProfile.error && accessByProfile.data?.length);
  }
  return false;
}

async function canManageAssignment(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, userId: string, assignmentId: string) {
  if (await isOwnerAdmin(admin, userId)) return true;
  const { data, error } = await admin
    .from("assignments")
    .select("coordination_owner_user_id")
    .eq("id", assignmentId)
    .maybeSingle();
  if (error) return false;
  return String((data as { coordination_owner_user_id?: string | null } | null)?.coordination_owner_user_id || "") === userId;
}

async function canAccessSubCall(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, userId: string, subCallId: string) {
  if (await isOwnerAdmin(admin, userId)) return true;
  const { data: call } = await admin.from("sub_calls").select("labor_day_id, assigned_coordinator_user_id").eq("id", subCallId).maybeSingle();
  const typedCall = call as { labor_day_id?: string | null; assigned_coordinator_user_id?: string | null } | null;
  if (typedCall?.assigned_coordinator_user_id === userId) return true;
  const laborDayId = String(typedCall?.labor_day_id || "");
  if (!laborDayId) return false;
  const { data: day } = await admin.from("labor_days").select("show_id").eq("id", laborDayId).maybeSingle();
  const showId = String((day as { show_id?: string | null } | null)?.show_id || "");
  if (!showId) return false;
  const { data: show } = await admin.from("shows").select("created_by, assigned_coordinator_user_id").eq("id", showId).maybeSingle();
  const typedShow = show as { created_by?: string | null; assigned_coordinator_user_id?: string | null } | null;
  if (typedShow?.created_by === userId || typedShow?.assigned_coordinator_user_id === userId) return true;
  const accessByUser = await admin.from("event_user_access").select("id").eq("show_id", showId).eq("user_id", userId).limit(1);
  if (!accessByUser.error && accessByUser.data?.length) return true;
  if (accessByUser.error?.message?.includes("user_id")) {
    const accessByProfile = await admin.from("event_user_access").select("id").eq("show_id", showId).eq("user_profile_id", userId).limit(1);
    return Boolean(!accessByProfile.error && accessByProfile.data?.length);
  }
  return false;
}

export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const auth = await requireSignedIn();
  if (!auth.ok) return auth.response;
  const admin = createSupabaseAdminClient();
  if (!admin) return NextResponse.json({ message: "SUPABASE_SERVICE_ROLE_KEY is missing." }, { status: 500 });
  const { id } = await params;
  if (!(await canAccessAssignment(admin, auth.user.id, id))) return NextResponse.json({ message: "Forbidden." }, { status: 403 });
  if (!(await canManageAssignment(admin, auth.user.id, id))) {
    return NextResponse.json({ message: "This crew member was booked by an admin or another coordinator and is view-only for your login." }, { status: 403 });
  }
  const body = await request.json();
  const { data: assignmentState, error: assignmentStateError } = await admin
    .from("assignments")
    .select("sub_call_id, crew_id, status, start_time, end_time")
    .eq("id", id)
    .maybeSingle();
  if (assignmentStateError) return NextResponse.json({ message: assignmentStateError.message }, { status: 400 });
  const currentSubCallId = String((assignmentState as { sub_call_id?: string | null } | null)?.sub_call_id || "");
  const crewId = String((assignmentState as { crew_id?: string | null } | null)?.crew_id || "");
  const currentStatus = String((assignmentState as { status?: string | null } | null)?.status || "confirmed");
  const currentStartTime = String((assignmentState as { start_time?: string | null } | null)?.start_time || "") || null;
  const currentEndTime = String((assignmentState as { end_time?: string | null } | null)?.end_time || "") || null;
  if (!currentSubCallId || !crewId) return NextResponse.json({ message: "Crew assignment was not found." }, { status: 404 });

  const payload: { sub_call_id?: string; sort_order?: number; status?: string; start_time?: string | null; end_time?: string | null; day_type?: string | null; coordination_owner_user_id?: string | null; coordination_owner_name?: string | null; coordination_fee_waived?: boolean } = {};
  let movingToDifferentSubCall = false;
  if (body.sub_call_id !== undefined) {
    const targetSubCallId = String(body.sub_call_id || "").trim();
    if (!targetSubCallId) return NextResponse.json({ message: "Choose a destination position/sub-call." }, { status: 400 });
    if (!(await canAccessSubCall(admin, auth.user.id, targetSubCallId))) return NextResponse.json({ message: "You cannot move this crew member to that position." }, { status: 403 });

    if (targetSubCallId !== currentSubCallId) {
      movingToDifferentSubCall = true;
      const duplicate = await admin
        .from("assignments")
        .select("id")
        .eq("sub_call_id", targetSubCallId)
        .eq("crew_id", crewId)
        .limit(1);
      if (duplicate.error) return NextResponse.json({ message: duplicate.error.message }, { status: 400 });
      if (duplicate.data?.length) return NextResponse.json({ message: "This crew member is already assigned to the destination position on that labor day." }, { status: 400 });

      const targetCall = await admin.from("sub_calls").select("crew_needed").eq("id", targetSubCallId).maybeSingle();
      if (targetCall.error) return NextResponse.json({ message: targetCall.error.message }, { status: 400 });
      const maxCrew = Math.max(0, Number((targetCall.data as { crew_needed?: number | null } | null)?.crew_needed || 0));
      if (maxCrew > 0) {
        const targetAssignments = await admin.from("assignments").select("id", { count: "exact", head: true }).eq("sub_call_id", targetSubCallId);
        if (targetAssignments.error) return NextResponse.json({ message: targetAssignments.error.message }, { status: 400 });
        const assignedCount = Number(targetAssignments.count || 0);
        if (assignedCount >= maxCrew) {
          return NextResponse.json({ message: `The destination position is full (${assignedCount}/${maxCrew}). Increase Crew Needed before moving this crew member.` }, { status: 400 });
        }
      }
    }
    payload.sub_call_id = targetSubCallId;
  }
  if (body.sort_order !== undefined) {
    const sortOrder = Math.max(1, Number(body.sort_order || 1));
    payload.sort_order = Number.isFinite(sortOrder) ? sortOrder : 1;
  }
  if (body.status !== undefined) payload.status = String(body.status || "confirmed").trim() || "confirmed";
  if (body.start_time !== undefined) payload.start_time = String(body.start_time || "").trim() || null;
  if (body.end_time !== undefined) payload.end_time = String(body.end_time || "").trim() || null;
  if (body.day_type !== undefined) {
    const dayType = String(body.day_type || "").trim();
    payload.day_type = ["full_day", "half_day", "hourly", "custom"].includes(dayType) ? dayType : null;
  }
  if (body.coordination_owner_user_id !== undefined || body.coordination_owner_name !== undefined || body.coordination_fee_waived !== undefined) {
    const ownerAdmin = await isOwnerAdmin(admin, auth.user.id);
    if (!ownerAdmin) return NextResponse.json({ message: "Only owner/admin users can change coordination fee ownership." }, { status: 403 });
    if (body.coordination_owner_user_id !== undefined) {
      const ownerId = String(body.coordination_owner_user_id || "").trim();
      const resolvedOwner = await resolveCoordinationOwner(admin, ownerId || null, crewId);
      if (!resolvedOwner.ok) return NextResponse.json({ message: resolvedOwner.message }, { status: 400 });
      payload.coordination_owner_user_id = resolvedOwner.userId;
      payload.coordination_owner_name = resolvedOwner.name;
    } else if (body.coordination_owner_name !== undefined) {
      payload.coordination_owner_name = String(body.coordination_owner_name || "").trim() || null;
    }
    if (body.coordination_fee_waived !== undefined) payload.coordination_fee_waived = Boolean(body.coordination_fee_waived);
  }
  if (!Object.keys(payload).length) return NextResponse.json({ message: "Nothing to update." }, { status: 400 });

  if (body.sub_call_id !== undefined || body.start_time !== undefined || body.end_time !== undefined || body.status !== undefined) {
    try {
      const conflict = await findCrewAssignmentConflict(admin, {
        crewId,
        targetSubCallId: payload.sub_call_id || currentSubCallId,
        targetStartTime: body.start_time !== undefined ? (payload.start_time || null) : currentStartTime,
        targetEndTime: body.end_time !== undefined ? (payload.end_time || null) : currentEndTime,
        targetStatus: payload.status || currentStatus,
        ignoreAssignmentIds: [id],
      });
      if (conflict) {
        return NextResponse.json({
          message: `This change would double-book this crew member with another call on ${conflict.laborDate} (${conflict.startTime || "TBD"}–${conflict.endTime || "TBD"}). Remove or move the other assignment first.`,
          code: "CREW_TIME_CONFLICT",
        }, { status: 409 });
      }
    } catch (error) {
      return NextResponse.json({ message: error instanceof Error ? error.message : "Could not verify crew booking conflicts." }, { status: 400 });
    }
  }

  if (movingToDifferentSubCall) {
    await recordRemovedAssignmentsAndCancelQueue(admin, {
      assignmentIds: [id],
      removedByUserId: auth.user.id,
      removedByName: auth.user.email || "ELS user",
      reason: "Crew moved to another sub-call.",
    });
  }

  const { data, error } = await admin
    .from("assignments")
    .update(payload)
    .eq("id", id)
    .select("id, sub_call_id, crew_id, status, sort_order, start_time, end_time, day_type, coordination_owner_user_id, coordination_owner_name, coordination_fee_waived")
    .single();
  if (error) return NextResponse.json({ message: error.message }, { status: 400 });
  return NextResponse.json({ ok: true, row: data, message: "Crew assignment updated." });
}

function truthyScope(value: unknown) {
  return value === true || value === "true" || value === "1" || value === "all_matching_sub_call";
}

export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  const auth = await requireSignedIn();
  if (!auth.ok) return auth.response;
  const admin = createSupabaseAdminClient();
  if (!admin) return NextResponse.json({ message: "SUPABASE_SERVICE_ROLE_KEY is missing." }, { status: 500 });
  const { id } = await params;
  if (!(await canAccessAssignment(admin, auth.user.id, id))) return NextResponse.json({ message: "Forbidden." }, { status: 403 });
  if (!(await canManageAssignment(admin, auth.user.id, id))) {
    return NextResponse.json({ message: "This crew member was booked by an admin or another coordinator and cannot be removed by your login." }, { status: 403 });
  }
  const body = await request.json().catch(() => ({})) as { all_matching_sub_call?: unknown; scope?: unknown };
  let assignmentIds = [id];
  const removeAllMatchingSubCallDays = truthyScope(body.all_matching_sub_call) || truthyScope(body.scope);
  if (removeAllMatchingSubCallDays) {
    const { data: anchorAssignment, error: assignmentError } = await admin
      .from("assignments")
      .select("id, sub_call_id, crew_id")
      .eq("id", id)
      .maybeSingle();
    if (assignmentError) return NextResponse.json({ message: assignmentError.message }, { status: 400 });
    const anchor = anchorAssignment as { id?: string | null; sub_call_id?: string | null; crew_id?: string | null } | null;
    const anchorSubCallId = String(anchor?.sub_call_id || "");
    const crewId = String(anchor?.crew_id || "");
    if (!anchorSubCallId || !crewId) return NextResponse.json({ message: "Crew assignment was not found." }, { status: 404 });

    const { data: anchorCall, error: callError } = await admin
      .from("sub_calls")
      .select("id, sub_call_group_id")
      .eq("id", anchorSubCallId)
      .maybeSingle();
    if (callError) return NextResponse.json({ message: callError.message }, { status: 400 });
    const groupId = String((anchorCall as { sub_call_group_id?: string | null } | null)?.sub_call_group_id || "").trim();
    let targetSubCallIds = [anchorSubCallId];
    if (groupId) {
      const { data: matchingCalls, error: matchingError } = await admin
        .from("sub_calls")
        .select("id")
        .eq("sub_call_group_id", groupId);
      if (matchingError) return NextResponse.json({ message: matchingError.message }, { status: 400 });
      targetSubCallIds = [...new Set((matchingCalls || []).map((row: { id?: string | null }) => String(row.id || "")).filter(Boolean))];
    }

    const { data: matchingAssignments, error: matchingAssignmentsError } = await admin
      .from("assignments")
      .select("id, coordination_owner_user_id")
      .eq("crew_id", crewId)
      .in("sub_call_id", targetSubCallIds);
    if (matchingAssignmentsError) return NextResponse.json({ message: matchingAssignmentsError.message }, { status: 400 });
    const ownerAdmin = await isOwnerAdmin(admin, auth.user.id);
    assignmentIds = [...new Set((matchingAssignments || [])
      .filter((row: { coordination_owner_user_id?: string | null }) => ownerAdmin || String(row.coordination_owner_user_id || "") === auth.user.id)
      .map((row: { id?: string | null }) => String(row.id || ""))
      .filter(Boolean))];
    if (!assignmentIds.length) assignmentIds = [id];
  }
  const cancellation = await recordRemovedAssignmentsAndCancelQueue(admin, {
    assignmentIds,
    removedByUserId: auth.user.id,
    removedByName: auth.user.email || "ELS user",
    reason: removeAllMatchingSubCallDays ? "Crew removed from all days in this sub-call." : "Crew removed from sub-call.",
  });
  const { error } = await admin.from("assignments").delete().in("id", assignmentIds);
  if (error) return NextResponse.json({ message: error.message }, { status: 400 });
  return NextResponse.json({
    ok: true,
    cancellation,
    assignmentIds,
    removedCount: assignmentIds.length,
    message: removeAllMatchingSubCallDays
      ? `Crew removed from ${assignmentIds.length} sub-call day${assignmentIds.length === 1 ? "" : "s"}.`
      : "Crew removed from sub-call.",
  });
}
