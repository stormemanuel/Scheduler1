...
export async function getCrewPageData() {
  ...
  const [cityPoolsRes, crewGroupsRes, crewResInitial, positionsRes, unavailableRes, ratesRes, extraPoolsRes] = await Promise.all([
  ...
  const error = cityPoolsRes.error || (crewGroupsMissing ? null : crewGroupsRes.error) || crewRes.error || positionsRes.error || unavailableRes.error || (ratesMissing ? null : ratesRes.error) || (extraPoolsMissing ? null : extraPoolsRes.error);
  if (error) {
    return {
      cityPools: [] as CityPoolRecord[],
      crewGroups: [] as CrewGroupRecord[],
  ...
  return { cityPools, crewGroups, crewRecords, masterRates, setupMissing: false, error: null as string | null };
}
