...
async function coordinatorOnboardingDashboard(admin: SupabaseAdmin) {
  ...
  const [crewRes, cityPoolsRes, extraPoolsRes] = await Promise.all([
    admin
      .from("crew")
      .select("id, name, phone, email, city_pool_id, group_name, created_by, coordinator_hidden_at, coordinator_hidden_by, onboarding_status, questionnaire_status, w9_status, contract_status, onboarding_request_sent_at, onboarding_completed_at")
  ...
  const rows = (crewRes.data || []).filter((row) => {
    ...
    return {
      ...
      onboarding_status: safeText(typed.onboarding_status) || "not_started",
      questionnaire_status: safeText(typed.questionnaire_status) || "missing",
      w9_status: safeText(typed.w9_status) || "missing",
      contract_status: safeText(typed.contract_status) || "missing",
      onboarding_request_sent_at: safeText(typed.onboarding_request_sent_at) || null,
      onboarding_completed_at: safeText(typed.onboarding_completed_at) || null,
    };
  });
...
async function listOnboardingReviewQueue(admin: SupabaseAdmin) {
  ...
  // Repair legacy/mistaken public contract uploads that point to an image instead of
  // the generated signed agreement. The in-app signature data is authoritative.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "create_request") {
      const auth = await onboardingSenderContext();
      if (!auth.ok) return auth.response;
      ...
      if (auth.context.role === "coordinator" && requestType !== "full_onboarding") {
        return NextResponse.json(
          { message: "Coordinators can send the full onboarding packet. W-9-only and contract-only requests remain Owner/Admin actions." },
          { status: 403 },
        );
      }

      const crew = await scopedCrewForOnboarding(admin, auth.context, crewId);
      if (!crew)
        return NextResponse.json(
          { message: "Crew contact was not found in your assigned crew pool." },
          { status: 404 },
        );
      ...
      const messageBody =
        requestType === "w9_only"
          ? [
              `Hi ${firstName}, this is Storm with Emanuel Labor Services.`,
              "I only need your W-9 for ELS tax/1099 records. Please complete and sign it through this secure link:",
              link,
      ...
      if (phone && body.queue_text !== false) {
        queued = await insertIntroQueue(admin, {
          queued_by_user_id: auth.context.userId,
          queued_by_email: auth.context.email,
          queued_by_name: auth.context.email,
          crew_id: crewId,
          crew_name: crewName,
          phone,
          body: messageBody,
