...
export default function OnboardingCenterPage() {
  const [pageMode, setPageMode] = useState<"loading" | "admin" | "coordinator">("loading");
  ...
  useEffect(() => {
    ...
    async function initializePage() {
      ...
      try {
        ...
        if (!response.ok || !result.ok) throw new Error(result.message || "Unable to load onboarding.");
        if (cancelled) return;
        if (result.mode === "coordinator") {
          setPageMode("coordinator");
          setCoordinatorReadOnly(Boolean(result.read_only));
          setCoordinatorRows(result.rows || []);
          setLoading(false);
          return;
        }
        setPageMode("admin");
        await Promise.all([loadRows(), loadGoogleStatus()]);
      } catch (error) {
        if (cancelled) return;
  ...
  async function sendCoordinatorOnboarding(row: CoordinatorOnboardingRow) {
    ...
    if (!row.phone) {
      setMessage("Add a phone number to this crew contact before sending onboarding.");
      return;
    }
    const confirmed = window.confirm(`Create a secure onboarding link for ${row.name} and queue the text?`);
    if (!confirmed) return;
    ...
    try {
      ...
      setCoordinatorRows((current) => current.map((item) => item.id === row.id ? {
        ...item,
  ...
  if (pageMode === "loading") {
    return (
      <section className="card">
        <h2>Onboarding</h2>
        <p className={message ? "error" : "muted"}>{message || "Loading onboarding..."}</p>
      </section>
    );
  }

  if (pageMode === "coordinator") {
    return (
      <div className="grid" style={{ gap: 16 }}>
        <section className="card">
          <div className="row" style={{ alignItems: "flex-start", gap: 12 }}>
            <div>
              <h2 style={{ marginBottom: 6 }}>Onboarding</h2>
              <p className="muted" style={{ margin: 0 }}>
                Send secure onboarding packets to crew in your assigned pool and track their progress. Owner/Admin reviews submitted W-9s, contracts, tax details, and final approvals.
              </p>
            </div>
            <a className="ghost" href="/crew">Open Crew</a>
          </div>
          {coordinatorReadOnly ? <p className="notice" style={{ marginTop: 12 }}>You are viewing this coordinator in read-only preview mode. Buttons are shown exactly where the coordinator sees them, but sending is disabled.</p> : null}
          {message ? <p className={message.toLowerCase().includes("unable") || message.toLowerCase().includes("read-only") ? "error" : "success"}>{message}</p> : null}
        </section>

        {!coordinatorRows.length ? (
          <section className="card"><p className="muted">No crew contacts are available in this coordinator pool yet. Add crew from the Crew page first.</p></section>
        ) : null}

        {coordinatorRows.map((row) => (
          <section key={row.id} className="card compact">
            <div className="row" style={{ alignItems: "flex-start", gap: 12 }}>
              <div>
                <h3 style={{ margin: 0 }}>{row.name || "Unnamed crew member"}</h3>
                <p className="muted small" style={{ margin: "4px 0 0" }}>
                  {[row.city_name, row.group_name, row.phone, row.email].filter(Boolean).join(" • ")}
                </p>
              </div>
              <button
                className="primary"
              ...
              >
                {coordinatorReadOnly ? "Read-only preview" : coordinatorBusyId === row.id ? "Sending..." : row.phone ? "Send onboarding" : "Phone needed"}
              </button>
            </div>
            <div className="grid grid-3" style={{ marginTop: 12 }}>
              <InfoLine label="Overall" value={row.onboarding_status.replace(/_/g, " ")} />
              <InfoLine label="Questionnaire" value={row.questionnaire_status.replace(/_/g, " ")} />
              <InfoLine label="W-9 progress" value={row.w9_status.replace(/_/g, " ")} />
              <InfoLine label="Contract progress" value={row.contract_status.replace(/_/g, " ")} />
              <InfoLine label="Request sent" value={row.onboarding_request_sent_at ? row.onboarding_request_sent_at.slice(0, 10) : "Not sent"} />
              <InfoLine label="Completed" value={row.onboarding_completed_at ? row.onboarding_completed_at.slice(0, 10) : "Not complete"} />
            </div>
          </section>
        ))}
      </div>
      ...
      <section className="card">
        <div className="row" style={{ alignItems: "center", gap: 12 }}>
          <div>
            ...
            <p className="muted" style={{ margin: 0 }}>
              Review submitted onboarding packets, open private files, compare saved W-9 tax data, then approve to apply Crew and Tax records and archive the approved PDFs to Google Drive.
            </p>
          </div>
          <div className="toolbar">
