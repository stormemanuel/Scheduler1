...
const checklistColumns: Array<{ field: ChecklistField; label: string; helper: string }> = [
  ...
  { field: "week_before_confirmed", label: "Week-before schedule confirmation", helper: "Confirmed again one week before the shift" },
...
function positionFullDayRateForCall(call: Pick<SubCallRecord, "role_name" | "master_rate_id">, rateCity: string, masterRates: MasterRateRecord[]) {
  ...
  if (call.master_rate_id) {
    const linkedRate = masterRates.find((rate) => rate.id === call.master_rate_id);
    const linkedFullDay = Number(linkedRate?.full_day || 0);
    if (linkedFullDay > 0) return linkedFullDay;
  }

  const keys = roleKeys(call.role_name);
  const matchesPosition = (rate: MasterRateRecord, city: string) => {
    if (normalize(rate.city_name) !== normalize(city)) return false;
    const rateRole = normalize(rate.role_name);
    if (keys.has(rateRole)) return true;
    const rateKeys = roleKeys(rate.role_name);
  ...
  const configuredRate = masterRates.find((rate) => matchesPosition(rate, rateCity))
    ?? masterRates.find((rate) => matchesPosition(rate, "Default"));
...
function EventsClientInner({
  ...
  const checklistText = useMemo(() => {
    return checklistRows.map((row) => {
      ...
      return `${row.crewName}\n  ${sent} Sent schedule   ${confirmed} Confirmed   ${weekBefore} Week-before schedule confirmation   ${dayBefore} Day-before confirmation`;
    }).join("\n\n");
  }, [checklistRows]);
  ...
  function buildChecklistDocument(): ExportDocument | null {
    ...
    return {
      ...
      sections: [{
        heading: "Checklist",
        columns: ["Worker", "Phone", "First schedule", "Sent schedule", "Confirmed", "Week-before schedule confirmation", "Day-before confirmation"],
        rows: checklistRows.map((row) => [row.crewName, row.phone ? formatPhone(row.phone) : "", row.firstSchedule, row.checklist?.schedule_sent ? "YES" : "NO", row.checklist?.confirmed ? "YES" : "NO", row.checklist?.week_before_confirmed ? "YES" : "NO", row.checklist?.day_before_confirmed ? "YES" : "NO"]),
      }],
    };
  }
                ...
                <div className="list">
                  ...
                  <div className="row">
                    <div>
                      ...
                      <div className="small muted">Track all confirmation steps in one place: sent schedule, confirmed, week-before schedule confirmation, and day-before confirmation.</div>
                    </div>
                    <div className="toolbar">
