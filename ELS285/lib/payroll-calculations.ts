...
export function estimateAssignmentRevenue(options: {
  ...
  const rateFull = rateNumber(cityOverride?.full_day) ?? rateNumber(defaultOverride?.full_day) ?? rateNumber(clientRate?.full_day);
  const rateHalf = rateNumber(cityOverride?.half_day) ?? rateNumber(defaultOverride?.half_day) ?? rateNumber(clientRate?.half_day);
  const overtimeMultiplier = rateNumber(cityOverride?.overtime_multiplier) ?? rateNumber(defaultOverride?.overtime_multiplier) ?? rateNumber(clientRate?.overtime_multiplier) ?? 1.5;
  const doubletimeMultiplier = rateNumber(cityOverride?.doubletime_multiplier) ?? rateNumber(defaultOverride?.doubletime_multiplier) ?? rateNumber(clientRate?.doubletime_multiplier) ?? 2;
  const fallbackFull = rateNumber(fallback?.full_day);
  const fallbackHalf = rateNumber(fallback?.half_day);
  ...
  const source = rateNumber(cityOverride?.full_day)
    ...
    : rateNumber(defaultOverride?.full_day)
      ...
      : rateNumber(clientRate?.full_day)
        ? `${clientRate?.city_name || "Default"} client rate`
