...
function assignmentRateOverrideMap(rows: unknown[]) {
  const map = new Map<string, number>();
  for (const row of rows) {
    const typed = row as { assignment_id?: string | null; note_code?: string | null; custom_note?: string | null };
    if (safeText(typed.note_code) !== "assignment_rate_override") continue;
    const assignmentId = safeText(typed.assignment_id);
    const rate = Number(String(typed.custom_note || "").replace(/[^0-9.]/g, ""));
    if (assignmentId && !map.has(assignmentId) && Number.isFinite(rate) && rate > 0) map.set(assignmentId, rate);
  }
  return map;
}
...
export function buildPayrollRows(options: {
  ...
  assignmentRateOverrides?: Map<string, number>;
}) {
  ...
  const assignmentRateOverrides = options.assignmentRateOverrides ?? new Map<string, number>();
  ...
  for (const assignment of assignments) {
    try {
      ...
      const cleanCall: SubCallRecord = {
        ...
        // Preserve the role actually saved on the sub-call. A stale linked
        // master-rate row must not silently relabel custom work for Payroll.
        role_name: safeText(call.role_name) || safeText(linkedMasterRate?.role_name, "General AV"),
      ...
      let estimate = estimateAssignmentPay({ call: cleanCall, crew, masterRates, rateCity: safeText(show.rate_city, "Default"), fullDayRateOverride: assignmentRateOverrides.get(assignmentId) });
      ...
      if (!grouped.has(key)) {
        grouped.set(key, {
          ...
          paid: paymentStatus === "paid",
          paymentStatus,
          notes: stripPurchaseOrderFromNotes(status?.notes),
          purchaseOrder: extractPurchaseOrderFromNotes(status?.notes),
...
export async function getPayrollSummaryData(requestedYear?: number): Promise<PayrollPageData> {
  ...
  try {
    ...
    const summaryAssignmentIds = (assignmentsRes.data ?? []).map((row) => safeText((row as { id?: string }).id)).filter(Boolean);
    const assignmentRateNotesRes = await fetchChunked(
      "assignment_notes",
      "assignment_id, note_code, custom_note, created_at",
      "assignment_id",
    ...
    const crewRows = buildPayrollRows({
      ...
      assignmentRateOverrides: assignmentRateNotesRes.error ? new Map<string, number>() : assignmentRateOverrideMap(assignmentRateNotesRes.data ?? []),
    });
...
export async function getPayrollPageData(requestedYear?: number, options: { summaryOnly?: boolean; showId?: string } = {}): Promise<PayrollPageData> {
  ...
  try {
    ...
    const assignmentRateNotesRes = showIds.length
      ? await readClient.from("assignment_notes").select("assignment_id, note_code, custom_note, created_at").in("show_id", showIds).eq("note_code", "assignment_rate_override").order("created_at", { ascending: false }).limit(20000)
      : emptyResult;
