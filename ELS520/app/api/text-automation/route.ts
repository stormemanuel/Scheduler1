...
export async function revalidateActiveEventMessageQueue(
  ...
  for (const showId of targetShowIds) {
    ...
    for (const queued of rows) {
      ...
      if (!crewId || !crewAssignments.length || !hasCurrentDateAssignment || !hasUpcomingAssignment) {
      ...
      } else if (/^(3_day|day_before|day_of|daily_after_first_day)_/.test(key)) {
        if (!reminderBySender.has(senderName)) {
          const plan = await buildFullShowReminderPlan(admin, settings, bundle, senderName, { resendCancelled: true });
          reminderBySender.set(senderName, new Map(plan.map((row) => [`${row.crew_id}|${logicalReminderKey(row.reminder_key)}`, row])));
        }
        nextBody = reminderBySender.get(senderName)?.get(`${crewId}|${key}`)?.body || "";
      } else {
      ...
      if (nextBody !== safeText(queued.body)) {
        const update = await admin.from("text_message_queue").update({ body: nextBody, error: null }).eq("id", id).in("status", ACTIVE_QUEUE_STATUSES);
        if (update.error) throw new Error(update.error.message);
        refreshed += 1;
      }
    }
