...
-- ELS86 iPhone Shortcut intro text queue.
...
create index if not exists crew_intro_text_queue_status_scheduled_idx
  on public.crew_intro_text_queue(status, scheduled_for);

create index if not exists crew_intro_text_queue_crew_id_idx
  on public.crew_intro_text_queue(crew_id);
