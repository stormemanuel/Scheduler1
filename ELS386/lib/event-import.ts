...
function parseCityStateFromAddress(value: string) {
  ...
  const stateMatch = clean.match(/\b([A-Z]{2})\s*(?:\d{5}(?:-\d{4})?)?$/);
  ...
  return { city: titleCaseLoose(city), state: stateMatch[1].toUpperCase() };
}
...
function estimateCrewPool(city: string, state: string) {
  ...
  return aliases[cityKey] || (city && state ? city : city || "Default");
}
...
function parseEstimateCsv(text: string, overrides: Partial<ImportedEventData["show"]>): ImportedEventData | null {
  ...
  return {
    show: {
      ...
      venue: compactSpaces(overrides.venue || titleCaseLoose(venue)),
...
function extractEstimateBillShip(lines: string[]) {
  ...
  const billTo = extractFollowingLines(lines, /^Bill\s+to$/i, stopLabels, 4);
  const shipTo = extractFollowingLines(lines, /^Ship\s+to$/i, stopLabels, 4);
  if (billTo.length || shipTo.length) return { billTo, shipTo };
...
function reconstructEstimateRows(lines: string[]) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    while (
      ...
      if ((combined.match(/\$[\d,]+(?:\.\d{2})?/g) || []).length >= 2 && /\d{1,2}:\d{2}\s*(?:AM|PM)/i.test(combined)) break;
      lookahead += 1;
    }
...
function parseEstimateLaborLine(line: string, currentGroup: { area: string; boothNumber: string; poNumber: string }, fallbackYear: number): ImportedCrewRow | null {
  ...
  const currencyMatches = [...rest.matchAll(/\$[\d,]+(?:\.\d{2})?/g)];
  const timeMatches = [...rest.matchAll(/\d{1,2}:\d{2}\s*(?:AM|PM)/gi)];
  if (currencyMatches.length < 2 || timeMatches.length < 2) return null;

  const rateMatch = currencyMatches[currencyMatches.length - 2];
  const amountMatch = currencyMatches[currencyMatches.length - 1];
  const startRaw = timeMatches[0][0];
  const endRaw = timeMatches[1][0];
  const rateRaw = rateMatch[0];
  const amountRaw = amountMatch[0];
  const beforeRate = compactSpaces(rest.slice(0, rateMatch.index ?? rest.length));
  const qtyRaw = beforeRate.match(/(?:^|\s)(\d+(?:\.\d+)?)\s*$/)?.[1] || "1";
  let roleRaw = rest;
  roleRaw = roleRaw.replace(/\$[\d,]+(?:\.\d{2})?/g, " ");
  roleRaw = roleRaw.replace(/\d{1,2}:\d{2}\s*(?:AM|PM)/gi, " ");
  roleRaw = roleRaw.replace(/\s*\|\s*/g, " ");
  roleRaw = roleRaw.replace(new RegExp(`(?:^|\\s)${qtyRaw.replace(".", "\\.")}(?=\\s|$)`), " ");
...
function parseEstimatePdfText(text: string, overrides: Partial<ImportedEventData["show"]>): ImportedEventData | null {
  ...
  const client = billTo[0] || "";
  const venue = shipTo[0] || "";
  const streetAddress = shipTo[1] || "";
  const cityState = parseCityStateFromAddress(shipTo[2] || shipTo[shipTo.length - 1] || "");
  ...
  for (const rawLine of reconstructedRows) {
    ...
    if (!line || /^Subtotal/i.test(line) || /^Service DateProduct/i.test(line) || /^#$/i.test(line)) continue;

    const areaMatch = line.match(/^\d+\.\s+(?!\d{1,2}\/\d{1,2}\/\d{2,4})(.+)$/);
    if (areaMatch) {
      currentGroup.area = compactSpaces(areaMatch[1]);
    ...
    const boothMatch = line.match(/^Booth\s+(.+)$/i);
    if (boothMatch) {
