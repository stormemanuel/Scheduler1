...
const pageKeys: AppPageKey[] = ["overview", "events", "crew", "clients", "pipelines", "payroll", "users", "settings"];
...
function defaultPages(role: string): AppPageKey[] {
  ...
  if (role === "coordinator") return ["overview", "events", "crew"];
  return ["overview"];
}
...
export default async function UsersPage() {
  ...
  if (!admin) {
    ...
    const [profilesRes, accessRes, poolsRes] = await Promise.all([
      admin.from("profiles").select("id, email, full_name, role, is_active").order("created_at", { ascending: true }),
      admin.from("user_access_settings").select("user_id, allowed_pages, restrict_events_to_owner, restrict_crew_to_owner, allowed_city_pool_ids"),
    ...
    <div className="grid" style={{ gap: 16 }}>
      ...
      <section className="grid grid-3">
        ...
        <div className="card compact accent-card">
          ...
          <div className="small muted">Events/Crew access. Can be restricted to only the events and crew they create, plus allowed pools.</div>
        </div>
        <div className="card compact accent-card">
      ...
      <div className="grid grid-2" style={{ alignItems: "start" }}>
        <section className="card">
          <h2>Invite user</h2>
          <p className="muted small">Choose the role up front. You can fine-tune page and pool access after the invite is created.</p>
          <InviteUserForm />
        </section>
        ...
        <section className="card">
          ...
          <div className="list small muted">
            ...
            <p><strong>Coordinator:</strong> can be limited to only the events and crew they create. Pool access must be granted here.</p>
      ...
      <section className="card">
        <h2>Current users</h2>
        <p className="muted small">Edit the role, page access, and coordinator restrictions for each user. Save each card separately.</p>
        <div className="list">
              ...
              <form key={profile.id} action={async (formData) => {
                ...
                <div className="row">
                  ...
                  <p className="muted small" style={{ margin: 0 }}>
                    Salesman should normally keep only Sales Pipeline checked.
                  </p>
                </div>
              </form>
            );
