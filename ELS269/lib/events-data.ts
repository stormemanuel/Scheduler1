...
export async function getEventsPageData(options: { search?: string; showLimit?: number } = {}) {
  ...
  if (searchFilter) {
    ...
    showsQueryInitial = showsQueryInitial.or(activeOrFutureOrRecentFilter).limit(showLimit);
  }
  ...
  if (searchFilter) {
    ...
    fallbackShowsQuery = fallbackShowsQuery.or(activeOrFutureOrRecentFilter).limit(showLimit);
  }
