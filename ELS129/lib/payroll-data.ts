...
export async function getPayrollPageData(): Promise<PayrollPageData> {
  try {
    ...
    const [showsRes, laborDaysRes, subCallsRes, assignmentsRes, crewRes, positionsRes, cityPoolsRes, ratesRes, clientRatesRes, payrollRes, financialsRes, expenseItemsRes, profilesRes, coordinatorPayrollRes] = await Promise.all([
      ...
      supabase.from("sub_calls").select("id, labor_day_id, area, role_name, master_rate_id, start_time, end_time, crew_needed, notes, day_type, one_hour_walkaway").order("start_time", { ascending: true }).limit(20000),
    ...
    const subCalls = (subCallsRes.data ?? []).map((row) => {
      const typed = row as { id: string; labor_day_id: string; area: string | null; role_name: string | null; master_rate_id?: string | null; start_time: string | null; end_time: string | null; crew_needed: number | null; notes: string | null; day_type?: string | null; one_hour_walkaway?: boolean | null };
