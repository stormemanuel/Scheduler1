            ...
            <section key={row.show.id} className="card">
              ...
              <div className="mobile-table" style={{ overflowX: "auto", marginTop: 14 }}>
                <table style={{ width: "100%", borderCollapse: "collapse" }}>
                  <thead><tr style={{ textAlign: "left", borderBottom: "1px solid var(--line)" }}><th style={{ padding: 8 }}>Date</th><th style={{ padding: 8 }}>Subgroup / area</th><th style={{ padding: 8 }}>Role</th><th style={{ padding: 8 }}>Filled</th><th style={{ padding: 8 }}>Open</th><th style={{ padding: 8 }}>Block</th></tr></thead>
                  <tbody>
                    {row.calls.map((call: SubCallRecord) => {
                      ...
                      return <tr key={call.id} style={{ borderBottom: "1px solid var(--line)" }}><td style={{ padding: 8 }}>{day?.labor_date || "—"}</td><td style={{ padding: 8 }}>{call.area || "Unassigned"}</td><td style={{ padding: 8 }}>{call.role_name || "General AV"}</td><td style={{ padding: 8 }}>{assigned}/{needed}</td><td style={{ padding: 8 }}>{open}</td><td style={{ padding: 8 }}>{isHalfDay(call) ? "Half day" : "Full day"}</td></tr>;
                    })}
                    {row.calls.length === 0 ? <tr><td colSpan={6} className="muted" style={{ padding: 12 }}>No sub-calls created yet.</td></tr> : null}
                  </tbody>
                </table>
              </div>
            </section>
