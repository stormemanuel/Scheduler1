...
export default function PayrollClient({ initialRows, initialEventSummaries, availableYears, initialYear, initialSummaryLoaded, initialError, initialDiagnostics = null }: Props) {
  ...
  const totalScopeLabel = summaryFailed
    ? "Payroll summary could not load. Open diagnostics below, then retry."
    : summaryRequestCompleted
    ? `${summaryEvents.length} summarized show${summaryEvents.length === 1 ? "" : "s"} · ${loadedEventCount} detail${loadedEventCount === 1 ? "" : "s"} opened`
    : "Payroll summary is loading";
  const summaryMoney = (value: number) => summaryFailed ? "Could not load" : summaryRequestCompleted ? money(value) : "Loading...";
  ...
  async function selectPayrollYear(nextYear: number, force = false) {
    ...
    try {
      slowNoticeTimer = window.setTimeout(() => {
        setMessage(`Still loading ${nextYear} payroll… The request is continuing and will not be cancelled.`);
      }, 8000);
      const response = await fetchPayrollRequest(`/api/payroll?mode=summary&year=${encodeURIComponent(String(nextYear))}`);
      ...
      if (!response.ok) throw new Error(result.message || result.error || `Unable to load ${nextYear} payroll (HTTP ${response.status}).`);
      ...
      setYearErrors((current) => {
        const next = { ...current };
        if (result.error) next[returnedYear] = result.error;
      ...
      const nextMessage = error instanceof Error ? error.message : `Unable to load ${nextYear} payroll.`;
...
(): Array<Array<string | number | null | undefined>> {
  ...
  if (rows.length === 0 && !loadedYears.has(year) && loadingYear === year) {
    return (
