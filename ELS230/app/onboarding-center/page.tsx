...
type ReviewRow = {
  ...
  crew_id: string;
  status: string;
  request_type: "full_onboarding" | "w9_only";
  submitted_at: string;
  updated_at: string;
...
export default function OnboardingCenterPage() {
  ...
  async function loadRows() {
    ...
    try {
      ...
      const result = (await response.json()) as { ok?: boolean; rows?: ReviewRow[]; message?: string };
          ...
          <section key={row.id} className="card">
            <div className="row" style={{ alignItems: "flex-start", gap: 14 }}>
              <div>
                ...
                <p className="muted small" style={{ margin: "4px 0 0" }}>
                  {row.request_type === "w9_only" ? "W-9 only" : "Full onboarding"} • Submitted {text(row.submitted_at) || "unknown"} • Request {statusBadge(row.status)}
                </p>
              </div>
              <div className="toolbar">
                <button className="primary" type="button" disabled={isApproved || busyId === row.id} onClick={() => void approveRow(row)}>
                  {isApproved ? "Approved" : busyId === row.id ? "Approving..." : "Approve + apply to Crew/Tax"}
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-3" style={{ marginTop: 14 }}>
              <div className="card compact" style={{ background: "#fbfcfd" }}>
                ...
                <InfoLine label="Address" value={payload.address || crew.address} />
                <InfoLine label="City/State" value={payload.city_state} />
              ...
              <div className="card compact" style={{ background: "#fbfcfd" }}>
                ...
                {[
                  ...
                  <div key={String(kind)} className="card compact" style={{ background: "#fff", marginBottom: 8 }}>
                    <strong>{label}</strong>
                    <div className="muted small" style={{ overflowWrap: "anywhere" }}>{text(path) || "Missing"}</div>
                ...
                {workPhotos.length ? (
                  <div className="card compact" style={{ background: "#fff" }}>
                    ...
                    <div className="toolbar" style={{ marginTop: 8 }}>
                      {workPhotos.slice(0, 4).map((path, index) => (
                        <button key={`${path}-${index}`} className="ghost" type="button" disabled={Boolean(busyId)} onClick={() => void openSecureDocument(row, "work_photo", path, false)}>
                          Open {index + 1}
                        </button>
                      ))}
                    </div>
