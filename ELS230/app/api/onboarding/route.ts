...
function buildSignedContractPdf(
  ...
  function checkbox(checked: boolean, text: string) {
    line(`${checked ? "[X]" : "[ ]"} ${text}`, 9.4, "F1", 13);
  }

  line("INDEPENDENT CONTRACTOR AGREEMENT", 16, "F2", 22);
  paragraph(
    `This Independent Contractor Agreement (Agreement) is made and entered into on ${contract.effective_date}, by and between Emanuel Labor Services LLC, a Louisiana limited liability company (the Company), and ${contract.contractor_name || fallbackName} (the Contractor).`,
  );
  paragraph(
  ...
  textLine(contract.contractor_name || fallbackName, margin, y - 18, 28, "F3");
  y -= 52;
  line(`Contractor name: ${contract.contractor_name || fallbackName}`, 9.5, "F1", 13);
  line(`Date signed: ${contract.effective_date}`, 9.5, "F1", 13);
  line(`Submitted through ELS secure onboarding: ${submittedAt}`, 8.5, "F1", 12);
  line("Company Representative (Emanuel Labor Services LLC): ________________________________   Date: ________________", 9.5, "F1", 14);
...
function buildDigitalW9Pdf(w9: DigitalW9Payload, submittedAt: string) {
  ...
  textLine(w9.signer_name, margin + 4, y - 26, 26, "F3");
...
async function approveOnboardingSubmission(admin: SupabaseAdmin, body: Record<string, unknown>) {
  ...
  const crewRes = await admin
    .from("crew")
    .select("id, name, notes, work_photo_urls, profile_photo_url, w9_document_url, contract_document_url, onboarding_status")
    .eq("id", crewId)
    .maybeSingle();
  if (crewRes.error) throw new Error(crewRes.error.message);
  ...
  const existingNotes = safeText(existingCrew.notes);
  const approvalNote = [
  ...
  const patch: Record<string, unknown> = {
    onboarding_status: requestType === "w9_only" ? safeText((existingCrew as { onboarding_status?: string }).onboarding_status) || "not_started" : "approved",
    questionnaire_status: requestType === "w9_only" ? undefined : "approved",
    w9_status: "approved",
    tax_profile_status: "approved",
    onboarding_successfully_onboarded: requestType !== "w9_only" ? true : undefined,
    onboarding_response: true,
    onboarding_paperwork_sent: requestType !== "w9_only" ? true : undefined,
    profile_photo_url: profilePhoto || undefined,
    w9_document_url: w9Document,
    contract_document_url: contractDocument || undefined,
    contract_status: requestType === "w9_only" ? undefined : "approved",
    onboarding_completed_at: requestType !== "w9_only" ? approvedAt : undefined,
  ...
  try {
    ...
    if (action === "submit") {
      ...
      const onboardingNote = [
        ...
        payload.work_photo_urls.length
          ? `Work photos uploaded: ${payload.work_photo_urls.length}`
          : "",
        w9UseDigital ? "Digital substitute W-9 completed in app: yes" : "",
        w9UseDigital
          ? `TIN: ${digitalW9.tin_type.toUpperCase()} ending ${digitalW9.tin_last4}`
        ...
        payload.profile_photo_note
          ...
          : "",
        payload.work_photo_note
          ? `Work photo note: ${payload.work_photo_note}`
          : "",
        payload.w9_status_note ? `W-9 note: ${payload.w9_status_note}` : "",
      ...
      const existingWorkPhotos = (
        currentCrewRes.data as { work_photo_urls?: string[] | null } | null
      )?.work_photo_urls;
      const updatePayload: Record<string, unknown> = {
      ...
      if (payload.work_photo_urls.length)
        updatePayload.work_photo_urls = mergePathLists(
          existingWorkPhotos,
          payload.work_photo_urls,
        );
      if (w9UseDigital) {
        updatePayload.w9_status = "uploaded";
