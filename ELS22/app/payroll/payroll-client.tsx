...
export default function PayrollClient({ initialRows, availableYears, initialError, initialLoading = false }: Props) {
  const [rows, setRows] = useState(initialRows);
  const [availableYearOptions, setAvailableYearOptions] = useState(availableYears);
  ...
  const [loadedYear, setLoadedYear] = useState<number | null>(initialLoading ? null : (availableYears[0] ?? new Date().getFullYear()));
  ...
  useEffect(() => {
    ...
    async function loadPayroll() {
      ...
      setRows([]);
      try {
        const response = await fetch(`/api/payroll?year=${encodeURIComponent(String(year))}`, { cache: "no-store" });
        ...
        if (!response.ok || !result.ok) throw new Error(result.message || result.error || "Payroll load failed.");
        if (cancelled) return;
        setRows(result.crewRows ?? []);
        setAvailableYearOptions((result.availableYears ?? []).length ? (result.availableYears ?? []) : [year]);
        setLoadedYear(year);
        if (result.setupMissing) setMessage("Supabase environment variables are missing, so Payroll cannot load yet.");
        else setMessage(result.error || "");
      } catch (error) {
        if (!cancelled) setMessage(error instanceof Error ? error.message : "Payroll load failed.");
      } finally {
  ...
  }, [year, initialLoading, loadedYear]);
  ...
  function exportOwnerPLWithReserves() {
    downloadCsv(`ELS_${year}_owner_PL_with_reserves.csv`, [
      ...
      [],
      ["Show", "Client", "Venue", "Dates", "Estimated Revenue", "Contract Labor", "Expenses", "Net Profit", "Tax Reserve", "Consecrated Hands", "Tax + Consecrated Hands", "Pure Profit", "Financial Notes"],
      ...events.map((event) => [
        event.showName,
        event.showClient,
        event.showVenue,
  ...
  return (
    <div className="grid" style={{ gap: 16 }}>
      {message ? <p className={message.toLowerCase().includes("failed") || message.toLowerCase().includes("run supabase") || message.toLowerCase().includes("missing") ? "error" : "success"}>{message}</p> : null}
      {loading ? <p className="muted">Loading payroll for {year}… The page is open; large imports will load in the background.</p> : null}
      ...
      <section className="card accent-card">
        <div className="row" style={{ alignItems: "center" }}>
          ...
          <div className="toolbar" style={{ justifyContent: "flex-end" }}><button className="ghost" type="button" onClick={exportOwnerPL}>Export Company P&L</button><button className="ghost" type="button" onClick={exportOwnerPLWithReserves}>Optional reserve export</button><button className="ghost" type="button" onClick={export1099Prep}>Export 1099-NEC Prep</button></div>
        </div>
        ...
        <div className="toolbar" style={{ marginTop: 16 }}>
          <label className="field" style={{ minWidth: 150 }}>
            ...
            <select value={year} onChange={(event) => setYear(Number(event.target.value))}>
              {availableYearOptions.map((item) => <option key={item} value={item}>{item}</option>)}
            </select>
          </label>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              ...
              {yearSummary.length === 0 ? (
                <tr><td colSpan={6} className="muted" style={{ padding: 16 }}>{loading ? `Loading ${year} payroll totals...` : `No assigned crew found for ${year}.`}</td></tr>
              ) : null}
            </tbody>
          </table>
        </div>
      ...
      <div className="list">
        ...
        {events.length === 0 ? <section className="card"><p className="muted">{loading ? `Loading ${year} event payroll...` : "No event payroll rows match the current filters."}</p></section> : null}
      </div>
    </div>
  );
}
