...
const PAYROLL_IN_CHUNK_SIZE = 200;

function chunkValues(values: string[], size = PAYROLL_IN_CHUNK_SIZE) {
  const clean = [...new Set(values.map((value) => safeText(value)).filter(Boolean))];
  const chunks: string[][] = [];
  for (let index = 0; index < clean.length; index += size) chunks.push(clean.slice(index, index + size));
  return chunks;
}

async function selectInChunks<T>(options: {
  supabase: any;
  table: string;
  select: string;
  column: string;
  ...
  const chunks = chunkValues(options.values);
  if (chunks.length === 0) return { data: [], error: null };

  const rows: T[] = [];
  for (const chunk of chunks) {
    let query = options.supabase.from(options.table).select(options.select).in(options.column, chunk);
    if (options.order) query = query.order(options.order.column, { ascending: options.order.ascending });
    if (options.limitPerChunk) query = query.limit(options.limitPerChunk);
...
export async function getPayrollPageData(targetYear?: number): Promise<PayrollPageData> {
  try {
    ...
    const selectedYear = Number.isFinite(Number(targetYear)) && Number(targetYear) > 1900 ? Number(targetYear) : new Date().getFullYear();

    // Load the small reference tables first. Payroll no longer loads every crew,
    // assignment, labor day, and sub-call in the database before the page opens.
    const [showsRes, cityPoolsRes, ratesRes, clientRatesRes] = await Promise.all([
    ...
    const firstError = showsRes.error || cityPoolsRes.error || ratesRes.error || (clientRatesMissing ? null : clientRatesRes.error);
    if (firstError) {
      return {
        eventSummaries: [],
        crewRows: [],
    ...
    const selectedShows = shows.filter((show) => showYear(show.show_start) === selectedYear);
    ...
    if (selectedShowIds.length === 0) {
      return {
        eventSummaries: [],
        crewRows: [],
        availableYears: years.length ? years : [selectedYear],
        setupMissing: false,
        error: null,
      };
    }

    const laborDaysRes = await selectInChunks<unknown>({
      supabase,
      table: "labor_days",
      select: "id, show_id, labor_date, label, notes",
      column: "show_id",
      values: selectedShowIds,
      order: { column: "labor_date", ascending: true },
    });
    ...
    if (laborDaysRes.error) {
      return {
        ...
        availableYears: years.length ? years : [selectedYear],
        setupMissing: false,
        error: laborDaysRes.error.message,
      };
    }
    ...
    const laborDayIds = laborDays.map((day) => day.id);
    const subCallsRes = await selectInChunks<unknown>({
      supabase,
      table: "sub_calls",
    ...
    if (subCallsRes.error) {
      return {
        ...
        availableYears: years.length ? years : [selectedYear],
        setupMissing: false,
        error: subCallsRes.error.message,
      };
    }
    ...
    const subCallIds = subCalls.map((call) => call.id);
    const assignmentsRes = await selectInChunks<unknown>({
      supabase,
      table: "assignments",
    ...
    if (assignmentsRes.error && !assignmentsMissing) {
      return {
        eventSummaries: [],
        crewRows: [],
        availableYears: years.length ? years : [selectedYear],
    ...
    const crewIds = assignments.map((assignment) => assignment.crew_id);

    const [crewRes, positionsRes, payrollRes, financialsRes] = await Promise.all([
      selectInChunks<unknown>({
        supabase,
        table: "crew",
      ...
      selectInChunks<unknown>({
        ...
        select: "id, crew_id, role_name, rate",
        column: "crew_id",
        values: crewIds,
    ...
    const secondError = crewRes.error || positionsRes.error || (payrollMissing ? null : payrollRes.error) || (financialsMissing ? null : financialsRes.error);
