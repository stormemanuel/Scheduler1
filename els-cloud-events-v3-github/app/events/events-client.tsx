...
export default function EventsClient({ initialShows, initialLaborDays, initialSubCalls, masterRates }: Props) {
  const [shows, setShows] = useState(initialShows);
  const [laborDays, setLaborDays] = useState(initialLaborDays);
  const [subCalls, setSubCalls] = useState(initialSubCalls);
  const [search, setSearch] = useState("");
  const [selectedShowId, setSelectedShowId] = useState(initialShows[0]?.id ?? null);
  const [selectedDayId, setSelectedDayId] = useState(initialLaborDays[0]?.id ?? null);
    ...
    <div className="grid" style={{ gap: 16 }}>
      ...
      <div className="grid" style={{ gridTemplateColumns: '280px 340px minmax(0,1fr)', gap: 16 }}>
        <section className="card">
          ...
          <p className="muted small" style={{ marginTop: 0 }}>Create shows, set the span, and choose the rate city used for crew pay estimates.</p>
          <label className="field">
            <span>Main search</span>
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Show, client, venue, city..." />
          </label>
          <div className="list" style={{ marginTop: 14, maxHeight: 420, overflowY: 'auto' }}>
            {filteredShows.map((show) => (
              <button key={show.id} type="button" className="ghost" onClick={() => { setSelectedShowId(show.id); setSelectedDayId(null); }} style={{ textAlign: 'left', borderColor: selectedShow?.id === show.id ? '#111827' : undefined }}>
                ...
                <div className="small muted">{showBucket(show)} • Rate city: {show.rate_city || 'Default'}</div>
              </button>
            ))}
          </div>
        ...
        <section className="card">
          ...
          <p className="muted small" style={{ marginTop: 0 }}>Inside a show, keep labor days sorted by date so the show span stays organized.</p>
          {selectedShow ? <p className="small"><strong>{selectedShow.name}</strong> • Estimated crew payout: <strong>${estimate.toLocaleString()}</strong></p> : <p className="muted small">Select a show first.</p>}
        ...
        <section className="card">
          ...
          <p className="muted small" style={{ marginTop: 0 }}>Each labor day can hold multiple areas and times. Sub-calls always sort by start time.</p>
