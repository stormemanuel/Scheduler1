        ...
        {selectedClient ? (
          <div className="card">
            ...
            <div className="card compact" style={{ marginTop: 14 }}>
              ...
              <p className="small muted">
                This separates the client rating system by city/state crew pool.
                Techs with multiple pool memberships can appear under each
                approved pool.
              </p>
              {poolTopTechs.length ? (
                <div className="grid grid-2" style={{ gap: 12 }}>
                  {poolTopTechs.map((group) => (
                    <div
                      key={group.poolName}
                      className="card compact"
                    ...
                    >
                      <div className="row" style={{ alignItems: "baseline" }}>
                        ...
                        <span className="badge">
                          {group.totalRatings} rating
                          {group.totalRatings === 1 ? "" : "s"}
                        </span>
                      </div>
                      {group.items.length ? (
                        ...
                        <p className="small muted">
                          No rated techs in this pool yet.
                        </p>
                      )}
                    </div>
                  ))}
            ...
            <div className="card compact" style={{ marginTop: 14 }}>
              <h4 style={{ marginTop: 0 }}>
                Project Manager / Contact Top Techs
              </h4>
              <p className="small muted">
              ...
              {selectedContacts.length ? (
                  ...
                  {selectedContacts.map((contact) => {
                    ...
                    return (
                      <div key={contact.id} className="card compact">
                        ...
                        >
                          <strong>
                            Top Techs by City/State Pool for {contact.name}
                          </strong>
                          <p className="small muted" style={{ marginTop: 4 }}>
                            This is the same project-manager list separated by
                            the tech&apos;s crew pool.
                          </p>
                          {(contactPoolTopTechs.get(contact.id) || [])
                            .length ? (
                            <div
                              className="grid grid-2"
                              style={{ gap: 10, marginTop: 8 }}
                            >
                              {(contactPoolTopTechs.get(contact.id) || []).map(
                                (group) => (
                                  <div
                                    key={group.poolName}
                                    className="card compact"
                            ...
                            <p
                              ...
                              style={{ marginBottom: 0 }}
                            >
                              No project-manager pool ratings yet.
                            </p>
                          )}
                        </div>
                      </div>
                ...
                <p className="small muted">
                  Add client contacts, then choose one on an event so
                  contact-specific Top Techs can build.
                </p>
              )}
            </div>
          </div>
