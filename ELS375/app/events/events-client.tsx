...
function checklistMessageStatusLabel(row: TextMessageQueueRecord | null | undefined, fallback: string) {
  ...
  if (row.status === "scheduled") return `${queueDueLabel(row)} · queued by ${sender}`;
...
function EventsClientInner({
  ...
  function sameSubCallSeriesPattern(a: SubCallRecord, b: SubCallRecord) {
    if (normalize(a.area) !== normalize(b.area)) return false;
    const exactRateMatches = Boolean(a.master_rate_id && b.master_rate_id && a.master_rate_id === b.master_rate_id);
    return exactRateMatches || normalize(a.role_name) === normalize(b.role_name);
  }
  ...
  function scheduleMessageCountsAsSent(row: TextMessageQueueRecord) {
    ...
    return ["scheduled", "queued", "waiting", "due_now", "sent"].includes(status);
  }
  ...
  async function scheduleCheckedReminderPlan(includeOverdueNow = false) {
    ...
    if (saved) {
      setTextAutomationSettings((current) => [...current.filter((row) => row.show_id !== selectedShow.id), saved]);
      setAutomationDraft((current) => ({ ...current, ...automationDraftFromRecord(selectedShow.id, saved), reminder_daily_after_first_day: current.reminder_daily_after_first_day }));
    }
...
<body>
  ...
  async function saveCall() {
    ...
    if (editingCallId) {
      ...
      for (const targetCall of targetCalls) {
        const payload = { ...mainPayloadForExistingCall(targetCall), labor_day_id: targetCall.labor_day_id, sort_order: Number(targetCall.sort_order || 0) };
      ...
      for (const row of multiPositionRows) {
        for (const dayId of extraPositionDayIds(row, legacyExtraDayIds)) {
          ...
          knownKeys.add(key);
          extraItems.push({ ...extraPayload, labor_day_id: dayId });
        }
      }
      const addMoreItems = addDayIds.map((dayId) => ({ ...mainPayloadForNewDay(dayId), labor_day_id: dayId }));
      ...
      const extraItems = multiPositionRows.flatMap((row) =>
        extraPositionDayIds(row, targetDayIds).map((dayId) => ({ ...callPayloadFromPosition(row, dayId), labor_day_id: dayId }))
      );
      const items = [
        ...targetDayIds.map((dayId) => ({ ...mainPayloadForNewDay(dayId), labor_day_id: dayId })),
        ...extraItems,
      ];
  ...
  function renderAutomationPanel() {
    ...
    const senderQueueSummary = Array.from(selectedTextQueue.reduce((map, row) => {
      ...
      const current = map.get(senderId) ?? { senderId, senderName, scheduled: 0, dueNow: 0, waiting: 0, sent: 0, failed: 0, cancelled: 0, total: 0 };
      current.total += 1;
      ...
      return map;
    }, new Map<string, { senderId: string; senderName: string; scheduled: number; dueNow: number; waiting: number; sent: number; failed: number; cancelled: number; total: number }>()).values())
      .sort((a, b) => b.scheduled - a.scheduled || b.sent - a.sent || a.senderName.localeCompare(b.senderName));
    const senderRepairStatuses = new Set(["scheduled", "waiting", "queued", "due_now", "pending", "needs_sender_review"]);
    ...
    const currentSenderWaiting = currentSenderId
      ...
      : 0;
    const legacyClaimableRows = selectedTextQueue.filter((row) => senderRepairStatuses.has(normalize(row.status)) && !safeText(row.queued_by_user_id));
    const legacyClaimableDueNow = legacyClaimableRows.filter((row) => normalize(row.status) !== "scheduled" || Date.parse(row.scheduled_for) <= nowMs).length;
        ...
        <div className="card compact">
          <div className="row" style={{ alignItems: "flex-start" }}>
            <div>
              ...
              <div className="small muted">Scheduled: {scheduled} • Due now: {dueNow} • Waiting: {waitingLater} • Custom queued: {customQueued}{customQueued ? ` (${customDueNow} due now)` : ""} • Sent: {sent} • Failed: {failed}{cancelled ? ` • Cancelled: ${cancelled}` : ""}{needsSenderReview ? ` • Sender review: ${needsSenderReview}` : ""}</div>
                ...
                <div className="toolbar" style={{ marginTop: 8, gap: 8 }}>
                  {senderQueueSummary.map((sender) => (
                    <span key={sender.senderId} className="badge" style={{ fontWeight: 800 }}>
                      {sender.senderName}: {sender.scheduled} queued{sender.dueNow ? ` (${sender.dueNow} due)` : ""} • {sender.sent} sent{sender.failed ? ` • ${sender.failed} failed` : ""}{sender.cancelled ? ` • ${sender.cancelled} cancelled` : ""}
                    </span>
                  ))}
                </div>
              ) : null}
                ...
                <div className="toolbar" style={{ marginTop: 12 }}>
                  <label className="field" style={{ minWidth: 260, padding: 10, border: "2px solid #d4a62a", borderRadius: 14, background: "#fff8db", boxShadow: "0 10px 24px rgba(212, 166, 42, 0.16)" }}>
                    ...
                    <select value={eventDisplayMode} onChange={(e) => setEventDisplayMode(e.target.value as EventDisplayMode)}>
                      ...
                      {!selectedShowHasPartialCoordinatorAccess ? <option value="automation">Send messages and reminders</option> : null}
