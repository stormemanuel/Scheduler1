...
type SubCallRow = { id: string; labor_day_id: string; area: string | null; location?: string | null; po_number?: string | null; sub_call_reference_number?: string | null; role_name: string | null; master_rate_id?: string | null; message_rate?: string | number | null; start_time: string | null; end_time: string | null; crew_needed: number | null; notes: string | null; sort_order?: number | null; day_type?: string | null; one_hour_walkaway?: boolean | null };
...
function scheduleBoothNumber(call: SubCallRow) {
  const reference = safeText(call.sub_call_reference_number);
...
async function fetchShowBundle(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, showId: string) {
  ...
  const callsRes = dayIds.length ? await admin.from("sub_calls").select("id, labor_day_id, area, location, po_number, sub_call_reference_number, role_name, master_rate_id, message_rate, start_time, end_time, crew_needed, notes, sort_order, day_type, one_hour_walkaway").in("labor_day_id", dayIds) : { data: [], error: null };
...
type ExistingQueueState = { id: string | null; status: string; reminder_key: string; queued_by_user_id?: string | null };
...
async function buildFullShowReminderPlan(
  ...
  for (const row of (existingRes.data || []) as Array<{ id?: string | null; crew_id: string | null; message_type: string | null; reminder_key: string | null; status: string | null; queued_by_user_id?: string | null }>) {
    const logicalKey = `${row.crew_id || ""}|${row.message_type || ""}|${logicalReminderKey(row.reminder_key || "")}`;
    existing.set(logicalKey, chooseExistingQueueState(existing.get(logicalKey), { id: row.id || null, status: row.status || "scheduled", reminder_key: row.reminder_key || "", queued_by_user_id: row.queued_by_user_id || null }));
  }
...
async function buildImmediateFullShowSchedulePlan(
  ...
  for (const row of (existingRes.data || []) as Array<{ id?: string | null; crew_id: string | null; message_type: string | null; reminder_key: string | null; status: string | null; queued_by_user_id?: string | null }>) {
    ...
    if (logicalReminderKey(key) === "full_show_schedule" || key.startsWith("manual_7_day")) {
      ...
      existing.set(logicalKey, chooseExistingQueueState(existing.get(logicalKey), { id: row.id || null, status: row.status || "scheduled", reminder_key: key, queued_by_user_id: row.queued_by_user_id || null }));
    }
  }
