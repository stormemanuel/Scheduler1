...
export async function middleware(request: NextRequest) {
  ...
  const isPwaAsset =
    ...
    (pathname === "/api/user-access" && ["manifest", "app_icon"].includes(request.nextUrl.searchParams.get("action") || ""));
  ...
  if (previewUserId) {
    ...
    const isPreviewCoordinatorChecklistRead =
      request.method === "GET" &&
      pathname === "/api/assignment-checklists" &&
      request.nextUrl.searchParams.get("scope") === "coordinator_process";
    if (pathname.startsWith("/api/") && !isPreviewControl && !isPreviewOnboardingRead && !isPreviewCoordinatorChecklistRead) {
