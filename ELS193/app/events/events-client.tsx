...
function EventsClientInner({
  ...
  function buildCrewListDocument(): ExportDocument | null {
    ...
    const boothSubCallSections = boothSections.map((section): ExportDocument["sections"][number] => {
      ...
      const rows = entries.reduce<ExportCell[][]>((acc, { day, call }) => {
        const currentDate = day.labor_date;
        const previousRow = acc[acc.length - 1];
        const previousDate = previousRow?.[0] === "" ? "" : safeText(previousRow?.[1]).split(" • ")[0];
        if (acc.length && previousDate && currentDate && previousDate !== currentDate) {
          acc.push(["", "", "", "", "", ""]);
        }
        acc.push(...buildRows(call, currentDate));
        return acc;
      }, []);
      return {
  ...
  function callPayloadFromPosition(row: MultiPositionDraft) {
    ...
    return {
      ...
      notes: [callForm.notes.trim(), row.notes.trim()].filter(Boolean).join("\n\n"),
    };
  }
        ...
        <section className="card">
          ...
          {selectedShow ? (
            <div className="grid" style={{ gap: 16 }}>
              ...
              <div className="card compact" style={{ borderLeft: "4px solid #d4a62a", boxShadow: "none" }}>
                <div className="row" style={{ alignItems: "flex-start" }}>
                  <div>
                    <strong>Lead full-day / half-day summary</strong>
                    <div className="small muted">Quick view for the lead to see who is full day and who is leaving on a half day.</div>
                  </div>
                  <div className="toolbar">
                    <span className="badge">Full day: {leadBlockSummary.full.length}</span>
                    <span className="badge">Half day: {leadBlockSummary.half.length}</span>
                    {leadBlockSummary.other.length ? <span className="badge">Hourly/custom: {leadBlockSummary.other.length}</span> : null}
                  </div>
                </div>
                <div className="grid grid-2" style={{ marginTop: 10 }}>
                  {[
                    { label: "Full day crew", rows: leadBlockSummary.full },
                    { label: "Half day / leaving early", rows: leadBlockSummary.half },
                  ].map((group) => (
                    <div key={group.label} className="card compact" style={{ boxShadow: "none" }}>
                      <strong>{group.label}</strong>
                      {group.rows.length ? (
                        <div className="list" style={{ marginTop: 8 }}>
                          {group.rows.map((row) => (
                            <div key={row.key} className="small" style={{ borderBottom: "1px solid var(--line)", paddingBottom: 6 }}>
                              <strong>{row.crewName}</strong>
                              <div className="muted">{row.date} • {row.time} • {row.area}</div>
                              <div className="muted">{row.role} • {row.blockLabel}</div>
                            </div>
                          ))}
                        </div>
                      ) : (
                        <p className="small muted" style={{ marginBottom: 0 }}>No assigned {group.label.toLowerCase()} yet.</p>
                      )}
                    </div>
                  ))}
                </div>
                {leadBlockSummary.other.length ? (
                  <div className="card compact" style={{ boxShadow: "none", marginTop: 10 }}>
                    <strong>Hourly / custom time crew</strong>
                    <div className="list" style={{ marginTop: 8 }}>
                      {leadBlockSummary.other.map((row) => (
                        <div key={row.key} className="small" style={{ borderBottom: "1px solid var(--line)", paddingBottom: 6 }}>
                          <strong>{row.crewName}</strong>
                          <div className="muted">{row.date} • {row.time} • {row.area}</div>
                          <div className="muted">{row.role} • {row.blockLabel}</div>
