...
async function serveCrewProfilePhoto(auth: Awaited<ReturnType<typeof authContext>>, crewId: string) {
  ...
  const { data: crew, error } = await admin
    .from("crew")
    .select("id, profile_photo_url, created_by, city_pool_id, onboarding_status, coordinator_hidden_at, coordinator_hidden_by")
    .eq("id", crewId)
    .maybeSingle();
  ...
  if (!isOwnerAdmin(auth.role)) {
    ...
    const typed = crew as { created_by?: string | null; city_pool_id?: string | null; coordinator_hidden_at?: string | null; coordinator_hidden_by?: string | null };
