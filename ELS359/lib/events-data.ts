...
export async function getEventsPageData(options: { search?: string; showLimit?: number; includeLatestPendingFeedback?: boolean; pastOnly?: boolean; pastOffset?: number } = {}) {
  const session = await getSessionUser();
  const role = normalizeRole(session.profile?.role as string | null | undefined);
