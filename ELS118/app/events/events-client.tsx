...
import { exportDocumentDocxQueued, exportDocumentPdfQueued, type ExportDocument } from "@/lib/export-documents";
...
const automationReminderOptions: Array<{ key: AutomationReminderKey; label: string; helper: string }> = [
  ...
  { key: "reminder_day_of", label: "Day-of morning reminder · 2 hours before call", helper: "Short same-day message: Good Morning, we are meeting at the meet-up location. See you soon." },
];
...
function EventsClientInner({
  ...
  useEffect(() => {
    ...
    try {
      ...
      const validDisplayModes: EventDisplayMode[] = ["day", "booth", "messages", "automation", "checklist", "feedback", "notes", "ratings"];
  ...
  const assignmentCountByCallId = useMemo(() => {
    ...
    assignments.forEach((assignment) => map.set(assignment.sub_call_id, (map.get(assignment.sub_call_id) || 0) + 1));
    return map;
  }, [assignments]);
  ...
  const assignableCoordinatorUsers = useMemo(() => appUsers.filter((user) => user.is_active && ["coordinator", "admin"].includes(normalize(user.role))), [appUsers]);
    ...
    return [...byCrew.entries()]
      .map(([crewId, item]) => {
        ...
        const scheduleLines = sortedCalls.map(({ day, call, assignment }) => {
          ...
          const blockText = [dayTypeLabel(assignmentDayType(assignment, call)), call.one_hour_walkaway ? "1 hr walkaway lunch" : ""].filter(Boolean).join(" / ");
          return `${formatMessageDate(day.labor_date)} – ${formatAssignmentMessageTimeRange(assignment, call)} – ${call.role_name} – ${boothLabel(call.area)}${locationText} – ${blockText} – ${messageRateLabelForCall(item.crew, call, rateCity, liveMasterRates).replace(/^Rate:\s*/i, "")}`;
        });
  ...
  const feedbackForms = useMemo(() => {
    ...
    return forms;
  }, [selectedShow, selectedBusinessClient, selectedClientContact, selectedCoordinatorContact, selectedShowMessageMeta.crew_lead_name, boothSections, ratingRows]);
      ...
      <div key={call.id} className="card compact sub-call-card">
        <div className="row" style={{ alignItems: "flex-start" }}>
          <div>
            ...
            <div className="small muted">{call.area} • {call.role_name}</div>
            <div className="small muted">{[dayTypeLabel(call.day_type), walkawayLabel(call), call.location ? `Location: ${call.location}` : "", call.message_rate ? `Message rate: $${call.message_rate}/hr` : ""].filter(Boolean).join(" • ")}</div>
  ...
  async function refreshMasterRatesFromServer(showToast = false) {
    ...
    try {
      const res = await fetch("/api/master-rates", { cache: "no-store" });
      const data = await res.json().catch(() => ({}));
      if (!res.ok) throw new Error(data.message || "Could not refresh positions.");
      setLiveMasterRates((data.masterRates ?? []) as MasterRateRecord[]);
      if (showToast) setMsg({ kind: "success", text: "Positions refreshed from Crew pay rates." });
    } catch (error) {
  ...
  async function saveAutomationSettings() {
    ...
    if (saved?.enabled && saved.sending_method === "shortcut" && saved.send_schedule) {
      const queueData = await request("/api/text-automation", "POST", { action: "queue_messages", show_id: selectedShow.id, mode: "schedule_reminders", settings: saved });
  ...
  async function queueManualDayReminderNow() {
    ...
    const manualSettings = { ...automationDraft, show_id: selectedShow.id, enabled: true, send_schedule: true };
    const data = await request("/api/text-automation", "POST", { action: "queue_manual_day_reminder", show_id: selectedShow.id, labor_day_id: laborDayId, settings: manualSettings });
    const queued = (data.queue || []) as TextMessageQueueRecord[];
    if (queued.length) {
      setTextMessageQueue((current) => [...queued, ...current.filter((row) => !(row.show_id === selectedShow.id && queued.some((next) => next.id === row.id)))]);
    }
    ...
    const sections: ExportDocument["sections"] = eventDisplayMode === "booth"
      ? boothSections.flatMap((section): ExportDocument["sections"] => {
          ...
          return dates.flatMap((date) => {
            ...
            return callsForDate.map(({ call }) => ({
              ...
              subheading: `${call.role_name} • ${formatTimeRange(call)}${call.one_hour_walkaway ? " • 1 hr walkaway" : ""} • ${call.crew_needed} needed`,
      ...
      : visibleLaborDays.flatMap((day): ExportDocument["sections"] => {
          ...
          return dayCalls.map(({ call }) => ({
            heading: `${day.labor_date}${day.label ? ` - ${day.label}` : ""}`,
            subheading: `${boothLabel(call.area)} • ${call.role_name} • ${formatTimeRange(call)}${call.one_hour_walkaway ? " • 1 hr walkaway" : ""} • ${call.crew_needed} needed`,
  ...
  async function saveShow() {
    ...
    if (canManageEventAccess) {
      ...
      const accessData = await request("/api/event-user-access", "POST", { show_id: nextShow.id, user_ids: accessUserIds });
  ...
  function renderAutomationPanel() {
    ...
    const reminderPlanRows = [
      ...
      { label: "Day-of morning reminder", enabled: automationDraft.reminder_day_of, when: "2 hours before each crew member's first call · Good Morning, we are meeting at the meet-up location. See you soon." },
    ];
        ...
        <div className="card compact">
          ...
          <div className="card compact" style={{ marginTop: 14, background: "#fbfcfd" }}>
            ...
            <div className="card compact" style={{ boxShadow: "none", marginTop: 12 }}>
              ...
              <div className="small muted">Use these when the reminder window has already passed, or when you want to send a reminder yourself. These texts are queued due now and will send on the next iPhone Shortcut run. Day-of morning reminder says: Good Morning, we are meeting at the meet-up location. See you soon. Each Shortcut run can pull up to 25 due texts.</div>
              <div className="toolbar" style={{ marginTop: 10 }}>
                ...
                <button type="button" className="ghost" onClick={() => queueManualReminderNow("day_of")} disabled={saving || !selectedShow || !automationDraft.send_schedule}>Queue day-of morning now</button>
              </div>
  ...
  function renderFeedbackPanel() {
    ...
    const linkForFeedbackForm = (form: FeedbackFormItem) => connectedFeedbackLinks.find((link) => {
      ...
      <div className="list">
        <div className="card compact feedback-intro-card">
          <div className="row" style={{ alignItems: "flex-start" }}>
            <div>
              ...
              <div className="small muted">Copy or open live client survey links. Previews stay closed until you open them.</div>
            </div>
          ...
          {connectedFeedbackLinks.length ? (
            ...
            <p className="small muted" style={{ marginBottom: 0 }}>Connected links open a client-only survey page. Tap Refresh Connected Survey Links if a card says Create link instead of Copy link.</p>
          )}
        </div>
        ...
        {feedbackForms.length ? feedbackForms.map((form) => {
            ...
            return (
              <div key={form.key} className="card compact feedback-survey-card" style={{ boxShadow: "none" }}>
                <div className="row" style={{ alignItems: "flex-start" }}>
                  ...
                  <div className="toolbar">
                    {liveLink ? <button type="button" className="primary" onClick={() => void copyConnectedFeedbackLink(liveLink)}>Copy link</button> : <button type="button" className="primary" onClick={() => void generateConnectedFeedbackLinks()}>Create link</button>}
                    {liveLink ? <button type="button" className="ghost" onClick={() => window.open(connectedFeedbackUrl(liveLink), "_blank", "noopener,noreferrer")}>Open live form</button> : null}
          ...
          <form key={form.key} className="card feedback-survey-card" onSubmit={(event) => event.preventDefault()}>
            ...
            <div className="toolbar" style={{ marginTop: 12 }}>
              {liveLink ? <button type="button" className="primary" onClick={() => void copyConnectedFeedbackLink(liveLink)}>Copy link</button> : <button type="button" className="primary" onClick={() => void generateConnectedFeedbackLinks()}>Create live link</button>}
              {liveLink ? <button type="button" className="ghost" onClick={() => window.open(connectedFeedbackUrl(liveLink), "_blank", "noopener,noreferrer")}>Open live form</button> : null}
              <button type="button" className="ghost" onClick={() => void copyText(form.text, form.title)}>Copy clean text</button>
            ...
            <div className="grid grid-2">
              ...
              <label className="field">
                ...
                <div className="toolbar">
                  ...
                  <span className="small muted">Options come from Settings → Crew pay rates. Refresh pulls your latest saved positions without leaving this event.</span>
                </div>
              </label>
            </div>
