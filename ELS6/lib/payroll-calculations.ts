...
export function estimateAssignmentPay(options: {
  ...
  const fullDayRate = crewFullDay ?? masterFull ?? fallback?.full_day ?? 0;
  const halfDayRate = crewFullDay ? Math.round((crewFullDay / 2) * 100) / 100 : masterHalf ?? fallback?.half_day ?? null;
