...
export async function getPayrollPageData(): Promise<PayrollPageData> {
  try {
    ...
    const [showsRes, laborDaysRes, subCallsRes, assignmentsRes, crewRes, positionsRes, cityPoolsRes, ratesRes, clientRatesRes, clientRateOverridesRes, payrollRes, financialsRes, expenseItemsRes, profilesRes, coordinatorPayrollRes, taxProfilesRes] = await Promise.all([
      supabase.from("shows").select("id, name, client, business_client_id, venue, rate_city, show_start, show_end, notes, assigned_coordinator_user_id").order("show_start", { ascending: false }).limit(5000),
