...
export default async function CoordinatorPage({ searchParams }: { searchParams?: Promise<CoordinatorSearchParams> }) {
  ...
  const rows = shows.map((show: ShowRecord) => {
    ...
    const coordinatorBuckets = [...coordinatorBucketMap.values()].map((bucket) => ({ ...bucket, fee: coordinatorFee(bucket.fullDayTechDays, bucket.halfDayTechs) }));
    return { show, days, calls, needed, filled, open, fee: coordinatorFee(fullDayTechDays, halfDayTechs), coordinatorBuckets, canViewFee: canViewCoordinatorFeeForShow(show) };
  });
