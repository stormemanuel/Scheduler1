...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .hero-logo { display:block; width:min(360px, 80%); height:auto; margin:0 0 18px; background:#fff; border-radius:12px; padding:8px 12px; }
...
function EventsClientInner({
  ...
  function positionChangeSourcesFor(anchorAssignment: AssignmentRecord, sourceCall: SubCallRecord) {
    ...
    return assignments
      ...
      .filter((item): item is { assignment: AssignmentRecord; call: SubCallRecord; day: LaborDayRecord } => Boolean(item && visibleDayIds.has(item.day.id) && sameAssignmentPattern(sourceCall, item.call)))
      .sort((a, b) => a.day.labor_date.localeCompare(b.day.labor_date) || a.call.start_time.localeCompare(b.call.start_time));
  }

  function positionChangeOptionsFor(sourceCall: SubCallRecord) {
    const optionsByRole = new Map<string, PositionOption>();
    subCallPositionOptions.forEach((option) => optionsByRole.set(normalize(option.role_name), option));
    const visibleDayIds = new Set(visibleLaborDays.map((day) => day.id));
    subCalls
      .filter((call) => visibleDayIds.has(call.labor_day_id) && normalize(call.area) === normalize(sourceCall.area))
      .forEach((call) => {
        const key = normalize(call.role_name);
        if (!key || optionsByRole.has(key)) return;
        optionsByRole.set(key, {
          id: call.master_rate_id || `custom:${key}`,
          city_name: activeRateCity || "Default",
    ...
    return [...optionsByRole.values()]
      .filter((option) => !callMatchesPosition(sourceCall, option.role_name, option.id.startsWith("special:") || option.id.startsWith("custom:") ? null : option.id))
      .sort((a, b) => positionPriority(a.role_name) - positionPriority(b.role_name) || a.role_name.localeCompare(b.role_name));
  }

  function targetPositionCallFor(sourceCall: SubCallRecord, roleName: string, masterRateId?: string | null) {
    return subCalls
      .filter((call) => call.labor_day_id === sourceCall.labor_day_id && normalize(call.area) === normalize(sourceCall.area) && callMatchesPosition(call, roleName, masterRateId))
      .sort((a, b) => {
        ...
        {callAssignments.length ? (
          <div className="list" style={{ marginTop: 10 }}>
            {callAssignments.map(({ assignment, crew }, assignmentIndex) => {
              ...
              const positionChangeOptions = positionChangeOpen ? positionChangeOptionsFor(call) : [];
              return (
                    ...
                    <div className="card compact sub-call-multi-day" style={{ marginTop: 10, boxShadow: "none" }}>
                      ...
                      <div className="grid grid-2" style={{ marginTop: 10 }}>
                        <label className="field">
                          ...
                          <select value={positionChangeTargetOptionId} onChange={(event) => setPositionChangeTargetOptionId(event.target.value)}>
                            <option value="">Choose position</option>
                            {positionChangeOptions.map((option) => <option key={option.id} value={option.id}>{option.role_name}</option>)}
                          </select>
                        </label>
                        <div className="small muted" style={{ alignSelf: "end", paddingBottom: 8 }}>If the destination position does not exist on a selected day, ELS creates it using that day’s current area, time, and call details with Crew Needed set to 1.</div>
                      </div>
                      <div className="grid grid-3" style={{ marginTop: 10 }}>
                      ...
                      <div className="toolbar" style={{ marginTop: 10 }}>
                        <button type="button" className="primary" disabled={saving || !positionChangeTargetOptionId || !positionChangeAssignmentIds.length} onClick={() => changeCrewPositionAcrossDays(assignment, call)}>Apply position change</button>
                        <button type="button" className="ghost" onClick={() => { setPositionChangeAssignmentId(null); setPositionChangeAssignmentIds([]); setPositionChangeTargetOptionId(""); }}>Cancel</button>
                      </div>
                    </div>
                  ) : null}
  ...
  async function changeCrewPositionAcrossDays(anchorAssignment: AssignmentRecord, sourceCall: SubCallRecord) {
    const targetOptions = positionChangeOptionsFor(sourceCall);
    const targetOption = targetOptions.find((option) => option.id === positionChangeTargetOptionId) || null;
    if (!targetOption) {
      setMsg({ kind: "error", text: "Choose the new position first." });
      return;
    }
    ...
    const targetMasterRateId = targetOption.id.startsWith("special:") || targetOption.id.startsWith("custom:") ? null : targetOption.id;
    ...
    for (const source of sourceItems) {
      const existingTarget = targetPositionCallFor(source.call, targetOption.role_name, targetMasterRateId);
      const key = [source.call.labor_day_id, normalize(source.call.area), normalize(targetOption.role_name), source.call.start_time, source.call.end_time].join("::");
      if (plannedCalls.some((item) => item.key === key)) {
        setMsg({ kind: "error", text: `Two selected ${sourceCall.role_name} rows would move ${crewById.get(anchorAssignment.crew_id)?.name || "this crew member"} into the same ${targetOption.role_name} row on ${source.day.labor_date}. Select only one of those source rows.` });
        return;
      }
      if (existingTarget) {
        ...
        if (duplicate) {
          setMsg({ kind: "error", text: `${crewById.get(anchorAssignment.crew_id)?.name || "This crew member"} is already assigned as ${targetOption.role_name} on ${source.day.labor_date}.` });
          return;
        }
        ...
        if (maxCrew > 0 && filled >= maxCrew) {
          setMsg({ kind: "error", text: `${targetOption.role_name} is already full on ${source.day.labor_date} (${filled}/${maxCrew}). Increase Crew Needed before changing the position.` });
          return;
        }
        ...
        missingPayloadByKey.set(key, {
          ...
          role_name: targetOption.role_name,
    ...
    try {
      ...
      for (const plan of plannedCalls) {
        const targetCall = plan.target || allTargetCalls
          .filter((call) => call.labor_day_id === plan.source.call.labor_day_id && normalize(call.area) === normalize(plan.source.call.area) && callMatchesPosition(call, targetOption.role_name, targetMasterRateId))
          .sort((a, b) => {
        ...
        if (!targetCall) throw new Error(`Could not find or create ${targetOption.role_name} on ${plan.source.day.labor_date}.`);
      ...
      setMsg({ kind: "success", text: `${crewName} changed from ${sourceCall.role_name} to ${targetOption.role_name} on ${updatedAssignments.length} selected day${updatedAssignments.length === 1 ? "" : "s"}.${createdCalls.length ? ` ${createdCalls.length} missing position row${createdCalls.length === 1 ? " was" : "s were"} created automatically.` : ""}` });
    } catch (error) {
  ...
  async function saveCall() {
    ...
    if (editingCallId) {
      ...
      const updated = await request(`/api/sub-calls/${editingCallId}`, "PATCH", payload);
      const nextCall: SubCallRecord = sanitizeSubCall((updated?.row ?? { id: editingCallId, ...payload }) as SubCallRecord);
      const extraDayIds = [...new Set((extraPositionTargetDayIds.length ? extraPositionTargetDayIds : targetDayIds).filter(Boolean))];
      const existingCalls = subCalls.map((call) => call.id === editingCallId ? nextCall : call);
      ...
      setMsg({ kind: "success", text: extraPositionPayloads.length ? `Sub-call saved; ${createdText}.${skippedText}` : "Sub-call saved." });
    } else {
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .feedback-export-logo { display:block; width:min(340px, 80%); height:auto; margin:0 0 14px; }
        ...
        {editorMode === "call" ? (
          <div className="grid" style={{ gap: 14, marginTop: 12 }}>
            ...
            <div className="toolbar">
              ...
              <button type="button" className="ghost" onClick={() => { setEditingCallId(null); setEditingDayTargetId(null); setCallTargetDayIds([]); setExtraPositionTargetDayIds([]); setCallForm(emptyCall); setMultiPositionRows([]); setEditorMode(null); setViewMode("overview"); }}>Cancel</button>
            </div>
          </div>
        ) : null}
      </div>
