...
async function listOnboardingReviewQueue(admin: SupabaseAdmin, request: Request) {
  ...
  if (crewIds.length) {
    const crewFields = canReview
      ? "id, name, email, phone, address, onboarding_status, questionnaire_status, w9_status, contract_status, tax_profile_status, profile_photo_url, work_photo_urls, w9_document_url, contract_document_url, tax_profile_notes, notes"
      : "id, name, email, phone, onboarding_status";
    const crewRes = await admin
      .from("crew")
      .select(crewFields)
      .in("id", crewIds);
