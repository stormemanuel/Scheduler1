...
function EventsClientInner({
  ...
  const clientTopTechs = useMemo(() => {
    ...
    if (!clientId) return [] as Array<{ crew: CrewRecord | undefined; average: number; count: number }>;
    const byCrew = new Map<string, { total: number; count: number }>();
    for (const rating of techRatings.filter((row) => row.client_id === clientId && Number(row.rating) > 0)) {
      const existing = byCrew.get(rating.crew_id) ?? { total: 0, count: 0 };
      existing.total += Number(rating.rating || 0);
      existing.count += 1;
      byCrew.set(rating.crew_id, existing);
    }
    return [...byCrew.entries()]
      .map(([crewId, row]) => ({ crew: crewRecords.find((crew) => crew.id === crewId), average: row.total / Math.max(1, row.count), count: row.count }))
  ...
  function renderNoteEditor(assignment: AssignmentRecord) {
    ...
    return (
      <div className="card compact" style={{ marginTop: 8, background: "#f9fafb" }}>
        ...
        <div className="small muted" style={{ marginTop: 4 }}>Rate this tech 1–5 stars for this show and add professional notes when needed. Ratings save to the tech contact and the selected client.</div>
    ...
    return (
      <div className="list">
        <div className="card compact accent-card">
          <div className="row" style={{ alignItems: "flex-start" }}>
            <div>
              ...
              <div className="small muted">
                Rate each assigned tech 1–5 stars for this show. When a saved business client is selected, the rating also builds that client&apos;s Top Techs list.
              </div>
          ...
          {!selectedShow.business_client_id ? (
            <p className="error" style={{ marginBottom: 0 }}>This show is not connected to a saved business client yet. Edit Event and choose one so ratings count toward a client-specific Top Techs list.</p>
          ) : null}
        </div>
        ...
        <div className="card compact">
              ...
              <div className="list">
                {clientTopTechs.map((item, index) => (
                  <div key={item.crew?.id || index} className="row" style={{ alignItems: "center" }}>
                    ...
                    <div style={{ textAlign: "right" }}>
                      <strong>{starDisplay(item.average)} {item.average.toFixed(1)}</strong>
          ...
          ) : <p className="small muted">Choose a saved business client on this event to activate the client-specific top tech list.</p>}
        </div>
      </div>
    );
  }
        ...
        {editorMode === "show" ? (
          <div className="grid" style={{ gap: 14, marginTop: 12 }}>
            ...
            <div className="grid grid-2">
              ...
              <label className="field">
                <span>Client contact for this event</span>
            ...
            <div className="grid" style={{ gap: 16 }}>
              <div className="card compact">
                ...
                <div className="toolbar small" style={{ marginTop: 10 }}>
                  ...
                  {selectedClientContact ? <span className="badge"><strong>Client contact:</strong> {selectedClientContact.name}{selectedClientContact.phone ? ` · ${formatPhone(selectedClientContact.phone)}` : ""}</span> : null}
