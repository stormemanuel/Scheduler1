...
function buildQueueRows(
  ...
  options: { forceNowReminderKey?: string } = {}
) {
  ...
  const manualBatchKey = `manual_${options.forceNowReminderKey || ""}_${new Date().toISOString().replace(/[^0-9]/g, "").slice(0, 14)}`;
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action !== "queue_messages" && action !== "queue_manual_reminder") return NextResponse.json({ message: "Unsupported action." }, { status: 400 });
    if (!showId) return NextResponse.json({ message: "show_id is required." }, { status: 400 });
    const mode = action === "queue_manual_reminder" ? "schedule_reminders" : body.mode === "availability" ? "availability" : "schedule_reminders";
    const manualReminderKey = safeText(body.reminder_key);
    ...
    const queueRows = buildQueueRows(mode, settings, bundle, action === "queue_manual_reminder" ? { forceNowReminderKey: manualReminderKey } : {});
    ...
    if (error) throw new Error(error.message);
    return NextResponse.json({ ok: true, settings: saved, queue: data || [], message: action === "queue_manual_reminder" ? `Queued ${queueRows.length} manual reminder text${queueRows.length === 1 ? "" : "s"} to send now.` : `Queued ${queueRows.length} text message${queueRows.length === 1 ? "" : "s"}.` });
  } catch (error) {
