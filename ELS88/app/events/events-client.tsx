...
function EventsClientInner({
  ...
  async function testShortcutJson(url: string, label = "Shortcut URL") {
    ...
    try {
      ...
      const data = JSON.parse(raw) as { ok?: boolean; message?: string; active_show_count?: number; mode?: string };
      ...
      setMsg({ kind: "success", text: `${label} is public and returning valid JSON.${activeCount}` });
    } catch (error) {
          ...
          {automationDraft.sending_method === "shortcut" ? (
            <div className="card compact accent-card" style={{ marginTop: 14 }}>
              ...
              <div className="toolbar">
                ...
                <button type="button" className="ghost" onClick={() => void copyText(`ELS Shortcut setup:\n1. Tap Test Shortcut JSON first. It must say valid JSON.\n2. Tap Open Shortcut Builder from your iPhone.\n3. The universal URL is:\n${universalShortcutPollUrl || "SAVE SETTINGS FIRST"}\n4. In Shortcuts, use URL → Get Contents of URL → Get Dictionary from Contents of URL → Get Dictionary Value messages → Repeat with Each Item.\n5. Inside Repeat, get phone, body, and mark_sent_url from Repeat Item. Send body to phone, then Get Contents of mark_sent_url.\n6. Add Personal Automations at 9:00am, 5:00pm, and hourly on active show days.`, "Shortcut setup steps")}>Copy setup steps</button>
              </div>
              <div className="small muted" style={{ marginTop: 8 }}>
                Universal mode only sends messages that are due, scheduled, and tied to events with Apple Shortcut Mode active. Test Shortcut JSON before building the iPhone Shortcut. If the test says HTML/text, the Shortcut will fail with the Rich Text to Dictionary error.
              </div>
            </div>
          ) : null}
