...
function normalizeAvailabilityTemplateForDisplay(value: string | null | undefined) {
  ...
  if (!text || text === oldDefault || text === priorDefault) return defaultAvailabilityTemplate;
  return text;
}
...
function EventsClientInner({
  ...
  const availabilityRequestDetails = useMemo(() => {
    ...
    const schedule = calls.length
      ? calls.map(({ day, call }) => `${formatMessageDate(day.labor_date)} · ${formatTimeRange(call)} · ${safeText(call.role_name) || "Crew"} · ${boothLabel(call.area)}`).join("; ")
