...
type ShowRow = { id: string; name: string | null; client: string | null; client_contact_id?: string | null; venue: string | null; event_location?: string | null; rate_city: string | null; show_start: string; show_end: string; notes: string | null };
...
async function upsertTextQueueRows(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, rows: QueueRow[]) {
  ...
  if (isMissingQueueAuditColumns(withSender.error.message)) return upsertTextQueueRows(admin, stripQueueAuditColumns(rows) as QueueRow[]);
  if (!isMissingSenderColumns(withSender.error.message)) throw new Error(withSender.error.message);
  const legacyRows = rows.map(({ queued_by_user_id, queued_by_email, queued_by_name, ...row }) => row);
...
function cancellationDateTimeLabel(row: RemovedCrewAssignmentRow) {
  const date = row.labor_date ? formatDate(row.labor_date) : "the removed call";
  const time = [formatTime(row.start_time || ""), formatTime(row.end_time || "")].filter(Boolean).join("-");
  const parts = [date, time, safeText(row.role), safeText(row.area) ? `Area: ${safeText(row.area)}` : ""].filter(Boolean);
  return parts.join(" · ");
}
...
function buildCancellationNoticeBody(showName: string, rows: RemovedCrewAssignmentRow[]) {
  ...
  const cancelled = rows
    .sort((a, b) => safeText(a.labor_date).localeCompare(safeText(b.labor_date)) || safeText(a.start_time).localeCompare(safeText(b.start_time)))
    .map(cancellationDateTimeLabel)
    .join("; ");
  return `Hi ${first}, your schedule for ${showName || "this event"} on ${cancelled || "the removed call"} has been canceled. We will be in touch if we get more additions to this call. Please confirm this cancellation. Thanks.`;
}
...
function nextScheduleByCrew(show: ShowRow, days: LaborDayRow[], calls: SubCallRow[], assignments: AssignmentRow[], crewRows: CrewRow[], masterRates: MasterRateRow[], settings: AutomationSettings, senderName = "", projectManagerContact: ClientContactRow | null = null) {
  ...
  const result = new Map<string, { crew: CrewRow; lines: Array<{ sortKey: string; text: string; area: string; dateLabel: string }>; meetUps: string[]; rates: string[]; poNumbers: string[]; areas: string[]; firstCall: { day: LaborDayRow; call: SubCallRow; assignment: AssignmentRow } | null; values: Record<string, string> }>();
...
async function fetchShowBundle(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, showId: string) {
  const showRes = await admin.from("shows").select("id, name, client, client_contact_id, venue, event_location, rate_city, show_start, show_end, notes").eq("id", showId).single();
...
async function buildImmediateFullShowSchedulePlan(
  ...
  options: { resendCancelled?: boolean; resendSent?: boolean } = {},
) {
...
async function saveSettings(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, settings: AutomationSettings, user?: { id: string }) {
  ...
  return { ...fallback.data, shortcut_token: secureShortcutToken };
}
...
async function queueAvailabilityCandidates(
  ...
  const availabilitySchedule = safeText(body.availability_schedule) || (availabilityCalls.length
    ? availabilityCalls.map((call) => {
      const timeRange = call.start_time || call.end_time ? `${formatTime(call.start_time)}-${formatTime(call.end_time)}` : "";
      return [
        call.labor_date ? formatDate(call.labor_date) : "",
        timeRange,
        call.role_name || "Crew",
        call.area ? `Area: ${call.area}` : "",
      ].filter(Boolean).join(" · ");
    }).join("; ")
    : `${formatDate(show.show_start)} through ${formatDate(show.show_end)}`);
  const availabilityArea = safeText(body.availability_area) || callAreas.join(" / ") || "Event areas";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "queue_cancellation_notices") {
      ...
      const eventName = [safeText(show?.show_reference_number), safeText(show?.name) || "this event"].filter(Boolean).join(" | ");
    ...
    if (action === "preview_combined_schedule_reminders" || action === "send_combined_schedule_reminders") {
      ...
      const [schedulePlan, reminderPlan] = await Promise.all([
        buildImmediateFullShowSchedulePlan(admin, effectiveSettings, bundle, senderName, safeText(body.additional_note), { resendCancelled, resendSent: resendSentSchedules }),
