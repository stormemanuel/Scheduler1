  ...
  try {
    ...
    if (action === "preview_updated_schedules" || action === "send_updated_schedules") {
      ...
      return NextResponse.json({
        ...
        summary,
        message: deliveryMode === "resend_email"
          ? `${serverSend?.sent || 0} updated schedule email${(serverSend?.sent || 0) === 1 ? "" : "s"} sent through Resend with the update notice.${serverSend?.failed ? ` ${serverSend.failed} failed.` : ""}`
          : `${data?.length || queueRows.length} updated schedule${(data?.length || queueRows.length) === 1 ? "" : "s"} queued with the update notice.`,
      });
    }
