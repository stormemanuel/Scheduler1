...
export async function getPayrollPageData(requestedYear?: number): Promise<PayrollPageData> {
  ...
  try {
    ...
    // Payroll used to download every labor day, sub-call, assignment, crew record,
    // expense, and tax profile before the page could open. On an installed iPhone
    // app that could stall the page. Load only the selected year, and let the
    // client request another year when the user changes the selector.
    const showYearsRes = await supabase
      .from("shows")
      .select("show_start")
    ...
    const availableYears = [...new Set([
      ...(showYearsRes.data ?? []).map((row) => showYear((row as { show_start?: string | null }).show_start)),
      currentYear,
    ])]
    ...
    if (!requestedIsValid) {
      loadedYear = availableYears.includes(currentYear) ? currentYear : (availableYears[0] ?? currentYear);
    }
    ...
    const [laborDaysRes, cityPoolsRes, ratesRes, clientRatesRes, clientRateOverridesRes, payrollRes, financialsRes, expenseItemsRes, profilesRes, coordinatorPayrollRes, coordinatorCompensationRes] = await Promise.all([
      showIds.length
        ? supabase.from("labor_days").select("id, show_id, labor_date, label, notes").in("show_id", showIds).order("labor_date", { ascending: true }).limit(10000)
      ...
      supabase.from("client_rate_overrides").select("id, client_id, city_name, role_name, full_day, half_day, overtime_multiplier, doubletime_multiplier, created_at, updated_at").limit(10000),
      showIds.length
        ? supabase.from("show_payroll").select("*").in("show_id", showIds).limit(20000)
        : Promise.resolve(emptyResult),
      showIds.length
        ? financialReadClient.from("show_financials").select("*").in("show_id", showIds).limit(10000)
        : Promise.resolve(emptyResult),
      showIds.length
        ? financialReadClient.from("show_expense_items").select("id, show_id, category, description, amount, tax_treatment, receipt_status, expense_date, notes, created_by, created_at, updated_at").in("show_id", showIds).order("created_at", { ascending: false }).limit(20000)
        : Promise.resolve(emptyResult),
    ...
    const laborDayIds = (laborDaysRes.data ?? []).map((row) => safeText((row as { id?: string }).id)).filter(Boolean);
    let subCallsRes: any = laborDayIds.length
      ? await supabase.from("sub_calls").select("id, labor_day_id, area, po_number, assigned_coordinator_user_id, role_name, master_rate_id, start_time, end_time, crew_needed, notes, day_type, one_hour_walkaway").in("labor_day_id", laborDayIds).order("start_time", { ascending: true }).limit(20000)
      : emptyResult;
    ...
    if (subCallCoordinatorColumnMissing && laborDayIds.length) {
      subCallsRes = await supabase.from("sub_calls").select("id, labor_day_id, area, po_number, role_name, master_rate_id, start_time, end_time, crew_needed, notes, day_type, one_hour_walkaway").in("labor_day_id", laborDayIds).order("start_time", { ascending: true }).limit(20000);
    }
    const payrollSubCallsRes = subCallsRes;
    ...
    let assignmentsRes: any = subCallIds.length
      ? await supabase.from("assignments").select("id, sub_call_id, crew_id, status, sort_order, start_time, end_time, day_type, coordination_owner_user_id, coordination_owner_name, coordination_fee_waived").in("sub_call_id", subCallIds).order("sort_order", { ascending: true }).limit(20000)
      : emptyResult;
    ...
    if (crewOnboardingColumnsMissing && crewIds.length) {
      crewRes = await supabase.from("crew").select("id, name, description, city_pool_id, group_name, tier, email, phone, address, lead_from, other_city, ob, onboarding_texted_called, onboarding_response, onboarding_paperwork_sent, onboarding_successfully_onboarded, onboarding_called_placed_tier, notes, conflict_companies").in("id", crewIds).order("name", { ascending: true }).limit(10000);
    }
    const payrollCrewRes = crewRes;

    const [positionsRes, taxProfilesRes] = await Promise.all([
      crewIds.length
        ? supabase.from("crew_positions").select("id, crew_id, role_name, rate").in("crew_id", crewIds).order("role_name", { ascending: true }).limit(20000)
        : Promise.resolve(emptyResult),
      crewIds.length
        ? (admin ?? supabase).from("crew_tax_profiles").select("crew_id, tax_legal_name, business_name, federal_tax_classification, llc_tax_classification, other_classification, tax_address_line_1, tax_city_state_zip, tin_type, tin_last4, tin_encrypted, signer_name, certification_confirmed, signed_at, source, updated_at").in("crew_id", crewIds).limit(10000)
        : Promise.resolve(emptyResult),
    ]);
