...
function importedCallKey(call: ImportedSubCall, index: number) {
  return `import-${index}-${[call.labor_date, call.area, call.booth_number || legacyBoothFromReference(call.sub_call_reference_number) || "", normalizeImportedRoleName(call.role_name), call.start_time, call.end_time || ""].map((part) => normalizeMatchValue(String(part || ""))).join("-")}`;
}
...
function existingSubCallBooth(row: ExistingSubCallRow) {
  return boothNumberFromNotes(row.notes) || legacyBoothFromReference(subCallReferenceNumberFromNotes(row.notes)) || "";
}
...
function meaningfulBoothUpdate(existingValue: string | null | undefined, importedValue: string | null | undefined) {
  const existing = normalizeBoothValue(existingValue);
  const imported = normalizeBoothValue(importedValue);

  // A quote that still says TBD/unknown must not create noise or erase a known booth.
  // Only a concrete imported booth can create a booth-number change.
  if (!imported || imported === "tbd") return false;

  // Concrete booth arriving after no booth/TBD is a legitimate update.
  if (!existing || existing === "tbd") return true;
...
function existingChangeValue(row: ExistingSubCallRow, laborDate: string) {
  return {
    ...
    area: row.area || "",
    reference: existingSubCallBooth(row),
...
function importedChangeValue(call: ImportedSubCall) {
  return {
    ...
    area: call.area || "",
    reference: call.booth_number || legacyBoothFromReference(call.sub_call_reference_number) || "",
...
async function renderPdfPageVisually(pageData: { getTextContent: (options?: Record<string, boolean>) => Promise<{ items: PdfTextItem[] }> }) {
  ...
  return [...rows.entries()]
    .sort((a, b) => b[0] - a[0])
    .map(([, items]) => {
      const sorted = items.sort((a, b) => Number(a.transform?.[4] || 0) - Number(b.transform?.[4] || 0));
      let row = "";
      ...
      for (const item of sorted) {
        ...
        const height = Math.max(1, Math.abs(Number(item.transform?.[3] || 0)) || previousHeight || 6);
        const gap = previousEnd == null ? 0 : x - previousEnd;

        // Chromium/QuickBooks PDFs often expose bold text as one PDF item per
        // glyph. Joining every item with a space turns "LED Stagehand" into
        // "L E D S t a g e h a n d" and also breaks bold totals. Preserve
        // explicit PDF space glyphs and use geometry only for real column/word gaps.
        if (/^\s+$/.test(raw)) {
          if (row && !row.endsWith(" ")) row += " ";
        } else {
          const needsGap = Boolean(row && !row.endsWith(" ") && gap > Math.max(1.25, Math.min(previousHeight || height, height) * 0.22));
          row += `${needsGap ? " " : ""}${raw}`;
        }
        previousEnd = x + width;
...
export async function buildImportComparison(admin: { from: (table: string) => any }, payload: ImportedPayload, existingEvent: ExistingImportShowMatch): Promise<ImportComparison> {
  ...
  for (const row of existingSubCalls) {
    ...
    const booth = normalizeBoothValue(existingSubCallBooth(row));
  ...
  payload.subCallGroups.forEach((call, index) => {
    ...
    const fallbackIdentity = normalizePoNumber(call.po_number) || normalizeBoothValue(call.booth_number || legacyBoothFromReference(call.sub_call_reference_number)) || String(index);
    ...
    const booth = normalizeBoothValue(call.booth_number || legacyBoothFromReference(call.sub_call_reference_number));
    if (booth) group.normalizedBooths.add(booth);
  ...
  for (const row of existingSubCalls) {
    ...
    const reference = existingSubCallBooth(row);
  ...
  const findExistingRowWithinMatchedArea = (call: ImportedSubCall, index: number) => {
    ...
    const importedReference = normalizeReferenceValue(call.booth_number || legacyBoothFromReference(call.sub_call_reference_number));
    ...
    const ranked = sameDateRole.map((row) => {
      ...
      const existingReference = normalizeReferenceValue(existingSubCallBooth(row));
  ...
  payload.subCallGroups.forEach((call, index) => {
    ...
    const comparable = comparableIdentity({ labor_date: call.labor_date, area: call.area, reference: call.booth_number || legacyBoothFromReference(call.sub_call_reference_number), role_name: call.role_name });
    ...
    if (!exactish) {
      ...
      const areaKey = `${areaMatch?.matchedAreaKey || areaMatch?.importedAreaKey || normalizeArea(call.area)}|${normalizeMatchValue(call.booth_number || legacyBoothFromReference(call.sub_call_reference_number) || "")}`;
    ...
    if (meaningfulBoothUpdate(existing.reference, imported.reference)) {
      fieldChanges.push({
        ...
        selectedDefault: true,
        matchReason,
        fields: { booth_number: { existing: existing.reference, imported: imported.reference } },
      });
    }
...
export async function buildImportPreview(
  ...
  const subCallPreview = payload.subCallGroups.map((call) => {
    ...
    return {
      ...
      booth_number: call.booth_number || legacyBoothFromReference(call.sub_call_reference_number) || null,
      sub_call_reference_number: legacyBoothFromReference(call.sub_call_reference_number) ? null : (call.sub_call_reference_number || null),
