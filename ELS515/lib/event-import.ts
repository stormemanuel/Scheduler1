...
const TIME_PATTERN = "\\b(?:\\d{1,2}(?:(?::|\\s)\\d{2})(?::\\d{2})?\\s*(?:a\\.?m\\.?|p\\.?m\\.?)?|\\d{1,2}\\s*(?:a\\.?m\\.?|p\\.?m\\.?))(?=\\s|$|[-–—|])";
...
function to24Hour(raw: string) {
  ...
  // normal times (8:00 AM), corrupted times (8 00 AM), and compact estimate times (8am).
...
function removeEstimateDayTypeQualifiers(value: string) {
  return compactSpaces(value
    .replace(/\(\s*(?:half|full)\s*day\s*\)/gi, " ")
    .replace(/\b(?:half|full)\s*day\b/gi, " "));
}

function canonicalizeImportedPosition(value: string) {
  const clean = compactSpaces(removeEstimateDayTypeQualifiers(value))
    .replace(/\b20\d{2}\b/g, " ")
    .replace(/\bPO\s*\d+\b/gi, " ")
  ...
  if (/\bworking\s+(?:lead|crew\s+lead)\b/.test(key)) {
    if (/\bled\b/.test(key) && /\bstagehand\b/.test(key)) return "LED Stagehand (Working Crew Lead)";
    if (/\bled\b/.test(key) && /\bassist\b/.test(key)) return "LED Assist (Working Crew Lead)";
    if (/\bvideo\b/.test(key) && /\bassist\b/.test(key)) return "AVT / Working Lead";
    if (/\b(?:av|audio\s+visual|audiovisual)\b/.test(key) && /\btech(?:nician)?\b/.test(key)) return "AVT / Working Lead";
    return compactSpaces(
      clean
        .replace(/\(\s*working\s+crew\s+lead\s*\)/gi, "(Working Crew Lead)")
        .replace(/\(\s*working\s+lead\s*\)/gi, "(Working Lead)")
    );
  }
  if (/\bled\b/.test(key) && /\bstagehand\b/.test(key)) return "LED Stagehand";
  if (key === "led assist" || key === "led assistant" || key === "led tech" || key === "led technician") return "LED Assist";
  ...
  if (key === "gav" || key === "general av" || key === "general audio visual" || key === "avt" || key === "av tech" || key === "av technician" || key === "audio visual tech" || key === "audio visual technician") {
    return "General AV";
  }

  if (key === "lighting stagehand" || key === "lighting hand") return "Lighting Stagehand";
  if (key === "l2" || key === "lighting assist" || key === "lighting assistant" || key === "lighting technician l2") return "Lighting Assist";
  if (key === "a2" || key === "audio assist" || key === "audio assistant") return "Audio Assist";
  if (key === "v2" || key === "video assist" || key === "video assistant" || key === "video assist v2" || key === "v2 video assist") return "Video Assist";
...
function normalizeEstimateRole(roleRaw: string, explicitDayTypeRaw = "") {
  ...
  const cleanRole = removeEstimateDayTypeQualifiers(roleRaw);
...
function extractEstimateBillShip(lines: string[]) {
  ...
  const venueMatch = first.match(/^(.+?)\s+([A-Z0-9&'.,\-\s]+(?:CONVENTION\s+CENTER|EXPO\s+CENTER|CENTER|STUDIOS?|HOTEL|RESORT|MARRIOTT|HYATT|HILTON|SHERATON|VENUE))$/);
...
function cleanEstimateGroupArea(value: string) {
  return compactSpaces(value)
    ...
    .replace(/\bBooth\s*(?:Number)?\s*:?\s*(?:[A-Z0-9-]+|TBD)\b/gi, " ")
    .replace(/^Area\s+/i, "")
    .replace(/\s+Booth\s*:?\s*TBD$/i, "")
...
function normalizeEstimateBoothNumber(value: string | null | undefined) {
  const clean = compactSpaces(String(value || "").replace(/^Booth\s*(?:Number)?\s*:?\s*/i, ""));
  if (!clean) return "";
  if (/^TBD$/i.test(clean)) return "TBD";
  return clean.replace(/^#\s*/, "").trim();
}

function parseEstimateBoothLine(line: string) {
  const clean = stripEstimateSubtotalPrefix(line);
  const match = clean.match(/\bBooth\s*(?:Number)?\s*:?\s*([A-Z0-9-]+|TBD)\b/i);
  return match ? normalizeEstimateBoothNumber(match[1]) : "";
}

function normalizeEstimateRoNumber(value: string | null | undefined) {
  const clean = compactSpaces(String(value || "").replace(/^RO\s*/i, ""));
  if (!clean) return "";
  return `RO${clean.toUpperCase().replace(/[^A-Z0-9.-]/g, "")}`;
}

function parseEstimateRoPoLine(line: string) {
  const clean = stripEstimateSubtotalPrefix(line);
  if (!clean || lineHasTime(clean) || lineHasPhone(clean)) return null;
  const roMatch = clean.match(/\b(RO\s*[A-Z0-9]+(?:[.-][A-Z0-9]+)*)\b/i);
  const poMatch = clean.match(/\b(PO\s*[A-Z0-9]+(?:[.-][A-Z0-9]+)*)\b/i);
  if (!roMatch && !poMatch) return null;
  const leftovers = compactSpaces(clean
    .replace(roMatch?.[0] || "", " ")
    .replace(poMatch?.[0] || "", " ")
    .replace(/\$0(?:\.00)?/gi, " ")
    .replace(/\b0(?:\.0+)?\b/g, " "));
  // Treat standalone RO/PO lines as metadata for the current Area, not as a new Area name.
  if (leftovers) return null;
  return {
    roNumber: normalizeEstimateRoNumber(roMatch?.[1] || ""),
    poNumber: poMatch ? compactSpaces(poMatch[1].replace(/\s+/g, "")) : "",
  };
}

function appendEstimateRoToArea(area: string, roNumber: string) {
  const cleanArea = cleanEstimateGroupArea(area);
  const cleanRo = normalizeEstimateRoNumber(roNumber);
  if (!cleanRo) return cleanArea;
  if (!cleanArea) return cleanRo;
  return normalizeMatchValue(cleanArea).includes(normalizeMatchValue(cleanRo)) ? cleanArea : compactSpaces(`${cleanArea} ${cleanRo}`);
}
...
function parseEstimateGroupHeaderLine(line: string, fallbackYear: number) {
  ...
  const poMatch = working.match(/\b(PO\s*[A-Z0-9]+(?:[.-][A-Z0-9]+)*)\b/i);
  const boothMatch = working.match(/\bBooth\s*(?:Number)?\s*:?\s*([A-Z0-9-]+|TBD)\b/i);
  const poNumber = poMatch ? compactSpaces(poMatch[1].replace(/\s+/g, "")) : "";
  const boothNumber = boothMatch ? normalizeEstimateBoothNumber(boothMatch[1]) || "TBD" : "";
...
function isEstimateGroupContinuationLine(line: string, fallbackYear: number) {
  ...
  if (/^(?:Booth(?:\s+Number)?|PO)\b/i.test(continuationText)) return false;
...
function lineHasQuickBooksLaborColumns(line: string) {
  ...
  const timeCount = timeMatches(clean).length;
...
function describeRejectedEstimateRow(line: string) {
  ...
  if (!lineHasTime(clean)) return `Row rejected: found date and rate but no start/end time — ${clean}`;
  const timeCount = timeMatches(clean).length;
...
function parseEstimateLaborLine(line: string, currentGroup: { area: string; boothNumber: string; poNumber: string }, fallbackYear: number): ImportedCrewRow | null {
  ...
  const times = timeMatches(rest);
  if (times.length < 2) return null;

  const startRaw = times[0].text;
  const endRaw = times[1].text;
  const roleRaw = compactSpaces(rest.slice(0, times[0].index));
  const afterEnd = rest.slice(times[1].index + endRaw.length);
...
function parseEstimatePdfText(text: string, overrides: Partial<ImportedEventData["show"]>): ImportedEventData | null {
  ...
  for (const rawLine of reconstructedRows) {
    ...
    const roPoLine = parseEstimateRoPoLine(line);
    if (roPoLine) {
      if (roPoLine.roNumber) currentGroup.area = appendEstimateRoToArea(currentGroup.area, roPoLine.roNumber);
      if (roPoLine.poNumber) currentGroup.poNumber = roPoLine.poNumber;
      groupingHeaders.push(currentGroup.area || roPoLine.roNumber || roPoLine.poNumber);
      groupOpenForContinuation = true;
      continue;
    }
    ...
    const boothNumber = parseEstimateBoothLine(line);
    if (boothNumber && !dateStartFromLine(line, fallbackYear)) {
      currentGroup.boothNumber = boothNumber;
      groupOpenForContinuation = true;
      continue;
    }
...
export function buildImportedEventPayload(data: ImportedEventData) {
  ...
  const canonicalByPo = new Map<string, { area: string; boothNumber: string }>();
  ...
  for (const row of data.rows) {
    const normalizedPoRaw = row.poNumber ? row.poNumber.toUpperCase().replace(/[^A-Z0-9]/g, "") : "";
    const normalizedPo = normalizedPoRaw ? (normalizedPoRaw.startsWith("PO") ? normalizedPoRaw : `PO${normalizedPoRaw}`) : "";
    const existingPoCanonical = normalizedPo ? canonicalByPo.get(normalizedPo) : null;
    if (existingPoCanonical) {
      row.area = existingPoCanonical.area;
      row.boothNumber = row.boothNumber || existingPoCanonical.boothNumber;
    } else if (normalizedPo) {
      canonicalByPo.set(normalizedPo, {
        area: cleanEstimateGroupArea(row.area || "Imported Call") || "Imported Call",
    ...
    const normalizedArea = cleanEstimateGroupArea(row.area || "Imported Call") || "Imported Call";
    const normalizedPosition = normalizeImportedRoleName(row.position);
    const key = [row.date, normalizedArea, normalizedPosition, row.startTime, row.endTime, row.poNumber || "", row.boothNumber || ""].join("|");
    ...
    if (existing) {
      ...
      const noteParts = [
        ...
        // Booth/RO/PO and quote pricing are structured import data. Do not duplicate client
        // financial values into operational Notes that may be visible to coordinators.
        row.reviewFlags?.length ? `Needs Review: ${row.reviewFlags.join("; ")}` : "",
      ].filter(Boolean);
      subCallGroups.set(key, {
        labor_date: row.date,
        area: normalizedArea,
        role_name: normalizedPosition,
        start_time: row.startTime,
        end_time: row.endTime,
        crew_needed: row.crewNeeded || 1,
        notes: noteParts.join("\n"),
        po_number: row.poNumber || null,
        booth_number: row.boothNumber || null,
        sub_call_reference_number: null,
...
export function normalizeImportedRoleName(value: string | null | undefined) {
  const raw = compactSpaces(String(value || ""));
  const roleOnly = removeEstimateDayTypeQualifiers(raw);
  const normalized = normalizeMatchValue(roleOnly);
  if (normalized === "led stagehand working crew lead" || normalized === "led stagehand working lead") return "LED Stagehand (Working Crew Lead)";
  if (normalized === "led assist working crew lead" || normalized === "led assistant working crew lead") return "LED Assist (Working Crew Lead)";
  if (normalized === "av tech working lead" || normalized === "av technician working lead" || normalized === "audio visual tech working lead" || normalized === "audio visual technician working lead" || normalized === "audiovisual tech working lead" || normalized === "audiovisual technician working lead") return "AVT / Working Lead";
  if (normalized === "led stagehand") return "LED Stagehand";
  if (normalized === "led assist" || normalized === "led assistant" || normalized === "led tech" || normalized === "led technician") return "LED Assist";
  if (normalized === "lighting stagehand" || normalized === "lighting hand") return "Lighting Stagehand";
  if (normalized === "lighting assist" || normalized === "lighting assistant" || normalized === "lighting technician l2" || normalized === "l2") return "Lighting Assist";
  if (normalized === "av tech" || normalized === "av technician" || normalized === "audio visual tech" || normalized === "audio visual technician") return "General AV";
  return raw;
}
...
export function runEstimateGroupHeaderSelfTest() {
  ...
  for (const area of ["SANOFI", "NOVARTIS", "PHAMAESSENTIA LABOR & EXPENSES", "PHARMA ESSENTIA LIGHTING EQUIPMENT", "Jazz AV"]) {
  ...
  const workingLeadHalfDayParsed = parseImportedEventFile("Estimate working lead half-day regression.pdf", [
    "Bill to",
    "NMR Events",
    "Ship to",
    "PHOENIX CONVENTION CENTER",
    "100 N THIRD STREET",
    "PHOENIX, AZ",
    "Estimate details Event: ACAAI26",
    "Estimate no.: 1040 (ACAAI26)",
    "Estimate date: 07/27/2026",
    "# Service Date Product/service Description Qty Rate Amount",
    "1. NMR LABOR",
    "RO328063 PO178025",
    "2. 11/15/2026 AV Tech (Working Lead) half day 1:30PM | 6:30PM 1 $325.00 $325.00",
    "3. 11/16/2026 AV Tech half day (Working Lead) 1:30PM | 6:30PM 1 $325.00 $325.00",
    "Total $650.00",
  ].join("\n"), {});
  const workingLeadHalfDayRows = workingLeadHalfDayParsed.rows.filter((row) => row.position === "AVT / Working Lead");
  if (workingLeadHalfDayRows.length !== 2 || workingLeadHalfDayRows.some((row) => row.dayType !== "half_day")) {
    throw new Error("Working Lead half-day regression did not normalize AV Tech half-day working lead rows to AVT / Working Lead half-day.");
  }
  if (normalizeImportedRoleName("AV Tech (Working Lead) half day") !== "AVT / Working Lead") {
    throw new Error("Working Lead half-day role matcher did not normalize AV Tech (Working Lead) half day.");
  }

  const roAreaParsed = parseImportedEventFile("Estimate 1028 (PSYCHC26) RO area regression.pdf", [
    "Bill to",
    "NMR Events",
    "Ship to",
    ...
    "NEW ORLEANS, LA",
    "Estimate details Event: PSYCHC26",
    "Estimate no.: 1028 (PSYCHC26)",
    "Estimate date: 06/09/2026",
    "# Service Date Product/service Description Qty Rate Amount",
    "20. JAZZ",
    "Booth TBD",
    "RO328574 PO178023",
    "21. 09/15/2026 AV Tech 8:00AM | 6:00PM 1 $450.00 $450.00",
    "22. 09/18/2026 AV Tech (half day) 3:30PM | 8:30PM 1 $225.00 $225.00",
    "Total $675.00",
  ].join("\n"), {});
  const jazzRows = roAreaParsed.rows.filter((row) => row.poNumber === "PO178023");
  if (jazzRows.length !== 2 || jazzRows.some((row) => row.area !== "JAZZ RO328574" || row.boothNumber !== "TBD")) {
    throw new Error("RO area regression did not preserve JAZZ with RO328574 on PO178023 rows.");
  }
  ...
  const estimate1049Parsed = parseImportedEventFile("Estimate 1049 (2).pdf", [
    "ESTIMATE",
    "Bill to Ship to",
    "NMR Events ASSEMBLY STUDIOS",
    "28 Abeel Rd. 3900 MOTORS INDUSTRIAL WAY",
    "Monroe Township, NJ 08831 DORAVILLE, GA",
    "United States",
    "Estimate details",
    "Event: ASSEMBLYSGD26",
    "Estimate no.: 1049",
    "Estimate date: 09/21/2026",
    "# Service Date Product/service Description Qty Rate Amount",
    "1. ASSEMBLY ASSEMBLYSGD26",
    "LED & VIDEO",
    "RO328829.01 PO179052",
    "2. 10/03/2026 LED Stagehand 8:00AM | 6:00PM 4 $500.00 $2,000.00",
    "3. 10/03/2026 Video Assist (V2) Working Lead 8:00AM | 6:00PM 1 $650.00 $650.00",
    "4. 10/04/2026 Video Assist (V2) 8:00AM | 6:00PM 1 $550.00 $550.00",
    "5. 10/05/2026 Video Assist (V2) 8:00AM | 6:00PM 1 $550.00 $550.00",
    "6. 10/06/2026 Video Assist (V2) 8:00AM | 6:00PM 1 $550.00 $550.00",
    "7. 10/07/2026 Video Assist (V2) 8:00AM | 6:00PM 1 $550.00 $550.00",
    "8. 10/08/2026 LED Stagehand 8:00AM | 6:00PM 4 $500.00 $2,000.00",
    "9. 10/08/2026 Video Assist (V2) Working Lead 8:00AM | 6:00PM 1 $650.00 $650.00",
    "Subtotal Subtotal $7,500.00",
    "10. ASSEMBLY ASSEMBLYSGD26",
    "AUDIO",
    "RO328829.02 PO179053",
    "11. 10/03/2026 General AV Tech 8:00AM | 6:00PM 2 $450.00 $900.00",
    "12. 10/08/2026 General AV Tech 8:00AM | 6:00PM 2 $450.00 $900.00",
    "Subtotal Subtotal $1,800.00",
    "13. ASSEMBLY ASSEMBLYSGD26",
    "LIGHTING / RIGGING",
    "RO328829.03 PO179054",
    "14. 10/02/2026 Lighting Stagehand 8:00AM | 6:00PM 6 $500.00 $3,000.00",
    "15. 10/03/2026 Lighting Stagehand 8:00AM | 6:00PM 6 $500.00 $3,000.00",
    "16. 10/03/2026 Lighting Technician L2 8:00AM | 6:00PM 1 $550.00 $550.00",
    "17. 10/04/2026 Lighting Technician L2 8:00AM | 6:00PM 1 $550.00 $550.00",
    "18. 10/05/2026 Lighting Technician L2 8:00AM | 6:00PM 1 $550.00 $550.00",
    "19. 10/06/2026 Lighting Technician L2 8:00AM | 6:00PM 1 $550.00 $550.00",
    "20. 10/07/2026 Lighting Technician L2 8:00AM | 6:00PM 1 $550.00 $550.00",
    "21. 10/08/2026 Lighting Stagehand 8:00AM | 6:00PM 6 $500.00 $3,000.00",
    "22. 10/08/2026 Lighting Technician L2 8:00AM | 6:00PM 1 $550.00 $550.00",
    "Subtotal Subtotal $12,300.00",
    "23. ASSEMBLY ASSEMBLYSGD26",
    "SCENIC 1",
    "RO328829.04 PO179055",
    "24. 10/02/2026 Stagehand 8:00AM | 6:00PM 4 $400.00 $1,600.00",
    "25. 10/09/2026 Stagehand 8:00AM | 6:00PM 4 $400.00 $1,600.00",
    "Subtotal Subtotal $3,200.00",
    "Total",
    "$24,800.00",
  ].join("\n"), {});
  if (estimate1049Parsed.rows.length !== 21) {
    throw new Error(`Estimate 1049 regression expected 21 labor rows, found ${estimate1049Parsed.rows.length}.`);
  }
  if (estimate1049Parsed.show.client !== "NMR Events" || estimate1049Parsed.show.venue !== "Assembly Studios") {
    throw new Error(`Estimate 1049 regression parsed ${estimate1049Parsed.show.client} / ${estimate1049Parsed.show.venue}, expected NMR Events / Assembly Studios.`);
  }
  if (estimate1049Parsed.show.rate_city !== "Atlanta") {
    throw new Error(`Estimate 1049 regression parsed crew pool ${estimate1049Parsed.show.rate_city}, expected Atlanta.`);
  }
  if (estimate1049Parsed.show.parsed_line_total !== 24800 || estimate1049Parsed.show.estimate_total !== 24800) {
    throw new Error(`Estimate 1049 regression totals were ${estimate1049Parsed.show.parsed_line_total} / ${estimate1049Parsed.show.estimate_total}, expected 24800.`);
  }
  const estimate1049References = new Set(estimate1049Parsed.rows.map((row) => row.area.match(/RO\d+\.\d+/)?.[0] || ""));
  for (const reference of ["RO328829.01", "RO328829.02", "RO328829.03", "RO328829.04"]) {
    if (!estimate1049References.has(reference)) throw new Error(`Estimate 1049 regression did not preserve ${reference}.`);
  }
  const estimate1049Positions = new Set(estimate1049Parsed.rows.map((row) => row.position));
  for (const position of ["LED Stagehand", "AVT / Working Lead", "Video Assist", "General AV", "Lighting Stagehand", "Lighting Assist", "Stagehand"]) {
    if (!estimate1049Positions.has(position)) throw new Error(`Estimate 1049 regression did not normalize ${position}.`);
  }

  const distinctStagehandAssistParsed = parseImportedEventFile("Distinct stagehand and assist roles.pdf", [
    "Bill to",
    "NMR Events",
    "Ship to",
    ...
    "100 TEST STREET",
    "ATLANTA, GA",
    "Estimate details Event: ROLE TEST",
    "Estimate no.: 1050",
    "Estimate date: 09/22/2026",
    "# Service Date Product/service Description Qty Rate Amount",
    "1. ROLE TEST",
    "RO328900.01 PO179100",
    "2. 10/03/2026 LED Stagehand 8:00AM | 6:00PM 1 $400.00 $400.00",
    "3. 10/03/2026 LED Assist 8:00AM | 6:00PM 1 $500.00 $500.00",
    "4. 10/03/2026 Lighting Stagehand 8:00AM | 6:00PM 1 $400.00 $400.00",
    "5. 10/03/2026 Lighting Assist 8:00AM | 6:00PM 1 $500.00 $500.00",
    "Total $1,800.00",
  ].join("\n"), {});
  const distinctRoles = distinctStagehandAssistParsed.rows.map((row) => row.position);
  for (const role of ["LED Stagehand", "LED Assist", "Lighting Stagehand", "Lighting Assist"]) {
    if (distinctRoles.filter((position) => position === role).length !== 1) {
      throw new Error(`Distinct stagehand/assist regression did not preserve ${role}.`);
    }
    if (normalizeImportedRoleName(role) !== role) {
      throw new Error(`Imported role matcher did not preserve ${role}.`);
    }
  }
}
