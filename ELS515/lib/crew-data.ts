...
function leadFromMatchesCurrentUser(value: unknown, session: Awaited<ReturnType<typeof getSessionUser>>) {
...
function stripPrivateOnboardingFields<T extends Record<string, unknown>>(record: T, canViewPrivateTaxInfo: boolean): T {
  ...
  return {
    ...record,
    // Coordinators may track operational onboarding progress for crew in their
    // own scoped pool, but private files and tax details remain owner/admin only.
...
export async function getCrewPageData() {
  ...
  const canViewPrivateTaxInfo = role === "owner" || role === "admin";
  ...
  let allowedPoolIds = new Set(session.access?.allowed_city_pool_ids ?? []);
  ...
  const [cityPoolsRes, crewGroupsRes, crewResInitial, positionsRes, unavailableRes, ratesRes, extraPoolsRes, ratingsRes, profilesRes, accessSettingsRes] = await Promise.all([
    dataClient.from("city_pools").select("id, name").order("name", { ascending: true }),
    dataClient.from("crew_groups").select("id, city_pool_id, name").order("name", { ascending: true }),
    dataClient
      .from("crew")
    ...
    dataClient.from("crew_unavailable_dates").select("crew_id, unavailable_date").order("unavailable_date", { ascending: true }),
    dataClient.from("master_rates").select("id, city_name, role_name, full_day, half_day, overtime_multiplier, doubletime_multiplier").order("city_name", { ascending: true }).order("role_name", { ascending: true }),
    dataClient.from("crew_city_pools").select("crew_id, city_pool_id"),
    dataClient.from("tech_ratings").select("id, show_id, client_id, client_contact_id, crew_id, assignment_id, rating, notes, created_at, updated_at").order("updated_at", { ascending: false }),
    dataClient.from("profiles").select("id, email, full_name, role, is_active"),
    dataClient.from("user_access_settings").select("user_id, restrict_crew_to_owner, allowed_city_pool_ids"),
  ]);

  const feedbackRatingsRes = await dataClient
  ...
  const crewBlacklistMissing = Boolean(crewResInitial.error && (crewResInitial.error.message.includes("blacklisted") || crewResInitial.error.message.includes("blacklist_reason") || crewResInitial.error.message.includes("created_by") || crewResInitial.error.message.includes("coordinator_hidden") || crewResInitial.error.message.includes("onboarding_") || crewResInitial.error.message.includes("w9_status") || crewResInitial.error.message.includes("contract_status") || crewResInitial.error.message.includes("questionnaire_status") || crewResInitial.error.message.includes("tax_profile_status") || crewResInitial.error.message.includes("profile_photo_url") || crewResInitial.error.message.includes("work_photo_urls") || crewResInitial.error.message.includes("lead_from") || crewResInitial.error.message.includes("address")));
  ...
  const sharedCoordinatorIdsByUser = new Map<string, string[]>();
  if (admin && (role === "owner" || role === "admin")) {
    ...
    if (!authUsersResult.error) {
      for (const authUser of authUsersResult.data.users) {
        sharedCoordinatorIdsByUser.set(authUser.id, sharedCoordinatorUserIds(authUser));
      }
    }
  } else if (session.user?.id) {
    sharedCoordinatorIdsByUser.set(session.user.id, sharedCoordinatorUserIds(session.user));
  }
  ...
  const allAppUsers = profilesMissing ? ([] as AppUserSummaryRecord[]) : ((profilesRes.data ?? []).map((row) => {
    ...
    return {
      ...
      shared_coordinator_user_ids: sharedCoordinatorIdsByUser.get(typed.id) ?? [],
      restrict_crew_to_owner: access?.restrict_crew_to_owner ?? false,
    } satisfies AppUserSummaryRecord;
  }));
  const appUsers = role === "coordinator" ? allAppUsers.filter((user) => user.id === session.user?.id) : allAppUsers;
  // ELS169: coordinator pool visibility must come from the saved access row,
  // not only from the session access snapshot. The session can be empty/stale
  // for coordinators, which made allowed city pools disappear from Crew.
  if (role === "coordinator" && session.user?.id) {
    const authoritativeAccess = accessByUserId.get(session.user.id);
    const authoritativePoolIds = authoritativeAccess?.allowed_city_pool_ids ?? [];
    if (authoritativePoolIds.length || allowedPoolIds.size === 0) {
      allowedPoolIds = new Set(authoritativePoolIds);
    }
  }
  ...
  const crewRecords: CrewRecord[] = (crewRes.data ?? []).filter((row) => {
    const typed = row as { id?: string | null; created_by?: string | null; city_pool_id?: string | null; lead_from?: string | null; onboarding_status?: string | null; coordinator_hidden_at?: string | null; coordinator_hidden_by?: string | null };
    if (String(typed.onboarding_status || "") === "pending_contact") return false;
    if (role === "coordinator" && typed.coordinator_hidden_at && typed.coordinator_hidden_by === session.user?.id) return false;
    if (role === "coordinator") {
      // Hard privacy boundary: a checked city pool gives the coordinator a city workspace only.
      // It must never reveal Storm/admin master contacts in that city. Coordinators can see
      // only contacts they personally added, plus their own unassigned draft records.
      if (coordinatorOwnerIds.has(String(typed.created_by || "")) || leadFromMatchesCurrentUser(typed.lead_from, session)) return true;
      return false;
    }
    return true;
  }).map((row) => {
    ...
    return stripPrivateOnboardingFields({
      ...
      onboarding_status: String(typed.onboarding_status || "not_started"),
      w9_status: String(typed.w9_status || "missing"),
      contract_status: String(typed.contract_status || "missing"),
      questionnaire_status: String(typed.questionnaire_status || "missing"),
      tax_profile_status: String(typed.tax_profile_status || "missing"),
      profile_photo_url: typed.profile_photo_url ? String(typed.profile_photo_url) : null,
      work_photo_urls: Array.isArray(typed.work_photo_urls) ? typed.work_photo_urls.map(String).filter(Boolean) : [],
      w9_document_url: typed.w9_document_url ? String(typed.w9_document_url) : null,
      contract_document_url: typed.contract_document_url ? String(typed.contract_document_url) : null,
      tax_profile_notes: typed.tax_profile_notes ? String(typed.tax_profile_notes) : "",
      onboarding_request_sent_at: typed.onboarding_request_sent_at ? String(typed.onboarding_request_sent_at) : null,
      onboarding_completed_at: typed.onboarding_completed_at ? String(typed.onboarding_completed_at) : null,
    ...
    } satisfies CrewRecord, canViewPrivateTaxInfo);
  });
  ...
  const visibleCityPools = role === "coordinator" ? cityPools.filter((pool) => allowedPoolIds.has(pool.id)) : cityPools;
  const visibleCrewGroups = role === "coordinator" ? crewGroups.filter((group) => allowedPoolIds.has(group.city_pool_id)) : crewGroups;

  return { cityPools: visibleCityPools, crewGroups: visibleCrewGroups, crewRecords, masterRates, techRatings, appUsers, currentUserId: session.user?.id || "", currentUserName: session.profile?.full_name || session.user?.email || "", currentUserRole: role, setupMissing: false, error: null as string | null };
}
