...
import { createSupabaseAdminClient, createSupabaseServerClient, invalidateShowScheduleStatus } from "@/lib/supabase-server";
...
function cleanSubCallPayload(row: Record<string, unknown>, allowCoordinatorAssignment = false) {
  const payload = {
    ...
    sub_call_group_id: String(row.sub_call_group_id || "").trim() || randomUUID(),
    ...
    notes: notesWithBoothNumberMarker(notesWithSubCallReferenceMarker(String(row.notes || "").trim(), String(row.sub_call_reference_number || "").trim()), String(row.booth_number || "").trim()),
...
export async function POST(request: Request) {
  ...
  // Prevent duplicate occurrences of the same logical sub-call on the same labor day.
  // This is intentionally enforced server-side because an earlier request can succeed at the
  // database layer and then fail in a post-save side effect; a retry must never create a
  // second copy of the same grouped sub-call.
  const targetDayIds = [...new Set(rows.map((row) => row.labor_day_id).filter(Boolean))];
  const targetGroupIds = [...new Set(rows.map((row) => row.sub_call_group_id).filter(Boolean))];
  if (targetDayIds.length && targetGroupIds.length) {
    const duplicateCheck = await admin
      .from("sub_calls")
      .select("id, labor_day_id, sub_call_group_id")
      .in("labor_day_id", targetDayIds)
      .in("sub_call_group_id", targetGroupIds);
    if (duplicateCheck.error) {
      return NextResponse.json({
        message: `Could not verify existing sub-call days before saving: ${duplicateCheck.error.message}`,
        code: "SUB_CALL_DUPLICATE_CHECK_FAILED",
      }, { status: 400 });
    }
    const existingPairs = new Set((duplicateCheck.data || []).map((existing) =>
      `${String((existing as { labor_day_id?: string | null }).labor_day_id || "")}::${String((existing as { sub_call_group_id?: string | null }).sub_call_group_id || "")}`
    ));
    const duplicateRows = rows.filter((row) => existingPairs.has(`${row.labor_day_id}::${row.sub_call_group_id}`));
    if (duplicateRows.length) {
      return NextResponse.json({
        message: "This sub-call already exists on one or more selected labor days. Refresh the event before trying again.",
        code: "SUB_CALL_DAY_ALREADY_EXISTS",
        labor_day_ids: [...new Set(duplicateRows.map((row) => row.labor_day_id))],
      }, { status: 409 });
    }
  }

  // Resolve sort order for every target labor day in one query instead of one round-trip per row.
  // Adding the same sub-call to several days should stay responsive even on larger events.
  const missingSortDayIds = [...new Set(rows.filter((row) => row.sort_order <= 0).map((row) => row.labor_day_id).filter(Boolean))];
  const nextSortByDay = new Map<string, number>();
  if (missingSortDayIds.length) {
    const sortRes = await admin
      .from("sub_calls")
      .select("labor_day_id, sort_order")
      .in("labor_day_id", missingSortDayIds);
    if (!sortRes.error) {
      for (const dayId of missingSortDayIds) nextSortByDay.set(dayId, 0);
      for (const existing of sortRes.data || []) {
        const dayId = String((existing as { labor_day_id?: string | null }).labor_day_id || "");
        if (!dayId) continue;
        const current = nextSortByDay.get(dayId) || 0;
        const candidate = Number((existing as { sort_order?: number | null }).sort_order || 0) + 1;
        if (candidate > current) nextSortByDay.set(dayId, candidate);
      }
    }
  }
  const orderedRows = rows.map((row) => {
    if (row.sort_order > 0) return row;
    const nextSortOrder = nextSortByDay.get(row.labor_day_id) || 0;
    nextSortByDay.set(row.labor_day_id, nextSortOrder + 1);
    return { ...row, sort_order: nextSortOrder };
  });
  ...
  const createdLaborDayIds = [...new Set(createdRows.map((row) => String(row.labor_day_id || "")).filter(Boolean))];
  let scheduleInvalidation: Awaited<ReturnType<typeof invalidateShowScheduleStatus>> | null = null;
  let showIds: string[] = [];
  if (createdLaborDayIds.length) {
    const daysRes = await admin.from("labor_days").select("show_id").in("id", createdLaborDayIds);
    if (!daysRes.error) {
      showIds = [...new Set((daysRes.data || []).map((row: { show_id?: string | null }) => String(row.show_id || "")).filter(Boolean))];
    }
  }
  // The sub-call insert above is the primary transaction. Post-save work must never turn a
  // successful insert into a failed HTTP response; otherwise the UI reports failure even
  // though the row already exists, and a retry can duplicate the day. Return any follow-up
  // problem as an explicit warning while preserving the successfully-created rows.
  const warnings: string[] = [];
  if (showIds.length) {
    try {
      scheduleInvalidation = await invalidateShowScheduleStatus(admin, showIds, {
        reason: "New position/sub-call added. Send updated schedules.",
        changedByUserId: auth.user.id,
      });
    } catch (error) {
      const message = error instanceof Error ? error.message : "Unknown schedule invalidation error.";
      warnings.push(`Sub-call saved, but schedule status cleanup needs attention: ${message}`);
    }
  }
  try {
    await createSubCallCoordinatorNotifications(admin, createdRows, auth.user.id);
  } catch (error) {
    const message = error instanceof Error ? error.message : "Unknown coordinator notification error.";
    warnings.push(`Sub-call saved, but the coordinator notification was not created: ${message}`);
  }
  return NextResponse.json({
    ok: true,
    id: createdRows[0]?.id,
