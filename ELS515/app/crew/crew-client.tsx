...
import { ChangeEvent, Fragment, useEffect, useMemo, useRef, useState, type PointerEvent } from "react";
...
import { crewRoleRateOptions, defaultNewCrewPositions, getDefaultCrewPayRate } from "@/lib/crew-pay-defaults";
...
type AdminUploadKind = "profile_photo" | "work_photo" | "w9" | "contract";

type AdminUploadStatus = Partial<Record<AdminUploadKind, { loading: boolean; text: string; kind: "success" | "error" | "info" }>>;

type AdminProfileCropState = {
  file: File;
  previewUrl: string;
  imageWidth: number;
  imageHeight: number;
...
const ADMIN_PROFILE_CROP_PREVIEW_SIZE = 240;
const ADMIN_PROFILE_CROP_OUTPUT_SIZE = 320;

const adminUploadLabels: Record<AdminUploadKind, string> = {
...
function uploadFileBaseName(name: string) {
  return name.replace(/\.[^.]+$/, "") || "photo";
}

async function uploadCanvasToBlob(canvas: HTMLCanvasElement, type: string, quality: number) {
...
async function compressCrewUploadImage(file: File, maxWidth: number, maxHeight: number, quality: number) {
  ...
  try {
    ...
    const blob = await uploadCanvasToBlob(canvas, "image/webp", quality);
    return new File([blob], `${uploadFileBaseName(file.name)}.webp`, { type: "image/webp", lastModified: Date.now() });
  } finally {
    URL.revokeObjectURL(objectUrl);
  }
}

async function prepareCrewUploadFile(kind: AdminUploadKind, file: File) {
  if (kind === "profile_photo") return compressCrewUploadImage(file, 320, 320, 0.76);
  if (kind === "work_photo") return compressCrewUploadImage(file, 1600, 1600, 0.75);
  return file;
}

function adminProfileCropLimits(crop: AdminProfileCropState) {
  const baseScale = Math.max(
    ADMIN_PROFILE_CROP_PREVIEW_SIZE / crop.imageWidth,
    ADMIN_PROFILE_CROP_PREVIEW_SIZE / crop.imageHeight,
  );
  ...
  return {
    displayWidth,
    displayHeight,
    maxOffsetX: Math.max(0, (displayWidth - ADMIN_PROFILE_CROP_PREVIEW_SIZE) / 2),
    maxOffsetY: Math.max(0, (displayHeight - ADMIN_PROFILE_CROP_PREVIEW_SIZE) / 2),
  };
}

function clampAdminProfileCropOffset(crop: AdminProfileCropState, offsetX: number, offsetY: number) {
  const limits = adminProfileCropLimits(crop);
  return {
...
async function createAdminCroppedProfileFile(crop: AdminProfileCropState) {
  const image = await new Promise<HTMLImageElement>((resolve, reject) => {
    const img = new Image();
    img.onload = () => resolve(img);
  ...
  const canvas = document.createElement("canvas");
  canvas.width = ADMIN_PROFILE_CROP_OUTPUT_SIZE;
  canvas.height = ADMIN_PROFILE_CROP_OUTPUT_SIZE;
  const context = canvas.getContext("2d");
  if (!context) throw new Error("Unable to crop the profile photo.");

  const scale =
    Math.max(
      ADMIN_PROFILE_CROP_OUTPUT_SIZE / image.naturalWidth,
      ADMIN_PROFILE_CROP_OUTPUT_SIZE / image.naturalHeight,
    ) * crop.zoom;
  const renderedWidth = image.naturalWidth * scale;
  const renderedHeight = image.naturalHeight * scale;
  const previewToOutput = ADMIN_PROFILE_CROP_OUTPUT_SIZE / ADMIN_PROFILE_CROP_PREVIEW_SIZE;
  const drawX =
    (ADMIN_PROFILE_CROP_OUTPUT_SIZE - renderedWidth) / 2 +
    crop.offsetX * previewToOutput;
  const drawY =
    (ADMIN_PROFILE_CROP_OUTPUT_SIZE - renderedHeight) / 2 +
    crop.offsetY * previewToOutput;

  context.drawImage(image, drawX, drawY, renderedWidth, renderedHeight);
  const blob = await uploadCanvasToBlob(canvas, "image/webp", 0.76);
  return new File([blob], `${uploadFileBaseName(crop.file.name)}-profile-thumb.webp`, {
    type: "image/webp",
...
function roleOptionLabel(option: { roleName: string; fullDay: number; halfDay: number | null }) {
  const halfDay = halfDayFromFullDay(option.fullDay);
  return `${option.roleName} — $${option.fullDay}${halfDay !== null ? ` / $${halfDay} half` : ""}`;
}
...
function CrewAvatar({ record, size = 38 }: { record: CrewRecord; size?: number }) {
  const photoPath = String(record.profile_photo_url || "").trim();
  ...
  const photoSrc = photoPath
    ? `/api/crew?profile_photo_for=${encodeURIComponent(record.id)}&v=${encodeURIComponent(photoPath)}`
    : "";

  return (
    <span
      aria-label={`${record.name || "Crew member"} profile photo`}
      style={{
        width: size,
        height: size,
        flex: `0 0 ${size}px`,
    ...
    >
      {photoSrc && !photoFailed ? (
        <img
          src={photoSrc}
          alt={`${record.name || "Crew member"} profile`}
          width={size}
          height={size}
          loading="lazy"
...
function blankDraft(cityPools: CityPoolRecord[], cityId?: string, groupName?: string): CrewDraft {
  ...
  return {
    ...
    positions: defaultNewCrewPositions(),
  };
}
...
async function queueOnboardingRequestForShortcut(record: CrewRecord, requestType: OnboardingRequestType = "full_onboarding", queueText = true) {
  const response = await fetch("/api/onboarding", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "create_request", crew_id: record.id, request_type: requestType, queue_text: queueText }),
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.message || (requestType === "w9_only" ? "Unable to queue W-9 request." : requestType === "contract_only" ? "Unable to queue contract request." : requestType === "profile_photo_only" ? "Unable to queue profile photo request." : "Unable to queue onboarding request."));
  return data as OnboardingMessageMaterial;
}
...
function smsHrefWithBody(phone: unknown, body: unknown) {
  const digits = phoneDigits(String(phone || ""));
  if (!digits) return "";
  const normalized = digits.length === 10 ? `1${digits}` : digits;
  return `sms:${normalized}?body=${encodeURIComponent(String(body || ""))}`;
}

function onboardingEmailHref(email: unknown, body: unknown) {
  const address = String(email || "").trim();
  if (!address) return "";
  const subject = "Emanuel Labor Services Onboarding";
  return `mailto:${encodeURIComponent(address)}?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(String(body || ""))}`;
}

type QueuedOnboardingText = { id?: string; phone?: string | null; body?: string | null };

type OnboardingMessageMaterial = {
  found?: boolean;
  link?: string;
  message?: string;
  message_body?: string;
  ...
  reused_link?: boolean;
  queued?: QueuedOnboardingText | null;
};

function maybeOpenAndroidOnboardingText(result: OnboardingMessageMaterial) {
  if (result.delivery_mode !== "android_messages") return false;
  const phone = result.phone || result.queued?.phone;
  const body = result.message_body || result.queued?.body;
  const href = smsHrefWithBody(phone, body);
  if (!href) return false;
  window.location.assign(href);
  return true;
}

async function loadActiveOnboardingMessage(record: CrewRecord) {
  const response = await fetch("/api/onboarding", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "get_active_request", crew_id: record.id }),
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.message || "Unable to load the active onboarding link.");
  return data as OnboardingMessageMaterial;
}

function isSarahLedfordUser(name: unknown) {
  return normalizeText(String(name || "")) === "sarah ledford";
}
...
function positionsFromSkillsets(skillsets: string): PositionInput[] {
  ...
  skillsets
    ...
    .forEach((skill) => {
      ...
      // Example: "Audio", "Video", or "Setup" should create General AV + LED Stagehand
      ...
      if (/\bgav\b|\bavt\b/.test(normalized)) addMappedRole(roles, "General AV");
...
function draftFromPastedContactForm(raw: string, cityPools: CityPoolRecord[], cityId: string, groupName: string, detectedCityName = ""): CrewDraft {
  ...
  return {
    ...
    positions: positions.length ? positions : defaultNewCrewPositions(),
  };
}
...
export default function CrewClient({ cityPools: initialCityPools, crewGroups: initialGroups, initialCrew, masterRates, initialRatings, appUsers, currentUserId, currentUserName, currentUserRole }: CrewClientProps) {
  ...
  const roleOptions = useMemo(() => {
    ...
    for (const rate of masterRates) {
      ...
      if (!map.has(key)) {
        map.set(key, { roleName, fullDay: Number(rate.full_day || 0), halfDay: halfDayFromFullDay(rate.full_day) });
      }
    }
    const topOrder = ["General AV", "AVT / Working Lead", "LED Stagehand", "Stagehand", "BO Tech", "Floater", "Crew Lead", "Warehouse Worker"].map(normalizeText);
  ...
  const [photoCompressionBusy, setPhotoCompressionBusy] = useState(false);
  ...
  const [coordinatorPoolSharesByUser, setCoordinatorPoolSharesByUser] = useState<Record<string, string[]>>(() => Object.fromEntries(appUsers.map((user) => [user.id, user.shared_coordinator_user_ids ?? []])));
  ...
  const canSendOnboarding = isAdminCrewView || normalizeText(currentUserRole) === "coordinator";
  ...
  const coordinatorShareCandidates = useMemo(() => appUsers
    .filter((user) => user.is_active !== false && normalizeText(user.role) === "coordinator" && user.id !== selectedOwnerUser?.id)
    .sort((a, b) => userDisplayName(a).localeCompare(userDisplayName(b))), [appUsers, selectedOwnerUser?.id]);
  const selectedOwnerSharedCoordinatorIds = coordinatorPoolSharesByUser[crewOwnerView] ?? selectedOwnerUser?.shared_coordinator_user_ids ?? [];
  const selectedOwnerSharedCoordinatorSet = new Set(selectedOwnerSharedCoordinatorIds);
  ...
  function toggleCoordinatorPoolShare(userId: string) {
    if (!selectedOwnerUser) return;
    setCoordinatorPoolSharesByUser((current) => {
      const existing = current[selectedOwnerUser.id] ?? selectedOwnerUser.shared_coordinator_user_ids ?? [];
      const next = existing.includes(userId) ? existing.filter((id) => id !== userId) : [...existing, userId];
      return { ...current, [selectedOwnerUser.id]: next };
    });
  }

  async function saveCoordinatorPoolSharing() {
    if (!selectedOwnerUser) return;
    ...
    try {
      const response = await fetch("/api/user-access", {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          action: "save_coordinator_pool_sharing",
          user_id: selectedOwnerUser.id,
          shared_coordinator_user_ids: selectedOwnerSharedCoordinatorIds,
        }),
      });
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.message || "Unable to save shared coordinator pool.");
      const nextByUser = result.shared_coordinator_user_ids_by_user && typeof result.shared_coordinator_user_ids_by_user === "object"
        ? Object.fromEntries(Object.entries(result.shared_coordinator_user_ids_by_user as Record<string, unknown>).map(([id, value]) => [id, Array.isArray(value) ? value.map(String) : []]))
        : {};
      setCoordinatorPoolSharesByUser((current) => ({ ...current, ...nextByUser }));
      setMessageKind("success");
      setMessage(selectedOwnerSharedCoordinatorIds.length
        ? `Shared coordinator pool saved for ${selectedOwnerSharedCoordinatorIds.length + 1} coordinators.`
        : "This coordinator's crew pool is now isolated.");
      router.refresh();
    } catch (error) {
      setMessageKind("error");
      setMessage(error instanceof Error ? error.message : "Unable to save shared coordinator pool.");
    } finally {
  ...
  async function acknowledgeCoordinatorHidden(id: string) {
    if (!window.confirm("Clear the red notice while keeping this contact hidden from the coordinator? The contact will remain preserved in Storm’s Master Pool.")) return;
    setSaving(true);
    setMessage(null);
    try {
      const response = await fetch(`/api/crew/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "acknowledge_coordinator_hidden" }),
      });
      const result = await response.json();
      if (!response.ok) throw new Error(result.message || "Unable to clear this notice.");
      const reviewedAt = String(result.reviewed_at || new Date().toISOString());
      setCrewRecords((current) => current.map((record) => record.id === id ? {
        ...record,
        coordinator_hidden_reviewed_at: reviewedAt,
      } : record));
      setMessageKind("success");
      setMessage(result.message || "Red notice cleared. The contact remains hidden from the coordinator.");
      router.refresh();
    } catch (error) {
      setMessageKind("error");
      setMessage(error instanceof Error ? error.message : "Unable to clear this notice.");
    } finally {
      setSaving(false);
    }
  }

  async function restoreCoordinatorHidden(id: string) {
    if (!window.confirm("Restore this contact to the coordinator’s view? This is separate from merely clearing the red admin notice.")) return;
    setSaving(true);
    setMessage(null);
    try {
      const response = await fetch(`/api/crew/${id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "restore_coordinator_hidden" }),
      });
      const result = await response.json();
      if (!response.ok) throw new Error(result.message || "Unable to restore this contact.");
      setCrewRecords((current) => current.map((record) => record.id === id ? {
        ...record,
      ...
      setMessageKind("success");
      setMessage(result.message || "Contact restored to the coordinator’s view.");
      router.refresh();
    } catch (error) {
      setMessageKind("error");
      setMessage(error instanceof Error ? error.message : "Unable to restore this contact.");
    } finally {
      setSaving(false);
    }
  }
  ...
  async function deleteRecord(id: string) {
    ...
    try {
      ...
      if (result.soft_deleted && isAdminCrewView) {
        ...
        setCrewRecords((current) => current.map((record) => record.id === id ? { ...record, coordinator_hidden_at: hiddenAt, coordinator_hidden_by: hiddenBy, coordinator_hidden_reviewed_at: null } : record));
      } else {
  ...
  async function compressExistingProfilePhotos() {
    if (!isAdminCrewView) return;
    const sourceRecords = selectedIds.length
      ? ownerScopedCrewRecords.filter((record) => selectedIds.includes(record.id))
      : visibleCrew;
    const recordsWithPhotos = sourceRecords.filter((record) => String(record.profile_photo_url || "").trim());
    if (!recordsWithPhotos.length) {
      setMessageKind("error");
      setMessage(selectedIds.length ? "Selected crew do not have profile photos to compress." : "No visible profile photos to compress.");
      return;
    }
    const confirmed = window.confirm(
      `Compress ${recordsWithPhotos.length} current profile photo${recordsWithPhotos.length === 1 ? "" : "s"} to 320×320 private WebP thumbnails? This replaces the active app photo path.`
    );
    if (!confirmed) return;

    setPhotoCompressionBusy(true);
    setMessageKind("success");
    setMessage(`Compressing 0 of ${recordsWithPhotos.length} profile photos...`);
    ...
    try {
      for (const record of recordsWithPhotos) {
        try {
          const photoPath = String(record.profile_photo_url || "").trim();
          const imageResponse = await fetch(`/api/crew?profile_photo_for=${encodeURIComponent(record.id)}&v=${encodeURIComponent(photoPath)}`, { cache: "no-store" });
          if (!imageResponse.ok) throw new Error("Unable to load current photo.");
          const blob = await imageResponse.blob();
          if (!blob.type.startsWith("image/")) throw new Error("Current photo is not an image.");
          const inputFile = new File([blob], `${uploadFileBaseName(record.name || record.id)}-current-profile.${blob.type.split("/")[1] || "webp"}`, { type: blob.type, lastModified: Date.now() });
          const compressedFile = await compressCrewUploadImage(inputFile, 320, 320, 0.76);
          const formData = new FormData();
          formData.append("action", "admin_upload_document");
          formData.append("crew_id", record.id);
          formData.append("crew_name", record.name || "");
          formData.append("document_type", "profile_photo");
          formData.append("file", compressedFile, `${uploadFileBaseName(record.name || record.id)}-profile-thumb.webp`);
          const uploadResponse = await fetch("/api/onboarding", { method: "POST", body: formData });
          const result = await uploadResponse.json().catch(() => ({}));
          if (!uploadResponse.ok) throw new Error(result.message || "Unable to save compressed photo.");
          const nextPath = String(result.path || result.crew_patch?.profile_photo_url || "").trim();
          if (nextPath) {
            setCrewRecords((current) => current.map((item) => item.id === record.id ? { ...item, profile_photo_url: nextPath } : item));
          }
          compressedCount += 1;
          setMessage(`Compressing ${compressedCount} of ${recordsWithPhotos.length} profile photos...`);
        } catch (error) {
          failures.push(`${record.name || "Crew"}: ${error instanceof Error ? error.message : "Unable to compress photo."}`);
        }
      }
      setMessageKind(failures.length ? "error" : "success");
      setMessage(
        failures.length
          ? `Compressed ${compressedCount} photo${compressedCount === 1 ? "" : "s"}. ${failures.slice(0, 3).join(" ")}${failures.length > 3 ? " ..." : ""}`
          : `Compressed ${compressedCount} profile photo${compressedCount === 1 ? "" : "s"} to 320×320 WebP thumbnails.`
      );
      router.refresh();
    } finally {
      setPhotoCompressionBusy(false);
    }
  }
  ...
  function exportCapitalOneVendors() {
    if (!isAdminCrewView) return;
    const selectedCrew = ownerScopedCrewRecords.filter((record) => selectedIds.includes(record.id));
    if (!selectedCrew.length) {
      setMessageKind("error");
      setMessage("Select one or more crew members first.");
      return;
    }

    const headers = ["Vendor Business name", "Contact name", "Email", "Phone NO."];
    const body = selectedCrew.map((record) => [
      record.name,
      record.name,
      record.email || "",
      record.phone || "",
    ...
    a.href = url;
    a.download = `capital-one-vendors-${new Date().toISOString().slice(0, 10)}.csv`;
    document.body.appendChild(a);
    a.click();
    a.remove();
    URL.revokeObjectURL(url);
    setMessageKind("success");
    setMessage(`Created Capital One vendor CSV for ${selectedCrew.length} crew member${selectedCrew.length === 1 ? "" : "s"}.`);
  }
  ...
  function buildCsvImportDraft(row: Record<string, string>, poolsForImport = cityPools, primaryPool?: CityPoolRecord | null, additionalPoolIds: string[] = []): CrewDraft | null {
    ...
    return {
      ...
      positions: ratedPositions.length ? ratedPositions : (rateValue > 0 ? [{ role_name: "General AV", rate: rateValue }] : defaultNewCrewPositions()),
    };
  }
  ...
  return (
        ...
        <section className="card compact" style={{ background: "#fbfcfd" }}>
          ...
          {selectedOwnerUser ? (
            <div className="card compact" style={{ background: "#fff", marginTop: 12 }}>
              ...
              <div className="card compact" style={{ background: "#f8fafc", marginTop: 14, borderColor: selectedOwnerSharedCoordinatorIds.length ? "#0f766e" : undefined }}>
                <div className="row" style={{ alignItems: "flex-start", gap: 12 }}>
                  <div>
                    <strong>Shared coordinator pool</strong>
                    <p className="small muted" style={{ margin: "4px 0 0" }}>
                      Combine this coordinator with selected coordinators so they can see and staff from one another&apos;s crew. The group is mutual; unchecked coordinators stay isolated. Admin always sees all crew, and the coordinator who added a contact remains its owner.
                    </p>
                  </div>
                  <button className="primary" type="button" disabled={coordinatorAccessSaving} onClick={() => void saveCoordinatorPoolSharing()}>
                    {coordinatorAccessSaving ? "Saving..." : "Save shared pool"}
                  </button>
                </div>
                {coordinatorShareCandidates.length ? (
                  <div className="grid grid-3" style={{ marginTop: 12 }}>
                    {coordinatorShareCandidates.map((user) => (
                      <label key={user.id} className="checkbox-card">
                        <input type="checkbox" checked={selectedOwnerSharedCoordinatorSet.has(user.id)} onChange={() => toggleCoordinatorPoolShare(user.id)} />
                        <span><strong>{userDisplayName(user)}</strong><small>{selectedOwnerSharedCoordinatorSet.has(user.id) ? "Combined" : "Isolated"}</small></span>
                      </label>
                    ))}
                  </div>
                ) : (
                  <p className="small muted" style={{ margin: "10px 0 0" }}>No other active coordinators are available to combine.</p>
                )}
              </div>
          ...
          <section style={{ borderRight: "1px solid var(--line)", padding: 16 }}>
            <div className="row" style={{ alignItems: "flex-start", marginBottom: 14 }}>
              ...
              <div className="toolbar" style={{ justifyContent: "flex-end" }}>
                ...
                {isAdminCrewView ? (
                  <button className="ghost" type="button" onClick={() => void compressExistingProfilePhotos()} disabled={photoCompressionBusy || saving}>
                    {photoCompressionBusy ? "Compressing..." : selectedIds.length ? `Compress ${selectedIds.length}` : "Compress visible photos"}
                  </button>
                ) : null}
            ...
            <div className="toolbar" style={{ marginBottom: 14 }}>
              ...
              {isAdminCrewView ? (
                <button className="primary" type="button" onClick={exportCapitalOneVendors} disabled={!selectedIds.length}>
                  {selectedIds.length ? `Create Capital One vendor spreadsheet (${selectedIds.length})` : "Create Capital One vendor spreadsheet"}
                </button>
              ) : null}
            ...
            {draft && adding ? (
              <div className="card compact" style={{ background: "#fbfcfd", marginBottom: 14 }}>
                ...
                <CrewEditor
                  ...
                  saving={saving}
                  canEditPrivateOnboarding={isAdminCrewView}
                />
              </div>
            ) : null}
    ...
    setFormImporterOpen(false);
                    ...
                    >
                      ...
                      <CrewAvatar record={record} size={38} />
                      <span style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {isAdminCrewView && record.coordinator_hidden_at && !record.coordinator_hidden_reviewed_at ? (
                      ...
                      <div className="crew-inline-detail" style={{ borderBottom: "1px solid var(--line)", background: "#fff", padding: 14 }}>
                        {draft && editingId === record.id ? (
                          ...
                          <ContactDetail
                            ...
                            onAcknowledgeCoordinatorHidden={() => acknowledgeCoordinatorHidden(record.id)}
                            onRestoreCoordinatorHidden={() => restoreCoordinatorHidden(record.id)}
                            onMove={(cityId, groupName) => moveSelected([record.id], cityId, groupName)}
                            onQueueOnboarding={async (options) => {
                              try {
                                const queueText = options?.queueText !== false;
                                const result = await queueOnboardingRequestForShortcut(record, "full_onboarding", queueText);
                                setMessageKind("success");
                                setMessage(result.message || "Onboarding request queued using your account messaging mode.");
                                setCrewRecords((current) => current.map((item) => item.id === record.id ? {
                                  ...item,
                                  onboarding_status: "request_sent",
                                  questionnaire_status: "requested",
                                  ...(isAdminCrewView ? { w9_status: "requested", contract_status: "requested" } : {}),
                                  onboarding_request_sent_at: new Date().toISOString(),
                                } : item));
                                router.refresh();
                                return result;
                              } catch (error) {
                                setMessageKind("error");
                                setMessage(error instanceof Error ? error.message : "Unable to queue onboarding request.");
                                return undefined;
                              }
                            }}
                            onQueueW9Only={async () => {
                              try {
                                const result = await queueOnboardingRequestForShortcut(record, "w9_only");
                                setMessageKind("success");
                                setMessage(result.message || "W-9 request queued using your account messaging mode.");
                                maybeOpenAndroidOnboardingText(result);
                                setCrewRecords((current) => current.map((item) => item.id === record.id ? {
                                  ...item,
                                  w9_status: "requested",
                                ...
                                setMessageKind("error");
                                setMessage(error instanceof Error ? error.message : "Unable to queue W-9 request.");
                              }
                            }}
                            onQueueContractOnly={async () => {
                              try {
                                const result = await queueOnboardingRequestForShortcut(record, "contract_only");
                                setMessageKind("success");
                                setMessage(result.message || "Contract-only request queued using your account messaging mode.");
                                maybeOpenAndroidOnboardingText(result);
                                setCrewRecords((current) => current.map((item) => item.id === record.id ? {
                                  ...item,
                                ...
                                setMessageKind("error");
                                setMessage(error instanceof Error ? error.message : "Unable to queue contract request.");
                              }
                            }}
                            onQueueProfilePhotoOnly={async () => {
                              try {
                                const result = await queueOnboardingRequestForShortcut(record, "profile_photo_only");
                                setMessageKind("success");
                                setMessage(result.message || "Profile photo request queued using your account messaging mode.");
                                maybeOpenAndroidOnboardingText(result);
                                setCrewRecords((current) => current.map((item) => item.id === record.id ? {
                                  ...item,
                                  onboarding_request_sent_at: new Date().toISOString(),
                                ...
                                setMessageKind("error");
                                setMessage(error instanceof Error ? error.message : "Unable to queue profile photo request.");
                              }
                            }}
                            onAdminDocumentUploaded={(recordId, patch) => {
                              setCrewRecords((current) => current.map((item) => item.id === recordId ? { ...item, ...patch } : item));
                              router.refresh();
                            }}
                            ...
                            canSendOnboarding={canSendOnboarding}
                            directAndroidOnboarding={isSarahLedfordUser(currentUserName)}
                          />
                        )}
                      </div>
                    ) : null}
...
type ContactDetailProps = {
  ...
  onEdit: () => void;
  onDelete: () => void;
  onAcknowledgeCoordinatorHidden: () => void;
  onRestoreCoordinatorHidden: () => void;
  onMove: (cityId: string, groupName: string) => void;
  onQueueOnboarding: (options?: { queueText?: boolean }) => Promise<OnboardingMessageMaterial | undefined>;
  onQueueW9Only: () => void;
  onQueueContractOnly: () => void;
  onQueueProfilePhotoOnly: () => void;
  onAdminDocumentUploaded: (recordId: string, patch: Partial<CrewRecord>) => void;
  onBulkCityChange: (value: string) => void;
  onBulkGroupChange: (value: string) => void;
  saving: boolean;
  canSeeAdminAudit: boolean;
  canSendOnboarding: boolean;
  directAndroidOnboarding: boolean;
};

function ContactDetail({ record, ratingSummary, cityPools, targetGroups, bulkCityId, bulkGroupName, detailTab, onTab, onEdit, onDelete, onAcknowledgeCoordinatorHidden, onRestoreCoordinatorHidden, onMove, onQueueOnboarding, onQueueW9Only, onQueueContractOnly, onQueueProfilePhotoOnly, onAdminDocumentUploaded, onBulkCityChange, onBulkGroupChange, saving, canSeeAdminAudit, canSendOnboarding, directAndroidOnboarding }: ContactDetailProps) {
  ...
  const onboardingComplete = Boolean(
    ...
    record.onboarding_successfully_onboarded ||
    normalizeText(record.onboarding_status || "") === "approved"
  );
  const onboardingProgressLabel = onboardingComplete
    ? "Complete"
    : ["submitted", "needs_review", "uploaded"].includes(normalizeText(record.onboarding_status || ""))
      ? "Submitted for Owner/Admin review"
      : record.onboarding_request_sent_at
        ? "In progress"
        : "Not started";
  const [uploadStatus, setUploadStatus] = useState<AdminUploadStatus>({});
  const [documentActionLoading, setDocumentActionLoading] = useState(false);
  const [documentActionMessage, setDocumentActionMessage] = useState<{ text: string; kind: "success" | "error" | "info" } | null>(null);
  const [profileCrop, setProfileCrop] = useState<AdminProfileCropState | null>(null);
  const [profileCropSaving, setProfileCropSaving] = useState(false);
  const [profileCropLoading, setProfileCropLoading] = useState(false);
  const [onboardingMessageMaterial, setOnboardingMessageMaterial] = useState<OnboardingMessageMaterial | null>(null);
  const [onboardingMessageLoading, setOnboardingMessageLoading] = useState(false);
  const [onboardingMessageNotice, setOnboardingMessageNotice] = useState<string>("");
  const [onboardingEmailSuccessToast, setOnboardingEmailSuccessToast] = useState("");
  const [onboardingEmailStatus, setOnboardingEmailStatus] = useState<"idle" | "sending" | "sent">("idle");
  const onboardingEmailToastTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  const profileCropDrag = useRef<{ pointerId: number; startX: number; startY: number; offsetX: number; offsetY: number } | null>(null);
  const profileFileInputRef = useRef<HTMLInputElement | null>(null);
  const uploadDisabled = saving || profileCropSaving || profileCropLoading || Object.values(uploadStatus).some((status) => status?.loading);
  ...
  useEffect(() => () => {
    if (onboardingEmailToastTimerRef.current) clearTimeout(onboardingEmailToastTimerRef.current);
  }, []);

  function showOnboardingEmailSuccessToast(recipientEmail: string) {
    const recipient = recipientEmail.trim();
    setOnboardingEmailSuccessToast(recipient ? `Onboarding email sent successfully to ${recipient}.` : "Onboarding email sent successfully.");
    if (onboardingEmailToastTimerRef.current) clearTimeout(onboardingEmailToastTimerRef.current);
    onboardingEmailToastTimerRef.current = setTimeout(() => {
      setOnboardingEmailSuccessToast("");
      setOnboardingEmailStatus("idle");
    }, 4500);
  }

  useEffect(() => {
    if (detailTab !== "onboarding" || !canSendOnboarding) return;
    let cancelled = false;
    setOnboardingMessageLoading(true);
    setOnboardingMessageNotice("");
    void loadActiveOnboardingMessage(record)
      .then((material) => {
        if (!cancelled) setOnboardingMessageMaterial(material.found ? material : null);
      })
      .catch((error) => {
        if (!cancelled) setOnboardingMessageNotice(error instanceof Error ? error.message : "Unable to load the onboarding message.");
      })
      .finally(() => {
        if (!cancelled) setOnboardingMessageLoading(false);
      });
    return () => {
      cancelled = true;
    };
  }, [detailTab, canSendOnboarding, record.id]);

  async function sendFullOnboardingFromContact() {
    if (!record.phone) return;
    if (directAndroidOnboarding) onTab("onboarding");
    setOnboardingMessageLoading(true);
    setOnboardingMessageNotice("");
    try {
      const result = await onQueueOnboarding();
      let material: OnboardingMessageMaterial | null =
        result?.link && result?.message_body ? result : await loadActiveOnboardingMessage(record);
      if (material?.found === false) material = null;
      if (material) setOnboardingMessageMaterial(material);
      if (directAndroidOnboarding && material) {
        const opened = maybeOpenAndroidOnboardingText(material);
        if (!opened) setOnboardingMessageNotice("Samsung Messages did not open automatically. Use Open Samsung Messages or Copy message below.");
      }
    } catch (error) {
      setOnboardingMessageNotice(error instanceof Error ? error.message : "Unable to prepare the onboarding message.");
    } finally {
      setOnboardingMessageLoading(false);
    }
  }

  async function sendFullOnboardingByEmailFromContact() {
    if (!record.email || onboardingEmailStatus === "sending") return;
    onTab("onboarding");
    if (onboardingEmailToastTimerRef.current) {
      clearTimeout(onboardingEmailToastTimerRef.current);
      onboardingEmailToastTimerRef.current = null;
    }
    setOnboardingEmailSuccessToast("");
    setOnboardingEmailStatus("sending");
    setOnboardingMessageNotice("");
    try {
      const result = await onQueueOnboarding({ queueText: false });
      let material: OnboardingMessageMaterial | null =
        result?.link && result?.message_body ? result : await loadActiveOnboardingMessage(record);
      if (material?.found === false) material = null;
      if (!material?.message_body) {
        setOnboardingEmailStatus("idle");
        setOnboardingMessageNotice("Unable to prepare the onboarding email message.");
        return;
      }
      setOnboardingMessageMaterial(material);
      const response = await fetch("/api/onboarding", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      const emailResult = (await response.json()) as { ok?: boolean; message?: string };
      if (!response.ok || !emailResult.ok) throw new Error(emailResult.message || "Unable to send onboarding email.");
      setOnboardingMessageNotice(emailResult.message || `Onboarding email sent to ${record.email}.`);
      setOnboardingEmailStatus("sent");
      showOnboardingEmailSuccessToast(record.email);
    } catch (error) {
      setOnboardingEmailStatus("idle");
      setOnboardingMessageNotice(error instanceof Error ? error.message : "Unable to send the onboarding email.");
    }
  }

  function copyOnboardingText(value: string, successMessage: string) {
    if (navigator.clipboard?.writeText) {
      void navigator.clipboard.writeText(value).then(
        () => setOnboardingMessageNotice(successMessage),
  ...
  function setUploadMessage(kind: AdminUploadKind, text: string, messageKind: "success" | "error" | "info", loading = false) {
    setUploadStatus((current) => ({ ...current, [kind]: { text, kind: messageKind, loading } }));
  }

  async function beginAdminProfileCrop(file: File | undefined) {
    if (!file) return;
    ...
    try {
      ...
      if (!dimensions.width || !dimensions.height) throw new Error("Unable to read the selected profile photo.");
      setProfileCrop({
        file,
        previewUrl,
      ...
      setUploadMessage("profile_photo", "Drag and zoom the photo to center the face, then save it.", "info");
    } catch (error) {
      URL.revokeObjectURL(previewUrl);
      setUploadMessage("profile_photo", error instanceof Error ? error.message : "Unable to read the selected profile photo.", "error");
    }
  }

  async function beginCropFromCurrentProfilePhoto() {
    const photoPath = String(record.profile_photo_url || "").trim();
    if (!photoPath) {
    ...
    setUploadMessage("profile_photo", "Loading the current profile picture...", "info", true);
    try {
      const response = await fetch(`/api/crew?profile_photo_for=${encodeURIComponent(record.id)}&v=${encodeURIComponent(photoPath)}`, { cache: "no-store" });
      if (!response.ok) throw new Error("Unable to load the current profile picture.");
      const blob = await response.blob();
      const file = new File([blob], `${uploadFileBaseName(record.name || "crew-profile")}-current-profile.webp`, {
        type: blob.type || "image/webp",
        lastModified: Date.now(),
      });
      await beginAdminProfileCrop(file);
    } catch (error) {
      setUploadMessage("profile_photo", error instanceof Error ? error.message : "Unable to load the current profile picture.", "error");
    } finally {
      setProfileCropLoading(false);
    }
  }

  function updateAdminProfileCropZoom(zoom: number) {
    setProfileCrop((current) => {
      if (!current) return current;
      const next = { ...current, zoom };
      return { ...next, ...clampAdminProfileCropOffset(next, next.offsetX, next.offsetY) };
    });
  }

  function handleAdminProfileCropPointerDown(event: PointerEvent<HTMLDivElement>) {
  ...
  function handleAdminProfileCropPointerMove(event: PointerEvent<HTMLDivElement>) {
    const drag = profileCropDrag.current;
    if (!drag || drag.pointerId !== event.pointerId) return;
    setProfileCrop((current) => {
      if (!current) return current;
      const offsets = clampAdminProfileCropOffset(
        current,
  ...
  function endAdminProfileCropDrag(event: PointerEvent<HTMLDivElement>) {
    if (profileCropDrag.current?.pointerId !== event.pointerId) return;
    profileCropDrag.current = null;
    if (event.currentTarget.hasPointerCapture(event.pointerId)) event.currentTarget.releasePointerCapture(event.pointerId);
  }

  async function saveAdminCroppedProfilePhoto() {
    if (!profileCrop) return;
    setProfileCropSaving(true);
    setUploadMessage("profile_photo", "Preparing and uploading the cropped profile picture...", "info", true);
    try {
      const croppedFile = await createAdminCroppedProfileFile(profileCrop);
      const uploaded = await uploadAdminFiles("profile_photo", [croppedFile]);
      if (uploaded) setProfileCrop(null);
    } catch (error) {
      setUploadMessage("profile_photo", error instanceof Error ? error.message : "Unable to crop the profile picture.", "error");
    } finally {
      setProfileCropSaving(false);
    }
  }

  async function openAdminDocument(kind: AdminUploadKind, storagePath: string | null | undefined, download = false) {
    const path = String(storagePath || "").trim();
    if (!path) return;
    setDocumentActionLoading(true);
    setDocumentActionMessage({ text: download ? "Preparing secure download link..." : "Preparing secure view link...", kind: "info" });
    try {
      const response = await fetch("/api/onboarding", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.message || "Unable to create secure file link.");
      const signedUrl = String(result.signed_url || "").trim();
      if (!signedUrl) throw new Error("Secure file link was not returned.");
      if (download) {
        ...
        anchor.click();
        anchor.remove();
        setDocumentActionMessage({ text: "Secure download started. It expires in 10 minutes.", kind: "success" });
        return;
      }
      const documentLabel = `${record.name || "Crew member"} ${adminUploadLabels[kind]}`;
      const returnPath = `${window.location.pathname}${window.location.search}`;
      openDocumentInApp(signedUrl, documentLabel, returnPath, result.viewer_kind === "image" ? "image" : "document");
      return;
    } catch (error) {
      setDocumentActionMessage({ text: error instanceof Error ? error.message : "Unable to open secure file.", kind: "error" });
    } finally {
  ...
  async function removeAdminProfilePhoto() {
    const path = String(record.profile_photo_url || "").trim();
    if (!path) return;
    const confirmed = window.confirm(`Remove ${record.name || "this crew member"}'s profile photo? The crew contact will stay in the Master Pool.`);
    if (!confirmed) return;
    setUploadMessage("profile_photo", "Removing profile photo...", "info", true);
    try {
      const response = await fetch("/api/onboarding", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      const result = await response.json().catch(() => ({}));
      if (!response.ok) throw new Error(result.message || "Unable to remove profile photo.");
      onAdminDocumentUploaded(record.id, { profile_photo_url: "" });
      setUploadMessage("profile_photo", "Profile photo removed.", "success");
    } catch (error) {
      setUploadMessage("profile_photo", error instanceof Error ? error.message : "Unable to remove profile photo.", "error");
    }
  }

  function secureDocumentActions(label: string, kind: AdminUploadKind, path: string | null | undefined) {
    const cleanPath = String(path || "").trim();
    return (
      <div className="card compact" style={{ background: "#fff" }}>
        <div className="row" style={{ alignItems: "center", gap: 10 }}>
          <div style={{ minWidth: 0 }}>
            <strong>{label}</strong>
            <div className="small muted" style={{ overflowWrap: "anywhere" }}>{cleanPath || "Missing"}</div>
          </div>
          <div className="toolbar" style={{ justifyContent: "flex-end" }}>
            <button className="ghost" type="button" disabled={!cleanPath || documentActionLoading} onClick={() => void openAdminDocument(kind, cleanPath, false)}>Open</button>
            <button className="ghost" type="button" disabled={!cleanPath || documentActionLoading} onClick={() => void openAdminDocument(kind, cleanPath, true)}>Download</button>
          </div>
        </div>
      </div>
    );
  ...
  async function uploadAdminFiles(kind: AdminUploadKind, list: FileList | File[] | null): Promise<boolean> {
    const files = Array.from(list || []);
    if (!files.length) return false;
    setUploadMessage(kind, kind === "work_photo" ? `Uploading ${files.length} work photo${files.length === 1 ? "" : "s"}...` : `Uploading ${adminUploadLabels[kind]}...`, "info", true);
    try {
      const uploadedPaths: string[] = [];
      for (const originalFile of files) {
        const file = await prepareCrewUploadFile(kind, originalFile);
        const formData = new FormData();
        formData.append("action", "admin_upload_document");
        formData.append("crew_id", record.id);
        ...
        const result = await response.json().catch(() => ({}));
        if (!response.ok) throw new Error(result.message || `Unable to upload ${adminUploadLabels[kind]}.`);
        const path = String(result.path || "").trim();
        if (path) uploadedPaths.push(path);
      }

      const patch: Partial<CrewRecord> = {};
      if (kind === "profile_photo") patch.profile_photo_url = uploadedPaths[uploadedPaths.length - 1] || record.profile_photo_url || "";
      if (kind === "w9") {
        patch.w9_document_url = uploadedPaths[uploadedPaths.length - 1] || record.w9_document_url || "";
        patch.w9_status = "uploaded";
        patch.tax_profile_status = "needs_review";
      }
      if (kind === "contract") {
        patch.contract_document_url = uploadedPaths[uploadedPaths.length - 1] || record.contract_document_url || "";
        patch.contract_status = "uploaded";
      }
      if (kind === "work_photo") {
        const merged = new Set([...(record.work_photo_urls || []), ...uploadedPaths]);
        patch.work_photo_urls = Array.from(merged);
      }
      onAdminDocumentUploaded(record.id, patch);
      setUploadMessage(kind, kind === "work_photo" ? `${uploadedPaths.length} work photo${uploadedPaths.length === 1 ? "" : "s"} uploaded securely.` : `${adminUploadLabels[kind]} uploaded securely.`, "success");
      return true;
    } catch (error) {
      setUploadMessage(kind, error instanceof Error ? error.message : `Unable to upload ${adminUploadLabels[kind]}.`, "error");
      return false;
    }
  }
  ...
  return (
    <div>
      ...
      <div style={{ border: "1px solid var(--line)", borderRadius: 18, padding: 18, background: "#fbfcfd", marginBottom: 14 }}>
        <div className="row" style={{ alignItems: "flex-start" }}>
          <div className="row" style={{ gap: 12, alignItems: "center", justifyContent: "flex-start" }}>
            <CrewAvatar record={record} size={56} />
            <div>
              ...
              {!canSeeAdminAudit && canSendOnboarding ? (
                <div style={{ marginTop: 6 }}>
                  <span className={`badge ${onboardingComplete ? "success" : "warning"}`}>
                    Onboarding: {onboardingProgressLabel}
                  </span>
                </div>
              ) : null}
              {canSeeAdminAudit && record.coordinator_hidden_at ? (
                <div style={{ marginTop: 4 }}>
                  {!record.coordinator_hidden_reviewed_at ? (
                    ...
                    <div className="small muted">
                      Hidden from the coordinator’s view. Review notice cleared; Master Pool record retained.
                    </div>
                  )}
                  <div className="toolbar" style={{ justifyContent: "flex-start", marginTop: 8, gap: 8 }}>
                    {!record.coordinator_hidden_reviewed_at ? (
                      <button
                        className="ghost"
                        type="button"
                        onClick={onAcknowledgeCoordinatorHidden}
                        disabled={saving}
                        style={{ padding: "7px 12px", color: "#166534", borderColor: "#86efac", background: "#f0fdf4" }}
                      >
                        Keep hidden and clear red notice
                      </button>
                    ) : null}
                    <button
                      className="ghost"
                      type="button"
                      onClick={onRestoreCoordinatorHidden}
                      disabled={saving}
                      style={{ padding: "7px 12px" }}
                    >
                      Restore to coordinator view
                    </button>
                  </div>
                </div>
              ) : null}
            ...
            {canSeeAdminAudit ? (
              <>
                <button
                  ...
                  onClick={() => {
                    if (record.profile_photo_url) void beginCropFromCurrentProfilePhoto();
                ...
                {record.profile_photo_url ? (
                  <button
                    ...
                    type="button"
                    disabled={uploadDisabled}
                    onClick={() => void removeAdminProfilePhoto()}
                  >
                    Remove photo
                  </button>
                ) : null}
                <input
                  ...
                  style={{ display: "none" }}
                  onChange={(event) => {
                    void beginAdminProfileCrop(event.target.files?.[0]);
                    event.target.value = "";
                  }}
                />
              </>
            ...
            <div style={{ display: "grid", gap: 6 }}>
              {canSeeAdminAudit ? (
                <button
                  className="primary"
                  type="button"
                  disabled={saving || !record.phone}
                  onClick={() => {
                    if (!record.phone) return;
                    const confirmed = window.confirm(
                      `Create a secure full-onboarding link for ${record.name} and send/queue it using your account messaging mode?`,
                    );
                    if (confirmed) void sendFullOnboardingFromContact();
                  }}
                  style={{ background: "#16843f", borderColor: "#16843f" }}
                >
                  {record.phone ? "Send onboarding" : "Phone needed for onboarding"}
                </button>
              ) : null}
              <button
                className="primary"
                type="button"
                disabled={saving || onboardingMessageLoading || onboardingEmailStatus === "sending" || !record.email}
                onClick={() => void sendFullOnboardingByEmailFromContact()}
                style={{ background: "#16843f", borderColor: "#16843f" }}
              >
                {!record.email
                  ? "Email needed for onboarding"
                  : onboardingEmailStatus === "sending"
                    ? "Email is sending..."
                    : onboardingEmailStatus === "sent"
                      ? "Email sent"
                      : "Send onboarding by email"}
              </button>
            </div>
          ) : null}
          {canSeeAdminAudit ? (
      ...
      {profileCrop ? (() => {
        const limits = adminProfileCropLimits(profileCrop);
        const imageLeft =
          (ADMIN_PROFILE_CROP_PREVIEW_SIZE - limits.displayWidth) / 2 +
          profileCrop.offsetX;
        const imageTop =
          (ADMIN_PROFILE_CROP_PREVIEW_SIZE - limits.displayHeight) / 2 +
          profileCrop.offsetY;
        return (
          <div
          ...
          >
            <div className="row" style={{ alignItems: "flex-start", gap: 18 }}>
              <div style={{ minWidth: 0 }}>
                <h3 style={{ margin: "0 0 4px" }}>Adjust profile picture</h3>
                <p className="muted small" style={{ margin: "0 0 12px" }}>
                  Drag the photo to center the face and use the zoom control. This crop will be used on the Crew contact and feedback surveys.
                </p>
                <p className="muted small" style={{ margin: 0 }}>
                  To reveal parts that were already cropped out of the saved picture, choose the original photo again.
                </p>
              </div>
              <div style={{ flex: "0 0 auto" }}>
                <div
                  ...
                  onPointerDown={handleAdminProfileCropPointerDown}
                  onPointerMove={handleAdminProfileCropPointerMove}
                  onPointerUp={endAdminProfileCropDrag}
                  onPointerCancel={endAdminProfileCropDrag}
                  style={{
                    position: "relative",
                    width: ADMIN_PROFILE_CROP_PREVIEW_SIZE,
                    height: ADMIN_PROFILE_CROP_PREVIEW_SIZE,
                    maxWidth: "100%",
                    margin: "0 auto",
                    overflow: "hidden",
                    borderRadius: "50%",
                ...
                >
                  <img
                    ...
                    alt="Admin profile crop preview"
                    draggable={false}
                    style={{
                      position: "absolute",
                      left: imageLeft,
            ...
            <label className="field" style={{ marginTop: 12 }}>
              ...
              <input
                ...
                onChange={(event) => updateAdminProfileCropZoom(Number(event.target.value))}
              />
            </label>
            <div className="toolbar" style={{ marginTop: 12 }}>
              <button
                ...
                onClick={() => void saveAdminCroppedProfilePhoto()}
                style={{ background: "#16843f", borderColor: "#16843f" }}
              >
                {profileCropSaving ? "Saving profile picture..." : "Save profile picture"}
              </button>
              <button
                type="button"
                className="ghost"
            ...
            {uploadStatus.profile_photo ? (
              <p
                className={uploadStatus.profile_photo.kind === "error" ? "error" : uploadStatus.profile_photo.kind === "success" ? "success" : "muted small"}
      ...
      <div className="toolbar" style={{ marginBottom: 14 }}>
        {(["info", "roles", "availability", ...(canSendOnboarding ? ["onboarding" as const] : []), "notes", "move"] as DetailTab[]).map((tab) => (
      ...
      {!canSeeAdminAudit && canSendOnboarding && detailTab === "onboarding" ? (
        <div className="list">
          <div className="card compact" style={{ background: "#fbfcfd" }}>
            <div className="row" style={{ alignItems: "flex-start", gap: 12 }}>
              <div>
                <h3 style={{ marginTop: 0 }}>Onboarding</h3>
                <p className="muted small">Send the secure full onboarding packet to crew in your pool and track whether onboarding is complete. Tax IDs, tax numbers, W-9 details, private documents, and approval controls remain available only to Owner/Admin.</p>
              </div>

            </div>
            <div className="grid grid-2">
              <InfoRow label="Onboarding complete" value={onboardingComplete ? "Yes" : "No"} />
              <InfoRow label="Progress" value={onboardingProgressLabel} />
              <InfoRow label="Questionnaire" value={statusLabel(record.questionnaire_status || "missing", documentStatusOptions)} />
              <InfoRow label="Private paperwork" value="Owner/Admin review only" />
            ...
            <div style={{ marginTop: 16, paddingTop: 16, borderTop: "1px solid var(--line)" }}>
              <h4 style={{ margin: "0 0 8px" }}>Onboarding message</h4>
              {onboardingMessageLoading ? <p className="muted small">Loading secure onboarding message...</p> : null}
              {!onboardingMessageLoading && onboardingMessageMaterial?.message_body ? (
                <textarea
                  readOnly
                  value={onboardingMessageMaterial.message_body}
                  aria-label="Onboarding message to copy"
                  style={{ width: "100%", minHeight: 190, resize: "vertical", font: "inherit", lineHeight: 1.45 }}
                />
              ) : !onboardingMessageLoading ? (
                <p className="muted small">No active secure onboarding link is available yet. Sending the onboarding email will create the secure link automatically.</p>
              ) : null}
              <div className="toolbar" style={{ marginTop: 10, flexWrap: "wrap" }}>
                {directAndroidOnboarding && onboardingMessageMaterial?.message_body && smsHrefWithBody(onboardingMessageMaterial.phone || record.phone, onboardingMessageMaterial.message_body) ? (
                  <a
                    className="primary"
                    href={smsHrefWithBody(onboardingMessageMaterial.phone || record.phone, onboardingMessageMaterial.message_body)}
                  >
                ...
                {record.email ? (
                  <button
                    className="primary"
                    type="button"
                    disabled={saving || onboardingEmailStatus === "sending"}
                    onClick={() => void sendFullOnboardingByEmailFromContact()}
                    style={{ background: "#16843f", borderColor: "#16843f" }}
                  >
                    {onboardingEmailStatus === "sending"
                  ...
                  <button className="primary" type="button" disabled>Email needed for onboarding</button>
                )}
                {onboardingMessageMaterial?.message_body ? (
                  <button
                    className="ghost"
                    type="button"
                    onClick={() => copyOnboardingText(onboardingMessageMaterial.message_body || "", "Onboarding message copied.")}
                  >
                    Copy message
                  </button>
                ) : null}
                {onboardingMessageMaterial?.link ? (
                  <button
                    className="ghost"
                    type="button"
                    onClick={() => copyOnboardingText(onboardingMessageMaterial.link || "", "Onboarding link copied.")}
                  >
                    Copy link
                  </button>
                ) : null}
              ...
              {onboardingMessageMaterial?.message_body ? (
                <p className="muted small" style={{ marginBottom: 0 }}>This uses the active secure onboarding link. Re-sending onboarding reuses the same active link when possible.</p>
              ) : null}
              {onboardingMessageNotice ? <p className="small" style={{ marginBottom: 0 }}>{onboardingMessageNotice}</p> : null}
            </div>
          </div>
        </div>
      ) : null}
      ...
      {canSeeAdminAudit && detailTab === "onboarding" ? (
        <div className="list">
          <div className="card compact" style={{ background: "#fbfcfd" }}>
            <div className="row" style={{ alignItems: "flex-start", gap: 12 }}>
              <div>
                ...
                <p className="muted small">Send onboarding using this existing contact, then track the W-9, contract, questionnaire, profile photo, and tax profile review. Sending from here never creates a duplicate Crew record.</p>
              </div>
              <div className="toolbar" style={{ justifyContent: "flex-end" }}>
                <button className="primary" type="button" disabled={saving || !record.phone} onClick={() => void onQueueOnboarding()}>
                  {record.phone ? "Send full onboarding" : "Phone needed"}
                </button>
                <button className="ghost" type="button" disabled={saving || !record.phone} onClick={onQueueW9Only}>
                  {record.phone ? "Queue W-9 only" : "Phone needed"}
                </button>
                <button className="ghost" type="button" disabled={saving || !record.phone} onClick={onQueueContractOnly}>
                  {record.phone ? "Queue contract only" : "Phone needed"}
                </button>
                <button className="ghost" type="button" disabled={saving || !record.phone} onClick={onQueueProfilePhotoOnly}>
                  {record.phone ? "Queue Profile Photo Request" : "Phone needed"}
                </button>
              </div>
            </div>
            <div className="grid grid-2">
          ...
          <div className="grid grid-2">
            <div className="card compact" style={{ background: "#fbfcfd" }}>
              <h3 style={{ marginTop: 0 }}>Secure files</h3>
              <p className="small muted" style={{ marginTop: -4 }}>Open/download buttons create owner-only temporary links. Links expire after 10 minutes and the API verifies the file is attached to this crew profile.</p>
              <div className="list" style={{ gap: 10 }}>
                {secureDocumentActions("Profile photo", "profile_photo", record.profile_photo_url)}
                {secureDocumentActions("W-9 document", "w9", record.w9_document_url)}
                {secureDocumentActions("Contract document", "contract", record.contract_document_url)}
              </div>
              {documentActionMessage ? (
                <p className={documentActionMessage.kind === "error" ? "error" : documentActionMessage.kind === "success" ? "success" : "muted small"} style={{ marginTop: 10 }}>
                  {documentActionMessage.text}
                </p>
              ) : null}
              <div className="grid" style={{ gap: 10, marginTop: 14 }}>
                <label className="field">
                  <span>{record.profile_photo_url ? "Replace photo" : "Add photo"}</span>
                  <input
                    type="file"
                    accept="image/*"
                    disabled={uploadDisabled}
                  ...
                  />
                  <span className={uploadStatus.profile_photo?.kind === "error" ? "error" : uploadStatus.profile_photo?.kind === "success" ? "success" : "muted small"}>{uploadStatus.profile_photo?.text || "Choose a photo, position the face, and save the 320×320 private WebP thumbnail."}</span>
                  {record.profile_photo_url ? (
                    <button
                      className="ghost"
                      type="button"
                      disabled={uploadDisabled}
                      onClick={() => void beginCropFromCurrentProfilePhoto()}
                    >
                      Replace from current photo
                    </button>
                  ) : null}
                  {record.profile_photo_url ? (
                    <button
                ...
                <label className="field">
                  ...
                  <input type="file" accept="application/pdf,image/*" disabled={uploadDisabled} onChange={(event) => { void uploadAdminFiles("w9", event.target.files); event.target.value = ""; }} />
                  <span className={uploadStatus.w9?.kind === "error" ? "error" : uploadStatus.w9?.kind === "success" ? "success" : "muted small"}>{uploadStatus.w9?.text || "Archived to secure Google Drive when connected; otherwise saved privately in the app."}</span>
                </label>
                <label className="field">
                  <span>Upload contract</span>
                  <input type="file" accept="application/pdf,image/*" disabled={uploadDisabled} onChange={(event) => { void uploadAdminFiles("contract", event.target.files); event.target.value = ""; }} />
                  <span className={uploadStatus.contract?.kind === "error" ? "error" : uploadStatus.contract?.kind === "success" ? "success" : "muted small"}>{uploadStatus.contract?.text || "Archived to secure Google Drive when connected; otherwise saved privately in the app."}</span>
                </label>
              </div>
            </div>
            <div className="card compact" style={{ background: "#fff7ed" }}>
              <h3 style={{ marginTop: 0 }}>Storage target</h3>
              <p className="small muted" style={{ margin: 0 }}>
                Profile photos are saved as private 320×320 WebP thumbnails for fast app loading. W-9 and contract PDFs should be readable and archived to secure Google Drive when connected.
              </p>
            </div>
          </div>
...
type CrewEditorProps = {
  ...
  onClose: () => void;
  saving: boolean;
  canEditPrivateOnboarding: boolean;
};

function CrewEditor({ cityPools, crewGroups, roleOptions, defaultRateForRole, draft, onChange, onPositionChange, onAddPosition, onRemovePosition, onSave, onClose, saving, canEditPrivateOnboarding }: CrewEditorProps) {
                ...
                <div className="grid grid-3" style={{ alignItems: "end" }}>
                  <label className="field">
                    ...
                    >
                      ...
                      <optgroup label="Top defaults">
                        {roleOptions.filter((option) => option.featured || ["general av", "avt working lead", "led stagehand", "stagehand", "bo tech", "floater", "crew lead", "warehouse worker"].includes(normalizeText(option.roleName))).map((option) => (
                  ...
                  <label className="field">
                    ...
                    <input
                      ...
                      placeholder="General AV, AVT / Working Lead, LED Stagehand..."
                    />
