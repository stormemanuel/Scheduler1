...
export default function PayrollClient({ initialRows, initialEventSummaries, availableYears, initialYear, initialSummaryLoaded, initialError, initialDiagnostics = null }: Props) {
  ...
  async function openTaxCenterForYear() {
    if (taxDataYears.has(year)) {
      setTaxCenterOpen(true);
      return;
    }
    if (loadingTaxYear === year) return;
    setLoadingTaxYear(year);
    setMessage(`Loading ${year} Tax Center data…`);
    try {
      const response = await fetchPayrollRequest(`/api/payroll?mode=tax&year=${encodeURIComponent(String(year))}`);
      ...
      if (!response.ok) throw new Error(result.message || result.error || `Unable to load ${year} Tax Center data (HTTP ${response.status}).`);
      const returnedYear = Number(result.loadedYear ?? year);
      ...
      setTaxDataYears((current) => new Set([...current, returnedYear]));
      setDiagnostics(result.diagnostics ?? null);
      setTaxCenterOpen(true);
      setMessage(result.error || "");
    } catch (error) {
      const nextMessage = error instanceof Error ? error.message : `Unable to load ${year} Tax Center data.`;
      setMessage(nextMessage);
    } finally {
...
(summary: PayrollYearTechSummary, reviewAction: "approve_w9" | "request_correction" | "mark_needs_review") {
  ...
  async function updateAndSaveEventFinancials(event: PayrollEventSummary, patch: EventFinancialPatch) {
    ...
    const statusChange = patch.taxReserveDone !== undefined || patch.consecratedHandsDone !== undefined;
    if (!statusChange) updateEventFinancials(event.showId, next);
    ...
    try {
      ...
      if (!response.ok || !result.ok) {
        const fallback = patch.consecratedHandsDone !== undefined
          ? "Could not save Consecrated Hands payment status."
      ...
      if (patch.consecratedHandsDone !== undefined) {
        setMessage(patch.consecratedHandsDone ? "Consecrated Hands marked paid." : "Consecrated Hands marked unpaid.");
              ...
              <div className="row" style={{ alignItems: "flex-end" }}>
                ...
                <label className="field" style={{ minWidth: 210 }}>
                  ...
                  <span className="small" style={{ display: "grid", gap: 8 }}>
                    ...
                    <label style={{ display: "flex", alignItems: "center", gap: 8 }}><input type="checkbox" checked={event.consecratedHandsDone} disabled={savingFinancialShowId === event.showId} onChange={(e) => void updateAndSaveEventFinancials(event, { consecratedHandsDone: e.target.checked })} /> Consecrated Hands ({money(event.consecratedHandsDonation)})</label>
                  </span>
                </label>
