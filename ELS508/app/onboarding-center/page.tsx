...
type ReviewRow = {
  ...
  crew_id: string;
  status: string;
  request_type: "full_onboarding" | "w9_only" | "contract_only" | "profile_photo_only";
  sent_by_user_id: string;
  sent_by_name: string;
  sent_by_email: string;
  sent_by_names?: string[];
...
type OnboardingAccess = {
  ...
  current_user_messaging_mode?: "apple_shortcut" | "android_messages" | string;
};

type OnboardingAndroidQueueRow = {
  id: string;
  crew_id: string | null;
...
function androidSmsHref(phone: unknown, body: unknown) {
  const digits = phoneDigits(phone);
  if (!digits) return "";
  const normalized = digits.length === 10 ? `1${digits}` : digits;
  return `smsto:${normalized}?body=${encodeURIComponent(text(body))}`;
}

function onboardingEmailHref(email: unknown, body: unknown) {
  const address = text(email);
  if (!address) return "";
  return `mailto:${encodeURIComponent(address)}?subject=${encodeURIComponent("Emanuel Labor Services Onboarding")}&body=${encodeURIComponent(text(body))}`;
}

function isDueNow(value: unknown) {
  const clean = text(value);
...
function requestTypeLabel(value: ReviewRow["request_type"]) {
  ...
  if (value === "profile_photo_only") return "Profile photo only";
  ...
  const requestLine = requestType === "w9_only"
    ...
    : requestType === "contract_only"
      ...
      : requestType === "profile_photo_only"
        ? "Please upload your current profile photo through this secure link:"
...
export default function OnboardingCenterPage() {
  ...
  const [emailSendingButtonId, setEmailSendingButtonId] = useState<string | null>(null);
  const [emailSentButtonId, setEmailSentButtonId] = useState<string | null>(null);
  ...
  const [lastInvitePhone, setLastInvitePhone] = useState("");
  ...
  const [access, setAccess] = useState<OnboardingAccess>({ viewer_role: "viewer", can_review: false, current_user_id: "", current_user_name: "", current_user_messaging_mode: "apple_shortcut" });
  const [androidOnboardingRows, setAndroidOnboardingRows] = useState<OnboardingAndroidQueueRow[]>([]);
  const [androidOnboardingLoading, setAndroidOnboardingLoading] = useState(false);
  ...
  function showEmailSuccessToast(recipientEmail: string) {
    const recipient = recipientEmail.trim();
    setEmailSuccessToast(recipient ? `Onboarding email sent successfully to ${recipient}.` : "Onboarding email sent successfully.");
  }

  useEffect(() => {
    if (!emailSuccessToast) return;
    const timer = window.setTimeout(() => setEmailSuccessToast(""), 4500);
    return () => window.clearTimeout(timer);
  }, [emailSuccessToast]);

  useEffect(() => {
    if (!emailSentButtonId) return;
    const timer = window.setTimeout(() => setEmailSentButtonId(null), 4500);
    return () => window.clearTimeout(timer);
  }, [emailSentButtonId]);
  ...
  async function loadRows() {
    ...
    try {
      ...
      const result = (await response.json()) as { ok?: boolean; rows?: ReviewRow[]; city_pools?: CityPoolOption[]; position_options?: string[]; viewer_role?: string; can_review?: boolean; current_user_id?: string; current_user_name?: string; current_user_messaging_mode?: string; message?: string };
      ...
      setAccess({
        ...
        current_user_messaging_mode: result.current_user_messaging_mode || "apple_shortcut",
      });
      if (result.current_user_messaging_mode === "android_messages") void loadAndroidOnboardingQueue();
    } catch (error) {
  ...
  async function loadAndroidOnboardingQueue() {
    setAndroidOnboardingLoading(true);
    try {
      const response = await fetch("/api/onboarding", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "list_android_onboarding_queue" }),
      });
      const result = (await response.json()) as { ok?: boolean; rows?: OnboardingAndroidQueueRow[]; current_user_messaging_mode?: string; message?: string };
      if (!response.ok || !result.ok) throw new Error(result.message || "Unable to load Android onboarding queue.");
      setAndroidOnboardingRows(result.current_user_messaging_mode === "android_messages" ? result.rows || [] : []);
      if (result.current_user_messaging_mode) {
        setAccess((current) => ({ ...current, current_user_messaging_mode: result.current_user_messaging_mode }));
      }
      if (result.message) setMessage(result.message);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to load Android onboarding queue.");
    } finally {
      setAndroidOnboardingLoading(false);
    }
  }

  async function markAndroidOnboardingOpened(id: string) {
    try {
      await fetch("/api/onboarding", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "mark_android_onboarding_opened", id }),
      });
      await loadAndroidOnboardingQueue();
    } catch {
      // Opening the Messages app is the important step. Leave the row visible if the audit marker cannot be saved.
    }
  }

  async function markAndroidOnboardingSent(id: string) {
    setBusyId(`android-onboarding-${id}`);
    setMessage("");
    try {
      const response = await fetch("/api/onboarding", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "mark_android_onboarding_sent", id }),
      });
      const result = (await response.json()) as { ok?: boolean; message?: string };
      if (!response.ok || !result.ok) throw new Error(result.message || "Unable to mark onboarding text sent.");
      setMessage(result.message || "Onboarding text marked sent.");
      await loadAndroidOnboardingQueue();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to mark onboarding text sent.");
    } finally {
      setBusyId(null);
    }
  }
  ...
  async function openSecureDocument(row: ReviewRow, documentType: DocumentType, storagePath: string, download = false) {
    ...
    try {
      ...
      const result = (await response.json()) as { ok?: boolean; signed_url?: string; message?: string; viewer_kind?: string };
      ...
      openDocumentInApp(result.signed_url, documentLabel, returnPath, result.viewer_kind === "image" ? "image" : "document");
      return;
    } catch (error) {
  ...
  async function approveRow(row: ReviewRow) {
    ...
    try {
      ...
      await loadRows();
      if (access.current_user_messaging_mode === "android_messages") await loadAndroidOnboardingQueue();
    } catch (error) {
  ...
  async function createNewOnboardingRequest(delivery: "queue" | "email" = access.can_review ? "queue" : "email") {
    ...
    const queueText = delivery === "queue";
    const emailMode = delivery === "email";
    if (queueText && !text(newInvite.phone) && !text(newInvite.email)) { setMessage("Enter at least a phone number or email address."); return; }
    if (emailMode && !text(newInvite.email)) { setMessage("Enter the crew member's email address to send onboarding."); return; }
    const emailButtonId = "new-invite";
    setEmailSentButtonId(null);
    if (emailMode) setEmailSendingButtonId(emailButtonId);
    ...
    try {
      ...
      const inviteEmail = newInvite.email;
      const response = await fetch("/api/onboarding", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "create_new_crew_request", ...newInvite, queue_text: queueText }),
      });
      const result = (await response.json()) as { ok?: boolean; crew_id?: string; link?: string; request?: { id?: string }; already_complete?: boolean; already_submitted?: boolean; message?: string };
      if (!response.ok || !result.ok) throw new Error(result.message || "Unable to create onboarding request.");
      if (result.already_complete || result.already_submitted) {
        setMessage(result.message || (result.already_complete ? "This person is already fully onboarded." : "This person's onboarding is already submitted for review."));
        setNewInvite({ name: "", phone: "", email: "" });
        await Promise.all([loadRows(), loadCoordinatorDashboard()]);
        return;
      }
      setLastInviteLink(result.link || "");
      const inviteMessage = onboardingInvitationMessage(inviteName, result.link || "", access.current_user_name);
      setLastInviteMessage(inviteMessage);
      setLastInvitePhone(newInvite.phone);

      if (emailMode) {
        if (!result.crew_id) throw new Error("The onboarding request was created, but the crew record could not be resolved for email delivery.");
        const emailResponse = await fetch("/api/onboarding", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
        ...
        if (!emailResponse.ok || !emailResult.ok) throw new Error(emailResult.message || "Unable to send onboarding email.");
        setMessage(emailResult.message || `Onboarding email sent to ${inviteEmail}.`);
        showEmailSuccessToast(inviteEmail);
        setEmailSentButtonId(emailButtonId);
      } else {
        setMessage(result.message || "Onboarding request created and queued.");
      }

      setNewInvite({ name: "", phone: "", email: "" });
      await Promise.all([loadRows(), loadCoordinatorDashboard()]);
      if (queueText && access.current_user_messaging_mode === "android_messages") await loadAndroidOnboardingQueue();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to create onboarding request.");
    } finally {
      if (emailMode) setEmailSendingButtonId(null);
  ...
  async function sendExistingOnboardingByEmail(row: ReviewRow) {
    const email = text(row.crew.email) || text(row.payload.invite_email);
    if (!email) {
      setMessage("This crew member needs an email address before onboarding can be emailed.");
      return;
    }
    const emailButtonId = `request-${row.id}`;
    setEmailSentButtonId(null);
    setEmailSendingButtonId(emailButtonId);
    setBusyId(`${row.id}-email`);
    setMessage("");
    try {
      const response = await fetch("/api/onboarding", {
        method: "POST",
      ...
      const result = (await response.json()) as { ok?: boolean; message?: string };
      if (!response.ok || !result.ok) throw new Error(result.message || "Unable to send onboarding email.");
      setMessage(result.message || `Onboarding email sent to ${email}.`);
      showEmailSuccessToast(email);
      setEmailSentButtonId(emailButtonId);
      await loadRows();
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to send onboarding email.");
    } finally {
      setEmailSendingButtonId(null);
      setBusyId(null);
    }
  }

  async function sendCoordinatorOnboardingByEmail(row: CoordinatorOnboardingRow) {
    if (coordinatorReadOnly) {
      setMessage("View as user mode is read-only. Exit the preview to send onboarding.");
      return;
    }
    if (!row.email) {
      setMessage(`${row.name || "This crew member"} needs an email address before onboarding can be emailed.`);
      return;
    }
    const emailButtonId = `coordinator-${row.id}`;
    setEmailSentButtonId(null);
    setEmailSendingButtonId(emailButtonId);
    ...
    try {
      const response = await fetch("/api/onboarding", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "create_request", crew_id: row.id, request_type: "full_onboarding", queue_text: false }),
      });
      const result = (await response.json()) as { ok?: boolean; crew_id?: string; already_complete?: boolean; already_submitted?: boolean; message?: string };
      if (!response.ok || !result.ok) throw new Error(result.message || "Unable to prepare onboarding email.");
      if (result.already_complete || result.already_submitted) {
        setMessage(result.message || (result.already_complete ? "This person is already fully onboarded." : "This person's onboarding is already submitted for review."));
        await Promise.all([loadCoordinatorDashboard(), loadRows()]);
        return;
      }
      const resolvedCrewId = result.crew_id || row.id;
      const emailResponse = await fetch("/api/onboarding", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "send_onboarding_email", crew_id: resolvedCrewId, request_type: "full_onboarding" }),
      });
      const emailResult = (await emailResponse.json()) as { ok?: boolean; message?: string };
      if (!emailResponse.ok || !emailResult.ok) throw new Error(emailResult.message || "Unable to send onboarding email.");
      setMessage(emailResult.message || `Onboarding email sent to ${row.email}.`);
      showEmailSuccessToast(row.email);
      setEmailSentButtonId(emailButtonId);
      await Promise.all([loadCoordinatorDashboard(), loadRows()]);
    } catch (error) {
      setMessage(error instanceof Error ? error.message : "Unable to prepare onboarding email.");
    } finally {
      setEmailSendingButtonId(null);
  ...
  async function resendRequest(row: ReviewRow) {
    ...
    try {
      ...
      setRows((current) => current.map((currentRow) => currentRow.id === row.id
        ? { ...currentRow, send_history_count: (Number(currentRow.send_history_count) || 1) + 1 }
        : currentRow));
      if (access.current_user_messaging_mode === "android_messages") await loadAndroidOnboardingQueue();
    } catch (error) {
  ...
  const androidDueRows = androidOnboardingRows.filter((row) => isDueNow(row.scheduled_for));
  const androidFutureRows = androidOnboardingRows.filter((row) => !isDueNow(row.scheduled_for));
  ...
  return (
    <div className="grid" style={{ gap: 16 }}>
          ...
          <div className="row" style={{ alignItems: "flex-start", gap: 14 }}>
            <div>
              ...
              <p className="muted" style={{ margin: 0 }}>
                See safe onboarding progress for crew in your own pool and people you referred through a shared ELS onboarding packet. Private W-9, tax-number, contract-document, and approval details remain visible only to Owner/Admin.
              </p>
            </div>
                ...
                <div className="row" style={{ alignItems: "flex-start", gap: 12 }}>
                  <div>
                    <div className="toolbar" style={{ gap: 7 }}>
                      ...
                      {row.shared_onboarding ? <span className="badge">Shared referral</span> : null}
                    </div>
                  ...
                  <div className="toolbar" style={{ justifyContent: "flex-end", flexWrap: "wrap" }}>
                    <button
                      className="primary"
                      type="button"
                      disabled={coordinatorReadOnly || row.onboarding_complete || coordinatorBusyId === row.id || !row.email || emailSentButtonId === `coordinator-${row.id}`}
                      onClick={() => void sendCoordinatorOnboardingByEmail(row)}
                    ...
                    >
                      {row.onboarding_complete
                        ...
                        : !row.email
                        ? "Email needed"
                        : emailSendingButtonId === `coordinator-${row.id}`
                          ? "Email is sending..."
                          : emailSentButtonId === `coordinator-${row.id}`
                            ? "Email sent"
                            : row.onboarding_request_sent_at
      ...
      {access.current_user_messaging_mode === "android_messages" ? (
        <section className="card">
          <div className="row" style={{ alignItems: "flex-start", gap: 12 }}>
            <div>
              <h2 style={{ marginBottom: 6 }}>My Android onboarding messages</h2>
              <p className="muted" style={{ margin: 0 }}>
                Sarah/coordinator Android workflow: open each onboarding text in Messages, send it from the phone, then mark it sent here. These do not use the iPhone Shortcut.
              </p>
            </div>
            <div className="toolbar">
              <span className="badge">{androidDueRows.length} ready · {androidFutureRows.length} waiting</span>
              <button className="ghost" type="button" onClick={() => void loadAndroidOnboardingQueue()} disabled={androidOnboardingLoading}>Refresh</button>
            </div>
          </div>
          {androidDueRows.length ? (
            <div className="list" style={{ marginTop: 12 }}>
              {androidDueRows.map((row) => {
                const href = androidSmsHref(row.phone, row.body);
                return (
                  <article key={row.id} className="card compact" style={{ boxShadow: "none" }}>
                    <div className="row" style={{ alignItems: "flex-start", gap: 12 }}>
                      <div>
                        <strong>{text(row.crew_name) || "Crew member"}</strong>
                        <div className="muted small">Due {dateTimeLabel(row.scheduled_for)} · {text(row.phone) || "No phone"}</div>
                        {row.opened_in_messages_at ? <div className="muted small">Opened: {dateTimeLabel(row.opened_in_messages_at)}</div> : null}
                      </div>
                      <div className="toolbar">
                        {href ? (
                          <a className="primary" href={href} onClick={() => void markAndroidOnboardingOpened(row.id)}>Open in Messages</a>
                        ) : null}
                        <button className="ghost" type="button" disabled={busyId === `android-onboarding-${row.id}`} onClick={() => void markAndroidOnboardingSent(row.id)}>
                          {busyId === `android-onboarding-${row.id}` ? "Saving..." : "Mark sent"}
                        </button>
                      </div>
                    </div>
                    <p className="small muted" style={{ whiteSpace: "pre-wrap", marginBottom: 0 }}>{text(row.body).slice(0, 420)}{text(row.body).length > 420 ? "…" : ""}</p>
                  </article>
                );
              })}
            </div>
          ) : (
            <p className="muted small" style={{ marginBottom: 0 }}>No Android onboarding texts are due for this signed-in user right now.</p>
          )}
          {androidFutureRows.length ? <p className="muted small" style={{ marginBottom: 0 }}>{androidFutureRows.length} future onboarding reminder text{androidFutureRows.length === 1 ? "" : "s"} will appear here when due.</p> : null}
        </section>
      ) : null}
      ...
      <section className="card">
        ...
        <p className="muted" style={{ marginTop: 0 }}>
          {access.can_review
            ? "Send a full onboarding packet even when the person is not in Crew yet. Matching email or phone records reuse the same active secure link. The Owner/Admin queue workflow can still schedule onboarding texts and automatic reminders."
            : "Create or reuse the secure onboarding packet and send it immediately by email through Emanuel Labor Services. No automatic two-week reminder texts are created for coordinator email onboarding."}
        </p>
        <div className="grid grid-3">
          ...
          <label className="field"><span>Phone</span><input value={newInvite.phone} onChange={(event) => setNewInvite((current) => ({ ...current, phone: event.target.value }))} placeholder={access.can_review ? "Used for the queued text" : "Optional contact phone"} /></label>
          <label className="field"><span>Email{access.can_review ? "" : " (required)"}</span><input type="email" value={newInvite.email} onChange={(event) => setNewInvite((current) => ({ ...current, email: event.target.value }))} /></label>
        </div>
        <div className="toolbar" style={{ marginTop: 12 }}>
          {access.can_review ? (
            <button
              className="primary"
              type="button"
              disabled={newInviteBusy || coordinatorReadOnly || (!text(newInvite.phone) && !text(newInvite.email))}
              onClick={() => void createNewOnboardingRequest("queue")}
            >
              {newInviteBusy && emailSendingButtonId !== "new-invite" ? "Creating..." : "Create/reuse + queue onboarding"}
            </button>
          ) : null}
          <button
            className="primary"
            type="button"
            disabled={newInviteBusy || coordinatorReadOnly || !text(newInvite.email)}
            onClick={() => void createNewOnboardingRequest("email")}
            style={{ background: "#16843f", borderColor: "#16843f" }}
          >
            {emailSendingButtonId === "new-invite"
              ? "Email is sending..."
              : emailSentButtonId === "new-invite"
                ? "Email sent"
                : "Create onboarding and send onboarding by email"}
          </button>
          ...
          {access.can_review && lastInviteMessage && lastInvitePhone ? (
            <a className="primary" href={androidSmsHref(lastInvitePhone, lastInviteMessage)}>
              Send using text message (cell phone)
            </a>
          ) : null}
          <span className="muted small">
            {access.can_review
              ? "Matches by email/phone, reuses an active link when available, and otherwise creates a hidden pending contact. Owner/Admin can continue using the text queue and automatic reminder workflow."
              : "Matches by email/phone, reuses an active link when available, and otherwise creates a hidden pending contact. Coordinator onboarding is sent by Resend email and does not schedule recurring text reminders."}
          </span>
        </div>
          ...
          <div>
            ...
            <p className="muted" style={{ margin: 0 }}>
              {access.can_review
                ...
                : "Track the onboarding links you sent, see when each person opens or submits the packet, and manually send a reminder using the same secure link. Full onboarding also queues automatic reminders every two weeks until submission."}
            </p>
          </div>
          <div className="toolbar">
            ...
            <section key={row.id} className="card">
              <div className="row" style={{ alignItems: "flex-start", gap: 14 }}>
                ...
                <div className="toolbar">
                  ...
                  {(text(row.crew.email) || text(row.payload.invite_email)) ? (
                    <button
                      className="primary"
                      type="button"
                      style={{ background: "#16843f", borderColor: "#16843f" }}
                      disabled={isApproved || Boolean(busyId) || emailSentButtonId === `request-${row.id}`}
                      onClick={() => void sendExistingOnboardingByEmail(row)}
                    >
                      {isApproved
                        ? "Already complete"
                        : emailSendingButtonId === `request-${row.id}`
                        ? "Email is sending..."
                        : emailSentButtonId === `request-${row.id}`
                          ? "Email sent"
                          : row.sent_at
                            ? "Send reminder by email"
                            : "Send onboarding by email"}
              ...
              <div className="grid grid-3" style={{ marginTop: 12 }}>
                ...
                {access.can_review ? <InfoLine label="All coordinators" value={(row.sent_by_names || []).join(", ")} /> : null}
              </div>
              <p className="muted small" style={{ marginBottom: 0 }}>
                This secure link has been used {row.send_history_count || 1} time{(row.send_history_count || 1) === 1 ? "" : "s"}. Admin can see this request and will review private W-9, contract, and tax information after submission. {row.request_type === "full_onboarding" ? "Coordinator reminders are sent by email when you choose Send reminder by email; no automatic two-week reminder texts are created by the coordinator email workflow." : ""}
              </p>
            </section>
          );
        }
          ...
          <section key={row.id} className="card">
            <div className="row" style={{ alignItems: "flex-start", gap: 14 }}>
              <div>
                ...
                {access.can_review && (row.sent_by_names || []).length ? <p className="muted small" style={{ margin: "4px 0 0" }}>All coordinators: {(row.sent_by_names || []).join(", ")}</p> : null}
              </div>
              <div className="toolbar">
                ...
                {!isApproved ? (
                  <button className="ghost" type="button" onClick={() => void copyRowLink(row)} disabled={!row.link || Boolean(busyId)}>
                    Copy same link
                  </button>
                ) : null}
                {!isApproved ? (
                  <button className="ghost" type="button" onClick={() => void copyRowInvitationMessage(row)} disabled={!row.link || Boolean(busyId)}>
                    Copy message
                  </button>
                ) : null}
                {(text(row.crew.email) || text(row.payload.invite_email)) ? (
