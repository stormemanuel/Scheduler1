...
const payrollChangeRequestCountCache: CachedCount = { value: 0, expiresAt: 0, promise: null };
...
async function loadPayrollChangeRequestCount({ force = false } = {}) {
  const now = Date.now();
  if (!force && payrollChangeRequestCountCache.promise) return payrollChangeRequestCountCache.promise;
  if (!force && payrollChangeRequestCountCache.expiresAt > now) return payrollChangeRequestCountCache.value;

  payrollChangeRequestCountCache.promise = fetch("/api/payroll?mode=change_request_count", { cache: "no-store" })
    .then(async (response) => {
      const result = (await response.json().catch(() => ({}))) as { ok?: boolean; unread_count?: number };
      const count = response.ok && result.ok ? Math.max(0, Number(result.unread_count || 0)) : 0;
      payrollChangeRequestCountCache.value = count;
      payrollChangeRequestCountCache.expiresAt = Date.now() + COUNT_CACHE_MS;
      return count;
    })
    .finally(() => {
      payrollChangeRequestCountCache.promise = null;
    });
  return payrollChangeRequestCountCache.promise;
}
...
export function PayrollChangeRequestNavLink({ href, label }: OnboardingNavLinkProps) {
  ...
  useEffect(() => {
    ...
    async function loadCount() {
      if (!shouldRunActivityFetch()) return;
      try {
        const count = await loadPayrollChangeRequestCount();
        if (!cancelled) setUnreadCount(count);
      } catch {
        // Payroll remains usable if the badge count cannot load.
      }
    }
    ...
    function handleCountEvent(event: Event) {
      const nextCount = Number((event as CustomEvent<number>).detail);
      if (Number.isFinite(nextCount) && nextCount >= 0) {
        clearCountCache(payrollChangeRequestCountCache, nextCount);
    ...
    const handleFocus = () => runAfterAppActivity(() => loadPayrollChangeRequestCount({ force: true }).then((count) => {
      if (!cancelled) setUnreadCount(count);
    }));
    window.addEventListener("focus", handleFocus);
    window.addEventListener("els:payroll-change-request-count", handleCountEvent);
    return () => {
      cancelled = true;
      window.clearInterval(intervalId);
      window.removeEventListener("focus", handleFocus);
      window.removeEventListener("els:payroll-change-request-count", handleCountEvent);
    };
  }, []);
  ...
  return (
    <a href={href} className="nav-link-with-badge">
      <span>{label}</span>
      {unreadCount > 0 ? (
        <span className="nav-notification-badge" aria-label={`${unreadCount} contractor Purchase Order change request${unreadCount === 1 ? "" : "s"} not viewed yet`}>
