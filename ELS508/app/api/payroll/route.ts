...
function purchaseOrderChangeRequestIsUnread(po: ContractorPurchaseOrder | null) {
  if (!po || po.status !== "changes_requested" || !String(po.changeRequest || "").trim()) return false;
  if (!po.changeRequestViewedAt) return true;
  if (!po.changeRequestedAt) return false;
  return new Date(po.changeRequestViewedAt).getTime() < new Date(po.changeRequestedAt).getTime();
}

async function unreadPurchaseOrderChangeRequestCount(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>) {
  const result = await admin.from("show_payroll").select("notes").ilike("notes", "%ELS_PURCHASE_ORDER_V1%").limit(5000);
  if (result.error) throw result.error;
  return ((result.data || []) as Array<{ notes?: string | null }>).reduce((count, row) => count + (purchaseOrderChangeRequestIsUnread(extractPurchaseOrderFromNotes(row.notes)) ? 1 : 0), 0);
}
...
export async function GET(request: Request) {
  ...
  if (url.searchParams.get("mode") === "change_request_count") {
    const admin = createSupabaseAdminClient();
    if (!admin) return NextResponse.json({ ok: false, message: "SUPABASE_SERVICE_ROLE_KEY is missing." }, { status: 500 });
    try {
      return NextResponse.json({ ok: true, unread_count: await unreadPurchaseOrderChangeRequestCount(admin) }, { headers: { "Cache-Control": "no-store" } });
    } catch (error) {
      return NextResponse.json({ ok: false, message: error instanceof Error ? error.message : "Unable to count Purchase Order change requests." }, { status: 400 });
    }
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "mark_purchase_order_change_viewed") {
      const session = await getSessionUser();
      const role = normalizeRole(session.profile?.role);
      if (!session.user || (role !== "owner" && role !== "admin")) {
        return NextResponse.json({ message: "Only owner/admin can review contractor Purchase Order change requests." }, { status: 403 });
      }
      ...
      if (!currentRow?.id || !existing) return NextResponse.json({ message: "Purchase Order not found." }, { status: 404 });
      let purchaseOrder = existing;
      if (purchaseOrderChangeRequestIsUnread(existing)) {
        const now = new Date().toISOString();
        purchaseOrder = {
          ...existing,
          changeRequestViewedAt: now,
          updatedAt: now,
          audit: [...existing.audit, { at: now, action: "change_request_viewed" }].slice(-60),
        };
        const update = await admin.from("show_payroll").update({ notes: mergePurchaseOrderIntoNotes(currentNotes, purchaseOrder) }).eq("id", currentRow.id);
        if (update.error) throw update.error;
      }
      return NextResponse.json({ ok: true, purchase_order: purchaseOrder, unread_count: await unreadPurchaseOrderChangeRequestCount(admin) });
    }
    ...
    if (action === "save_purchase_order" || action === "send_purchase_orders") {
      ...
      if (action === "save_purchase_order") {
        ...
        // Keep one stable contractor review URL across revisions so every
        // previously emailed link opens the newest Purchase Order.
        ...
        const purchaseOrder: ContractorPurchaseOrder = {
          version: 1,
          token,
          previousTokens: existing?.previousTokens ?? [],
          referenceNumber,
          revision,
          status: nextStatus,
          baseAmount,
          ...
          changeRequestViewedAt: existing?.changeRequestViewedAt ?? null,
      ...
      for (const crewId of crewIds) {
        ...
        try {
          const emailId = await sendPurchaseOrderEmail({
            ...
            link: `${origin}/onboarding/${encodeURIComponent(prepared.token)}?mode=purchase_order`,
            po: prepared,
          });
