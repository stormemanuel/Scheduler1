...
function canonicalizeImportedPosition(value: string) {
  ...
  if (/\bworking\s+(?:lead|crew\s+lead)\b/.test(key)) {
    if (/\bled\b/.test(key) && /\bassist\b/.test(key)) return "LED Stagehand (Working Crew Lead)";
  ...
  if (key === "led" || key === "led tech" || key === "led technician" || key === "led assist" || key === "led assistant") return "LED Stagehand";
  ...
  if (key === "l2" || key === "lighting assist" || key === "lighting assistant" || key === "lighting technician l2" || key === "lighting stagehand") return "Lighting Assist";
...
export function normalizeImportedRoleName(value: string | null | undefined) {
  ...
  if (normalized === "led assist working crew lead" || normalized === "led assistant working crew lead") return "LED Stagehand (Working Crew Lead)";
  ...
  if (normalized === "led assist" || normalized === "led assistant") return "LED Stagehand";
...
export function runEstimateGroupHeaderSelfTest() {
  ...
  for (const position of ["LED Stagehand", "AVT / Working Lead", "Video Assist", "General AV", "Lighting Assist", "Stagehand"]) {
