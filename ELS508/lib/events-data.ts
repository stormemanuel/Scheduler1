...
import { getSessionUser, normalizeRole, sharedCoordinatorUserIds } from "@/lib/auth";
...
export type CoordinatorScheduleStatusRecord = {
  checklist_id: string;
  show_id: string;
  coordinator_user_id: string;
  schedule_sent_through_3b: boolean;
  week_before_complete: boolean;
};
...
function subCallReferenceNumberFromNotes(notes: string | null | undefined) {
  const match = String(notes || "").match(SUB_CALL_REFERENCE_MARKER_RE);
...
function boothNumberFromNotes(notes: string | null | undefined) {
  const match = String(notes || "").match(BOOTH_NUMBER_MARKER_RE);
  return match ? String(match[1] || "").trim() : "";
}

function legacyBoothFromReference(reference: string | null | undefined) {
  const match = String(reference || "").trim().match(/^Booth\s*(?:Number|No\.?|#)?\s*:?\s*(.+)$/i);
  return match?.[1]?.trim() || "";
}
...
function missingMessagingModeColumns(message: string | null | undefined) {
  const text = String(message || "");
  return text.includes("messaging_mode") || text.includes("device_type") || text.toLowerCase().includes("schema cache");
}

function missingQueueDeliveryColumns(message: string | null | undefined) {
  const text = String(message || "");
  return text.includes("delivery_mode") || text.includes("opened_in_messages_at") || text.toLowerCase().includes("schema cache");
}

function leadFromMatchesCoordinator(value: unknown, session: Awaited<ReturnType<typeof getSessionUser>>) {
  const lead = normalizeText(value);
  if (!lead || !session.user?.id) return false;
  const fullName = normalizeText(session.profile?.full_name);
  const emailName = normalizeText(String(session.profile?.email || session.user.email || "").split("@")[0] || "");
...
function eventCrewRecord(row: Record<string, unknown>, cityMap: Map<string, string>, extraPoolsByCrew: Map<string, string[]>, positionsByCrew: Map<string, CrewRecord["positions"]>, unavailableByCrew: Map<string, string[]>): CrewRecord {
  ...
  return {
    ...
    address: "",
    lead_from: String(row.lead_from || ""),
...
export async function getEventsCrewLookupData(options: { includeCrewIds?: string[] } = {}) {
  ...
  const coordinatorOwnerIds = new Set([session.user?.id || "", ...sharedCoordinatorUserIds(session.user)].filter(Boolean));
  const includeCrewIds = new Set((options.includeCrewIds ?? []).map((id) => String(id || "")).filter(Boolean));
  const supabase = await createSupabaseServerClient();
  if (!supabase) return { crewRecords: [] as CrewRecord[] };
  const supabaseClient = supabase;

  const fullCrewSelect = "id, name, description, city_pool_id, group_name, tier, email, phone, lead_from, other_city, ob, notes, conflict_companies, blacklisted, blacklist_reason, created_by, coordinator_hidden_at, coordinator_hidden_by";
  const compatibleCrewSelect = "id, name, description, city_pool_id, group_name, tier, email, phone, lead_from, other_city, ob, notes, conflict_companies, blacklisted, blacklist_reason, created_by";
  const baseCrewSelect = "id, name, description, city_pool_id, group_name, tier, email, phone, other_city, ob, notes";
  const coordinatorName = String(session.profile?.full_name || "").replace(/[%_,]/g, " ").replace(/\s+/g, " ").trim();

  function crewLookupQuery(selectColumns: string, ownershipColumnsAvailable: boolean) {
    let query = supabaseClient.from("crew").select(selectColumns).order("name", { ascending: true });
    if (role === "coordinator" && session.user?.id) {
      if (ownershipColumnsAvailable) {
        const leadFilter = coordinatorName ? `,lead_from.ilike.*${coordinatorName}*` : "";
        const assignedIdFilter = includeCrewIds.size ? `,id.in.(${[...includeCrewIds].join(",")})` : "";
        const ownerFilter = `created_by.in.(${[...coordinatorOwnerIds].join(",")})`;
        query = query.or(`${ownerFilter}${leadFilter}${assignedIdFilter}`);
      } else if (includeCrewIds.size) {
        // Compatibility fallback for older crew schemas: only return crew already
        // assigned to visible event rows. This keeps assignment contact info usable
        // without exposing the master crew pool to a coordinator.
        query = query.in("id", [...includeCrewIds]);
      } else {
        // No ownership columns and no visible assignment IDs means there is no
        // privacy-safe coordinator lookup to perform.
  ...
  const [cityPoolsRes, crewResInitial, positionsRes, unavailableRes, extraPoolsRes] = await Promise.all([
    supabase.from("city_pools").select("id, name").order("name", { ascending: true }),
    crewLookupQuery(fullCrewSelect, true),
  ...
  let crewRes = crewResInitial;
  if (crewRes.error) {
    // Events should not lose every crew name/phone just because a newer optional
    // crew column has not reached this Supabase schema yet. Retry with the same
    // compatibility shape used by the Crew page, then a minimal contact shape.
    crewRes = await crewLookupQuery(compatibleCrewSelect, true);
  }
  if (crewRes.error) {
    crewRes = await crewLookupQuery(baseCrewSelect, false);
  }
  ...
  const crewRecords = (crewRes.data ?? [])
    .filter((row) => {
      ...
      const crewId = String(typed.id || "");
      if (role === "coordinator" && typed.coordinator_hidden_at && typed.coordinator_hidden_by === session.user?.id) return false;
      if (role !== "coordinator") return true;
      if (includeCrewIds.has(crewId)) return true;
      const ownCrew = coordinatorOwnerIds.has(String(typed.created_by || "")) || leadFromMatchesCoordinator((row as { lead_from?: string | null }).lead_from, session);
      // Allowed city pools make the city available to the coordinator's workspace.
      // They do not reveal every admin/master contact in that city.
      if (ownCrew) return true;
      return !restrictToOwner;
    })
    .map((row) => eventCrewRecord(row as unknown as Record<string, unknown>, cityMap, extraPoolsByCrew, positionsByCrew, unavailableByCrew));
...
export async function getEventsPageData(options: { search?: string; selectedShowId?: string; showLimit?: number; includeLatestPendingFeedback?: boolean; pastOnly?: boolean; pastOffset?: number } = {}) {
  const session = await getSessionUser();
  const role = normalizeRole(session.profile?.role as string | null | undefined);
  ...
  if (!supabase) {
    return {
      ...
      coordinatorScheduleStatuses: [] as CoordinatorScheduleStatusRecord[],
  ...
  const showLimit = Math.min(Math.max(options.showLimit ?? 180, 1), 250);
  const pastOffset = Math.max(0, Math.trunc(Number(options.pastOffset || 0)));
  const todayKey = new Date().toISOString().slice(0, 10);
  const activeOrFutureFilter = `show_end.gte.${todayKey},show_start.gte.${todayKey}`;
  const pastFilter = `show_end.lt.${todayKey}`;
  ...
  const effectiveDefaultFilter = [options.pastOnly ? pastFilter : activeOrFutureFilter, reviewShowFilter].filter(Boolean).join(",");

  const showFullSelect = "id, name, show_reference_number, client, business_client_id, client_contact_id, coordinator_contact_id, assigned_coordinator_user_id, venue, event_location, rate_city, show_start, show_end, notes, pending_action_text, pending_action_visible_to_coordinator, created_by";
  const showFallbackSelect = "id, name, show_reference_number, client, business_client_id, client_contact_id, coordinator_contact_id, assigned_coordinator_user_id, venue, event_location, rate_city, show_start, show_end, notes, created_by";

  let showsQueryInitial = supabase
    .from("shows")
    .select(showFullSelect)
    .order("show_start", { ascending: !options.pastOnly });
  if (searchFilter) {
    showsQueryInitial = showsQueryInitial.or(effectiveSearchFilter).limit(showLimit);
  } else {
    showsQueryInitial = options.pastOnly
      ? showsQueryInitial.or(effectiveDefaultFilter).range(pastOffset, pastOffset + showLimit - 1)
      : showsQueryInitial.or(effectiveDefaultFilter).limit(showLimit);
  }
  const showsResInitial = await showsQueryInitial;
  const showClientColumnsMissing = Boolean(showsResInitial.error && (showsResInitial.error.message.includes("show_reference_number") || showsResInitial.error.message.includes("business_client_id") || showsResInitial.error.message.includes("client_contact_id") || showsResInitial.error.message.includes("coordinator_contact_id") || showsResInitial.error.message.includes("assigned_coordinator_user_id") || showsResInitial.error.message.includes("created_by") || showsResInitial.error.message.includes("event_location")));
  const showPendingActionColumnsMissing = Boolean(showsResInitial.error && (showsResInitial.error.message.includes("pending_action_text") || showsResInitial.error.message.includes("pending_action_visible_to_coordinator") || showsResInitial.error.message.includes("schema cache")));
  let fallbackShowsQuery = supabase
    .from("shows")
    .select(showClientColumnsMissing ? "id, name, client, venue, rate_city, show_start, show_end, notes" : showFallbackSelect)
    .order("show_start", { ascending: !options.pastOnly });
  if (searchFilter) {
    fallbackShowsQuery = fallbackShowsQuery.or(effectiveSearchFilter).limit(showLimit);
  } else {
    fallbackShowsQuery = options.pastOnly
      ? fallbackShowsQuery.or(effectiveDefaultFilter).range(pastOffset, pastOffset + showLimit - 1)
      : fallbackShowsQuery.or(effectiveDefaultFilter).limit(showLimit);
  }
  const showsRes = showClientColumnsMissing || showPendingActionColumnsMissing ? await fallbackShowsQuery : showsResInitial;

  if (options.selectedShowId && !showsRes.error && !(showsRes.data ?? []).some((row) => String((row as { id?: string }).id || "") === options.selectedShowId)) {
    const selectedShowRes = await supabase
      .from("shows")
      .select(showClientColumnsMissing ? "id, name, client, venue, rate_city, show_start, show_end, notes" : showPendingActionColumnsMissing ? showFallbackSelect : showFullSelect)
      .eq("id", options.selectedShowId)
      .maybeSingle();
    if (!selectedShowRes.error && selectedShowRes.data) {
      showsRes.data = [selectedShowRes.data, ...(showsRes.data ?? [])] as typeof showsRes.data;
    }
  }
  ...
  const subCallSelect = restrictToOwner
    ? "id, labor_day_id, area, location, po_number, sub_call_group_id, area_lead_contact_id, area_lead_name, area_lead_phone, assigned_coordinator_user_id, role_name, master_rate_id, start_time, end_time, crew_needed, notes, sort_order, day_type, one_hour_walkaway"
    : "id, labor_day_id, area, location, po_number, sub_call_group_id, area_lead_contact_id, area_lead_name, area_lead_phone, assigned_coordinator_user_id, role_name, master_rate_id, message_rate, start_time, end_time, crew_needed, notes, sort_order, day_type, one_hour_walkaway";
  const subCallsResInitial = visibleLaborDayIds.length
    ? await supabase
        .from("sub_calls")
        .select(subCallSelect)
        .in("labor_day_id", visibleLaborDayIds)
        .order("labor_day_id", { ascending: true })
        .order("sort_order", { ascending: true })
        .order("start_time", { ascending: true })
    : emptyResult;
  const subCallExtraColumnsMissing = Boolean(subCallsResInitial.error && (subCallsResInitial.error.message.includes("master_rate_id") || subCallsResInitial.error.message.includes("message_rate") || subCallsResInitial.error.message.includes("location") || subCallsResInitial.error.message.includes("po_number") || subCallsResInitial.error.message.includes("sub_call_group_id") || subCallsResInitial.error.message.includes("area_lead") || subCallsResInitial.error.message.includes("sort_order") || subCallsResInitial.error.message.includes("day_type") || subCallsResInitial.error.message.includes("one_hour_walkaway") || subCallsResInitial.error.message.includes("assigned_coordinator_user_id")));
  ...
  const [notesRes, checklistRes, automationResInitial, textQueueRes, ratesRes, ratingsRes] = await Promise.all([
    ...
    visibleShowIdList.length
      ? supabase
          ...
          .select("show_id, enabled, sending_method, shortcut_token, send_availability, send_schedule, reminder_7_day, reminder_3_day, reminder_day_before, reminder_day_of, reminder_daily_after_first_day, timezone, availability_template, schedule_template, reminder_template, updated_at")
          .in("show_id", visibleShowIdList)
      : emptyResult,
    visibleShowIdList.length
      ? supabase
  ...
  let automationRes = automationResInitial as { data: unknown[] | null; error: { message: string } | null };
  if (visibleShowIdList.length && automationResInitial.error?.message.includes("reminder_daily_after_first_day")) {
    automationRes = await supabase
      .from("show_text_automations")
      ...
      .in("show_id", visibleShowIdList) as { data: unknown[] | null; error: { message: string } | null };
  }

  let textQueueResFinal = textQueueRes as { data: unknown[] | null; error: { message: string } | null };
  if (visibleShowIdList.length && missingQueueDeliveryColumns(textQueueRes.error?.message)) {
    textQueueResFinal = await supabase
      .from("text_message_queue")
      ...
      .in("show_id", visibleShowIdList)
      .order("scheduled_for", { ascending: false })
      .limit(150) as { data: unknown[] | null; error: { message: string } | null };
  }
  ...
  const scheduleTextStatusRes = visibleShowIdList.length
    ? await supabase
        .from("text_message_queue")
        .select("id, show_id, crew_id, message_type, reminder_key, scheduled_for, status, sent_at, created_at")
        .in("show_id", visibleShowIdList)
        .in("status", ["scheduled", "queued", "waiting", "due_now", "sending", "sent"])
        .or("message_type.eq.schedule,reminder_key.eq.7_day,reminder_key.like.manual_7_day%")
        .order("scheduled_for", { ascending: false })
        .limit(5000)
    : emptyResult;

  const coordinatorChecklistsRes = visibleShowIdList.length
    ? await supabase
        .from("event_coordinator_checklists")
        .select("id, event_id, coordinator_user_id")
        .in("event_id", visibleShowIdList)
    : emptyResult;

  let profilesRes = await supabase.from("profiles").select("id, email, full_name, role, is_active, messaging_mode, device_type");
  if (missingMessagingModeColumns(profilesRes.error?.message)) {
    profilesRes = await supabase.from("profiles").select("id, email, full_name, role, is_active");
  }
  const appUsers = (profilesRes.data ?? []).map((row) => {
    ...
    return {
      ...
      messaging_mode: typed.messaging_mode === "android_messages" ? "android_messages" : "apple_shortcut",
      device_type: typed.device_type || null,
  ...
  const textQueueMissing = Boolean(textQueueResFinal.error && textQueueResFinal.error.message.includes('relation "text_message_queue" does not exist'));
  ...
  const scheduleTextStatusMissing = Boolean(scheduleTextStatusRes.error && (scheduleTextStatusRes.error.message.includes('relation "text_message_queue" does not exist') || scheduleTextStatusRes.error.message.includes('text_message_queue') || scheduleTextStatusRes.error.message.includes('schema cache')));
  const coordinatorChecklistsMissing = Boolean(coordinatorChecklistsRes.error && (coordinatorChecklistsRes.error.message.includes('relation "event_coordinator_checklists" does not exist') || coordinatorChecklistsRes.error.message.includes("event_coordinator_checklists") || coordinatorChecklistsRes.error.message.includes("schema cache")));
  const error = showsRes.error || laborDaysRes.error || (subCallExtraColumnsMissing ? null : subCallsRes.error) || (assignmentsMissing ? null : assignmentsRes.error) || (notesMissing ? null : notesRes.error) || (checklistsMissing ? null : checklistRes.error) || (automationMissing ? null : automationRes.error) || (textQueueMissing ? null : textQueueResFinal.error) || (scheduleTextStatusMissing ? null : scheduleTextStatusRes.error) || (ratingsMissing ? null : ratingsRes.error) || (feedbackRatingsMissing ? null : feedbackRatingsRes.error) || (feedbackResponsesMissing ? null : feedbackResponsesRes.error) || (feedbackScoresMissing ? null : feedbackScoresRes.error) || (feedbackTechRowsMissing ? null : feedbackTechRowsRes.error) || ratesRes.error;
  if (error) {
    return {
      shows: [] as ShowRecord[],
      laborDays: [] as LaborDayRecord[],
  ...
  const shows = (showsRes.data ?? []).filter((row) => visibleShowIds.has(String((row as { id: string }).id))).map((row) => {
    const typed = row as { id: string; name: string | null; show_reference_number?: string | null; client: string | null; business_client_id?: string | null; client_contact_id?: string | null; coordinator_contact_id?: string | null; assigned_coordinator_user_id?: string | null; venue: string | null; event_location?: string | null; rate_city: string | null; show_start: string; show_end: string; notes: string | null; pending_action_text?: string | null; pending_action_visible_to_coordinator?: boolean | null; created_by?: string | null };
    const coordinatorMaySeePendingAction = !restrictToOwner || Boolean(typed.pending_action_visible_to_coordinator);
    return {
      id: typed.id,
      name: typed.name ?? "",
      show_reference_number: typed.show_reference_number ?? null,
      client: typed.client ?? "",
      ...
      notes: typed.notes ?? "",
      pending_action_text: coordinatorMaySeePendingAction ? typed.pending_action_text ?? "" : "",
      pending_action_visible_to_coordinator: coordinatorMaySeePendingAction ? Boolean(typed.pending_action_visible_to_coordinator) : false,
  ...
  const subCalls = scopedSubCallRows.map((row) => {
    ...
    return {
      ...
      booth_number: boothNumberFromNotes(typed.notes) || legacyBoothFromReference(subCallReferenceNumberFromNotes(typed.notes)) || null,
      sub_call_reference_number: legacyBoothFromReference(subCallReferenceNumberFromNotes(typed.notes)) ? null : (subCallReferenceNumberFromNotes(typed.notes) || null),
      sub_call_group_id: typed.sub_call_group_id ?? null,
      ...
      message_rate: restrictToOwner ? null : typed.message_rate != null ? String(typed.message_rate) : null,
      start_time: typed.start_time,
      end_time: typed.end_time ?? "",
      crew_needed: typed.crew_needed ?? 1,
      notes: restrictToOwner
        ? String(typed.notes ?? "").split(/\r?\n/).filter((line) => !/^\s*(?:Client rate|Line amount)\s*:/i.test(line)).join("\n").trim()
        : typed.notes ?? "",
  ...
  const textAutomationSettings = automationMissing ? [] : ((automationRes.data ?? []).filter((row) => !restrictToOwner || fullAccessShowIds.has(String((row as { show_id: string }).show_id || ""))).map((row) => {
    ...
    return {
      ...
      sending_method: typed.sending_method === "provider" && typed.shortcut_token === "__resend_email__" ? "resend_email" : typed.sending_method === "provider" ? "provider" : typed.sending_method === "android_messages" ? "android_messages" : "shortcut",
      ...
      reminder_daily_after_first_day: Boolean(typed.reminder_daily_after_first_day),
  ...
  const rawTextMessageRows = (() => {
    if (textQueueMissing) return [];
    // Keep the full queue row authoritative for fields such as crew_name, phone, body,
    // and queued_by_* while allowing the lightweight schedule-status query to refresh
    // status/timing fields. Previously the partial status row replaced the full row by ID,
    // which made queue cards lose their body/sender after the event-detail refresh.
    const byId = new Map<string, Record<string, unknown>>();
    for (const row of (textQueueResFinal.data ?? [])) {
      const id = String((row as { id?: string | null }).id || "");
      if (id) byId.set(id, { ...(row as Record<string, unknown>) });
    }
    if (!scheduleTextStatusMissing) {
      for (const row of (scheduleTextStatusRes.data ?? [])) {
        const id = String((row as { id?: string | null }).id || "");
        if (!id) continue;
  ...
  const textMessageQueue = rawTextMessageRows.filter((row) => !restrictToOwner || fullAccessShowIds.has(String((row as { show_id: string }).show_id || ""))).map((row) => {
    ...
    return {
      ...
      delivery_mode: typed.delivery_mode || null,
      opened_in_messages_at: typed.opened_in_messages_at || null,
  ...
  const coordinatorChecklistRows = coordinatorChecklistsMissing ? [] : ((coordinatorChecklistsRes.data ?? []) as Array<{ id?: string | null; event_id?: string | null; coordinator_user_id?: string | null }>)
    .filter((row) => {
      const showId = String(row.event_id || "");
      if (!visibleShowIds.has(showId)) return false;
      if (!restrictToOwner) return true;
      return userAccessIds.has(String(row.coordinator_user_id || ""));
    });
  const coordinatorChecklistIds = coordinatorChecklistRows.map((row) => String(row.id || "")).filter(Boolean);
  const coordinatorItemsRes = coordinatorChecklistIds.length
    ? await supabase
        .from("event_coordinator_checklist_items")
        .select("checklist_id, item_key, is_complete")
        .in("checklist_id", coordinatorChecklistIds)
        .in("item_key", ["event_received", "staffing_completed", "onboarding_sent", "schedules_sent", "week_before_reminders"])
    : emptyResult;
  const coordinatorItemsMissing = Boolean(coordinatorItemsRes.error && (coordinatorItemsRes.error.message.includes('relation "event_coordinator_checklist_items" does not exist') || coordinatorItemsRes.error.message.includes("event_coordinator_checklist_items") || coordinatorItemsRes.error.message.includes("schema cache")));
  const coordinatorItemsByChecklist = new Map<string, Map<string, boolean>>();
  if (!coordinatorItemsMissing) {
    for (const row of coordinatorItemsRes.data ?? []) {
      const typed = row as { checklist_id?: string | null; item_key?: string | null; is_complete?: boolean | null };
      const checklistId = String(typed.checklist_id || "");
      if (!checklistId) continue;
      const items = coordinatorItemsByChecklist.get(checklistId) ?? new Map<string, boolean>();
      items.set(String(typed.item_key || ""), Boolean(typed.is_complete));
      coordinatorItemsByChecklist.set(checklistId, items);
    }
  }
  const coordinatorScheduleStatuses: CoordinatorScheduleStatusRecord[] = coordinatorItemsMissing ? [] : coordinatorChecklistRows.map((row) => {
    const checklistId = String(row.id || "");
    const items = coordinatorItemsByChecklist.get(checklistId) ?? new Map<string, boolean>();
    return {
      checklist_id: checklistId,
      show_id: String(row.event_id || ""),
      coordinator_user_id: String(row.coordinator_user_id || ""),
      schedule_sent_through_3b: ["event_received", "staffing_completed", "onboarding_sent", "schedules_sent"].every((key) => items.get(key)),
      week_before_complete: Boolean(items.get("week_before_reminders")),
    };
  }).filter((row) => row.checklist_id && row.show_id && row.coordinator_user_id);
