...
export default async function CoordinatorPage() {
  ...
  const canViewCoordinatorPay = !isCoordinator;
      ...
      <section className="card accent-card">
        <div className="row" style={{ alignItems: "center" }}>
          <div>
            ...
            <p className="muted" style={{ margin: 0 }}>{canViewCoordinatorPay ? "Upcoming assigned shows, fill progress, open crew slots, and internal coordinator fee totals." : "Upcoming assigned shows, fill progress, open crew slots, and coordinator-safe event details."}</p>
          </div>
          <div className="toolbar" style={{ justifyContent: "flex-end" }}><Link className="ghost" href="/events">Open Events</Link>{canViewCoordinatorPay ? <Link className="ghost" href="/payroll">Open Payroll</Link> : null}</div>
        </div>
      </section>

      <section className={canViewCoordinatorPay ? "grid grid-4" : "grid grid-3"}>
        ...
        {canViewCoordinatorPay ? <div className="card compact"><div className="muted small">Projected coordinator pay</div><strong style={{ fontSize: 28 }}>{money(totalProjectedPay)}</strong></div> : null}
      </section>
              ...
              <div className="row" style={{ alignItems: "flex-start" }}>
                <div>
                  ...
                  <div className="toolbar" style={{ marginTop: 10 }}>
                    ...
                    {canViewCoordinatorPay ? <span className="badge">Projected fee {money(row.fee.total)}</span> : null}
                  </div>
                </div>
              </div>
              <div className={canViewCoordinatorPay ? "grid grid-3" : "grid"} style={{ marginTop: 12 }}>
                {canViewCoordinatorPay ? <div className="card compact" style={{ boxShadow: "none" }}><div className="muted small">Full-day tech-days</div><strong>{row.fee.fullDayTechDays}</strong><div className="small muted">Fee: {money(row.fee.fullDayFee)}</div></div> : null}
                {canViewCoordinatorPay ? <div className="card compact" style={{ boxShadow: "none" }}><div className="muted small">Half-day techs</div><strong>{row.fee.halfDayTechs}</strong><div className="small muted">Fee: {money(row.fee.halfDayFee)}</div></div> : null}
