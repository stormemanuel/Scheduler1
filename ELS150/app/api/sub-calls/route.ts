...
function cleanSubCallPayload(row: Record<string, unknown>) {
  const payload = {
    ...
    day_type: ["full_day", "half_day", "custom"].includes(String(row.day_type || "")) ? String(row.day_type) : "full_day",
...
export async function POST(request: Request) {
  ...
  if (error && error.message.includes("po_number")) {
    const fallbackRows = orderedRows.map(({ po_number: _po_number, ...row }) => row);
    ...
    const fallbackRowsData = (fallback.data ?? []).map((row) => ({ ...row, po_number: null, notes: row.notes || "" }));
    return NextResponse.json({
      ok: true,
      id: fallbackRowsData[0]?.id,
      rows: fallbackRowsData,
      message: `${fallbackRowsData.length} sub-call${fallbackRowsData.length === 1 ? "" : "s"} saved. Run the ELS130 SQL to enable PO numbers on sub-calls.`,
    });
  }
