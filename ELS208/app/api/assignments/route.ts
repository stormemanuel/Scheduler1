...
export async function POST(request: Request) {
  ...
  const coordination_owner_user_id = ownerAdmin && requestedCoordinationOwnerId ? requestedCoordinationOwnerId : null;
  const coordination_owner_name = ownerAdmin && requestedCoordinationOwnerId ? requestedCoordinationOwnerName || auth.user.email || "Admin" : null;
  const coordination_fee_waived = ownerAdmin && requestedCoordinationOwnerId ? requestedFeeWaived : false;
  if (!subCallId || !crewId) {
