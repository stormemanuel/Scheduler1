...
function parseEstimateGroupHeaderLine(line: string, fallbackYear: number) {
  const clean = compactSpaces(line);
  if (!clean || /^Subtotal\b/i.test(clean) || /^Total\b/i.test(clean) || /^Note to customer\b/i.test(clean)) return null;
  if (dateStartFromLine(clean, fallbackYear) || lineHasTime(clean) || lineHasPhone(clean)) return null;

  const leadingNumber = clean.match(/^\d+\.\s*/);
  let working = compactSpaces(clean.replace(/^\d+\.\s*/, ""));
  ...
  const hasHeaderEvidence = Boolean(leadingNumber || poNumber || boothNumber || currencyValues.length);
...
function isEstimateGroupContinuationLine(line: string, fallbackYear: number) {
  const clean = compactSpaces(line);
  if (!clean || /^Subtotal\b/i.test(clean) || /^Total\b/i.test(clean) || /^Note to customer\b/i.test(clean)) return false;
  if (dateStartFromLine(clean, fallbackYear) || lineHasTime(clean) || lineHasPhone(clean) || isZeroEstimateArtifactLine(clean)) return false;
  if (/\$[\d,]+(?:\.\d{2})?/.test(clean)) return false;
  if (/^\d+\./.test(clean)) return false;
  if (/^(?:Booth|PO)\b/i.test(clean)) return false;
  return /^[A-Z0-9&'.,\-/ ]{3,}$/.test(clean);
}
