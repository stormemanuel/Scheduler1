...
export async function getEventsPageData(options: { search?: string; selectedShowId?: string; showLimit?: number; includeLatestPendingFeedback?: boolean; pastOnly?: boolean; pastOffset?: number } = {}) {
  ...
  let showsQueryInitial = supabase
    .from("shows")
    .select("id, name, show_reference_number, client, business_client_id, client_contact_id, coordinator_contact_id, assigned_coordinator_user_id, venue, event_location, rate_city, show_start, show_end, notes, created_by")
  ...
  if (options.selectedShowId && !showsRes.error && !(showsRes.data ?? []).some((row) => String((row as { id?: string }).id || "") === options.selectedShowId)) {
    const selectedShowRes = await supabase
      .from("shows")
      .select(showClientColumnsMissing ? "id, name, client, venue, rate_city, show_start, show_end, notes" : "id, name, show_reference_number, client, business_client_id, client_contact_id, coordinator_contact_id, assigned_coordinator_user_id, venue, event_location, rate_city, show_start, show_end, notes, created_by")
  ...
  const error = showsRes.error || laborDaysRes.error || (subCallExtraColumnsMissing ? null : subCallsRes.error) || (assignmentsMissing ? null : assignmentsRes.error) || (notesMissing ? null : notesRes.error) || (checklistsMissing ? null : checklistRes.error) || (automationMissing ? null : automationRes.error) || (textQueueMissing ? null : textQueueRes.error) || (scheduleTextStatusMissing ? null : scheduleTextStatusRes.error) || (ratingsMissing ? null : ratingsRes.error) || (feedbackRatingsMissing ? null : feedbackRatingsRes.error) || (feedbackResponsesMissing ? null : feedbackResponsesRes.error) || (feedbackScoresMissing ? null : feedbackScoresRes.error) || (feedbackTechRowsMissing ? null : feedbackTechRowsRes.error) || ratesRes.error;
  if (error) {
    return {
      shows: [] as ShowRecord[],
      laborDays: [] as LaborDayRecord[],
  ...
  const rawTextMessageRows = (() => {
    ...
    const rows = [...(textQueueRes.data ?? []), ...(scheduleTextStatusMissing ? [] : (scheduleTextStatusRes.data ?? []))];
