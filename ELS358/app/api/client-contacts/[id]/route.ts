...
async function canManageContact(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, contactId: string, userId: string, role: string) {
  if (role === "owner" || role === "admin") return true;
  const { data, error } = await admin.from("client_contacts").select("id, created_by").eq("id", contactId).maybeSingle();
  if (error) {
    if (isSchemaColumnError(error.message)) {
      const fallback = await admin.from("client_contacts").select("id").eq("id", contactId).maybeSingle();
      return Boolean(fallback.data);
    }
    return false;
  }
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!(await canManageContact(admin, id, auth.user.id, auth.role))) {
    return NextResponse.json({ message: "You can only edit client contacts you created." }, { status: 403 });
  }
  ...
  if (!(await canUseClient(admin, payload.client_id, auth.user.id, auth.role))) {
    return NextResponse.json({ message: "You can only move contacts to client records you created." }, { status: 403 });
  }
  ...
  let result: any = await admin
    ...
    .update(payload)
    .eq("id", id)
    .select("id, client_id, name, title, email, phone, cell_phone, notes, contact_type, created_by, is_primary, is_onsite_contact, is_billing_contact, created_at, updated_at")
    .single();

  if (result.error && isSchemaColumnError(result.error.message)) {
    result = await admin
      .from("client_contacts")
      .update(legacyContactPayload(payload))
      .eq("id", id)
      .select("id, client_id, name, title, email, phone, notes, created_at, updated_at")
      .single();
  }

  if (result.error) return NextResponse.json({ message: result.error.message }, { status: 400 });
  return NextResponse.json({ ok: true, contact: { ...result.data, contact_type: (result.data?.contact_type as string | undefined) || payload.contact_type }, message: "Client contact updated." });
}
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { id } = await params;
  if (!(await canManageContact(admin, id, auth.user.id, auth.role))) {
    return NextResponse.json({ message: "You can only delete client contacts you created." }, { status: 403 });
  }

  await admin.from("shows").update({ client_contact_id: null }).eq("client_contact_id", id);
  await admin.from("shows").update({ coordinator_contact_id: null }).eq("coordinator_contact_id", id);
