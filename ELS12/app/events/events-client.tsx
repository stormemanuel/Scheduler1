...
type EventDisplayMode = "day" | "booth" | "messages" | "checklist" | "notes";
  ...
  function exportCrewList() {
    ...
    const buildRows = (call: SubCallRecord) => {
      ...
      if (!callAssignments.length) {
        return `<tr><td colspan="6" class="empty">No crew assigned yet.</td></tr>`;
      }
      return callAssignments
        .map(({ assignment, crew }) => {
          const notes = notesForAssignment(assignment.id);
          const notesText = noteSummary(notes, true);
          return `<tr><td>${escapeHtml(crew?.name || assignment.crew_id)}</td><td>${escapeHtml(formatTimeRange(call))}</td><td>${escapeHtml(call.role_name)}</td><td>${escapeHtml(crew ? formatPhone(crew.phone) : "")}</td><td>${escapeHtml(assignment.status)}</td><td>${escapeHtml(notesText)}</td></tr>`;
        })
        .join("");
    };
            ...
            ? dayCalls
                .map(({ call }) => `
                  <div class="section">
                    ...
                    <table>
                      <thead><tr><th>Name</th><th>Times</th><th>Position</th><th>Contact Number</th><th>Status</th><th>Notes</th></tr></thead>
        ...
        {editorMode === "show" ? (
          <div className="grid" style={{ gap: 14, marginTop: 12 }}>
            ...
            <div className="card compact accent-card">
              ...
              <div className="grid grid-2">
                <label className="field"><span>Meet-up location</span><input value={showForm.meet_up_location} onChange={(e) => setShowForm((c) => ({ ...c, meet_up_location: e.target.value }))} placeholder="3rd Floor Bissonet Ballroom" /></label>
              ...
              <div className="card compact">
                ...
                <div className="toolbar" style={{ marginTop: 12 }}>
                  ...
                  {eventDisplayMode === "messages" ? (
                    <button type="button" className="ghost" onClick={exportScheduleMessages}>Export iMessage Schedules</button>
                  ) : eventDisplayMode === "checklist" ? (
                    <button type="button" className="ghost" onClick={exportChecklist}>Export Checklist</button>
                  ) : eventDisplayMode === "notes" ? (
                    <button type="button" className="ghost" onClick={exportWorkerNotes}>Export Worker Notes</button>
                  ) : (
              ...
              {eventDisplayMode === "messages" ? (
                <div className="list">
                  <div className="row">
                    ...
                    <button type="button" className="ghost" onClick={exportScheduleMessages}>Export all schedules</button>
                  </div>
                ...
                <div className="list">
                  <div className="row">
                    <div>
                      <h4 style={{ margin: 0 }}>Crew checklist</h4>
                      <div className="small muted">Unique assigned crew in the same order they appear in the event schedule.</div>
                    </div>
                    <div className="toolbar">
                  ...
                  <div className="card compact">
                    <pre style={{ margin: 0, whiteSpace: "pre-wrap", fontFamily: "inherit", fontSize: 16 }}>{checklistText || "No assigned crew checklist yet."}</pre>
                  </div>
                </div>
              ) : eventDisplayMode === "notes" ? (
                <div className="list">
