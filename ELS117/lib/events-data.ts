...
export async function getEventsPageData(options: { search?: string; showLimit?: number } = {}) {
  ...
  if (allLoadedShowRows.length) {
    const accessRes = await supabase
      ...
      .in("show_id", allLoadedShowRows.map((show) => show.id));
    if (!accessRes.error) eventAccessRows = (accessRes.data ?? []) as EventUserAccessRecord[];
  }
  const sharedShowIdsForUser = new Set(eventAccessRows.filter((row) => row.user_id === session.user?.id).map((row) => row.show_id));
  ...
  const subCallsResInitial = visibleLaborDayIds.length
    ...
    : emptyResult;
  const subCallExtraColumnsMissing = Boolean(subCallsResInitial.error && (subCallsResInitial.error.message.includes("master_rate_id") || subCallsResInitial.error.message.includes("message_rate") || subCallsResInitial.error.message.includes("location") || subCallsResInitial.error.message.includes("sort_order") || subCallsResInitial.error.message.includes("day_type") || subCallsResInitial.error.message.includes("one_hour_walkaway")));
