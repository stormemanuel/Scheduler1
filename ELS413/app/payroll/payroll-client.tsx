...
export default function PayrollClient({ initialRows, initialEventSummaries, availableYears, initialYear, initialError, initialDiagnostics = null }: Props) {
  ...
  const [loadedYears, setLoadedYears] = useState<Set<number>>(() => new Set(initialRows.length || initialEventSummaries.length ? [initialYear] : []));
  ...
  const hasLoadedPayrollDetails = loadedEventCount > 0;
  const totalScopeLabel = hasLoadedPayrollDetails ? `${loadedEventCount} loaded show${loadedEventCount === 1 ? "" : "s"}` : "Open a show to load totals";
    ...
    <>
      ...
      {diagnostics ? (
        <details className="card compact">
          <summary><strong>Payroll performance diagnostics</strong> <span className="muted small">({diagnostics.mode}, {diagnostics.totalMs} ms)</span></summary>
      ...
      <section className="grid grid-3">
        <div className="card compact">
          ...
          <div style={{ fontSize: 30, fontWeight: 850 }}>{hasLoadedPayrollDetails ? money(payableTotal) : "Not loaded"}</div>
          <div className="muted small">{totalScopeLabel}</div>
        </div>
        <div className="card compact">
          <div className="muted small">Marked paid</div>
          <div style={{ fontSize: 30, fontWeight: 850, color: "#067647" }}>{hasLoadedPayrollDetails ? money(paidTotal) : "Not loaded"}</div>
          <div className="muted small">{totalScopeLabel}</div>
        </div>
        <div className="card compact">
          <div className="muted small">Still unpaid</div>
          <div style={{ fontSize: 30, fontWeight: 850, color: "#b42318" }}>{hasLoadedPayrollDetails ? money(unpaidTotal) : "Not loaded"}</div>
          <div className="muted small">{totalScopeLabel}</div>
        </div>
      </section>
      ...
      <section className="card accent-card">
        <div className="row" style={{ alignItems: "center" }}>
          <div>
            ...
            <p className="muted" style={{ margin: 0 }}>{viewLabel(reportView)}. Totals are calculated from payroll details you have opened in this session. Current view excludes future shows from payable, unpaid, and P&L totals. Estimated view includes all scheduled shows for planning. Contract labor includes tech payouts plus non-waived coordinator pay.</p>
        ...
        <div className="pl-grid" style={{ marginTop: 14 }}>
          <div className="card compact"><div className="muted small">Estimated revenue</div><strong>{hasLoadedPayrollDetails ? money(plTotals.estimatedRevenue) : "Not loaded"}</strong></div>
          <div className="card compact"><div className="muted small">Contract labor</div><strong>{hasLoadedPayrollDetails ? money(plTotals.contractLabor) : "Not loaded"}</strong><div className="muted small">Tech payouts + non-waived coordinator pay</div></div>
          <div className="card compact"><div className="muted small">Expenses</div><strong>{hasLoadedPayrollDetails ? money(plTotals.expenses) : "Not loaded"}</strong></div>
          <div className="card compact"><div className="muted small">Net profit</div><strong>{hasLoadedPayrollDetails ? money(plTotals.estimatedProfit) : "Not loaded"}</strong></div>
        </div>
      </section>
            ...
            <div className="row" style={{ alignItems: "flex-start" }}>
              <div>
                ...
                <div className="toolbar" style={{ marginTop: 10 }}>
                  ...
                  <span className="badge">Tech payable {eventDetailsLoaded ? money(event.payableTotal) : "open show"}</span>
                  <span className="badge">Paid {eventDetailsLoaded ? money(event.paidTotal) : "open show"}</span>
                  <span className="badge">Unpaid {eventDetailsLoaded ? money(event.unpaidTotal) : "open show"}</span>
                </div>
                <div className="pl-grid" style={{ marginTop: 12 }}>
                  <span className="badge">Revenue {eventDetailsLoaded ? money(event.estimatedRevenue) : "open show"}</span>
                  <span className="badge">Labor {eventDetailsLoaded ? money(event.payableTotal + coordinatorCostForEvent(event)) : "open show"}</span>
                  <span className="badge">Expenses {eventDetailsLoaded ? money(event.expenses) : "open show"}</span>
                  <span className="badge">Net profit {eventDetailsLoaded ? money(event.estimatedProfit) : "open show"}</span>
