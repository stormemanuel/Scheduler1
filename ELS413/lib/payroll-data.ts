...
import type { PayrollEventSummary, PayrollPageData, PayrollStatusRecord, PayrollCrewShowRow, PayrollPaymentStatus, PayrollTaxProfileSummary, PayrollPerformanceDiagnostics, PayrollPerformanceTiming } from "@/lib/payroll-types";
...
export async function getPayrollPageData(requestedYear?: number, options: { summaryOnly?: boolean; showId?: string } = {}): Promise<PayrollPageData> {
  ...
  try {
    ...
    if (options.summaryOnly) {
      ...
      let summaryFinancialsRes: any = showIds.length
        ? await financialReadClient
          .from("show_financials")
          .select("show_id, estimated_revenue_override, expenses, notes, tax_reserve_done, tax_reserve_done_at, consecrated_hands_done, consecrated_hands_done_at")
          .in("show_id", showIds)
          .limit(500)
        : emptyResult;
      const financialsColumnMissing = Boolean(summaryFinancialsRes.error && (
        safeText(summaryFinancialsRes.error.message).includes("tax_reserve_done") ||
        safeText(summaryFinancialsRes.error.message).includes("consecrated_hands_done") ||
        safeText(summaryFinancialsRes.error.message).includes("schema cache")
      ));
      if (financialsColumnMissing && showIds.length) {
        summaryFinancialsRes = await financialReadClient
          .from("show_financials")
          .select("show_id, estimated_revenue_override, expenses, notes")
          .in("show_id", showIds)
          .limit(500);
      }
      timer.mark("summary financial flags");
      const summaryFinancialsMissing = Boolean(summaryFinancialsRes.error && summaryFinancialsRes.error.message.includes('relation "show_financials" does not exist'));
      if (summaryFinancialsRes.error && !summaryFinancialsMissing && !financialsColumnMissing) {
        return {
          eventSummaries: [],
          crewRows: [],
          availableYears,
          loadedYear,
          setupMissing: false,
          error: summaryFinancialsRes.error.message,
          diagnostics: timer.done(showIds.length, 0),
        };
      }
      const financialByShow = new Map<string, ShowFinancialRecord>();
      for (const row of summaryFinancialsMissing ? [] : (summaryFinancialsRes.data ?? [])) {
        const typed = row as ShowFinancialRecord;
        const showId = safeText(typed.show_id);
        if (showId) financialByShow.set(showId, typed);
      }
      return {
        eventSummaries: shows.map((show) => payrollSummaryForShow(show, financialByShow.get(show.id))),
        crewRows: [],
        availableYears,
        loadedYear,
        setupMissing: false,
    ...
    return {
      ...
      diagnostics: timer.done(shows.length, crewRows.length),
    };
  } catch (error) {
    return {
      eventSummaries: [],
