...
export default function PayrollClient({ initialRows, availableYears, initialError }: Props) {
  ...
  function exportYearSummary() {
    downloadCsv(`ELS_${year}_1099_paid_totals.csv`, [
      ["Year P&L", "Estimated Revenue", "Contract Labor", "Expenses", "Profit", "Consecrated Hands 10%", "Taxes 25%", "CH + Taxes", "Pure Profit"],
      [year, plTotals.estimatedRevenue, plTotals.contractLabor, plTotals.expenses, plTotals.estimatedProfit, plTotals.consecratedHandsDonation, plTotals.taxReserve, plTotals.combinedReserve, plTotals.pureProfit],
      [],
      ["Year", "Crew", "Email", "Phone", "Paid Total", "Unpaid/Open", "Paid Events", "Unpaid Events"],
      ...yearSummary.map((row) => [year, row.crewName, row.crewEmail, row.crewPhone, row.paidTotal, row.unpaidTotal, row.eventCountPaid, row.eventCountUnpaid]),
    ]);
  }

  function exportEvent(event: PayrollEventSummary) {
    downloadCsv(`${event.showName.replace(/[^a-z0-9]+/gi, "_")}_payroll.csv`, [
      ["Show P&L", "Estimated Revenue", "Contract Labor", "Expenses", "Profit", "Consecrated Hands 10%", "Taxes 25%", "CH + Taxes", "Pure Profit"],
      [event.showName, event.estimatedRevenue, event.payableTotal, event.expenses, event.estimatedProfit, event.consecratedHandsDonation, event.taxReserve, event.combinedReserve, event.pureProfit],
      [],
    ...
    <div className="grid" style={{ gap: 16 }}>
      ...
      <section className="card accent-card">
        <div className="row" style={{ alignItems: "center" }}>
          <div>
            ...
            <p className="muted" style={{ margin: 0 }}>Estimated revenue minus contract labor and expenses, then reserves for Consecrated Hands and taxes.</p>
          </div>
          <button className="ghost" type="button" onClick={exportYearSummary}>Export P&L / 1099 CSV</button>
        </div>
        <div className="pl-grid" style={{ marginTop: 14 }}>
          ...
          <div className="card compact"><div className="muted small">Profit</div><strong>{money(plTotals.estimatedProfit)}</strong></div>
          <div className="card compact"><div className="muted small">Consecrated Hands 10%</div><strong>{money(plTotals.consecratedHandsDonation)}</strong></div>
          <div className="card compact"><div className="muted small">Taxes 25%</div><strong>{money(plTotals.taxReserve)}</strong></div>
          <div className="card compact"><div className="muted small">Taxes + CH</div><strong>{money(plTotals.combinedReserve)}</strong></div>
          <div className="card compact"><div className="muted small">Pure profit</div><strong>{money(plTotals.pureProfit)}</strong></div>
        </div>
      </section>
      ...
      <section className="card">
        <div className="row" style={{ alignItems: "center" }}>
          ...
          <button className="ghost" type="button" onClick={exportYearSummary}>Export {year} 1099 CSV</button>
        </div>
      ...
      <section className="card">
        <div className="row" style={{ alignItems: "center" }}>
          <div>
            ...
            <p className="muted" style={{ margin: 0 }}>This is the year-to-date paid total per tech. The paid column is the number to use when preparing 1099 totals.</p>
          </div>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            ...
            <tbody>
              ...
              {yearSummary.length === 0 ? (
                <tr><td colSpan={5} className="muted" style={{ padding: 16 }}>No assigned crew found for {year}.</td></tr>
              ) : null}
            </tbody>
          </table>
        </div>
            ...
            <div className="row" style={{ alignItems: "flex-start" }}>
              <div>
                ...
                <div className="pl-grid" style={{ marginTop: 12 }}>
                  ...
                  <span className="badge">Profit {money(event.estimatedProfit)}</span>
                  <span className="badge">CH 10% {money(event.consecratedHandsDonation)}</span>
                  <span className="badge">Taxes 25% {money(event.taxReserve)}</span>
                  <span className="badge">Pure profit {money(event.pureProfit)}</span>
                </div>
              </div>
