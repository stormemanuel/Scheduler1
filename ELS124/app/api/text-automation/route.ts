...
function buildCustomQueueRows(
  settings: AutomationSettings,
  bundle: Awaited<ReturnType<typeof fetchShowBundle>>,
  options: { laborDayId: string; subCallId?: string; startTime?: string; endTime?: string; scheduledLocal?: string; body: string }
) {
  const targetDayIds = new Set([options.laborDayId].filter(Boolean));
  const targetCalls = bundle.calls.filter((call) => targetDayIds.has(call.labor_day_id) && (!options.subCallId || call.id === options.subCallId));
