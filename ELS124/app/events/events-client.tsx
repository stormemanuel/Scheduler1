...
function localInputTime(minuteOffset = 15) {
  const date = new Date(Date.now() + minuteOffset * 60000);
...
function EventsClientInner({
  ...
  const [customTextSubCallId, setCustomTextSubCallId] = useState("");
  ...
  async function queueCustomTextMessage() {
    ...
    const data = await request("/api/text-automation", "POST", {
      ...
      sub_call_id: customTextSubCallId || null,
  ...
  async function generateConnectedFeedbackLinks() {
    ...
    setMsg({ kind: "success", text: data?.message || `${links.length} survey link${links.length === 1 ? "" : "s"} ready.` });
    return links;
  }
  ...
  async function createAndCopyFeedbackLink(form: FeedbackFormItem) {
    ...
    if (!link) {
      setMsg({ kind: "error", text: "Could not create this survey link. Tap Refresh Connected Links and try again." });
      return;
    }
        ...
        <div className="card compact">
          ...
          <div className="card compact" style={{ marginTop: 14, background: "#fbfcfd" }}>
            ...
            <div className="card compact" style={{ boxShadow: "none", marginTop: 12 }}>
              ...
              <div className="small muted">Use these when the reminder window has already passed, or when you want to send a reminder yourself. These texts are queued due now and will send on the next iPhone Shortcut run. Day-of morning reminder says: Good Morning, we are meeting at the meet-up location and event location. See you soon. Please confirm. Each Shortcut run can pull up to 25 due texts.</div>
            ...
            <div className="card compact" style={{ boxShadow: "none", marginTop: 12 }}>
              ...
              <div className="grid grid-3" style={{ marginTop: 12 }}>
                <label className="field">
                  <span>Labor day</span>
                  <select value={customTextDayId || selectedCustomTextDay?.id || ""} onChange={(event) => { setCustomTextDayId(event.target.value); setCustomTextSubCallId(""); }}>
                    {selectedShowLaborDays.map((day) => (
                      <option key={day.id} value={day.id}>{formatMessageDate(day.labor_date)}{day.label ? ` · ${day.label}` : ""}</option>
                    ))}
                  </select>
                ...
                <label className="field">
                  <span>Sub-call</span>
                  <select value={customTextSubCallId} onChange={(event) => setCustomTextSubCallId(event.target.value)}>
                    <option value="">All sub-calls on selected day</option>
                    {customTextSubCallOptions.map((call) => (
                      <option key={call.id} value={call.id}>{call.start_time || "--:--"}–{call.end_time || "--:--"} · {call.role_name || "Crew"} · {call.area || "Area TBD"}</option>
                    ))}
                  </select>
                </label>
                <label className="field">
                  ...
                  <div className="row" style={{ gap: 8 }}>
                    <input type="date" value={customTextScheduledDate} onChange={(event) => setCustomTextScheduledDate(event.target.value)} />
                    <input type="time" value={customTextScheduledTime} onChange={(event) => setCustomTextScheduledTime(event.target.value)} />
                  </div>
                </label>
              </div>
              <div className="grid grid-3" style={{ marginTop: 10 }}>
                <label className="field">
                  <span>Only start time</span>
                  <input type="time" value={customTextStartTime} onChange={(event) => setCustomTextStartTime(event.target.value)} />
                </label>
                <label className="field">
                  <span>Only end time</span>
                  <input type="time" value={customTextEndTime} onChange={(event) => setCustomTextEndTime(event.target.value)} />
                </label>
                <div className="field">
                  <span>Action</span>
      ...
      <div className="list">
        <div className="card compact feedback-intro-card">
          ...
          {connectedFeedbackLinks.length ? (
            ...
            <p className="small muted" style={{ marginBottom: 0 }}>Connected links open a client-only survey page. Use Create + copy link on any survey card, or Refresh Connected Survey Links to generate every link at once.</p>
          )}
        </div>
              ...
              <div key={form.key} className="card compact feedback-survey-card" style={{ boxShadow: "none" }}>
                <div className="row" style={{ alignItems: "flex-start" }}>
                  <div>
                    ...
                    {liveLink ? <div className="small muted" style={{ wordBreak: "break-all", marginTop: 6 }}>{connectedFeedbackUrl(liveLink)}</div> : null}
                  </div>
                  <div className="toolbar">
                    {liveLink ? <button type="button" className="primary" onClick={() => void copyConnectedFeedbackLink(liveLink)}>Copy link</button> : <button type="button" className="primary" onClick={() => void createAndCopyFeedbackLink(form)}>Create + copy link</button>}
