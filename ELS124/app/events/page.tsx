...
export default async function EventsPage({ searchParams }: { searchParams?: Promise<{ q?: string; feedback?: string }> }) {
  ...
  const { shows, laborDays, subCalls, assignments, assignmentNotes, assignmentChecklists, textAutomationSettings, textMessageQueue, techRatings, clientFeedbackResponses, clientFeedbackScores, feedbackTechRatings, eventUserAccess, appUsers, masterRates, setupMissing, error } = await getEventsPageData({ search: initialSearch });
  const { crewRecords } = await getCrewPageData();
  const { businessClients, clientContacts } = await getClientDirectoryData();
      ...
      <section className="card">
        <div className="row">
          ...
          <div className="toolbar" style={{ justifyContent: "flex-end" }}>
            <Link href="/events?feedback=1" className={pendingFeedbackCount ? "primary" : "ghost"}>
