...
function ContactDetail({ record, ratingSummary, cityPools, targetGroups, bulkCityId, bulkGroupName, detailTab, onTab, onEdit, onDelete, onMove, onQueueOnboarding, onQueueW9Only, onQueueContractOnly, onAdminDocumentUploaded, onBulkCityChange, onBulkGroupChange, saving, canSeeAdminAudit }: ContactDetailProps) {
  ...
  async function uploadAdminFiles(kind: AdminUploadKind, list: FileList | null) {
            ...
            <div className="card compact" style={{ background: "#fbfcfd" }}>
              ...
              <div className="grid" style={{ gap: 10, marginTop: 14 }}>
                <label className="field">
                  ...
                  <input type="file" accept="image/*" disabled={uploadDisabled} onChange={(event) => { void uploadAdminFiles("profile_photo", event.target.files); event.target.value = ""; }} />
                  <span className={uploadStatus.profile_photo?.kind === "error" ? "error" : uploadStatus.profile_photo?.kind === "success" ? "success" : "muted small"}>{uploadStatus.profile_photo?.text || "Compressed to private WebP upload."}</span>
                </label>
                <label className="field">
