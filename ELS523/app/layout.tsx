...
import OnboardingTutorial, { AppBadgeSync, CoordinatorInboxButton, CoordinatorNotificationNavLink, CoordinatorNotificationPageMarker, OnboardingReviewNavLink, PayrollChangeRequestNavLink } from "@/app/components/onboarding-tutorial";
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  const isPasswordRecoveryRoute = pathname === "/update-password";

  // A fresh Supabase recovery link does not have an authenticated server cookie
  // yet. Render this route without waiting on the authenticated app shell so
  // its client can exchange the recovery code on the very first visit.
  if (isPublicFeedbackRoute || isPublicOnboardingRoute || isPasswordRecoveryRoute) {
    return (
      <html lang="en">
        <body>
          <div
            className={isPasswordRecoveryRoute ? "shell" : "feedback-route-shell"}
            aria-label={isPasswordRecoveryRoute ? "Password recovery" : isPublicOnboardingRoute ? "Public onboarding form" : "Public feedback survey"}
          >
            {children}
          </div>
          <SpeedInsights />
  ...
  const mustChangePassword = Boolean(session.user?.user_metadata?.force_password_change);
  if (session.user && mustChangePassword && !isViewingAs && pathname !== "/account" && pathname !== "/update-password") {
        ...
        <div className="shell">
          <header className="app-header" aria-label="Application header">
            <div className="brand-search-card card">
              ...
              <GlobalSearch enabled={Boolean(session.user)} readOnly={isViewingAs} />
            </div>
            <nav className="page-nav-card card nav" aria-label="Primary navigation">
              {session.user ? (
                ...
                ) : page === "payroll" && (role === "owner" || role === "admin") ? (
                  <PayrollChangeRequestNavLink key={page} href={pageHrefByKey[page]} label={pageLabelByKey[page]} />
          ...
          {session.user ? (
            <div className="sessionbar card compact">
              ...
              <div className="toolbar">
                {!isViewingAs && session.user && (role === "coordinator" || role === "owner" || role === "admin") ? <CoordinatorInboxButton role={role} /> : null}
