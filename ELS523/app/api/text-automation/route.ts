...
import { createSupabaseAdminClient, createSupabaseServerClient, repairActiveScheduleQueueBoothLines, syncAssignmentChecklistFromSentMessage } from "@/lib/supabase-server";
import { extractPurchaseOrderFromNotes, mergePurchaseOrderIntoNotes, purchaseOrderDisplayNumber, type ContractorPurchaseOrder } from "@/lib/payroll-calculations";
...
function boothNumberFromNotes(notes: string | null | undefined) {
  return safeText(notes).match(BOOTH_NUMBER_MARKER_RE)?.[1]?.trim() || "";
}

function callBoothMeetupLocation(call: { area?: string | null; location?: string | null; notes?: string | null }) {
  const booth = scheduleBoothNumber(call as SubCallRow);
...
type AutomationSettings = {
  show_id: string;
  enabled: boolean;
  sending_method: "manual" | "shortcut" | "android_messages" | "provider" | "resend_email";
...
type QueueRow = {
  ...
  delivery_mode?: "apple_shortcut" | "android_messages" | "resend_email" | null;
...
function cleanQueueIds(value: unknown, limit = 250): string[] {
  const raw: unknown[] = Array.isArray(value) ? value : [];
  const normalized: string[] = raw
    .map((item: unknown): string => safeText(item))
    .filter((item: string): boolean => Boolean(item));
  return Array.from(new Set<string>(normalized)).slice(0, limit);
}
...
async function requireSignedIn() {
  ...
  const displayName = safeText((profile as { full_name?: string | null } | null)?.full_name)
    ...
    || safeText(user.email)
    || "ELS user";
  const messagingMode = normalizeMessagingMode(
    (profile as SenderProfileRow | null)?.messaging_mode ||
    (user.user_metadata as { els_messaging_mode?: string | null } | null)?.els_messaging_mode
  );
  return { ok: true as const, user: { ...user, email: safeText((profile as { email?: string | null } | null)?.email) || user.email, display_name: displayName, messaging_mode: messagingMode } };
}
...
type QueueDeliveryMode = "apple_shortcut" | "android_messages" | "resend_email";

function queueIdentity(
  user: { id: string; email?: string | null; display_name?: string | null; messaging_mode?: string | null },
  deliveryModeOverride?: QueueDeliveryMode,
) {
  return {
    queued_by_user_id: user.id,
    queued_by_email: user.email || null,
    queued_by_name: user.display_name || user.email || null,
    delivery_mode: deliveryModeOverride || deliveryModeForUser(user),
  };
}

function stampRows<T extends object>(
  rows: T[],
  user: { id: string; email?: string | null; display_name?: string | null; messaging_mode?: string | null },
  deliveryModeOverride?: QueueDeliveryMode,
) {
  const identity = queueIdentity(user, deliveryModeOverride);
...
function deliveryModeForSettings(settings: AutomationSettings, user: { messaging_mode?: string | null }): QueueDeliveryMode {
  return settings.sending_method === "resend_email" ? "resend_email" : deliveryModeForUser(user);
}
...
function messageRateForCall(call: SubCallRow, _crew: CrewRow, show: ShowRow, masterRates: MasterRateRow[], _meta: Record<string, string>, fullDayOverride?: number | null) {
  ...
  if (Number(fullDayOverride || 0) > 0) return formatHourlyRate(Number(fullDayOverride) / 10);
...
function normalizeReminderTemplate(value: unknown) {
  ...
  const priorDefault = "Hi {first_name}, quick confirmation for {show_name} at {venue}. Your next call is {next_call}. Meet-up Location: {meet_up_location}. Please reply confirmed.\n\nCoordinator: {coordinator_name}\nEmanuel Labor Services";
  if (!text || text === oldDefault || text === priorDefault) return defaultReminderTemplate;
  if (/\{booth_line\}/i.test(text)) return text;
  if (/\{next_call\}/i.test(text)) return text.replace(/(\{next_call\})/i, "$1\n{booth_line}");
  return `${text}\n{booth_line}`;
}
...
const defaultReminderTemplate = "Hi {first_name}, quick confirmation for {show_name} at {venue}. Your next call is {next_call}.\n{booth_line}\nMeet-up Location: {meet_up_location}. Please reply confirmed.\n\nCoordinator: {coordinator_name}\nEmanuel Labor Services";
const defaultDayOfReminderTemplate = "Good Morning, we are meeting at {meet_up_location} for {show_name} at {venue}.\n{booth_line}\nSee you soon. Please confirm.\n\nCoordinator: {coordinator_name}\nEmanuel Labor Services";
const defaultDailyAfterFirstDayTemplate = "Good Morning {first_name}, quick confirmation for {show_name} at {venue}. Your next call is {next_call}.\n{booth_line}\n{meet_up_sentence} Please reply confirmed.\n\nCoordinator: {coordinator_name}\nEmanuel Labor Services";
...
function normalizeSettings(bodySettings: Partial<AutomationSettings>, showId: string): AutomationSettings {
  ...
  return {
    ...
    sending_method: rawMethod === "resend_email" ? "resend_email" : rawMethod === "provider" ? "provider" : rawMethod === "android_messages" ? "android_messages" : rawMethod === "manual" ? "manual" : "shortcut",
...
function scheduleBoothNumber(call: SubCallRow) {
  const storedBooth = boothNumberFromNotes(call.notes);
  if (storedBooth) return storedBooth;
  // Compatibility for imported rows created before Booth became its own field.
  const reference = safeText(call.notes).match(SUB_CALL_REFERENCE_MARKER_RE)?.[1]?.trim() || "";
  const referenceMatch = reference.match(/^booth\s*(?:(?:number|no\.?|#)\s*)?:?\s*(.+)$/i);
  if (referenceMatch?.[1]) return safeText(referenceMatch[1]);
  const areaMatch = safeText(call.area).match(/\bbooth\s*(?:(?:number|no\.?|#)\s*)?:?\s*(TBD|[A-Z0-9-]+)/i);
  return safeText(areaMatch?.[1]);
}

function nextScheduleByCrew(show: ShowRow, days: LaborDayRow[], calls: SubCallRow[], assignments: AssignmentRow[], crewRows: CrewRow[], masterRates: MasterRateRow[], settings: AutomationSettings, senderName = "", projectManagerContact: ClientContactRow | null = null, rateOverrides = new Map<string, number>()) {
  ...
  const result = new Map<string, { crew: CrewRow; lines: Array<{ sortKey: string; text: string; area: string; booth: string; dateLabel: string; availabilityText: string }>; meetUps: string[]; rates: string[]; poNumbers: string[]; areas: string[]; booths: string[]; firstCall: { day: LaborDayRow; call: SubCallRow; assignment: AssignmentRow } | null; values: Record<string, string> }>();
  ...
  for (const assignment of [...assignments].sort((a, b) => Number(a.sort_order || 0) - Number(b.sort_order || 0))) {
    ...
    const current = result.get(crew.id) || { crew, lines: [], meetUps: [], rates: [], poNumbers: [], areas: [], booths: [], firstCall: null, values: {} as Record<string, string> };
    const blockLabel = dayTypeLabel(assignmentDayType(assignment, call));
    const rateText = messageRateForCall(call, crew, show, masterRates, meta, rateOverrides.get(assignment.id));
    const area = safeText(call.area) || "Area TBD";
    const booth = scheduleBoothNumber(call);
    current.lines.push({
    ...
    if (booth) current.booths.push(booth);
  ...
  for (const item of result.values()) {
    ...
    const uniqueBooths = [...new Set(item.booths)];
    const boothLine = uniqueBooths.length > 1
      ? `Booth:\n${[...new Set(sortedLines.filter((line) => line.booth).map((line) => `${line.dateLabel}: ${line.booth}`))].join("\n")}`
...
async function fetchShowBundle(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, showId: string) {
  ...
  const rateOverrideRes = assignmentIds.length
    ? await admin.from("assignment_notes").select("assignment_id, custom_note, created_at").in("assignment_id", assignmentIds).eq("note_code", "assignment_rate_override").order("created_at", { ascending: false })
    : { data: [], error: null };
  if (rateOverrideRes.error && !/assignment_notes|schema cache|relation/i.test(rateOverrideRes.error.message || "")) throw new Error(rateOverrideRes.error.message);
  const rateOverrides = new Map<string, number>();
  for (const row of (rateOverrideRes.data || []) as Array<{ assignment_id: string | null; custom_note: string | null }>) {
    const assignmentId = safeText(row.assignment_id);
    const rate = Number(String(row.custom_note || "").replace(/[^0-9.]/g, ""));
    if (assignmentId && !rateOverrides.has(assignmentId) && Number.isFinite(rate) && rate > 0) rateOverrides.set(assignmentId, rate);
  }
  ...
  return { show, days, calls, assignments, crewRows, masterRates: (masterRatesRes.data || []) as MasterRateRow[], projectManagerContact, rateOverrides };
}
...
function buildQueueRows(
  ...
  const byCrew = nextScheduleByCrew(bundle.show, filteredDays, filteredCalls, filteredAssignments, bundle.crewRows, bundle.masterRates, settings, senderName, bundle.projectManagerContact, bundle.rateOverrides);
  const selectedCrewSet: Set<string> = new Set<string>(cleanQueueIds(options.selectedCrewIds || []));
  const rows: QueueRow[] = [];
...
type ExistingQueueState = { id: string | null; status: string; reminder_key: string; queued_by_user_id?: string | null; delivery_mode?: string | null };

function existingQueueNeedsSender(state: ExistingQueueState | undefined) {
  return Boolean(state && activeQueueStatus(state.status) && !safeText(state.queued_by_user_id));
}
...
async function readExistingQueueRowsForPlanning(
  admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>,
  showId: string,
  requireDeliveryMode: boolean,
  schedulesOnly = false,
) {
  let query = admin
    .from("text_message_queue")
    .select("id, crew_id, message_type, reminder_key, status, queued_by_user_id, delivery_mode")
    .eq("show_id", showId);
  if (schedulesOnly) query = query.eq("message_type", "schedule");
  const withDelivery = await query.limit(5000) as { data: ExistingQueuePlanningRow[] | null; error: { message: string } | null };
  if (!withDelivery.error) return withDelivery.data || [];
  if (!isMissingDeliveryModeColumns(withDelivery.error.message)) throw new Error(withDelivery.error.message);
  if (requireDeliveryMode) {
    throw new Error("Email via Resend requires the Supabase text_message_queue.delivery_mode column. Run the ELS email queue SQL, then refresh this event.");
  }
  let legacyQuery = admin
    .from("text_message_queue")
    .select("id, crew_id, message_type, reminder_key, status, queued_by_user_id")
    .eq("show_id", showId);
  if (schedulesOnly) legacyQuery = legacyQuery.eq("message_type", "schedule");
  const legacy = await legacyQuery.limit(5000) as { data: ExistingQueuePlanningRow[] | null; error: { message: string } | null };
  if (legacy.error) throw new Error(legacy.error.message);
  return (legacy.data || []).map((row) => ({ ...row, delivery_mode: null }));
}
...
async function buildFullShowReminderPlan(
  ...
  const baseByCrew = nextScheduleByCrew(bundle.show, bundle.days, bundle.calls, bundle.assignments, bundle.crewRows, bundle.masterRates, settings, senderName, bundle.projectManagerContact, bundle.rateOverrides);
  ...
  const existingRows = await readExistingQueueRowsForPlanning(admin, settings.show_id, settings.sending_method === "resend_email");
  const existing = new Map<string, ExistingQueueState>();
  for (const row of existingRows) {
    const rowDelivery = safeText(row.delivery_mode);
    if (settings.sending_method === "resend_email" ? rowDelivery !== "resend_email" : rowDelivery === "resend_email") continue;
    const logicalKey = `${row.crew_id || ""}|${row.message_type || ""}|${logicalReminderKey(row.reminder_key || "")}`;
    existing.set(logicalKey, chooseExistingQueueState(existing.get(logicalKey), { id: row.id || null, status: row.status || "scheduled", reminder_key: row.reminder_key || "", queued_by_user_id: row.queued_by_user_id || null, delivery_mode: row.delivery_mode || null }));
  }
  ...
  for (const [crewId, item] of baseByCrew.entries()) {
    ...
    const phone = cleanPhone(item.crew.phone || "");
    const email = normalizeEmail(item.crew.email);
    const emailMode = settings.sending_method === "resend_email";
    const hasRecipient = emailMode ? validEmail(email) : Boolean(phone);
    ...
    const pushPlan = (args: { type: string; enabled: boolean; dueIso: string; messageType: string; reminderKey: string; body: string; row: typeof firstRow }) => {
      ...
      const needsSenderRepair = existingQueueNeedsSender(existingState);
      const dueMs = Date.parse(args.dueIso);
      const result: ReminderPlanRow["result"] = !hasRecipient
        ? "no_phone"
        : needsSenderRepair
          ? "will_schedule"
      ...
      const resultLabel = result === "will_schedule"
        ? needsSenderRepair ? "Already scheduled — attach current sender" : "Will schedule"
        : result === "overdue"
          ? "Overdue"
          : result === "no_phone"
            ? emailMode ? "Skipped: no email" : "Skipped: no phone"
            : result === "already_sent"
              ? "Already sent"
...
async function refreshActiveQueuedRemindersForCrew(
  admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>,
  settings: AutomationSettings,
  bundle: Awaited<ReturnType<typeof fetchShowBundle>>,
  crewIds: string[],
  ...
  const selectedCrewIds = cleanQueueIds(crewIds);
  if (!selectedCrewIds.length) return 0;

  const { data: queuedData, error: queuedError } = await admin
    .from("text_message_queue")
    .select("id, crew_id, reminder_key, body, scheduled_for, status, queued_by_name")
  ...
  const queuedRows = (queuedData || []) as Array<{
    id: string;
    crew_id: string | null;
    reminder_key: string | null;
    body: string | null;
  ...
  const senderNames = Array.from(new Set(queuedRows.map((row) => safeText(row.queued_by_name) || fallbackSenderName).filter(Boolean)));
  const planBySender = new Map<string, Map<string, ReminderPlanRow>>();
  for (const senderName of senderNames) {
    const plan = await buildFullShowReminderPlan(admin, settings, bundle, senderName, { resendCancelled: true });
    const map = new Map<string, ReminderPlanRow>();
    for (const row of plan) {
      map.set(`${row.crew_id}|${logicalReminderKey(row.reminder_key)}`, row);
    }
    planBySender.set(senderName, map);
  }

  let refreshed = 0;
  for (const queued of queuedRows) {
    const id = safeText(queued.id);
    const crewId = safeText(queued.crew_id);
    const reminderKey = logicalReminderKey(safeText(queued.reminder_key));
    if (!id || !crewId || !reminderKey) continue;
    const senderName = safeText(queued.queued_by_name) || fallbackSenderName;
    const fresh = planBySender.get(senderName)?.get(`${crewId}|${reminderKey}`);
    if (!fresh) continue;
    const nextBody = safeText(fresh.body);
    const nextScheduledFor = safeText(fresh.scheduled_for);
    if (!nextBody || !nextScheduledFor) continue;
    if (nextBody === safeText(queued.body) && nextScheduledFor === safeText(queued.scheduled_for)) continue;
    const { error } = await admin
      .from("text_message_queue")
      .update({ body: nextBody, scheduled_for: nextScheduledFor, error: null })
      .eq("id", id)
      .in("status", ACTIVE_QUEUE_STATUSES);
    if (error) throw new Error(error.message);
    refreshed += 1;
...
async function buildSelectedDayReminderPlan(
  ...
  for (const [crewId, row] of firstSelectedRowByCrew.entries()) {
    ...
    const email = normalizeEmail(row.crew.email);
    const emailMode = settings.sending_method === "resend_email";
    const hasRecipient = emailMode ? validEmail(email) : Boolean(phone);
    ...
    const baseValues = item?.values || {
      ...
      booth_line: scheduleBoothNumber(row.call) ? `Booth: ${scheduleBoothNumber(row.call)}` : "",
      po_line: safeText(row.call.po_number) ? `PO: ${safeText(row.call.po_number)}` : "",
      crew_lead_line: "Crew Lead: ____________________\nPhone: ____________________",
      schedule: `${formatDate(row.day.labor_date)} - ${assignmentTimeRange(row.assignment, row.call)} - ${dayTypeLabel(assignmentDayType(row.assignment, row.call))} - ${messageRateForCall(row.call, row.crew, bundle.show, bundle.masterRates, parseEventMeta(bundle.show.notes), bundle.rateOverrides.get(row.assignment.id))}`,
      next_call: `${formatDate(row.day.labor_date)} ${assignmentTimeRange(row.assignment, row.call)}`,
      rate: messageRateForCall(row.call, row.crew, bundle.show, bundle.masterRates, parseEventMeta(bundle.show.notes), bundle.rateOverrides.get(row.assignment.id)),
...
function queueRowsFromReminderPlan(showId: string, plan: ReminderPlanRow[], user: { id: string; email?: string | null; display_name?: string | null; messaging_mode?: string | null }, includeOverdueNow: boolean, forceNow = false, deliveryModeOverride?: QueueDeliveryMode) {
  const nowIso = new Date(deliveryModeOverride === "resend_email" ? Date.now() : Date.now() + 30_000).toISOString();
...
async function buildImmediateFullShowSchedulePlan(
  ...
  const byCrew = nextScheduleByCrew(bundle.show, bundle.days, bundle.calls, bundle.assignments, bundle.crewRows, bundle.masterRates, settings, senderName, bundle.projectManagerContact, bundle.rateOverrides);
  const selectedCrewSet: Set<string> = new Set<string>(cleanQueueIds(options.selectedCrewIds || []));
  const existingRows = await readExistingQueueRowsForPlanning(admin, settings.show_id, settings.sending_method === "resend_email", true);
  ...
  for (const row of existingRows) {
    ...
    if (logicalReminderKey(key) === "full_show_schedule" || key.startsWith("manual_7_day")) {
      ...
      existing.set(logicalKey, chooseExistingQueueState(existing.get(logicalKey), { id: row.id || null, status: row.status || "scheduled", reminder_key: key, queued_by_user_id: row.queued_by_user_id || null, delivery_mode: row.delivery_mode || null }));
    }
  }
  ...
  for (const [crewId, item] of byCrew.entries()) {
    ...
    const result: ImmediateSchedulePlanRow["result"] = !hasRecipient
      ? "no_phone"
      : needsSenderRepair
        ? "will_queue"
      : options.forceQueue
    ...
    const resultLabel = result === "will_queue"
      ? needsSenderRepair
        ? "Already queued — attach current sender"
      : options.forceQueue
        ? options.forceUpdatedScheduleNotice ? "Selected — will queue updated schedule" : "Selected — will queue schedule"
      : existingResult === "already_sent" && options.resendSent
    ...
    rows.push({
      ...
      phone,
      email,
      scheduled_for: emailMode ? new Date().toISOString() : nowIso,
      scheduled_local: emailMode ? "Due now · sends by email through Resend" : "Due now · sends on next Shortcut run",
      result,
      result_label: resultLabel,
      message_type: "schedule",
...
function queueRowsFromImmediateSchedulePlan(showId: string, plan: ImmediateSchedulePlanRow[], user: { id: string; email?: string | null; display_name?: string | null; messaging_mode?: string | null }, forceNow = false, deliveryModeOverride?: QueueDeliveryMode) {
  const nowIso = new Date(deliveryModeOverride === "resend_email" ? Date.now() : Date.now() + 30_000).toISOString();
...
function buildCustomQueueRows(
  ...
  const byCrew = nextScheduleByCrew(bundle.show, targetDays, targetCalls, targetAssignments, bundle.crewRows, bundle.masterRates, settings, senderName, bundle.projectManagerContact, bundle.rateOverrides);
...
async function saveSettings(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, settings: AutomationSettings, user?: { id: string }) {
  ...
  const secureShortcutToken = settings.sending_method === "shortcut"
    ...
    : settings.sending_method === "resend_email"
      ? "__resend_email__"
      : settings.shortcut_token;
  const storedSendingMethod = settings.sending_method === "android_messages"
    ? "shortcut"
    : settings.sending_method === "resend_email"
      ? "provider"
      : settings.sending_method;
  const payload = {
    ...settings,
...
async function queueAvailabilityCandidates(
  ...
  const crewIds: string[] = cleanQueueIds(body.crew_ids, 250);
  const template = safeText(body.body);
...
function coordinatorNameFromQueueBody(body: unknown) {
  const match = safeText(body).match(/^Coordinator:\s*([^\r\n]+)/im);
  return normalizedLookupText(match?.[1] || "");
}

async function repairLegacyRowsMatchingSignedInSender(
  admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>,
  showId: string,
  user: { id: string; email?: string | null; display_name?: string | null; messaging_mode?: string | null },
) {
  const senderName = normalizedLookupText(senderNameForMessage(user));
  const senderEmail = normalizedLookupText(user.email);
  if (!senderName && !senderEmail) return 0;
  ...
  const legacy = await admin
    ...
    .limit(500);
  if (legacy.error) {
    if (isMissingSenderColumns(legacy.error.message)) return 0;
    throw new Error(legacy.error.message);
  }

  const ids = ((legacy.data || []) as Array<{ id: string; body?: string | null }>)
    .filter((row) => {
      const coordinator = coordinatorNameFromQueueBody(row.body);
      return Boolean(coordinator && (coordinator === senderName || coordinator === senderEmail));
    })
    .map((row) => row.id);
  ...
  let update = await admin
    ...
    .is("queued_by_user_id", null)
    .select("id");
  if (update.error && isMissingDeliveryModeColumns(update.error.message)) {
    const { delivery_mode: _deliveryMode, ...withoutDelivery } = identity;
    update = await admin
      .from("text_message_queue")
...
function resendEmailDomain() {
  return safeText(process.env.RESEND_EMAIL_DOMAIN || "emanuel-labor-services.com")
    .toLowerCase()
    .replace(/^https?:\/\//, "")
    .replace(/\/.*$/, "");
}

function scheduleEmailSubject(row: QueueRow, show: { name?: string | null; show_reference_number?: string | null } | null | undefined) {
  const showLabel = safeText(show?.show_reference_number) || safeText(show?.name) || "ELS Event";
  const key = safeText(row.reminder_key).toLowerCase();
  const body = safeText(row.body);
  if (key.includes("update") || body.startsWith("*****SCHEDULE UPDATED PLEASE CONFIRM CHANGES*****")) return `ELS Updated Schedule — ${showLabel}`;
  if (safeText(row.message_type).toLowerCase() === "schedule") return `ELS Schedule — ${showLabel}`;
  return `ELS Reminder — ${showLabel}`;
}

async function sendScheduleEmailThroughResend(args: {
  to: string;
  crewName: string;
  senderName: string;
  replyTo: string;
  ...
  const domain = resendEmailDomain();
  if (!domain) throw new Error("RESEND_EMAIL_DOMAIN is not configured in Vercel.");
  const displayName = safeText(args.senderName).replace(/[<>"\r\n]/g, "") || "ELS Coordinator";
  const first = firstName(safeText(args.crewName));
  const from = `${displayName} via Emanuel Labor Services <schedules@${domain}>`;
  const replyTo = normalizeEmail(args.replyTo);
  const messageHtml = escapeHtml(args.textBody).replace(/\r?\n/g, "<br>");
  const html = `<!doctype html>
<html><body style="margin:0;padding:0;background:#f5f7f8;font-family:Arial,Helvetica,sans-serif;color:#102a30;">
  <div style="max-width:680px;margin:0 auto;padding:28px 16px;">
    <div style="background:#ffffff;border:1px solid #d9e2e5;border-radius:14px;padding:28px;">
      <div style="font-size:24px;font-weight:700;margin-bottom:18px;">${escapeHtml(args.subject)}</div>
      <p style="font-size:16px;line-height:1.5;margin:0 0 16px;">Hi ${escapeHtml(first)},</p>
      <div style="font-size:16px;line-height:1.6;white-space:normal;">${messageHtml}</div>
      <p style="font-size:14px;line-height:1.5;color:#61757a;margin:24px 0 0;">Reply to this email to reach ${escapeHtml(displayName)}.</p>
    </div>
  </div>
</body></html>`;
  ...
  if (!response.ok) throw new Error(safeText(payload.message || payload.error?.message) || `Resend email failed with HTTP ${response.status}.`);
  return { id: safeText(payload.id) };
}
...
async function sendTwilioMessage(to: string, body: string) {
  ...
  if (!accountSid || !authToken || !fromNumber) throw new Error("Twilio is not configured.");
  ...
  if (!response.ok) throw new Error(safeText(data.message) || "Twilio send failed.");
  return data;
}

function localDateKey(timeZone: string, value = new Date()) {
  const parts = new Intl.DateTimeFormat("en-US", {
...
function dateFromReminderKey(value: unknown) {
  return safeText(value).match(/\b(20\d{2}-\d{2}-\d{2})\b/)?.[1] || "";
}

function standardReminderFamily(value: unknown) {
  const key = logicalReminderKey(safeText(value)).toLowerCase();
  if (key === "7_day" || key.startsWith("7_day_")) return "7_day";
  if (key === "3_day" || key.startsWith("3_day_")) return "3_day";
  if (key === "day_before" || key.startsWith("day_before_")) return "day_before";
  if (key === "day_of" || key.startsWith("day_of_")) return "day_of";
  if (key === "daily_after_first_day" || key.startsWith("daily_after_first_day_")) return "daily_after_first_day";
  return "";
}

async function blockOutdatedQueueRows(
  admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>,
  ids: string[],
  reason: string,
) {
  const cleanIds = [...new Set(ids.map(safeText).filter(Boolean))];
  if (!cleanIds.length) return 0;
  let blocked = await admin
    .from("text_message_queue")
    .update({ status: "schedule_outdated", error: reason })
    .in("id", cleanIds)
    .in("status", ACTIVE_QUEUE_STATUSES)
    .select("id");
  if (blocked.error) {
  ...
  if (blocked.error) throw new Error(`Safety scan could not block an outdated message: ${blocked.error.message}`);
  return blocked.data?.length || 0;
}

/**
 * Safety gate for every active schedule/reminder queue row. It rebuilds
 * standard messages from live event data and cancels rows that no longer map
 * to a current crew assignment. Both the recurring server sender and the
 * iPhone Shortcut call this immediately before a message can leave ELS.
 */
export async function revalidateActiveEventMessageQueue(
  admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>,
  showIds: string[] = [],
  options: { senderUserId?: string; dueOnly?: boolean } = {},
) {
  const cleanShowIds = [...new Set(showIds.map(safeText).filter(Boolean))];
  let queueQuery = admin
    .from("text_message_queue")
    .select("id, show_id, crew_id, message_type, reminder_key, scheduled_for, status, body, queued_by_name")
    .in("status", ACTIVE_QUEUE_STATUSES)
    .in("message_type", ["schedule", "reminder"])
    .limit(5000);
  if (cleanShowIds.length) queueQuery = queueQuery.in("show_id", cleanShowIds);
  if (safeText(options.senderUserId)) queueQuery = queueQuery.eq("queued_by_user_id", safeText(options.senderUserId));
  if (options.dueOnly) queueQuery = queueQuery.lte("scheduled_for", new Date().toISOString());
  ...
  const queueRows = (queueRes.data || []) as Array<{
    id: string; show_id: string; crew_id: string | null; message_type: string | null;
    reminder_key: string | null; scheduled_for: string | null; status: string | null;
    body: string | null; queued_by_name: string | null;
  }>;
  if (!queueRows.length) return { checked: 0, refreshed: 0, cancelled: 0 };

  const targetShowIds = [...new Set(queueRows.map((row) => safeText(row.show_id)).filter(Boolean))];
  const automationRes = await admin
    .from("show_text_automations")
    .select("show_id, enabled, sending_method, shortcut_token, send_availability, send_schedule, reminder_7_day, reminder_3_day, reminder_day_before, reminder_day_of, reminder_daily_after_first_day, timezone, availability_template, schedule_template, reminder_template")
    .in("show_id", targetShowIds);
  if (automationRes.error) throw new Error(automationRes.error.message);
  const automationByShow = new Map(((automationRes.data || []) as Array<Record<string, unknown>>).map((row) => [safeText(row.show_id), row] as const));

  let refreshed = 0;
  let cancelled = 0;
  for (const showId of targetShowIds) {
    const rows = queueRows.filter((row) => row.show_id === showId);
    let bundle: Awaited<ReturnType<typeof fetchShowBundle>>;
    try {
      bundle = await fetchShowBundle(admin, showId);
    } catch (error) {
      const ids = rows.map((row) => row.id).filter(Boolean);
      if (ids.length) {
        cancelled += await blockOutdatedQueueRows(admin, ids, `Blocked before sending: current event data could not be loaded. ${error instanceof Error ? error.message : ""}`.trim());
      }
      continue;
    }
    const rawSettings = automationByShow.get(showId) || {};
    const settings = normalizeSettings(rawSettings, showId);
    const today = localDateKey(settings.timezone);
    const activeRows = activeAssignmentRows(bundle);
    const assignmentsByCrew = new Map<string, typeof activeRows>();
    for (const row of activeRows) assignmentsByCrew.set(row.crew.id, [...(assignmentsByCrew.get(row.crew.id) || []), row]);
    const scheduleBySender = new Map<string, ReturnType<typeof nextScheduleByCrew>>();
    const reminderBySender = new Map<string, Map<string, ReminderPlanRow>>();
    ...
    for (const queued of rows) {
      ...
      const reminderDate = dateFromReminderKey(queued.reminder_key);
      const reminderFamily = standardReminderFamily(queued.reminder_key);
      const hasCurrentDateAssignment = !reminderDate || crewAssignments.some((row) => row.day.labor_date.slice(0, 10) === reminderDate);
      const hasUpcomingAssignment = crewAssignments.some((row) => row.day.labor_date.slice(0, 10) >= today);
      // Standard reminders are rebuilt below. Do not block one merely because
      // its old dated key points at the day that was moved; the live plan will
      // reschedule that same reminder to the contractor's current labor date.
      if (!crewId || !crewAssignments.length || (!reminderFamily && !hasCurrentDateAssignment) || !hasUpcomingAssignment) {
        const reason = !crewAssignments.length
          ? "Blocked before sending: this person is no longer assigned to this event."
          : reminderDate && !hasCurrentDateAssignment
            ? `Blocked before sending: this person is no longer assigned on ${reminderDate}.`
            : "Blocked before sending: every assigned labor date is already past.";
        cancelled += await blockOutdatedQueueRows(admin, [id], reason);
        continue;
      }

      const senderName = safeText(queued.queued_by_name) || "Storm Leigh";
      let nextBody = "";
      let nextScheduledFor = safeText(queued.scheduled_for);
      let nextReminderKey = safeText(queued.reminder_key);
      const messageType = safeText(queued.message_type).toLowerCase();
      const key = logicalReminderKey(safeText(queued.reminder_key));
      if (reminderFamily) {
        if (!reminderBySender.has(senderName)) {
          const plan = await buildFullShowReminderPlan(admin, settings, bundle, senderName, { resendCancelled: true });
          const planMap = new Map<string, ReminderPlanRow>();
          for (const row of plan) {
            const exactKey = `${row.crew_id}|${logicalReminderKey(row.reminder_key)}`;
            planMap.set(exactKey, row);
            const familyKey = `${row.crew_id}|family:${standardReminderFamily(row.reminder_key)}`;
            if (!planMap.has(familyKey) || row.scheduled_for < (planMap.get(familyKey)?.scheduled_for || "")) planMap.set(familyKey, row);
          }
          reminderBySender.set(senderName, planMap);
        }
        const senderPlan = reminderBySender.get(senderName);
        const fresh = senderPlan?.get(`${crewId}|${key}`) || senderPlan?.get(`${crewId}|family:${reminderFamily}`);
        if (fresh) {
          nextBody = safeText(fresh.body);
          nextScheduledFor = safeText(fresh.scheduled_for);
          nextReminderKey = safeText(fresh.reminder_key);
        }
      } else if (messageType === "schedule") {
        if (!scheduleBySender.has(senderName)) {
          scheduleBySender.set(senderName, nextScheduleByCrew(bundle.show, bundle.days, bundle.calls, bundle.assignments, bundle.crewRows, bundle.masterRates, settings, senderName, bundle.projectManagerContact, bundle.rateOverrides));
        }
        const item = scheduleBySender.get(senderName)?.get(crewId);
        if (item) {
          nextBody = ensureSenderIdentity(applyTemplate(settings.schedule_template || defaultScheduleTemplate, item.values), senderName);
          if (safeText(queued.body).startsWith("*****SCHEDULE UPDATED PLEASE CONFIRM CHANGES*****")) nextBody = prependUpdatedScheduleNotice(nextBody);
        }
      } else {
        // Custom reminder keys are not rewritten, but they still pass the live
        // assignment/date safety checks above before they can send.
        nextBody = safeText(queued.body);
      }

      if (!nextBody) {
        cancelled += await blockOutdatedQueueRows(admin, [id], "Blocked before sending: a current schedule/reminder could not be rebuilt from the live event.");
        continue;
      }
      if (nextBody !== safeText(queued.body) || nextScheduledFor !== safeText(queued.scheduled_for) || nextReminderKey !== safeText(queued.reminder_key)) {
        let update = await admin.from("text_message_queue").update({ body: nextBody, scheduled_for: nextScheduledFor, reminder_key: nextReminderKey, error: null }).eq("id", id).in("status", ACTIVE_QUEUE_STATUSES);
        // A prior queue revision can already own the fresh reminder key. The
        // row being sent still gets the live body and time; retaining its old
        // key avoids violating the historical queue's uniqueness constraint.
        if (update.error && /duplicate key value violates unique constraint/i.test(update.error.message)) {
          update = await admin.from("text_message_queue").update({ body: nextBody, scheduled_for: nextScheduledFor, error: null }).eq("id", id).in("status", ACTIVE_QUEUE_STATUSES);
        }
        if (update.error) throw new Error(update.error.message);
        refreshed += 1;
      }
  ...
  return { checked: queueRows.length, refreshed, cancelled };
}

async function processDueServerMessages(
  admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>,
  showId?: string | null,
) {
  await revalidateActiveEventMessageQueue(admin, showId ? [showId] : [], { dueOnly: true });
  let query = admin
    .from("text_message_queue")
    .select(TEXT_QUEUE_SELECT)
    .in("status", ACTIVE_QUEUE_STATUSES)
  ...
  let queueRes = await query as { data: unknown[] | null; error: { message: string } | null };
  if (queueRes.error && isMissingDeliveryModeColumns(queueRes.error.message)) {
    let legacy = admin
      .from("text_message_queue")
      .select("id, show_id, crew_id, crew_name, phone, message_type, reminder_key, scheduled_for, status, body, sent_at, error, created_at, queued_by_user_id, queued_by_email, queued_by_name")
      .in("status", ACTIVE_QUEUE_STATUSES)
      ...
      .order("scheduled_for", { ascending: true })
      .limit(100);
    if (showId) legacy = legacy.eq("show_id", showId);
    queueRes = await legacy as { data: unknown[] | null; error: { message: string } | null };
  }
  if (queueRes.error) throw new Error(queueRes.error.message);
  const rows = (queueRes.data || []) as QueueRow[];
  if (!rows.length) return { processed: 0, sent: 0, failed: 0, rows: [] as QueueRow[] };

  const showIds = Array.from(new Set(rows.map((row) => safeText(row.show_id)).filter(Boolean)));
  const crewIds = Array.from(new Set(rows.filter((row) => safeText(row.delivery_mode) === "resend_email").map((row) => safeText(row.crew_id)).filter(Boolean)));
  const [showsRes, crewRes, automationRes] = await Promise.all([
    showIds.length ? admin.from("shows").select("id, name, show_reference_number").in("id", showIds) : Promise.resolve({ data: [], error: null }),
    crewIds.length ? admin.from("crew").select("id, name, email").in("id", crewIds) : Promise.resolve({ data: [], error: null }),
    showIds.length ? admin.from("show_text_automations").select("show_id, sending_method, shortcut_token").in("show_id", showIds) : Promise.resolve({ data: [], error: null }),
  ]);
  if (showsRes.error) throw new Error(showsRes.error.message);
  if (crewRes.error) throw new Error(crewRes.error.message);
  if (automationRes.error) throw new Error(automationRes.error.message);
  const showById = new Map(((showsRes.data || []) as Array<{ id: string; name?: string | null; show_reference_number?: string | null }>).map((row) => [row.id, row]));
  const crewById = new Map(((crewRes.data || []) as Array<{ id: string; name?: string | null; email?: string | null }>).map((row) => [row.id, row]));
  const providerShowIds = new Set(((automationRes.data || []) as Array<{ show_id: string; sending_method?: string | null; shortcut_token?: string | null }>)
    .filter((row) => safeText(row.sending_method) === "provider" && safeText(row.shortcut_token) !== "__resend_email__")
    .map((row) => row.show_id));
  ...
  for (const row of rows) {
    ...
    const providerMode = !emailMode && providerShowIds.has(row.show_id);
    if (!emailMode && !providerMode) continue;
    ...
    const claimed = claim.data as QueueRow;

    try {
      if (!safeText(claimed.body)) throw new Error("Missing message body.");
      if (emailMode) {
        const crew = crewById.get(safeText(claimed.crew_id));
        const to = normalizeEmail(crew?.email);
        if (!validEmail(to)) throw new Error("Crew member does not have a valid email address.");
        await sendScheduleEmailThroughResend({
          to,
          crewName: safeText(crew?.name) || safeText(claimed.crew_name) || "Crew member",
          senderName: safeText(claimed.queued_by_name) || safeText(claimed.queued_by_email) || "ELS Coordinator",
          replyTo: safeText(claimed.queued_by_email),
          subject: scheduleEmailSubject(claimed, showById.get(claimed.show_id)),
          textBody: safeText(claimed.body),
        });
      } else {
        if (!safeText(claimed.phone)) throw new Error("Missing phone number.");
        await sendTwilioMessage(safeText(claimed.phone), safeText(claimed.body));
      }
      ...
      const sentRow = saved.data as QueueRow;
      await syncAssignmentChecklistFromSentMessage(admin, {
        show_id: sentRow.show_id,
        crew_id: sentRow.crew_id || null,
        message_type: sentRow.message_type || null,
        reminder_key: sentRow.reminder_key || null,
        sent_at: sentRow.sent_at || null,
      }).catch(() => null);
      updated.push(sentRow);
      sent += 1;
    } catch (error) {
      ...
      const saved = await admin
        ...
        .select(TEXT_QUEUE_SELECT)
        .single();
      if (saved.data) updated.push(saved.data as QueueRow);
      failed += 1;
    }
  }
  return { processed: sent + failed, sent, failed, rows: updated };
}

function cronRequestAuthorized(request: Request) {
  const configured = safeText(process.env.CRON_SECRET);
  if (!configured) return true;
  return safeText(request.headers.get("authorization")) === `Bearer ${configured}`;
}

async function resendDeliveryEvent(emailId: string) {
  const key = resendApiKey();
  if (!key || !emailId) return "";
  const response = await fetch(`https://api.resend.com/emails/${encodeURIComponent(emailId)}`, { headers: { Authorization: `Bearer ${key}` }, cache: "no-store" });
  if (!response.ok) return "";
  const result = await response.json().catch(() => ({})) as { last_event?: string };
  return safeText(result.last_event).toLowerCase();
}

async function sendPurchaseOrderReminderEmail(args: { to: string; name: string; showName: string; displayNumber: string; total: number; link: string; dueAt: string }) {
  const key = resendApiKey();
  const domain = resendEmailDomain();
  if (!key || !domain || !validEmail(args.to)) throw new Error("Purchase Order reminder email is not configured or the contractor email is invalid.");
  const due = new Date(args.dueAt).toLocaleString("en-US", { dateStyle: "medium", timeStyle: "short", timeZone: "America/New_York" });
  const reference = args.displayNumber ? ` ${args.displayNumber}` : "";
  const response = await fetch("https://api.resend.com/emails", {
    method: "POST",
    headers: { Authorization: `Bearer ${key}`, "Content-Type": "application/json" },
    body: JSON.stringify({
      from: `Emanuel Labor Services <payroll@${domain}>`, reply_to: `info@${domain}`, to: [args.to],
      subject: `Reminder: review Purchase Order${reference} for ${args.showName}`,
      text: `Hello ${args.name},\n\nYour Purchase Order${reference} for ${args.showName} is still awaiting your approval or change request. Total: ${new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(args.total)}. Please respond by ${due}. If no response is received within 48 hours of delivery, it will be recorded as approved and payment will be processed for the stated amount.\n\nReview: ${args.link}`,
    }),
  });
  if (!response.ok) throw new Error(`Purchase Order reminder returned HTTP ${response.status}.`);
}

async function processPurchaseOrderAutomation(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, origin: string) {
  const payrollResult = await admin.from("show_payroll").select("id,show_id,crew_id,notes").ilike("notes", "%ELS_PURCHASE_ORDER_V1%").limit(5000);
  if (payrollResult.error) throw new Error(payrollResult.error.message);
  const records = ((payrollResult.data || []) as Array<{ id: string; show_id: string; crew_id: string; notes?: string | null }>).map((row) => ({ row, po: extractPurchaseOrderFromNotes(row.notes) })).filter((item): item is { row: { id: string; show_id: string; crew_id: string; notes?: string | null }; po: ContractorPurchaseOrder } => Boolean(item.po));
  const active = records.filter(({ po }) => po.status === "sent_awaiting_delivery" || po.status === "delivered");
  if (!active.length) return { checked: 0, delivered: 0, reminded: 0, autoApproved: 0, failed: 0 };
  const showIds = [...new Set(active.map(({ row }) => row.show_id))];
  const crewIds = [...new Set(active.map(({ row }) => row.crew_id))];
  const [showsResult, crewResult] = await Promise.all([
    admin.from("shows").select("id,name").in("id", showIds),
    admin.from("crew").select("id,name,email").in("id", crewIds),
  ]);
  const showById = new Map(((showsResult.data || []) as Array<{ id: string; name?: string | null }>).map((row) => [row.id, row]));
  const crewById = new Map(((crewResult.data || []) as Array<{ id: string; name?: string | null; email?: string | null }>).map((row) => [row.id, row]));
  const stats = { checked: active.length, delivered: 0, reminded: 0, autoApproved: 0, failed: 0 };
  for (const { row, po: original } of active) {
    let po = original;
    const now = new Date();
    const at = now.toISOString();
    if (po.status === "sent_awaiting_delivery" && po.resendEmailId) {
      const event = await resendDeliveryEvent(po.resendEmailId);
      if (event === "delivered") {
        const due = new Date(now.getTime() + 48 * 60 * 60 * 1000).toISOString();
        po = { ...po, status: "delivered", deliveredAt: at, approvalDueAt: due, lastDeliveryEvent: event, updatedAt: at, audit: [...po.audit, { at, action: "delivery_confirmed", detail: `Response due ${due}` }].slice(-60) };
        stats.delivered += 1;
      } else if (["bounced", "complained", "failed", "canceled"].includes(event)) {
        po = { ...po, status: "delivery_failed", lastDeliveryEvent: event, approvalDueAt: null, updatedAt: at, audit: [...po.audit, { at, action: "delivery_failed", detail: event }].slice(-60) };
        stats.failed += 1;
      } else if (event && event !== po.lastDeliveryEvent) {
        po = { ...po, lastDeliveryEvent: event, updatedAt: at };
      }
    }
    if (po.status === "delivered" && po.approvalDueAt) {
      const dueMs = new Date(po.approvalDueAt).getTime();
      const deliveredMs = new Date(po.deliveredAt || po.updatedAt).getTime();
      if (now.getTime() >= dueMs) {
        po = { ...po, status: "auto_approved", approvedAt: at, updatedAt: at, audit: [...po.audit, { at, action: "auto_approved", detail: "No response within 48 hours of confirmed delivery" }].slice(-60) };
        stats.autoApproved += 1;
      } else if (!po.reminderSentAt && now.getTime() >= deliveredMs + 24 * 60 * 60 * 1000) {
        const crew = crewById.get(row.crew_id);
        try {
          await sendPurchaseOrderReminderEmail({ to: safeText(crew?.email), name: safeText(crew?.name) || "Contractor", showName: safeText(showById.get(row.show_id)?.name) || "your event", displayNumber: purchaseOrderDisplayNumber(po), total: po.total, link: `${origin}/onboarding/${encodeURIComponent(po.token)}?mode=purchase_order`, dueAt: po.approvalDueAt });
          po = { ...po, reminderSentAt: at, updatedAt: at, audit: [...po.audit, { at, action: "24_hour_reminder_sent" }].slice(-60) };
          stats.reminded += 1;
        } catch (error) {
          po = { ...po, audit: [...po.audit, { at, action: "reminder_failed", detail: error instanceof Error ? error.message : "Reminder failed" }].slice(-60) };
        }
      }
    }
    if (JSON.stringify(po) !== JSON.stringify(original)) {
      const update = await admin.from("show_payroll").update({ notes: mergePurchaseOrderIntoNotes(row.notes, po) }).eq("id", row.id);
      if (update.error) throw new Error(update.error.message);
    }
  }
  return stats;
...
export async function GET(request: Request) {
  const route = safeText(new URL(request.url).searchParams.get("route"));
  if (route !== "send_due") return NextResponse.json({ message: "Not found." }, { status: 404 });
  if (!cronRequestAuthorized(request)) return NextResponse.json({ message: "Unauthorized." }, { status: 401 });
  const admin = createSupabaseAdminClient();
  if (!admin) return NextResponse.json({ message: "SUPABASE_SERVICE_ROLE_KEY is missing." }, { status: 500 });
  try {
    const [result, purchaseOrders] = await Promise.all([processDueServerMessages(admin, null), processPurchaseOrderAutomation(admin, new URL(request.url).origin)]);
    return NextResponse.json({ ok: true, ...result, purchase_orders: purchaseOrders, message: `${result.processed ? `Processed ${result.processed} due server message${result.processed === 1 ? "" : "s"}.` : "No due server messages."} Checked ${purchaseOrders.checked} Purchase Order${purchaseOrders.checked === 1 ? "" : "s"}.` });
  } catch (error) {
    return NextResponse.json({ message: error instanceof Error ? error.message : "Unable to process due server messages." }, { status: 400 });
  }
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (route === "send_due") {
      ...
      const result = await processDueServerMessages(admin, safeText(body.show_id) || null);
      return NextResponse.json({ ok: true, ...result, message: result.processed ? `Processed ${result.processed} due server message${result.processed === 1 ? "" : "s"}: ${result.sent} sent, ${result.failed} failed.` : "No due Resend/provider messages found." });
    }
    const body = await request.json();
    ...
    if (action === "mark_cancellation_notices") {
      if (!showId) return NextResponse.json({ message: "show_id is required." }, { status: 400 });
      const ids: string[] = cleanQueueIds(body.ids, 250);
    ...
    if (action === "refresh_queue") {
      if (!showId) return NextResponse.json({ message: "show_id is required." }, { status: 400 });
      await repairLegacyRowsMatchingSignedInSender(admin, showId, auth.user);
      await repairActiveScheduleQueueBoothLines(admin, [showId]);
      const { data, error } = await admin
        .from("text_message_queue")
        .select("id, show_id, crew_id, crew_name, phone, message_type, reminder_key, scheduled_for, status, body, sent_at, error, created_at, queued_by_user_id, queued_by_email, queued_by_name")
        .eq("show_id", showId)
    ...
    if (action === "admin_update_selected_sender") {
      ...
      if (senderUserId !== auth.user.id) {
        ...
        if (targetRes.error && /messaging_mode|device_type|schema cache/i.test(targetRes.error.message)) {
          ...
          const authLookup = await admin.auth.admin.getUserById(senderUserId);
          const metadataMode = (authLookup.data.user?.user_metadata as { els_messaging_mode?: string | null } | undefined)?.els_messaging_mode;
          targetUser = { id: profile.id, email: profile.email || null, display_name: profile.full_name || profile.email || null, messaging_mode: normalizeMessagingMode(metadataMode) };
        } else {
    ...
    if (action === "requeue_cancelled") {
      ...
      if (cancelled.message_type === "schedule" && (logicalKey === "full_show_schedule" || logicalKey.startsWith("manual_7_day"))) {
        ...
        queueRows = queueRowsFromImmediateSchedulePlan(showId, plan.filter((row) => row.crew_id === crewId), auth.user, true, deliveryModeForSettings(effectiveSettings, auth.user));
      } else {
        const plan = await buildFullShowReminderPlan(admin, effectiveSettings, bundle, senderName, { resendCancelled: true });
        queueRows = queueRowsFromReminderPlan(showId, plan.filter((row) => row.crew_id === crewId && logicalReminderKey(row.reminder_key) === logicalKey), auth.user, true, Boolean(body.requeue_now), deliveryModeForSettings(effectiveSettings, auth.user));
      }
      if (!queueRows.length) {
    ...
    if (action === "preview_selected_schedules" || action === "send_selected_schedules") {
      if (!showId) return NextResponse.json({ message: "show_id is required." }, { status: 400 });
      const crewIds: string[] = cleanQueueIds(body.crew_ids, 200);
      if (!crewIds.length) return NextResponse.json({ message: "Select at least one tech to receive a schedule." }, { status: 400 });
      const settings = normalizeSettings(body.settings || {}, showId);
      const effectiveSettings = { ...settings, enabled: true, send_schedule: true };
      const saved = action === "send_selected_schedules" ? await saveSettings(admin, effectiveSettings, auth.user) : null;
      const bundle = await fetchShowBundle(admin, showId);
      const senderName = senderNameForMessage(auth.user);
      const selectedKey = `full_show_schedule_selected_${new Date().toISOString().replace(/[^0-9]/g, "").slice(0, 14)}`;
      const schedulePlan = await buildImmediateFullShowSchedulePlan(admin, effectiveSettings, bundle, senderName, safeText(body.additional_note), {
        ...
        forceQueue: true,
        forceUpdatedScheduleNotice: false,
        reminderKey: selectedKey,
      });
      const summary = combinedScheduleSummary(schedulePlan, []);
      if (action === "preview_selected_schedules") {
        return NextResponse.json({
          ok: true,
          schedule_plan: schedulePlan,
          summary,
          message: `${summary.schedules_now} selected schedule${summary.schedules_now === 1 ? "" : "s"} will be sent/queued now.`,
        });
      }

      const deliveryMode = deliveryModeForSettings(effectiveSettings, auth.user);
      const selectedDueNowIso = new Date().toISOString();
      const queueRows = queueRowsFromImmediateSchedulePlan(showId, schedulePlan, auth.user, true, deliveryMode)
        .map((row) => deliveryMode === "resend_email" ? row : { ...row, scheduled_for: selectedDueNowIso });
      const data = queueRows.length ? await upsertTextQueueRows(admin, queueRows) : [];
      if (queueRows.length && (data?.length || 0) < queueRows.length) {
        throw new Error(`Expected to queue ${queueRows.length} selected schedule${queueRows.length === 1 ? "" : "s"}, but only ${data?.length || 0} queue row${(data?.length || 0) === 1 ? " was" : "s were"} saved.`);
      }
      const serverSend = deliveryMode === "resend_email" ? await processDueServerMessages(admin, showId) : null;
      return NextResponse.json({
        ok: true,
        settings: saved,
        queue: data || [],
        schedule_plan: schedulePlan,
        summary,
        message: deliveryMode === "resend_email"
          ? `${serverSend?.sent || 0} selected schedule email${(serverSend?.sent || 0) === 1 ? "" : "s"} sent through Resend.${serverSend?.failed ? ` ${serverSend.failed} failed.` : ""}`
          : `${data?.length || queueRows.length} selected schedule${(data?.length || queueRows.length) === 1 ? "" : "s"} queued due now.`,
      });
    }
    if (action === "preview_updated_schedules" || action === "send_updated_schedules") {
      ...
      const queueRows = queueRowsFromImmediateSchedulePlan(showId, schedulePlan, auth.user, true, deliveryMode);
      const data = queueRows.length ? await upsertTextQueueRows(admin, queueRows) : [];
      const remindersRefreshed = await refreshActiveQueuedRemindersForCrew(admin, effectiveSettings, bundle, crewIds, senderName);
      const serverSend = deliveryMode === "resend_email" ? await processDueServerMessages(admin, showId) : null;
      return NextResponse.json({
        ok: true,
        settings: saved,
        ...
        schedule_plan: schedulePlan,
        summary,
        reminders_refreshed: remindersRefreshed,
        message: deliveryMode === "resend_email"
          ? `${serverSend?.sent || 0} updated schedule email${(serverSend?.sent || 0) === 1 ? "" : "s"} sent through Resend with the update notice.${remindersRefreshed ? ` ${remindersRefreshed} future reminder${remindersRefreshed === 1 ? " was" : "s were"} refreshed with current times.` : ""}${serverSend?.failed ? ` ${serverSend.failed} failed.` : ""}`
          : `${data?.length || queueRows.length} updated schedule${(data?.length || queueRows.length) === 1 ? "" : "s"} queued with the update notice.${remindersRefreshed ? ` ${remindersRefreshed} future reminder${remindersRefreshed === 1 ? " was" : "s were"} refreshed with current times.` : ""}`,
      });
    }
    if (action === "preview_combined_schedule_reminders" || action === "send_combined_schedule_reminders") {
      ...
      const deliveryMode = deliveryModeForSettings(effectiveSettings, auth.user);
      const queueRows = [
        ...queueRowsFromImmediateSchedulePlan(showId, schedulePlan, auth.user, false, deliveryMode),
        ...queueRowsFromReminderPlan(showId, reminderPlan, auth.user, includeOverdueNow, false, deliveryMode),
      ];
      const data = queueRows.length ? await upsertTextQueueRows(admin, queueRows) : [];
      const serverSend = deliveryMode === "resend_email" ? await processDueServerMessages(admin, showId) : null;
      return NextResponse.json({
        ...
        summary,
        message: deliveryMode === "resend_email"
          ? `Schedules sent and future reminders queued by email. ${serverSend?.sent || 0} schedule/reminder email${(serverSend?.sent || 0) === 1 ? "" : "s"} sent now through Resend, ${summary.future} future reminder${summary.future === 1 ? "" : "s"} queued for automatic Resend delivery.${serverSend?.failed ? ` ${serverSend.failed} email${serverSend.failed === 1 ? "" : "s"} failed.` : ""}${summary.overdue && !includeOverdueNow ? ` ${summary.overdue} overdue reminder${summary.overdue === 1 ? " was" : "s were"} skipped.` : ""}`
          : `Schedules queued and future reminders scheduled. ${summary.schedules_now} schedule${summary.schedules_now === 1 ? "" : "s"} due now, ${summary.future} future reminder${summary.future === 1 ? "" : "s"} scheduled.${includeOverdueNow && summary.overdue ? ` ${summary.overdue} overdue reminder${summary.overdue === 1 ? " was" : "s were"} queued due now.` : summary.overdue ? ` ${summary.overdue} overdue reminder${summary.overdue === 1 ? " was" : "s were"} skipped.` : ""}`,
      });
    }
    if (action === "preview_checked_reminders" || action === "schedule_checked_reminders") {
      ...
      const includeOverdueNow = Boolean(body.include_overdue_now);
      const deliveryMode = deliveryModeForSettings(effectiveSettings, auth.user);
      const queueRows = queueRowsFromReminderPlan(showId, plan, auth.user, includeOverdueNow, false, deliveryMode);
      if (!queueRows.length) {
        return NextResponse.json({
          ok: true,
          settings: saved,
      ...
      return NextResponse.json({
        ...
        summary: { future: futureCount, overdue: overdueCount, skipped: skippedCount },
        message: deliveryMode === "resend_email"
          ? `Queued ${data?.length || queueRows.length} email reminder${(data?.length || queueRows.length) === 1 ? "" : "s"} for automatic Resend delivery.${serverSend?.sent ? ` ${serverSend.sent} due email${serverSend.sent === 1 ? " was" : "s were"} sent now.` : ""}`
          : `Scheduled ${data?.length || queueRows.length} reminder${(data?.length || queueRows.length) === 1 ? "" : "s"} for this show.${includeOverdueNow && overdueCount ? ` ${overdueCount} overdue reminder${overdueCount === 1 ? " was" : "s were"} queued due now.` : ""}`,
      });
    }
    ...
    if (isCustomMessageAction) {
      ...
      const queueRows = stampRows(buildCustomQueueRows(effectiveSettings, bundle, senderName, {
        ...
        subCallIds: cleanQueueIds(body.sub_call_ids, 250),
    ...
    if (scheduleManualDayAtNormalTime && manualDay) {
      ...
      const deliveryMode = deliveryModeForSettings(effectiveSettings, auth.user);
      const queueRows = queueRowsFromReminderPlan(showId, plan, auth.user, true, false, deliveryMode);
      if (!queueRows.length) {
      ...
      const data = await upsertTextQueueRows(admin, queueRows);
      if (deliveryMode === "resend_email") await processDueServerMessages(admin, showId);
