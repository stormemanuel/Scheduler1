...
import { createSupabaseAdminClient, repairActiveScheduleQueueBoothLines, syncAssignmentChecklistFromSentMessage } from "@/lib/supabase-server";
import { revalidateActiveEventMessageQueue } from "@/app/api/text-automation/route";
...
async function loadDueEventMessages(request: Request, admin: ReturnType<typeof createSupabaseAdminClient>, showIds: string[], token: string, limit: number, universal = true, senderUserId = "", claim = false) {
  ...
  await recoverStaleSendingRows(admin, showIds, senderUserId);
  await revalidateActiveEventMessageQueue(admin, showIds, { senderUserId, dueOnly: true });
  let query = admin
    .from("text_message_queue")
  ...
  let dueRows = (data || []) as QueueRow[];
  const boothRepairIds = dueRows
    .filter((row) => safeText(row.message_type).toLowerCase() === "schedule" && !/(^|\n)Booth\s*:/i.test(safeText(row.body)))
    .map((row) => safeText(row.id))
    .filter(Boolean);
  if (boothRepairIds.length) {
    const repaired = await repairActiveScheduleQueueBoothLines(admin, showIds, { queueIds: boothRepairIds });
    if (Object.keys(repaired.bodies).length) {
      dueRows = dueRows.map((row) => repaired.bodies[row.id] ? { ...row, body: repaired.bodies[row.id] } : row);
    }
  }
  const readyRows = dueRows.filter((row) => isAppleShortcutQueueRow(row) && safeText(row.phone) && safeText(row.body));
...
async function loadDueEventMessagesGlobal(request: Request, admin: ReturnType<typeof createSupabaseAdminClient>, showIds: string[], token: string, limit: number, senderUserId = "", claim = false) {
  ...
  let dueRows = (data || []) as QueueRow[];
  const boothRepairRows = dueRows.filter((row) => safeText(row.message_type).toLowerCase() === "schedule" && !/(^|\n)Booth\s*:/i.test(safeText(row.body)));
  const boothRepairIds = boothRepairRows.map((row) => safeText(row.id)).filter(Boolean);
  const boothRepairShowIds = Array.from(new Set(boothRepairRows.map((row) => safeText(row.show_id)).filter(Boolean)));
  if (boothRepairIds.length && boothRepairShowIds.length) {
    const repaired = await repairActiveScheduleQueueBoothLines(admin, boothRepairShowIds, { queueIds: boothRepairIds });
    if (Object.keys(repaired.bodies).length) {
      dueRows = dueRows.map((row) => repaired.bodies[row.id] ? { ...row, body: repaired.bodies[row.id] } : row);
    }
  }
