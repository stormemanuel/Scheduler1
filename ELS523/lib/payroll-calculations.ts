...
export const PURCHASE_ORDER_NOTES_START = "[[ELS_PURCHASE_ORDER_V1]]";
export const PURCHASE_ORDER_NOTES_END = "[[/ELS_PURCHASE_ORDER_V1]]";

export type PurchaseOrderStatus =
  | "draft"
  | "ready"
  | "sent_awaiting_delivery"
  | "delivered"
  | "approved"
  | "auto_approved"
  | "changes_requested"
...
export type PurchaseOrderAdjustments = {
  ...
  shortTurnHours: number;
  shortTurnRate: number;
...
export type PurchaseOrderDayLine = {
  key: string;
  laborDate: string;
  description: string;
  note: string;
...
export type ContractorPurchaseOrder = {
  version: 1;
  token: string;
  ...
  baseAmount: number;
  days: PurchaseOrderDayLine[];
  adjustments: PurchaseOrderAdjustments;
  total: number;
  createdAt: string;
  updatedAt: string;
  sentAt?: string | null;
  ...
  changeRequestViewedAt?: string | null;
  resendEmailId?: string | null;
  lastDeliveryEvent?: string | null;
  audit: PurchaseOrderAuditEvent[];
};

function poNumber(value: unknown) {
  const parsed = Number(value);
  return Number.isFinite(parsed) && parsed >= 0 ? Math.round(parsed * 100) / 100 : 0;
}

export function normalizePurchaseOrderDays(value: unknown): PurchaseOrderDayLine[] {
  if (!Array.isArray(value)) return [];
  return value.slice(0, 100).map((item, index) => {
    const day = item && typeof item === "object" ? item as Partial<PurchaseOrderDayLine> : {};
    const laborDate = String(day.laborDate || "").trim().slice(0, 10);
    return {
      key: String(day.key || laborDate || `day-${index + 1}`).trim().slice(0, 120),
      laborDate,
      description: String(day.description || "").trim().slice(0, 240),
      note: String(day.note || "").trim().slice(0, 1000),
      defaultAmount: poNumber(day.defaultAmount),
      overtimeHours: poNumber(day.overtimeHours),
      overtimeRate: poNumber(day.overtimeRate),
      doubleTimeHours: poNumber(day.doubleTimeHours),
      doubleTimeRate: poNumber(day.doubleTimeRate),
    };
  });
}

export function purchaseOrderDayAmounts(day: PurchaseOrderDayLine) {
  return {
    defaultAmount: poNumber(day.defaultAmount),
    overtime: Math.round(poNumber(day.overtimeHours) * poNumber(day.overtimeRate) * 100) / 100,
    doubleTime: Math.round(poNumber(day.doubleTimeHours) * poNumber(day.doubleTimeRate) * 100) / 100,
  };
}

export function normalizePurchaseOrderAdjustments(value: Partial<PurchaseOrderAdjustments> | null | undefined): PurchaseOrderAdjustments {
  return {
    overtimeHours: poNumber(value?.overtimeHours),
    overtimeRate: poNumber(value?.overtimeRate),
    doubleTimeHours: poNumber(value?.doubleTimeHours),
    doubleTimeRate: poNumber(value?.doubleTimeRate),
    mealPenaltyCount: Math.max(0, Math.floor(poNumber(value?.mealPenaltyCount))),
    mealPenaltyRate: poNumber(value?.mealPenaltyRate),
    shortTurnHours: poNumber(value?.shortTurnHours),
    shortTurnRate: poNumber(value?.shortTurnRate),
    otherDescription: String(value?.otherDescription || "").trim().slice(0, 240),
    otherAmount: poNumber(value?.otherAmount),
    lateHours: poNumber(value?.lateHours),
    lateHourlyRate: poNumber(value?.lateHourlyRate),
    negativeDescription: String(value?.negativeDescription || "").trim().slice(0, 240),
    negativeAmount: poNumber(value?.negativeAmount),
  };
}

export function purchaseOrderAdjustmentAmounts(value: Partial<PurchaseOrderAdjustments> | null | undefined) {
  const adjustments = normalizePurchaseOrderAdjustments(value);
  return {
    overtime: Math.round(adjustments.overtimeHours * adjustments.overtimeRate * 100) / 100,
    doubleTime: Math.round(adjustments.doubleTimeHours * adjustments.doubleTimeRate * 100) / 100,
    mealPenalty: Math.round(adjustments.mealPenaltyCount * adjustments.mealPenaltyRate * 100) / 100,
    shortTurn: Math.round(adjustments.shortTurnHours * adjustments.shortTurnRate * 100) / 100,
    other: adjustments.otherAmount,
    lateDeduction: Math.round(adjustments.lateHours * adjustments.lateHourlyRate * 100) / 100,
    negativeAdjustment: adjustments.negativeAmount,
  };
}

export function purchaseOrderTotal(baseAmount: unknown, value: Partial<PurchaseOrderAdjustments> | null | undefined, dayValue?: unknown) {
  const amounts = purchaseOrderAdjustmentAmounts(value);
  const days = normalizePurchaseOrderDays(dayValue);
  const dayTotal = days.reduce((sum, day) => {
    const dayAmounts = purchaseOrderDayAmounts(day);
    return sum + dayAmounts.defaultAmount + dayAmounts.overtime + dayAmounts.doubleTime;
  }, 0);
  const legacyBaseAndPremiums = poNumber(baseAmount) + amounts.overtime + amounts.doubleTime;
  return Math.max(0, Math.round(((days.length ? dayTotal : legacyBaseAndPremiums) + amounts.mealPenalty + amounts.shortTurn + amounts.other - amounts.lateDeduction - amounts.negativeAdjustment) * 100) / 100);
}

export function purchaseOrderDisplayNumber(purchaseOrder: Pick<ContractorPurchaseOrder, "referenceNumber" | "revision">) {
  const reference = String(purchaseOrder.referenceNumber || "").trim();
  return reference ? `${reference}-R${Math.max(1, Math.floor(Number(purchaseOrder.revision) || 1))}` : "";
}

export function extractPurchaseOrderFromNotes(notes: unknown): ContractorPurchaseOrder | null {
  const text = String(notes || "");
  const start = text.indexOf(PURCHASE_ORDER_NOTES_START);
  const end = text.indexOf(PURCHASE_ORDER_NOTES_END);
  if (start < 0 || end <= start) return null;
  try {
    const parsed = JSON.parse(text.slice(start + PURCHASE_ORDER_NOTES_START.length, end).trim()) as ContractorPurchaseOrder;
    if (!parsed || parsed.version !== 1 || !String(parsed.token || "").trim()) return null;
    const adjustments = normalizePurchaseOrderAdjustments(parsed.adjustments);
    const days = normalizePurchaseOrderDays(parsed.days);
    const baseAmount = days.length ? Math.round(days.reduce((sum, day) => sum + day.defaultAmount, 0) * 100) / 100 : poNumber(parsed.baseAmount);
    return {
      ...parsed,
      previousTokens: Array.isArray(parsed.previousTokens)
        ? [...new Set(parsed.previousTokens.map((token) => String(token || "").trim()).filter((token) => token.length >= 20))].slice(-20)
        : [],
      referenceNumber: String(parsed.referenceNumber || "").trim().slice(0, 40),
      revision: Math.max(1, Math.floor(Number(parsed.revision) || 1)),
      baseAmount,
      days,
      adjustments,
      total: purchaseOrderTotal(baseAmount, adjustments, days),
      audit: Array.isArray(parsed.audit) ? parsed.audit.slice(-60) : [],
    };
  } catch {
    return null;
  }
...
export function stripPurchaseOrderFromNotes(notes: unknown) {
  const text = String(notes || "");
  const start = text.indexOf(PURCHASE_ORDER_NOTES_START);
  const end = text.indexOf(PURCHASE_ORDER_NOTES_END);
  if (start < 0 || end <= start) return text.trim();
  return `${text.slice(0, start)}${text.slice(end + PURCHASE_ORDER_NOTES_END.length)}`.trim();
}

export function mergePurchaseOrderIntoNotes(notes: unknown, purchaseOrder: ContractorPurchaseOrder | null) {
  const visible = stripPurchaseOrderFromNotes(notes);
  if (!purchaseOrder) return visible;
  const marker = `${PURCHASE_ORDER_NOTES_START}\n${JSON.stringify(purchaseOrder)}\n${PURCHASE_ORDER_NOTES_END}`;
  return [visible, marker].filter(Boolean).join("\n\n");
}
...
function matchingCrewPositionRate(crew: CrewRecord | undefined, roleName: string) {
  ...
  let bestScore = 0;
  for (const position of crew.positions) {
    const rate = rateNumber(position.rate);
    if (rate === null) continue;
    const score = roleMatchScore(position.role_name, roleName);
    if (score > bestScore) {
      bestScore = score;
      bestRate = rate;
    }
...
function roleMatchScore(rateRoleName: string | null | undefined, requestedRoleName: string | null | undefined) {
  ...
  // Exact role names must always win, but a Working Lead rate must never be
  // selected for a plain General AV / AVT sub-call through broad aliases.
...
function matchingMasterRate(masterRates: MasterRateRecord[], roleName: string, rateCity: string, masterRateId?: string | null) {
  ...
  // Never fall back to a stale stored master-rate ID when its role no longer
  // matches the sub-call. An old General AV ID must not price a later custom
  // role such as Show Prep/Post Show.
  return bestRate;
}
...
export function estimateAssignmentPay(options: {
  ...
  // Payroll is contractor pay, so the crew member's saved rate for the exact
  // position must win. The master crew-pay card is only the default when that
  // contractor has no matching position rate; hard-coded legacy rates are the
  // final fallback. Client billing rates are never used here.
  const assignmentOverride = rateNumber(options.fullDayRateOverride);
  const fullDayRate = assignmentOverride ?? crewFullDay ?? masterFull ?? fallbackFull ?? 0;
  const halfDayRate = halfDayFromFullDay(fullDayRate);
  const source = assignmentOverride
    ? "Show-specific crew rate override"
    : crewFullDay
    ? "Crew profile position rate"
    : masterFull
      ? `${masterRate?.city_name || "Default"} master crew-pay rate`
      : fallback
        ? "ELS legacy crew-pay fallback"
        : "No crew-pay rate found";
...
function estimateByBlock(options: { duration: number | null; fullDayRate: number; halfDayRate: number | null; source: string; blockType?: string | null; overtimeMultiplier?: number; doubletimeMultiplier?: number }): AssignmentPayEstimate {
  ...
  // Infer a half day only when no block was saved. If the event explicitly
  // says Full Day, Payroll must honor that selection even when the visible
  // call time is five hours or less.
  const shouldUseTimeBasedHalfDay = duration !== null && duration <= 5 && halfDayRate !== null && blockType === "";
