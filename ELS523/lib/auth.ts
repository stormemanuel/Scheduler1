...
function safeAuthText(value: unknown) {
...
export function createPersonalShortcutToken(userId: string) {
  const cleanUserId = safeAuthText(userId);
  if (!cleanUserId) return "";
  return `elsu_${base64UrlEncode(cleanUserId)}_${shortcutSignature(cleanUserId)}`;
}

export function resolvePersonalShortcutToken(token: unknown) {
  const raw = safeAuthText(token);
  const prefix = "elsu_";
  ...
  const encodedUserId = remainder.slice(0, separatorIndex);
  const given = remainder.slice(separatorIndex + 1);
  if (!given) return "";
...
export function normalizeRole(role: string | null | undefined): AppRole {
  ...
  const compact = value.replace(/[_-]+/g, " ").replace(/\s+/g, " ");
  if (value === "owner") return "owner";
  if (value === "admin") return "admin";
  if (compact === "coordinator" || compact === "labor coordinator") return "coordinator";
  if (value === "sales" || value === "salesman") return "salesman";
  return "viewer";
}

export function sharedCoordinatorUserIds(user: { user_metadata?: Record<string, unknown> | null } | null | undefined) {
  const value = user?.user_metadata?.els_shared_coordinator_user_ids;
...
function sanitizePageList(value: unknown, role: AppRole): AppPageKey[] {
  ...
  const resolved = pages.length ? pages : defaultPagesForRole(role);
  // Onboarding is an operational coordinator tool. Add it for existing coordinator
  // access rows too, so older saved page lists do not hide the coordinator tracker.
  if (role === "coordinator" && !resolved.includes("onboarding")) return [...resolved, "onboarding"];
  return resolved;
}
...
async function loadAccessSettings(
  ...
  let accessResult = await client
    ...
    .eq("user_id", userId)
    .maybeSingle();
  if (String(accessResult.error?.message || "").includes("can_edit_event_details")) {
    accessResult = await client
      .from("user_access_settings")
      .select("user_id, allowed_pages, restrict_events_to_owner, restrict_crew_to_owner, allowed_city_pool_ids")
      .eq("user_id", userId)
      .maybeSingle();
  }
  if (String(accessResult.error?.message || "").includes("allowed_city_pool_ids")) {
    accessResult = await client
      .from("user_access_settings")
      .select("user_id, allowed_pages, restrict_events_to_owner, restrict_crew_to_owner")
      .eq("user_id", userId)
      .maybeSingle();
  }
  ...
  if (!accessError && accessRow) {
    return {
      ...
      // Coordinator event isolation is mandatory. A stale/unchecked legacy
      // access row must never widen a coordinator to the full show list.
      restrict_events_to_owner: role === "coordinator" ? true : Boolean((accessRow as { restrict_events_to_owner?: boolean }).restrict_events_to_owner),
  ...
  if (actorRole === "owner" || actorRole === "admin") {
    ...
    if (requestedUserId && requestedUserId !== actorUser.id) {
      ...
      if (targetProfile && targetProfile.is_active !== false) {
        ...
        const targetAuthResult = admin ? await admin.auth.admin.getUserById(targetProfile.id) : null;
        const targetAuthMetadata = targetAuthResult?.data?.user?.user_metadata || {};
        user = {
          ...actorUser,
          id: targetProfile.id,
