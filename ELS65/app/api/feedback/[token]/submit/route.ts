...
const crewLeadQuestions = [
  ["onsite", "Overall on-site execution"],
  ["communication", "Communication with ELS and client"],
  ["crew_readiness", "Crew readiness and punctuality"],
  ["staff_competence", "Staff competence and ability"],
  ["teamwork", "Teamwork and attitude"],
] as const;
...
async function resolveClientContactId(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, link: LinkRow, body: Record<string, unknown>) {
  ...
  const { data, error } = await admin
    ...
    .insert({
      ...
      title: cleanText(body.respondent_title) || (link.form_kind === "area-manager" ? "Booth / Area Manager" : link.form_kind === "crew-lead" ? "Crew Lead" : "Project Manager"),
      email: respondentEmail,
      is_primary: false,
      is_onsite_contact: link.form_kind === "area-manager" || link.form_kind === "crew-lead",
...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  try {
    ...
    const questionPairs = effectiveLink.form_kind === "area-manager" ? areaManagerQuestions : effectiveLink.form_kind === "crew-lead" ? crewLeadQuestions : projectManagerQuestions;
