...
function DateArrowInput({
  ...
  const moveDate = (delta: number) => {
    ...
    const base = /^\d{4}-\d{2}-\d{2}$/.test(normalized) ? normalized : new Date().toISOString().slice(0, 10);
    onChange(addDaysToDateString(base, delta));
  };

  return (
    <div className="date-arrow-input" style={{ display: "grid", gridTemplateColumns: "44px minmax(0, 1fr) 44px", gap: 8, alignItems: "center" }}>
      <button type="button" className="ghost" disabled={disabled} aria-label="Previous day" title="Previous day" onClick={() => moveDate(-1)}>←</button>
      <input
        type="text"
        inputMode="numeric"
        value={dateInputDisplay(value)}
        placeholder={placeholder}
        disabled={disabled}
        onChange={(event) => onChange(event.target.value)}
        onBlur={(event) => onChange(normalizeDateEntry(event.currentTarget.value))}
      />
      <button type="button" className="ghost" disabled={disabled} aria-label="Next day" title="Next day" onClick={() => moveDate(1)}>→</button>
    </div>
  );
}
...
function EventsClientInner({
      ...
      <div key={call.id} className="card compact sub-call-card">
        <div className="row" style={{ alignItems: "flex-start" }}>
          <div>
            ...
            <div className="small muted">{call.area} • {call.role_name}</div>
            <div className="small muted">{[dayTypeLabel(call.day_type), walkawayLabel(call), call.location ? `Location: ${call.location}` : "", call.po_number ? `PO: ${call.po_number}` : "", call.message_rate ? `Message rate: $${call.message_rate}/hr` : ""].filter(Boolean).join(" • ")}</div>
          ...
          <div className="toolbar">
            ...
            {canEditEventDetails ? <button type="button" className="ghost" onClick={() => startEditCall(day.id, call)}>Edit</button> : null}
  ...
  function buildCrewListDocument(): ExportDocument | null {
    ...
    const boothSubCallSections = boothSections.map((section): ExportDocument["sections"][number] => {
      ...
      const rows = entries.flatMap(({ day, call }) => buildRows(call, day.labor_date));
      return {
    ...
    const daySeparatedSections: ExportDocument["sections"] = visibleLaborDays.map((day): ExportDocument["sections"][number] => {
      const dayCalls = displayCalls
        ...
        .map((item) => item.call)
        .sort((a, b) => a.start_time.localeCompare(b.start_time) || boothLabel(a.area).localeCompare(boothLabel(b.area)) || safeText(a.role_name).localeCompare(safeText(b.role_name)));
      ...
      const rows = dayCalls.flatMap((call) => {
        const callAssignments = getCallAssignments(call.id);
        const callLabel = `${boothLabel(call.area)} • ${call.role_name} • ${formatTimeRange(call)}${call.one_hour_walkaway ? " • 1 hr walkaway" : ""}${call.po_number ? ` • PO: ${call.po_number}` : ""} • ${call.crew_needed} needed`;

        if (!callAssignments.length) {
          return [["No crew assigned yet.", callLabel, formatTimeRange(call), call.role_name, "", "", ""]];
        }
      ...
      return {
        heading: `${day.labor_date}${day.label ? ` - ${day.label}` : ""}`,
        subheading: `${dayCalls.length} sub-call${dayCalls.length === 1 ? "" : "s"} • ${assignedTotal}/${neededTotal || 0} assigned`,
        accentColor: "#17313b",
  ...
  async function addCrewToCall(crewId: string) {
    ...
    const fullTargetCalls = targetCallIds.filter((subCallId) => {
      ...
      const maxCrew = Math.max(0, Number(targetCall.crew_needed || 0));
      if (!maxCrew) return false;
      const assignedCount = assignments.filter((assignment) => assignment.sub_call_id === subCallId).length;
              ...
              <div className="grid grid-3" style={{ marginTop: 12 }}>
                ...
                <label className="field">
                  ...
                  <div className="row" style={{ gap: 8, marginTop: 8 }}>
                    <DateArrowInput value={customTextScheduledDate} onChange={(value) => { setCustomTextScheduledDate(value); setCustomTextSendTimePreset(""); }} />
          ...
          <div className="grid grid-2" style={{ marginTop: 12 }}>
            ...
            {changeRequestDraft.request_type === "time_change" ? (
              ...
              <label className="field">
                ...
                <DateArrowInput value={changeRequestDraft.requested_labor_date} onChange={(value) => setChangeRequestDraft((current) => ({ ...current, requested_labor_date: value }))} />
              </label>
            )}
          </div>
            ...
            <div className="grid grid-2">
              ...
              <div className="grid grid-2">
                <label className="field">
                  ...
                  <DateArrowInput value={showForm.show_start} onChange={(value) => setShowForm((c) => ({ ...c, show_start: value }))} />
                </label>
                <label className="field">
                  <span>Show end</span>
                  <DateArrowInput value={showForm.show_end} onChange={(value) => setShowForm((c) => ({ ...c, show_end: value }))} />
                </label>
              </div>
            </div>
            <div className="card compact accent-card">
        ...
        {editorMode === "day" ? (
          <div className="grid" style={{ gap: 14, marginTop: 12 }}>
            <div className="grid grid-2">
              <label className="field">
                <span>Date</span>
                <DateArrowInput value={dayForm.labor_date} onChange={(value) => setDayForm((c) => ({ ...c, labor_date: value }))} />
              </label>
                      ...
                      <div key={day.id} className="card compact" style={{ borderTop: `4px solid ${accentColorForArea(dayAccentArea, areaAccentMap)}` }}>
                        <div className="row">
                          ...
                          <div className="toolbar">
                            ...
                            {canEditEventDetails ? <button type="button" className="ghost" onClick={() => startAddCall(day.id)}>Add Sub-Call</button> : null}
