...
function importErrorText(error: unknown, fallback: string) {
  if (error instanceof TypeError && /fetch/i.test(error.message)) {
    return "Import request could not reach the server. This usually means the PDF import function crashed or timed out. Try preview again; if it repeats, use the exact PDF name when reporting it.";
  }
...
export default function EventsClient({
  ...
  async function previewImportFile() {
    ...
    try {
      ...
      setMsg({ kind: "error", text: importErrorText(error, "Preview failed.") });
    } finally {
      setImporting(false);
    }
  }
  ...
  async function runImport() {
    ...
    try {
      ...
      setMsg({ kind: "error", text: importErrorText(error, "Import failed.") });
    } finally {
      setImporting(false);
    }
  }

  function exportCrewList(modeOverride?: EventDisplayMode) {
    if (!selectedShow) {
      setMsg({ kind: "error", text: "Select a show first." });
      return;
    }

    const exportMode = modeOverride ?? eventDisplayMode;

    const buildDayRows = (call: SubCallRecord) => {
      const callAssignments = getCallAssignments(call.id);
    ...
    const buildBoothAssignmentRows = (items: typeof displayCalls) => {
      const rows: string[] = [];
      let lastDate = "";

      for (const { day, call } of items) {
        if (day.labor_date !== lastDate) {
          rows.push(`<tr class="date-separator"><td colspan="5">${escapeHtml(day.labor_date)}</td></tr>`);
          lastDate = day.labor_date;
        }

        const callAssignments = getCallAssignments(call.id);
        if (!callAssignments.length) {
          rows.push(`<tr><td>${escapeHtml(day.labor_date)}</td><td class="empty">No crew assigned yet</td><td>${escapeHtml(formatTimeRange(call))}</td><td>${escapeHtml(call.role_name)}</td><td></td></tr>`);
          continue;
        }

        for (const { assignment, crew } of callAssignments) {
          rows.push(`<tr><td>${escapeHtml(day.labor_date)}</td><td>${escapeHtml(crew?.name || assignment.crew_id)}</td><td>${escapeHtml(formatTimeRange(call))}</td><td>${escapeHtml(call.role_name)}</td><td>${escapeHtml(crew ? formatPhone(crew.phone) : "")}</td></tr>`);
        }
      }
    ...
    const boothHtml = boothSections
      .map((section) => `
        <div class="booth-break">
          ...
          <div class="muted">${section.calls.length} sub-call${section.calls.length === 1 ? "" : "s"}</div>
        </div>
        <table class="booth-table">
          <thead><tr><th>Date</th><th>Name</th><th>Times</th><th>Position</th><th>Contact Number</th></tr></thead>
          <tbody>${buildBoothAssignmentRows(section.calls)}</tbody>
        </table>`)
      .join("");
...
<body>
  <div class="header">
    ...
    <div class="meta">
      ...
      <strong>View:</strong><span>${exportMode === "booth" ? "Booth-separated" : "Day-separated"}</span>
    </div>
    <div class="view-label">${exportMode === "booth" ? "Grouped by booth / area" : "Grouped by labor day"}</div>
  </div>
  ${exportMode === "booth" ? boothHtml || `<p class="empty">No booth/sub-call information found.</p>` : dayHtml || `<p class="empty">No labor days found.</p>`}
</body>
</html>`;
    ...
    if (!win) {
      ...
      a.href = url;
      a.download = `${selectedShow.name.replace(/[^a-z0-9]+/gi, "_")}_${exportMode}_crew_list.html`;
      a.click();
    }
            ...
            <div className="grid" style={{ gap: 16 }}>
              <div className="card compact">
                ...
                <div className="toolbar" style={{ marginTop: 12 }}>
                  ...
                  <button type="button" className="ghost" onClick={() => exportCrewList("day")}>Export Day Crew List</button>
                  <button type="button" className="ghost" onClick={() => exportCrewList("booth")}>Export Booth Crew List</button>
