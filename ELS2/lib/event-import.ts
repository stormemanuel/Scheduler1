...
function collapseSpacedPdfTokens(value: string) {
  let output = value;

  // Some PDFs extract normal text as spaced glyphs, for example:
  // "0 5 / 1 2 D a n F o x 8: 0 0 a m".
  // Collapse those back into normal words/dates/times before row parsing.
  output = output.replace(/\b(?:[A-Za-z]\s+){1,}[A-Za-z]\b/g, (match) => {
    ...
    for (const part of parts) {
      ...
      if (/^[A-Z]$/.test(part) && /[a-z]/.test(current)) {
        words.push(current);
        current = part;
      } else {
        current += part;
  ...
  output = output
    .replace(/\bBOO\s+TH\b/gi, "BOOTH")
    .replace(/\bP\s*O\s*(\d{2})\s+(\d{4})\b/gi, "PO$1$2")
    .replace(/\bP\s*O\s*(\d)\s+(\d)\s+(\d)\s+(\d)\s+(\d)\s+(\d)\b/gi, "PO$1$2$3$4$5$6")
    .replace(/\b(\d)\s+(\d)\s*\/\s*(\d)\s+(\d)(?:\s+(\d)\s+(\d)\s+(\d)\s+(\d))?\b/g, (_m, a, b, c, d, e, f, g, h) => `${a}${b}/${c}${d}${e ? `/${e}${f}${g}${h}` : ""}`)
    .replace(/\b(\d{1,2})\s*:\s*(\d)\s+(\d)\s*([ap])\s*m\b/gi, "$1:$2$3 $4m")
    .replace(/\b(\d{1,2})\s*:\s*(\d{2})\s*([ap])\s*m\b/gi, "$1:$2 $3m")
    .replace(/\(\s*(\d)\s+(\d)\s+(\d)\s*\)/g, "($1$2$3)")
    .replace(/\b(\d)\s+(\d)\s+(\d)\s*-\s*(\d)\s+(\d)\s+(\d)\s+(\d)\b/g, "$1$2$3-$4$5$6$7")
    .replace(/\b(\d)\s+(\d)\s+(\d)\s+(\d)\b/g, "$1$2$3$4")
    .replace(/\b([A-Z]{2,})\s+(\d)\s+(\d)\b/g, "$1$2$3");

  return output;
}

function cleanPdfArtifacts(value: string) {
  const withKnownGlyphs = value
    .replace(/\uE092/g, ":")
    .replace(/\uE088/g, "-")
    .replace(/\uE081/g, "(")
    .replace(/\uE082/g, ")");

  return collapseSpacedPdfTokens(withKnownGlyphs)
...
function stripPdfHeaderGlue(line: string) {
  ...
  // Keep standalone table-header lines so the parser can use them as section boundaries.
  // Earlier builds stripped these to blank, which made page-break booth headers such as
  // ESPERION/GENENTECH stay queued and caused rows to fall back under the prior booth.
  if (isTableHeaderLine(clean) || /^Date$/i.test(clean) || /^Dates$/i.test(clean) || /^Name$/i.test(clean) || /^Times?$/i.test(clean) || /^Position$/i.test(clean) || /^Contact\s+Number$/i.test(clean) || /^Contact$/i.test(clean) || /^Number$/i.test(clean)) {
    return clean;
  }
...
function canonicalizeImportedPosition(value: string) {
  ...
  // These keep PDF wording variations from creating $0 roles like "LED", "Assist", or "CF AVT".
  if (/\bled\b/.test(key) && /\bstagehand\b/.test(key)) return "LED Assist";
  if (/\bled\b/.test(key) && /\bassist/.test(key)) return "LED Assist";
  if (key === "led" || key === "led tech" || key === "led technician" || key === "led assist") return "LED Assist";

  if (/\blighting\b/.test(key) && /\bassist/.test(key)) return "Lighting Assist";
  ...
  if (key === "assist") return "Lighting Assist";
...
function choosePosition(afterEnd: string, betweenTimes: string) {
  ...
  // or "1:00 PM | Lighting" then "11:00 PM Assist".
  if (/(?:\bled\b|\blighting\b|\baudio\b|\bvideo\b|\bcf\b|\bclient\s+facing\b|\bgav\b|\bavt\b)/i.test(betweenRaw) && /(assist|tech|visual|av|stagehand|operator)/i.test(afterEnd)) {
...
function parseWeekdayFragment(line: string) {
  const match = compactSpaces(line).match(/^(mon(?:day)?|tues(?:day)?|tue(?:sday)?|wednes(?:day)?|wed(?:nesday)?|thurs(?:day)?|thu(?:rsday)?|fri(?:day)?|satur(?:day)?|sat(?:urday)?|sun(?:day)?)[,\s]*(.*)$/i);
  if (!match) return null;
...
function splitWeekdayDateContinuation(line: string, nextLine: string | undefined) {
  ...
  // IMPORTANT: handle "April 28," before the month-only case.
  // The old parser treated "April 28," as month="April" + rest="28," and then
  // parsed the next line "2026" as day=20/year=26, creating fake dates like 2026-04-20.
  ...
  if (monthDay) {
    ...
    if (!year && nextLine) {
      const next = compactSpaces(nextLine);
      const yearLine = next.match(/^(\d{4})(?:\s+(.*))?$/);
      if (yearLine) {
        year = yearLine[1];
  ...
  if (monthOnly && nextLine && !/^\d/.test(monthOnly[2] || "")) {
    const next = compactSpaces(nextLine);
    const dayYear = next.match(/^(\d{1,2})(?:,?\s*(\d{4}))?,?(?:\s+(.*))?$/);
    if (dayYear) {
      return {
        consumed: 2,
...
function repairSplitDateLines(lines: string[]) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (weekday && !monthStartInfo(line)) {
      ...
      // Layout extraction can put the weekday at the beginning, the worker details next,
      // and the actual month/day near the end of the same visual line, with the year
      // alone on the next line, for example:
      //   Saturday Megan Willis PO172371 2:00:00 PM LED Assist (256) 583-8022, May 2, 11:59:00 PM
      //   2026
      // Reorder that into a normal date-starting row so it does not get swallowed into
      // the prior person's role or skipped entirely.
      if (weekday.rest && next) {
        const embeddedMonthDate = weekday.rest.match(/^(.*?),\s*((?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t|tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\s+\d{1,2}),?\s*(.*)$/i);
        const yearOnly = compactSpaces(next).match(/^(20\d{2})(?:\s+(.*))?$/);
        if (embeddedMonthDate && yearOnly) {
          const rawDate = `${embeddedMonthDate[2]}, ${yearOnly[1]}`;
          repaired.push(mergeSplitDateRow(rawDate, embeddedMonthDate[1], compactSpaces(`${embeddedMonthDate[3] || ""} ${yearOnly[2] || ""}`)));
          index += 1;
          continue;
        }
      }

      // Handles rows where PDF extraction already joined the split weekday pieces,
      // for example "Wednesday, April" + "29, 2026".
      if (weekday.rest) {
        const restContinuation = splitWeekdayDateContinuation(weekday.rest, next);
        if (restContinuation) {
          repaired.push(mergeSplitDateRow(restContinuation.rawDate, "", restContinuation.rest));
          index += Math.max(0, restContinuation.consumed - 1);
          continue;
        }
      }
...
type FlexibleDatePrefix = {
  rawDate: string;
  rest: string;
  consumed: number;
};

function isRoleContinuationLine(line: string) {
  const clean = compactSpaces(line);
  if (!clean) return false;
  if (isPageBreakLine(clean) || isTableHeaderLine(clean) || isMetadataLine(clean) || isLikelySectionLine(clean)) return false;
  if (dateStartFromLine(clean, inferDefaultYear([clean]))) return false;
  if (lineHasPhone(clean)) return false;
  return /^(?:led|lighting|audio|video|client\s*facing|cf|gav|avt|assist|assistant|tech|technician|stagehand|operator|general\s+av|visual|assist\/lighting)(?:\b|\s|\/)/i.test(clean);
}

function parseFlexibleDatePrefix(lines: string[], index: number, fallbackYear: number): FlexibleDatePrefix | null {
  const line = compactSpaces(lines[index] || "");
  const next = compactSpaces(lines[index + 1] || "");
  const third = compactSpaces(lines[index + 2] || "");
  if (!line || isPageBreakLine(line)) return null;

  const direct = dateStartFromLine(line, fallbackYear);
  if (direct) return { rawDate: direct.rawDate, rest: direct.rest, consumed: 1 };

  let match = line.match(/^(mon(?:day)?|tue(?:sday)?|tues|wed(?:nesday)?|wednes|thu(?:rsday)?|thurs?|fri(?:day)?|sat(?:urday)?|satur|sun(?:day)?)[,\s]+((?:jan(?:uary)?|feb(?:ruary)?|mar(?:ch)?|apr(?:il)?|may|jun(?:e)?|jul(?:y)?|aug(?:ust)?|sep(?:t|tember)?|oct(?:ober)?|nov(?:ember)?|dec(?:ember)?)\s+\d{1,2}),?\s*(.*)$/i);
  if (match) {
    const maybeYear = next.match(/^(20\d{2})(?:\s+(.*))?$/i);
    if (maybeYear) {
      return {
        rawDate: `${match[2]}, ${maybeYear[1]}`,
        rest: compactSpaces(`${match[3] || ""} ${maybeYear[2] || ""}`),
        consumed: 2,
      };
    }
    return { rawDate: match[2], rest: compactSpaces(match[3] || ""), consumed: 1 };
  }

  match = line.match(/^(mon(?:day)?|tue(?:sday)?|tues|wed(?:nesday)?|wednes|thu(?:rsday)?|thurs?|fri(?:day)?|sat(?:urday)?|satur|sun(?:day)?)[,\s]*(.*)$/i);
  if (match) {
    const lineRest = compactSpaces(match[2] || "");

    const lineRestDate = splitWeekdayDateContinuation(lineRest, next);
    if (lineRestDate) {
      return { rawDate: lineRestDate.rawDate, rest: lineRestDate.rest, consumed: lineRestDate.consumed };
    }

    const nextDate = splitWeekdayDateContinuation(next, third);
    if (nextDate) {
      return { rawDate: nextDate.rawDate, rest: compactSpaces(`${lineRest} ${nextDate.rest}`), consumed: 1 + nextDate.consumed };
    }

    const splitDay = splitDayMonthInfo(next, third);
    if (splitDay) {
      return { rawDate: splitDay.rawDate, rest: compactSpaces(`${lineRest} ${splitDay.rest}`), consumed: 3 };
    }
  }
...
function flexibleRecordLooksComplete(parts: string[]) {
  const record = compactSpaces(parts.join(" "));
  return lineHasPhone(record) && timeMatches(record).length >= 2;
}

function parseFlexibleCrewRecord(record: string, area: string, fallbackYear: number): ImportedCrewRow | null {
  const cleanRecord = removeTrailingOnCallNotes(stripPdfHeaderGlue(record));
  if (!cleanRecord || isNonLaborNote(cleanRecord)) return null;

  const dateInfo = dateStartFromLine(cleanRecord, fallbackYear);
  if (!dateInfo) return null;

  const { withoutPhones, phone } = removePhoneNumbers(dateInfo.rest);
  ...
  const firstTime = times[0];
  const secondTime = times[1];
  const beforeFirstTime = compactSpaces(withoutPhones.slice(0, firstTime.index));
  const betweenTimes = compactSpaces(withoutPhones.slice(firstTime.index + firstTime.text.length, secondTime.index));
  const afterSecondTime = compactSpaces(withoutPhones.slice(secondTime.index + secondTime.text.length));

  let nameBlock = beforeFirstTime;
  const poInName = nameBlock.match(/^(.*?)\s+PO\s*\d{4,}\b.*$/i);
  if (poInName) nameBlock = poInName[1];
  const nameText = cleanImportedName(nameBlock);
  if (!nameText || /^(name|dates?|date|po)$/i.test(nameText)) return null;
  if (/\bon\s*call\b/i.test(nameText)) return null;

  let positionText = compactSpaces(`${betweenTimes} ${afterSecondTime}`)
    .replace(/^\|+/, " ")
    .replace(/\|+/g, " ")
    .replace(/\b20\d{2}\b/g, " ")
    .replace(/\bPO\s*\d+\b/gi, " ");

  const nextDateInPosition = positionText.search(new RegExp(`(?:${MONTH_DATE_PATTERN}|${NUMERIC_DATE_PATTERN})`, "i"));
  if (nextDateInPosition > 0) positionText = positionText.slice(0, nextDateInPosition);

  const position = cleanPosition(positionText);
  if (!position || position === "Imported Crew" || /\bon\s*call\b/i.test(position)) return null;
...
function parseFlexiblePdfLaborRows(lines: string[], fallbackYear: number): ImportedCrewRow[] {
  const rows: ImportedCrewRow[] = [];
  let currentSection = "Imported Call";
  let pendingSection = "";
  let continuationSectionBeforePending = "";
  let afterPageBreak = false;
  let lastParsedDate = "";

  const useSection = (value: string) => {
    const clean = compactSpaces(value);
    if (!clean) return;
    currentSection = clean;
    pendingSection = "";
    continuationSectionBeforePending = "";
  };

  const appendToActiveSection = (value: string) => {
    const clean = compactSpaces(value);
    if (!clean) return;
    if (pendingSection) pendingSection = compactSpaces(`${pendingSection} ${clean}`);
    else currentSection = compactSpaces(`${currentSection} ${clean}`);
  };

  let index = 0;
  while (index < lines.length) {
    const line = stripPdfHeaderGlue(lines[index]);
    if (!line) { index += 1; continue; }

    if (isPageBreakLine(line)) { afterPageBreak = true; index += 1; continue; }

    if (isTableHeaderLine(line)) {
      if (pendingSection) useSection(pendingSection);
      afterPageBreak = false;
      index += 1;
      continue;
    }

    if (isNonLaborNote(line) || isMetadataLine(line)) { index += 1; continue; }

    if (isLikelySectionLine(line)) {
      if (isSectionContinuation(line)) {
        appendToActiveSection(line);
      } else if (afterPageBreak && currentSection !== "Imported Call" && !pendingSection) {
        pendingSection = compactSpaces(line);
        continuationSectionBeforePending = currentSection;
      } else {
        useSection(line);
      }
      index += 1;
    ...
    const datePrefix = parseFlexibleDatePrefix(lines, index, fallbackYear);
    if (!datePrefix) { index += 1; continue; }

    const recordStartedAfterPageBreak = afterPageBreak;
    const parts = [`${datePrefix.rawDate} ${datePrefix.rest}`];
    index += datePrefix.consumed;

    while (index < lines.length) {
      const next = stripPdfHeaderGlue(lines[index]);
      if (!next) { index += 1; continue; }
      if (isPageBreakLine(next)) { afterPageBreak = true; index += 1; continue; }

      const complete = flexibleRecordLooksComplete(parts);
      const nextStartsDate = Boolean(parseFlexibleDatePrefix(lines, index, fallbackYear));
      const nextIsHardBoundary = isTableHeaderLine(next) || isMetadataLine(next) || isLikelySectionLine(next) || isNonLaborNote(next);

      if (complete && (nextStartsDate || nextIsHardBoundary)) break;
      if (complete && !isRoleContinuationLine(next) && !lineHasTime(next) && !lineHasPhone(next)) break;
      if (!complete && nextIsHardBoundary) break;
    ...
    if (pendingSection) {
      const joined = compactSpaces(parts.join(" "));
      const pendingPos = extractPoNumbers(pendingSection);
      const rowPos = extractPoNumbers(joined);
      const rowDate = parseFlexibleDatePrefix([joined], 0, fallbackYear)?.rawDate || "";
      const rowIsoDate = rowDate ? parseImportDate(rowDate, fallbackYear) : "";
      const dateMovedBackAfterContinuation = Boolean(rowIsoDate && lastParsedDate && rowIsoDate < lastParsedDate && !afterPageBreak);
      const rowBelongsToPendingSection = Boolean(
        (pendingPos.length && rowPos.some((po) => pendingPos.includes(po))) ||
        dateMovedBackAfterContinuation
      );
      if (rowBelongsToPendingSection) {
        useSection(pendingSection);
        rowSection = currentSection;
      } else if (continuationSectionBeforePending) {
        rowSection = continuationSectionBeforePending;
      }
    }

    const row = parseFlexibleCrewRecord(compactSpaces(parts.join(" ")), rowSection, fallbackYear);
    if (row) {
      rows.push(row);
      lastParsedDate = row.date;
    }
    if (recordStartedAfterPageBreak) afterPageBreak = false;
  }
...
function isLooseRowDetailLine(line: string, fallbackYear: number) {
  const clean = stripPdfHeaderGlue(line);
  if (!clean) return false;
  if (isPageBreakLine(clean) || isTableHeaderLine(clean) || isMetadataLine(clean) || isLikelySectionLine(clean) || isNonLaborNote(clean)) return false;
  if (dateStartFromLine(clean, fallbackYear)) return false;
  return lineHasPhone(clean) && timeMatches(clean).length >= 1;
}

function lineLooksLikeIncompleteDateRow(line: string, fallbackYear: number) {
  const clean = stripPdfHeaderGlue(line);
  const dateInfo = dateStartFromLine(clean, fallbackYear);
  if (!dateInfo) return false;
  // Visual PDF extraction can place the time/role/phone line above the date/name line.
  // In that case the date line has the worker name but not the full row details.
  return timeMatches(clean).length < 2 || !lineHasPhone(clean);
}

function mergeLeadingDetailLinesIntoDateRows(lines: string[], fallbackYear: number) {
  const repaired: string[] = [];
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      next &&
      isLooseRowDetailLine(line, fallbackYear) &&
      lineLooksLikeIncompleteDateRow(next, fallbackYear)
    ) {
      const nextDate = dateStartFromLine(next, fallbackYear);
      if (nextDate) {
        repaired.push(compactSpaces(`${nextDate.rawDate} ${nextDate.rest} ${line}`));
        index += 1;
        continue;
      }
    }
...
function preprocessPdfLines(text: string) {
  ...
  flushPage();
  return mergeLeadingDetailLinesIntoDateRows(repaired, inferDefaultYear(repaired));
}

function parsePdfTextLegacy(text: string, overrides: Partial<ImportedEventData["show"]>): ImportedEventData {
  ...
  const useFlexibleRecordParser = /vs\s+exchange/i.test(showName) || /vs\s+exchange/i.test(text);
  const rows: ImportedCrewRow[] = useFlexibleRecordParser ? parseFlexiblePdfLaborRows(lines, fallbackYear) : [];

  if (!rows.length) {
  let currentSection = "Imported Call";
  const sectionQueue: string[] = [];
  const sectionByPo = new Map<string, string>();
  let pendingRow = "";
  let rowBlockActive = false;
  let pageContinuationMode = false;
  let deferQueuedSectionUntilMatchingPo = false;
  let forceActivateQueuedSectionOnNextDateRow = false;

  const registerSectionPoNumbers = (section: string) => {
    const clean = compactSpaces(section);
    for (const po of extractPoNumbers(clean)) sectionByPo.set(po, clean);
  };

  const queueSection = (line: string) => {
    const clean = compactSpaces(line);
    if (!clean) return;
    const lastQueued = sectionQueue[sectionQueue.length - 1] || "";
    if (lastQueued && /[-/]\s*$/.test(lastQueued)) {
      sectionQueue[sectionQueue.length - 1] = compactSpaces(`${lastQueued} ${clean}`);
      registerSectionPoNumbers(sectionQueue[sectionQueue.length - 1]);
      return;
    }
    sectionQueue.push(clean);
    registerSectionPoNumbers(clean);
  };
  ...
  const activateQueuedSection = () => {
    ...
    if (next) {
      currentSection = next;
      registerSectionPoNumbers(next);
    }
    pageContinuationMode = false;
    deferQueuedSectionUntilMatchingPo = false;
    forceActivateQueuedSectionOnNextDateRow = false;
  };
  ...
  const shouldActivateQueuedSectionForDateRow = (line: string) => {
    ...
    if (forceActivateQueuedSectionOnNextDateRow) return true;
    ...
    // If a normal section header appears after prior rows on the same page, the next
    // date row should belong to that new section. The only time we delay activation
    // is page-continuation mode above, where some PDFs place a next-booth header
    // before rows that still belong to the prior booth.
    return true;
  };
  ...
  const appendSectionContinuation = (line: string) => {
    ...
    if (sectionQueue.length) {
      ...
      registerSectionPoNumbers(currentSection);
    } else {
      queueSection(clean);
    }
  };

  const areaForPendingRow = () => {
    const rowPos = extractPoNumbers(pendingRow);
    for (const po of rowPos) {
      const mappedSection = sectionByPo.get(po);
  ...
  const finishPendingRow = () => {
    ...
    const queued = queuedSection();
    const rowPos = extractPoNumbers(pendingRow);
    const mappedSection = rowPos.map((po) => sectionByPo.get(po)).find(Boolean) || "";

    if (queued && mappedSection && normalize(queued) === normalize(mappedSection)) {
      // PO-column tables often have date on one line and the PO later in the continued row.
      // Wait until the whole row is assembled, then move to the section whose PO matches.
      activateQueuedSection();
    } else if (queued && deferQueuedSectionUntilMatchingPo && rowMatchesSectionPo(pendingRow, queued)) {
      activateQueuedSection();
    } else if (queued && deferQueuedSectionUntilMatchingPo && rowPos.length) {
      // We saw a PO-coded continuation row that did NOT belong to the queued section.
      // Keep that row under the previous section, then let the next date row activate the queued section.
      pageContinuationMode = false;
      deferQueuedSectionUntilMatchingPo = false;
      forceActivateQueuedSectionOnNextDateRow = true;
    }

    const parsed = parseCrewCandidateRow(pendingRow, areaForPendingRow(), fallbackYear);
    if (parsed) {
      rows.push(parsed);
      rowBlockActive = true;
    }
  ...
  const rangeValidatedRows = showStart && showEnd
    ? dedupedRows.filter((row) => row.date >= showStart && row.date <= showEnd)
    : dedupedRows;

  if (showStart && showEnd && rangeValidatedRows.length < dedupedRows.length) {
    const skippedDates = [...new Set(dedupedRows.filter((row) => row.date < showStart || row.date > showEnd).map((row) => row.date))].join(", ");
    if (skippedDates) noteBits.push(`Skipped out-of-range imported date(s): ${skippedDates}`);
  }

  if (!rangeValidatedRows.length) {
  ...
  const dates = rangeValidatedRows.map((row) => row.date).sort();
  const show = {
  ...
  return { show, rows: rangeValidatedRows, sourceType: "pdf" };
}


type ParsedPdfCandidate = ImportedEventData & {
  _parser?: string;
  _score?: number;
};

function splitPdfTextVariants(text: string) {
  const normalized = text.replace(/\r\n/g, "\n").replace(/\r/g, "\n");
  const variants: Array<{ name: string; text: string }> = [];

  const markerRegex = /ELS_IMPORT_VARIANT_([A-Z_]+)/g;
  const markerMatches = [...normalized.matchAll(markerRegex)];
  if (markerMatches.length) {
    for (let index = 0; index < markerMatches.length; index += 1) {
      ...
      if (body) variants.push({ name: match[1].toLowerCase(), text: body });
    }
  }

  if (!variants.length) variants.push({ name: "single", text: normalized });
  variants.push({ name: "combined", text: normalized.replace(/ELS_IMPORT_VARIANT_[A-Z_]+/g, "\n") });

  const seen = new Set<string>();
  return variants.filter((variant) => {
    const key = variant.text.replace(/\s+/g, " ").trim().slice(0, 5000);
...
function extractPdfMetadataFromLines(lines: string[], overrides: Partial<ImportedEventData["show"]>) {
  let showName = overrides.name || "Imported Event";
  let client = overrides.client || "";
  let venue = overrides.venue || "";
  let location = "";
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (/labor\s+call\s+list/i.test(line) && showName === "Imported Event") {
      const next = lines[index + 1];
      if (next && !isTableHeaderLine(next) && !isMetadataLine(next) && !isPageBreakLine(next)) showName = next;
      continue;
    }
    if (/^event\s*name\s*:/i.test(line)) {
      showName = compactSpaces(line.replace(/^event\s*name\s*:/i, "")) || showName;
  ...
  return { showName, client, venue, location, rateCity, showStart, showEnd, noteBits, fallbackYear };
}

function removeSectionFromQueue(queue: string[], index: number) {
  const [value] = queue.splice(index, 1);
  return value || "";
}

function findQueuedSectionForRow(queue: string[], row: string) {
  const rowPos = extractPoNumbers(row);
  if (!rowPos.length) return -1;
  return queue.findIndex((section) => {
    const sectionPos = extractPoNumbers(section);
    return Boolean(sectionPos.length && rowPos.some((po) => sectionPos.includes(po)));
  });
}

function betterAreaName(area: string) {
  return compactSpaces(area)
    .replace(/\s*[-•]\s*$/g, "")
    .replace(/\(\s*PO\s*(\d+)\s*\)/gi, "PO$1")
    .replace(/\s+/g, " ") || "Imported Call";
}

function parseTableDrivenPdfRows(lines: string[], fallbackYear: number) {
  const rows: ImportedCrewRow[] = [];
  let currentSection = "Imported Call";
  const sectionQueue: string[] = [];
  let pendingRow = "";
  ...
  let afterPageBreak = false;
  let rowsSincePageBreak = 0;
  let rowsForCurrentSection = 0;

  const queueSection = (line: string) => {
    const clean = betterAreaName(line);
    if (!clean) return;
    if (isSectionContinuation(clean)) {
      if (sectionQueue.length) sectionQueue[sectionQueue.length - 1] = betterAreaName(`${sectionQueue[sectionQueue.length - 1]} ${clean}`);
      else currentSection = betterAreaName(`${currentSection} ${clean}`);
      return;
    }
    const lastQueued = sectionQueue[sectionQueue.length - 1] || "";
    if (lastQueued && (isSectionContinuation(clean) || /^\(?\s*PO\d+/i.test(clean))) {
      sectionQueue[sectionQueue.length - 1] = betterAreaName(`${lastQueued} ${clean}`);
    } else {
      sectionQueue.push(clean);
    }
  };
  ...
  const activateSection = (section: string) => {
    if (section) currentSection = betterAreaName(section);
    tableActive = true;
    afterPageBreak = false;
    rowsSincePageBreak = 0;
    rowsForCurrentSection = 0;
  };

  const activateNextQueuedSection = () => {
    activateSection(sectionQueue.shift() || currentSection);
  };

  const finishPendingRow = () => {
    if (!pendingRow) return;
    const rowSectionIndex = findQueuedSectionForRow(sectionQueue, pendingRow);
    const area = rowSectionIndex >= 0 ? removeSectionFromQueue(sectionQueue, rowSectionIndex) : currentSection;
    const parsed = parseCrewCandidateRow(pendingRow, area, fallbackYear) || parseFlexibleCrewRecord(pendingRow, area, fallbackYear);
    if (parsed) {
      rows.push(parsed);
      rowsForCurrentSection += 1;
      if (afterPageBreak) rowsSincePageBreak += 1;
    }
    pendingRow = "";
  };

  const shouldConsumeQueuedSectionForDateRow = (line: string) => {
    if (!sectionQueue.length) return false;
    if (tableActive) return false;
    if (findQueuedSectionForRow(sectionQueue, line) >= 0) return true;

    // Page-continuation guard: when a page starts with multiple booth headers and then
    // a lone date row before the next table header, that first row is usually the tail
    // of the prior booth. ADA and VS both do this.
    if (afterPageBreak && sectionQueue.length > 1 && rowsForCurrentSection > 0 && rowsSincePageBreak === 0) return false;
  ...
  for (const rawLine of lines) {
    ...
    if (isTableHeaderLine(line)) {
      finishPendingRow();
      if (sectionQueue.length) activateNextQueuedSection();
      else tableActive = true;
      continue;
    }
    ...
    if (dateInfo) {
      finishPendingRow();
      if (shouldConsumeQueuedSectionForDateRow(line)) {
        const matchIndex = findQueuedSectionForRow(sectionQueue, line);
        if (matchIndex >= 0) activateSection(removeSectionFromQueue(sectionQueue, matchIndex));
        else activateNextQueuedSection();
      }
      pendingRow = line;
      continue;
    }
    ...
    if (pendingRow) {
      if (isLikelySectionLine(line) || isMetadataLine(line) || isTableHeaderLine(line)) {
        finishPendingRow();
      } else {
        pendingRow = compactSpaces(`${pendingRow} ${line}`);
        continue;
  ...
  for (const row of rows) {
    const key = [row.date, normalize(row.name), row.startTime, row.endTime, normalize(row.position), normalize(row.phone), normalize(row.area)].join("|");
    if (!uniqueRows.has(key)) uniqueRows.set(key, { ...row, area: betterAreaName(row.area) });
  }
  return [...uniqueRows.values()];
}

function buildPdfEventFromRows(
  rows: ImportedCrewRow[],
  lines: string[],
  overrides: Partial<ImportedEventData["show"]>,
  parserName: string
): ParsedPdfCandidate {
  const meta = extractPdfMetadataFromLines(lines, overrides);
  const uniqueRows = new Map<string, ImportedCrewRow>();
  for (const row of rows) {
    const key = [row.date, normalize(row.name), row.startTime, row.endTime, normalize(row.position), normalize(row.phone), normalize(row.area)].join("|");
    if (!uniqueRows.has(key)) uniqueRows.set(key, row);
  ...
  const dedupedRows = [...uniqueRows.values()];
  const rangeValidatedRows = meta.showStart && meta.showEnd
    ? dedupedRows.filter((row) => row.date >= meta.showStart && row.date <= meta.showEnd)
    : dedupedRows;

  const dates = rangeValidatedRows.map((row) => row.date).sort();
  const show = {
    name: compactSpaces(meta.showName),
    client: compactSpaces(meta.client),
    venue: compactSpaces([meta.venue, meta.location].filter(Boolean).join(" • ")),
    rate_city: compactSpaces(meta.rateCity || "Default"),
    show_start: meta.showStart || dates[0] || "",
    show_end: meta.showEnd || dates[dates.length - 1] || "",
    notes: [overrides.notes, ...meta.noteBits, `PDF parser: ${parserName}`].filter(Boolean).map((note) => compactSpaces(String(note))).join("\n"),
  };
  return { show, rows: rangeValidatedRows, sourceType: "pdf", _parser: parserName };
}

function scorePdfCandidate(candidate: ImportedEventData) {
  const areas = new Set(candidate.rows.map((row) => normalize(row.area)).filter(Boolean));
  const dates = new Set(candidate.rows.map((row) => row.date).filter(Boolean));
  const names = new Set(candidate.rows.map((row) => normalize(row.name)).filter(Boolean));
  const malformedPenalty = candidate.rows.reduce((sum, row) => {
    const joined = normalize(`${row.area} ${row.name} ${row.position}`);
    let penalty = 0;
    if (!row.startTime || !row.endTime || !row.phone) penalty += 10;
    if (/\b(?:paid|confirmed|contact|number|date|times)\b/.test(joined)) penalty += 15;
    if (/\b\d{4}\b/.test(normalize(row.name))) penalty += 15;
    if (normalize(row.position).split(" ").length > 5) penalty += 12;
    return sum + penalty;
  }, 0);

  return candidate.rows.length * 100 + areas.size * 35 + dates.size * 15 + names.size * 5 - malformedPenalty;
}

function parsePdfTextCandidate(text: string, overrides: Partial<ImportedEventData["show"]>, parserName: string): ParsedPdfCandidate[] {
  const lines = preprocessPdfLines(text);
  const fallbackYear = inferDefaultYear(lines);
  const candidates: ParsedPdfCandidate[] = [];

  const tableRows = parseTableDrivenPdfRows(lines, fallbackYear);
  if (tableRows.length) candidates.push(buildPdfEventFromRows(tableRows, lines, overrides, `${parserName}:table`));

  const flexibleRows = parseFlexiblePdfLaborRows(lines, fallbackYear);
  if (flexibleRows.length) candidates.push(buildPdfEventFromRows(flexibleRows, lines, overrides, `${parserName}:flexible`));

  return candidates;
}

function parsePdfText(text: string, overrides: Partial<ImportedEventData["show"]>): ImportedEventData {
  const variants = splitPdfTextVariants(text);
  const candidates: ParsedPdfCandidate[] = [];
  const errors: string[] = [];

  for (const variant of variants) {
    try {
      candidates.push(...parsePdfTextCandidate(variant.text, overrides, variant.name));
    } catch (error) {
      errors.push(`${variant.name}: ${error instanceof Error ? error.message : String(error)}`);
    }

    try {
      const legacy = parsePdfTextLegacy(variant.text, overrides) as ParsedPdfCandidate;
      legacy._parser = `${variant.name}:legacy`;
      candidates.push(legacy);
    } catch (error) {
      errors.push(`${variant.name}:legacy: ${error instanceof Error ? error.message : String(error)}`);
    }
  }

  const valid = candidates.filter((candidate) => candidate.rows.length && candidate.show.show_start && candidate.show.show_end);
  if (!valid.length) {
    const lines = preprocessPdfLines(text);
    const fallbackYear = inferDefaultYear(lines);
    const dateLike = lines.filter((line) => dateStartFromLine(line, fallbackYear)).slice(0, 6).join(" | ");
    throw new Error(
      dateLike
        ? `PDF import found date-looking lines but could not split them into clean crew rows. First date lines: ${dateLike}${errors.length ? ` Error detail: ${errors.slice(0, 3).join("; ")}` : ""}`
        : `PDF import could not find labor call rows. It may be scanned/image-only or the table extraction failed.${errors.length ? ` Error detail: ${errors.slice(0, 3).join("; ")}` : ""}`
    );
  }

  valid.forEach((candidate) => { candidate._score = scorePdfCandidate(candidate); });
  valid.sort((a, b) => (b._score || 0) - (a._score || 0));
  const best = valid[0];
  return { show: best.show, rows: best.rows, sourceType: "pdf" };
}
