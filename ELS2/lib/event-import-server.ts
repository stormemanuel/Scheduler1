...
export async function readImportFileText(file: File) {
  ...
  if (lower.endsWith(".pdf")) {
    ...
    // Read the PDF two ways. Some client PDFs parse better in natural text order,
    // while others parse better when rebuilt visually by x/y position. The event
    // importer scores both variants and picks the cleanest one instead of failing
    // on whichever extraction happens to be worse.
    const defaultParsed = await pdfParse(buffer);

    let pageNumber = 0;
    const visualParsed = await pdfParse(buffer, {
      pagerender: async (pageData: { getTextContent: (options?: Record<string, boolean>) => Promise<{ items: PdfTextItem[] }> }) => {
        pageNumber += 1;
        const pageText = await renderPdfPageVisually(pageData);
        return `
ELS_PDF_PAGE_BREAK_${pageNumber}
${pageText}`;
      },
    } as any);

    return [
      "ELS_IMPORT_VARIANT_DEFAULT",
      defaultParsed.text || "",
      "ELS_IMPORT_VARIANT_VISUAL",
      visualParsed.text || "",
    ].join("\n");
  }
