...
function EventsClientInner({
  ...
  async function queueManualDayReminderNow() {
    ...
    if (manualReminderDayKey === "daily_after_first_day" && typeof window !== "undefined") {
      const ok = window.confirm("Daily reminder after first day normally begins after a crew member's first scheduled labor day. Queue it manually for the selected day anyway?");
      if (!ok) return;
    }
    const manualSettings = { ...automationDraft, show_id: selectedShow.id, enabled: true, send_schedule: true };
  ...
  function buildCrewListDocument(): ExportDocument | null {
    ...
    const buildRows = (call: SubCallRecord, dayLabel?: string) => {
      ...
      const referenceNumber = subCallReferenceNumber(call);
      if (!callAssignments.length) return [["No crew assigned yet.", datedTimeLabel, referenceNumber, call.role_name, "", "", ""]];
    ...
    const boothSubCallSections = boothSections.map((section): ExportDocument["sections"][number] => {
      ...
      const sortedRowItems = entries.flatMap(({ day, call }) => {
        ...
        if (!callAssignments.length) {
          return [{
            ...
            row: ["No crew assigned yet.", `${currentDate} • ${formatTimeRange(call)}`, subCallReferenceNumber(call), call.role_name, "", "", ""] as ExportCell[],
      ...
      return {
        ...
        accentColor: "#d4a62a",
        columns: ["Name", "Date / Times", "RO", "Position", "Contact Number", "Status", "Notes"],
        rows,
        rowKinds,
      };
    });
    ...
    const daySeparatedSections: ExportDocument["sections"] = visibleLaborDays.map((day): ExportDocument["sections"][number] => {
      ...
      const rows = dayCalls.reduce<ExportCell[][]>((acc, call, index) => {
        ...
        if (!callAssignments.length) {
          acc.push(["No crew assigned yet.", areaLabel, referenceNumber, timeLabel, call.role_name, "", "", callDetails]);
          return acc;
        }
      ...
      return {
        ...
        accentColor: "#17313b",
        columns: ["Name", "Subgroup / Area", "RO", "Times", "Position", "Contact Number", "Status", "Notes"],
        rows,
      };
    });
...
<body>
  ...
  function buildLinkedCrewListDocument(mode: "day" | "booth"): ExportDocument | null {
    ...
    const rowsForCall = (item: typeof calls[number], includeShow = true): ExportCell[][] => {
      const referenceNumber = subCallReferenceNumber(item.call);
    ...
    const sections: ExportDocument["sections"] = mode === "booth"
      ? [...new Set(calls.map((item) => boothLabel(item.call.area)).filter(Boolean))]
          ...
          .map((area) => {
            ...
            return {
              heading: area,
              subheading: `${areaCalls.length} linked sub-call${areaCalls.length === 1 ? "" : "s"}`,
              accentColor: "#d4a62a",
              columns: ["Name", "Show", "Date", "Area", "RO", "Times", "Position", "Contact Number", "Notes"],
      ...
      : [...new Set(calls.map((item) => item.day.labor_date))]
          ...
          .map((date) => {
            ...
            return {
              ...
              subheading: `${dayCalls.length} linked sub-call${dayCalls.length === 1 ? "" : "s"} • ${assignedTotal}/${neededTotal || 0} assigned`,
              accentColor: "#17313b",
              columns: ["Name", "Show", "Date", "Area", "RO", "Times", "Position", "Contact Number", "Notes"],
  ...
  async function testShortcutJson(url: string, label = "Shortcut URL") {
    ...
    try {
      ...
      setMsg({ kind: "success", text: `${label} is public and returning valid JSON.${connected}${activeCount}${dueCount}` });
    } catch (error) {
  ...
  function renderAutomationPanel() {
    ...
    const effectiveShortcutToken = automationDraft.shortcut_token || sharedShortcutToken;
    const universalShortcutPollUrl = buildUniversalShortcutPollUrl(origin, effectiveShortcutToken);
    const shortcutRunUrl = buildShortcutRunUrl(universalShortcutPollUrl);
    const shortcutInstallUrl = buildShortcutInstallUrl(universalShortcutPollUrl);
        ...
        <div className="card compact">
          ...
          {automationDraft.sending_method === "shortcut" ? (
            <div className="card compact accent-card" style={{ marginTop: 14 }}>
              ...
              <div className="toolbar">
                ...
                <button type="button" className="ghost" onClick={() => universalShortcutPollUrl ? void copyText(universalShortcutPollUrl, "My Shortcut URL") : setMsg({ kind: "error", text: "Save settings in Apple Shortcut Mode first." })}>Copy My Shortcut URL</button>
                ...
                <button type="button" className="ghost" onClick={() => void copyText(buildCoordinatorShortcutSetupText(universalShortcutPollUrl), "Coordinator Shortcut setup steps")}>Copy coordinator setup steps</button>
          ...
          <div className="card compact" style={{ marginTop: 14, background: "#fbfcfd" }}>
            ...
            <div className="card compact accent-card" style={{ boxShadow: "none", marginTop: 12 }}>
              ...
              <div className="grid grid-2" style={{ marginTop: 10 }}>
                ...
                <label className="row small" style={{ alignItems: "flex-start", justifyContent: "flex-start" }}>
                  ...
                  <span><strong>Resend schedules already marked sent</strong><span className="muted" style={{ display: "block" }}>Default off to avoid accidental duplicate texts.</span></span>
                </label>
              </div>
              {combinedScheduleSummary ? (
                <div className="card compact" style={{ boxShadow: "none", marginTop: 10, background: "#fbfcfd" }}>
                  ...
                  <div className="toolbar" style={{ marginTop: 10 }}>
                    <button type="button" className="primary" onClick={() => void sendCombinedScheduleAndReminders(false)} disabled={saving || (!combinedScheduleSummary.schedules_now && !combinedScheduleSummary.future)}>Send schedules and schedule future reminders</button>
                    <button type="button" className="ghost" onClick={() => void sendCombinedScheduleAndReminders(true)} disabled={saving || (!combinedScheduleSummary.schedules_now && !combinedScheduleSummary.future && !combinedScheduleSummary.overdue)}>Send schedules and schedule future reminders including overdue now</button>
            ...
            <div className="card compact accent-card" style={{ boxShadow: "none", marginTop: 18, borderTopWidth: 4 }}>
              <strong>Queue one reminder now</strong>
              <div className="small muted">Manual actions queue texts due now for the selected labor day or full-show schedule. Use this for an emergency send, not the normal future reminder plan. Each Shortcut run can pull up to 25 due texts.</div>
              <label className="field" style={{ marginTop: 10 }}>
                <span>Additional note at bottom of full-show schedule <span className="muted">(optional)</span></span>
                <textarea
                  rows={3}
              ...
              <div className="grid grid-3" style={{ marginTop: 12 }}>
                ...
                <div className="field">
                  ...
                  <button type="button" className="primary" onClick={queueManualDayReminderNow} disabled={saving || !selectedShow || !automationDraft.send_schedule || !selectedShowLaborDays.length}>Queue one reminder for selected day now</button>
                </div>
              </div>
              <div className="small muted" style={{ marginTop: 8 }}>
                ...
                <div className="toolbar" style={{ marginTop: 12 }}>
                  <label className="field" style={{ minWidth: 260, padding: 10, border: "2px solid #d4a62a", borderRadius: 14, background: "#fff8db", boxShadow: "0 10px 24px rgba(212, 166, 42, 0.16)" }}>
                    ...
                    <select value={eventDisplayMode} onChange={(e) => setEventDisplayMode(e.target.value as EventDisplayMode)}>
                      ...
                      {!selectedShowHasPartialCoordinatorAccess ? <option value="automation">Confirmation Center / Shortcut Queue</option> : null}
