  ...
  try {
    ...
    if (action === "submit") {
      ...
      if (requestType === "w9_only") {
        if (!payload.w9_document_url && !payload.w9_status_note) {
          return NextResponse.json({ message: "Please upload your signed W-9 or add a note before submitting." }, { status: 400 });
        }
        ...
        const updatePayload: Record<string, unknown> = {
          ...
          w9_status: payload.w9_document_url ? "uploaded" : "needs_review",
          notes: [existingNotes, w9Note].filter(Boolean).join("\n\n"),
          updated_at: payload.submitted_at,
        };
        if (payload.legal_name) updatePayload.tax_profile_notes = `W-9-only request submitted by ${payload.legal_name} on ${payload.submitted_at.slice(0, 10)}.`;
