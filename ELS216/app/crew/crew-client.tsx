...
async function queueOnboardingRequestForShortcut(record: CrewRecord, requestType: "full_onboarding" | "w9_only" = "full_onboarding") {
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.message || (requestType === "w9_only" ? "Unable to queue W-9 request." : "Unable to queue onboarding request."));
...
function ContactDetail({ record, ratingSummary, cityPools, targetGroups, bulkCityId, bulkGroupName, detailTab, onTab, onEdit, onDelete, onMove, onQueueOnboarding, onQueueW9Only, onAdminDocumentUploaded, onBulkCityChange, onBulkGroupChange, saving, canSeeAdminAudit }: ContactDetailProps) {
  ...
  async function openAdminDocument(kind: AdminUploadKind, storagePath: string | null | undefined, download = false) {
    ...
    if (!openedWindow) {
      ...
      setDocumentActionMessage({ text: "Your browser blocked the file window. Allow pop-ups for this app and try again.", kind: "error" });
      return;
    }
    try {
      openedWindow.document.write("Preparing secure Emanuel Labor Services file link...");
      const response = await fetch("/api/onboarding", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      setDocumentActionMessage({ text: download ? "Secure download link opened. It expires in 10 minutes." : "Secure file link opened. It expires in 10 minutes.", kind: "success" });
    } catch (error) {
      openedWindow.close();
          ...
          <div className="grid grid-2">
            <div className="card compact" style={{ background: "#fbfcfd" }}>
              ...
              <div className="list" style={{ gap: 10 }}>
                ...
                {(record.work_photo_urls || []).length ? (record.work_photo_urls || []).map((path, index) => (
                  <Fragment key={`${record.id}-work-photo-${index}-${path}`}>
                    {secureDocumentActions(`Work photo ${index + 1}`, "work_photo", path)}
                  </Fragment>
                )) : secureDocumentActions("Work photos", "work_photo", "")}
              </div>
              ...
              <div className="grid" style={{ gap: 10, marginTop: 14 }}>
                ...
                <label className="field">
                  ...
                  <input type="file" accept="image/*" multiple disabled={uploadDisabled} onChange={(event) => { void uploadAdminFiles("work_photo", event.target.files); event.target.value = ""; }} />
                  <span className={uploadStatus.work_photo?.kind === "error" ? "error" : uploadStatus.work_photo?.kind === "success" ? "success" : "muted small"}>{uploadStatus.work_photo?.text || "Compressed and appended to saved work photos."}</span>
                </label>
                <label className="field">
                  ...
                  <span className={uploadStatus.w9?.kind === "error" ? "error" : uploadStatus.w9?.kind === "success" ? "success" : "muted small"}>{uploadStatus.w9?.text || "Saved privately; visible only to owner/admin."}</span>
                </label>
                <label className="field">
                  ...
                  <span className={uploadStatus.contract?.kind === "error" ? "error" : uploadStatus.contract?.kind === "success" ? "success" : "muted small"}>{uploadStatus.contract?.text || "Saved privately; visible only to owner/admin."}</span>
                </label>
              </div>
            </div>
