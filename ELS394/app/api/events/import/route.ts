  ...
  try {
    ...
    const selectedChangedImportChanges = comparison
      ? comparison.changes.filter((change) => {
          ...
          if (change.kind === "new") {
            if (typeof change.importedIndex === "number") selectedNewImportIndexes.add(change.importedIndex);
    ...
    const subCallsToConsider = mergeHasExplicitSelection
      ...
      : payload.subCallGroups;
    const missingSubCalls = subCallsToConsider.filter(
