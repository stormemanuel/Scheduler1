          ...
          <div className="list" style={{ marginTop: 12 }}>
            ...
            {importPreview ? (
              <div className="card compact" style={{ marginTop: 12 }}>
                <div className="row" style={{ alignItems: "flex-start" }}>
                  <div>
                    <div style={{ fontWeight: 800 }}>{importPreview.importFormat === "estimate" ? "Estimate preview" : "Import preview"}: {importPreview.show.name}</div>
                  ...
                  <div className="toolbar" style={{ justifyContent: "flex-end" }}>
                    <span className="badge">{importPreview.importFormat === "estimate" ? "Estimate format" : "Crew-list format"}</span>
                ...
                {importPreview.existingEventMatch ? (
                  <div className="card compact" style={{ marginTop: 10, boxShadow: "none", borderColor: "#fbbf24", background: "#fffbe6" }}>
                    <div className="row" style={{ alignItems: "flex-start" }}>
                      <div>
                ...
                <div className="grid grid-2" style={{ marginTop: 10 }}>
                  ...
                  <div className="card compact" style={{ boxShadow: "none" }}>
                    ...
                    <div className="small" style={{ marginTop: 8 }}>
                      {importPreview.laborDays.length} labor days • {importPreview.subCallPreview.length} sub-calls
                      {importPreview.importFormat !== "estimate" ? ` • ${importPreview.matchedCrewCount} matched • ${importPreview.unmatchedCrewCount} unmatched` : " • no crew assigned"}
                    </div>
                  </div>
                </div>
                ...
                {importPreview.comparison ? (() => {
                  ...
                  const actionableChanges = comparisonChanges.filter((change) => change.kind === "new" || change.kind === "changed");
                  ...
                  const summary = importPreview.comparison?.summary || {};
                    ...
                    <div className="card compact" style={{ marginTop: 10, boxShadow: "none", borderColor: "#fbbf24", background: "#fffdf2" }}>
                      <div className="row" style={{ alignItems: "flex-start" }}>
                        <div>
                          ...
                          <div className="small muted" style={{ marginTop: 4 }}>
                            New days {summary.newLaborDays || 0} • New calls {summary.newSubCalls || 0} • New positions {summary.newPositions || 0} • Changes {(summary.timeChanges || 0) + (summary.poChanges || 0) + (summary.boothUpdates || 0) + (summary.rateChanges || 0) + (summary.crewCountIncreases || 0) + (summary.crewCountDecreases || 0)} • Removed from quote {summary.removedLabor || 0}
                          </div>
                        </div>
                        <span className="badge">{selectedImportChangeIds.length} selected</span>
                      </div>
                      <div className="toolbar" style={{ marginTop: 10 }}>
                        <button type="button" className="ghost" onClick={() => setSelectedImportChangeIds((current) => [...new Set([...current, ...additionIds])])}>
                          Select all additions
                        </button>
                        <button type="button" className="ghost" onClick={() => setSelectedImportChangeIds((current) => [...new Set([...current, ...changeIds])])}>
                          Select all changes
                        </button>
                        <button type="button" className="ghost" onClick={() => setSelectedImportChangeIds([])}>
                          Clear selected
                        </button>
                      </div>
                      <div className="list" style={{ marginTop: 10, maxHeight: 260, overflowY: "auto" }}>
                        {comparisonChanges.map((change) => {
                          const selectable = change.kind === "new" || change.kind === "changed";
                          return (
                            <label
                              key={change.id}
                              className="card compact"
                              style={{
                                boxShadow: "none",
                                borderColor: change.kind === "removed" ? "#ef4444" : change.kind === "unchanged" ? "#dbe4e8" : "#d9e6ea",
                                background: change.kind === "removed" ? "#fff7f7" : change.kind === "unchanged" ? "#f8fafc" : "#ffffff",
                            ...
                            >
                              <div className="row" style={{ alignItems: "flex-start", gap: 10 }}>
                                {selectable ? (
                                  ...
                                  <span className="badge">{change.kind === "removed" ? "Review only" : "Unchanged"}</span>
                                )}
                                <div style={{ flex: 1 }}>
                                  <div style={{ fontWeight: 800 }}>{change.title}</div>
                                  <div className="small muted">{change.detail}</div>
                                  {change.warning ? <div className="small error" style={{ marginTop: 4 }}>{change.warning}</div> : null}
                                </div>
                              </div>
                            </label>
                          );
                        })}
                        {!actionableChanges.length ? <div className="small muted">No additions or changes found.</div> : null}
                      </div>
                    </div>
                  );
                })() : null}
                  ...
                  <div className="card compact" style={{ marginTop: 10, boxShadow: "none", borderColor: "#ef4444", background: "#fff7f7" }}>
                    ...
                    <div className="list" style={{ marginTop: 6 }}>
                      {importPreview.needsReview.map((item, index) => (
                        <div key={`${item}-${index}`} className="small error">✕ {item}</div>
                      ))}
                    </div>
                  </div>
                ) : (
                  ...
                  <details className="card compact" style={{ marginTop: 10, boxShadow: "none" }}>
                    ...
                    <div className="grid grid-2" style={{ marginTop: 10 }}>
                      <div>
                        ...
                        <div className="small muted">{importPreview.importDebug.detectedDates?.join(", ") || "None"}</div>
                      </div>
                      <div>
                        <strong>Detected money values</strong>
                        <div className="small muted">{importPreview.importDebug.detectedCurrencyValues?.join(", ") || "None"}</div>
                      </div>
                      <div>
                        <strong>Detected booth / area headers</strong>
                        <div className="small muted">{importPreview.importDebug.detectedGroupingHeaders?.join(", ") || "None"}</div>
                      </div>
                      <div>
                        <strong>Rejected labor-like rows</strong>
                        <div className="small muted">{importPreview.importDebug.rejectedRows?.length || 0}</div>
                      </div>
                    </div>
                ...
                <div className="list" style={{ marginTop: 10, maxHeight: 260, overflowY: "auto" }}>
                  {importPreview.subCallPreview.map((call) => (
                    <div key={call.key} className="card compact">
                      <div style={{ fontWeight: 700 }}>{call.area}</div>
                      <div className="small muted">
                        {[
                          call.sub_call_reference_number || "",
                          call.po_number ? `PO: ${call.po_number}` : "",
                          call.labor_date,
                          `${formatDisplayTime(call.start_time)}-${formatDisplayTime(call.end_time)}`,
                        ].filter(Boolean).join(" • ")}
                      </div>
                      <div className="small muted">
                        {call.role_name} • {dayTypeLabel(call.day_type || "full_day")} • Qty {call.crew_needed}
                        {call.message_rate ? ` • Client rate ${moneyLabel(Number(call.message_rate))}` : ""}
                      </div>
                      {call.reviewFlags?.length ? (
                        <div className="small error" style={{ marginTop: 4 }}>Needs Review: {call.reviewFlags.join("; ")}</div>
                      ) : null}
                      {importPreview.importFormat === "estimate" ? (
                        <div className="grid" style={{ gridTemplateColumns: "repeat(4, minmax(0, 1fr))", gap: 8, marginTop: 10 }}>
                          <label className="field"><span>Booth / Area</span><input value={call.area} onChange={(event) => setImportPreview((current) => current ? { ...current, subCallPreview: current.subCallPreview.map((item) => item.key === call.key ? { ...item, area: event.target.value } : item) } : current)} /></label>
                          <label className="field"><span>Booth Number</span><input value={call.sub_call_reference_number || ""} onChange={(event) => setImportPreview((current) => current ? { ...current, subCallPreview: current.subCallPreview.map((item) => item.key === call.key ? { ...item, sub_call_reference_number: event.target.value } : item) } : current)} placeholder="Booth 300, Booth TBD" /></label>
                          <label className="field"><span>PO Number</span><input value={call.po_number || ""} onChange={(event) => setImportPreview((current) => current ? { ...current, subCallPreview: current.subCallPreview.map((item) => item.key === call.key ? { ...item, po_number: event.target.value } : item) } : current)} placeholder="PO177607" /></label>
                          <label className="field"><span>Labor Date</span><input type="date" value={call.labor_date} onChange={(event) => setImportPreview((current) => current ? { ...current, subCallPreview: current.subCallPreview.map((item) => item.key === call.key ? { ...item, labor_date: event.target.value } : item) } : current)} /></label>
                          <label className="field"><span>Role</span><input value={call.role_name} onChange={(event) => setImportPreview((current) => current ? { ...current, subCallPreview: current.subCallPreview.map((item) => item.key === call.key ? { ...item, role_name: event.target.value } : item) } : current)} /></label>
                          <label className="field"><span>Start Time</span><input type="time" value={call.start_time} onChange={(event) => setImportPreview((current) => current ? { ...current, subCallPreview: current.subCallPreview.map((item) => item.key === call.key ? { ...item, start_time: event.target.value } : item) } : current)} /></label>
                          <label className="field"><span>End Time</span><input type="time" value={call.end_time || ""} onChange={(event) => setImportPreview((current) => current ? { ...current, subCallPreview: current.subCallPreview.map((item) => item.key === call.key ? { ...item, end_time: event.target.value } : item) } : current)} /></label>
                          <label className="field">
                            <span>Day Type</span>
                            <select value={call.day_type || "full_day"} onChange={(event) => setImportPreview((current) => current ? { ...current, subCallPreview: current.subCallPreview.map((item) => item.key === call.key ? { ...item, day_type: event.target.value } : item) } : current)}>
                          ...
                          <label className="field"><span>Qty</span><input type="number" min="1" value={String(call.crew_needed || 1)} onChange={(event) => setImportPreview((current) => current ? { ...current, subCallPreview: current.subCallPreview.map((item) => item.key === call.key ? { ...item, crew_needed: Math.max(1, Number(event.target.value || 1)) } : item) } : current)} /></label>
                          <label className="field"><span>Client Rate</span><input value={call.message_rate || ""} onChange={(event) => setImportPreview((current) => current ? { ...current, subCallPreview: current.subCallPreview.map((item) => item.key === call.key ? { ...item, message_rate: event.target.value } : item) } : current)} placeholder="450" /></label>
                        </div>
                      ) : null}
