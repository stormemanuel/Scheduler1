...
export async function getEventsPageData() {
  ...
  const feedbackResponsesExcludeColumnsMissing = Boolean(feedbackResponsesResInitial.error && (feedbackResponsesResInitial.error.message.includes("excluded_from_ratings") || feedbackResponsesResInitial.error.message.includes("excluded_reason") || feedbackResponsesResInitial.error.message.includes("excluded_at")));
  ...
  const subCalls = (subCallsRes.data ?? []).map((row) => {
    const typed = row as { id: string; labor_day_id: string; area: string | null; role_name: string | null; master_rate_id?: string | null; start_time: string; end_time: string | null; crew_needed: number | null; notes: string | null; created_by?: string | null };
