...
export async function POST(request: Request) {
  ...
  const [invalidationResult] = await Promise.all([
    showIds.length
      ? invalidateShowScheduleStatus(admin, showIds, {
          reason: "New position/sub-call added. Send updated schedules.",
          changedByUserId: auth.user.id,
        })
      : Promise.resolve(null),
    createSubCallCoordinatorNotifications(admin, createdRows, auth.user.id),
  ]);
  scheduleInvalidation = invalidationResult;
  return NextResponse.json({
    ok: true,
    id: createdRows[0]?.id,
    rows: createdRows,
