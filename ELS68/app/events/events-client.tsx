...
const crewLeadFeedbackQuestions: FeedbackQuestion[] = [
  { key: "crew_readiness", label: "Crew readiness", helper: "Did the crew arrive prepared, dressed correctly, and ready to work?" },
  { key: "communication", label: "Communication with ELS", helper: "How clear was communication before and during the call?" },
  { key: "schedule_accuracy", label: "Schedule accuracy", helper: "Were the call times, locations, and assignments accurate?" },
  { key: "onsite_execution", label: "On-site execution", helper: "How smoothly did the work execute under the crew lead?" },
  { key: "team_professionalism", label: "Team professionalism", helper: "How professional was the team overall?" },
];
...
function EventsClientInner({
  ...
  const [automationDraft, setAutomationDraft] = useState<AutomationDraft>(() => defaultAutomationDraft(pickDefaultShowId(initialShows.map(sanitizeShow).filter((show) => show.id)) || ""));
  const [crewRecords] = useState(() => initialCrew.map(sanitizeCrew).filter((crew) => crew.id));

  const [search, setSearch] = useState(initialSearch);
  const [selectedShowId, setSelectedShowId] = useState<string | null>(() => pickDefaultShowId(initialShows.map(sanitizeShow).filter((show) => show.id)));
  ...
  const crewScheduleMessages = useMemo(() => {
    ...
    return [...byCrew.entries()]
      .map(([crewId, item]) => {
        ...
        const scheduleLines = sortedCalls.map(({ day, call }) => {
          const locationText = call.location ? ` – ${call.location}` : "";
          return `${formatMessageDate(day.labor_date)} – ${formatMessageTimeRange(call)} – ${call.role_name} – ${boothLabel(call.area)}${locationText} – ${messageRateLabelForCall(item.crew, call, rateCity, masterRates).replace(/^Rate:\s*/i, "")}`;
        });
  ...
  const feedbackForms = useMemo(() => {
    ...
    if (selectedShowMessageMeta.crew_lead_name || ratingRows.length) {
      ...
      forms.push(makeForm({
        ...
        subtitle: "Crew-lead survey for readiness, schedule accuracy, communication, and notes on tech performance.",
        target,
        intro: "A short crew-lead survey for internal show quality control and tech performance feedback.",
      ...
      <div key={call.id} className="card compact sub-call-card">
        <div className="row" style={{ alignItems: "flex-start" }}>
          <div>
            ...
            <div className="small muted">{call.area} • {call.role_name}</div>
            <div className="small muted">{[call.location ? `Location: ${call.location}` : "", call.message_rate ? `Message rate: $${call.message_rate}/hr` : ""].filter(Boolean).join(" • ")}</div>
