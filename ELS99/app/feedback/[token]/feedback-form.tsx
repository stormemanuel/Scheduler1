...
function normalizeSurvey(survey: SurveyData): SurveyData {
  ...
  return {
    ...
    form_kind: survey?.form_kind === "area-manager" ? "area-manager" : survey?.form_kind === "crew-lead" ? "crew-lead" : "project-manager",
    ...
    try {
      ...
      const res = await fetch(`/api/feedback/${encodeURIComponent(safeSurvey.token)}/submit`, {
        ...
        body: JSON.stringify({
          ...
          ratings,
          request_again: safeSurvey.form_kind === "crew-lead" ? "" : requestAgain,
    ...
    <main className="feedback-public-wrap">
      ...
      <form className="feedback-public-card" onSubmit={submitFeedback} noValidate>
        <div className="survey-heading-row">
          <div>
            <p className="feedback-kicker">{safeSurvey.form_kind === "area-manager" ? "Area manager survey" : safeSurvey.form_kind === "crew-lead" ? "Crew lead survey" : "Project manager survey"}</p>
        ...
        <div className="feedback-grid two">
          {safeSurvey.form_kind === "crew-lead" ? null : (
        ...
        <label>{safeSurvey.form_kind === "area-manager" ? "What was your overall experience with Emanuel Labor Services?" : safeSurvey.form_kind === "crew-lead" ? "How was the show? What can be improved?" : "What went well?"}
        ...
        <h3>{safeSurvey.form_kind === "area-manager" ? "Tech ratings for this booth / area" : safeSurvey.form_kind === "crew-lead" ? "Crew lead tech ratings" : "Tech feedback"}</h3>
