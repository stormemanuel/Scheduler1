...
import { getSessionUser, sharedCoordinatorUserIds } from "@/lib/auth";
import {
...
type OnboardingDocumentType =
  ...
  | "contract"
  | "general";
type OnboardingRequestType = "full_onboarding" | "w9_only" | "contract_only" | "profile_photo_only";
...
async function purchaseOrderRequest(admin: SupabaseAdmin, token: string) {
  if (!token || token.length < 20) return null;
  const payrollResult = await admin
    .from("show_payroll")
    .select("id,show_id,crew_id,notes")
    .ilike("notes", "%ELS_PURCHASE_ORDER_V1%")
    .limit(5000);
  if (payrollResult.error) throw new Error(payrollResult.error.message);
  const match = ((payrollResult.data || []) as Array<{ id: string; show_id: string; crew_id: string; notes?: string | null }>).find((row) => {
    const purchaseOrder = extractPurchaseOrderFromNotes(row.notes);
    return purchaseOrder?.token === token || purchaseOrder?.previousTokens?.includes(token);
  });
  if (!match) return null;
  const purchaseOrder = extractPurchaseOrderFromNotes(match.notes);
  if (!purchaseOrder) return null;
  const [showResult, crewResult] = await Promise.all([
    admin.from("shows").select("id,name,client,venue,show_start,show_end").eq("id", match.show_id).maybeSingle(),
    admin.from("crew").select("id,name,email").eq("id", match.crew_id).maybeSingle(),
  ]);
  if (showResult.error) throw new Error(showResult.error.message);
  if (crewResult.error) throw new Error(crewResult.error.message);
  return { payrollRow: match, purchaseOrder, show: showResult.data as Record<string, unknown> | null, crew: crewResult.data as Record<string, unknown> | null };
}

function publicPurchaseOrder(record: NonNullable<Awaited<ReturnType<typeof purchaseOrderRequest>>>) {
  const po = record.purchaseOrder;
  return {
    revision: po.revision,
    referenceNumber: po.referenceNumber,
    displayNumber: purchaseOrderDisplayNumber(po),
    status: po.status,
    baseAmount: po.baseAmount,
    days: po.days,
    adjustments: po.adjustments,
    adjustmentAmounts: purchaseOrderAdjustmentAmounts(po.adjustments),
    total: po.total,
    sentAt: po.sentAt ?? null,
    deliveredAt: po.deliveredAt ?? null,
    approvalDueAt: po.approvalDueAt ?? null,
    approvedAt: po.approvedAt ?? null,
    changeRequest: po.changeRequest || "",
    showName: safeText(record.show?.name) || "Event",
    showClient: safeText(record.show?.client),
    showVenue: safeText(record.show?.venue),
    showStart: safeText(record.show?.show_start),
    showEnd: safeText(record.show?.show_end),
    contractorName: safeText(record.crew?.name) || "Contractor",
  };
}

async function savePublicPurchaseOrderAction(admin: SupabaseAdmin, body: Record<string, unknown>) {
  const token = safeText(body.token);
  const record = await purchaseOrderRequest(admin, token);
  if (!record) return NextResponse.json({ message: "This Purchase Order link is invalid or no longer available." }, { status: 404 });
  const now = new Date();
  let po: ContractorPurchaseOrder = record.purchaseOrder;
  if (po.approvalDueAt && now.getTime() >= new Date(po.approvalDueAt).getTime() && po.status === "delivered") {
    ...
    await admin.from("show_payroll").update({ notes: mergePurchaseOrderIntoNotes(record.payrollRow.notes, po) }).eq("id", record.payrollRow.id);
    return NextResponse.json({ message: "The 48-hour review period has ended and this Purchase Order is already recorded as approved.", purchase_order: publicPurchaseOrder({ ...record, purchaseOrder: po }) }, { status: 409 });
  }
  if (po.status !== "delivered" && po.status !== "sent_awaiting_delivery") {
    return NextResponse.json({ message: ["approved", "auto_approved"].includes(po.status) ? "This Purchase Order is already approved." : "This Purchase Order is not currently awaiting a response." }, { status: 409 });
  }
  const action = safeText(body.action);
  const at = now.toISOString();
  if (action === "approve_purchase_order") {
    po = { ...po, status: "approved", approvedAt: at, updatedAt: at, audit: [...po.audit, { at, action: "contractor_approved", detail: `Revision ${po.revision}` }].slice(-60) };
  } else {
    const requestText = safeText(body.change_request).slice(0, 2000);
    if (!requestText) return NextResponse.json({ message: "Tell us what needs to be changed." }, { status: 400 });
    po = { ...po, status: "changes_requested", changeRequestedAt: at, changeRequest: requestText, updatedAt: at, audit: [...po.audit, { at, action: "changes_requested", detail: requestText }].slice(-60) };
  }
  const update = await admin.from("show_payroll").update({ notes: mergePurchaseOrderIntoNotes(record.payrollRow.notes, po) }).eq("id", record.payrollRow.id);
  if (update.error) throw new Error(update.error.message);
  return NextResponse.json({ ok: true, purchase_order: publicPurchaseOrder({ ...record, purchaseOrder: po }), message: action === "approve_purchase_order" ? "Purchase Order approved. Thank you." : "Your change request was sent to Emanuel Labor Services." });
}
...
function sanitizeOwnerTaxProfile(value: unknown): OwnerTaxProfilePayload {
  ...
  return {
    ...
    retain_existing_tin: Boolean(source.retain_existing_tin),
  };
}
...
function ownerTaxProfileMissingFields(
  w9: OwnerTaxProfilePayload,
  canRetainExistingTin = false,
) {
  const missing: string[] = [];
  ...
  if (
    w9.tin_digits.length !== 9 &&
    !(w9.retain_existing_tin && canRetainExistingTin)
  )
...
async function saveOwnerTaxProfile(
  ...
  w9: OwnerTaxProfilePayload,
  submittedAt: string,
  existingTin?: { encrypted: string; last4: string; type: string } | null,
) {
  const retainExisting =
    w9.retain_existing_tin && Boolean(existingTin?.encrypted);
  const tinLast4 = retainExisting ? safeText(existingTin?.last4) : w9.tin_last4;
  const tinType = retainExisting
    ? normalizeTinType(existingTin?.type)
    : w9.tin_type;
  const tinEncrypted = retainExisting
    ? safeText(existingTin?.encrypted)
    : encryptTin(w9.tin_digits);
  const row = {
    crew_id: crewId,
    request_id: null,
    tax_legal_name: w9.tax_legal_name,
...
function isSarahLedfordName(value: unknown) {
  return safeText(value).toLowerCase().replace(/\s+/g, " ") === "sarah ledford";
}

function onboardingDeliveryMode(auth: { messagingMode?: string | null }) {
  return normalizeMessagingMode(auth.messagingMode);
}

function onboardingQueueMessage(auth: { messagingMode?: string | null }, base: string) {
  return onboardingDeliveryMode(auth) === "android_messages"
    ? base.replace(/iPhone Shortcut/g, "Android Messages")
    : base;
}
...
type OnboardingSenderContext = {
  ...
  sharedCoordinatorUserIds: string[];
  isViewingAs: boolean;
  messagingMode: "apple_shortcut" | "android_messages";
};
...
async function onboardingSenderContext(allowPreview = false) {
  ...
  return {
    ...
    context: {
      ...
      sharedCoordinatorUserIds: sharedCoordinatorUserIds(session.user),
      isViewingAs: Boolean(session.isViewingAs),
      messagingMode: isSarahLedfordName((session.profile as { full_name?: string | null } | null)?.full_name)
        ? "android_messages"
        : normalizeMessagingMode(
            (session.profile as { messaging_mode?: string | null } | null)?.messaging_mode ||
            (session.user.user_metadata as { els_messaging_mode?: string | null } | null)?.els_messaging_mode
          ),
...
async function scopedCrewForOnboarding(admin: SupabaseAdmin, context: OnboardingSenderContext, crewId: string) {
  ...
  const sharedOwnerIds = new Set([context.userId, ...context.sharedCoordinatorUserIds]);
  const ownsCrew = safeText(crew.created_by) === context.userId;
  const sharedCrew = !ownsCrew && sharedOwnerIds.has(safeText(crew.created_by));
  const referredCrew = ownsCrew || sharedCrew ? false : await coordinatorHasOnboardingAssociation(admin, crewId, context.userId);
  if (!ownsCrew && !sharedCrew && !referredCrew) return null;
  if (crew.coordinator_hidden_at && safeText(crew.coordinator_hidden_by) === context.userId) return null;

  // A coordinator who invited a shared Master Pool contact may continue the
  // onboarding workflow and see safe progress even when they do not own the
  // underlying Crew record. Private documents and tax fields are never selected.
  if (sharedCrew || referredCrew) return crew;
...
async function coordinatorOnboardingDashboard(admin: SupabaseAdmin) {
  ...
  const requestRes = await admin
    ...
    .select("crew_id, status, sent_by, sent_at, submitted_at, submission_payload, updated_at")
    .order("updated_at", { ascending: false })
    .limit(5000);
  if (requestRes.error) throw new Error(requestRes.error.message);
  const sharedOwnerIds = new Set([context.userId, ...context.sharedCoordinatorUserIds]);
  const coordinatorRequests = ((requestRes.data || []) as Record<string, unknown>[]).filter((row) => [...sharedOwnerIds].some((userId) => requestWasSentByUser(row, userId)));
  const referredCrewIds = Array.from(new Set(coordinatorRequests.map((row) => safeText(row.crew_id)).filter(Boolean)));

  const [ownCrewRes, referredCrewRes, cityPoolsRes, extraPoolsRes] = await Promise.all([
    admin
      .from("crew")
      ...
      .in("created_by", [...sharedOwnerIds])
      .order("name", { ascending: true }),
    referredCrewIds.length
      ? admin.from("crew").select("id, name, phone, email, city_pool_id, group_name, created_by, coordinator_hidden_at, coordinator_hidden_by, onboarding_status, onboarding_request_sent_at, onboarding_completed_at").in("id", referredCrewIds)
      : Promise.resolve({ data: [], error: null }),
    admin.from("city_pools").select("id, name"),
    admin.from("crew_city_pools").select("crew_id, city_pool_id"),
  ]);
  if (ownCrewRes.error) throw new Error(ownCrewRes.error.message);
  if (referredCrewRes.error) throw new Error(referredCrewRes.error.message);
  ...
  const latestRequestByCrew = new Map<string, Record<string, unknown>>();
  for (const row of coordinatorRequests) {
    const crewId = safeText(row.crew_id);
    if (crewId && !latestRequestByCrew.has(crewId)) latestRequestByCrew.set(crewId, row);
  }
  const crewById = new Map<string, Record<string, unknown>>();
  for (const row of [...(ownCrewRes.data || []), ...(referredCrewRes.data || [])] as Record<string, unknown>[]) {
    const crewId = safeText(row.id);
    if (crewId) crewById.set(crewId, row);
  }
  const allowedPools = new Set(context.allowedCityPoolIds.map(safeText).filter(Boolean));
  const eligibleCrewRows = [...crewById.values()].filter((row) => {
    ...
    const referred = latestRequestByCrew.has(crewId);
    if (referred) return true;
    if (sharedOwnerIds.has(safeText((row as { created_by?: string | null }).created_by))) return true;
  ...
  const rows = eligibleCrewRows
    .sort((a, b) => Number(latestRequestByCrew.has(safeText(b.id))) - Number(latestRequestByCrew.has(safeText(a.id))))
    .filter((row) => {
    ...
    const latestRequest = latestRequestByCrew.get(crewId);
    const requestStatus = safeText(latestRequest?.status);
    const onboardingComplete = onboardingStatus === "approved" || requestStatus === "approved" || Boolean(completedAt);
    const submittedOrReview = ["submitted", "needs_review"].includes(onboardingStatus) || requestStatus === "submitted";
    const requestSent = ["request_sent", "requested", "opened", "correction_requested", "pending_contact"].includes(onboardingStatus) || ["sent", "opened", "correction_requested"].includes(requestStatus) || Boolean(typed.onboarding_request_sent_at);
    return {
      id: crewId,
      ...
      shared_onboarding: safeText(typed.created_by) !== context.userId,
      };
...
async function findMasterCrewByIdentity(admin: SupabaseAdmin, emailValue: string, phoneValue: string) {
  const email = safeText(emailValue).toLowerCase();
  const digits = phoneDigits(phoneValue);
  const select = "id, name, phone, email, onboarding_status, onboarding_completed_at, created_at";
  const [emailRes, phoneRes] = await Promise.all([
    email ? admin.from("crew").select(select).ilike("email", email).limit(100) : Promise.resolve({ data: [], error: null }),
    digits ? admin.from("crew").select(select).limit(5000) : Promise.resolve({ data: [], error: null }),
  ]);
  ...
  for (const row of [...(emailRes.data || []), ...(phoneRes.data || [])] as Record<string, unknown>[]) {
    const id = safeText(row.id);
    if (!id) continue;
    const emailMatches = email && safeText(row.email).toLowerCase() === email;
    const phoneMatches = digits && phoneDigits(row.phone) === digits;
    if (emailMatches || phoneMatches) candidates.set(id, row);
  }
  return [...candidates.values()].sort((a, b) => {
    const rank = (row: Record<string, unknown>) => safeText(row.onboarding_status) === "approved" ? 3 : safeText(row.onboarding_completed_at) ? 2 : 1;
    return rank(b) - rank(a) || safeText(a.created_at).localeCompare(safeText(b.created_at));
  })[0] || null;
}
...
function parseCityStateZip(value: unknown) {
...
type OnboardingSendHistoryEntry = {
  ...
  user_role: string;
  sent_at: string;
  action: "created" | "reused" | "reminder" | "existing_submitted" | "existing_complete";
...
function onboardingLinkForRequest(
  ...
  const suffix = requestType === "w9_only"
    ? "?mode=w9"
...
function onboardingRequestMessage(
  requestType: OnboardingRequestType,
  firstName: string,
  senderName: string,
  link: string,
) {
  return requestType === "w9_only"
    ? [
        `Hi ${firstName}, ${senderName} with Emanuel Labor Services is requesting your W-9 for ELS tax/1099 records.`,
        ...
        onboardingSenderFooter(senderName),
      ].join("\n\n")
    : requestType === "contract_only"
      ? [
          `Hi ${firstName}, ${senderName} with Emanuel Labor Services is requesting your Independent Contractor Agreement.`,
          "Please complete and sign it through this secure link:",
          link,
          "Thank you.",
          onboardingSenderFooter(senderName),
      ...
      : requestType === "profile_photo_only"
        ? [
            `Hi ${firstName}, ${senderName} with Emanuel Labor Services is requesting a current profile photo for your crew profile.`,
            "Please upload your photo using this secure link:",
            link,
            "Thank you.",
            onboardingSenderFooter(senderName),
          ].join("\n\n")
        : [
            `Hi ${firstName}, ${senderName} with Emanuel Labor Services has invited you to complete your onboarding.`,
...
function onboardingEmailSubject(requestType: OnboardingRequestType) {
  if (requestType === "w9_only") return "Emanuel Labor Services W-9 Request";
  if (requestType === "contract_only") return "Emanuel Labor Services Contractor Agreement";
  if (requestType === "profile_photo_only") return "Emanuel Labor Services Profile Photo Request";
  return "Emanuel Labor Services Onboarding";
}

function onboardingEmailHeading(requestType: OnboardingRequestType) {
  if (requestType === "w9_only") return "Complete Your ELS W-9";
  if (requestType === "contract_only") return "Complete Your ELS Contractor Agreement";
  if (requestType === "profile_photo_only") return "Upload Your ELS Profile Photo";
  return "Complete ELS Onboarding";
}

function onboardingEmailIntro(requestType: OnboardingRequestType, senderName: string) {
  if (requestType === "w9_only") return `${senderName} with Emanuel Labor Services is requesting your W-9 for ELS tax and 1099 records.`;
  if (requestType === "contract_only") return `${senderName} with Emanuel Labor Services is requesting your Independent Contractor Agreement.`;
  if (requestType === "profile_photo_only") return `${senderName} with Emanuel Labor Services is requesting a current profile photo for your crew profile.`;
  return `${senderName} with Emanuel Labor Services has invited you to complete your onboarding packet.`;
}

async function sendOnboardingEmailThroughResend(args: {
  to: string;
  crewName: string;
  senderName: string;
  replyTo: string;
  ...
  const firstName = safeText(args.crewName).split(/\s+/)[0] || "there";
  const displayName = safeText(args.senderName).replace(/[<>"\r\n]/g, "") || "ELS Coordinator";
  const from = `${displayName} via Emanuel Labor Services <onboarding@${domain}>`;
  const replyTo = safeText(args.replyTo);
  const link = safeText(args.link);
  const heading = onboardingEmailHeading(args.requestType);
  const intro = onboardingEmailIntro(args.requestType, displayName);
  const sensitiveNote = args.requestType === "full_onboarding" || args.requestType === "w9_only"
    ? "Please do not send your SSN, EIN, or tax information by regular email. Use the secure ELS form only."
    : "Please use the secure ELS link below to complete this request.";

  const html = `<!doctype html>
<html><body style="margin:0;padding:0;background:#f5f7f8;font-family:Arial,Helvetica,sans-serif;color:#102a30;">
  <div style="max-width:640px;margin:0 auto;padding:32px 18px;">
    <div style="background:#ffffff;border:1px solid #d9e2e5;border-radius:14px;padding:30px;">
      <div style="font-size:24px;font-weight:700;margin-bottom:18px;">${escapeHtml(heading)}</div>
      <p style="font-size:16px;line-height:1.6;margin:0 0 14px;">Hi ${escapeHtml(firstName)},</p>
      <p style="font-size:16px;line-height:1.6;margin:0 0 22px;">${escapeHtml(intro)}</p>
      <p style="margin:0 0 24px;"><a href="${escapeHtml(link)}" style="display:inline-block;background:#16843f;color:#ffffff;text-decoration:none;font-size:16px;font-weight:700;padding:14px 22px;border-radius:9px;">${escapeHtml(heading)}</a></p>
      <p style="font-size:14px;line-height:1.6;margin:0 0 8px;color:#53676c;">If the button does not open, use this secure link:</p>
      <p style="font-size:14px;line-height:1.6;margin:0 0 22px;word-break:break-all;"><a href="${escapeHtml(link)}">${escapeHtml(link)}</a></p>
      <p style="font-size:14px;line-height:1.6;margin:0 0 22px;color:#53676c;">${escapeHtml(sensitiveNote)}</p>
      <p style="font-size:15px;line-height:1.6;margin:0;">Coordinator: ${escapeHtml(displayName)}<br>Emanuel Labor Services</p>
    </div>
  </div>
</body></html>`;
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      from,
      to: [safeText(args.to)],
      subject: onboardingEmailSubject(args.requestType),
      html,
      text: args.textBody,
  ...
  if (!response.ok) {
    throw new Error(safeText(payload.message || payload.error?.message) || `Resend email failed with HTTP ${response.status}.`);
  }
...
async function findLatestOnboardingRequest(
  admin: SupabaseAdmin,
  crewId: string,
  requestType: OnboardingRequestType,
) {
  const result = await admin
    ...
    .limit(50);
  if (result.error) throw new Error(result.error.message);
  return ((result.data || []) as Record<string, unknown>[]).find((row) => requestTypeFromRow(row) === requestType) || null;
}

async function recordExistingOnboardingAssociation(
  admin: SupabaseAdmin,
  requestRow: Record<string, unknown>,
  auth: OnboardingAccess,
  action: "existing_submitted" | "existing_complete",
) {
  const nowIso = new Date().toISOString();
  const payload = appendOnboardingSendHistory(getPayloadObject(requestRow.submission_payload), auth, nowIso, action, false);
  const updated = await admin
    .from("crew_onboarding_requests")
    .update({ submission_payload: payload, updated_at: nowIso })
    .eq("id", safeText(requestRow.id));
...
async function coordinatorHasOnboardingAssociation(admin: SupabaseAdmin, crewId: string, userId: string) {
  const requests = await admin
    .from("crew_onboarding_requests")
    .select("sent_by, submission_payload")
    .eq("crew_id", crewId)
    .order("updated_at", { ascending: false })
    .limit(100);
  if (requests.error) throw new Error(requests.error.message);
  return ((requests.data || []) as Record<string, unknown>[]).some((row) => requestWasSentByUser(row, userId));
}
...
async function ensureDefaultCrewPositions(admin: SupabaseAdmin, crewId: string) {
  const cleanCrewId = safeText(crewId);
  if (!cleanCrewId) return;
  const existing = await admin.from("crew_positions").select("id").eq("crew_id", cleanCrewId).limit(1);
  if (existing.error) {
    if (/crew_positions|schema cache|relation/i.test(existing.error.message || "")) return;
    throw new Error(existing.error.message);
  }
  if ((existing.data || []).length) return;
  const rows = defaultNewCrewPositions().map((position) => ({
    crew_id: cleanCrewId,
    role_name: position.role_name,
    rate: Number(position.rate || getDefaultCrewPayRate(position.role_name) || 0),
  }));
  const inserted = await admin.from("crew_positions").insert(rows);
  if (inserted.error && !/crew_positions|schema cache|relation/i.test(inserted.error.message || "")) {
...
async function requireOnboardingAccess() {
  ...
  let profileRes = await supabase
    .from("profiles")
    .select("role, full_name, email, messaging_mode, device_type")
    .eq("id", user.id)
    .maybeSingle();
  ...
  return {
    ...
    fullName,
    replyToEmail,
    messagingMode: isSarahLedfordName(fullName)
      ? "android_messages"
      : normalizeMessagingMode(
          (profile as { messaging_mode?: string | null } | null)?.messaging_mode ||
          (user.user_metadata as { els_messaging_mode?: string | null } | null)?.els_messaging_mode
        ),
  } satisfies { ok: true; user: OnboardingAccess["user"]; role: string; fullName: string; replyToEmail: string; messagingMode: OnboardingAccess["messagingMode"] };
}
...
function onboardingRequestsTableMissing(message: string | null | undefined) {
  const text = String(message || "").toLowerCase();
  return text.includes("crew_onboarding_requests") && (text.includes("does not exist") || text.includes("schema cache") || text.includes("relation"));
}

async function lightweightOnboardingStatus(admin: SupabaseAdmin, request: Request, action: string) {
  const auth = await requireOnboardingAccess();
  if (!auth.ok) return auth.response;
  ...
  if (action === "crew_status") {
    const crewIds = (url.searchParams.get("crew_ids") || "")
      .split(",")
      .map((id) => id.trim())
      .filter(Boolean)
      .slice(0, 200);
    if (!crewIds.length) return NextResponse.json({ ok: true, rows: [] });

    let query = admin
      .from("crew_onboarding_requests")
      .select("id, crew_id, status, request_type, sent_at, opened_at, submitted_at, updated_at")
      .in("crew_id", crewIds)
      .in("status", ["sent", "opened", "submitted", "approved", "correction_requested"])
      .order("updated_at", { ascending: false })
      .limit(250);
    if (!canReview) query = query.eq("sent_by", auth.user.id);

    const { data, error } = await query;
    if (error && onboardingRequestsTableMissing(error.message)) return NextResponse.json({ ok: true, setup_missing: true, rows: [] });
    if (error) return NextResponse.json({ message: error.message }, { status: 400 });

    const latestByCrewId = new Map<string, Record<string, unknown>>();
    for (const row of data || []) {
      const crewId = safeText((row as { crew_id?: string | null }).crew_id);
      if (crewId && !latestByCrewId.has(crewId)) latestByCrewId.set(crewId, row as Record<string, unknown>);
    }
    return NextResponse.json({
      ok: true,
      rows: Array.from(latestByCrewId.values()).map((row) => ({
        id: safeText(row.id),
        crew_id: safeText(row.crew_id),
        status: safeText(row.status),
        request_type: safeText(row.request_type) || "full_onboarding",
        sent_at: safeText(row.sent_at),
        opened_at: safeText(row.opened_at),
        submitted_at: safeText(row.submitted_at),
  ...
  let countQuery = admin
    ...
    .eq("status", "submitted");
  if (!canReview) countQuery = countQuery.eq("sent_by", auth.user.id);

  const { count, error } = await countQuery;
  if (error && onboardingRequestsTableMissing(error.message)) return NextResponse.json({ ok: true, setup_missing: true, review_count: 0 });
  if (error) return NextResponse.json({ message: error.message }, { status: 400 });
  return NextResponse.json({ ok: true, setup_missing: false, viewer_role: auth.role, review_count: count || 0 });
}
...
const INTRO_QUEUE_SELECT = "id, crew_id, crew_name, phone, body, status, scheduled_for, sent_at, error, created_at, queued_by_user_id, queued_by_email, queued_by_name, delivery_mode, opened_in_messages_at";
...
const ONBOARDING_SENDABLE_STATUSES = ["scheduled", "queued", "sending"];

function introQueueMissingDeliveryColumns(message: string) {
  return /delivery_mode|opened_in_messages_at|schema cache/i.test(message || "");
}

function stripIntroDeliveryColumns<T extends Record<string, unknown>>(row: T) {
  const { delivery_mode: _deliveryMode, opened_in_messages_at: _openedInMessagesAt, ...rest } = row;
  return rest;
}
...
async function insertIntroQueue(
  ...
  if (introQueueMissingDeliveryColumns(message)) {
    const withoutDelivery = await admin
      .from("crew_intro_text_queue")
      .insert(stripIntroDeliveryColumns(row))
      .select(
        "id, crew_id, crew_name, phone, body, status, scheduled_for, created_at",
      )
      .single();
    if (!withoutDelivery.error) return withoutDelivery.data;
    if (!/queued_by_user_id|queued_by_email|queued_by_name|schema cache/i.test(withoutDelivery.error.message || "")) {
      throw new Error(withoutDelivery.error.message);
    }
  } else if (
    !(
    ...
    throw new Error(message);
  const { queued_by_user_id, queued_by_email, queued_by_name, delivery_mode, opened_in_messages_at, ...legacyRow } =
    row;
...
async function listAndroidOnboardingQueue(admin: SupabaseAdmin) {
  const auth = await requireOnboardingAccess();
  if (!auth.ok) return auth.response;
  let query = admin
    .from("crew_intro_text_queue")
    ...
    .eq("delivery_mode", "android_messages")
    .in("status", ONBOARDING_SENDABLE_STATUSES)
    .order("scheduled_for", { ascending: true })
    .limit(150);
  let result = await query as { data: unknown[] | null; error: { message: string } | null };
  if (result.error && introQueueMissingDeliveryColumns(result.error.message || "")) {
    return NextResponse.json({
      ok: true,
      current_user_messaging_mode: auth.messagingMode,
      rows: [],
      message: "Android onboarding queue columns are not installed yet. Run the text queue migration with delivery_mode/opened_in_messages_at to enable Android onboarding outbox.",
    });
  }
  if (result.error) throw new Error(result.error.message);
  return NextResponse.json({
    ok: true,
    current_user_messaging_mode: auth.messagingMode,
    rows: (result.data || []).map((row) => ({ ...(row as Record<string, unknown>), queue_type: "onboarding" })),
  });
}

async function markAndroidOnboardingTextOpened(admin: SupabaseAdmin, body: Record<string, unknown>) {
  const auth = await requireOnboardingAccess();
  if (!auth.ok) return auth.response;
  ...
  if (result.error && introQueueMissingDeliveryColumns(result.error.message || "")) {
    result = await admin
      .from("crew_intro_text_queue")
      .update({ error: "Opened in Android Messages. Mark sent after sending from the phone." })
      .eq("id", id)
      .eq("queued_by_user_id", auth.user.id)
  ...
  if (result.error) throw new Error(result.error.message);
  return NextResponse.json({ ok: true, row: result.data, message: "Opened Android Messages. Mark sent after sending from the phone." });
}

async function markAndroidOnboardingTextSent(admin: SupabaseAdmin, body: Record<string, unknown>) {
  const auth = await requireOnboardingAccess();
  if (!auth.ok) return auth.response;
  const id = safeText(body.id);
  if (!id) return NextResponse.json({ message: "id is required." }, { status: 400 });
  ...
  let result = await admin
    .from("crew_intro_text_queue")
    .update({ status: "sent", sent_at: sentAt, error: null, opened_in_messages_at: sentAt })
    .eq("id", id)
    .eq("queued_by_user_id", auth.user.id)
    .select(INTRO_QUEUE_SELECT)
    .maybeSingle();
  ...
  if (result.error) throw new Error(result.error.message);
  return NextResponse.json({ ok: true, row: result.data, message: "Onboarding text marked sent." });
}

const ONBOARDING_AUTO_REMINDER_INTERVAL_DAYS = 14;
const ONBOARDING_AUTO_REMINDER_COUNT = 26;

function requestExpiryIso(currentExpiry: string, lifetimeDays: number) {
  const target = Date.now() + lifetimeDays * 24 * 60 * 60 * 1000;
  const current = currentExpiry ? new Date(currentExpiry).getTime() : 0;
  return new Date(Math.max(Number.isFinite(current) ? current : 0, target)).toISOString();
}

async function cancelAutomaticOnboardingReminders(
  admin: SupabaseAdmin,
  crewId: string,
) {
  if (!crewId) return;
  const result = await admin
    ...
    .eq("crew_id", crewId)
    .eq("status", "scheduled")
    .ilike("body", "%automatic two-week reminder%");
  if (
    result.error &&
    !/crew_intro_text_queue|schema cache|relation|column/i.test(
...
function automaticOnboardingReminderBody(
  firstName: string,
  senderName: string,
  link: string,
) {
  return [
    `Hi ${firstName}, this is your automatic two-week reminder from ${senderName} with Emanuel Labor Services to finish your onboarding.`,
    "Please complete the remaining onboarding form using your secure link:",
    link,
    "Please use the secure form for W-9 and tax information. Reminders stop automatically after you submit. Thank you.",
    onboardingSenderFooter(senderName),
  ].join("\n\n");
}

async function replaceAutomaticOnboardingReminders(
  admin: SupabaseAdmin,
  args: {
    requestType: OnboardingRequestType;
    auth: {
  ...
  await cancelAutomaticOnboardingReminders(admin, args.crewId);
  if (args.requestType !== "full_onboarding" || !args.phone) return 0;
  const firstName = args.crewName.split(/\s+/)[0] || "there";
  const createdMs = new Date(args.createdAt).getTime();
  const baseMs = Number.isFinite(createdMs) ? createdMs : Date.now();
  const rows = Array.from(
    { length: ONBOARDING_AUTO_REMINDER_COUNT },
    (_, index) => ({
      queued_by_user_id: args.auth.user.id,
      queued_by_email: args.auth.user.email || null,
      queued_by_name: args.auth.fullName,
      delivery_mode: onboardingDeliveryMode(args.auth),
      crew_id: args.crewId,
      crew_name: args.crewName,
      phone: args.phone,
      body: automaticOnboardingReminderBody(
        firstName,
        args.auth.fullName,
        args.link,
      ),
      ...
      scheduled_for: new Date(
        baseMs +
          (index + 1) *
            ONBOARDING_AUTO_REMINDER_INTERVAL_DAYS *
            24 *
            60 *
            60 *
            1000,
  ...
  const result = await admin.from("crew_intro_text_queue").insert(rows);
  if (result.error) {
    const message = result.error.message || "";
    if (/crew_intro_text_queue|schema cache|relation|column/i.test(message))
      return 0;
    throw new Error(message);
  }
  return rows.length;
...
async function createSignedDocumentUrl(
  ...
  return NextResponse.json({
    ...
    viewer_kind: documentType === "profile_photo" || documentType === "work_photo" ? "image" : "document",
    expires_in_seconds: 600,
...
async function updateTaxDocumentReview(
  ...
  const stamp = new Date(reviewedAt).toLocaleDateString("en-US", {
  ...
  const reviewedBy = auth.fullName || auth.user.email || auth.user.id;
  const reviewNote = incomingNotes
    ? `${stamp}: ${notePrefix} by ${reviewedBy} at ${reviewedAt} — ${incomingNotes}`
    : `${stamp}: ${notePrefix} by ${reviewedBy} at ${reviewedAt}`;
...
async function deleteOnboardingSubmission(admin: SupabaseAdmin, body: Record<string, unknown>) {
  ...
  if (requestType !== "contract_only" && requestType !== "profile_photo_only" && currentW9Status !== "approved" && currentTaxProfileStatus !== "approved") {
    crewPatch.w9_status = "missing";
    crewPatch.tax_profile_status = "missing";
  }
  if (requestType !== "w9_only" && requestType !== "profile_photo_only" && currentContractStatus !== "approved") {
    crewPatch.contract_status = "missing";
  }
...
async function sendOnboardingCorrection(admin: SupabaseAdmin, request: Request, body: Record<string, unknown>) {
  ...
  if (requestType !== "w9_only" && requestType !== "profile_photo_only") crewPatch.contract_status = "needs_correction";
  if (requestType === "full_onboarding") {
  ...
  const linkSuffix = requestType === "w9_only"
    ? "?mode=w9"
    : requestType === "contract_only"
      ? "?mode=contract"
      : requestType === "profile_photo_only"
  ...
  if (phone && body.queue_text !== false) {
    queued = await insertIntroQueue(admin, {
      ...
      delivery_mode: onboardingDeliveryMode(auth),
      crew_id: crewId,
      crew_name: crewName,
      phone,
      body: [
  ...
  return NextResponse.json({ ok: true, link, queued, payload: nextPayload, message: queued ? onboardingQueueMessage(auth, "Correction request queued for the iPhone Shortcut.") : "Correction request saved. No text was queued because this contact has no valid phone number." });
}
...
async function listOnboardingReviewQueue(admin: SupabaseAdmin, request: Request) {
  ...
  return NextResponse.json({
    ...
    rows: rows.map((requestRow) => {
      ...
      const senderLabels = Array.from(new Set([
        ...history.map((entry) => safeText(entry.user_name) || safeText(entry.user_email) || senderById.get(safeText(entry.user_id))?.full_name || senderById.get(safeText(entry.user_id))?.email || ""),
        originalSenderId ? (senderById.get(originalSenderId)?.full_name || senderById.get(originalSenderId)?.email || "") : "",
      ].filter(Boolean)));
      ...
      return {
        ...
        sent_by_names: canReview ? senderLabels : [],
...
async function approveOnboardingSubmission(admin: SupabaseAdmin, body: Record<string, unknown>) {
  ...
  if (requestType !== "w9_only" && requestType !== "profile_photo_only" && (!isPdfDocumentReference(contractDocument) || contractDocument === profilePhoto)) {
    const signatureData = safeText(payload.contract_signature_data_url);
  ...
  if (requestType === "full_onboarding") {
    ...
    if (!contractDocument) missing.push("signed contractor agreement PDF");
  } else if (requestType === "profile_photo_only") {
    if (!profilePhoto) missing.push("profile photo");
  }
  if (missing.length) {
  ...
  if (requestType !== "contract_only" && requestType !== "profile_photo_only" && w9Document) {
  ...
  if (requestType !== "w9_only" && requestType !== "profile_photo_only" && contractDocument) {
  ...
  const patch: Record<string, unknown> = {
    ...
    w9_status: requestType === "contract_only" || requestType === "profile_photo_only" ? undefined : "approved",
    tax_profile_status: requestType === "contract_only" || requestType === "profile_photo_only" ? undefined : "approved",
    ...
    contract_status: requestType === "w9_only" || requestType === "profile_photo_only" ? undefined : "approved",
  ...
  await cancelAutomaticOnboardingReminders(admin, crewId);
...
export async function GET(request: Request) {
  ...
  if (action === "review_count" || action === "crew_status") {
    try {
      return await lightweightOnboardingStatus(admin, request, action);
    } catch (error) {
      return NextResponse.json({ message: error instanceof Error ? error.message : "Unable to load onboarding status." }, { status: 400 });
    }
  }
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "approve_purchase_order" || action === "request_purchase_order_changes") {
      return await savePublicPurchaseOrderAction(admin, body as Record<string, unknown>);
    }

    if (action === "read_request") {
      const token = safeText(body.token);
      const purchaseOrderRecord = await purchaseOrderRequest(admin, token);
      if (purchaseOrderRecord) {
        let purchaseOrder = purchaseOrderRecord.purchaseOrder;
        const now = new Date().toISOString();
        if (!purchaseOrder.audit.some((event) => event.action === "contractor_opened")) {
          purchaseOrder = { ...purchaseOrder, updatedAt: now, audit: [...purchaseOrder.audit, { at: now, action: "contractor_opened" }].slice(-60) };
          await admin.from("show_payroll").update({ notes: mergePurchaseOrderIntoNotes(purchaseOrderRecord.payrollRow.notes, purchaseOrder) }).eq("id", purchaseOrderRecord.payrollRow.id);
        }
        return NextResponse.json({ ok: true, request_type: "purchase_order", canonical_token: purchaseOrder.token, purchase_order: publicPurchaseOrder({ ...purchaseOrderRecord, purchaseOrder }) });
      }
    }
    ...
    if (action === "save_owner_tax_profile") {
      ...
      const existingTaxResult = await admin
        .from("crew_tax_profiles")
        .select("tin_encrypted, tin_last4, tin_type")
        .eq("crew_id", crewId)
        .maybeSingle();
      const existingTaxMessage = existingTaxResult.error?.message || "";
      if (
        existingTaxResult.error &&
        !/crew_tax_profiles|schema cache|relation|column/i.test(
          existingTaxMessage,
        )
      )
        throw new Error(existingTaxMessage);
      const existingTax = existingTaxResult.data as
        | {
        ...
        | null;
      const existingTin = existingTax?.tin_encrypted
        ? {
            encrypted: safeText(existingTax.tin_encrypted),
            last4: safeText(existingTax.tin_last4),
            type: safeText(existingTax.tin_type),
          }
        : null;

      const ownerW9 = sanitizeOwnerTaxProfile(body.tax_profile);
      const missing = ownerTaxProfileMissingFields(ownerW9, Boolean(existingTin));
      if (missing.length)
        return NextResponse.json(
          {
      ...
      const { data: crew, error: crewError } = await admin
        .from("crew")
        .select("id, name, tax_profile_notes, w9_document_url")
        .eq("id", crewId)
        .maybeSingle();
      if (crewError) throw new Error(crewError.message);
      if (!crew)
      ...
      await saveOwnerTaxProfile(admin, crewId, ownerW9, savedAt, existingTin);
      const crewName =
        safeText((crew as { name?: string | null }).name) || "crew member";
      const currentNotes = safeText(
        (crew as { tax_profile_notes?: string | null }).tax_profile_notes,
      );
      const effectiveTinLast4 =
        ownerW9.retain_existing_tin && existingTin
          ? existingTin.last4
          : ownerW9.tin_last4;
      const effectiveTinType =
        ownerW9.retain_existing_tin && existingTin
          ? normalizeTinType(existingTin.type)
          : ownerW9.tin_type;
      const reviewedBy = auth.fullName || auth.user.email || auth.user.id;
      const reviewNote = [
        `${savedAt.slice(0, 10)}: Tax profile entered, verified, and approved from the uploaded W-9.`,
        ...
        `TIN type: ${effectiveTinType.toUpperCase()} ending ${effectiveTinLast4}`,
        `Signed by: ${ownerW9.signer_name}`,
        "Tax information confirmed and approved in the Tax Center.",
      ]
        .filter(Boolean)
        .join("\n");
      const sourceW9Available = Boolean(
        safeText((crew as { w9_document_url?: string | null }).w9_document_url),
      );

      const patch = {
        w9_status: sourceW9Available ? "approved" : "missing",
      ...
      return NextResponse.json({
        ...
        message: sourceW9Available
          ? `Tax profile approved for ${crewName}.`
          : `Tax profile approved for ${crewName}. Tax info approved — source W-9 missing`,
      });
    }
    ...
    if (action === "resend_request") {
      ...
      if (!auth.ok) return auth.response;
      if (!isOwnerAdmin(auth.role)) {
        return NextResponse.json({ message: "Coordinator onboarding reminders are sent by email. Use Send reminder by email." }, { status: 403 });
      }
      const requestId = safeText(body.request_id);
      if (!requestId) return NextResponse.json({ message: "request_id is required." }, { status: 400 });
      ...
      const expiresAt = requestExpiryIso(
        currentExpiry,
        requestType === "full_onboarding" ? 365 : 30,
      );
      ...
      const reminderBody =
        requestType === "w9_only"
          ? [
              `Hi ${firstName}, ${auth.fullName} with Emanuel Labor Services is reminding you to complete your W-9 request.`,
              "Please reopen the same secure link:",
              link,
              "Please use the secure form for your W-9 and tax information. Thank you.",
              onboardingSenderFooter(auth.fullName),
            ].join("\n\n")
          : requestType === "contract_only"
            ? [
                `Hi ${firstName}, ${auth.fullName} with Emanuel Labor Services is reminding you to complete your Independent Contractor Agreement.`,
                "Please reopen the same secure link:",
                link,
                "Thank you.",
                onboardingSenderFooter(auth.fullName),
            ...
            : requestType === "profile_photo_only"
              ? [
                  `Hi ${firstName}, ${auth.fullName} with Emanuel Labor Services is reminding you to upload your current profile photo.`,
                  "Please reopen the same secure link:",
                  link,
                  "Thank you.",
                  onboardingSenderFooter(auth.fullName),
      ...
      const automaticReminderCount = await replaceAutomaticOnboardingReminders(
        admin,
        {
          requestType,
          auth,
      ...
      return NextResponse.json({
        ...
        link,
        queued,
        automatic_reminders_queued: automaticReminderCount,
        delivery_mode: onboardingDeliveryMode(auth),
        message:
          requestType === "full_onboarding"
            ? "Onboarding reminder queued using the same secure link. Automatic reminders are queued every two weeks until the form is submitted."
            : "Reminder queued using the same secure link.",
      });
    }

    if (action === "list_android_onboarding_queue") {
      return await listAndroidOnboardingQueue(admin);
    }

    if (action === "mark_android_onboarding_opened") {
      return await markAndroidOnboardingTextOpened(admin, body);
    }

    if (action === "mark_android_onboarding_sent") {
      return await markAndroidOnboardingTextSent(admin, body);
    }
    ...
    if (action === "create_new_crew_request") {
      ...
      const masterContact = await findMasterCrewByIdentity(admin, email, phone);
      if (masterContact) crewId = safeText(masterContact.id);
      existingContact = Boolean(crewId);

      if (existingContact) {
        const [crewStateRes, latestRequest] = await Promise.all([
          admin.from("crew").select("onboarding_status, onboarding_completed_at").eq("id", crewId).maybeSingle(),
          findLatestOnboardingRequest(admin, crewId, "full_onboarding"),
        ]);
        if (crewStateRes.error) throw new Error(crewStateRes.error.message);
        const crewState = crewStateRes.data as { onboarding_status?: string | null; onboarding_completed_at?: string | null } | null;
        const latestStatus = safeText(latestRequest?.status);
        const alreadyComplete = safeText(crewState?.onboarding_status) === "approved" || Boolean(safeText(crewState?.onboarding_completed_at)) || latestStatus === "approved";
        const alreadySubmitted = latestStatus === "submitted" || ["submitted", "needs_review"].includes(safeText(crewState?.onboarding_status));
        if (alreadyComplete || alreadySubmitted) {
          if (latestRequest) await recordExistingOnboardingAssociation(admin, latestRequest, auth, alreadyComplete ? "existing_complete" : "existing_submitted");
          return NextResponse.json({
            ok: true,
            crew_id: crewId,
            ...
            request: latestRequest ? { id: safeText(latestRequest.id), status: latestStatus } : null,
            message: alreadyComplete
              ? "This person is already fully onboarded. No duplicate packet or Crew profile was created."
              : "This person's onboarding is already submitted for Owner/Admin review. No duplicate packet was created.",
          });
        }
      }
      ...
      if (!crewId) {
        ...
        await ensureDefaultCrewPositions(admin, crewId);
      }

      const requestType: OnboardingRequestType = "full_onboarding";
      const normalizedPhone = cleanPhone(phone);
      const willQueue = Boolean(normalizedPhone && body.queue_text !== false && isOwnerAdmin(auth.role));
      ...
      const expiresAt = requestExpiryIso(currentExpiry, 365);
      ...
      const automaticReminderCount = willQueue
        ? await replaceAutomaticOnboardingReminders(admin, {
            requestType,
            auth,
            crewId,
            crewName: name,
      ...
      return NextResponse.json({
        ...
        message: reusable
          ...
          : queued
            ? existingContact
              ? onboardingQueueMessage(auth, "Existing contact found. Full onboarding request queued for the iPhone Shortcut.")
              : onboardingQueueMessage(auth, "Pending onboarding record created and request queued for the iPhone Shortcut.")
            : existingContact
    ...
    if (action === "get_active_request") {
      ...
      const crewId = safeText(body.crew_id);
      if (!crewId)
        return NextResponse.json({ message: "crew_id is required." }, { status: 400 });
      ...
      const reusable = await findReusableOnboardingRequest(admin, crewId, "full_onboarding");
      if (!reusable) {
        return NextResponse.json({
          ok: true,
          found: false,
      ...
      const token = safeText(reusable.token);
      const link = onboardingLinkForRequest(request, token, "full_onboarding");
      const crewName = safeText((crew as { name?: string | null }).name) || "there";
      const firstName = crewName.split(/\s+/)[0] || "there";
      const phone = cleanPhone((crew as { phone?: string | null }).phone);
      const messageBody = onboardingRequestMessage("full_onboarding", firstName, auth.fullName, link);
      ...
      return NextResponse.json({
        ...
        delivery_mode: onboardingDeliveryMode(auth),
        request_id: safeText(reusable.id),
        reused_link: true,
      });
    }
    ...
    if (action === "send_onboarding_email") {
      ...
      if (auth.role === "coordinator" && requestType !== "full_onboarding") {
        return NextResponse.json({ message: "Coordinators can email the full onboarding packet. W-9-only and contract-only requests remain owner/admin actions." }, { status: 403 });
      }
      ...
      if (auth.role === "coordinator") {
        ...
        const crewResult = await admin.from("crew").select("id, name, phone, email, onboarding_status").eq("id", crewId).maybeSingle();
        if (crewResult.error) throw new Error(crewResult.error.message);
        crew = crewResult.data as Record<string, unknown> | null;
      }
      if (!crew) return NextResponse.json({ message: auth.role === "coordinator" ? "Crew contact was not found in your assigned crew pool." : "Crew contact not found." }, { status: 404 });

      const to = safeText((crew as { email?: string | null }).email);
      if (!to || !to.includes("@")) return NextResponse.json({ message: "This crew member needs a valid email address before onboarding can be emailed." }, { status: 400 });
      ...
      if (requestId) {
        ...
        reusable = exact.data as Record<string, unknown> | null;
      }
      if (!reusable) reusable = await findReusableOnboardingRequest(admin, crewId, requestType);
      if (!reusable) return NextResponse.json({ message: "Create the secure onboarding link first, then send the email." }, { status: 400 });

      const requestStatus = safeText(reusable.status);
      if (requestStatus === "approved" || safeText((crew as { onboarding_status?: string | null }).onboarding_status) === "approved") {
        return NextResponse.json({ message: "This person is already fully onboarded. A duplicate onboarding email was not sent." }, { status: 409 });
      }
      if (requestStatus === "submitted") {
        return NextResponse.json({ message: "This person's onboarding is already submitted for Owner/Admin review. A duplicate onboarding email was not sent." }, { status: 409 });
      }

      const token = safeText(reusable.token);
      if (!token) return NextResponse.json({ message: "The active onboarding request is missing its secure token." }, { status: 400 });
      const link = onboardingLinkForRequest(request, token, requestType);
      const crewName = safeText((crew as { name?: string | null }).name) || "there";
      const firstName = crewName.split(/\s+/)[0] || "there";
      const messageBody = onboardingRequestMessage(requestType, firstName, auth.fullName, link);
      const replyTo = safeText(auth.replyToEmail || auth.user.email);
      const sent = await sendOnboardingEmailThroughResend({
        to,
        crewName,
      ...
      const nowIso = new Date().toISOString();
      const existingPayload = getPayloadObject(reusable.submission_payload);
      const emailHistory = Array.isArray(existingPayload.email_send_history)
        ? existingPayload.email_send_history.filter((entry) => entry && typeof entry === "object").slice(-49)
        : [];
      ...
      const saved = await admin
        .from("crew_onboarding_requests")
        .update({ sent_by: auth.user.id, sent_at: nowIso, submission_payload: updatedPayload, updated_at: nowIso })
        .eq("id", safeText(reusable.id));
      ...
      return NextResponse.json({
        ...
        link,
        message_body: messageBody,
        message: `Onboarding email sent to ${to}. Replies will go to ${replyTo || "the sending coordinator"}.`,
      });
    }
    ...
    if (action === "create_request") {
      ...
      let crewId = safeText(body.crew_id);
      const requestType = normalizeRequestType(body.request_type ?? body.mode);
      if (auth.role === "coordinator" && requestType !== "full_onboarding") {
      ...
      if (auth.role === "coordinator") {
        ...
        const crewResult = await admin
          .from("crew")
          .select("id, name, phone, email, onboarding_status")
          .eq("id", crewId)
          .maybeSingle();
        if (crewResult.error) throw new Error(crewResult.error.message);
        crew = crewResult.data as Record<string, unknown> | null;
      ...
      if (requestType === "full_onboarding") {
        const masterContact = await findMasterCrewByIdentity(admin, safeText(crew.email), safeText(crew.phone));
        const masterCrewId = safeText(masterContact?.id);
        if (masterCrewId && masterCrewId !== crewId) {
          crewId = masterCrewId;
          crew = masterContact;
        }
      }
      ...
      if (requestType === "full_onboarding") {
        const latestRequest = await findLatestOnboardingRequest(admin, crewId, requestType);
        const latestStatus = safeText(latestRequest?.status);
        const alreadyComplete = safeText((crew as { onboarding_status?: string | null }).onboarding_status) === "approved" || latestStatus === "approved";
        ...
        if (alreadyComplete || alreadySubmitted) {
          ...
          return NextResponse.json({
            ...
            request: latestRequest ? { id: safeText(latestRequest.id), status: latestStatus } : null,
            message: alreadyComplete
              ? "This person is already fully onboarded. No duplicate packet was created."
              : "This person's onboarding is already submitted for Owner/Admin review. No duplicate packet was created.",
          });
        }
      }
      ...
      const nowIso = new Date().toISOString();
      const phone = cleanPhone((crew as { phone?: string | null }).phone);
      const willQueue = Boolean(phone && body.queue_text !== false && isOwnerAdmin(auth.role));
      const reusable = await findReusableOnboardingRequest(admin, crewId, requestType);
      const token = reusable ? safeText(reusable.token) : crypto.randomBytes(32).toString("base64url");
      const currentExpiry = reusable ? safeText(reusable.expires_at) : "";
      const expiresAt = requestExpiryIso(
      ...
      return NextResponse.json({
        ...
        message: reusable
          ? queued
            ? onboardingQueueMessage(auth, "Existing active onboarding link reused and queued again.")
            : "Existing active onboarding link reused."
          : queued
            ? requestType === "w9_only"
              ? onboardingQueueMessage(auth, "W-9 request link created and queued for the iPhone Shortcut.")
              : requestType === "contract_only"
                ? onboardingQueueMessage(auth, "Contract-only request link created and queued for the iPhone Shortcut.")
                : requestType === "profile_photo_only"
                  ? onboardingQueueMessage(auth, "Profile photo request link created and queued for the iPhone Shortcut.")
                : onboardingQueueMessage(auth, "Onboarding request link created and queued for the iPhone Shortcut.")
            : requestType === "w9_only"
              ...
              : requestType === "contract_only"
                ...
                : requestType === "profile_photo_only"
                  ? "Profile photo request link created. No text was queued because this contact has no valid phone number."
    ...
    if (action === "submit") {
      ...
      const w9UseDigital = requestType !== "contract_only" && requestType !== "profile_photo_only";
      ...
      const digitalW9MailingAddress = [digitalW9.tax_address_line_1, digitalW9.tax_city_state_zip].filter(Boolean).join(", ");
      const digitalW9CityStateZip = parseCityStateZip(digitalW9.tax_city_state_zip);
      if (!payload.address && digitalW9MailingAddress) payload.address = digitalW9MailingAddress;
      if (!payload.home_address_line1 && digitalW9.tax_address_line_1) payload.home_address_line1 = digitalW9.tax_address_line_1;
      if (!payload.home_city && digitalW9CityStateZip.city) payload.home_city = digitalW9CityStateZip.city;
      if (!payload.home_state && digitalW9CityStateZip.state) payload.home_state = digitalW9CityStateZip.state;
      if (!payload.home_zip && digitalW9CityStateZip.zip) payload.home_zip = digitalW9CityStateZip.zip;
      ...
      if (requestType === "full_onboarding") {
        ...
        const hasCompleteHomeAddress = Boolean(payload.home_address_line1 && payload.home_city && payload.home_state && payload.home_zip);
        const hasDigitalW9MailingAddress = Boolean(w9UseDigital && digitalW9.tax_address_line_1 && digitalW9.tax_city_state_zip);
        if (!hasCompleteHomeAddress && !hasDigitalW9MailingAddress) {
          const missingHomeFields = [
            payload.home_address_line1 ? "" : "street address",
            payload.home_city ? "" : "city",
            payload.home_state ? "" : "state",
            payload.home_zip ? "" : "ZIP",
          ].filter(Boolean);
          requiredMissing.push(`Home address section near the top of the form (${missingHomeFields.join(", ")})`);
        }
      ...
      if (requestType === "profile_photo_only") {
        ...
        const photoNote = [
          "[[ELS_PROFILE_PHOTO_ONLY_SUBMISSION]]",
          `Submitted: ${payload.submitted_at}`,
        ...
        const { error: updateError } = await admin
          ...
          .update({
            ...
            notes: [existingNotes, photoNote].filter(Boolean).join("\n\n"),
            updated_at: payload.submitted_at,
          })
          .eq("id", crewId);
        if (updateError) throw new Error(updateError.message);
        ...
        const audit = await admin
          ...
          .insert({
            crew_id: crewId,
            action: "profile_photo_only_submitted",
            details: payload,
            created_at: payload.submitted_at,
          });
        if (
        ...
        return NextResponse.json({
          ok: true,
          message:
            "Profile photo submitted securely. Emanuel Labor Services will review it for the crew profile.",
        });
      }
