...
export async function GET(request: Request) {
  ...
  if (canUsePage(role, session.access, "events")) {
    ...
    const { data: subCallRows } = await supabase
      .from("sub_calls")
      .select("id, labor_day_id, area, location, po_number, role_name, start_time, end_time, notes")
    ...
    const { data: subCallShowRows } = showIds.length
      ? await supabase.from("shows").select("id, name, client, venue, show_start, created_by").in("id", showIds)
      : { data: [] as Array<Record<string, unknown>> };
    ...
    for (const row of (subCallRows ?? []) as Array<Record<string, unknown>>) {
      ...
      if (restrictEvents && clean(show.created_by) !== session.user.id && !sharedIds.has(clean(show.id))) continue;
