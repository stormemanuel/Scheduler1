...
export default function PayrollClient({ initialRows, initialEventSummaries, availableYears, initialYear, initialSummaryLoaded, initialError, initialDiagnostics = null }: Props) {
  ...
  const [paidShowsOpen, setPaidShowsOpen] = useState(false);
  ...
  const totalScopeLabel = summaryFailed
    ...
    : partialSummary
      ...
      : summaryRequestCompleted
        ? `${savedSnapshotCount}/${summaryEvents.length} saved show totals · ${loadedEventCount} detail${loadedEventCount === 1 ? "" : "s"} opened`
        : currentLoadProgress?.total
          ? `Loading saved show totals: ${currentLoadProgress.done}/${currentLoadProgress.total}`
          : "Loading saved Payroll totals";
  const summaryMoney = (value: number) => summaryFailed ? "Could not load" : (summaryRequestCompleted || (currentLoadProgress?.done ?? 0) > 0) ? money(value) : "Loading...";
  ...
  async function selectPayrollYear(nextYear: number, force = false) {
    ...
    setMessage(`Loading ${nextYear} saved Payroll totals…`);
    ...
    try {
      ...
      setYearLoadProgress((current) => ({ ...current, [returnedYear]: { done: 0, total: showIndex.length, failed: 0 } }));
      ...
      setYearLoadProgress((current) => ({ ...current, [returnedYear]: { done: showIndex.length, total: showIndex.length, failed: 0 } }));
      setLoadedYears((current) => new Set([...current, returnedYear]));
      ...
      setMessage(`Payroll loaded from saved show totals for ${showIndex.length} ${returnedYear} show${showIndex.length === 1 ? "" : "s"}. Open a show to load the full Payroll detail rows.`);
    } catch (error) {
      ...
      <section className="card accent-card">
        <div className="row" style={{ alignItems: "center" }}>
          <div>
            ...
            <p className="muted" style={{ margin: 0 }}>{viewLabel(reportView)}. Totals load from lightweight event summaries first; open a show only when you need worker rows, payment controls, expense details, or tax review. Current view excludes future shows from payable, unpaid, and P&L totals. Estimated view includes all scheduled shows for planning. Contract labor includes tech payouts plus non-waived coordinator pay.</p>
          ...
          <section key={event.showId} className="card" style={{ borderTop: "4px solid var(--brand)" }}>
              ...
              <div>
                ...
                <div className="toolbar" style={{ marginTop: 10 }}>
                  ...
                  <span className={event.payrollSnapshotStatus === "saved" ? "badge event-badge-current" : "badge"}>
                    {event.payrollSnapshotStatus === "saved" ? "Saved totals" : event.payrollSnapshotStatus === "not_completed" ? "Complete show to save totals" : "No saved totals"}
                  </span>
            ...
            {!eventDetailsLoaded ? (
              <div className="card compact" style={{ marginTop: 14, background: "#fbfcfd", boxShadow: "none" }}>
                ...
                <p className="muted small" style={{ marginBottom: 0 }}>Open this show to load its tech rows, coordinator pay, P&amp;L controls, and tax details. Completed shows use saved totals here until opened.</p>
              </div>
            ) : (
              <>
          ...
          <section className="card" style={{ borderTop: "4px solid #16a34a" }}>
            <div className="row" style={{ alignItems: "center" }}>
              <div>
                ...
                <p className="muted" style={{ margin: 0 }}>Collapsed by default. Open this section only when you need historical paid-show controls; paid-show payroll details remain lazy until you expand an individual show.</p>
              </div>
              <div className="toolbar" style={{ justifyContent: "flex-end" }}>
                ...
                <button className="ghost" type="button" onClick={() => { setPaidShowsOpen((current) => !current); setPaidShowsVisible(25); }}>
                  {paidShowsOpen ? "Collapse paid shows" : "Open paid shows"}
                </button>
              </div>
            </div>
            {paidShowsOpen ? (
