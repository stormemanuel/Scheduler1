              ...
              <tbody>
                {visibleRows.map((row) => (
                  <tr key={row.key}>
                    ...
                    <td style={{ padding: "10px 8px", borderBottom: "1px solid var(--line)" }}>
                      <input type="number" step="0.01" value={row.half_day === "0" ? "" : row.half_day} disabled={!canManage} onFocus={(e) => e.currentTarget.select()} onChange={(e) => updateRow(row.key, { half_day: e.target.value })} style={{ width: 110, padding: "10px 12px", borderRadius: 12, border: "1px solid var(--line)" }} placeholder={selectedGroup === DEFAULT_GROUP ? "225" : "use default"} />
                    </td>
                    <td style={{ padding: "10px 8px", borderBottom: "1px solid var(--line)" }}>
