...
function roleMatches(left: unknown, right: unknown) {
  const leftRole = normalizeRole(left);
  const rightRole = normalizeRole(right);
  if (!leftRole || !rightRole) return false;
  const leftKeys = roleKeys(leftRole);
  const rightKeys = roleKeys(rightRole);
  if (leftKeys.has(rightRole) || rightKeys.has(leftRole)) return true;
  for (const key of leftKeys) if (rightKeys.has(key)) return true;
  return leftRole.includes(rightRole) || rightRole.includes(leftRole);
}
...
function queueIdentity(user: { id: string; email?: string | null }) {
  return {
    queued_by_user_id: user.id,
    queued_by_email: user.email || null,
    queued_by_name: user.email || null,
  };
}

function stampRows<T extends object>(rows: T[], user: { id: string; email?: string | null }) {
...
async function insertIntroRows(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, rows: Record<string, unknown>[]) {
  const selectColumns = "id, crew_id, crew_name, phone, body, status, scheduled_for, sent_at, error, created_at";
...
async function upsertTextQueueRows(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, rows: QueueRow[]) {
  const selectColumns = "id, show_id, crew_id, crew_name, phone, message_type, reminder_key, scheduled_for, status, body, sent_at, error, created_at";
...
function messageRateForCall(call: SubCallRow, crew: CrewRow, show: ShowRow, masterRates: MasterRateRow[], meta: Record<string, string>) {
  const customRate = formatHourlyRate(call.message_rate);
  if (customRate) return customRate;

  const crewPosition = crew.positions.find((position) => roleMatches(position.role_name, call.role_name) && Number(position.rate) > 0);
  if (crewPosition) return formatHourlyRate(Number(crewPosition.rate) / 10);

  const linkedMasterRate = call.master_rate_id ? masterRates.find((rate) => rate.id === call.master_rate_id) : null;
  if (linkedMasterRate && Number(linkedMasterRate.full_day) > 0) return formatHourlyRate(Number(linkedMasterRate.full_day) / 10);

  const targetCity = normalizeRole(show.rate_city || "Default") || "default";
  const cityMasterRate = masterRates.find((rate) => normalizeRole(rate.city_name || "Default") === targetCity && roleMatches(rate.role_name, call.role_name));
  const defaultMasterRate = masterRates.find((rate) => normalizeRole(rate.city_name || "Default") === "default" && roleMatches(rate.role_name, call.role_name));
  const matchedMasterRate = cityMasterRate || defaultMasterRate;
  if (matchedMasterRate && Number(matchedMasterRate.full_day) > 0) return formatHourlyRate(Number(matchedMasterRate.full_day) / 10);
  ...
  return formatHourlyRate(meta.default_hourly_rate) || "Rate TBD";
}
...
function nextScheduleByCrew(show: ShowRow, days: LaborDayRow[], calls: SubCallRow[], assignments: AssignmentRow[], crewRows: CrewRow[], masterRates: MasterRateRow[], settings: AutomationSettings) {
  ...
  const result = new Map<string, { crew: CrewRow; lines: string[]; meetUps: string[]; rates: string[]; firstCall: { day: LaborDayRow; call: SubCallRow; assignment: AssignmentRow } | null; values: Record<string, string> }>();
  ...
  for (const assignment of [...assignments].sort((a, b) => Number(a.sort_order || 0) - Number(b.sort_order || 0))) {
    ...
    current.lines.push(`${formatDate(day.labor_date)} - ${assignmentTimeRange(assignment, call)} - ${blockLabel} - ${rateText}`);
  ...
  for (const item of result.values()) {
    ...
    const rate = uniqueRates.length === 1
      ...
      : uniqueRates.length > 1
        ...
        : formatHourlyRate(meta.default_hourly_rate) || "TBD";
...
function buildQueueRows(
  ...
  const byCrew = nextScheduleByCrew(bundle.show, filteredDays, filteredCalls, filteredAssignments, bundle.crewRows, bundle.masterRates, settings);
  const rows: QueueRow[] = [];
  ...
  for (const [crewId, item] of byCrew.entries()) {
    ...
    if (mode === "schedule_reminders" && settings.send_schedule) {
      ...
      for (const reminder of reminders) {
        ...
        const messageBody = applyTemplate(reminder.template, item.values);
        rows.push({
          show_id: settings.show_id,
          crew_id: crewId,
          crew_name: item.crew.name || "Crew member",
...
function buildCustomQueueRows(
  ...
  const byCrew = nextScheduleByCrew(bundle.show, targetDays, targetCalls, targetAssignments, bundle.crewRows, bundle.masterRates, settings);
  ...
  for (const [crewId, item] of byCrew.entries()) {
    ...
    rows.push({
      ...
      status: "scheduled",
      body: applyTemplate(options.body, item.values),
    });
  }
...
async function queueIntroText(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, body: Record<string, unknown>, user: { id: string; email?: string | null }) {
...
async function queueCrewBulkMessage(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, body: Record<string, unknown>, user: { id: string; email?: string | null }) {
  ...
  const rows = contacts.flatMap((contact) => {
    ...
    return [{
      ...
      crew_name: crewName,
      phone,
      body: applyTemplate(template, values),
      status: "scheduled",
      scheduled_for: scheduledFor,
      created_at: nowIso,
      error: null,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (isCustomMessageAction) {
      ...
      const queueRows = stampRows(buildCustomQueueRows(effectiveSettings, bundle, {
        laborDayId,
        eventWide,
    ...
    const queueOptions = isManualReminderAction
      ...
      : {};
    const queueRows = stampRows(buildQueueRows(mode, effectiveSettings, bundle, queueOptions), auth.user);
