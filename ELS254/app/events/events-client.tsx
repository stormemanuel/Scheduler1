      ...
      <div key={call.id} className="card compact sub-call-card">
        <div className="row" style={{ alignItems: "flex-start" }}>
          <div>
            ...
            <div className="small muted">{call.area} • {call.role_name}</div>
            <div className="small muted">{[dayTypeLabel(effectiveCallDayType(call)), walkawayLabel(call), call.location ? `Location: ${call.location}` : "", call.po_number ? `PO: ${call.po_number}` : "", areaLeadLine(call), call.message_rate ? `Message rate: $${call.message_rate}/hr` : ""].filter(Boolean).join(" • ")}</div>
