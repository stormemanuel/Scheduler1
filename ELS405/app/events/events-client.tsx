...
const defaultAvailabilityTemplate = "Hello {first_name}, are you available for {show_name}?\n\nPosition: {position}\nArea: {area}\nRate: {rate}\nDates Needed:\n{availability_schedule}\n\nPlease respond ASAP, as the positions are filled quickly.";
...
function normalizeAvailabilityTemplateForDisplay(value: string | null | undefined) {
  ...
  if (!text || text === oldDefault || text === priorDefault || text === oldNeededDefault) return defaultAvailabilityTemplate;
  return text.replace(/Needed:\s*\{availability_schedule\}/i, "Dates Needed:\n{availability_schedule}");
}
...
function EventsClientInner({
  ...
  const availabilityRequestDetails = useMemo(() => {
    ...
    const rates = [...new Set(calls.map((item) => messageRateLabelForCall(undefined, item.call, selectedShow?.rate_city || activeRateCity, liveMasterRates).replace(/^Rate:\s*/i, "")).filter(Boolean))];
    ...
    return {
      ...
      area: areas.length ? areas.join(" / ") : "Event areas",
      rate: rates.length ? rates.join(" / ") : "TBD",
      schedule,
    };
  }, [selectedAvailabilitySubCalls, selectedShow?.rate_city, selectedShow?.show_start, selectedShow?.show_end, activeRateCity, liveMasterRates, availabilityCandidateRole]);
        ...
        <div className="list" style={{ maxHeight: 260, overflowY: "auto", marginTop: 12 }}>
          {availableCrew.length ? availableCrew.map((crew) => {
            ...
            <div key={crew.id} className="row small">
              ...
              <button type="button" className="ghost" disabled={addDisabled} title={fullTargetCallIds.length ? "One or more selected subgroups are full. Increase Crew Needed or use Replacement mode." : ""} onClick={() => addCrewToCall(crew.id)}>
                {crewReplaceSourceCrewId ? `Replace on ${crewAssignmentTargetCallIds.length || 1} day${(crewAssignmentTargetCallIds.length || 1) === 1 ? "" : "s"}` : `Add to ${crewAssignmentTargetCallIds.length || 1} day${(crewAssignmentTargetCallIds.length || 1) === 1 ? "" : "s"}`}
              </button>
            </div>
  ...
  async function queueSelectedAvailabilityMessages() {
    ...
    const data = await request("/api/text-automation", "POST", {
      ...
      availability_calls: selectedAvailabilitySubCalls.map(({ day, call }) => ({
        ...
        rate: messageRateLabelForCall(undefined, call, selectedShow.rate_city || activeRateCity, liveMasterRates).replace(/^Rate:\s*/i, ""),
      })),
          ...
          <div className="card compact accent-card" style={{ marginTop: 14 }}>
            ...
            <div className="card compact" style={{ marginTop: 12, boxShadow: "none", background: "#fbfcfd" }}>
              ...
              <div className="small" style={{ marginTop: 10 }}>
                <strong>Message will use:</strong> {availabilityRequestDetails.position} · {availabilityRequestDetails.area} · {availabilityRequestDetails.rate} · {availabilityRequestDetails.schedule}
              </div>
            </div>
            ...
            <label className="field" style={{ marginTop: 12 }}>
              ...
              <span className="small muted">Placeholders: {"{first_name}"}, {"{show_name}"}, {"{venue}"}, {"{show_start}"}, {"{show_end}"}, {"{position}"}, {"{area}"}, {"{rate}"}, {"{availability_schedule}"}, {"{city}"}, {"{coordinator_name}"}.</span>
            </label>
            <div className="toolbar" style={{ marginTop: 10 }}>
