...
async function serveCrewProfilePhoto(auth: Awaited<ReturnType<typeof authContext>>, crewId: string) {
  ...
  if (!isOwnerAdmin(auth.role)) {
    ...
    let allowed = String(typed.created_by || "") === auth.user.id || leadFromMatchesCurrentUser(typed.lead_from, auth) || Boolean(typed.city_pool_id && allowedPools.has(String(typed.city_pool_id)));
    if (!allowed) {
...
export async function GET(request: Request) {
  ...
  if (!isOwnerAdmin(auth.role) && auth.role === "coordinator") {
    ...
    rawCrewRows = rawCrewRows.filter((row) => {
      ...
      return String(typed.created_by || "") === auth.user.id || leadFromMatchesCurrentUser(typed.lead_from, auth);
    });
  }
