  ...
  return (
    <form className="list" action={(formData) => {
      ...
      <label className="field">
        ...
        <select name="messagingMode" defaultValue="apple_shortcut">
      ...
      <label className="field">
        ...
        <input name="deviceType" type="text" placeholder="Samsung Android, iPhone, etc." />
      </label>
      <label className="field">
...
export function AdminPasswordControls({ userId, userName, email }: { userId: string; userName: string; email: string }) {
  ...
  function run(action: "temporary" | "email", formData: FormData) {
    ...
    startTransition(async () => {
      try {
        ...
        setError(cause instanceof Error ? cause.message : "The password action could not be completed.");
      } finally {
  ...
  function handleSubmit(action: "temporary" | "email", event: FormEvent<HTMLFormElement>) {
  ...
  return (
    <div className="card compact accent-card">
      ...
      <form className="list" onSubmit={(event) => handleSubmit("temporary", event)}>
      ...
      <form onSubmit={(event) => handleSubmit("email", event)} style={{ marginTop: 10 }}>
        <input type="hidden" name="userId" value={userId} />
