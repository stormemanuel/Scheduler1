...
export async function POST(request: Request) {
  ...
  if (!payload.labor_day_id || !payload.area || !payload.role_name || !payload.start_time || !payload.end_time) return NextResponse.json({ message: 'Area, role, start time, and end time are required.' }, { status: 400 });
