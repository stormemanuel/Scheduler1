  ...
  try {
    ...
    if (String(body.record_type || "").trim() === "coordinator") {
      ...
      try {
        ...
        const message = error instanceof Error ? error.message : String((error as { message?: string } | null)?.message || error || "");
        if (!message.includes('relation "coordinator_payroll" does not exist')) throw error;
        const data = await upsertCoordinatorFallbackWithSchemaFallback(admin, payload);
        return NextResponse.json({ ok: true, row: data, message: "Coordinator payroll status updated using payroll fallback." });
      }
    }
    ...
    if (message.includes('relation "coordinator_payroll" does not exist')) {
      return NextResponse.json({ message: "Run supabase/ELS129_required_migrations.sql once to create coordinator payroll tracking." }, { status: 400 });
    }
