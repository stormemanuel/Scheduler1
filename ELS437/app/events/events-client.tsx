                ...
                <div key={row.id} className="card compact" style={{ padding: 10 }}>
                  <div className="row small" style={{ alignItems: "flex-start" }}>
                    <div>
                      ...
                      <div className="muted">Queued by: {row.queued_by_name || row.queued_by_email || "Legacy / unknown user"}</div>
                  ...
                  <div className="toolbar" style={{ marginTop: 8 }}>
                    ...
                    {ACTIVE_TEXT_QUEUE_STATUSES.has(normalize(row.status)) ? <button type="button" className="ghost danger" onClick={() => void cancelQueuedTexts(row.id)} disabled={saving}>Cancel queued</button> : null}
