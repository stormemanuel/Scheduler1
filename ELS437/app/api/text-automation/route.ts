...
async function buildImmediateFullShowSchedulePlan(
  ...
  options: { resendCancelled?: boolean; resendSent?: boolean; updatedScheduleNotice?: boolean } = {},
) {
  ...
  for (const [crewId, item] of byCrew.entries()) {
    ...
    const shouldAddUpdatedNotice = Boolean(options.updatedScheduleNotice && options.resendSent && existingResult === "already_sent");
