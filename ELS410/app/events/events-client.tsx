...
function importPreviewGroupKey(call: ImportPreviewCall) {
  return [
    importPreviewGroupPart(call.area || "Area TBD"),
    importPreviewGroupPart(call.sub_call_reference_number || ""),
    importPreviewGroupPart(call.po_number || ""),
  ].join("|");
}
...
function buildGroupedImportPreview(preview: ImportPreviewState, selectedChangeIds: string[]) {
  ...
  const areaIdentityMap = new Map<string, Set<string>>();
  ...
  preview.subCallPreview.forEach((call, index) => {
    ...
    const booth = safeText(call.sub_call_reference_number);
    const po = safeText(call.po_number);
    const areaKey = importPreviewGroupPart(areaName);
    const identities = areaIdentityMap.get(areaKey) || new Set<string>();
    identities.add(`${importPreviewGroupPart(booth)}|${importPreviewGroupPart(po)}`);
    areaIdentityMap.set(areaKey, identities);
  ...
  for (const section of groupMap.values()) {
    const areaKey = importPreviewGroupPart(section.area);
    if ((areaIdentityMap.get(areaKey)?.size || 0) > 1) {
      section.warnings.unshift("Possible duplicate Sub-Call name: same area name appears with a different booth or PO.");
    }
...
function EventsClientInner({
  ...
  function defaultOpenCrewAssignmentCallIds(callId: string) {
    const openMatches = defaultCrewAssignmentCallIds(callId).filter((targetId) => {
      const targetCall = subCallById.get(targetId);
      return targetCall ? callCapacitySummary(targetCall).open > 0 : false;
    });
    return openMatches.length ? openMatches : [callId];
  }
  ...
  function openCrewPickerForBookingCall(callId: string) {
    ...
    setCrewAssignmentCallIds(closing ? [] : defaultOpenCrewAssignmentCallIds(callId));
    setCrewSearch("");
      ...
      <div className="card compact" style={{ marginTop: 12 }}>
        ...
        {canManageEventAccess ? (
          <div className="card compact" style={{ marginTop: 10, borderLeft: "4px solid #d4a62a", boxShadow: "none" }}>
            {crewAssignmentTargetCallIds.some((callId) => {
              const targetCall = subCallById.get(callId);
              return targetCall ? callCapacitySummary(targetCall).full : false;
            }) ? (
          ...
          {availableCrew.length ? availableCrew.map((crew) => {
            ...
            <div key={crew.id} className="row small">
              ...
              <button type="button" className="ghost" disabled={Boolean(assigningCrewId) || addDisabled} title={fullTargetCallIds.length && !(canManageEventAccess && allowCrewOverstaff) ? "One or more selected subgroups are full. Increase Crew Needed, use Replacement mode, or enable overstaffing." : ""} onClick={() => addCrewToCall(crew.id)}>
                {assigningCrewId === crew.id ? "Adding..." : crewReplaceSourceCrewId ? `Replace on ${crewAssignmentTargetCallIds.length || 1} day${(crewAssignmentTargetCallIds.length || 1) === 1 ? "" : "s"}` : `Add to ${crewAssignmentTargetCallIds.length || 1} day${(crewAssignmentTargetCallIds.length || 1) === 1 ? "" : "s"}`}
              </button>
            </div>
    ...
    return (
      <details className="card compact" style={{ marginTop: 10, boxShadow: "none", background: "#fbfcfd" }}>
        ...
        <div className="grid" style={{ gridTemplateColumns: "repeat(auto-fit, minmax(170px, 1fr))", gap: 8, marginTop: 10 }}>
          ...
          <label className="field">
            <span>Booth / RO</span>
            <input value={call.sub_call_reference_number || ""} onChange={(event) => updateImportPreviewCall(call.key, { sub_call_reference_number: event.target.value })} placeholder="RO327982, Booth 300" />
          </label>
          <label className="field">
            <span>PO Number</span>
                        ...
                        <summary style={{ cursor: "pointer", listStyle: "none" }}>
                          <div className="row" style={{ alignItems: "flex-start" }}>
                            <div>
                              ...
                              <div className="small muted">
                                {[group.booth ? `RO/Booth: ${group.booth}` : "", group.po ? `PO: ${group.po}` : "", `${group.dates.length} date${group.dates.length === 1 ? "" : "s"}`].filter(Boolean).join(" • ")}
                              </div>
                            </div>
