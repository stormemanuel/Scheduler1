...
function EventsClientInner({
  ...
  function buildCrewListDocument(): ExportDocument | null {
    ...
    const boothSubCallSections = boothSections.flatMap((section): ExportDocument["sections"] => {
      const grouped = new Map<string, Array<{ day: LaborDayRecord; call: SubCallRecord }>>();
      for (const item of section.calls) {
        const signature = [
          normalize(section.booth),
          normalize(item.call.area),
          normalize(item.call.role_name),
          normalize(item.call.start_time),
          normalize(item.call.end_time),
          normalize(item.call.po_number || ""),
          item.call.one_hour_walkaway ? "walkaway" : "",
        ].join("|");
        grouped.set(signature, [...(grouped.get(signature) ?? []), { day: item.day, call: item.call }]);
      }

      return [...grouped.values()]
        .map((entries) => entries.sort((a, b) => a.day.labor_date.localeCompare(b.day.labor_date) || a.call.start_time.localeCompare(b.call.start_time)))
        .map((entries) => {
          const first = entries[0];
          const dateSummary = entries.map((entry) => entry.day.labor_date).join(", ");
          const neededValues = [...new Set(entries.map((entry) => Number(entry.call.crew_needed || 0)).filter((value) => value > 0))];
          const neededLabel = neededValues.length === 1
            ? `${neededValues[0]} needed`
            : neededValues.length > 1
              ? `Need varies (${neededValues.join(", ")})`
              : "Need TBD";
          const rows = entries.flatMap(({ day, call }) => buildRows(call, day.labor_date));
          return {
            heading: section.booth,
            subheading: `${first.call.role_name} • ${dateSummary} • ${formatTimeRange(first.call)}${first.call.one_hour_walkaway ? " • 1 hr walkaway" : ""}${first.call.po_number ? ` • PO: ${first.call.po_number}` : ""} • ${neededLabel}`,
    ...
    const sections: ExportDocument["sections"] = eventDisplayMode === "booth"
      ...
      : visibleLaborDays.flatMap((day): ExportDocument["sections"] => {
          ...
          return dayCalls.map(({ call }) => ({
            heading: `${day.labor_date}${day.label ? ` - ${day.label}` : ""}`,
            subheading: `${boothLabel(call.area)} • ${call.role_name} • ${formatTimeRange(call)}${call.one_hour_walkaway ? " • 1 hr walkaway" : ""}${call.po_number ? ` • PO: ${call.po_number}` : ""} • ${call.crew_needed} needed`,
              ...
              <div className="grid grid-3" style={{ marginTop: 12 }}>
                ...
                <label className="field">
                  ...
                  <div className="row" style={{ gap: 8, marginTop: 8 }}>
                    <input type="date" value={customTextScheduledDate} onChange={(event) => { setCustomTextScheduledDate(event.target.value); setCustomTextSendTimePreset(""); }} />
      ...
      <div className="card compact" style={{ borderColor: "#111827", marginTop: 12 }}>
        ...
        {editorMode === "show" ? (
          <div className="grid" style={{ gap: 14, marginTop: 12 }}>
            ...
            <div className="grid grid-2">
              <label className="field"><span>Staffing city / crew pool</span><input value={showForm.rate_city} onChange={(e) => setShowForm((c) => ({ ...c, rate_city: e.target.value }))} placeholder="Houston TX, New Orleans, Default" /></label>
              <div className="grid grid-2">
