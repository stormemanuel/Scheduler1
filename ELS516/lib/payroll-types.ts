...
import type { ContractorPurchaseOrder, CoordinatorCompensationSchedule } from "@/lib/payroll-calculations";
...
export type PayrollCallDetail = {
  ...
  coordinationRateSchedule?: CoordinatorCompensationSchedule | null;
};
...
export type PayrollCrewShowRow = {
  ...
  coordinatorRateSchedule?: CoordinatorCompensationSchedule | null;
  crewId: string;
  crewName: string;
  crewEmail: string;
  crewPhone: string;
  ...
  purchaseOrder?: ContractorPurchaseOrder | null;
...
export type PayrollCoordinatorPaymentSummary = {
  ...
  notes: string;
  statusId: string | null;
  rateSchedule?: CoordinatorCompensationSchedule | null;
};
...
export type PayrollEventSummary = {
  ...
  paidTotal: number;
  unpaidTotal: number;
  payrollSnapshotStatus?: "saved" | "missing" | "not_completed";
  payrollSnapshotSavedAt?: string | null;
  payrollSnapshotSource?: string | null;
};
...
export type PayrollPerformanceTiming = {
  label: string;
  ms: number;
};

export type PayrollPerformanceDiagnostics = {
  mode: "summary" | "event" | "full";
  loadedYear: number;
  showCount: number;
  rowCount: number;
  totalMs: number;
  generatedAt: string;
  timings: PayrollPerformanceTiming[];
  summaryRequest?: {
    request: string;
    status: "ok" | "failed";
    httpStatus: number;
