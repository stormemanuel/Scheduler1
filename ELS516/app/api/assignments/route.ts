...
import { createSupabaseAdminClient, createSupabaseServerClient, findCrewAssignmentConflict } from "@/lib/supabase-server";
...
function assignmentCountsAsFilled(status: string | null | undefined) {
  const text = String(status || "").trim().toLowerCase();
  return !["no_show_replaced", "called_in_replacement_used", "cancelled", "canceled", "removed", "declined", "unavailable"].includes(text);
}
...
async function canAddToSubCall(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, subCallId: string, replacingAssignmentId: string | null) {
  ...
  const assignedRes = await admin.from("assignments").select("id, status").eq("sub_call_id", subCallId);
  if (assignedRes.error) throw assignedRes.error;
  const assignedCount = ((assignedRes.data ?? []) as Array<{ status?: string | null }>).filter((row) => assignmentCountsAsFilled(row.status)).length;
...
export async function POST(request: Request) {
  ...
  const requestedOverstaff = Boolean(body.allow_overstaff);
  if (!subCallId || !crewId) {
  ...
  let coordination_owner_user_id: string | null = null;
  let coordination_owner_name: string | null = null;
  let coordination_fee_waived = false;
  if (access.ownerAdmin) {
    const resolvedOwner = await resolveCoordinationOwner(admin, requestedCoordinationOwnerId, crewId);
    if (!resolvedOwner.ok) return NextResponse.json({ message: resolvedOwner.message }, { status: 400 });
    coordination_owner_user_id = resolvedOwner.userId;
    coordination_owner_name = resolvedOwner.name;
    coordination_fee_waived = Boolean(resolvedOwner.userId && requestedFeeWaived);
  } else if (access.coordinatorOwnsCall) {
    coordination_owner_user_id = auth.user.id;
    coordination_owner_name = access.coordinatorName;
  }

  const capacity = await canAddToSubCall(admin, subCallId, replacingAssignmentId);
  if (!capacity.ok && !(access.ownerAdmin && requestedOverstaff)) {
  ...
  try {
    ...
    if (conflict) {
      return NextResponse.json({
        message: `This crew member is already assigned to another overlapping call on ${conflict.laborDate} (${conflict.startTime || "TBD"}–${conflict.endTime || "TBD"}). Remove or move the other assignment before adding them here.`,
  ...
  if (!missingAssignmentColumns(existingWithOrder.error)) {
    ...
    try {
      const conflict = await findCrewAssignmentConflict(admin, {
        ...
        targetSubCallId: subCallId,
        targetStatus: status,
        ignoreAssignmentIds: [String(data.id || "")],
      });
      if (conflict) {
        await admin.from("assignments").delete().eq("id", data.id);
        return NextResponse.json({
          message: `This crew member became double-booked with another call on ${conflict.laborDate} (${conflict.startTime || "TBD"}–${conflict.endTime || "TBD"}). The new assignment was rolled back.`,
          code: "CREW_TIME_CONFLICT",
        }, { status: 409 });
      }
    } catch (verifyError) {
      await admin.from("assignments").delete().eq("id", data.id);
      return NextResponse.json({ message: verifyError instanceof Error ? verifyError.message : "Could not verify the saved crew assignment. The new assignment was rolled back." }, { status: 400 });
    }
