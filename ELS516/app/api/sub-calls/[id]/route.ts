...
function subCallTimeMinutes(value: unknown) {
...
function sameSubCallTime(a: unknown, b: unknown) {
  const aMinutes = subCallTimeMinutes(a);
  const bMinutes = subCallTimeMinutes(b);
  if (aMinutes !== null && bMinutes !== null) return aMinutes === bMinutes;
  return String(a || "").trim() === String(b || "").trim();
}

function assignmentUsesSubCallTime(value: unknown, oldCallValue: unknown) {
  const clean = String(value || "").trim();
  return !clean || sameSubCallTime(clean, oldCallValue);
}
...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  for (const assignment of (assignedCrewRows || []) as Array<{ id?: string | null; crew_id?: string | null; status?: string | null; start_time?: string | null; end_time?: string | null }>) {
    ...
    try {
      const targetStartTime = assignmentUsesSubCallTime(assignment.start_time, existingForSchedule?.start_time)
        ? payload.start_time
        : String(assignment.start_time || "").trim();
      const targetEndTime = assignmentUsesSubCallTime(assignment.end_time, existingForSchedule?.end_time)
        ? payload.end_time
        : String(assignment.end_time || "").trim();
  ...
  // Assignments normally inherit the sub-call time. Older ELS rows often saved
  // that inherited time directly on the assignment, so changing the sub-call
  // alone left worker schedules and reminder messages on the old hours. Move
  // only inherited values forward; preserve any intentionally customized worker time.
  const assignmentTimeChanged = !sameSubCallTime(existingForSchedule?.start_time, payload.start_time)
    || !sameSubCallTime(existingForSchedule?.end_time, payload.end_time);
  const warnings: string[] = [];
  if (assignmentTimeChanged) {
    for (const assignment of (assignedCrewRows || []) as Array<{ id?: string | null; start_time?: string | null; end_time?: string | null }>) {
      const assignmentId = String(assignment.id || "").trim();
      if (!assignmentId) continue;
      const patch: { start_time?: string | null; end_time?: string | null } = {};
      if (assignmentUsesSubCallTime(assignment.start_time, existingForSchedule?.start_time)) patch.start_time = payload.start_time || null;
      if (assignmentUsesSubCallTime(assignment.end_time, existingForSchedule?.end_time)) patch.end_time = payload.end_time || null;
      if (!Object.keys(patch).length) continue;
      const assignmentUpdate = await admin.from("assignments").update(patch).eq("id", assignmentId);
      if (assignmentUpdate.error) {
        // The sub-call update above is already committed. Never report the whole
        // save as failed after that point or an admin may retry a successful edit.
        warnings.push(`Assigned worker time could not be synchronized: ${assignmentUpdate.error.message}`);
      }
    }
  }
  ...
  if (scheduleImpactingChange) {
    try {
      ...
      warnings.push(`Schedule status could not be refreshed: ${invalidationError instanceof Error ? invalidationError.message : "unknown error"}`);
    }
  }
  ...
  if (nextCoordinatorUserId && nextCoordinatorUserId !== previousCoordinatorUserId) {
    try {
      ...
      warnings.push(`Coordinator notification could not be created: ${notificationError instanceof Error ? notificationError.message : "unknown error"}`);
    }
  }
  return NextResponse.json({
    ok: true,
    ...
    scheduleInvalidation,
    warnings,
    message: warnings.length ? `Sub-call updated. ${warnings.join(" ")}` : 'Sub-call updated.',
  });
}
