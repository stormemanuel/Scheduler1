...
## Feedback tech notes layout fix
- Moved submitted tech-rating notes out from directly under the crew name.
- Notes now display on the right side under the star/request badges.
- Added the label `Client Notes:` before submitted note text.
- Added responsive behavior so mobile still stacks cleanly.


## Public Feedback Privacy Shell Fix

- Feedback survey routes (`/feedback/...`) now bypass the internal app shell entirely.
- Public feedback links no longer render the global search bar, admin navigation, session bar, or sign-out/login controls.
- Middleware now passes the request pathname to the root layout using `x-els-pathname` so the server can hide app chrome before HTML is sent to the browser.
- The public survey remains token-protected and still uses the existing feedback submit endpoint.

### Validation for public feedback privacy shell patch

- Syntax parsed successfully for `app/layout.tsx` and `middleware.ts` using TypeScript transpile diagnostics.
- Full TypeScript/Next validation could not be rerun inside this sandbox because this unpacked zip does not include `node_modules`, so dependencies such as `next`, `react`, and `@supabase/ssr` were not locally available.
