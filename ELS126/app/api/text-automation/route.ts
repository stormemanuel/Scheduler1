...
function buildCustomQueueRows(
  settings: AutomationSettings,
  bundle: Awaited<ReturnType<typeof fetchShowBundle>>,
  options: { laborDayId: string; subCallId?: string; subCallIds?: string[]; startTime?: string; endTime?: string; scheduledLocal?: string; body: string }
) {
  const targetDayIds = new Set([options.laborDayId].filter(Boolean));
  ...
  const callById = new Map(targetCalls.map((call) => [call.id, call]));
  ...
  const batchKey = `custom_${safeText(options.laborDayId).slice(0, 8)}_${new Date().toISOString().replace(/[^0-9]/g, "").slice(0, 14)}`;

  for (const assignment of bundle.assignments) {
    if (!targetCallIds.has(assignment.sub_call_id)) continue;
    const call = callById.get(assignment.sub_call_id);
    const day = call ? dayById.get(call.labor_day_id) : null;
    const crew = crewById.get(assignment.crew_id);
    if (!call || !day || !crew) continue;
    if (options.startTime && !sameClockTime(assignmentStartTime(assignment, call), options.startTime)) continue;
    if (options.endTime && !sameClockTime(assignmentEndTime(assignment, call), options.endTime)) continue;
    const phone = cleanPhone(crew.phone || "");
    if (!phone) continue;
    const byCrew = nextScheduleByCrew(bundle.show, [day], targetCalls, [assignment], bundle.crewRows, settings);
    const values = byCrew.get(crew.id)?.values || {
      first_name: firstName(crew.name || ""),
      crew_name: crew.name || "Crew member",
      show_name: bundle.show.name || "ELS Show",
      client: bundle.show.client || "",
      venue: bundle.show.venue || "",
      location: bundle.show.venue || bundle.show.rate_city || "",
      show_start: formatDate(bundle.show.show_start),
      show_end: formatDate(bundle.show.show_end),
      meet_up_location: parseEventMeta(bundle.show.notes).meet_up_location || "TBD onsite",
      schedule: "",
      next_call: `${formatDate(day.labor_date)} ${assignmentTimeRange(assignment, call)} ${call.role_name || "Crew"} ${call.area || ""}`.trim(),
      rate: "TBD",
      coordinator_name: parseEventMeta(bundle.show.notes).coordinator_name || "Storm Leigh",
      coordinator_phone: parseEventMeta(bundle.show.notes).coordinator_phone || "504-657-6618",
    };
    rows.push({
      show_id: settings.show_id,
      crew_id: crew.id,
      ...
      status: "scheduled",
      body: applyTemplate(options.body, values),
    });
  }
...
async function queueCrewBulkMessage(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, body: Record<string, unknown>) {
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "queue_crew_bulk_message") {
      const result = await queueCrewBulkMessage(admin, body);
    ...
    if (isCustomMessageAction) {
      ...
      if (!laborDayId) return NextResponse.json({ message: "Choose the labor day for the custom message." }, { status: 400 });
      if (!customBody) return NextResponse.json({ message: "Enter a custom message before queueing." }, { status: 400 });
      const queueRows = buildCustomQueueRows(effectiveSettings, bundle, {
        laborDayId,
      ...
      if (!queueRows.length) return NextResponse.json({ ok: true, settings: saved, queue: [], message: "No custom texts were queued. Check that the selected day/sub-call/time has assigned crew with phone numbers." });
      const { data, error } = await admin
        .from("text_message_queue")
      ...
      if (error) throw new Error(error.message);
      return NextResponse.json({ ok: true, settings: saved, queue: data || [], message: `Queued ${queueRows.length} custom text${queueRows.length === 1 ? "" : "s"}.` });
    }
