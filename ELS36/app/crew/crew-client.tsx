...
export default function CrewClient({ cityPools: initialCityPools, crewGroups: initialGroups, initialCrew, masterRates }: CrewClientProps) {
  ...
  async function deleteSelectedCityPool() {
    ...
    const assignedCount = crewRecords.filter((record) => record.city_pool_id === selectedCity.id).length;
    ...
    try {
      ...
      setCrewRecords((current) => current.map((record) => record.city_pool_id === selectedCity.id
        ? { ...record, city_pool_id: null, city_name: "Unassigned" }
        : record));
      setSelectedIds([]);
      setSelectedGroup(ALL_GROUPS);
      setCitySearch("");
                  ...
                  <div className="card compact" style={{ background: "#fff", marginBottom: 14 }}>
                    <div className="row" style={{ alignItems: "flex-start", marginBottom: 10 }}>
                      <div>
                        ...
                        <div className="muted small">Paste the website form text. The app will auto-fill name, email, phone, notes, roles, and city pool. If the city pool does not exist yet, it will create it.</div>
                      </div>
