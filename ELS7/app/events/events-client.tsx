...
type EventDisplayMode = "day" | "booth" | "messages";
...
const roleAliases: Record<string, string[]> = {
  ...
  "client facing audio visual tech": [
    "cf avt",
    "client facing avt",
    "client facing av tech",
    "client facing audiovisual tech",
    "cf av tech",
  ],
...
const fallbackRoleRates: Record<
  string,
  { full_day: number; half_day: number | null }
> = {
...
function splitVenueAndAddress(venue: string) {
  const parts = safeText(venue)
    .split("•")
    .map((part) => part.trim())
    .filter(Boolean);
  return {
    venueName: parts[0] || safeText(venue) || "the venue",
...
function parseLegacyEventMeta(notes: string) {
  ...
  for (const line of notes.split(/\r?\n/)) {
    ...
    if (!value) continue;
    if (
      key === "meet up location" ||
      key === "meetup location" ||
      key === "report location"
    )
      meta.meet_up_location = value;
    if (key === "crew lead" || key === "report to")
      meta.crew_lead_name = value.replace(/,?\s*crew lead$/i, "").trim();
    if (key === "crew lead phone" || key === "report to phone")
      meta.crew_lead_phone = value;
    if (key === "rate" || key === "hourly rate" || key === "message rate")
      meta.default_hourly_rate = value.replace(/[^0-9.]/g, "");
...
function composeShowNotes(displayNotes: string, meta: EventMessageMeta) {
  ...
  const cleanedMeta: EventMessageMeta = {
    ...
    default_hourly_rate: safeText(meta.default_hourly_rate).replace(
      /[^0-9.]/g,
      "",
    ),
...
function roleKeys(roleName: string) {
  ...
  const keys = new Set(
    [target, ...(roleAliases[target] ?? [])].filter(Boolean),
  );
  ...
  if (
    (/\bbo\b/.test(target) || /\bbreakout\b/.test(target)) &&
    /\btech\b|\btechnician\b|\boperator\b|\broom\b|\bbreakouts\b/.test(target)
  ) {
    ["breakout operator", "bo", "bo tech", "breakout tech"].forEach((item) =>
      keys.add(item),
    );
  }
...
function rateForCall(
  ...
  const builtInRate =
    fallbackRoleRates[[...keys].find((key) => fallbackRoleRates[key]) || ""];
...
function crewFullDayRateForRole(
  crew: CrewRecord | undefined,
  roleName: string,
) {
  if (!crew) return null;
  const keys = roleKeys(roleName);
  const exact = crew.positions.find(
    (position) =>
      keys.has(normalize(position.role_name)) && Number(position.rate) > 0,
  );
  ...
  const fuzzy = crew.positions.find((position) => {
    const role = normalize(position.role_name);
    return (
      Number(position.rate) > 0 &&
      [...keys].some((key) => role.includes(key) || key.includes(role))
    );
  });
...
function hourlyRateLabel(
  ...
  const fullDay =
    crewFullDayRateForRole(crew, call.role_name) ??
    rateForCall(
      { ...call, start_time: "08:00", end_time: "18:00" },
      rateCity,
      masterRates,
    );
...
function resolveEventPool(show: ShowRecord | null, crewRecords: CrewRecord[]) {
  ...
  const cities = [
    ...new Set(crewRecords.map((crew) => crew.city_name).filter(Boolean)),
  ];
  const haystack = normalize(
    [show.rate_city, show.venue, show.notes, show.name].join(" "),
  );

  if (show.rate_city && normalize(show.rate_city) !== "default") {
    const exact = cities.find(
      (city) => normalize(city) === normalize(show.rate_city),
    );
  ...
  for (const hint of venueToPoolHints) {
    if (hint.hints.some((item) => haystack.includes(normalize(item)))) {
      const city = cities.find(
        (name) => normalize(name) === normalize(hint.pool),
      );
...
export default function EventsClient({
  ...
  const [expandedDayIds, setExpandedDayIds] = useState<string[]>(
    initialLaborDays[0]?.id ? [initialLaborDays[0].id] : [],
  );
  ...
  const [eventDisplayMode, setEventDisplayMode] =
    useState<EventDisplayMode>("day");
  const [editorMode, setEditorMode] = useState<EditorMode>(null);
  const [checklistOpen, setChecklistOpen] = useState(false);
  const [checkedCrew, setCheckedCrew] = useState<Set<string>>(new Set());
  ...
  const [editingDayTargetId, setEditingDayTargetId] = useState<string | null>(
    null,
  );
  ...
  const selectedShow =
    ...
    filteredShows[0] ??
    null;
  const selectedShowNoteParts = useMemo(
    () => splitShowNotes(selectedShow?.notes || ""),
    [selectedShow?.notes],
  );
  ...
  const estimate = useMemo(() => {
    ...
    const allDayIds = laborDays
      .filter((day) => day.show_id === selectedShow.id)
      .map((day) => day.id);
    const allCalls = subCalls.filter((call) =>
      allDayIds.includes(call.labor_day_id),
    );
  ...
  const activeCrewCall =
    subCalls.find((call) => call.id === crewPickerCallId) ?? null;
  ...
  const poolGroups = useMemo(() => {
    const poolCrew = crewRecords.filter(
      (crew) =>
        !eventPool || normalize(crew.city_name) === normalize(eventPool),
    );
    const groups: string[] = [
      ...new Set(poolCrew.map((crew) => crew.group_name || "Ungrouped")),
  ...
  const availableCrew = useMemo(() => {
    ...
    const alreadyAssigned = new Set(
      assignedCrew.map((item) => item.assignment.crew_id),
    );

    const poolCrew = crewRecords.filter((crew) => {
      if (alreadyAssigned.has(crew.id)) return false;
      if (eventPool && normalize(crew.city_name) !== normalize(eventPool))
        return false;
      if (
        crewGroupFilter !== "All groups" &&
        (crew.group_name || "Ungrouped") !== crewGroupFilter
      )
        return false;
      if (crew.unavailable_dates.includes(activeCrewDay.labor_date))
        return false;
  ...
  const displayCalls = useMemo(() => {
    return visibleLaborDays
        ...
        subCalls
          ...
          .map((call) => ({
            ...
            callAssignments: assignments
              ...
              .map((assignment) => ({
                assignment,
                crew: crewRecords.find(
                  (crew) => crew.id === assignment.crew_id,
                ),
              })),
          })),
      )
      .sort((a, b) => {
        ...
        const areaCompare = boothLabel(a.call.area).localeCompare(
          boothLabel(b.call.area),
        );
  ...
  const crewScheduleMessages = useMemo(() => {
    ...
    const byCrew = new Map<
      ...
      {
        crew: CrewRecord | undefined;
        calls: Array<{ day: LaborDayRecord; call: SubCallRecord }>;
      }
    >();
    for (const item of displayCalls) {
      for (const assignment of item.callAssignments) {
        const existing = byCrew.get(assignment.assignment.crew_id) ?? {
    ...
    return [...byCrew.entries()]
      .map(([crewId, item]) => {
        const sortedCalls = item.calls.sort((a, b) =>
          `${a.day.labor_date} ${a.call.start_time} ${a.call.area}`.localeCompare(
            `${b.day.labor_date} ${b.call.start_time} ${b.call.area}`,
          ),
        );
        ...
        const rateLine =
          Number.isFinite(savedHourlyRate) && savedHourlyRate > 0
            ...
            : firstCall
              ? hourlyRateLabel(item.crew, firstCall, rateCity, masterRates)
              : "Rate: TBD";
        const scheduleLines = sortedCalls.map(
          ({ day, call }) =>
            `${formatMessageDate(day.labor_date)} – ${formatMessageTimeRange(call)} – ${call.role_name} – ${boothLabel(call.area)}`,
        );
        ...
        const leadCall = displayCalls.find(
          ({ call }) =>
            normalize(call.role_name).includes("crew lead") ||
            normalize(call.role_name) === "lead",
        );
  ...
  const checklistNames = useMemo(() => {
    ...
    for (const item of displayCalls) {
      for (const assignment of item.callAssignments) {
        const name = assignment.crew?.name || assignment.assignment.crew_id;
  ...
  const checklistText = useMemo(() => {
    return checklistNames.map((name) => `□ ${name}`).join("\n");
  }, [checklistNames]);

  function toggleChecklistName(name: string) {
    const key = normalize(name);
    setCheckedCrew((current) => {
  ...
  function clearChecklist() {
    setCheckedCrew(new Set());
  }

  function checkAllCrew() {
    setCheckedCrew(new Set(checklistNames.map((name) => normalize(name))));
  }
  ...
  function renderCrewPicker(call: SubCallRecord) {
    return (
      <div className="card compact" style={{ marginTop: 12 }}>
        <div className="row" style={{ alignItems: "flex-start" }}>
          <div>
            ...
            <div className="small muted">
              {call.area} • {call.role_name}
            </div>
            <div className="small muted">Pool: {eventPool || "All crew"}</div>
          </div>
          <span className="small muted">
            Role matches are suggested first. Anyone in the pool can be
            assigned.
          </span>
        </div>
        <label className="field">
          <span>Search this pool</span>
          <input
            value={crewSearch}
            onChange={(e) => setCrewSearch(e.target.value)}
            placeholder="Name, group, role, phone..."
          />
        </label>
        ...
        <div
          className="list"
          style={{ maxHeight: 260, overflowY: "auto", marginTop: 12 }}
        >
          {availableCrew.length ? (
            availableCrew.map((crew) => (
              <div key={crew.id} className="row small">
                <div>
                  ...
                  <div className="muted">
                    {crew.city_name} • {crew.group_name || "Ungrouped"} •{" "}
                    {crew.positions
                      .map((position) => position.role_name)
                      .join(", ") || "No saved positions"}
                  </div>
                  {matchesRole(crew, call.role_name) ? (
                    <div className="small">Suggested role match</div>
                  ) : (
                    <div className="small muted">Available in this pool</div>
                  )}
                </div>
                <button
                  type="button"
                  className="ghost"
                  onClick={() => addCrewToCall(crew.id)}
                >
                  Add
                </button>
              </div>
            ...
            <div className="small muted">
              No available crew in this pool for this search.
            </div>
          )}
        </div>
      </div>
      ...
      <div key={call.id} className="card compact">
        <div className="row" style={{ alignItems: "flex-start" }}>
          <div>
            ...
            <div className="small muted">
              {callAssignments.length}/{call.crew_needed} assigned
            </div>
          </div>
          <div className="toolbar">
            <button
              type="button"
              className="ghost"
              onClick={() => startEditCall(day.id, call)}
            >
              Edit
            </button>
            <button
              type="button"
              className="ghost"
              onClick={() => openCrewPickerForCall(call.id)}
            >
              Add Crew
            </button>
            <button
        ...
        {editorMode === "call" && editingCallId === call.id
          ? renderEditorPanel()
          : null}
              ...
              <div key={assignment.id} className="row small">
                <div>
                  ...
                  <span className="muted">
                    {" "}
                    • {crew?.phone
                      ? formatPhone(crew.phone)
                      : "No phone"} • {assignment.status}
                  </span>
                </div>
                <button
                  type="button"
                  className="ghost danger"
                  onClick={() => removeCrewFromCall(assignment.id)}
                >
                  Remove
                </button>
              </div>
  ...
  async function runImport() {
    ...
    try {
      ...
      if (importMode === "merge" && selectedShowId)
        formData.append("target_show_id", selectedShowId);
      Object.entries(importForm).forEach(([key, value]) =>
        formData.append(key, String(value)),
      );
      ...
      const nextDays: LaborDayRecord[] = (data.laborDays ?? []).map(
        (row: LaborDayRecord) => ({
          id: String(row.id),
      ...
      const nextCalls: SubCallRecord[] = (data.subCalls ?? []).map(
        (row: SubCallRecord) => ({
          id: String(row.id),
      ...
      const nextAssignments: AssignmentRecord[] = (data.assignments ?? []).map(
      ...
      setShows((current) =>
        mergeById(current, [nextShow]).sort((a, b) =>
          a.show_start.localeCompare(b.show_start),
        ),
      );
      setLaborDays((current) =>
        mergeById(current, nextDays).sort((a, b) =>
          a.labor_date.localeCompare(b.labor_date),
        ),
      );
      setSubCalls((current) =>
        mergeById(current, nextCalls).sort((a, b) =>
  ...
  function exportCrewList() {
    ...
    const buildRows = (call: SubCallRecord) => {
      ...
      return callAssignments
        .map(
          ({ assignment, crew }) =>
            `<tr><td>${escapeHtml(crew?.name || assignment.crew_id)}</td><td>${escapeHtml(formatTimeRange(call))}</td><td>${escapeHtml(call.role_name)}</td><td>${escapeHtml(crew ? formatPhone(crew.phone) : "")}</td><td>${escapeHtml(assignment.status)}</td></tr>`,
        )
        .join("");
    };
    ...
    const boothHtml = boothSections
      .map((section) => {
        const dates = [
          ...new Set(section.calls.map((item) => item.day.labor_date)),
        ];
        return `
          ...
          ${dates
            .map((date) => {
              const callsForDate = section.calls.filter(
                (item) => item.day.labor_date === date,
              );
              return `
...
<body>
  ...
  function exportScheduleMessages() {
    if (!selectedShow) {
      setMsg({ kind: "error", text: "Select a show first." });
      return;
    }
    const text = crewScheduleMessages
    ...
    downloadTextFile(
      `${selectedShow.name.replace(/[^a-z0-9]+/gi, "_")}_imessage_schedules.txt`,
      text,
    );
  }
  ...
  function exportChecklist() {
    ...
    downloadTextFile(
      `${selectedShow.name.replace(/[^a-z0-9]+/gi, "_")}_crew_checklist.txt`,
      `Here’s the checklist in the same order:\n\n${checklistText}`,
    );
  }
    ...
    if (data?.row) {
      setAssignments((current) => {
        const next = current.filter(
          (assignment) =>
            !(
              assignment.sub_call_id === data.row.sub_call_id &&
              assignment.crew_id === data.row.crew_id
            ),
        );
  ...
  async function saveShow() {
    ...
    if (!payload.name || !payload.show_start || !payload.show_end) {
      setMsg({
        kind: "error",
        text: "Show name, start, and end are required.",
      });
      return;
    }
    ...
    const nextShow: ShowRecord = editingShowId
      ? { id: editingShowId, ...payload }
      : { id: data.id, ...payload };
    setShows((current) =>
      editingShowId
        ? current.map((show) => (show.id === editingShowId ? nextShow : show))
        : [...current, nextShow].sort((a, b) =>
            a.show_start.localeCompare(b.show_start),
          ),
    );
    setSelectedShowId(nextShow.id);
  ...
  async function deleteShow(id: string) {
    if (!confirm("Delete this show, its labor days, and its sub-calls?"))
      return;
    await request(`/api/shows/${id}`, "DELETE");
    const nextDayIds = laborDays
      .filter((day) => day.show_id === id)
      .map((day) => day.id);
    const nextCallIds = subCalls
      .filter((call) => nextDayIds.includes(call.labor_day_id))
      .map((call) => call.id);
    setAssignments((current) =>
      current.filter(
        (assignment) => !nextCallIds.includes(assignment.sub_call_id),
      ),
    );
    setSubCalls((current) =>
      current.filter((call) => !nextDayIds.includes(call.labor_day_id)),
    );
  ...
  async function saveDay() {
    ...
    const nextDay: LaborDayRecord = editingDayId
      ? { id: editingDayId, ...payload }
      : { id: data.id, ...payload };
    setLaborDays((current) =>
      editingDayId
        ? current.map((day) => (day.id === editingDayId ? nextDay : day))
        : [...current, nextDay].sort((a, b) =>
            a.labor_date.localeCompare(b.labor_date),
          ),
    );
    setExpandedDayIds((current) =>
      current.includes(nextDay.id) ? current : [...current, nextDay.id],
    );
    setEditingDayId(null);
    setDayForm(emptyDay);
    setEditorMode(null);
  ...
  async function deleteDay(id: string) {
    ...
    const callIds = subCalls
      .filter((call) => call.labor_day_id === id)
      .map((call) => call.id);
    setAssignments((current) =>
      current.filter((assignment) => !callIds.includes(assignment.sub_call_id)),
    );
    setSubCalls((current) =>
      current.filter((call) => call.labor_day_id !== id),
    );
  ...
  async function saveCall() {
    ...
    if (!payload.area || !payload.role_name || !payload.start_time) {
      setMsg({
        kind: "error",
        text: "Area, role, and start time are required.",
      });
      return;
    }
    ...
    const nextCall: SubCallRecord = editingCallId
      ? { id: editingCallId, ...payload }
      : { id: data.id, ...payload };
    setSubCalls((current) =>
      editingCallId
        ? current.map((call) => (call.id === editingCallId ? nextCall : call))
        : [...current, nextCall].sort((a, b) =>
            a.start_time.localeCompare(b.start_time),
          ),
    );
    setEditingCallId(null);
  ...
  async function deleteCall(id: string) {
    ...
    setAssignments((current) =>
      current.filter((assignment) => assignment.sub_call_id !== id),
    );
  ...
  function renderEditorPanel() {
    ...
    return (
      <div
        ...
        style={{ borderColor: "#111827", marginTop: 12 }}
      >
        <div className="row">
          ...
          <div className="grid" style={{ gap: 14, marginTop: 12 }}>
            <div className="grid grid-2">
              <label className="field">
                ...
                <input
                  value={showForm.name}
                  onChange={(e) =>
                    setShowForm((c) => ({ ...c, name: e.target.value }))
                  }
                />
              </label>
              <label className="field">
                <span>Client</span>
                <input
                  value={showForm.client}
                  onChange={(e) =>
                    setShowForm((c) => ({ ...c, client: e.target.value }))
                  }
                />
              </label>
            </div>
            <label className="field">
              ...
              <input
                ...
                onChange={(e) =>
                  setShowForm((c) => ({ ...c, venue: e.target.value }))
                }
              />
            </label>
            <div className="grid grid-2">
              <label className="field">
                <span>Rate city</span>
                <input
                  value={showForm.rate_city}
                  onChange={(e) =>
                    setShowForm((c) => ({ ...c, rate_city: e.target.value }))
                  }
                />
              </label>
              <div className="grid grid-2">
                <label className="field">
                  ...
                  <input
                    type="date"
                    value={showForm.show_start}
                    onChange={(e) =>
                      setShowForm((c) => ({ ...c, show_start: e.target.value }))
                    }
                  />
                </label>
                <label className="field">
                  ...
                  <input
                    type="date"
                    value={showForm.show_end}
                    onChange={(e) =>
                      setShowForm((c) => ({ ...c, show_end: e.target.value }))
                    }
                  />
                </label>
              </div>
            ...
            <div className="card compact accent-card">
              <div style={{ fontWeight: 800, marginBottom: 10 }}>
                Crew schedule message details
              </div>
              <div className="grid grid-2">
                <label className="field">
                  <span>Meet-up location</span>
                  <input
                    value={showForm.meet_up_location}
                    onChange={(e) =>
                      setShowForm((c) => ({
                        ...c,
                        meet_up_location: e.target.value,
                      }))
                    }
                    placeholder="3rd Floor Bissonet Ballroom"
                  />
                </label>
                <label className="field">
                  <span>Default message rate / hr</span>
                  <input
                    value={showForm.default_hourly_rate}
                    onChange={(e) =>
                      setShowForm((c) => ({
                        ...c,
                        default_hourly_rate: e.target.value.replace(
                          /[^0-9.]/g,
                          "",
                        ),
                      }))
              ...
              <div className="grid grid-2" style={{ marginTop: 10 }}>
                <label className="field">
                  <span>Crew lead name</span>
                  <input
                    value={showForm.crew_lead_name}
                    onChange={(e) =>
                      setShowForm((c) => ({
                        ...c,
                        crew_lead_name: e.target.value,
                      }))
                    }
                ...
                <label className="field">
                  <span>Crew lead phone</span>
                  <input
                    value={showForm.crew_lead_phone}
                    onChange={(e) =>
                      setShowForm((c) => ({
                        ...c,
                        crew_lead_phone: e.target.value,
                      }))
                    }
                    placeholder="310-698-9092"
                  />
                </label>
              </div>
              <div className="grid grid-2" style={{ marginTop: 10 }}>
                <label className="field">
                  ...
                  <input
                    value={showForm.coordinator_name}
                    onChange={(e) =>
                      setShowForm((c) => ({
                        ...c,
                ...
                <label className="field">
                  ...
                  <input
                    value={showForm.coordinator_phone}
                    onChange={(e) =>
                      setShowForm((c) => ({
                        ...c,
            ...
            <label className="field">
              ...
              <textarea
                ...
                onChange={(e) =>
                  setShowForm((c) => ({ ...c, notes: e.target.value }))
                }
              />
            </label>
            <div className="toolbar">
          ...
          <div className="grid" style={{ gap: 14, marginTop: 12 }}>
            <div className="grid grid-2">
              <label className="field">
                ...
                <input
                  type="date"
                  value={dayForm.labor_date}
                  onChange={(e) =>
                    setDayForm((c) => ({ ...c, labor_date: e.target.value }))
                  }
                />
              </label>
              <label className="field">
                ...
                <input
                  value={dayForm.label}
                  onChange={(e) =>
                    setDayForm((c) => ({ ...c, label: e.target.value }))
                  }
                  placeholder="Load in, show day, strike..."
                />
              </label>
            </div>
            <label className="field">
              ...
              <textarea
                ...
                value={dayForm.notes}
                onChange={(e) =>
                  setDayForm((c) => ({ ...c, notes: e.target.value }))
                }
              />
            </label>
            <div className="toolbar">
        ...
        {editorMode === "call" ? (
          <div className="grid" style={{ gap: 14, marginTop: 12 }}>
            <div className="small muted">
              Labor day:{" "}
              {visibleLaborDays.find((day) => day.id === editingDayTargetId)
                ?.labor_date || "None selected"}
            </div>
            <div className="grid grid-2">
              <label className="field">
                <span>Area</span>
                <input
                  value={callForm.area}
                  onChange={(e) =>
                    setCallForm((c) => ({ ...c, area: e.target.value }))
                  }
                  placeholder="Booth, GS, breakouts..."
                />
              </label>
              <label className="field">
                <span>Role</span>
                <input
                  ...
                  onChange={(e) =>
                    setCallForm((c) => ({ ...c, role_name: e.target.value }))
                  }
                  placeholder="General AV, LED Assist..."
                />
              </label>
            </div>
            <div
            ...
            >
              <label className="field">
                ...
                <input
                  type="time"
                  value={callForm.start_time}
                  onChange={(e) =>
                    setCallForm((c) => ({ ...c, start_time: e.target.value }))
                  }
                />
              </label>
              <label className="field">
                ...
                <input
                  type="time"
                  value={callForm.end_time}
                  onChange={(e) =>
                    setCallForm((c) => ({ ...c, end_time: e.target.value }))
                  }
                />
              </label>
              <label className="field">
                ...
                <input
                  type="number"
                  min="1"
                  value={callForm.crew_needed}
                  onChange={(e) =>
                    setCallForm((c) => ({ ...c, crew_needed: e.target.value }))
                  }
                />
              </label>
            </div>
            <label className="field">
              ...
              <textarea
                ...
                onChange={(e) =>
                  setCallForm((c) => ({ ...c, notes: e.target.value }))
                }
              />
            </label>
            <div className="toolbar">
  ...
  return (
    <div className="grid" style={{ gap: 16 }}>
      {msg ? (
        <p className={msg.kind === "error" ? "error" : "success"}>{msg.text}</p>
      ) : null}
      {checklistOpen ? (
        <div
          ...
          role="dialog"
          aria-modal="true"
          aria-label="Event crew checklist"
        >
          <div className="card checklist-modal">
            <div className="row" style={{ alignItems: "flex-start" }}>
              <div>
                <h2 style={{ marginBottom: 4 }}>Crew checklist</h2>
                <p className="muted small" style={{ margin: 0 }}>
                  {selectedShow?.name || "Selected event"} •{" "}
                  {checklistNames.length} assigned crew
                </p>
              </div>
              <button
                type="button"
            ...
            <div className="toolbar" style={{ marginTop: 12 }}>
              <button type="button" className="ghost" onClick={checkAllCrew}>
                Check all
              </button>
              <button type="button" className="ghost" onClick={clearChecklist}>
                Clear
              </button>
              <button
                type="button"
              ...
              <button type="button" className="ghost" onClick={exportChecklist}>
                Export checklist
              </button>
            </div>
            <div className="checklist-modal-list">
              {checklistNames.length ? (
                checklistNames.map((name) => {
                  const key = normalize(name);
                  const checked = checkedCrew.has(key);
                  return (
                    <label
                      key={key}
                    ...
                    >
                      <input
                        type="checkbox"
                        checked={checked}
                        onChange={() => toggleChecklistName(name)}
                      />
                      <span>{name}</span>
                    </label>
                  );
                ...
                <p className="muted">No assigned crew yet.</p>
              )}
            </div>
          </div>
        </div>
      ...
      <div
        className="grid events-workspace"
        style={{ gridTemplateColumns: "320px minmax(0,1fr)", gap: 16 }}
      >
        <section className="card">
          <div className="row" style={{ alignItems: "flex-start" }}>
            <div>
              ...
              <p className="muted small" style={{ marginTop: 0 }}>
                Clean overview first, edit tools second. Import and preview from
                the event list.
              </p>
            </div>
          ...
          <label className="field">
            ...
            <input
              ...
              placeholder="Show, client, venue, city..."
            />
          </label>
          ...
          <div className="list" style={{ marginTop: 16 }}>
            ...
            >
              <div className="small muted">
                Drag a CSV or PDF here, preview first, then create new or merge
              ...
              <div className="toolbar" style={{ marginTop: 10 }}>
                ...
                {importFile ? (
                  <span className="small">{importFile.name}</span>
                ) : (
                  <span className="small muted">No file selected</span>
                )}
              </div>
              <input
                ref={fileInputRef}
            ...
            {importMode === "merge" ? (
              <p className="small muted">
                Selected event for merge:{" "}
                <strong>{selectedShow?.name || "Choose an event first"}</strong>
              </p>
            ) : null}

            <label className="field">
              <span>Show name override</span>
              <input
                value={importForm.show_name}
                onChange={(e) =>
                  setImportForm((c) => ({ ...c, show_name: e.target.value }))
                }
            ...
            <label className="field">
              ...
              <input
                value={importForm.client}
                onChange={(e) =>
                  setImportForm((c) => ({ ...c, client: e.target.value }))
                }
                placeholder="Optional override"
              />
            </label>
            <label className="field">
              ...
              <input
                value={importForm.venue}
                onChange={(e) =>
                  setImportForm((c) => ({ ...c, venue: e.target.value }))
                }
                placeholder="Optional override"
              />
            </label>
            <label className="field">
              <span>Rate city</span>
              <input
                value={importForm.rate_city}
                onChange={(e) =>
                  setImportForm((c) => ({ ...c, rate_city: e.target.value }))
                }
            ...
            <div className="toolbar">
              ...
              <button
                type="button"
                className="primary"
                disabled={importing || importMode === "preview"}
                onClick={runImport}
              >
                {importing
                  ? "Working..."
            ...
            {importPreview ? (
              <div className="card compact" style={{ marginTop: 12 }}>
                ...
                <div className="small muted">
                  {importPreview.show.client || "No client"} •{" "}
                  {importPreview.show.venue || "No venue"}
                </div>
                <div className="small muted">
                  {importPreview.show.show_start} to{" "}
                  {importPreview.show.show_end} •{" "}
                  {importPreview.sourceType.toUpperCase()}
                </div>
                <div className="small" style={{ marginTop: 8 }}>
                  {importPreview.laborDays.length} labor days •{" "}
                  {importPreview.subCallPreview.length} sub-calls •{" "}
                  {importPreview.matchedCrewCount} matched •{" "}
                  {importPreview.unmatchedCrewCount} unmatched
                </div>
                <div
                  className="list"
                  ...
                  {importPreview.subCallPreview.map((call) => (
                    <div key={call.key} className="card compact">
                      <div style={{ fontWeight: 700 }}>
                        {call.labor_date} • {call.start_time}
                        {call.end_time ? `-${call.end_time}` : ""}
                      </div>
                      <div className="small muted">
                        {call.area} • {call.role_name} • {call.crew_needed}{" "}
                        needed
                      </div>
                      {call.matchedCrew.length ? (
                        <div className="small">
                          Matched:{" "}
                          {call.matchedCrew.map((row) => row.name).join(", ")}
                        </div>
                      ) : null}
                      {call.unmatchedCrew.length ? (
                        <div className="small muted">
                          Unmatched: {call.unmatchedCrew.join(", ")}
                        </div>
                      ) : null}
                    </div>
                  ))}
              ...
              {(showsByBucket[bucket] ?? []).map((show) => (
                <button
                  ...
                  style={{
                    textAlign: "left",
                    borderColor:
                      selectedShow?.id === show.id ? "#111827" : undefined,
                  }}
                >
                  <div style={{ fontWeight: 700 }}>{show.name}</div>
                  <div className="small muted">
                    {show.client || "No client"} • {show.show_start} to{" "}
                    {show.show_end}
                  </div>
                  <div className="small muted">{show.venue || "No venue"}</div>
                </button>
              ))}
              {!showsByBucket[bucket].length ? (
                <p className="small muted">No {bucket.toLowerCase()} shows.</p>
              ) : null}
            </div>
          ))}
        </section>
        ...
        <section className="card event-panel">
          <div className="row event-panel-header">
            <div>
              <h3 style={{ marginBottom: 6 }}>Event overview</h3>
              <p className="muted small" style={{ marginTop: 0 }}>
                Review the show first. Open edit mode only when you need to
                change details.
              </p>
            </div>
            <div className="toolbar">
              <button
                type="button"
                className={viewMode === "overview" ? "primary" : "ghost"}
                onClick={() => {
                  setViewMode("overview");
                  setEditorMode(null);
                }}
              ...
              <button
                type="button"
                className={viewMode === "edit" ? "primary" : "ghost"}
                onClick={() => selectedShow && startEditEvent(selectedShow)}
          ...
          {selectedShow ? (
            <div className="grid" style={{ gap: 16 }}>
              <div className="card compact event-summary-card">
                <div
                  className="row event-summary-main"
                ...
                >
                  <div>
                    ...
                    <div className="muted">
                      {selectedShow.client || "No client"} •{" "}
                      {selectedShow.venue || "No venue"}
                    </div>
                    <div className="muted">
                      {selectedShow.show_start} to {selectedShow.show_end} •{" "}
                      {showBucket(selectedShow)} • Rate city:{" "}
                      {selectedShow.rate_city || "Default"}
                    </div>
                    {eventPool ? (
                      <div className="small" style={{ marginTop: 8 }}>
                        <strong>Staffing pool:</strong> {eventPool}
                      </div>
                    ) : null}
                  </div>
                ...
                <div className="toolbar small" style={{ marginTop: 10 }}>
                  {selectedShowMessageMeta.meet_up_location ? (
                    <span className="badge">
                      <strong>Meet-up:</strong>{" "}
                      {selectedShowMessageMeta.meet_up_location}
                    </span>
                  ) : null}
                  {selectedShowMessageMeta.default_hourly_rate ? (
                    <span className="badge">
                      <strong>Message rate:</strong> $
                      {selectedShowMessageMeta.default_hourly_rate}/hr
                    </span>
                  ) : null}
                  {selectedShowMessageMeta.crew_lead_name ? (
                    <span className="badge">
                      <strong>Crew lead:</strong>{" "}
                      {selectedShowMessageMeta.crew_lead_name}
                      {selectedShowMessageMeta.crew_lead_phone
                        ? ` · ${formatPhone(selectedShowMessageMeta.crew_lead_phone)}`
                        : ""}
                    </span>
                  ) : null}
                </div>
                {selectedShowNoteParts.displayNotes ? (
                  <div className="small muted" style={{ marginTop: 10 }}>
                    {selectedShowNoteParts.displayNotes}
                  </div>
                ) : null}
                <div className="toolbar" style={{ marginTop: 12 }}>
                  <label className="field" style={{ minWidth: 230 }}>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        setEventDisplayMode(e.target.value as EventDisplayMode)
                      }
                    >
                  ...
                  {eventDisplayMode === "messages" ? (
                    <button
                      type="button"
                      className="ghost"
                      onClick={exportScheduleMessages}
                    >
                      Export iMessage Schedules
                    </button>
                  ) : (
                    <button
                      type="button"
                    ...
                    >
                      Export {eventDisplayMode === "booth" ? "Booth" : "Day"}{" "}
                      Crew List
                    </button>
                  )}
                  <button
                  ...
                  <button
                    type="button"
                    className="ghost"
                    onClick={() => startEditEvent(selectedShow)}
                  >
                    Edit Event
                  </button>
                  <button type="button" className="ghost" onClick={startAddDay}>
                    Add Labor Day
                  </button>
                  <button
                    type="button"
              ...
              {editorMode === "show" || (editorMode === "day" && !editingDayId)
                ? renderEditorPanel()
                : null}
                ...
                <div className="list">
                  <div className="row">
                    <div>
                      ...
                      <div className="small muted">
                        One copy-ready schedule per assigned crew member for
                        this event.
                      </div>
                    </div>
                    <button
                  ...
                  {crewScheduleMessages.length ? (
                    crewScheduleMessages.map((message) => (
                    ...
                    <p className="small muted">
                      No assigned crew schedules yet.
                    </p>
                  )}
                </div>
                ...
                <div className="list">
                  <div className="row">
                    <div>
                      ...
                      <div className="small muted">
                        Separated by date with clear day breaks.
                      </div>
                    </div>
                  </div>
                  {visibleLaborDays.length ? (
                    visibleLaborDays.map((day) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          {editorMode === "day" && editingDayId === day.id
                            ? renderEditorPanel()
                            : null}
                          {editorMode === "call" &&
                          !editingCallId &&
                          editingDayTargetId === day.id
                            ? renderEditorPanel()
                            : null}
                          ...
                          {isOpen ? (
                            ...
                            >
                              {day.notes ? (
                                <div className="small muted">{day.notes}</div>
                              ) : null}
                              {dayCalls.length ? (
                                dayCalls.map((call) =>
                                  renderCallCard(day, call),
                                )
                              ) : (
                                <div className="small muted">
                                  No sub-calls yet for this labor day.
                                </div>
                              )}
                            </div>
                          ) : null}
                    ...
                    <p className="small muted">
                      No labor days yet for this event.
                    </p>
                  )}
                </div>
              ) : (
                <div className="list">
                  <div className="row">
                    <div>
                      ...
                      <div className="small muted">
                        Same event data grouped like the imported crew lists:
                        booth first, then dates and calls.
                      </div>
                    </div>
                    <button
                      type="button"
                  ...
                  {boothSections.length ? (
                    boothSections.map((section) => {
                      const dates = [
                        ...new Set(
                          section.calls.map((item) => item.day.labor_date),
                        ),
                      ];
                      return (
                        <div
                        ...
                        >
                          ...
                          >
                            <div>
                              ...
                              <div className="small muted">
                                {section.calls.length} sub-call
                                {section.calls.length === 1 ? "" : "s"}
                              </div>
                            </div>
                          </div>
                          <div
                              ...
                              return (
                                ...
                                >
                                  ...
                                  >
                                    {callsForDate.map(
                                      ({ day: callDay, call }) =>
                                        renderCallCard(callDay, call, true),
                                    )}
                                  </div>
                                </div>
                              );
                    ...
                    <p className="small muted">
                      No booth or sub-call information yet for this event.
                    </p>
                  )}
                </div>
              )}
            ...
            <div className="card compact">
              ...
              <div className="small muted" style={{ marginTop: 8 }}>
                Choose an event from the list on the left or add a new one.
              </div>
