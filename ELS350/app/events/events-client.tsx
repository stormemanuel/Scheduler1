  ...
  async function saveCall() {
    ...
    if (editingCallId) {
      ...
      for (const targetCall of targetCalls) {
        const payload = { ...basePayload, labor_day_id: targetCall.labor_day_id, sort_order: Number(targetCall.sort_order || 0) };
      ...
      if (extraItems.length) {
        const data = await request("/api/sub-calls", "POST", { items: extraItems });
        createdCalls = ((data?.rows ?? []) as SubCallRecord[]).map(sanitizeSubCall).filter((call) => call.id && call.labor_day_id);
        if (createdCalls.length !== extraItems.length) {
      ...
      setCrewPickerCallId(null);
      setExpandedDayIds((current) => [...new Set([...current, ...targetDayIds, ...extraDayIds])]);
      ...
      setMsg({ kind: "success", text: extraPositionPayloads.length ? `${updatedText}; ${createdText}.${skippedText}` : `${updatedText}.` });
    } else {
        ...
        {editorMode === "call" ? (
          <div className="grid" style={{ gap: 14, marginTop: 12 }}>
            ...
            {!editingCallId ? (
              <div className="card compact sub-call-multi-day">
                <div className="row" style={{ alignItems: "flex-start" }}>
                  ...
                  <button type="button" className="ghost" onClick={() => setCallTargetDayIds(visibleLaborDays.map((day) => day.id))} disabled={!visibleLaborDays.length}>Select all days</button>
                </div>
                    ...
                    <label className="checkbox-line" style={{ margin: 0 }}>
                      <input
                        ...
                        onChange={(event) => {
                          ...
                          if (checked) setExtraPositionTargetDayIds([...new Set(seriesItems.map((item) => item.day.id))]);
                        }}
                      />
            ...
            <div className="toolbar">
              ...
              <button type="button" className="ghost" onClick={() => { setEditingCallId(null); setEditingDayTargetId(null); setCallTargetDayIds([]); setExtraPositionTargetDayIds([]); setEditCallApplyAllDays(false); setEditCallTargetIds([]); setCallForm(emptyCall); setMultiPositionRows([]); setEditorMode(null); setViewMode("overview"); }}>Cancel</button>
            </div>
          </div>
        ) : null}
      </div>
