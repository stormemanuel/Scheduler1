...
type ReminderPlanRow = {
  ...
  timezone: string;
  result: "will_schedule" | "overdue" | "already_scheduled" | "already_sent" | "already_cancelled" | "skipped" | "no_phone";
  result_label: string;
  message_type: string;
  reminder_key: string;
  body: string;
...
type ImmediateSchedulePlanRow = {
  ...
  scheduled_for: string;
  scheduled_local: string;
  result: "will_queue" | "already_scheduled" | "already_sent" | "already_cancelled" | "no_phone";
  result_label: string;
...
function activeAssignmentRows(bundle: Awaited<ReturnType<typeof fetchShowBundle>>) {
  ...
  return bundle.assignments
    ...
    .filter(({ assignment }) => {
      ...
      return !["removed", "cancelled", "canceled", "declined", "unavailable"].includes(status);
    })
    .sort((a, b) =>
...
function resultForExisting(status: string | null | undefined): ReminderPlanRow["result"] | null {
  ...
  if (clean === "cancelled" || clean === "canceled") return "already_cancelled";
...
async function buildFullShowReminderPlan(
  ...
  const baseByCrew = nextScheduleByCrew(bundle.show, bundle.days, bundle.calls, bundle.assignments, bundle.crewRows, bundle.masterRates, settings, senderName);
  ...
  const existingRes = await admin
    .from("text_message_queue")
    .select("crew_id, message_type, reminder_key, status")
    .eq("show_id", settings.show_id)
    .limit(5000);
  if (existingRes.error) throw new Error(existingRes.error.message);
  const existing = new Map<string, string>();
  for (const row of (existingRes.data || []) as Array<{ crew_id: string | null; message_type: string | null; reminder_key: string | null; status: string | null }>) {
    existing.set(`${row.crew_id || ""}|${row.message_type || ""}|${row.reminder_key || ""}`, row.status || "scheduled");
  }
  ...
  for (const [crewId, item] of baseByCrew.entries()) {
    ...
    const pushPlan = (args: { type: string; enabled: boolean; dueIso: string; messageType: string; reminderKey: string; body: string; row: typeof firstRow }) => {
      ...
      const existingResult = resultForExisting(existingStatus);
      ...
      plan.push({
        ...
        assignment_label: `${formatDate(args.row.day.labor_date)} ${assignmentTimeRange(args.row.assignment, args.row.call)} · ${args.row.call.role_name || "Crew"}${args.row.call.area ? ` · ${args.row.call.area}` : ""}`,
...
function queueRowsFromReminderPlan(showId: string, plan: ReminderPlanRow[], user: { id: string; email?: string | null; display_name?: string | null }, includeOverdueNow: boolean) {
  const nowIso = new Date(Date.now() + 30_000).toISOString();
  const rows = plan.filter((row) => row.result === "will_schedule" || (includeOverdueNow && row.result === "overdue"));
  return stampRows(rows.map((row) => ({
    ...
    scheduled_for: row.result === "overdue" ? nowIso : row.scheduled_for,
    status: "scheduled",
    body: row.body,
  })), user);
}
...
async function buildImmediateFullShowSchedulePlan(
  ...
  const byCrew = nextScheduleByCrew(bundle.show, bundle.days, bundle.calls, bundle.assignments, bundle.crewRows, bundle.masterRates, settings, senderName);
  const existingRes = await admin
    .from("text_message_queue")
    .select("crew_id, message_type, reminder_key, status")
    .eq("show_id", settings.show_id)
  ...
  for (const row of (existingRes.data || []) as Array<{ crew_id: string | null; message_type: string | null; reminder_key: string | null; status: string | null }>) {
    ...
    if (key === "full_show_schedule" || key.startsWith("manual_7_day")) {
      existing.set(`${row.crew_id || ""}|schedule|full_show_schedule`, row.status || "scheduled");
    }
  }
  ...
  for (const [crewId, item] of byCrew.entries()) {
    const phone = cleanPhone(item.crew.phone || "");
    const existingStatus = existing.get(`${crewId}|schedule|full_show_schedule`);
    const existingResult = resultForExisting(existingStatus);
    const body = ensureSenderIdentity(applyTemplate(settings.schedule_template || defaultScheduleTemplate, item.values), senderName);
    const result: ImmediateSchedulePlanRow["result"] = !phone
      ? "no_phone"
      : existingResult === "already_sent"
        ? "already_sent"
        : existingResult === "already_cancelled"
          ? "already_cancelled"
          : existingResult
            ? "already_scheduled"
            : "will_queue";
    ...
    rows.push({
      ...
      result_label: resultLabel,
      message_type: "schedule",
      reminder_key: "full_show_schedule",
      body: note ? `${body}\n\n${note}` : body,
    });
  }
...
function queueRowsFromImmediateSchedulePlan(showId: string, plan: ImmediateSchedulePlanRow[], user: { id: string; email?: string | null; display_name?: string | null }) {
  return stampRows(plan.filter((row) => row.result === "will_queue").map((row) => ({
    show_id: showId,
    crew_id: row.crew_id,
    crew_name: row.crew_name,
    phone: row.phone,
...
function combinedScheduleSummary(schedulePlan: ImmediateSchedulePlanRow[], reminderPlan: ReminderPlanRow[]) {
  const scheduleNow = schedulePlan.filter((row) => row.result === "will_queue").length;
  const future = reminderPlan.filter((row) => row.result === "will_schedule").length;
...
async function saveSettings(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, settings: AutomationSettings) {
  ...
  const { reminder_daily_after_first_day: _unsavedDailyReminder, ...persistedSettings } = settings;
  const payload = {
    ...persistedSettings,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (action === "preview_combined_schedule_reminders" || action === "send_combined_schedule_reminders") {
      ...
      const saved = action === "send_combined_schedule_reminders" ? await saveSettings(admin, effectiveSettings) : null;
      ...
      const [schedulePlan, reminderPlan] = await Promise.all([
        buildImmediateFullShowSchedulePlan(admin, effectiveSettings, bundle, senderName, safeText(body.additional_note)),
        buildFullShowReminderPlan(admin, effectiveSettings, bundle, senderName),
      ]);
    ...
    if (action === "preview_checked_reminders" || action === "schedule_checked_reminders") {
      ...
      const settings = normalizeSettings(body.settings || {}, showId);
      const effectiveSettings = { ...settings, enabled: true, send_schedule: true };
      const saved = action === "schedule_checked_reminders" ? await saveSettings(admin, effectiveSettings) : null;
      const bundle = await fetchShowBundle(admin, showId);
      const senderName = senderNameForMessage(auth.user);
