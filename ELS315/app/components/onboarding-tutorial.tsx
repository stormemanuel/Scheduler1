...
async function loadInboxUnreadCount() {
  const [coordinatorResponse, directResponse] = await Promise.all([
    fetch("/api/user-access?action=coordinator_notifications", { cache: "no-store" }),
  ...
  return Math.max(0, coordinatorCount) + Math.max(0, directCount);
}
...
export function OnboardingReviewNavLink({ href, label }: OnboardingNavLinkProps) {
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => void loadReviewCount(), 180000);
...
export function CoordinatorNotificationNavLink({ href, label }: OnboardingNavLinkProps) {
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => void loadUnreadCount(), 180000);
...
export function AppBadgeSync({ role }: { role: string }) {
  useEffect(() => {
    ...
    void loadCount();
    const intervalId = window.setInterval(() => void loadCount(), 180000);
...
export function CoordinatorMessagesPanel({ role }: { role: string }) {
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => void loadRows(), 180000);
    const directIntervalId = window.setInterval(() => void loadDirectMessages(), 180000);
    window.addEventListener("focus", loadRows);
    window.addEventListener("focus", loadDirectMessages);
    return () => {
      window.clearInterval(intervalId);
      window.clearInterval(directIntervalId);
      window.removeEventListener("focus", loadRows);
      window.removeEventListener("focus", loadDirectMessages);
    };
  }, [role]);
  ...
  return (
    <section id="assignment-chat" className="card">
      ...
      <div className="card compact" style={{ marginTop: 12, boxShadow: "none" }}>
        ...
        {directSetupMissing ? <p className="error">Run the direct message SQL to enable user-to-user messages.</p> : null}
