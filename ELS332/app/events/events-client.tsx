...
function EventsClientInner({
  ...
  async function updateCoordinatorDailyCheck(checklistId: string, row: CoordinatorDailyCheckRecord, checked: boolean) {
    ...
    const data = await request("/api/assignment-checklists", "POST", {
      ...
      signed_out_count: row.signed_out_count || 0,
      missing_count: row.missing_count || 0,
      replacement_needed: row.replacement_needed,
      replacement_assigned: row.replacement_assigned,
  ...
  async function addCoordinatorIncident(checklistId: string) {
    if (!selectedShow) return;
    const data = await request("/api/assignment-checklists", "POST", {
      scope: "coordinator_process",
      action: "add_incident",
      ...
      checklist_id: checklistId,
      incident_type: "Other",
      notes: "Incident reported from coordinator checklist.",
    });
    applyCoordinatorChecklistPayload(data);
  }
                ...
                <div key={checklist.id} className="card compact" style={{ background: percent === 100 ? "#f0fdf4" : overdue ? "#fff1f2" : "#fffdf5" }}>
                  ...
                  <div className="card compact" style={{ boxShadow: "none", marginTop: 12 }}>
                    ...
                    <div className="list" style={{ marginTop: 8 }}>
                      {dailyRows.map((row) => (
                        <label key={row.id} className="checkbox-line" style={{ alignItems: "flex-start" }}>
                          <input
                            type="checkbox"
                          ...
                          <span>
                            ...
                            <small style={{ display: "block" }}>Scheduled {row.scheduled_count ?? 0} • signed in {row.signed_in_count ?? 0} • signed out {row.signed_out_count ?? 0} • missing {row.missing_count ?? 0}</small>
                            <small style={{ display: "block" }}>Replacement needed: {row.replacement_needed ? "Yes" : "No"} • assigned: {row.replacement_assigned ? "Yes" : "No"}</small>
                  ...
                  <div className="card compact" style={{ boxShadow: "none", marginTop: 12 }}>
                    <div className="row">
                      ...
                      <button type="button" className="ghost" disabled={saving} onClick={() => void addCoordinatorIncident(checklist.id)}>Add incident</button>
                    </div>
