...
type SubCallRow = { id: string; labor_day_id: string; area: string | null; location?: string | null; role_name: string | null; start_time: string | null; end_time: string | null; crew_needed: number | null; notes: string | null; sort_order?: number | null; day_type?: string | null; one_hour_walkaway?: boolean | null };
...
function nextScheduleByCrew(show: ShowRow, days: LaborDayRow[], calls: SubCallRow[], assignments: AssignmentRow[], crewRows: CrewRow[], settings: AutomationSettings) {
  ...
  const result = new Map<string, { crew: CrewRow; lines: string[]; meetUps: string[]; firstCall: { day: LaborDayRow; call: SubCallRow; assignment: AssignmentRow } | null; values: Record<string, string> }>();
  ...
  for (const assignment of [...assignments].sort((a, b) => Number(a.sort_order || 0) - Number(b.sort_order || 0))) {
    ...
    const current = result.get(crew.id) || { crew, lines: [], meetUps: [], firstCall: null, values: {} as Record<string, string> };
    const blockLabel = dayTypeLabel(assignmentDayType(assignment, call));
    const rateText = meta.default_hourly_rate ? `$${meta.default_hourly_rate}/hr` : "Rate TBD";
...
async function fetchShowBundle(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, showId: string) {
  ...
  const callsRes = dayIds.length ? await admin.from("sub_calls").select("id, labor_day_id, area, location, po_number, role_name, start_time, end_time, crew_needed, notes, sort_order, day_type, one_hour_walkaway").in("labor_day_id", dayIds) : { data: [], error: null };
...
function buildCustomQueueRows(
  ...
  const byCrew = nextScheduleByCrew(bundle.show, targetDays, targetCalls, targetAssignments, bundle.crewRows, settings);
