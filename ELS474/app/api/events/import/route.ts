...
type ImportMasterRateRow = { id: string; city_name: string | null; role_name: string | null };

function masterRateForImportedRole(rates: ImportMasterRateRow[], roleName: string | null | undefined, rateCity: string | null | undefined) {
  const roleKey = normalizeImportRole(roleName);
  if (!roleKey) return null;
  const cityKey = normalizeMatchValue(rateCity || "Default") || "default";
  const matches = rates.filter((rate) => normalizeImportRole(rate.role_name) === roleKey);
  return matches.find((rate) => (normalizeMatchValue(rate.city_name || "Default") || "default") === cityKey)
    || matches.find((rate) => (normalizeMatchValue(rate.city_name || "Default") || "default") === "default")
    || matches[0]
    || null;
}
...
function makeSubCallSeriesKey(call: { area?: string | null; role_name?: string | null; po_number?: string | null; booth_number?: string | null; sub_call_reference_number?: string | null; notes?: string | null }) {
  ...
  const reference = normalizeImportReference(call.booth_number || boothNumberFromNotes(call.notes) || legacyBoothFromReference(call.sub_call_reference_number || subCallReferenceNumberFromNotes(call.notes)));
  return [
...
function applyImportPreviewOverrides(payload: ImportPayloadShape, preview: ImportPreviewShape, rawOverrides: string) {
  ...
  preview.subCallPreview = preview.subCallPreview.map((call, index) => {
    ...
    return {
      ...
      booth_number: editedCall.booth_number || null,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const masterRatesRes = await admin
      .from("master_rates")
      .select("id, city_name, role_name")
      .limit(5000);
    if (masterRatesRes.error) {
      return NextResponse.json({ message: `Could not load Crew Rate positions for import: ${masterRatesRes.error.message}` }, { status: 400 });
    }
    const importMasterRates = (masterRatesRes.data ?? []) as ImportMasterRateRow[];
    const importRateCity = showRow.rate_city || payload.show.rate_city || "Default";
    payload.subCallGroups = payload.subCallGroups.map((call) => {
      const normalizedRole = normalizeImportedRoleName(call.role_name);
      const matchedRate = masterRateForImportedRole(importMasterRates, normalizedRole, importRateCity);
      return { ...call, role_name: matchedRate?.role_name || normalizedRole };
    });
    ...
    const existingSubCallsRes = dayIds.length
      ? await admin
          .from("sub_calls")
          .select("id, labor_day_id, area, po_number, sub_call_group_id, role_name, master_rate_id, message_rate, start_time, end_time, crew_needed, notes, day_type")
    ...
    const subCallGroupIdForImport = (call: { area: string; role_name: string; po_number?: string | null; booth_number?: string | null; sub_call_reference_number?: string | null; notes?: string | null }) => {
    ...
    for (const change of selectedChangedImportChanges) {
      ...
      if (change.fields?.booth_number || change.fields?.sub_call_reference_number) {
        const withReference = notesWithSubCallReferenceMarker(existingCall.notes || importedCall.notes || null, importedCall.sub_call_reference_number || null);
        patch.notes = notesWithBoothNumberMarker(withReference, importedCall.booth_number || null);
      }
      ...
      const insertRes = await admin
        ...
        .insert(
          missingSubCalls.map((call) => ({
            ...
            master_rate_id: masterRateForImportedRole(importMasterRates, call.role_name, importRateCity)?.id || null,
            message_rate: call.message_rate || null,
            start_time: call.start_time,
            end_time: call.end_time || null,
            crew_needed: call.crew_needed,
            notes: notesWithBoothNumberMarker(notesWithSubCallReferenceMarker(call.notes || null, call.sub_call_reference_number || null), call.booth_number || null),
    ...
    const importedScheduleChanged = createdLaborDays.length > 0 || createdSubCalls.length > 0 || updatedSubCallCount > 0 || assignmentRows.length > 0;
    if (mode === "merge" && importedScheduleChanged) {
      scheduleInvalidation = await invalidateShowScheduleStatus(admin, [showRow.id], {
        reason: "Import merge changed positions, dates, times, or crew. Send updated schedules.",
