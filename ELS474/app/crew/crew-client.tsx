...
function ContactDetail({ record, ratingSummary, cityPools, targetGroups, bulkCityId, bulkGroupName, detailTab, onTab, onEdit, onDelete, onAcknowledgeCoordinatorHidden, onRestoreCoordinatorHidden, onMove, onQueueOnboarding, onQueueW9Only, onQueueContractOnly, onQueueProfilePhotoOnly, onAdminDocumentUploaded, onBulkCityChange, onBulkGroupChange, saving, canSeeAdminAudit, canSendOnboarding, directAndroidOnboarding }: ContactDetailProps) {
  ...
  function showOnboardingEmailSuccessToast(recipientEmail: string) {
    ...
    if (onboardingEmailToastTimerRef.current) clearTimeout(onboardingEmailToastTimerRef.current);
    onboardingEmailToastTimerRef.current = setTimeout(() => setOnboardingEmailSuccessToast(""), 4500);
  }
          ...
          {canSendOnboarding ? (
            <div style={{ display: "grid", gap: 6 }}>
              ...
              <button
                className="primary"
                type="button"
                disabled={saving || onboardingMessageLoading || !record.email}
                onClick={() => void sendFullOnboardingByEmailFromContact()}
                style={{ background: "#16843f", borderColor: "#16843f" }}
              >
                {record.email ? "Send onboarding by email" : "Email needed for onboarding"}
              </button>
            </div>
          ) : null}
          {canSeeAdminAudit ? (
          ...
          <div className="card compact" style={{ background: "#fbfcfd" }}>
            <div className="row" style={{ alignItems: "flex-start", gap: 12 }}>
              ...
              <div className="toolbar" style={{ justifyContent: "flex-end", flexWrap: "wrap" }}>
                <button className="primary" type="button" disabled={saving || onboardingMessageLoading || !record.phone} onClick={() => void sendFullOnboardingFromContact()}>
                  {record.phone ? (onboardingMessageLoading ? "Preparing..." : "Send full onboarding") : "Phone needed"}
                </button>
                <button className="primary" type="button" disabled={saving || onboardingMessageLoading || !record.email} onClick={() => void sendFullOnboardingByEmailFromContact()} style={{ background: "#16843f", borderColor: "#16843f" }}>
                  {record.email ? (onboardingMessageLoading ? "Preparing..." : "Send onboarding by email") : "Email needed"}
                </button>
              </div>
            </div>
            <div className="grid grid-2">
              ...
              {!onboardingMessageLoading && onboardingMessageMaterial?.message_body ? (
                <>
                  ...
                  <div className="toolbar" style={{ marginTop: 10, flexWrap: "wrap" }}>
                    {directAndroidOnboarding && smsHrefWithBody(onboardingMessageMaterial.phone || record.phone, onboardingMessageMaterial.message_body) ? (
                      <a
                        className="primary"
                    ...
                    {record.email ? (
                      <button
                        className="primary"
                        type="button"
                        disabled={saving || onboardingMessageLoading}
                        onClick={() => void sendFullOnboardingByEmailFromContact()}
                        style={{ background: "#16843f", borderColor: "#16843f" }}
                      >
                        {onboardingMessageLoading ? "Sending..." : "Send onboarding by email"}
                      </button>
                    ) : null}
                    <button
                      className="ghost"
                ...
                <p className="muted small">No active secure onboarding link is available yet. Press Send full onboarding to create one.</p>
              ) : null}
