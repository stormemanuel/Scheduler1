...
function buildUniversalShortcutPollUrl(origin: string, token: string) {
  ...
  return `${origin}/api/text-automation/shortcut?all=1&token=${encodeURIComponent(token)}&limit=5`;
}
...
function buildShortcutInstallUrl(pollUrl: string) {
  ...
  const installNote = encodeURIComponent(`ELS Shortcut API URL:\n${pollUrl}\n\nBuild the shortcut with: URL → Get Contents of URL → Get Dictionary Value messages → Repeat each item → Send body to phone → Get Contents of mark_sent_url.`);
        ...
        <div className="card compact">
            ...
            <div className="card compact accent-card" style={{ marginTop: 14 }}>
              <div className="row" style={{ alignItems: "flex-start" }}>
                <div>
                  ...
                  <div className="small muted">
                    Save settings first. The app now provides a universal Shortcut URL, so one iPhone Shortcut can check all active events that use this same Shortcut token.
                  </div>
                </div>
              ...
              <div className="toolbar">
                <button type="button" className="primary" onClick={() => shortcutInstallUrl ? window.location.href = shortcutInstallUrl : setMsg({ kind: "error", text: "Save settings in Apple Shortcut Mode first." })}>Install / Build iPhone Shortcut</button>
                ...
                <button type="button" className="ghost" onClick={() => void copyText(`ELS Shortcut setup:\n1. Tap Install / Build iPhone Shortcut from your iPhone.\n2. The universal URL is:\n${universalShortcutPollUrl || "SAVE SETTINGS FIRST"}\n3. In Shortcuts, use URL → Get Contents of URL → Get Dictionary Value messages → Repeat each item → Send body to phone → Get Contents of mark_sent_url.\n4. Add Personal Automations at 9:00am, 5:00pm, and hourly on active show days.`, "Shortcut setup steps")}>Copy setup steps</button>
              </div>
              <div className="small muted" style={{ marginTop: 8 }}>
                Universal mode only sends messages that are due, scheduled, and tied to events with Apple Shortcut Mode active. To use one permanent Shortcut across events, keep the same Shortcut token when activating future events.
              </div>
            </div>
          ) : null}
          ...
          <div className="card compact" style={{ marginTop: 14, background: "#fbfcfd" }}>
            ...
            <div className="card compact" style={{ boxShadow: "none", marginTop: 12 }}>
              ...
              <div className="small muted">Use these when the reminder window has already passed, or when you want to queue that reminder yourself. The text is queued as due now and the iPhone Shortcut can send it on the next run.</div>
              <div className="toolbar" style={{ marginTop: 10 }}>
                ...
                <button type="button" className="ghost" onClick={() => queueManualReminderNow("day_of")} disabled={saving || !selectedShow || !automationDraft.send_schedule}>Queue day-of now</button>
              </div>
            </div>
          </div>
