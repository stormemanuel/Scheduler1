...
async function saveCoordinatorCompensationSettings(admin: SupabaseAdminClient, payload: CoordinatorCompensationSettingsPayload) {
  ...
  const fullInsertPayload = { coordinator_user_id, ...settingsPayload };
  const insertPayloadWithoutNotes = { coordinator_user_id, ...settingsPayloadWithoutNotes };

  let updateResult = await admin
    .from("coordinator_compensation_settings")
    .update(settingsPayload)
    .eq("coordinator_user_id", coordinator_user_id)
