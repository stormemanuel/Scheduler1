...
type FeedbackFormKind = "project-manager" | "area-manager";
...
function feedbackQuestionsForForm(form: Pick<FeedbackFormItem, "kind">): FeedbackQuestion[] {
  return form.kind === "area-manager" ? areaManagerFeedbackQuestions : projectManagerFeedbackQuestions;
}
...
function buildFriendlyFeedbackText(form: Omit<FeedbackFormItem, "text">, show: ShowRecord, clientName: string, venue: string) {
  ...
  return [
    ...
    "",
    "Quick questions",
    "Would you request Emanuel Labor Services again? □ Yes  □ No  □ Not sure",
    "May we use your comments for a testimonial? □ Yes  □ No  □ Ask first",
    "",
    form.kind === "area-manager" ? "What was your overall experience with Emanuel Labor Services?" : "What went well?",
    ...
    "________________________________________________",
    "",
    form.kind === "area-manager" ? "Tech ratings and notes for this booth / area" : "Tech feedback",
    ...techLines,
    "",
    "Additional comments",
    "________________________________________________",
...
function buildFeedbackSurveyHtml(forms: FeedbackFormItem[], show: ShowRecord, clientName: string, venue: string) {
  ...
  const formCards = forms.map((form, formIndex) => {
    ...
    return `<section class="survey-card" data-title="${escapeHtml(form.title)}">
      <div class="card-head">
        <div>
          <p class="eyebrow">${formIndex === 0 ? "Overall event survey" : "Area manager survey"}</p>
      ...
      <div class="question-grid two-col">
        <label>Would you request ELS again?
      ...
      <label>${form.kind === "area-manager" ? "What was your overall experience with Emanuel Labor Services?" : "What went well?"}
      ...
      <h3>${form.kind === "area-manager" ? "Tech ratings and notes for this booth / area" : "Tech feedback"}</h3>
...
const crewListAccentPalette = [
  "#0f766e", "#7c3aed", "#2563eb", "#c2410c", "#047857", "#be123c", "#4338ca", "#0e7490",
  "#a16207", "#6d28d9", "#dc2626", "#0891b2", "#16a34a", "#ea580c", "#4f46e5", "#db2777",
  "#65a30d", "#0284c7", "#9333ea", "#b45309", "#059669", "#e11d48", "#1d4ed8", "#7e22ce",
  "#0d9488", "#ca8a04", "#c026d3", "#0369a1", "#b91c1c", "#15803d", "#6b21a8", "#9a3412",
];

function generatedAccentColor(index: number) {
  if (index < crewListAccentPalette.length) return crewListAccentPalette[index];
  const hue = (index * 137.508) % 360;
  return `hsl(${Math.round(hue)} 72% 34%)`;
}
...
function EventsClientInner({
  ...
  }, [assignments, crewRecords, activeCrewCall, crewAssignmentTargetCallIds]);
  ...
  const availableCrew = useMemo(() => {
    ...
    const alreadyAssignedToEveryTarget = new Set(
      crewRecords
        .filter((crew) => targetIds.every((callId) => assignments.some((assignment) => assignment.crew_id === crew.id && assignment.sub_call_id === callId)))
        .map((crew) => crew.id)
    );
    ...
    const medianForCrew = (crewId: string, scope: "contact" | "client") => {
      const clientId = selectedShow?.business_client_id || "";
      const contactId = selectedShow?.client_contact_id || "";
      const rows = techRatings.filter((row) => {
        if (row.crew_id !== crewId) return false;
        if (scope === "contact") return Boolean(contactId && row.client_contact_id === contactId);
        return Boolean(clientId && row.client_id === clientId);
      });
      return medianRating(rows.map((row) => row.rating));
    };
    ...
    return poolCrew.sort((a, b) => {
      ...
      const aPm = medianForCrew(a.id, "contact");
      const bPm = medianForCrew(b.id, "contact");
      if (aPm !== bPm) return bPm - aPm;
      const aClient = medianForCrew(a.id, "client");
      const bClient = medianForCrew(b.id, "client");
      if (aClient !== bClient) return bClient - aClient;
      return a.name.localeCompare(b.name);
    });
  }, [activeCrewCall, activeCrewDay, crewAssignmentTargetCallIds, crewGroupFilter, crewRecords, crewSearch, eventPool, crewAvailabilityScope, crewAvailabilityDates, assignments, subCallById, laborDayById, showById, selectedShow?.business_client_id, selectedShow?.client_contact_id, techRatings]);
  ...
  const feedbackForms = useMemo(() => {
    ...
    return forms;
  }, [selectedShow, selectedBusinessClient, selectedClientContact, boothSections, ratingRows]);
  ...
  const submittedFeedbackActiveCount = selectedFeedbackResponses.filter((response) => !response.excluded_from_ratings).length;
  const submittedFeedbackExcludedCount = selectedFeedbackResponses.length - submittedFeedbackActiveCount;
  ...
  function ratingForCrewOnShow(crewId: string) {
    return techRatings.find((row) => row.show_id === selectedShow?.id && row.crew_id === crewId && row.rating_source !== "client_feedback") ?? null;
  }
    ...
    return (
      <div className="card compact" style={{ marginTop: 12 }}>
        <div className="row" style={{ alignItems: "flex-start" }}>
          ...
          <span className="small muted">Position matches are suggested first. Anyone in the pool can be assigned.</span>
        </div>
          ...
          {availableCrew.length ? availableCrew.map((crew) => (
            <div key={crew.id} className="row small">
              <div>
                ...
                {selectedShow?.business_client_id && medianRating(techRatings.filter((row) => row.client_id === selectedShow.business_client_id && row.crew_id === crew.id).map((row) => row.rating)) > 0 ? (
                  <div className="small">Client Top Tech: {starDisplay(medianRating(techRatings.filter((row) => row.client_id === selectedShow.business_client_id && row.crew_id === crew.id).map((row) => row.rating)))} median</div>
                ) : null}
              </div>
  ...
  async function setFeedbackResponseRatingStatus(responseId: string, excluded: boolean) {
    ...
    const affectedFeedbackRows = feedbackTechRatingsByResponseId.get(responseId) ?? [];
    const affectedFeedbackRatingIds = new Set(affectedFeedbackRows.map((rating) => `feedback-${rating.id}`));
    if (excluded && affectedFeedbackRatingIds.size) {
      setTechRatings((current) => current.filter((rating) => !affectedFeedbackRatingIds.has(rating.id)));
    } else if (!excluded && affectedFeedbackRows.length) {
      const restoredRatings = affectedFeedbackRows.map((row) => ({
        id: `feedback-${row.id}`,
        show_id: row.show_id,
      ...
      setTechRatings((current) => mergeById(current, restoredRatings));
    }
  }
        ...
        return [
          {
            ...
            paragraphs: [
              ...
              "Would you request ELS again?  Yes / No / Not sure",
              "May we use your comments for a testimonial?  Yes / No / Ask first",
              form.kind === "area-manager" ? "What was your overall experience with Emanuel Labor Services?" : "What went well?",
              "",
              "Anything we should fix or follow up on?",
          ...
          {
            heading: form.kind === "area-manager" ? "Tech ratings and notes for this booth / area" : "Tech feedback",
      ...
      <div className="card compact feedback-intro-card">
        <div className="row" style={{ alignItems: "flex-start" }}>
          ...
          <div className="toolbar">
            ...
            {submittedFeedbackActiveCount ? <span className="badge">{submittedFeedbackActiveCount} active in ratings</span> : null}
              ...
              return (
                <details key={response.id} className="card compact" style={{ boxShadow: "none" }}>
                  <summary style={{ cursor: "pointer" }}>
                    <div className="row" style={{ alignItems: "center" }}>
                      <div>
                        <strong>{response.form_kind === "area-manager" ? "Booth / Area Manager" : "Project Manager / Overall Event"}</strong>
                        <div className="small muted">
                      ...
                      <div className="toolbar">
                        ...
                        {response.excluded_from_ratings ? <span className="badge danger">Removed from rating database</span> : <span className="badge">Active in ratings</span>}
                      </div>
                    </div>
                  </summary>
                  <div className="grid" style={{ gap: 12, marginTop: 12 }}>
                    ...
                    <div className="toolbar">
                      {response.excluded_from_ratings ? (
                        <button type="button" className="primary" onClick={() => void setFeedbackResponseRatingStatus(response.id, false)}>Restore to rating database</button>
                      ) : (
                        <button type="button" className="ghost danger" onClick={() => void setFeedbackResponseRatingStatus(response.id, true)}>Remove from rating database</button>
                      )}
    ...
    return (
      <div className="list">
        <div className="card compact feedback-intro-card">
          ...
          <div className="toolbar" style={{ marginTop: 12 }}>
            <button type="button" className="primary" onClick={() => void generateConnectedFeedbackLinks()}>Create Connected Survey Links</button>
                ...
                <div key={link.id} className="card compact" style={{ boxShadow: "none" }}>
                  <div className="row" style={{ alignItems: "center" }}>
                    <div>
                      ...
                      <div className="small muted">{link.form_kind === "area-manager" ? `Area manager${link.area_name ? ` · ${link.area_name}` : ""}` : "Project manager / overall event"}</div>
        ...
        {feedbackForms.length ? feedbackForms.map((form) => (
          <form key={form.key} className="card feedback-survey-card" onSubmit={(event) => event.preventDefault()}>
            ...
            <div className="grid grid-2">
              <label className="field"><span>Would you request ELS again?</span><select defaultValue=""><option value="">Choose one</option><option>Yes</option><option>No</option><option>Not sure</option></select></label>
            ...
            <div className="grid grid-2">
              <label className="field"><span>{form.kind === "area-manager" ? "What was your overall experience with Emanuel Labor Services?" : "What went well?"}</span><textarea rows={4} placeholder="Positive feedback, testimonial, or quick notes" /></label>
            ...
            <div className="feedback-section-title">{form.kind === "area-manager" ? "Tech ratings and notes for this booth / area" : "Tech feedback"}</div>
            <p className="small muted" style={{ marginTop: -4 }}>Rate only the techs this manager worked with. Notes are optional.</p>
            <div className="list">
            ...
            <div className="toolbar" style={{ marginTop: 12 }}>
              <button type="button" className="primary" onClick={() => void generateConnectedFeedbackLinks()}>Create connected link</button>
              ...
              <div className="card compact">
                ...
                <div className="toolbar" style={{ marginTop: 12 }}>
                  ...
                  {eventDisplayMode === "messages" ? (
                    ...
                    <>
                      <button type="button" className="primary" onClick={() => void generateConnectedFeedbackLinks()}>Create Connected Links</button>
