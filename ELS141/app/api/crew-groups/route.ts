...
export async function PATCH(request: Request) {
  ...
  if (!isOwnerAdmin(auth.role)) {
    return NextResponse.json({ message: "Only admin/owner can rename shared crew groups. Coordinator users can create/use groups, but cannot rename shared groups for the master pool." }, { status: 403 });
  }
