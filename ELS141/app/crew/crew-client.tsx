...
export default function CrewClient({ cityPools: initialCityPools, crewGroups: initialGroups, initialCrew, masterRates, initialRatings, appUsers, currentUserId, currentUserName, currentUserRole }: CrewClientProps) {
  ...
  async function deleteRecord(id: string) {
    ...
    try {
      ...
      if (editingId === id && !result.soft_deleted) closeEditor();
      if (selectedContactId === id && !result.soft_deleted) setSelectedContactId(visibleCrew.find((record) => record.id !== id)?.id || null);
      setMessageKind("success");
      setMessage(result.soft_deleted ? "Contact hidden from the coordinator view. Storm’s Master Pool record is preserved." : "Crew contact deleted.");
      router.refresh();
    } catch (error) {
      setMessageKind("error");
