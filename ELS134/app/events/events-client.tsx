...
function EventsClientInner({
      ...
      ? boothSections.flatMap((section): ExportDocument["sections"] => {
          ...
          return dates.flatMap((date) => {
            ...
            return callsForDate.map(({ call }) => ({
              ...
              subheading: `${call.role_name} • ${formatTimeRange(call)}${call.one_hour_walkaway ? " • 1 hr walkaway" : ""}${call.po_number ? ` • PO: ${call.po_number}` : ""} • ${call.crew_needed} needed`,
  ...
  function renderAutomationPanel() {
    ...
    const customTextSendPresetOptions = customTextSubCallOptions.flatMap((call) => ([
      { value: `start:${call.id}`, label: `Send at ${call.start_time || "start"} · ${call.role_name || "Crew"} · ${call.area || "Area TBD"}`, date: selectedCustomTextDay?.labor_date || "", time: call.start_time || "" },
      { value: `end:${call.id}`, label: `Send at ${call.end_time || "end"} · ${call.role_name || "Crew"} · ${call.area || "Area TBD"}`, date: selectedCustomTextDay?.labor_date || "", time: call.end_time || "" },
    ]));
