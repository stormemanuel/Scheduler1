    ...
    where allowed_pages is not null
      and exists (
        ...
        where p.id = user_access_settings.user_id
          and lower(coalesce(p.role::text, '')) in ('owner', 'admin', 'coordinator')
      );
  end if;
end $$;
...
-- ELS133 event coordinator access compatibility fix
-- The app now uses user_id as the canonical assigned user. Older schemas may require
-- user_profile_id or have a foreign key there that points at an older profile table.
-- Keeping user_profile_id nullable prevents coordinator assignment from being blocked
-- while user_id continues to control event visibility.
alter table if exists public.event_user_access
