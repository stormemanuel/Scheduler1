...
export async function POST(request: Request) {
  ...
  const userIds: string[] = Array.isArray(body.user_ids)
    ...
    : [];
  const assignedCoordinatorUserId = body.assigned_coordinator_user_id === null
    ? null
    : String(body.assigned_coordinator_user_id || "").trim() || undefined;
  if (!showId) return NextResponse.json({ message: "Show is required." }, { status: 400 });

  await ensureProfileRows(admin, userIds);
  if (assignedCoordinatorUserId !== undefined) {
    const { error: assignedError } = await admin
      .from("shows")
      .update({ assigned_coordinator_user_id: assignedCoordinatorUserId })
      .eq("id", showId);
    if (assignedError && !assignedError.message.includes("assigned_coordinator_user_id")) {
      return NextResponse.json({ message: assignedError.message }, { status: 400 });
    }
  }
