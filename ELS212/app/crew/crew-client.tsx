...
async function queueOnboardingRequestForShortcut(record: CrewRecord) {
  const response = await fetch("/api/onboarding", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ action: "create_request", crew_id: record.id, queue_text: true }),
  });
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.message || "Unable to queue onboarding request.");
  return data as { link?: string; message?: string };
}
                        ...
                        {draft && editingId === record.id ? (
                          ...
                          <ContactDetail
                            ...
                            onQueueOnboarding={async () => {
                              try {
                                const result = await queueOnboardingRequestForShortcut(record);
                                setMessageKind("success");
...
function ContactDetail({ record, ratingSummary, cityPools, targetGroups, bulkCityId, bulkGroupName, detailTab, onTab, onEdit, onDelete, onMove, onQueueOnboarding, onAdminDocumentUploaded, onBulkCityChange, onBulkGroupChange, saving, canSeeAdminAudit }: ContactDetailProps) {
