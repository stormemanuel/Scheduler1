...
import { getPayrollAvailableYears, getPayrollPageData } from "@/lib/payroll-data";
...
export async function GET(request: Request) {
  ...
  if (url.searchParams.get("mode") === "years") {
    const availableYears = await getPayrollAvailableYears();
  ...
  const year = Number.isFinite(rawYear) && rawYear > 1900 && rawYear < 3000 ? Math.trunc(rawYear) : undefined;
  ...
  const data = mode === "event"
    ? await getPayrollPageData(year, { showId })
    : await getPayrollPageData(year, { summaryOnly: true });
