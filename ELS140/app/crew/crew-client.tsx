...
export default function CrewClient({ cityPools: initialCityPools, crewGroups: initialGroups, initialCrew, masterRates, initialRatings, appUsers, currentUserId, currentUserName, currentUserRole }: CrewClientProps) {
  ...
  const [poolOptionMode, setPoolOptionMode] = useState<"" | "add-city" | "travel" | "rename" | "delete" | "create-group">("");
  ...
  const ownerScopedCrewRecords = useMemo(() => {
    if (!isAdminCrewView || crewOwnerView === MASTER_CREW_VIEW) return crewRecords;
  ...
  async function deleteRecord(id: string) {
    ...
    try {
      ...
      setMessageKind("success");
      setMessage(result.soft_deleted ? "Contact removed from coordinator view. Admin record is preserved." : "Crew contact deleted.");
      router.refresh();
    } catch (error) {
      setMessageKind("error");
          ...
          <div className="row" style={{ alignItems: "flex-start" }}>
            <div>
              ...
              <p className="small muted" style={{ margin: "4px 0 0" }}>
                Master Pool shows every contact. Coordinator pools show contacts added by that coordinator without creating fake city pools.
              </p>
            </div>
            ...
            <div className="card compact" style={{ background: "#fff", marginTop: 12 }}>
              <div className="row" style={{ alignItems: "flex-start", gap: 12 }}>
                <div>
                  ...
                  <p className="small muted" style={{ margin: "4px 0 0" }}>
                    Contacts this coordinator adds appear here and in Storm’s Master Pool. These checkboxes control which city pools are visible in the coordinator’s own account. Unchecking removes the pool from their view; it does not delete the city pool or master contact records.
                  </p>
                </div>
              ...
              <div className="grid grid-3" style={{ marginTop: 12 }}>
                {cityPoolsForDisplay.map((pool) => (
                  <label key={pool.id} className="checkbox-card">
                    ...
                    <span><strong>{pool.name}</strong><small>{selectedOwnerAllowedPoolSet.has(pool.id) ? "Visible to coordinator" : "Hidden from coordinator"}</small></span>
                  </label>
                ))}
              </div>
            ...
            <div style={{ borderTop: "1px solid var(--line)", paddingTop: 14, marginTop: 14 }}>
              <label className="field">
                ...
                <select value={poolOptionMode} onChange={(event) => setPoolOptionMode(event.target.value as typeof poolOptionMode)}>
                  ...
                  <option value="rename">Rename selected city pool</option>
                  <option value="delete">Delete selected city pool</option>
              ...
              {poolOptionMode === "delete" ? (
                <div style={{ marginTop: 10 }}>
                  <p className="muted small" style={{ marginTop: 0 }}>Contacts in a deleted pool move to Unassigned. This is an admin cleanup tool, not a daily coordinator tool.</p>
                  <button
    ...
    <div>
      ...
      {detailTab === "info" ? (
        <div className="list">
          ...
          <InfoRow label="Added by" value={record.created_by ? "Tracked coordinator/admin user" : "Master/imported record"} />
          {record.coordinator_hidden_at ? <InfoRow label="Coordinator delete note" value={`Hidden by coordinator on ${record.coordinator_hidden_at.slice(0, 10)}. This is still retained in Master Pool.`} /> : null}
