...
async function authWithRole() {
  ...
  const { data: profile, error } = await admin.from("profiles").select("role, full_name, email").eq("id", auth.user.id).maybeSingle();
  if (error) return { ok: false as const, response: NextResponse.json({ message: error.message }, { status: 400 }) };
  const role = clean((profile as { role?: string | null } | null)?.role).toLowerCase();
  return { ok: true as const, user: auth.user, admin, role, profile };
}
