...
function buildGroupedImportPreview(preview: ImportPreviewState, selectedChangeIds: string[]) {
  ...
  for (const section of groupMap.values()) {
    ...
    section.dates.sort((a, b) => a.date.localeCompare(b.date));
    for (const dateGroup of section.dates) {
      dateGroup.rows.sort((a, b) => {
        const timeSort = safeText(a.call.start_time).localeCompare(safeText(b.call.start_time));
  ...
  const groups = [...groupMap.values()].sort((a, b) => {
    ...
    const boothSort = a.booth.localeCompare(b.booth);
    if (boothSort) return boothSort;
    ...
    return [...map.entries()]
      .map(([booth, calls]) => ({
        ...
        calls: calls.sort((a, b) => {
          const dateCompare = a.day.labor_date.localeCompare(b.day.labor_date);
          if (dateCompare !== 0) return dateCompare;
          const timeCompare = a.call.start_time.localeCompare(b.call.start_time);
          if (timeCompare !== 0) return timeCompare;
          return Number(a.call.sort_order || 0) - Number(b.call.sort_order || 0);
        }),
      }))
                  ...
                  <div className="row small" style={{ alignItems: "flex-start" }}>
                    ...
                    <div className="toolbar">
                      ...
                      {removalSeriesAssignments.length > 1 ? (
                        <button type="button" className="ghost danger" onClick={() => removeCrewFromCall(assignment.id, true)}>Remove all days ({removalSeriesAssignments.length})</button>
                      ) : null}
                    </div>
                  </div>
                  {positionChangeOpen ? (
...
<body>
  ...
  async function removeCrewFromCall(assignmentId: string, allMatchingDays = false) {
    ...
    if (allMatchingDays && assignmentIds.length > 1) {
      const crewName = crewById.get(anchorAssignment?.crew_id || "")?.name || "this crew member";
      if (!confirm(`Remove ${crewName} from this matching sub-call on all ${assignmentIds.length} selected days?`)) return;
    }
    await Promise.all(assignmentIds.map((id) => request(`/api/assignments/${id}`, "DELETE")));
    const removed = new Set(assignmentIds);
    ...
    if (allMatchingDays) {
      setMsg({ kind: "success", text: `Crew member removed from ${assignmentIds.length} matching sub-call day${assignmentIds.length === 1 ? "" : "s"}.` });
    }
  }
