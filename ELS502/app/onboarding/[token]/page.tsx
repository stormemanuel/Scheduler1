...
export default function OnboardingPage({ params }: PageProps) {
  ...
  if (requestType === "purchase_order") {
    ...
    const dayLines = (purchaseOrder.days || []).flatMap((day) => {
      ...
      return [
        { label: `${date} — Default day pay`, detail: day.description, amount: day.defaultAmount },
    ...
    const awaitingResponse = purchaseOrder.status === "delivered";
    const statusLabel: Record<string, string> = { sent_awaiting_delivery: "Sent — delivery confirmation pending", delivered: "Delivered — awaiting your response", approved: "Approved", auto_approved: "Automatically approved — no response", changes_requested: "Changes requested", delivery_failed: "Delivery failed", revised: "Revised" };
    return (
      <main style={{ maxWidth: 820, margin: "32px auto", padding: "0 16px" }}>
        ...
        <section className="card" style={{ marginTop: 16 }}>
          ...
          {awaitingResponse ? <><label className="field" style={{ marginTop: 16 }}>Request a change<textarea rows={4} value={purchaseOrderChangeRequest} onChange={(event) => setPurchaseOrderChangeRequest(event.target.value)} placeholder="Explain the hours, rate, penalty, or other item that needs review…" /></label><div className="toolbar" style={{ marginTop: 12 }}><button className="primary" type="button" disabled={purchaseOrderSaving} onClick={() => void submitPurchaseOrderAction("approve_purchase_order")}>{purchaseOrderSaving ? "Saving…" : `Approve ${poMoney(purchaseOrder.total)}`}</button><button className="ghost" type="button" disabled={purchaseOrderSaving || !purchaseOrderChangeRequest.trim()} onClick={() => void submitPurchaseOrderAction("request_purchase_order_changes")}>Request changes</button></div></> : <p className="muted" style={{ marginBottom: 0 }}>{purchaseOrder.status === "changes_requested" ? "ELS has received your request and will send a revised Purchase Order if the amount changes." : purchaseOrder.status === "sent_awaiting_delivery" ? "Your response buttons will appear as soon as email delivery is confirmed." : "No action is currently required."}</p>}
        </section>
      </main>
    );
  }
