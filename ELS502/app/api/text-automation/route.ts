  ...
  for (const { row, po: original } of active) {
    ...
    if (po.status === "delivered" && po.approvalDueAt) {
      ...
      if (now.getTime() >= dueMs) {
        ...
        try {
          await sendPurchaseOrderReminderEmail({ to: safeText(crew?.email), name: safeText(crew?.name) || "Contractor", showName: safeText(showById.get(row.show_id)?.name) || "your event", displayNumber: purchaseOrderDisplayNumber(po), total: po.total, link: `${origin}/onboarding/${encodeURIComponent(po.token)}`, dueAt: po.approvalDueAt });
