              ...
              {matchingCrewAssignmentCalls.map((item) => {
                ...
                return (
                  <label key={matchingCall.id} className="checkline">
                    ...
                    />
                    <span>{day.labor_date}{day.label ? ` · ${day.label}` : ""} · {formatTimeRange(matchingCall)}</span>
                  </label>
                );
              })}
            </div>
  ...
  function buildCrewListDocument(): ExportDocument | null {
    ...
    const boothSubCallSections = boothSections.map((section): ExportDocument["sections"][number] => {
      ...
      return {
        ...
        subheading: `${roleSummary} • ${dateSummary} • ${entries.length} sub-call${entries.length === 1 ? "" : "s"} • ${totalNeeded || "Need TBD"} needed total`,
    ...
    return {
      ...
      subtitle: `Event: ${selectedShow.name}`,
      meta: [
