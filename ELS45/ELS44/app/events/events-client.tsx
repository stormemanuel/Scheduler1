...
import type { BusinessClientRecord, ClientContactRecord, ProjectManagerFavoriteRecord, TechRatingRecord } from "@/lib/client-types";
...
type Props = {
  ...
  initialProjectManagerFavorites: ProjectManagerFavoriteRecord[];
...
type EventDisplayMode = "day" | "booth" | "messages" | "automation" | "checklist" | "notes" | "ratings";
...
const crewListAccentPalette = ["#0f766e", "#7c3aed", "#2563eb", "#c2410c", "#047857", "#be123c", "#4338ca", "#0e7490", "#a16207", "#6d28d9"];

function accentColorForArea(area: string) {
  const key = normalize(boothLabel(area));
  let hash = 0;
  for (let index = 0; index < key.length; index += 1) {
    hash = (hash * 31 + key.charCodeAt(index)) >>> 0;
  }
  return crewListAccentPalette[hash % crewListAccentPalette.length];
}
    ...
    if (this.state.hasError) {
      return (
        <div className="card">
          ...
          <p className="muted">The page was protected from a full app crash. Refresh the page, then continue from the event list. If this repeats, check for a partial sub-call with missing area, role, start, or end time.</p>
...
function EventsClientInner({
  ...
  initialTechRatings,
  initialProjectManagerFavorites,
  initialBusinessClients,
  initialClientContacts,
  initialCrew,
  masterRates,
}: Props) {
  const [shows, setShows] = useState(() => initialShows.map(sanitizeShow).filter((show) => show.id));
  ...
  const [projectManagerFavorites, setProjectManagerFavorites] = useState(initialProjectManagerFavorites);
  ...
  const ratingRows = useMemo(() => {
    if (!selectedShow) return [] as Array<{ crewId: string; crewName: string; phone: string; email: string; firstSchedule: string; assignmentId: string | null; rating: TechRatingRecord | null }>;
    ...
    return [...seen.entries()]
      .map(([crewId, item]) => {
        ...
        return {
          ...
          rating: techRatings.find((row) => row.show_id === selectedShow.id && row.crew_id === crewId) ?? null,
        };
      })
      .sort((a, b) => a.crewName.localeCompare(b.crewName));
  ...
  function projectManagerFavoriteForCrew(crewId: string) {
    if (!selectedShow?.client_contact_id) return null;
    return projectManagerFavorites.find((row) => row.client_contact_id === selectedShow.client_contact_id && row.crew_id === crewId) ?? null;
  }

  const projectManagerFavoriteTechs = useMemo(() => {
    const contactId = selectedShow?.client_contact_id || "";
    if (!contactId) return [] as Array<{ favorite: ProjectManagerFavoriteRecord; crew: CrewRecord | undefined; median: number; average: number; count: number }>;
    return projectManagerFavorites
      .filter((favorite) => favorite.client_contact_id === contactId)
      .map((favorite) => {
        const ratings = techRatings.filter((row) => row.client_contact_id === contactId && row.crew_id === favorite.crew_id).map((row) => Number(row.rating || 0)).filter((value) => value > 0);
        const average = ratings.length ? ratings.reduce((sum, value) => sum + value, 0) / ratings.length : 0;
        return {
          favorite,
          crew: crewRecords.find((crew) => crew.id === favorite.crew_id),
          median: medianRating(ratings),
          average,
          count: ratings.length,
        };
      })
      .sort((a, b) => String(b.favorite.updated_at || b.favorite.created_at || "").localeCompare(String(a.favorite.updated_at || a.favorite.created_at || "")));
  }, [selectedShow?.client_contact_id, projectManagerFavorites, techRatings, crewRecords]);
  ...
  const workerNoteRatingSummaries = useMemo(() => {
    if (!selectedShow) return [] as Array<{ crewId: string; crewName: string; phone: string; notes: AssignmentNoteRecord[]; rating: TechRatingRecord | null; favorite: ProjectManagerFavoriteRecord | null }>;
    return ratingRows.map((row) => ({
      ...
      rating: row.rating,
      favorite: selectedShow.client_contact_id ? (projectManagerFavorites.find((favorite) => favorite.client_contact_id === selectedShow.client_contact_id && favorite.crew_id === row.crewId) ?? null) : null,
    }));
  }, [selectedShow, ratingRows, assignmentNotes, projectManagerFavorites]);
  ...
  async function saveTechRating(row: { crewId: string; assignmentId: string | null }, rating: number, notes?: string) {
    ...
    if (data?.rating) {
      const nextRating = data.rating as TechRatingRecord;
      setTechRatings((current) => {
        const without = current.filter((item) => !(item.show_id === nextRating.show_id && item.crew_id === nextRating.crew_id));
  ...
  function rowRatingNotes(crewId: string) {
    return techRatings.find((row) => row.show_id === selectedShow?.id && row.crew_id === crewId)?.notes || "";
  }

  function ratingForCrewOnShow(crewId: string) {
    return techRatings.find((row) => row.show_id === selectedShow?.id && row.crew_id === crewId) ?? null;
  }
  ...
  async function toggleProjectManagerFavorite(row: { crewId: string; assignmentId: string | null }, active: boolean, favoriteNote?: string) {
    if (!selectedShow) return;
    if (!selectedShow.client_contact_id) {
      setMsg({ kind: "error", text: "Choose a project manager/client contact on this event before marking PM favorites." });
      return;
    }
    const data = await request("/api/project-manager-favorites", "POST", {
      show_id: selectedShow.id,
      client_id: selectedShow.business_client_id || null,
      client_contact_id: selectedShow.client_contact_id,
      crew_id: row.crewId,
      assignment_id: row.assignmentId,
      active,
      favorite_note: favoriteNote ?? projectManagerFavoriteForCrew(row.crewId)?.favorite_note ?? "",
    });
    if (data?.deleted) {
      setProjectManagerFavorites((current) => current.filter((favorite) => !(favorite.client_contact_id === selectedShow.client_contact_id && favorite.crew_id === row.crewId)));
      setMsg({ kind: "success", text: "Project manager favorite removed." });
      return;
    }
    if (data?.favorite) {
      const nextFavorite = data.favorite as ProjectManagerFavoriteRecord;
      setProjectManagerFavorites((current) => {
        const without = current.filter((favorite) => !(favorite.client_contact_id === nextFavorite.client_contact_id && favorite.crew_id === nextFavorite.crew_id));
        return [nextFavorite, ...without];
      });
      setMsg({ kind: "success", text: "Marked as requested back / project manager favorite." });
    }
  }
    ...
    return (
      <div className="card compact" style={{ marginTop: 8, background: "#f9fafb" }}>
        ...
        <div className="card compact" style={{ marginTop: 10, background: "#fff" }}>
          <div className="row" style={{ alignItems: "center" }}>
            <div>
              <strong>Requested back / PM favorite</strong>
              <div className="small muted">Use this when the project manager asks for this tech back. It saves under that project manager/contact.</div>
            </div>
            <button
              type="button"
              className={projectManagerFavoriteForCrew(assignment.crew_id) ? "primary" : "ghost"}
              disabled={saving || !selectedShow?.client_contact_id}
              onClick={() => void toggleProjectManagerFavorite({ crewId: assignment.crew_id, assignmentId: assignment.id }, !projectManagerFavoriteForCrew(assignment.crew_id))}
              title={selectedShow?.client_contact_id ? "Toggle requested back / PM favorite" : "Choose a project manager/contact on this event first"}
            >
              {projectManagerFavoriteForCrew(assignment.crew_id) ? "PM favorite ✓" : "Mark PM favorite"}
            </button>
          </div>
          {!selectedShow?.client_contact_id ? <div className="small muted" style={{ marginTop: 8 }}>Choose a project manager/client contact on this event to use this favorite button.</div> : null}
        </div>
    ...
    const sections: ExportDocument["sections"] = eventDisplayMode === "booth"
      ? boothSections.flatMap((section): ExportDocument["sections"] => {
          ...
          return dates.flatMap((date) => {
            ...
            return callsForDate.map(({ call }) => ({
              ...
              accentColor: accentColorForArea(section.booth),
      ...
      : visibleLaborDays.flatMap((day): ExportDocument["sections"] => {
          ...
          return dayCalls.map(({ call }) => ({
            ...
            accentColor: accentColorForArea(call.area),
            columns: ["Name", "Times", "Position", "Contact Number", "Status", "Notes"],
            rows: buildRows(call),
          }));
        });
  ...
  async function saveCall() {
    ...
    if (!basePayload.area || !basePayload.role_name || !basePayload.start_time || !basePayload.end_time) {
      setMsg({ kind: "error", text: "Area, role, start time, and end time are required." });
      return;
    }
  ...
  function renderRatingsPanel() {
    ...
    return (
      <div className="list">
        ...
        {ratingRows.length ? ratingRows.map((row) => (
          <div key={row.crewId} className="card compact">
            ...
            <div className="toolbar" style={{ marginTop: 12 }}>
              ...
              <button
                type="button"
                className={projectManagerFavoriteForCrew(row.crewId) ? "primary" : "ghost"}
                disabled={saving || !selectedShow.client_contact_id}
                onClick={() => void toggleProjectManagerFavorite(row, !projectManagerFavoriteForCrew(row.crewId))}
                title={selectedShow.client_contact_id ? "Toggle requested back / PM favorite" : "Choose a project manager/contact on this event first"}
              >
                {projectManagerFavoriteForCrew(row.crewId) ? "PM favorite ✓" : "Mark PM favorite"}
              </button>
            </div>
            <label className="field" style={{ marginTop: 10 }}>
        ...
        <div className="card compact">
          <h4 style={{ marginTop: 0 }}>Project Manager Favorites / Requested Back</h4>
          {selectedShow.client_contact_id ? (
            projectManagerFavoriteTechs.length ? (
              <div className="list">
                {projectManagerFavoriteTechs.map((item, index) => (
                  <div key={item.favorite.id} className="row" style={{ alignItems: "center" }}>
                    <div>
                      <strong>★ {item.crew?.name || "Unknown tech"}</strong>
                      <div className="small muted">{item.crew?.phone ? formatPhone(item.crew.phone) : "No phone"}</div>
                      {item.favorite.favorite_note ? <div className="small muted">{item.favorite.favorite_note}</div> : null}
                    </div>
                    <div style={{ textAlign: "right" }}>
                      {item.count ? <strong>{starDisplay(item.median)} {item.median.toFixed(1)} median</strong> : <strong>Favorite</strong>}
                      <div className="small muted">Marked requested back</div>
                      <button type="button" className="ghost danger" disabled={saving} onClick={() => void toggleProjectManagerFavorite({ crewId: item.favorite.crew_id, assignmentId: item.favorite.source_assignment_id }, false)}>Remove</button>
                    </div>
                  </div>
                ))}
              </div>
            ) : <p className="small muted">No project manager favorites marked yet.</p>
          ) : <p className="small muted">Choose a project manager/client contact on this event to activate favorites.</p>}
        </div>
      </div>
    );
  }
                    ...
                    <div key={worker.crewId} className="card compact">
                      <div className="row" style={{ alignItems: "flex-start" }}>
                        ...
                        <div style={{ textAlign: "right" }}>
                          ...
                          {worker.favorite ? <div className="badge" style={{ marginTop: 6 }}>★ PM favorite</div> : null}
                        </div>
                      </div>
                      {worker.notes.length ? (
