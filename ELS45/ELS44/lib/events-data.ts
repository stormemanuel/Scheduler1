...
import type { ProjectManagerFavoriteRecord, TechRatingRecord } from "@/lib/client-types";
...
export async function getEventsPageData() {
  ...
  if (!supabase) {
    return {
      ...
      projectManagerFavorites: [] as ProjectManagerFavoriteRecord[],
      setupMissing: true,
  ...
  const [laborDaysRes, subCallsRes, assignmentsRes, notesRes, checklistRes, automationRes, textQueueRes, ratesRes, ratingsRes, pmFavoritesRes] = await Promise.all([
    ...
    supabase.from("project_manager_favorites").select("id, client_id, client_contact_id, crew_id, source_show_id, source_assignment_id, favorite_note, created_at, updated_at").order("updated_at", { ascending: false }),
  ]);
  ...
  const pmFavoritesMissing = Boolean(pmFavoritesRes.error && pmFavoritesRes.error.message.includes('relation "project_manager_favorites" does not exist'));
  const error = showsRes.error || laborDaysRes.error || subCallsRes.error || (assignmentsMissing ? null : assignmentsRes.error) || (notesMissing ? null : notesRes.error) || (checklistsMissing ? null : checklistRes.error) || (automationMissing ? null : automationRes.error) || (textQueueMissing ? null : textQueueRes.error) || (ratingsMissing ? null : ratingsRes.error) || (pmFavoritesMissing ? null : pmFavoritesRes.error) || ratesRes.error;
  if (error) {
    return {
      shows: [] as ShowRecord[],
      laborDays: [] as LaborDayRecord[],
  ...
  const projectManagerFavorites = pmFavoritesMissing ? [] : ((pmFavoritesRes.data ?? []).map((row) => {
    const typed = row as Partial<ProjectManagerFavoriteRecord> & { id: string; client_contact_id: string; crew_id: string };
    return {
      id: typed.id,
      client_id: typed.client_id || null,
      client_contact_id: typed.client_contact_id,
      crew_id: typed.crew_id,
      source_show_id: typed.source_show_id || null,
      source_assignment_id: typed.source_assignment_id || null,
      favorite_note: typed.favorite_note || "",
      created_at: typed.created_at || "",
      updated_at: typed.updated_at || null,
    } as ProjectManagerFavoriteRecord;
  }));
