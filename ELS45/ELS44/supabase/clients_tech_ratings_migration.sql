...
-- Project Manager / Client Contact favorites.
-- Use this when a project manager requests a tech back.
create table if not exists project_manager_favorites (
  id uuid primary key default gen_random_uuid(),
  ...
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now(),
  unique (client_contact_id, crew_id)
);
...
create index if not exists project_manager_favorites_client_id_idx on project_manager_favorites(client_id);
create index if not exists project_manager_favorites_client_contact_id_idx on project_manager_favorites(client_contact_id);
create index if not exists project_manager_favorites_crew_id_idx on project_manager_favorites(crew_id);

alter table business_clients enable row level security;
alter table client_contacts enable row level security;
alter table tech_ratings enable row level security;
alter table project_manager_favorites enable row level security;
...
drop policy if exists "project_manager_favorites_authenticated_all" on project_manager_favorites;
create policy "project_manager_favorites_authenticated_all"
  on project_manager_favorites
  for all
  to authenticated
  using (true)
  with check (true);
...
drop view if exists project_manager_favorite_techs;
...
create or replace view project_manager_favorite_techs as
select
  pmf.client_id,
  pmf.client_contact_id,
  cc.name as client_contact_name,
  pmf.crew_id,
  c.name as crew_name,
  c.phone,
  c.email,
  pmf.source_show_id,
  pmf.source_assignment_id,
  pmf.favorite_note,
  pmf.created_at,
  pmf.updated_at,
  round((percentile_cont(0.5) within group (order by tr.rating))::numeric, 2) as median_rating,
  round(avg(tr.rating)::numeric, 2) as average_rating,
  count(tr.id)::integer as rating_count
from project_manager_favorites pmf
join crew c on c.id = pmf.crew_id
join client_contacts cc on cc.id = pmf.client_contact_id
left join tech_ratings tr on tr.client_contact_id = pmf.client_contact_id and tr.crew_id = pmf.crew_id
group by pmf.client_id, pmf.client_contact_id, cc.name, pmf.crew_id, c.name, c.phone, c.email, pmf.source_show_id, pmf.source_assignment_id, pmf.favorite_note, pmf.created_at, pmf.updated_at;
