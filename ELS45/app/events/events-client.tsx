...
function EventsClientInner({
  ...
  }, [activeCrewCall, activeCrewDay, crewAssignmentTargetCallIds, crewGroupFilter, crewRecords, crewSearch, eventPool, crewAvailabilityScope, crewAvailabilityDates, assignments, subCallById, laborDayById, showById]);
  ...
  const feedbackForms = useMemo(() => {
    if (!selectedShow) return [] as Array<{ key: string; title: string; subtitle: string; target: string; text: string }>;
    ...
    const dateRange = `${selectedShow.show_start} to ${selectedShow.show_end}`;
    const buildCrewLines = (rows: typeof ratingRows) => rows.length
      ? rows.map((row, index) => `${index + 1}. ${row.crewName} — Position/Schedule: ${row.firstSchedule}\n   Rating 1 2 3 4 5: ____   Asked back? Yes / No   Notes: ________________________________`).join("\n")
      : "No assigned techs listed yet.";
    const baseQuestions = [
      "Overall event rating 1 2 3 4 5: ____",
      "ELS communication before/during show 1 2 3 4 5: ____",
      "Professionalism / appearance 1 2 3 4 5: ____",
      "Punctuality / readiness 1 2 3 4 5: ____",
      "Work quality / technical support 1 2 3 4 5: ____",
      "Safety / site conduct 1 2 3 4 5: ____",
      "Would you request Emanuel Labor Services again? Yes / No",
      "May we use your comments as a testimonial? Yes / No / Ask first",
      "Testimonial or positive feedback:",
      "________________________________________________________________________",
      "Problems, concerns, or follow-up needed:",
      "________________________________________________________________________",
    ];
    const header = (target: string) => [
      "Emanuel Labor Services Feedback Form",
      `Show: ${showTitle}`,
      `Client: ${clientName}`,
      `Venue: ${venue}`,
      `Dates: ${dateRange}`,
    ...
    const forms: Array<{ key: string; title: string; subtitle: string; target: string; text: string }> = [{
      key: "project-manager",
      title: `${showTitle} - Project Manager Feedback Form`,
      subtitle: "Overall event feedback, testimonial request, problem capture, and tech ratings.",
      target: projectManagerTarget,
      text: [
        ...header(projectManagerTarget),
        "OVERALL EVENT FEEDBACK",
        ...baseQuestions,
        "",
        "TECH RATING / SHOW NOTES",
        buildCrewLines(ratingRows),
        "",
        "Additional notes:",
        "________________________________________________________________________",
      ].join("\n"),
    ...
    for (const section of boothSections) {
      const crewMap = new Map<string, typeof ratingRows[number]>();
      ...
      forms.push({
        key: `area-${normalize(section.booth).replace(/[^a-z0-9]+/g, "-") || section.booth}`,
        title: `${showTitle} - ${section.booth} Feedback Form`,
        subtitle: "Area-specific feedback, testimonial request, issue capture, and tech ratings.",
        target,
        text: [
          ...header(target),
          `AREA / BOOTH: ${section.booth}`,
          "",
          "AREA FEEDBACK",
          ...baseQuestions,
          "",
          "TECH RATING / AREA NOTES",
          buildCrewLines(areaRows),
          "",
          "Area-specific notes:",
          "________________________________________________________________________",
        ].join("\n"),
  ...
  function buildFeedbackDocument(): ExportDocument | null {
    ...
    return {
      title: `${selectedShow.name} Feedback Forms`,
      subtitle: `${selectedShow.show_start} - ${selectedShow.show_end}`,
      meta: [["Client", selectedBusinessClient?.name || selectedShow.client || ""], ["Venue", selectedShow.venue || ""]],
      sections: feedbackForms.map((form) => ({
        heading: form.title,
        subheading: form.target,
        paragraphs: form.text.split("\n"),
      })),
    };
  }
    ...
    return (
      <div className="list">
        <div className="card compact accent-card">
          <div className="row" style={{ alignItems: "flex-start" }}>
            <div>
              <h4 style={{ margin: 0 }}>Client feedback forms</h4>
              <div className="small muted">Auto-generated forms for the project manager over the full event and each booth/area manager. Use them to collect testimonials, identify problems, and capture tech ratings from the client side.</div>
            </div>
            <span className="badge">{feedbackForms.length} form{feedbackForms.length === 1 ? "" : "s"}</span>
          </div>
        </div>
        {feedbackForms.length ? feedbackForms.map((form) => (
          <div key={form.key} className="card compact">
            <div className="row" style={{ alignItems: "flex-start" }}>
              <div>
                <strong>{form.title}</strong>
                <div className="small muted">{form.subtitle}</div>
              </div>
              <button type="button" className="ghost" onClick={() => void copyText(form.text, form.title)}>Copy</button>
            </div>
            <textarea readOnly value={form.text} rows={18} style={{ width: "100%", marginTop: 10, fontFamily: "ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, monospace" }} />
          </div>
        )) : <p className="small muted">Add labor days/sub-calls first to generate area-specific feedback forms.</p>}
      </div>
    );
  }
              ...
              <div className="card compact">
                ...
                <div className="toolbar" style={{ marginTop: 12 }}>
                  ...
                  {eventDisplayMode === "messages" ? (
                    ...
                    <>
                      <button type="button" className="ghost" onClick={exportFeedbackFormsPdf}>PDF Feedback Forms</button>
