...
function makeSubCallKey(laborDate: string, area: string, roleName: string, startTime: string, endTime: string | null | undefined, poNumber?: string | null) {
  ...
  return [laborDate, area || "", roleName || "", normalizeTimeKey(startTime), normalizeTimeKey(endTime), poNumber || ""].join("|");
}
...
function applyImportPreviewOverrides(payload: ImportPayloadShape, preview: ImportPreviewShape, rawOverrides: string) {
  ...
  payload.subCallGroups = payload.subCallGroups.map((call, index) => {
    ...
    return {
      ...
      role_name: String(edit.role_name || call.role_name || "").trim() || call.role_name,
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const missingSubCalls = payload.subCallGroups.filter(
      (call) => !subCallIdByKey.has(makeSubCallKey(call.labor_date, call.area, call.role_name, call.start_time, call.end_time, call.po_number))
    );
    ...
    for (const call of preview.subCallPreview) {
      const subCallId = subCallIdByKey.get(makeSubCallKey(call.labor_date, call.area, call.role_name, call.start_time, call.end_time, call.po_number));
    ...
    return NextResponse.json({
      ...
      message:
        mode === "merge"
          ? `Merged import into ${showRow.name}. Added ${createdLaborDays.length} labor days, ${createdSubCalls.length} sub-calls, and assigned ${assignmentRows.length} matched crew${preview.unmatchedCrewCount ? `, ${preview.unmatchedCrewCount} unmatched` : ""}.`
