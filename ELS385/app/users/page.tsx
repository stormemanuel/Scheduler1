...
function coordinatorCompensationSetupMessage(error: SupabaseLikeError | null | undefined) {
  ...
  if (code === "42P01" || /relation .*coordinator_compensation_settings.* does not exist/i.test(message)) {
    return "Coordinator compensation settings are not installed yet. Page access and crew-pool access can still be saved; negotiated coordinator payout rates will be skipped until the ELS295 SQL is installed.";
  }
  if (code.startsWith("PGRST") || lower.includes("schema cache")) {
    return "Coordinator compensation table appears to be waiting on Supabase API schema refresh. Page access and crew-pool access can still be saved; negotiated coordinator payout rates will be skipped until Supabase refreshes.";
  }
  if (lower.includes("coordinator_compensation_settings")) {
    return `Coordinator compensation settings could not load: ${message}`;
  }
  return null;
}
            ...
            return (
              <div key={profile.id} className="card compact" style={{ gap: 14 }}>
                ...
                <div className="card compact accent-card">
                  ...
                  {!authUser ? <p className="error small" style={{ marginBottom: 0 }}>This ELS profile does not currently match a Supabase Auth user ID. Password resets cannot work until the Auth user is created or relinked.</p> : null}
                </div>
                ...
                {role === "coordinator" ? (
                  <div className="card compact accent-card">
                    ...
                    {coordinatorCompensationSetupMessageText ? (
                      <p className="error small" style={{ marginTop: 0 }}>
                        Payout rate fields are temporarily skipped on save. Page access and crew-pool access will still save.
                      </p>
                    ) : null}
