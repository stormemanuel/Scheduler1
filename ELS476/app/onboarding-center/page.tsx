    ...
    <div className="grid" style={{ gap: 16 }}>
              ...
              <article key={row.id} className="card compact" style={{ boxShadow: "none" }}>
                <div className="row" style={{ alignItems: "flex-start", gap: 12 }}>
                  ...
                  <div className="toolbar" style={{ justifyContent: "flex-end", flexWrap: "wrap" }}>
                    ...
                    >
                      {coordinatorBusyId === row.id ? "Working..." : row.onboarding_complete ? "Send onboarding again" : row.onboarding_request_sent_at ? "Send reminder with link" : "Send onboarding with link"}
                    </button>
                    <button
                      className="primary"
                      type="button"
      ...
      <section className="card">
        ...
        <p className="muted" style={{ marginTop: 0 }}>
          Send a full onboarding packet even when the person is not in Crew yet. Matching email or phone records reuse the same active secure link. For full onboarding, automatic reminder texts are queued every two weeks until the form is submitted.
        </p>
        ...
        <div className="toolbar" style={{ marginTop: 12 }}>
          ...
          {lastInviteMessage && lastInvitePhone ? (
            <a className="primary" href={androidSmsHref(lastInvitePhone, lastInviteMessage)}>
              Send using text message (cell phone)
            </a>
          ) : null}
          <span className="muted small">Matches by email/phone, reuses an active link when available, and otherwise creates a hidden pending contact. Full-onboarding reminders repeat every two weeks and stop automatically after submission. Text-message sending must be opened from the app on the sender's cell phone.</span>
        </div>
          ...
          return (
            <section key={row.id} className="card">
              <div className="row" style={{ alignItems: "flex-start", gap: 14 }}>
                ...
                <div className="toolbar">
                  ...
                  <button className="primary" type="button" onClick={() => void resendRequest(row)} disabled={!canRemind || Boolean(busyId)}>
                    {busyId === `${row.id}-resend` ? "Queueing..." : row.status === "submitted" ? "Submitted" : row.status === "approved" ? "Approved" : "Send reminder with link"}
                  </button>
                </div>
              </div>
              ...
              <p className="muted small" style={{ marginBottom: 0 }}>
                This secure link has been used {row.send_history_count || 1} time{(row.send_history_count || 1) === 1 ? "" : "s"}. Admin can see this request and will review private W-9, contract, and tax information after submission. {row.request_type === "full_onboarding" ? "Automatic reminders are queued every two weeks until submission." : ""}
              </p>
            </section>
          );
        }
          ...
          <section key={row.id} className="card">
            <div className="row" style={{ alignItems: "flex-start", gap: 14 }}>
              ...
              <div className="toolbar">
                ...
                {!isApproved ? (
                  <button className="ghost" type="button" onClick={() => void resendRequest(row)} disabled={!canRemind || Boolean(busyId)}>
                    {busyId === `${row.id}-resend` ? "Queueing..." : canRemind ? "Send reminder with link" : "Submitted"}
                  </button>
                ) : null}
                {!isApproved ? (
