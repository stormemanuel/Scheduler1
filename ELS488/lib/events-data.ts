...
export async function getEventsCrewLookupData(options: { includeCrewIds?: string[] } = {}) {
  ...
  let crewQuery = supabase
    .from("crew")
    .select("id, name, description, city_pool_id, group_name, tier, email, phone, lead_from, other_city, ob, notes, conflict_companies, blacklisted, blacklist_reason, created_by, coordinator_hidden_at, coordinator_hidden_by")
  ...
  if (role === "coordinator" && session.user?.id) {
    ...
    crewQuery = crewQuery.or(`created_by.eq.${session.user.id}${leadFilter}${assignedIdFilter}`);
  }
