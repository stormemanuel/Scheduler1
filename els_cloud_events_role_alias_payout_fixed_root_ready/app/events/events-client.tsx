      ...
      <table>
        ...
        <tbody>
          ${section.rows.length
            ? section.rows
                .map(
                  (row) => `<tr><td>${section.date}</td><td>${row.crew?.name || row.assignment.crew_id}</td><td>${section.timeText}</td><td>${row.crew?.positions[0]?.role_name || "Assigned"}</td><td>${row.crew ? formatPhone(row.crew.phone) : ""}</td></tr>`
                )
                .join("")
        ...
        <section className="card">
          <div className="row">
            ...
            <div className="toolbar">
              ...
              <button type="button" className={viewMode === "edit" ? "primary" : "ghost"} onClick={() => { setViewMode("edit"); if (!editorMode) setEditorMode("show"); }} disabled={!selectedShow}>
                Edit mode
              </button>
            </div>
          </div>
          ...
          {selectedShow ? (
            <div className="grid" style={{ gap: 16 }}>
              ...
              {viewMode === "edit" && editorMode ? (
                <div className="card compact" style={{ borderColor: "#111827" }}>
                  <div className="row">
                    ...
                    <div key={day.id} className="card compact">
                      ...
                      {isOpen ? (
                        <div className="grid" style={{ gap: 12, marginTop: 12 }}>
                          ...
                          {dayCalls.length ? dayCalls.map((call) => {
                            const callAssignments = assignments
                              .filter((assignment) => assignment.sub_call_id === call.id)
                              .map((assignment) => ({
                                assignment,
                                ...
                                <div className="row" style={{ alignItems: "flex-start" }}>
                                  ...
                                  <div className="toolbar">
                                    ...
                                    <button type="button" className="ghost" onClick={() => {
                                      setCrewPickerCallId(isCrewOpen ? null : call.id);
                                      setCrewSearch("");
                          ...
                          }) : <div className="small muted">No sub-calls yet for this labor day.</div>}
                        </div>
                      ) : null}
                    </div>
                  );
