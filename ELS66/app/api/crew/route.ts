...
export async function GET() {
  ...
  const [crewRes, positionsRes, cityPoolsRes, extraPoolsRes] = await Promise.all([
    supabase.from("crew").select("id, name, description, city_pool_id, group_name, tier, email, phone, other_city, ob, blacklisted, blacklist_reason, notes, conflict_companies").order("name", { ascending: true }),
  ...
  const rows = (crewRes.data ?? []).map((row) => {
    const typed = row as { id: string; name: string | null; description: string | null; city_pool_id: string | null; group_name: string | null; tier: string | null; email: string | null; phone: string | null; other_city: string | null; ob: boolean | null; blacklisted?: boolean | null; blacklist_reason?: string | null; notes: string | null; conflict_companies: string[] | null };
    return {
      id: typed.id,
      name: typed.name ?? "",
