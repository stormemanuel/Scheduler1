...
export async function getPayrollPageData(): Promise<PayrollPageData> {
  try {
    ...
    const [showsRes, laborDaysRes, subCallsRes, assignmentsRes, crewRes, positionsRes, cityPoolsRes, ratesRes, clientRatesRes, payrollRes, financialsRes] = await Promise.all([
      ...
      supabase.from("assignments").select("id, sub_call_id, crew_id, status, sort_order").order("sort_order", { ascending: true }).limit(20000),
      supabase.from("crew").select("id, name, description, city_pool_id, group_name, tier, email, phone, other_city, ob, onboarding_texted_called, onboarding_response, onboarding_paperwork_sent, onboarding_successfully_onboarded, onboarding_called_placed_tier, notes, conflict_companies").order("name", { ascending: true }).limit(10000),
    ...
    const assignments = assignmentsMissing ? [] : (assignmentsRes.data ?? []).map((row, index) => {
      ...
      return { id: safeText(typed.id), sub_call_id: safeText(typed.sub_call_id), crew_id: safeText(typed.crew_id), status: safeText(typed.status, "confirmed"), sort_order: typed.sort_order ?? index + 1 } satisfies AssignmentRecord;
    ...
    const crewRecords = (crewRes.data ?? []).map((row) => {
      const typed = row as { id: string; name: string | null; description: string | null; city_pool_id: string | null; group_name: string | null; tier: string | null; email: string | null; phone: string | null; other_city: string | null; ob: boolean | null; onboarding_texted_called?: boolean | null; onboarding_response?: boolean | null; onboarding_paperwork_sent?: boolean | null; onboarding_successfully_onboarded?: boolean | null; onboarding_called_placed_tier?: boolean | null; notes: string | null; conflict_companies: string[] | null };
