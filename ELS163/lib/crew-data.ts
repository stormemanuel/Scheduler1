...
export async function getCrewPageData() {
  ...
  let allowedCityPoolIds = session.access?.allowed_city_pool_ids ?? [];
  ...
  if (role === "coordinator" && session.user?.id && allowedCityPoolIds.length === 0 && admin) {
    const { data: currentAccess } = await admin
      .from("user_access_settings")
      .select("allowed_city_pool_ids")
      .eq("user_id", session.user.id)
      .maybeSingle();
    const ids = (currentAccess as { allowed_city_pool_ids?: unknown } | null)?.allowed_city_pool_ids;
    allowedCityPoolIds = Array.isArray(ids) ? ids.map((id) => String(id || "").trim()).filter(Boolean) : [];
  }
  const allowedPoolIds = new Set(allowedCityPoolIds);
