...
async function requireOwnerAdmin() {
  ...
  const { data: profile, error } = await dataClient.from("profiles").select("role").eq("id", user.id).maybeSingle();
...
export async function PATCH(request: Request) {
  ...
  const { data: existing } = await admin
    ...
    .select("allowed_pages, restrict_events_to_owner, restrict_crew_to_owner, can_edit_event_details, allowed_city_pool_ids")
    .eq("user_id", userId)
    .maybeSingle();
