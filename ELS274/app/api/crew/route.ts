...
async function serveCrewProfilePhoto(auth: Awaited<ReturnType<typeof authContext>>, crewId: string) {
  ...
  const { data: crew, error } = await admin
    .from("crew")
    // Keep profile-photo delivery working even when an unrelated optional crew
    // column has not yet been added by a newer migration.
    .select("*")
    .eq("id", crewId)
    .maybeSingle();
...
export async function GET(request: Request) {
  ...
  const [crewRes, positionsRes, cityPoolsRes, extraPoolsRes] = await Promise.all([
    ...
    supabase.from("crew").select("*").order("name", { ascending: true }),
