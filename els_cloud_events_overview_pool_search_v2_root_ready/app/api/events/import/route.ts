...
function makeSubCallKey(laborDate: string, area: string, roleName: string, startTime: string, endTime: string | null | undefined) {
  return [laborDate, area || "", roleName || "", startTime || "", endTime || ""].join("|");
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    for (const call of preview.subCallPreview) {
      const subCallId = subCallIdByKey.get(call.key);
      if (!subCallId) continue;
      for (const match of call.matchedCrew) {
        assignmentInsert.push({ sub_call_id: subCallId, crew_id: match.crew_id, status: "confirmed" });
      }
    }
    ...
    return NextResponse.json({
      ...
      message:
        mode === "merge"
          ? `Merged import into ${showRow.name}. Added ${createdLaborDays.length} labor days, ${createdSubCalls.length} sub-calls, and matched ${preview.matchedCrewCount} crew${preview.unmatchedCrewCount ? `, ${preview.unmatchedCrewCount} unmatched` : ""}.`
          : `Imported ${showRow.name}. Created ${allLaborDays.length} labor days, ${existingSubCalls.length + createdSubCalls.length} sub-calls, and matched ${preview.matchedCrewCount} crew${preview.unmatchedCrewCount ? `, ${preview.unmatchedCrewCount} unmatched` : ""}.`,
      show: showRow,
