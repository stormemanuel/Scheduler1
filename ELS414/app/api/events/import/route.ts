...
function makeSubCallKey(laborDate: string, area: string, roleName: string, startTime: string, endTime: string | null | undefined, poNumber?: string | null) {
  ...
  return [laborDate, normalizeImportArea(area), roleName || "", normalizeTimeKey(startTime), normalizeTimeKey(endTime), normalizePoNumber(poNumber)].join("|");
}
...
function makePoDayKey(laborDate: string, poNumber: string | null | undefined, roleName: string | null | undefined) {
  ...
  if (!po) return "";
  return [laborDate, po, normalizeMatchValue(roleName || "")].join("|");
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const existingByPo = new Map<string, SubCallRow>();
    ...
    for (const call of payload.subCallGroups) {
      ...
      if (!existingForPo) continue;
      if (existingForPo.area && normalizeImportArea(existingForPo.area) !== normalizeImportArea(call.area)) {
