...
export default function PayrollClient({ initialRows, initialEventSummaries, availableYears, initialYear, initialError, initialDiagnostics = null }: Props) {
  ...
  const hasPayrollSummary = summaryEvents.length > 0 || loadedEventCount > 0;
  const totalScopeLabel = hasPayrollSummary
    ? `${summaryEvents.length} summarized show${summaryEvents.length === 1 ? "" : "s"} · ${loadedEventCount} detail${loadedEventCount === 1 ? "" : "s"} opened`
    : "Payroll summary not loaded";
    ...
    <>
      ...
      <section className="grid grid-3">
        <div className="card compact">
          ...
          <div style={{ fontSize: 30, fontWeight: 850 }}>{hasPayrollSummary ? money(payableTotal) : "Not loaded"}</div>
          <div className="muted small">{totalScopeLabel}</div>
        </div>
        <div className="card compact">
          <div className="muted small">Marked paid</div>
          <div style={{ fontSize: 30, fontWeight: 850, color: "#067647" }}>{hasPayrollSummary ? money(paidTotal) : "Not loaded"}</div>
          <div className="muted small">{totalScopeLabel}</div>
        </div>
        <div className="card compact">
          <div className="muted small">Still unpaid</div>
          <div style={{ fontSize: 30, fontWeight: 850, color: "#b42318" }}>{hasPayrollSummary ? money(unpaidTotal) : "Not loaded"}</div>
          <div className="muted small">{totalScopeLabel}</div>
        </div>
      </section>
      ...
      <section className="card accent-card">
        ...
        <div className="pl-grid" style={{ marginTop: 14 }}>
          <div className="card compact"><div className="muted small">Estimated revenue</div><strong>{hasPayrollSummary ? money(plTotals.estimatedRevenue) : "Not loaded"}</strong></div>
          <div className="card compact"><div className="muted small">Contract labor</div><strong>{hasPayrollSummary ? money(plTotals.contractLabor) : "Not loaded"}</strong><div className="muted small">Tech payouts + non-waived coordinator pay</div></div>
          <div className="card compact"><div className="muted small">Expenses</div><strong>{hasPayrollSummary ? money(plTotals.expenses) : "Not loaded"}</strong></div>
          <div className="card compact"><div className="muted small">Net profit</div><strong>{hasPayrollSummary ? money(plTotals.estimatedProfit) : "Not loaded"}</strong></div>
        </div>
      </section>
