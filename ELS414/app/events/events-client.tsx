...
function buildGroupedImportPreview(preview: ImportPreviewState, selectedChangeIds: string[]) {
  ...
  const laborDayCount = new Set(preview.subCallPreview.map((call) => call.labor_date).filter(Boolean)).size;
  const crewRequested = preview.subCallPreview.reduce((total, call) => total + Math.max(0, Number(call.crew_needed || 0)), 0);
      ...
      <div className="grid events-shell">
        <section className="card">
          ...
          <div className="list" style={{ marginTop: 12 }}>
            ...
            {importPreview ? (() => {
              const groupedPreview = buildGroupedImportPreview(importPreview, selectedImportChangeIds);
              const comparisonChanges = importPreview.comparison?.changes || [];
              const additionIds = comparisonChanges.filter((change) => change.kind === "new").map((change) => change.id);
