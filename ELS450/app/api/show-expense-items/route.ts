...
export async function GET(request: Request) {
  ...
  const showId = safeText(url.searchParams.get("show_id"));
  ...
  if (error) {
    if (error.message.includes('relation "show_expense_items" does not exist')) {
      return NextResponse.json({ ok: true, items: [], total_expenses: 0 });
    }
    return NextResponse.json({ message: error.message }, { status: 400 });
  }
  const items = data ?? [];
  const total_expenses = Math.round(items.reduce((sum, row) => sum + Number((row as { amount?: number | string | null }).amount || 0), 0) * 100) / 100;
  return NextResponse.json({ ok: true, items, total_expenses });
}
