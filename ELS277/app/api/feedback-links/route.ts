...
const feedbackLinkSelect = "id, token, show_id, client_id, client_contact_id, form_kind, area_name, title, target_label, status";
...
function feedbackFormKindConstraintError(error: { message?: string } | null | undefined) {
  const message = String(error?.message || "").toLowerCase();
  return message.includes("feedback_survey_links_form_kind_check") || (message.includes("form_kind") && message.includes("check constraint"));
}

async function loadShowForFeedback(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, showId: string) {
  const withClientColumns = await admin
    .from("shows")
    .select("id, name, client, business_client_id, client_contact_id, coordinator_contact_id, venue")
    .eq("id", showId)
    .single();

  if (!withClientColumns.error && withClientColumns.data) return withClientColumns.data as ShowForFeedback;
  if (!missingColumnError(withClientColumns.error)) throw new Error(withClientColumns.error?.message || "Show not found.");

  // Some live Supabase projects are behind the newest app schema. Keep link generation
  // working instead of hiding the survey URLs just because newer client/contact columns
  // are not present yet. The SQL handoff can still be run later to restore full linking.
  const fallback = await admin
    .from("shows")
    .select("id, name, client, venue")
    .eq("id", showId)
    .single();
  if (fallback.error || !fallback.data) throw new Error(fallback.error?.message || "Show not found.");
  return { ...(fallback.data as ShowForFeedback), business_client_id: null, client_contact_id: null, coordinator_contact_id: null };
}

function formatLinks(links: SurveyLinkRow[]) {
  return links.map((link) => ({ ...link, url_path: `/feedback/${link.token}` }));
}
...
export async function GET(request: Request) {
  ...
  const { data, error } = await admin
    ...
    .select(feedbackLinkSelect)
    .eq("show_id", showId)
    .order("form_kind", { ascending: true })
    .order("area_name", { ascending: true });
  if (error) return NextResponse.json({ message: error.message }, { status: 400 });

  const links = (data || []) as SurveyLinkRow[];
  return NextResponse.json({
    ok: true,
    links: formatLinks(links),
    message: links.length ? `${links.length} existing survey link${links.length === 1 ? "" : "s"} loaded.` : "No survey links have been created for this event yet.",
  });
}
...
export async function POST(request: Request) {
  ...
  let show: ShowForFeedback;
  try {
    show = await loadShowForFeedback(admin, showId);
  } catch (error) {
    return NextResponse.json({ message: error instanceof Error ? error.message : "Show not found." }, { status: 404 });
  }
  ...
  for (const spec of specs) {
    try {
      ...
      const message = error instanceof Error ? error.message : "Unable to save feedback survey link.";
      if (spec.form_kind === "labor-coordinator" && feedbackFormKindConstraintError(error as { message?: string })) {
        warnings.push("Client Labor Coordinator survey skipped because Supabase has the older feedback_survey_links form_kind check constraint. Run supabase/feedback_form_kind_labor_coordinator_fix.sql to enable it.");
        continue;
      }
  ...
  const baseMessage = `${links.length} connected feedback survey link${links.length === 1 ? "" : "s"} ready.`;
  return NextResponse.json({
    ok: true,
    links: formatLinks(links),
    warnings,
    message: warnings.length ? `${baseMessage} ${warnings.join(" ")}` : baseMessage,
  });
}
