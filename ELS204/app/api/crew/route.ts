...
async function ensureUserCityPoolAccess(admin: ReturnType<typeof createSupabaseAdminClient>, userId: string, cityPoolId: string | null | undefined) {
  ...
  const { error } = await admin.from("user_access_settings").upsert({
    user_id: userId,
    allowed_pages: row?.allowed_pages ?? ["overview", "coordinator", "events", "crew"],
...
export async function GET() {
  ...
  if (!isOwnerAdmin(auth.role) && auth.role === "coordinator") {
    ...
    rawCrewRows = rawCrewRows.filter((row) => {
      const typed = row as { created_by?: string | null; city_pool_id?: string | null; id?: string | null; coordinator_hidden_at?: string | null; coordinator_hidden_by?: string | null };
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const crewPayload = {
      ...
      onboarding_status: pickIncoming(String(body.onboarding_status || "").trim() || null, existingCrew?.onboarding_status as string | null | undefined) || "not_started",
      w9_status: pickIncoming(String(body.w9_status || "").trim() || null, existingCrew?.w9_status as string | null | undefined) || "missing",
      contract_status: pickIncoming(String(body.contract_status || "").trim() || null, existingCrew?.contract_status as string | null | undefined) || "missing",
      questionnaire_status: pickIncoming(String(body.questionnaire_status || "").trim() || null, existingCrew?.questionnaire_status as string | null | undefined) || "missing",
      tax_profile_status: pickIncoming(String(body.tax_profile_status || "").trim() || null, existingCrew?.tax_profile_status as string | null | undefined) || "missing",
      profile_photo_url: pickIncoming(String(body.profile_photo_url || "").trim() || null, existingCrew?.profile_photo_url as string | null | undefined) || null,
      work_photo_urls: Array.isArray(body.work_photo_urls) ? body.work_photo_urls.map(String).filter(Boolean) : Array.isArray(existingCrew?.work_photo_urls) ? existingCrew?.work_photo_urls : [],
      w9_document_url: pickIncoming(String(body.w9_document_url || "").trim() || null, existingCrew?.w9_document_url as string | null | undefined) || null,
      contract_document_url: pickIncoming(String(body.contract_document_url || "").trim() || null, existingCrew?.contract_document_url as string | null | undefined) || null,
      tax_profile_notes: pickIncoming(String(body.tax_profile_notes || "").trim() || null, existingCrew?.tax_profile_notes as string | null | undefined) || null,
      onboarding_request_sent_at: body.onboarding_request_sent_at || existingCrew?.onboarding_request_sent_at || null,
      onboarding_completed_at: body.onboarding_completed_at || existingCrew?.onboarding_completed_at || null,
