  ...
  try {
    ...
    const { error: updateError } = await admin
      ...
      .update({
        ...
        onboarding_status: String(body.onboarding_status || "not_started").trim() || "not_started",
        w9_status: String(body.w9_status || "missing").trim() || "missing",
        contract_status: String(body.contract_status || "missing").trim() || "missing",
        questionnaire_status: String(body.questionnaire_status || "missing").trim() || "missing",
        tax_profile_status: String(body.tax_profile_status || "missing").trim() || "missing",
        profile_photo_url: String(body.profile_photo_url || "").trim() || null,
        work_photo_urls: Array.isArray(body.work_photo_urls) ? body.work_photo_urls.map(String).filter(Boolean) : [],
        w9_document_url: String(body.w9_document_url || "").trim() || null,
        contract_document_url: String(body.contract_document_url || "").trim() || null,
        tax_profile_notes: String(body.tax_profile_notes || "").trim() || null,
        onboarding_request_sent_at: String(body.onboarding_request_sent_at || "").trim() || null,
        onboarding_completed_at: String(body.onboarding_completed_at || "").trim() || null,
...
export async function DELETE(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (softDeleteOnly || !isOwnerAdmin(auth.role)) {
    const hiddenPatch = { coordinator_hidden_at: new Date().toISOString(), coordinator_hidden_by: hiddenByUserId, updated_at: new Date().toISOString() };
    ...
    if (hideError) {
      if (hideError.message.includes("coordinator_hidden_at") || hideError.message.includes("coordinator_hidden_by") || hideError.message.includes("schema cache")) {
        return NextResponse.json({ ok: false, message: "Coordinator soft-delete columns are missing. Run supabase/ELS137_required_migrations.sql, then retry." }, { status: 400 });
      }
