...
export async function getEventsPageData() {
  ...
  const [showsRes, laborDaysRes, subCallsRes, assignmentsRes, notesRes, checklistRes, automationRes, textQueueRes, ratesRes] = await Promise.all([
  ...
  const error = showsRes.error || laborDaysRes.error || subCallsRes.error || (assignmentsMissing ? null : assignmentsRes.error) || (notesMissing ? null : notesRes.error) || (checklistsMissing ? null : checklistRes.error) || (automationMissing ? null : automationRes.error) || (textQueueMissing ? null : textQueueRes.error) || ratesRes.error;
  if (error) {
    return {
      shows: [] as ShowRecord[],
      laborDays: [] as LaborDayRecord[],
