...
export async function getCrewPageData() {
  ...
  const [cityPoolsRes, crewGroupsRes, crewResInitial, positionsRes, unavailableRes] = await Promise.all([
  ...
  const error = cityPoolsRes.error || (crewGroupsMissing ? null : crewGroupsRes.error) || crewRes.error || positionsRes.error || unavailableRes.error;
  if (error) {
    return {
      cityPools: [] as CityPoolRecord[],
      crewGroups: [] as CrewGroupRecord[],
  ...
  return { cityPools, crewGroups, crewRecords, setupMissing: false, error: null as string | null };
}
