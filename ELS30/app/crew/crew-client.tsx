...
function matchingDefaultRole(roleName: string) {
  const normalized = normalizeText(roleName);
  if (!normalized) return null;
  return crewRoleRateOptions.find((option) => normalizeText(option.roleName) === normalized) ?? null;
}
      ...
      <div className="field">
        ...
        <div className="list">
          {draft.positions.map((position, index) => {
            const selectedDefault = matchingDefaultRole(position.role_name);
            const currentRate = Number(position.rate || 0);
            const roleDefaultRate = getDefaultCrewPayRate(position.role_name);
            return (
                ...
                <div className="grid grid-3" style={{ alignItems: "end" }}>
                  <label className="field">
                    ...
                    <select
                      ...
                      onChange={(event) => {
                        const option = crewRoleRateOptions.find((item) => item.roleName === event.target.value);
                        if (!option) return;
                    ...
                    >
                      ...
                      <optgroup label="Top defaults">
                        {crewRoleRateOptions.filter((option) => option.featured).map((option) => (
                  ...
                  <label className="field">
                    ...
                    <input
                      ...
                      onChange={(event) => {
                        ...
                        const nextDefaultRate = getDefaultCrewPayRate(nextRole);
