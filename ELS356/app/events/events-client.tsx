...
function EventsClientInner({
    ...
    return [...byCrew.entries()]
      .map(([crewId, item]) => {
        ...
        const scheduleLines = sortedCalls.map(({ show, day, call, assignment }) => {
          ...
          return `${formatMessageDate(day.labor_date)}${showLabel} – ${formatAssignmentMessageTimeRange(assignment, call)} – ${boothLabel(call.area)} – ${blockText} – ${rateText}`;
        });
  ...
  function coordinatorScheduleCardStatus(show: ShowRecord) {
    ...
    const sent = statuses.filter((status) => status.schedule_sent_through_3b).length;
  ...
  function renderCoordinatorScheduleEventStatus(show: ShowRecord) {
    const status = coordinatorScheduleCardStatus(show);
    if (!status.total) return null;
    if (status.dueCount && status.dueChecklistId) {
      return (
        <button
          type="button"
          ...
          onClick={(event) => {
            ...
            void sendCoordinatorWeekBeforeReminder(status.dueChecklistId, show.id);
          }}
          style={{ marginTop: 6, padding: "5px 10px", borderRadius: 999, fontSize: "0.82rem", fontWeight: 800 }}
        >
          ✕ Send reminder
        </button>
      );
    ...
    if (status.complete) {
      return (
        <div className="small" style={{ marginTop: 5, fontWeight: 800, color: "#15803d" }}>
    ...
    if (status.sent > 0) {
      return (
        <div className="small" style={{ marginTop: 5, fontWeight: 800, color: "#8a5a00" }}>
          Coordinator schedule sent {status.sent}/{status.total}
        </div>
      );
    }
    return null;
      ...
      <div key={call.id} className="card compact sub-call-card">
        <div className="row" style={{ alignItems: "flex-start" }}>
          <div>
            ...
            <div className="small muted">{call.area} • {call.role_name}</div>
            <div className="small muted">{[dayTypeLabel(effectiveCallDayType(call)), walkawayLabel(call), call.location ? `Location: ${call.location}` : "", call.po_number ? `PO: ${call.po_number}` : "", areaLeadLine(call), `Schedule rate: ${messageRateLabelForCall(undefined, call, activeRateCity, liveMasterRates).replace(/^Rate:\s*/i, "")}`].filter(Boolean).join(" • ")}</div>
  ...
  async function saveAutomationSettings() {
    ...
    setMsg({ kind: "success", text: "Text settings saved. Use Schedule checked reminders for full show when you are ready to create the reminder plan." });
  }
  ...
  async function previewCheckedReminderPlan() {
    if (!selectedShow) return;
    const data = await request("/api/text-automation", "POST", {
      action: "preview_checked_reminders",
      show_id: selectedShow.id,
    ...
    setMsg({ kind: "success", text: safeText(data.message) || "Reminder plan preview loaded." });
  }
  ...
  function buildCrewListDocument(): ExportDocument | null {
    ...
    const daySeparatedSections: ExportDocument["sections"] = visibleLaborDays.map((day): ExportDocument["sections"][number] => {
      ...
      const rows = dayCalls.reduce<ExportCell[][]>((acc, call, index) => {
        ...
        const timeLabel = formatTimeRange(call);
        const callDetails = [call.role_name, call.po_number ? `PO: ${call.po_number}` : "", areaLeadLine(call), call.one_hour_walkaway ? "1 hr walkaway" : ""].filter(Boolean).join(" • ");
          ...
          <div className="card compact" style={{ marginTop: 14, background: "#fbfcfd" }}>
            ...
            <div className="card compact accent-card" style={{ boxShadow: "none", marginTop: 12 }}>
              <div className="row" style={{ alignItems: "flex-start" }}>
                ...
                {combinedScheduleSummary ? <span className="badge">{combinedScheduleSummary.schedules_now} now · {combinedScheduleSummary.future} future · {combinedScheduleSummary.overdue} overdue</span> : null}
              </div>
              <label className="field" style={{ marginTop: 10 }}>
            ...
            <div className="card compact accent-card" style={{ boxShadow: "none", marginTop: 12 }}>
              <div className="row" style={{ alignItems: "flex-start" }}>
                <div>
                  <strong>Schedule checked reminders for full show</strong>
                  <div className="small muted">Uses the checked Reminder Timing options, every assigned crew member, and the selected timezone. Future reminders are scheduled for their actual due times instead of being marked due now.</div>
                </div>
                {reminderPlanSummary ? <span className="badge">{reminderPlanSummary.future} future · {reminderPlanSummary.overdue} overdue · {reminderPlanSummary.skipped} skipped</span> : null}
              </div>
              <div className="toolbar" style={{ marginTop: 10 }}>
                <button type="button" className="ghost" onClick={() => void previewCheckedReminderPlan()} disabled={saving || !selectedShow || !automationDraft.send_schedule}>Preview reminder plan</button>
                <button type="button" className="primary" onClick={() => void scheduleCheckedReminderPlan(false)} disabled={saving || !selectedShow || !automationDraft.send_schedule}>Schedule future only</button>
                <button type="button" className="ghost" onClick={() => void scheduleCheckedReminderPlan(true)} disabled={saving || !selectedShow || !automationDraft.send_schedule}>Schedule future and queue overdue now</button>
              </div>
              {reminderPlanPreview.length ? (
                <div style={{ overflowX: "auto", marginTop: 10 }}>
                  <table className="mini-table">
                    <thead>
                      <tr>
                    ...
                    <tbody>
                      {reminderPlanPreview.slice(0, 80).map((row, index) => (
                        <tr key={`${row.crew_id}-${row.reminder_label}-${row.labor_date}-${index}`}>
                          <td>{row.crew_name}</td>
                          <td>{row.reminder_label}</td>
                          <td>{row.assignment_label}</td>
                          <td>{row.scheduled_local} <span className="muted">({row.timezone})</span></td>
                          <td>{row.result_label}</td>
                        </tr>
                      ))}
                  ...
                  {reminderPlanPreview.length > 80 ? <div className="small muted" style={{ marginTop: 6 }}>Showing first 80 of {reminderPlanPreview.length} preview rows.</div> : null}
                </div>
              ) : null}
            </div>
            <div className="card compact" style={{ boxShadow: "none", marginTop: 12 }}>
              ...
              <div key={bucket} className="list" style={{ marginTop: 14 }}>
                ...
                {visibleBucketShows.map((show) => (
                  ...
                  >
                    ...
                    {renderCoordinatorScheduleEventStatus(show)}
                    {canManageEventAccess ? (
