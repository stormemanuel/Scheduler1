...
import InviteUserForm, { ViewAsUserButton } from "./invite-user-form";
...
function coordinatorCompensationSetupMessage(error: SupabaseLikeError | null | undefined) {
  ...
  if (code === "42P01" || /relation .*coordinator_compensation_settings.* does not exist/i.test(message)) {
    return "Coordinator compensation settings are not installed yet. Run the ELS295 coordinator compensation SQL before saving negotiated coordinator rates.";
  }
  if (code.startsWith("PGRST") || lower.includes("schema cache")) {
    return "Coordinator compensation table appears to be waiting on Supabase API schema refresh. Since the table exists in SQL, wait a minute, refresh the app, then try saving again.";
  }
