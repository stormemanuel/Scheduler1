...
import type { ShowRecord, LaborDayRecord, SubCallRecord, AssignmentRecord, AssignmentNoteRecord } from "@/lib/events-types";
...
export async function getEventsPageData() {
  ...
  const [showsRes, laborDaysRes, subCallsRes, assignmentsRes, notesRes, ratesRes] = await Promise.all([
  ...
  const error = showsRes.error || laborDaysRes.error || subCallsRes.error || (assignmentsMissing ? null : assignmentsRes.error) || (notesMissing ? null : notesRes.error) || ratesRes.error;
  if (error) {
    return {
      shows: [] as ShowRecord[],
      laborDays: [] as LaborDayRecord[],
