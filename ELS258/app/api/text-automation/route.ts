...
function messageRateForCall(call: SubCallRow, _crew: CrewRow, show: ShowRow, masterRates: MasterRateRow[], _meta: Record<string, string>) {
  ...
  const linkedFullDay = Number(linkedMasterRate?.full_day || 0);
  if (linkedFullDay > 0) return formatHourlyRate(linkedFullDay / 10);
  ...
  const matchedFullDay = Number((cityMasterRate || defaultMasterRate)?.full_day || 0);
