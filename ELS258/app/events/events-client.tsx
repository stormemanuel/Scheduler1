...
function EventsClientInner({
  ...
  function buildLeadBlockSummaryDocument(): ExportDocument | null {
    ...
    const buildRows = (rows: LeadSummaryRow[], rowKinds: Array<"data" | "day-divider" | "spacer" | "group-label">, headerLabel: string) => {
      ...
      for (const row of rows) {
        if (row.area !== currentArea) {
          ...
          built.push([currentArea, "", "", "", ""]);
          rowKinds.push("group-label");
        }
        built.push([row.area, row.crewName, row.role, row.phone || "", row.blockLabel]);
        rowKinds.push("data");
      }
      return built;
    };
    ...
    const sections: ExportDocument["sections"] = [...byDay.entries()]
      .sort(([a], [b]) => a.localeCompare(b))
      .map(([date, grouped]) => {
        const rowKinds: Array<"data" | "day-divider" | "spacer" | "group-label"> = [];
        const rows: Array<Array<string>> = [];
        if (grouped.full.length) rows.push(...buildRows(grouped.full, rowKinds, "FULL DAY CREW"));
        if (grouped.half.length) rows.push(...buildRows(grouped.half, rowKinds, "HALF DAY / LEAVING EARLY"));
        if (grouped.other.length) rows.push(...buildRows(grouped.other, rowKinds, "HOURLY / CUSTOM TIME CREW"));
        if (!rows.length) {
        ...
        return {
          ...
          accentColor: "#17313b",
          columns: ["Booth / Area", "Name", "Position", "Contact Number", "Block"],
          rows,
          rowKinds,
        };
      });
                      ...
                      <div key={section.booth} className="card compact" style={{ borderTop: `4px solid ${accentColorForArea(section.booth, areaAccentMap)}` }}>
                        ...
                        <div className="grid" style={{ gap: 12, marginTop: 12 }}>
                          {dates.map((date) => {
                            ...
                            return (
                              <div key={`${section.booth}-${date}`} className="card compact" style={{ background: "#f9fafb", borderLeft: `4px solid ${accentColorForArea(section.booth, areaAccentMap)}`, padding: 0, overflow: "hidden" }}>
                                <div style={{ background: "#0b2d55", color: "#ffffff", padding: "10px 14px", fontWeight: 800, fontSize: 16 }}>
                                  {formatFullDate(date)}
                                </div>
