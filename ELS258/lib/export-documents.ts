...
export type ExportSection = {
  ...
  columns?: string[];
  rows?: ExportCell[][];
  rowKinds?: Array<"data" | "day-divider" | "spacer" | "group-label">;
      ...
      <table>
        ...
        <tbody>${section.rows?.length ? section.rows.map((row, rowIndex) => {
          ...
          if (rowKind === "group-label") {
            return `<tr class="group-label-row">${section.columns?.map((_, index) => `<td>${index === 0 ? htmlEscape(row[0]) : ""}</td>`).join("")}</tr>`;
          }
          if (rowKind === "spacer") {
...
<style>
  ...
  .group-label-row td { background: #ffffff !important; color: #102a31; font-weight: 700; border-top: 0; border-bottom: 0; padding-top: 8px; padding-bottom: 4px; }
  .group-label-row td:not(:first-child) { color: transparent; }
...
function tableXml(columns: string[], rows: ExportCell[][], rowKinds?: ExportSection["rowKinds"]) {
  ...
  const body = rows.length
    ? rows.map((row, rowIndex) => {
        ...
        if (rowKind === "group-label") {
          return `<w:tr>${columns.map((_, index) => tableCell(index === 0 ? row[0] : "", false, widths[index], { bold: true })).join("")}</w:tr>`;
        }
        if (rowKind === "spacer") {
...
function callSheetSectionXml(section: ExportSection) {
  ...
  const useTableLayout = (section.columns?.length ?? 0) >= 7 || Boolean(section.rowKinds?.includes("day-divider"));
