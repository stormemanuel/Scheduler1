-- ELS48 connected client feedback surveys
...
where r.client_id is not null and s.rating is not null
...
-- Median Top Techs now includes both admin show ratings and submitted client feedback ratings.
...
with all_ratings as (
  ...
  union all
  select client_id, client_contact_id, crew_id, rating, submitted_at as rating_at
...
select 'connected_feedback_surveys_ready' as status;
