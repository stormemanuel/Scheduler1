...
export default async function FeedbackPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const { data: assignments } = callIds.length
    ? await admin.from("assignments").select("id, sub_call_id, crew_id").in("sub_call_id", callIds)
    : { data: [] };
  ...
  const survey = {
    ...
    form_kind: link.form_kind as "project-manager" | "area-manager",
    title: (link.title as string) || `${show.name} Feedback Survey`,
    target_label: (link.target_label as string | null) || (link.form_kind === "area-manager" ? "Booth / Area Manager" : "Project Manager"),
    ...
    questions: link.form_kind === "area-manager" ? areaManagerQuestions : projectManagerQuestions,
...
function PublicStyles() {
  return <style>{`
    ...
    .rating-row label { margin: 0; display: inline-flex; align-items: center; border: 1px solid var(--line); border-radius: 999px; padding: 7px 10px; background: #fff; cursor: pointer; }
    .rating-row label.selected { background: var(--brand); border-color: var(--brand); color: #fff; }
    .rating-row input { width: auto; margin-right: 5px; accent-color: var(--brand); }
    ...
    @media (max-width: 680px) { .feedback-hero-card, .feedback-public-card { border-radius: 20px; padding: 18px; } .survey-heading-row, .submit-row { display: grid; } }
  `}</style>;
}
