...
const RATE_MODE_META: Record<RateMode, {
  ...
  client: {
    ...
    defaultCopy: "Edit every billable role in one place. Half Day is calculated automatically as 50% of Full Day for revenue, P&L, and invoice estimates.",
    overrideCopy: "Edit this city’s client billing overrides in one place. Half Day stays at 50% of Full Day; leave Full Day blank to inherit the Default billing card.",
...
function toRateString(value: number | null | undefined) {
...
function buildRows<T extends { id: string; city_name: string; role_name: string; full_day: number; half_day: number | null; overtime_multiplier: number; doubletime_multiplier: number }>(selectedGroup: string, rates: T[], mirrorRates: T[] = []): RateRow[] {
  const defaults = rates.filter((rate) => rate.city_name === DEFAULT_GROUP);
  const defaultMap = byRoleName(defaults);
  const mirroredDefaults = mirrorRates.filter((rate) => rate.city_name === DEFAULT_GROUP);
  const mirroredForGroup = mirrorRates.filter((rate) => rate.city_name === selectedGroup);

  if (selectedGroup === DEFAULT_GROUP) {
    const roleNames = Array.from(new Set([...defaults.map((rate) => rate.role_name), ...mirroredDefaults.map((rate) => rate.role_name)])).sort((a, b) => a.localeCompare(b));
    return roleNames.map((roleName) => {
      const rate = defaultMap.get(normalize(roleName)) ?? null;
      return {
        key: rate?.id ?? `mirror-${DEFAULT_GROUP}-${roleName}`,
        id: rate?.id,
        role_name: rate?.role_name ?? roleName,
        full_day: toRateString(rate?.full_day),
        half_day: toRateString(halfDayFromFullDay(rate?.full_day)),
        overtime_multiplier: rate ? toMultiplierString(rate.overtime_multiplier, 1.5) : "1.5",
        doubletime_multiplier: rate ? toMultiplierString(rate.doubletime_multiplier, 2.0) : "2",
        hasBase: false,
        isNew: !rate,
      };
    });
  ...
  const roleNames = Array.from(new Set([
    ...defaults.map((rate) => rate.role_name),
    ...overrides.map((rate) => rate.role_name),
    ...mirroredDefaults.map((rate) => rate.role_name),
    ...mirroredForGroup.map((rate) => rate.role_name),
...
export default function MasterRatesClient({ cityPools, initialRates, initialClientRates, canManage, clientRatesMissing }: Props) {
  ...
  useEffect(() => {
    setRows(buildRows(selectedGroup, activeRates, rateMode === "client" ? crewRates : []));
    setPendingDeletes([]);
  }, [selectedGroup, activeRates, rateMode, crewRates]);

  const allGroups = useMemo(() => {
    const mirroredGroups = rateMode === "client" ? crewRates.map((rate) => rate.city_name) : [];
    const values = new Set<string>([DEFAULT_GROUP, ...cityPools.map((pool) => pool.name), ...activeRates.map((rate) => rate.city_name), ...mirroredGroups]);
    return Array.from(values).sort((a, b) => (a === DEFAULT_GROUP ? -1 : b === DEFAULT_GROUP ? 1 : a.localeCompare(b)));
  }, [cityPools, activeRates, rateMode, crewRates]);
  ...
  function updateRow(key: string, patch: Partial<RateRow>) {
    setRows((current) => current.map((row) => {
      ...
      if (Object.prototype.hasOwnProperty.call(patch, "full_day")) {
        ...
        return { ...row, ...patch, full_day: fullDay, half_day: toRateString(halfDayFromFullDay(fullDay)) };
      }
      return { ...row, ...patch };
    }));
  }
  ...
  function applyBulkAdjust() {
    ...
    setRows((current) => current.map((row) => {
      ...
      return {
        ...row,
        full_day: fullDay,
        half_day: toRateString(halfDayFromFullDay(fullDay)),
            ...
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {visibleRows.map((row) => (
                  <tr key={row.key}>
                    ...
                    <td style={{ padding: "10px 8px", borderBottom: "1px solid var(--line)" }}>
                      <input type="number" step="0.01" value={row.full_day === "0" ? "" : row.full_day} disabled={!canManage} onFocus={(e) => e.currentTarget.select()} onChange={(e) => updateRow(row.key, { full_day: e.target.value })} style={{ width: 110, padding: "10px 12px", borderRadius: 12, border: "1px solid var(--line)" }} placeholder={selectedGroup === DEFAULT_GROUP ? "450" : "use default"} />
                    </td>
                    <td style={{ padding: "10px 8px", borderBottom: "1px solid var(--line)" }}>
                ...
                {visibleRows.length === 0 ? (
                  <tr>
                    <td colSpan={selectedGroup === DEFAULT_GROUP ? 6 : 10} className="muted" style={{ padding: 16 }}>No rates match this search.</td>
                  </tr>
                ) : null}
              </tbody>
            </table>
