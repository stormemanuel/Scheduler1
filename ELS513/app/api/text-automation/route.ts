    ...
    for (const queued of rows) {
      ...
      if (messageType === "schedule") {
        if (!scheduleBySender.has(senderName)) {
          scheduleBySender.set(senderName, nextScheduleByCrew(bundle.show, bundle.days, bundle.calls, bundle.assignments, bundle.crewRows, bundle.masterRates, settings, senderName, bundle.projectManagerContact));
        }
