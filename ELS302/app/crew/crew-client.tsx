...
const ADMIN_PROFILE_CROP_OUTPUT_SIZE = 800;
...
async function prepareCrewUploadFile(kind: AdminUploadKind, file: File) {
  if (kind === "profile_photo") return compressCrewUploadImage(file, 800, 800, 0.75);
...
async function createAdminCroppedProfileFile(crop: AdminProfileCropState) {
  ...
  const blob = await uploadCanvasToBlob(canvas, "image/webp", 0.82);
  return new File([blob], `${uploadFileBaseName(crop.file.name)}-profile.webp`, {
    type: "image/webp",
          ...
          <div className="grid grid-2">
            <div className="card compact" style={{ background: "#fbfcfd" }}>
              ...
              <div className="grid" style={{ gap: 10, marginTop: 14 }}>
                <label className="field">
                  ...
                  />
                  <span className={uploadStatus.profile_photo?.kind === "error" ? "error" : uploadStatus.profile_photo?.kind === "success" ? "success" : "muted small"}>{uploadStatus.profile_photo?.text || "Choose a photo, position the face, and save the 800×800 private WebP image."}</span>
                  {record.profile_photo_url ? (
                    <button
                      className="ghost"
                      type="button"
            ...
            <div className="card compact" style={{ background: "#fff7ed" }}>
              ...
              <p className="small muted" style={{ margin: 0 }}>
                Profile photos should be compressed to WebP around 800×800. W-9 and contract PDFs should be readable and archived to secure Google Drive when connected.
              </p>
            </div>
          </div>
