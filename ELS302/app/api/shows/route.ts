...
export async function GET(request: Request) {
  ...
  if (url.searchParams.get("scope") !== "past") {
    return NextResponse.json({ message: "Unsupported shows request." }, { status: 400 });
  }
  ...
  if (data.setupMissing) return NextResponse.json({ message: "Supabase is not configured." }, { status: 500 });
  if (data.error) return NextResponse.json({ message: data.error }, { status: 400 });
