...
type Props = {
  ...
  initialPastMatchTotal?: number;
  initialPastPage?: number;
...
function mergeByShowId<T extends { show_id: string }>(current: T[], incoming: T[]) {
  const map = new Map(current.map((item) => [item.show_id, item]));
  for (const item of incoming) map.set(item.show_id, item);
  return [...map.values()];
}
...
function EventsClientInner({
  ...
  const initialPastShowIds = initialShowRows
    .filter((show) => showBucket(show) === "Past")
    .sort((a, b) => b.show_end.localeCompare(a.show_end) || b.show_start.localeCompare(a.show_start))
    .map((show) => show.id);
  ...
  const [clientFeedbackScores, setClientFeedbackScores] = useState(initialClientFeedbackScores);
  const [feedbackTechRatings, setFeedbackTechRatings] = useState(initialFeedbackTechRatings);
  ...
  const [bucketLimits, setBucketLimits] = useState<Record<EventBucket, number>>({ Upcoming: 18, Current: 18, Past: 25 });
  const [showPastShows, setShowPastShows] = useState(false);
  const [pastShowIds, setPastShowIds] = useState<string[]>(initialPastShowIds);
  const [pastShowsTotal, setPastShowsTotal] = useState(Math.max(0, initialPastTotal));
  const [pastShowsMatchTotal, setPastShowsMatchTotal] = useState(Math.max(0, initialPastMatchTotal));
  const [pastShowsPage, setPastShowsPage] = useState(Math.max(1, initialPastPage));
  const [pastSearchDraft, setPastSearchDraft] = useState("");
  const [pastSearchApplied, setPastSearchApplied] = useState("");
  ...
  function mergePastShowsPage(data: PastShowsPageResponse, replace: boolean) {
    const incomingShows = safeArray(data.shows).map(sanitizeShow).filter((show) => show.id);
    const incomingPastIds = incomingShows
      .filter((show) => showBucket(show) === "Past")
      .sort((a, b) => b.show_end.localeCompare(a.show_end) || b.show_start.localeCompare(a.show_start))
      .map((show) => show.id);

    setShows((current) => mergeById(current, incomingShows));
    setLaborDays((current) => mergeById(current, safeArray(data.laborDays).map(sanitizeLaborDay).filter((day) => day.id && day.show_id)));
    setSubCalls((current) => mergeById(current, safeArray(data.subCalls).map(sanitizeSubCall).filter((call) => call.id && call.labor_day_id)));
    setAssignments((current) => mergeById(current, safeArray(data.assignments).map(sanitizeAssignment).filter((assignment) => assignment.id && assignment.sub_call_id && assignment.crew_id)));
    setAssignmentNotes((current) => mergeById(current, safeArray(data.assignmentNotes)));
    setAssignmentChecklists((current) => mergeById(current, safeArray(data.assignmentChecklists)));
    setTextAutomationSettings((current) => mergeByShowId(current, safeArray(data.textAutomationSettings)));
    setTextMessageQueue((current) => mergeById(current, safeArray(data.textMessageQueue)));
    setTechRatings((current) => mergeById(current, safeArray(data.techRatings)));
    setClientFeedbackResponses((current) => mergeById(current, safeArray(data.clientFeedbackResponses)));
    setClientFeedbackScores((current) => mergeById(current, safeArray(data.clientFeedbackScores)));
    setFeedbackTechRatings((current) => mergeById(current, safeArray(data.feedbackTechRatings)));
    setEventUserAccess((current) => mergeById(current, safeArray(data.eventUserAccess)));
    setPastShowIds((current) => replace ? incomingPastIds : [...new Set([...current, ...incomingPastIds])]);
    setPastShowsMatchTotal(Math.max(0, Number(data.pastTotal || 0)));
    setPastShowsTotal(Math.max(0, Number(data.pastGrandTotal || data.pastTotal || 0)));
    setPastShowsPage(Math.max(1, Number(data.pastPage || 1)));
  }

  async function loadPastShowsPage(page: number, query: string, replace: boolean) {
    setLoadingPastShows(true);
    setMsg(null);
    try {
      const params = new URLSearchParams({ scope: "past", page: String(Math.max(1, page)) });
      const cleanQuery = query.trim();
      if (cleanQuery) params.set("q", cleanQuery);
      const response = await fetch(`/api/shows?${params.toString()}`, { cache: "no-store" });
      const data = await response.json().catch(() => ({})) as Partial<PastShowsPageResponse> & { message?: string };
      if (!response.ok) throw new Error(data.message || "Could not load past shows.");
      mergePastShowsPage(data as PastShowsPageResponse, replace);
      setPastSearchApplied(cleanQuery);
      setShowPastShows(true);
    } catch (error) {
  ...
  function searchPastShows() {
    void loadPastShowsPage(1, pastSearchDraft, true);
  }

  function clearPastShowsSearch() {
    setPastSearchDraft("");
    void loadPastShowsPage(1, "", true);
  }
  ...
  async function syncSelectedShowToGoogle() {
    ...
    await postGoogleCalendarAction("sync-show", { show_id: selectedShowId });
  }
  ...
  const pastShowIdSet = useMemo(() => new Set(pastShowIds), [pastShowIds]);
  const showsByBucket = useMemo(
    () => ({
      ...
      Past: filteredShows
        .filter((show) => showBucket(show) === "Past" && pastShowIdSet.has(show.id))
        .sort((a, b) => b.show_end.localeCompare(a.show_end) || b.show_start.localeCompare(a.show_start)),
    }),
    [filteredShows, pastShowIdSet]
  );
  ...
  useEffect(() => {
    ...
    void refreshQueue();
    const timer = window.setInterval(() => void refreshQueue(), 15000);
  ...
  }, [selectedShow, displayCalls, liveMasterRates, currentUserName, currentUserEmail]);
  ...
  async function saveShow() {
    ...
    const previousShow = editingShowId ? shows.find((show) => show.id === editingShowId) ?? null : null;
    ...
    const wasPast = Boolean(previousShow && showBucket(previousShow) === "Past");
    const isPast = showBucket(nextShow) === "Past";
    if (wasPast && !isPast) {
      const wasLoaded = pastShowIds.includes(nextShow.id);
      setPastShowIds((current) => current.filter((id) => id !== nextShow.id));
      setPastShowsTotal((current) => Math.max(0, current - 1));
      if (wasLoaded) setPastShowsMatchTotal((current) => Math.max(0, current - 1));
    } else if (!wasPast && isPast) {
      setPastShowsTotal((current) => current + 1);
      if (!pastSearchApplied) {
        setPastShowIds((current) => [nextShow.id, ...current.filter((id) => id !== nextShow.id)]);
        setPastShowsMatchTotal((current) => current + 1);
      }
    }
    if (editingShowId) {
      setTechRatings((current) =>
  ...
  async function deleteShow(id: string) {
    ...
    const deletedShow = shows.find((show) => show.id === id) ?? null;
    const deletedPastWasLoaded = pastShowIds.includes(id);
    ...
    setPastShowIds((current) => current.filter((showId) => showId !== id));
    if (deletedShow && showBucket(deletedShow) === "Past") {
      setPastShowsTotal((current) => Math.max(0, current - 1));
      if (deletedPastWasLoaded) setPastShowsMatchTotal((current) => Math.max(0, current - 1));
    }
      ...
      <div className="grid events-shell">
        <section className="card">
          ...
          <div className="toolbar" style={{ marginTop: 8 }}>
            ...
            <span className="small muted">Local search filters the events already loaded. Use Search all events for event details, crew, areas, and other full-page matches.</span>
          </div>
          {!initialSearch ? (
            <div className="small muted" style={{ marginTop: 6 }}>
              Active/current shows load first. Past Shows loads historical events in pages of 25 so older records stay accessible without slowing this page.
            </div>
          ) : null}
          ...
          <div className="list" style={{ marginTop: 14 }}>
            ...
            >
              <span className={`event-bucket-heading ${showBucketBadgeClass("Past")}`}>Past Shows ({pastShowsTotal})</span>
              <span>{showPastShows ? "Collapse" : "Expand"}</span>
            </button>

            {!showPastShows ? (
              <p className="small muted" style={{ marginTop: 8 }}>Past Shows is collapsed. Expand it to browse or search every historical event.</p>
            ) : (
              <>
                <div className="card compact" style={{ marginTop: 8 }}>
                  <label className="field">
                    <span>Search past shows</span>
                    <input
                      value={pastSearchDraft}
                      onChange={(event) => setPastSearchDraft(event.currentTarget.value)}
                      onKeyDown={(event) => {
                        if (event.key === "Enter") searchPastShows();
                      }}
                      placeholder="Event name, client, venue, or location"
                    />
                  </label>
                  <div className="toolbar" style={{ marginTop: 8 }}>
                    <button type="button" className="primary" disabled={loadingPastShows} onClick={searchPastShows}>
                      {loadingPastShows ? "Loading..." : "Search"}
                    </button>
                    {pastSearchDraft || pastSearchApplied ? (
                      <button type="button" className="ghost" disabled={loadingPastShows} onClick={clearPastShowsSearch}>Clear</button>
                    ) : null}
                  </div>
                  <div className="small muted" style={{ marginTop: 8 }}>
                    {pastSearchApplied
                      ? `${pastShowsMatchTotal} matching past show${pastShowsMatchTotal === 1 ? "" : "s"} for “${pastSearchApplied}” · ${pastShowsTotal} total`
                      : `Loaded ${Math.min(pastShowIds.length, pastShowsMatchTotal)} of ${pastShowsMatchTotal} past shows`}
                  </div>
                </div>

                {(showsByBucket.Past ?? []).map((show) => (
                  <button
                    key={show.id}
                    type="button"
                  ...
                  >
                    <div style={{ fontWeight: 700 }}>{show.name}</div>
                    <div className="small muted">{show.client || "No client"} • {show.show_start} to {show.show_end}</div>
                    <div className="small muted">{[show.venue || "No venue", eventLocationLabel(show)].filter(Boolean).join(" • ")}</div>
                    {canManageEventAccess ? (
                      <div className="small" style={{ marginTop: 5, fontWeight: 700, color: showProcessStatus(show.id).complete ? "#15803d" : "#b91c1c" }}>
                        {showProcessStatus(show.id).complete ? "✓" : "✕"} {showProcessStatus(show.id).label}
                      </div>
                ...
                {!(showsByBucket.Past ?? []).length ? (
                  <p className="small muted">{pastSearchApplied ? "No past shows matched this search." : "No past shows."}</p>
                ) : null}
                {pastShowIds.length < pastShowsMatchTotal ? (
                  <button
                    type="button"
                    className="ghost"
                    disabled={loadingPastShows}
                    onClick={() => void loadPastShowsPage(pastShowsPage + 1, pastSearchApplied, false)}
                  >
                    {loadingPastShows ? "Loading..." : `Load more (${pastShowsMatchTotal - pastShowIds.length} remaining)`}
                  </button>
                ) : null}
              </>
            )}
                  ...
                  <div className="card compact" style={{ marginTop: 12 }}>
                    <div className="row" style={{ alignItems: "flex-start" }}>
                      <div>
                        ...
                        <div className="small muted" style={{ marginTop: 6 }}>
                          Each sync creates or updates four light-green calendar entries: the show, a one-month crew reminder, a one-week crew reminder, and a tech-payment reminder 21 days after the show ends.
                        </div>
                      </div>
                      <div className="toolbar" style={{ justifyContent: "flex-end" }}>
                        {googleCalendarStatus?.connected ? (
                          <>
                            <button type="button" className="primary" disabled={loadingGoogleCalendar || !selectedShow} onClick={() => void syncSelectedShowToGoogle()}>{loadingGoogleCalendar ? "Syncing..." : googleCalendarStatus.show_synced ? "Update show + reminders" : "Sync show + reminders"}</button>
