...
function eventSearchOrFilter(search: string, includeEventLocation = true) {
  const token = cleanEventSearchTerm(search);
  if (!token) return null;
  const pattern = `*${token}*`;
  const fields = ["name", "client", "venue", ...(includeEventLocation ? ["event_location"] : []), "rate_city"];
  return fields.map((field) => `${field}.ilike.${pattern}`).join(",");
}

type EventsPageDataOptions = {
  search?: string;
  showLimit?: number;
  includeLatestPendingFeedback?: boolean;
  mode?: "default" | "past";
  pastPage?: number;
  pastPageSize?: number;
  pastSearch?: string;
...
const fullShowSelect = "id, name, client, business_client_id, client_contact_id, coordinator_contact_id, assigned_coordinator_user_id, venue, event_location, rate_city, show_start, show_end, notes, created_by";
const fallbackShowSelect = "id, name, client, venue, rate_city, show_start, show_end, notes";

function showOptionalColumnsMissing(message: string | undefined) {
  const text = message || "";
  return ["business_client_id", "client_contact_id", "coordinator_contact_id", "assigned_coordinator_user_id", "created_by", "event_location"]
    .some((column) => text.includes(column));
}

export async function getEventsPageData(options: EventsPageDataOptions = {}) {
  ...
  const requestedPastPage = Math.max(1, Math.floor(options.pastPage ?? 1));
  const requestedPastPageSize = Math.min(50, Math.max(1, Math.floor(options.pastPageSize ?? 25)));
  const requestedPastSearch = cleanEventSearchTerm(options.pastSearch);
  if (!supabase) {
    return {
      shows: [] as ShowRecord[],
      ...
      pastTotal: 0,
      pastGrandTotal: 0,
      pastPage: requestedPastPage,
      pastPageSize: requestedPastPageSize,
      pastSearch: requestedPastSearch,
      setupMissing: true,
  ...
  // Resolve the coordinator's complete visible-show set before pagination. This keeps
  // counts and page boundaries accurate instead of filtering inaccessible shows after
  // a 25-row database page has already been selected.
  let coordinatorVisibleShowIds: string[] | null = null;
  if (restrictToOwner) {
    ...
    if (!directRes.error) {
      for (const row of directRes.data ?? []) {
        const typed = row as { id: string; created_by?: string | null; assigned_coordinator_user_id?: string | null };
        if (typed.created_by === currentUserId || userAccessIds.has(String(typed.assigned_coordinator_user_id || ""))) visibleIds.add(String(typed.id));
      }
    } else {
      const [createdRes, assignedRes] = await Promise.all([
        supabase.from("shows").select("id, created_by"),
        supabase.from("shows").select("id, assigned_coordinator_user_id"),
      ]);
      if (!createdRes.error) {
        for (const row of createdRes.data ?? []) {
          const typed = row as { id: string; created_by?: string | null };
          if (typed.created_by === currentUserId) visibleIds.add(String(typed.id));
        }
      }
      if (!assignedRes.error) {
        for (const row of assignedRes.data ?? []) {
          const typed = row as { id: string; assigned_coordinator_user_id?: string | null };
          if (userAccessIds.has(String(typed.assigned_coordinator_user_id || ""))) visibleIds.add(String(typed.id));
        }
      }
    }
    ...
    const accessFallbackRes = accessRes.error && accessRes.error.message.includes("user_profile_id")
      ? await supabase.from("event_user_access").select("show_id, user_id")
      : accessRes;
    if (!accessFallbackRes.error) {
      for (const row of accessFallbackRes.data ?? []) {
        const typed = row as { show_id: string; user_id?: string | null; user_profile_id?: string | null };
        if (userAccessIds.has(String(typed.user_id || "")) || userAccessIds.has(String(typed.user_profile_id || ""))) visibleIds.add(String(typed.show_id));
      }
    }

    if (userAccessIds.size) {
      const partialCallRes = await supabase
        .from("sub_calls")
        .select("labor_day_id")
        .in("assigned_coordinator_user_id", [...userAccessIds]);
      if (!partialCallRes.error) {
        const partialDayIds = [...new Set((partialCallRes.data ?? []).map((row) => String((row as { labor_day_id: string }).labor_day_id)).filter(Boolean))];
        if (partialDayIds.length) {
          const partialDayRes = await supabase
            .from("labor_days")
            .select("show_id")
            .in("id", partialDayIds);
          if (!partialDayRes.error) {
            for (const row of partialDayRes.data ?? []) visibleIds.add(String((row as { show_id: string }).show_id));
          }
        }
      }
    }

    coordinatorVisibleShowIds = [...visibleIds];
  }

  type ShowQueryConfig = {
    scope: "active" | "past" | "all" | "single";
    search?: string;
    page?: number;
    pageSize?: number;
    limit?: number;
  ...
  async function queryShowRows(config: ShowQueryConfig) {
    if (coordinatorVisibleShowIds !== null && !coordinatorVisibleShowIds.length) {
      return { data: [] as unknown[], error: null, count: 0 };
    }

    const runQuery = async (selectColumns: string, includeEventLocation: boolean) => {
      let query = config.count
        ? readClient.from("shows").select(selectColumns, { count: "exact" })
        : readClient.from("shows").select(selectColumns);

      if (coordinatorVisibleShowIds !== null) query = query.in("id", coordinatorVisibleShowIds);
      if (config.scope === "active") query = query.gte("show_end", todayKey);
      if (config.scope === "past") query = query.lt("show_end", todayKey);
      if (config.scope === "single" && config.showId) query = query.eq("id", config.showId);

      const searchFilter = eventSearchOrFilter(config.search || "", includeEventLocation);
      if (searchFilter) query = query.or(searchFilter);

      if (config.scope === "past") {
        query = query.order("show_end", { ascending: false }).order("show_start", { ascending: false });
      } else {
        query = query.order("show_start", { ascending: true });
      }

      if (config.page && config.pageSize) {
        const from = (config.page - 1) * config.pageSize;
        query = query.range(from, from + config.pageSize - 1);
    ...
    const initial = await runQuery(fullShowSelect, true);
    return initial.error && showOptionalColumnsMissing(initial.error.message)
      ? runQuery(fallbackShowSelect, false)
      : initial;
  }

  let pastTotal = 0;
  let pastGrandTotal = 0;
  let combinedShowRows: unknown[] = [];
  let showsError: { message: string } | null = null;

  if (mode === "past") {
    const [pastPageRes, pastGrandTotalRes] = await Promise.all([
      queryShowRows({ scope: "past", search: requestedPastSearch, page: requestedPastPage, pageSize: requestedPastPageSize, count: true }),
      requestedPastSearch
        ? queryShowRows({ scope: "past", page: 1, pageSize: 1, count: true })
        : Promise.resolve(null),
    ]);
    combinedShowRows = pastPageRes.data ?? [];
    showsError = pastPageRes.error;
    pastTotal = pastPageRes.count ?? combinedShowRows.length;
    pastGrandTotal = requestedPastSearch ? (pastGrandTotalRes?.count ?? 0) : pastTotal;
    if (!showsError && pastGrandTotalRes?.error) showsError = pastGrandTotalRes.error;
  } else if (serverSearch) {
    const [searchRes, pastPageRes] = await Promise.all([
      queryShowRows({ scope: "all", search: serverSearch, limit: showLimit }),
      queryShowRows({ scope: "past", page: 1, pageSize: requestedPastPageSize, count: true }),
    ]);
    combinedShowRows = [...(searchRes.data ?? []), ...(pastPageRes.data ?? [])];
    showsError = searchRes.error || pastPageRes.error;
    pastTotal = pastPageRes.count ?? (pastPageRes.data ?? []).length;
    pastGrandTotal = pastTotal;
  } else {
    const [activeRes, pastPageRes] = await Promise.all([
      queryShowRows({ scope: "active", limit: showLimit }),
      queryShowRows({ scope: "past", page: 1, pageSize: requestedPastPageSize, count: true }),
    ]);
    combinedShowRows = [...(activeRes.data ?? []), ...(pastPageRes.data ?? [])];
    showsError = activeRes.error || pastPageRes.error;
    pastTotal = pastPageRes.count ?? (pastPageRes.data ?? []).length;
    pastGrandTotal = pastTotal;
  }

  if (mode !== "past" && latestPendingFeedbackShowId && !combinedShowRows.some((row) => String((row as { id?: string }).id || "") === latestPendingFeedbackShowId)) {
    const reviewShowRes = await queryShowRows({ scope: "single", showId: latestPendingFeedbackShowId, limit: 1 });
    if (!reviewShowRes.error && reviewShowRes.data?.length) combinedShowRows.push(...reviewShowRes.data);
    if (!showsError && reviewShowRes.error) showsError = reviewShowRes.error;
  }

  const dedupedShowRows = new Map<string, unknown>();
  for (const row of combinedShowRows) {
    const id = String((row as { id?: string }).id || "");
    if (id) dedupedShowRows.set(id, row);
  }
  const showsRes = { data: [...dedupedShowRows.values()], error: showsError };
  ...
  const profilesRes = mode === "past"
    ? emptyResult
    : await supabase.from("profiles").select("id, email, full_name, role, is_active");
