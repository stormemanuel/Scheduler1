...
function softAccentBackground(hexColor: string) {
  const match = /^#?([0-9a-f]{6})$/i.exec(hexColor.trim());
  if (!match) return "#ecfeff";
  const hex = match[1];
  const r = parseInt(hex.slice(0, 2), 16);
  const g = parseInt(hex.slice(2, 4), 16);
  const b = parseInt(hex.slice(4, 6), 16);
  return `rgba(${r}, ${g}, ${b}, 0.085)`;
}
...
function buildExportHtml(document: ExportDocument) {
  ...
  const sections = document.sections.map((section) => {
    const accentColor = /^#[0-9a-f]{6}$/i.test(asText(section.accentColor)) ? asText(section.accentColor) : "#062a31";
    const accentBackground = softAccentBackground(accentColor);
    ...
    const table = section.columns?.length ? `
      <table style="border-top: 4px solid ${accentColor}; border-left: 1px solid ${accentColor}; border-right: 1px solid ${accentColor};">
        <thead><tr>${section.columns.map((column) => `<th style="border-color: ${accentColor};">${htmlEscape(column)}</th>`).join("")}</tr></thead>
    ...
    return `<section class="export-section" style="border-left-color: ${accentColor}; border-top-color: ${accentColor}; border-right-color: ${accentColor}; background: ${accentBackground};">
      ${section.heading ? `<div class="section-heading-bar" style="background: ${accentColor};"><h2>${htmlEscape(section.heading)}</h2></div>` : ""}
      ${section.subheading ? `<div class="muted section-subheading" style="border-bottom-color: ${accentColor};">${htmlEscape(section.subheading)}</div>` : ""}
      ${paragraphs}
      ${table}
    </section>`;
  }).join("");
...
<style>
  ...
  .header { border-bottom: 3px solid #062a31; padding-bottom: 12px; margin-bottom: 16px; }
  ...
  .export-section { border-left: 7px solid #062a31; border-top: 2px solid #062a31; border-right: 1px solid #0f766e; border-radius: 5px; padding: 0 0 10px 12px; page-break-inside: avoid; }
  .section-heading-bar { margin-left: -12px; padding: 8px 12px; border-radius: 4px 4px 0 0; }
  .section-heading-bar h2 { color: #ffffff; margin: 0; }
  .section-subheading { border-bottom: 2px solid #062a31; padding: 7px 0; margin-bottom: 8px; font-weight: 700; }
...
function callSheetSectionXml(section: ExportSection) {
  ...
  return [
    section.heading ? paragraph(section.heading, true, 28, { keepNext: true, color: "065F46" }) : "",
