...
export async function POST(request: Request) {
  ...
  try {
    ...
    const payload = {
      ...
      scheduled_for: optionalDate(body.scheduled_for),
    };

    const { data, error } = await admin
      .from("show_payroll")
      .upsert(payload, { onConflict: "show_id,crew_id,role_name" })
      .select("id, show_id, crew_id, role_name, paid, payout_override, notes, scheduled_for")
      .single();

    if (error) {
      if (error.message.toLowerCase().includes("scheduled_for") || error.message.toLowerCase().includes("schema cache")) {
        return NextResponse.json({ message: "Run the latest ELS127 payroll scheduled-for SQL once, then try again." }, { status: 400 });
      }
