...
async function coordinatorPoolId(admin: NonNullable<ReturnType<typeof createSupabaseAdminClient>>, fullName: string, email: string) {
  const labelSource = fullName || email || "Coordinator";
  const poolName = `Coordinator - ${labelSource}`.slice(0, 120);
  const { data, error } = await admin.from("city_pools").upsert({ name: poolName }, { onConflict: "name" }).select("id").single();
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const coordinatorPrimaryPoolId = coordinatorOwned ? await coordinatorPoolId(admin, String(auth.profile?.full_name || "").trim(), String(auth.profile?.email || auth.user.email || "").trim()) : null;
    const cityPoolId = coordinatorOwned ? coordinatorPrimaryPoolId : requestedCityPoolId;
    ...
    const coordinatorRequestedPool = coordinatorOwned && requestedCityPoolId && requestedCityPoolId !== finalPrimaryCityPoolId ? [requestedCityPoolId] : [];
    await syncAdditionalCityPools(admin, crewId, Array.from(new Set([...existingAdditionalCityPoolIds, ...incomingAdditionalCityPoolIds, ...importedPrimaryAsAdditional, ...coordinatorRequestedPool])).filter((id) => id !== finalPrimaryCityPoolId));
