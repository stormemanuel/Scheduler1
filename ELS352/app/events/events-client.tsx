...
function EventsClientInner({
  ...
  function defaultSubCallDayTime(): SubCallDayTimeDraft {
    return { start_time: callForm.start_time, end_time: callForm.end_time, use_default: true };
  }
  ...
  function applyDefaultTimeToDays(dayIds: string[]) {
    setCallDayTimes((current) => {
      const next = { ...current };
      for (const dayId of dayIds) next[dayId] = defaultSubCallDayTime();
      return next;
    });
  }
  ...
  async function startAddCall(dayId: string, defaults?: { area?: string }) {
    ...
    setCallDayTimes(dayId ? { [dayId]: { start_time: emptyCall.start_time, end_time: emptyCall.end_time, use_default: true } } : {});
                  ...
                  <div className="card compact" style={{ marginTop: 12, boxShadow: "none", background: "#fff" }}>
                    <div className="row" style={{ alignItems: "flex-start" }}>
                      <div>
                        ...
                        <div className="small muted">Newly selected days start with the default times. Change only the days that differ.</div>
                      </div>
                      <button type="button" className="ghost" onClick={() => applyDefaultTimeToDays(callTargetDayIds)}>Apply Default to All Selected Days</button>
                    </div>
                    <div className="list" style={{ marginTop: 10 }}>
                      {callTargetDayIds.map((dayId, index) => {
                        ...
                        return (
                          <div key={dayId} className="grid" style={{ gridTemplateColumns: "minmax(180px, 1fr) 140px 140px 120px 150px", gap: 8, alignItems: "end" }}>
                            ...
                            <label className="field"><span>Start Time</span><input type="time" value={draft.use_default ? callForm.start_time : draft.start_time} onChange={(event) => setDayTimeDraft(dayId, { start_time: event.target.value })} /></label>
                            <label className="field"><span>End Time</span><input type="time" value={draft.use_default ? callForm.end_time : draft.end_time} onChange={(event) => setDayTimeDraft(dayId, { end_time: event.target.value })} /></label>
                ...
                <div className="card compact sub-call-multi-day">
                  ...
                  <div className="card compact" style={{ marginTop: 12, boxShadow: "none", background: "#fff" }}>
                    ...
                    <div className="list" style={{ marginTop: 10 }}>
                      {seriesItems.map((item, index) => {
                        ...
                        return (
                          <div key={item.call.id} className="grid" style={{ gridTemplateColumns: "minmax(180px, 1fr) 140px 140px 135px 150px", gap: 8, alignItems: "end", opacity: selected || item.call.id === editingCallId ? 1 : 0.68 }}>
                            ...
                            <label className="field"><span>Start Time</span><input type="time" disabled={!selected && item.call.id !== editingCallId} value={draft.use_default ? callForm.start_time : draft.start_time} onChange={(event) => setDayTimeDraft(item.day.id, { start_time: event.target.value })} /></label>
                            <label className="field"><span>End Time</span><input type="time" disabled={!selected && item.call.id !== editingCallId} value={draft.use_default ? callForm.end_time : draft.end_time} onChange={(event) => setDayTimeDraft(item.day.id, { end_time: event.target.value })} /></label>
                  ...
                  <div className="card compact" style={{ marginTop: 12, boxShadow: "none", background: "#fff" }}>
                    <div className="row" style={{ alignItems: "flex-start" }}>
                      ...
                      <button type="button" className="ghost" onClick={() => applyDefaultTimeToDays(editCallAddDayIds)} disabled={!editCallAddDayIds.length}>Apply Default to Selected Days</button>
                    </div>
