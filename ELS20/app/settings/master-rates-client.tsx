...
const RATE_MODE_META: Record<RateMode, {
  ...
  crew: {
    ...
    defaultCopy: "Edit every role in one place. These are your default crew pay rates.",
    overrideCopy: "Edit this city’s crew pay overrides in one place. Leave a row blank to fall back to the Default crew pay card.",
  ...
  client: {
    ...
    defaultCopy: "Edit every billable role in one place. These are the rates used for client revenue, P&L, and invoice estimates.",
    overrideCopy: "Edit this city’s client billing overrides in one place. Leave a row blank to fall back to the Default client billing card.",
...
function buildRows<T extends { id: string; city_name: string; role_name: string; full_day: number; half_day: number | null; overtime_multiplier: number; doubletime_multiplier: number }>(selectedGroup: string, rates: T[]): RateRow[] {
  ...
  if (selectedGroup === DEFAULT_GROUP) {
    return defaults
      ...
      .map((rate) => ({
        ...
        half_day: toRateString(rate.half_day),
  ...
  return roleNames.map((roleName) => {
    ...
    return {
      ...
      half_day: toRateString(override?.half_day),
...
export default function MasterRatesClient({ cityPools, initialRates, initialClientRates, canManage, clientRatesMissing }: Props) {
  ...
  useEffect(() => {
    setRows(buildRows(selectedGroup, activeRates));
    setPendingDeletes([]);
  }, [selectedGroup, activeRates, rateMode]);

  const allGroups = useMemo(() => {
    const values = new Set<string>([DEFAULT_GROUP, ...cityPools.map((pool) => pool.name), ...activeRates.map((rate) => rate.city_name)]);
    return Array.from(values).sort((a, b) => (a === DEFAULT_GROUP ? -1 : b === DEFAULT_GROUP ? 1 : a.localeCompare(b)));
  }, [cityPools, activeRates]);
  ...
  function submitSave() {
    ...
    startTransition(async () => {
      ...
      const insertedRates = upserts.map((row) => ({
      ...
      setActiveRates([...nextRates, ...insertedRates].sort((a, b) => (a.city_name === b.city_name ? a.role_name.localeCompare(b.role_name) : a.city_name.localeCompare(b.city_name))));
      setPendingDeletes([]);
                ...
                {visibleRows.map((row) => (
                  <tr key={row.key}>
                    ...
                    <td style={{ padding: "10px 8px", borderBottom: "1px solid var(--line)" }}>
                      <input type="number" step="0.01" value={row.full_day} disabled={!canManage} onChange={(e) => updateRow(row.key, { full_day: e.target.value })} style={{ width: 110, padding: "10px 12px", borderRadius: 12, border: "1px solid var(--line)" }} placeholder={selectedGroup === DEFAULT_GROUP ? "450" : "use default"} />
                    </td>
                    <td style={{ padding: "10px 8px", borderBottom: "1px solid var(--line)" }}>
                      <input type="number" step="0.01" value={row.half_day} disabled={!canManage} onChange={(e) => updateRow(row.key, { half_day: e.target.value })} style={{ width: 110, padding: "10px 12px", borderRadius: 12, border: "1px solid var(--line)" }} placeholder={selectedGroup === DEFAULT_GROUP ? "225" : "use default"} />
                    </td>
                    <td style={{ padding: "10px 8px", borderBottom: "1px solid var(--line)" }}>
                      <input type="number" step="0.01" value={row.overtime_multiplier} disabled={!canManage} onChange={(e) => updateRow(row.key, { overtime_multiplier: e.target.value })} style={{ width: 90, padding: "10px 12px", borderRadius: 12, border: "1px solid var(--line)" }} placeholder="1.5" />
                    </td>
                    <td style={{ padding: "10px 8px", borderBottom: "1px solid var(--line)" }}>
                      <input type="number" step="0.01" value={row.doubletime_multiplier} disabled={!canManage} onChange={(e) => updateRow(row.key, { doubletime_multiplier: e.target.value })} style={{ width: 90, padding: "10px 12px", borderRadius: 12, border: "1px solid var(--line)" }} placeholder="2" />
                    </td>
                    {canManage ? (
                      <td style={{ padding: "10px 8px", borderBottom: "1px solid var(--line)" }}>
