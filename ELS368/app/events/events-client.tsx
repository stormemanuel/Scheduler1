...
function poAndAreaLeadSummary(calls: SubCallRecord[]) {
  ...
  return [poNumbers.length ? `PO: ${poNumbers.join(", ")}` : "", ...leads].filter(Boolean).join(" • ");
}
...
function EventsClientInner({
  ...
  async function sendCombinedScheduleAndReminders(includeOverdueNow = false) {
    ...
    setMsg({ kind: "success", text: safeText(data.message) || "Schedules queued and future reminders scheduled." });
  }
  ...
  function buildCrewListDocument(): ExportDocument | null {
    ...
    const daySeparatedSections: ExportDocument["sections"] = visibleLaborDays.map((day): ExportDocument["sections"][number] => {
      ...
      const rows = dayCalls.reduce<ExportCell[][]>((acc, call, index) => {
        ...
        const timeLabel = formatTimeRange(call);
        const callDetails = [call.role_name, subCallReferenceNumber(call) ? `Reference: ${subCallReferenceNumber(call)}` : "", call.po_number ? `PO: ${call.po_number}` : "", areaLeadLine(call), call.one_hour_walkaway ? "1 hr walkaway" : ""].filter(Boolean).join(" • ");
...
${checklistText}`, "Checklist")}>Copy checklist</button>
                ...
                <div className="list">
                  ...
                  {editorMode === "call" && !editingCallId && !boothSections.length ? renderEditorPanel() : null}
