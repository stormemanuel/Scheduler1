...
function parsePdfText(text: string, overrides: Partial<ImportedEventData["show"]>): ImportedEventData {
  ...
  const finishPendingRow = () => {
    ...
    if (parsed) {
      const queued = queuedSection();
      if (queued && deferQueuedSectionUntilMatchingPo && rowMatchesSectionPo(pendingRow, queued)) {
        activateQueuedSection();
        parsed.area = currentSection;
      } else if (queued && deferQueuedSectionUntilMatchingPo && extractPoNumbers(pendingRow).length) {
