...
import { openDocumentInApp } from "@/lib/types";
...
async function queueOnboardingRequestForShortcut(record: CrewRecord, requestType: "full_onboarding" | "w9_only" | "contract_only" = "full_onboarding") {
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) throw new Error(data.message || (requestType === "w9_only" ? "Unable to queue W-9 request." : requestType === "contract_only" ? "Unable to queue contract request." : "Unable to queue onboarding request."));
...
function ContactDetail({ record, ratingSummary, cityPools, targetGroups, bulkCityId, bulkGroupName, detailTab, onTab, onEdit, onDelete, onAcknowledgeCoordinatorHidden, onRestoreCoordinatorHidden, onMove, onQueueOnboarding, onQueueW9Only, onQueueContractOnly, onAdminDocumentUploaded, onBulkCityChange, onBulkGroupChange, saving, canSeeAdminAudit, canSendOnboarding }: ContactDetailProps) {
  ...
  async function openAdminDocument(kind: AdminUploadKind, storagePath: string | null | undefined, download = false) {
    ...
    try {
      ...
      openDocumentInApp(signedUrl, documentLabel, returnPath);
      return;
    } catch (error) {
          ...
          <div className="grid grid-2">
            <div className="card compact" style={{ background: "#fbfcfd" }}>
              ...
              <div className="grid" style={{ gap: 10, marginTop: 14 }}>
                <label className="field">
                  <span>Crop or replace profile photo</span>
                  <input
                    type="file"
                    accept="image/*"
                    disabled={uploadDisabled}
