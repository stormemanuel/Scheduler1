...
function purchaseOrderLines(po: ContractorPurchaseOrder) {
  ...
  return [
    { label: "Base show pay", detail: "", amount: po.baseAmount },
    amounts.overtime > 0 ? { label: "Overtime", detail: `${po.adjustments.overtimeHours} hr × ${money(po.adjustments.overtimeRate)}`, amount: amounts.overtime } : null,
    amounts.doubleTime > 0 ? { label: "Double time", detail: `${po.adjustments.doubleTimeHours} hr × ${money(po.adjustments.doubleTimeRate)}`, amount: amounts.doubleTime } : null,
...
async function sendPurchaseOrderEmail(args: { to: string; crewName: string; showName: string; showClient: string; showVenue: string; showDates: string; link: string; po: ContractorPurchaseOrder }) {
  ...
  const rows = lines.map((line) => `<tr><td style="padding:8px;border-bottom:1px solid #e5e7eb"><strong>${escapeHtml(line.label)}</strong>${line.detail ? `<br><span style="color:#64748b">${escapeHtml(line.detail)}</span>` : ""}</td><td style="padding:8px;border-bottom:1px solid #e5e7eb;text-align:right">${escapeHtml(money(line.amount))}</td></tr>`).join("");
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      to: [args.to],
      subject: `Purchase Order for ${args.showName} — ${money(args.po.total)}`,
      text: `Hello ${args.crewName},\n\nPurchase Order revision ${args.po.revision} for ${args.showName}.\n${args.showClient}${args.showVenue ? ` · ${args.showVenue}` : ""}\n${args.showDates}\n\n${textLines}\nTotal: ${money(args.po.total)}\n\n${notice}\n\nReview: ${args.link}`,
      html: `<div style="font-family:Arial,sans-serif;max-width:680px;margin:auto;color:#172033"><h2>Emanuel Labor Services Purchase Order</h2><p>Hello ${escapeHtml(args.crewName)},</p><p><strong>${escapeHtml(args.showName)}</strong><br>${escapeHtml(args.showClient)}${args.showVenue ? ` · ${escapeHtml(args.showVenue)}` : ""}<br>${escapeHtml(args.showDates)}<br>Revision ${args.po.revision}</p><table style="width:100%;border-collapse:collapse">${rows}<tr><td style="padding:12px 8px;font-size:18px"><strong>Total</strong></td><td style="padding:12px 8px;text-align:right;font-size:18px"><strong>${escapeHtml(money(args.po.total))}</strong></td></tr></table><p style="background:#fff7d6;border-left:4px solid #d4a62a;padding:12px">${escapeHtml(notice)}</p><p><a href="${escapeHtml(args.link)}" style="display:inline-block;background:#0f5f3d;color:white;text-decoration:none;padding:12px 18px;border-radius:8px;font-weight:bold">Review Purchase Order</a></p></div>`,
    }),
  });
  ...
  try {
    ...
    if (action === "save_purchase_order" || action === "send_purchase_orders") {
      ...
      if (action === "save_purchase_order") {
        ...
        const baseAmount = optionalMoney(body.base_amount) ?? existing?.baseAmount ?? optionalMoney(currentRow?.payout_override) ?? 0;
        const total = purchaseOrderTotal(baseAmount, adjustments);
        const now = new Date().toISOString();
        const contentChanged = !existing || existing.baseAmount !== baseAmount || JSON.stringify(existing.adjustments) !== JSON.stringify(adjustments);
        ...
        const purchaseOrder: ContractorPurchaseOrder = {
          ...
          audit: [...(existing?.audit ?? []), { at: now, action: existing ? (revision > existing.revision ? "revision_created" : "draft_updated") : "draft_created", detail: `Revision ${revision} total ${money(total)}` }].slice(-60),
        };
        ...
        return NextResponse.json({ ok: true, purchase_order: purchaseOrder, row: { ...publicRow(data, payload), notes: stripPurchaseOrderFromNotes(data.notes) }, message: `Purchase Order revision ${revision} saved and ready to send.` });
      }
      ...
      for (const crewId of crewIds) {
        ...
        const now = new Date().toISOString();
        let next: ContractorPurchaseOrder = { ...existing };
        try {
          const emailId = await sendPurchaseOrderEmail({
            ...
            link: `${origin}/onboarding/${encodeURIComponent(existing.token)}`,
            po: existing,
          });
          next = { ...existing, status: "sent_awaiting_delivery", updatedAt: now, sentAt: now, deliveredAt: null, approvalDueAt: null, reminderSentAt: null, approvedAt: null, resendEmailId: emailId, lastDeliveryEvent: "sent", audit: [...existing.audit, { at: now, action: "email_sent", detail: `Resend email ${emailId}; delivery confirmation pending` }].slice(-60) };
          ...
          next = { ...existing, status: "delivery_failed", updatedAt: now, sentAt: null, deliveredAt: null, approvalDueAt: null, resendEmailId: null, lastDeliveryEvent: "failed", audit: [...existing.audit, { at: now, action: "delivery_failed", detail: errorMessage }].slice(-60) };
        ...
        await upsertWithSchemaFallback(admin, {
          ...
          paid: paymentStatus === "paid", payment_status: paymentStatus, payout_override: existing.total,
