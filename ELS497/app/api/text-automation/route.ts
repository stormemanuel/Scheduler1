...
import { extractPurchaseOrderFromNotes, mergePurchaseOrderIntoNotes, type ContractorPurchaseOrder } from "@/lib/payroll-calculations";
...
async function sendPurchaseOrderReminderEmail(args: { to: string; name: string; showName: string; total: number; link: string; dueAt: string }) {
  const key = resendApiKey();
  const domain = resendEmailDomain();
  ...
  const response = await fetch("https://api.resend.com/emails", {
    ...
    body: JSON.stringify({
      ...
      subject: `Reminder: review Purchase Order for ${args.showName}`,
      text: `Hello ${args.name},\n\nYour Purchase Order for ${args.showName} is still awaiting your approval or change request. Total: ${new Intl.NumberFormat("en-US", { style: "currency", currency: "USD" }).format(args.total)}. Please respond by ${due}. If no response is received within 48 hours of delivery, it will be recorded as approved and payment will be processed for the stated amount.\n\nReview: ${args.link}`,
    }),
  });
  ...
  for (const { row, po: original } of active) {
    ...
    if (po.status === "delivered" && po.approvalDueAt) {
      ...
      if (now.getTime() >= dueMs) {
        ...
        try {
          await sendPurchaseOrderReminderEmail({ to: safeText(crew?.email), name: safeText(crew?.name) || "Contractor", showName: safeText(showById.get(row.show_id)?.name) || "your event", total: po.total, link: `${origin}/onboarding/${encodeURIComponent(po.token)}`, dueAt: po.approvalDueAt });
