...
function purchaseOrderDraftFor(row: PayrollCrewShowRow): PurchaseOrderDraft {
  ...
  return {
    baseAmount: currencyText(po?.baseAmount ?? row.overrideAmount ?? row.estimatedTotal),
...
function purchaseOrderDraftTotal(draft: PurchaseOrderDraft) {
  ...
  return Math.round((poDraftNumber(draft.baseAmount) + a.overtimeHours * a.overtimeRate + a.doubleTimeHours * a.doubleTimeRate + a.mealPenaltyCount * a.mealPenaltyRate + a.shortTurnHours * a.shortTurnRate + a.otherAmount) * 100) / 100;
}
  ...
  async function savePurchaseOrder(row: PayrollCrewShowRow, quiet = false) {
    ...
    try {
      const response = await fetch("/api/payroll", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ action: "save_purchase_order", show_id: row.showId, crew_id: row.crewId, base_amount: poDraftNumber(draft.baseAmount), adjustments: purchaseOrderAdjustmentsFromDraft(draft) }),
      });
                          ...
                          {isOpen ? (
                            <div className="card compact" style={{ marginTop: 10, minWidth: 360 }}>
                              <div style={{ borderBottom: "1px solid var(--line)", paddingBottom: 14, marginBottom: 14 }}>
                                ...
                                <div className="toolbar" style={{ marginTop: 10 }}><button className="primary" type="button" disabled={savingPurchaseOrderKey === row.key} onClick={() => void savePurchaseOrder(row)}>{savingPurchaseOrderKey === row.key ? "Saving…" : row.purchaseOrder ? "Save Purchase Order revision" : "Save Purchase Order"}</button><span className="small muted">Only non-zero adjustment categories appear to the contractor.</span></div>
                              </div>
                              <label className="field">
                                Payroll notes
                                <textarea
