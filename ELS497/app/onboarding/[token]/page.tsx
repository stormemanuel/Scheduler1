...
type PublicPurchaseOrder = {
  ...
  adjustmentAmounts: { overtime: number; doubleTime: number; mealPenalty: number; shortTurn: number; other: number };
  total: number;
  sentAt: string | null;
...
export default function OnboardingPage({ params }: PageProps) {
  ...
  if (requestType === "purchase_order") {
    ...
    const lines = [
      { label: "Base show pay", detail: "", amount: purchaseOrder.baseAmount },
      purchaseOrder.adjustmentAmounts.overtime > 0 ? { label: "Overtime", detail: `${purchaseOrder.adjustments.overtimeHours} hr × ${poMoney(purchaseOrder.adjustments.overtimeRate)}`, amount: purchaseOrder.adjustmentAmounts.overtime } : null,
      purchaseOrder.adjustmentAmounts.doubleTime > 0 ? { label: "Double time", detail: `${purchaseOrder.adjustments.doubleTimeHours} hr × ${poMoney(purchaseOrder.adjustments.doubleTimeRate)}`, amount: purchaseOrder.adjustmentAmounts.doubleTime } : null,
    ...
    return (
      <main style={{ maxWidth: 820, margin: "32px auto", padding: "0 16px" }}>
        ...
        <section className="card" style={{ marginTop: 16 }}>
          ...
          <div style={{ border: "1px solid var(--line)", borderRadius: 12, overflow: "hidden" }}>{lines.map((line) => <div key={line.label} className="row" style={{ padding: "12px 14px", borderBottom: "1px solid var(--line)" }}><div><strong>{line.label}</strong>{line.detail ? <div className="muted small">{line.detail}</div> : null}</div><strong>{poMoney(line.amount)}</strong></div>)}<div className="row" style={{ padding: "15px 14px", background: "#f5faf7", fontSize: 20 }}><strong>Total</strong><strong>{poMoney(purchaseOrder.total)}</strong></div></div>
