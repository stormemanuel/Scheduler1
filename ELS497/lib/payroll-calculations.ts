...
export function purchaseOrderTotal(baseAmount: unknown, value: Partial<PurchaseOrderAdjustments> | null | undefined) {
  const amounts = purchaseOrderAdjustmentAmounts(value);
  return Math.round((poNumber(baseAmount) + amounts.overtime + amounts.doubleTime + amounts.mealPenalty + amounts.shortTurn + amounts.other) * 100) / 100;
}
...
export function extractPurchaseOrderFromNotes(notes: unknown): ContractorPurchaseOrder | null {
  ...
  try {
    ...
    return {
      ...
      baseAmount: poNumber(parsed.baseAmount),
      adjustments,
      total: purchaseOrderTotal(parsed.baseAmount, adjustments),
